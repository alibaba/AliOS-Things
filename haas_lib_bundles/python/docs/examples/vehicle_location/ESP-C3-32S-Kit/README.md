...
* ESP-C3-32S-Kit开发板  1套
* 杜邦线若干
* GNSS定位模块 HT2828Z3G5L

&emsp;&emsp;
其中，HT2828Z3G5L与ESP-C3-32S-Kit的连接如下：

| 定位模块 |                                                   |    ESP-C3-32S-Kit     |          |
| -------- | ------------------------------------------------- | :----------: | -------- |
| 引脚名   | 引脚功能                                          |  连接到引脚  | 引脚序号 |
| VBAT/PPS | 备用电池 / 时间标准脉冲输出                       | NC（不连接） | ——       |
| VCC      | 模块供电，可选3.3V至5V                            |     3.3V     | 3V3        |
| TX       | 串口发送端                                        |     U1TXD     | IO9   |
| RX       | 串口接收端                                        |     U1RXD     | IO3   |
| GND      | 接地                                              |     GND      | GND       |
...
如下图实物图所示：
<div align="center">
<img src=./../../../images/5_3_车辆定位硬件连接_ESP-C3-32S-Kit.png width=90%/>
</div>
...
&emsp;&emsp;
对于 ESP-C3-32S-Kit 等搭载 Wi-Fi 的设备而言，联网方式可以选择“Wi-Fi”。
...
<div align="center">
<img src=./../../../images/5_3_创建工程_ESP-C3-32S-Kit.png width=100%/>
</div>

&emsp;&emsp;
如下图所示，打开VS Code之后在新建一个基于helloworld的python工程，设定好工程名称（“vehicle_location”）及工作区路径之后，硬件类型选择ESP-C3-32S-Kit，解决方案选择“helloworld”，点击立即创建，创建一个Python轻应用的解决方案。
...
### 本地查看
&emsp;&emsp;
推送此脚本到ESP-C3-32S-Kit之后，串口会输出获取到的定位信息，同时按照脚本中的逻辑，当定位信息发生变化时，会将更新的定位信息上报至物联网平台。如下面这段串口日志所示。
```log
...
...
sleep for 1 s
establish tcp connection with server(host='xxxxxxxxxx.iot-as-mqtt.cn-shanghai.aliyuncs.com', port=[443])
