...
int sal_add_dev(char* driver_name, void* data)
{
    ...
    if((driver_name == NULL) && (current != NULL))
    {
        ...
        if (current->add_dev(data) == 0) {
            ...
            SAL_ERROR("Use default sal module(%s), but init failed!\n", current->name);
            return -1;
        }
    }
    ...
    while(current != NULL)
    {
        if((strcmp(current->name, driver_name) == 0))
        {
            if (current->add_dev(data) == 0) {
                ...
                SAL_ERROR("Use module(%s), but init failed!\n", current->name);
                return -1;
            }
        }
        current = current->next;
