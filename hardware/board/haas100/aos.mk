...
$(NAME)_COMPONENTS += $(HOST_MCU_FAMILY) kernel_init yloop debug
...
export A7_SRAM ?= 2
ifeq ($(A7_SRAM),0)
export DEBUG_LASTWORD_RAM_ADDR ?= 0x201e6000
export DSP_RAM_SHARE_SIZE ?= 0x15000
GLOBAL_CFLAGS += -DDSP_RAM_SHARE_SIZE=$(DSP_RAM_SHARE_SIZE)
else ifeq ($(A7_SRAM),1)
export DEBUG_LASTWORD_RAM_ADDR ?= 0x20166000
GLOBAL_CFLAGS += -DDSP_SRAM_SOLO
else
export DEBUG_LASTWORD_RAM_ADDR ?= 0x20166000
endif
GLOBAL_CFLAGS += -DDEBUG_LASTWORD_RAM_ADDR=$(DEBUG_LASTWORD_RAM_ADDR)
GLOBAL_ASMFLAGS += -DDEBUG_LASTWORD_RAM_ADDR=$(DEBUG_LASTWORD_RAM_ADDR)
