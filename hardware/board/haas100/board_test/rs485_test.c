...
static int32_t uart_rs485_test_process()
{
    ...
    ret = haas_rs485_init(&uart_rs485);
    if (ret) {
    ...
    for(i=0; i < 2; i++)
    {
        ...
        ret = haas_rs485_recv(recv_data, sizeof(recv_data), &recv_size, 8000);
        if (ret) {
        ...
        ret = haas_rs485_send(recv_data, recv_size, 3000);
        if (ret) {
