...
K_CONFIG_MMU_TABLE_REGION uint32_t g_pgtbl_lvl1[MMU_SECTION_ENTRY_CNT]
...
int32_t os_mmu_sec_attr_convert(mmu_mem_attr_t mem_attr)
{
                        ...
                        mmu_mem_attr_t  mem_attr)
{
    ...
    entry_flags = os_mmu_sec_attr_convert(mem_attr);
                     ...
                     mmu_mem_attr_t  mem_attr)
{
    ...
    for (i = 0; i < entry_num;) {
        ...
        if (paddr % MMU_LARGEPAGE_SIZE == 0
            ...
            entry_flags = os_mmu_lpg_attr_convert(mem_attr);
            entry = paddr | entry_flags;
            ...
            /* add a small page */
            entry_flags = os_mmu_spg_attr_convert(mem_attr);
            entry = paddr | entry_flags;
            *p_entry = entry;
            k_dcache_clean((uintptr_t)p_entry, sizeof(*p_entry));
            p_entry++;
...
int32_t os_mmu_traversal(int32_t  is_unmap,
                         os_mmu_func_t handler)
{
    ...
    for (lvl1_idx = 0; lvl1_idx < MMU_SECTION_ENTRY_CNT; lvl1_idx++) {
        ...
        if (lvl1_idx % MMU_SUPERSECTION_REPEAT == 0
            ...
            if (handler != NULL) {
                handler(lvl1_vaddr, paddr, MMU_SUPERSECTION_SIZE, 1);
            }
            ...
            if (handler != NULL) {
                handler(lvl1_vaddr, paddr, MMU_SECTION_SIZE, 1);
            }
        }
#if (K_CONFIG_MMU_LVL2 == 1)
            ...
            for (lvl2_idx = 0; lvl2_idx < MMU_PAGE_ENTRY_CNT; lvl2_idx++) {
                ...
                if (lvl2_idx % MMU_LARGEPAGE_REPEAT == 0
                    ...
                    if (handler != NULL) {
                        handler(lvl1_vaddr + lvl2_vaddr, paddr, MMU_LARGEPAGE_SIZE, 1);
                    }
                    ...
                    if (handler != NULL) {
                        handler(lvl1_vaddr + lvl2_vaddr, paddr, MMU_SMALLPAGE_SIZE, 1);
                    }
                } else {
                    /* Invalid */
                }
...
{
    /* set TTBR0 as Translation Table Base */
    os_set_TTBCR(0);

    memset(g_pgtbl_lvl1, 0, sizeof(g_pgtbl_lvl1));
#if (K_CONFIG_MMU_LVL2 == 1)
    memset(&s_pgtbl_pool, 0, sizeof(s_pgtbl_pool));
    memset(g_pgtbl_lvl2, 0, sizeof(g_pgtbl_lvl2));
#endif
}
...
void k_mmu_enable(void)
{
    ...
    os_mmu_ttb_set((uintptr_t)g_pgtbl_lvl1);
               ...
               mmu_mem_attr_t  mem_attr)
{
    ...
    ret = k_mmu_mmap(g_pgtbl_lvl1, vaddr, paddr, len, mem_attr);
    return ret;
}
...
void k_mmu_show(void)
{
    ...
    printf("[mmu show] SCTLR = 0x%lx, PA of TableBase = %p, VA of TableBase = %p\n",
           os_get_SCTLR(), (void *)os_mmu_ttb_get(), g_pgtbl_lvl1);
    ret = os_mmu_traversal(0, os_mmu_print);
    if (ret != 0) {
        printf("[mmu show] return %ld", ret);
    }
}
