...
struct wpa_scan_res {
    ...
    uint8_t auth_mode;    //Pengfei add, for connect without security mode
    uint8_t encrypt_type; //Pengfei add, for connect without security mode
#endif
	/*
...
struct wpa_driver_associate_params {
	 ...
	 */
	const uint8_t *htcaps;       /* struct ieee80211_ht_capabilities * */
	const uint8_t *htcaps_mask;  /* struct ieee80211_ht_capabilities * */
};
...
struct wpa_driver_ap_params {
	 ...
	 */
	const uint8_t *proberesp;

	/**
	 * proberesp_len - Length of the proberesp buffer in octets
	 */
	uint32_t proberesp_len;
...
struct wpa_driver_ops {
...
#if 0
	int (*set_ptk_kck_kek)(void *priv,
               const uint8_t *key, uint32_t key_len);
#endif
    int (*gen4m_driver_cmd)(void *priv,
               char *cmd, const uint8_t *buf, uint8_t buf_len);
	 ...
	 * Returns: 0 on success, -1 on failure
	 */
	int (*deauthenticate)(void *priv, const uint8_t *addr, int reason_code);
	 ...
	 * Returns: 0 on success, -1 on failure
	 */
	int (*disassociate)(void *priv, const uint8_t *addr, int reason_code);
	 ...
	 */
	int (*add_pmkid)(void *priv, const uint8_t *bssid, const uint8_t *pmkid);
	 ...
	 */
	int (*remove_pmkid)(void *priv, const uint8_t *bssid, const uint8_t *pmkid);
	 ...
	 */
	int (*send_eapol)(void *priv, const uint8_t *dest, uint16_t proto,
	 ...
	 * Returns: 0 on success, -1 on failure
	 */
	int (*send_mlme)(void *priv, const uint8_t *data, uint32_t data_len,
			 int noack);
	 ...
	 * transition.
	 */
	int (*send_ft_action)(void *priv, uint8_t action, const uint8_t *target_ap,
			      const uint8_t *ies, uint32_t ies_len);
	 ...
	 * DEPRECATED - use set_ap() instead
	 */
	int (*set_generic_elem)(void *priv, const uint8_t *elem, uint32_t elem_len);
	...
	int (*send_action)(void *priv, unsigned int freq, unsigned int wait,
			   ...
			   const uint8_t *data, uint32_t data_len, int no_cck);
	 ...
	 */
	int (*send_frame)(void *priv, const uint8_t *data, uint32_t data_len,
			  int encrypt);
	 ...
	 */
	int (*get_noa)(void *priv, uint8_t *buf, uint32_t buf_len);
	 ...
	 * struct wpa_driver_capa.
	 */
	int (*p2p_connect)(void *priv, const uint8_t *peer_addr, int wps_method,
			   int go_intent, const uint8_t *own_interface_addr,
	 ...
	 * struct wpa_driver_capa.
	 */
	int (*wps_success_cb)(void *priv, const uint8_t *peer_addr);
	 ...
	 * struct wpa_driver_capa.
	 */
	int (*p2p_prov_disc_req)(void *priv, const uint8_t *peer_addr,
				 uint16_t config_methods, int join);
	 ...
	 * struct wpa_driver_capa.
	 */
	uint64_t (*p2p_sd_request)(void *priv, const uint8_t *dst,
	 ...
	 * struct wpa_driver_capa.
	 */
	int (*p2p_sd_cancel_request)(void *priv, uint64_t req);
	 ...
	 * struct wpa_driver_capa.
	 */
	int (*p2p_sd_response)(void *priv, int freq, const uint8_t *dst,
	 ...
	 * Returns: 0 on success, -1 on failure
	 */
	int (*p2p_reject)(void *priv, const uint8_t *addr);
	 ...
	 * Returns: 0 on success, -1 on failure
	 */
	int (*p2p_invite)(void *priv, const uint8_t *peer, int role,
			  const uint8_t *bssid, const uint8_t *ssid, uint32_t ssid_len,
			  const uint8_t *go_dev_addr, int persistent_group);
	 ...
	 * Returns: 0 on success, -1 on failure
	 */
	 int (*driver_cmd)(void *priv, char *cmd, char *buf, uint32_t buf_len);
#endif
	/**
	 ...
	 */
	void (*set_rekey_info)(void *priv, const uint8_t *kek, const uint8_t *kck,
	 ...
	 */
	 int (*sta_assoc)(void *priv, const uint8_t *own_addr, const uint8_t *addr,
			  int reassoc, uint16_t status, const uint8_t *ie, uint32_t len);
	 ...
	 * to the station.
	 */
	 int (*sta_auth)(void *priv, const uint8_t *own_addr, const uint8_t *addr,
			 uint16_t seq, uint16_t status, const uint8_t *ie, uint32_t len);
    ...
    void (*stat_change)(void *priv, uint8_t stat);
};
