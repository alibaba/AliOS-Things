...
{
    unsigned short tmp = vol | (vol << 8);

    printf("ac97_set_vol vol:%d \r\n", vol);
    if (vol < 0 || vol > 0x3f)
    {
        return -1;
    }
...
int ac97_get_vol(void)
{
    ...
    printf("ac97_get_vol vol:%d\r\n", 0x3F & v);
    return 0x3F & v;
}

int ac97_set_rate(int rate)
{
    printf("ac97_set_rate rate:%d \r\n", rate);
    if (rate < 0)
    {
        return -1;
    }
