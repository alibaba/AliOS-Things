...
//	aos_debug_printf("MacAddr = 0x%x\t0x%x\t0x%x\t0x%x\t0x%x\t0x%x\n",MacAddr[0],MacAddr[1],MacAddr[2],MacAddr[3],MacAddr[4],MacAddr[5]);
...
//	aos_debug_printf("gmacdev->DmaBase = 0x%x\n", gmacdev->DmaBase);
//	aos_debug_printf("dmaBase = 0x%x\n", dmaBase);
	{
		int i,j;
		u16 data;
		...
		if(j==32) {
			aos_debug_printf("phy_detect: can't find PHY!\n");
		}
		gmacdev->PhyBase = i;
	}
