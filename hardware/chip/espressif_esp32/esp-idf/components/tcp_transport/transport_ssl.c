...
static int ssl_poll_read(esp_transport_handle_t t, int timeout_ms)
{
    ...
    if (ret > 0 && FD_ISSET(ssl->tls->sockfd, &errset)) {
        ...
        ESP_LOGE(TAG, "ssl_poll_read select error %d, errno = %s, fd = %d", sock_errno, strerror(sock_errno),
                 ssl->tls->sockfd);
        ret = -1;
    }
    return ret;
...
static int ssl_poll_write(esp_transport_handle_t t, int timeout_ms)
{
    ...
    if (ret > 0 && FD_ISSET(ssl->tls->sockfd, &errset)) {
        ...
        uint32_t optlen = sizeof(sock_errno);
        getsockopt(ssl->tls->sockfd, SOL_SOCKET, SO_ERROR, &sock_errno, &optlen);
        ESP_LOGE(TAG, "ssl_poll_write select error %d, errno = %s, fd = %d", sock_errno, strerror(sock_errno),
                 ssl->tls->sockfd);
        ret = -1;
    }
    return ret;
...
static int ssl_write(esp_transport_handle_t t, const char *buffer, int len, int timeout_ms)
{
    ...
    if ((poll = esp_transport_poll_write(t, timeout_ms)) <= 0) {
        ESP_LOGW(TAG, "Poll timeout or error, errno=%s, fd=%d, timeout_ms=%d", strerror(errno), ssl->tls->sockfd,
                 timeout_ms);
        return poll;
    }
