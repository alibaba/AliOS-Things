...
SSL_ROM_TEXT_SECTION
void aes128_ecb_security_boot(unsigned char *buf_in, unsigned char *buf_out, int mode, unsigned char *key)
{
	aes_context ctx;
	...
	*/
	if (mode == AES_ENCRYPT) {
		...
		aes_crypt_ecb( &ctx, 1, buf_in, buf_out );
	} else {
		aes_setkey_dec( &ctx, key, 128);
		aes_crypt_ecb( &ctx, 0, buf_in, buf_out );
	}
