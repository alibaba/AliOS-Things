...
#define uart0_rx_buf_count() ((RX_BUFFER_LENGTH + uart0_rx_buf_head - uart0_rx_buf_tail) & (RX_BUFFER_LENGTH - 1))
#define uart0_rx_buf_space() (RX_BUFFER_LENGTH - 1 - uart0_rx_buf_count())
    ...
    if (uart->port != 0) {
        ...
        if ((uart_rxbuffer[uart->port].length) <= expect_size) {
            ...
            *recv_size = uart_rxbuffer[uart->port].length;
            ...
            _memcpy(uart_rxbuffer[uart->port].rxbuffer, uart_rxbuffer[uart->port].rxbuffer + expect_size,
        ...
        while (1) {
            ...
            r = aos_event_get(&uart0_event, EVENT_RX_READY, AOS_EVENT_OR, &val, timeout);
            if (r)
                break;
        }
