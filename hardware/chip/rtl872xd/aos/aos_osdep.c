...
void aos_interrupt_enter()
{
	krhino_intrpt_enter();
}

void aos_interrupt_leave()
{
	krhino_intrpt_exit();
}
void cli()
...
static u32 _aos_down_sema(_sema *sema, u32 timeout)
{
	...
	if(aos_sem_wait(sema, timeout) != 0) {
                printf("_aos_down_sema 0x%x, timeout %d\n", sema, timeout);
		return FALSE;
	}
...
{
	uxSavedInterruptStatus = cpu_intrpt_save( );
}

static void _aos_exit_critical_from_isr(_lock *plock, _irqL *pirqL)
{
	cpu_intrpt_restore(uxSavedInterruptStatus);
}
#endif
							  ...
							  TIMER_FUN pxCallbackFunction )
{
    ...
    ret = aos_timer_new_ext(&t_handler->timer, pxCallbackFunction,(void *)t_handler, krhino_ticks_to_ms(xTimerPeriodInTicks), 0, 0);
...
int _aos_timer_change_no_repeat(aos_timer_t *timer, int ms)
{
    ...
    ret = aos_timer_change_once(timer, ms);

	ret = aos_timer_start(timer);
...
{
	int ret;
	ret = aos_timer_start(&xTimer->timer);
	return  !ret; //!aos_timer_start(&xTimer->timer);	
}
