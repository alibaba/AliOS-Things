...
def get_config():
    """ get configuration from .config_burn file, if it is not existed,
        generate default configuration of chip_rtl8721d """
    configs = {}
    ...
    if os.path.isfile(config_file):
        ...
        if not configs:
            configs = {}
    if 'chip_rtl8721d' not in configs:
        configs['chip_rtl8721d'] = {}
    if 'serialport' not in configs['chip_rtl8721d']:
        configs['chip_rtl8721d']['serialport'] = ""
    if 'baudrate' not in configs['chip_rtl8721d']:
        configs['chip_rtl8721d']['baudrate'] = "115200"
    if 'binfile' not in configs['chip_rtl8721d']:
        configs['chip_rtl8721d']['binfile'] = []

    return configs['chip_rtl8721d']

def save_config(config):
    """ save configuration to .config_burn file, only update chip_rtl8721d portion """
    if config:
        configs = {}
        config_file = os.path.join(os.getcwd(), '.config_burn')
        if os.path.isfile(config_file):
