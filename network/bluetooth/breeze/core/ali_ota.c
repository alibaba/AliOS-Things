...
static uint32_t g_page_erase_total = 0;
static uint32_t g_page_erase_already = 0;

static void
  *m_ptr; /**< Pointer to ali_ota, used only in
             bootloader_settings_event_handler() and flash_event_handler(). */


/**@brief Try to split software version string into 3 parts:
 *        v0.v2.v2.
 */
static uint32_t split_sw_ver(char *data, uint32_t *v0, uint32_t *v1,
                             uint32_t *v2)
{
    int i = sscanf(data, "%d.%d.%d", (int *)v0, (int *)v1, (int *)v2);
    return ((i == 3) ? BREEZE_SUCCESS : BREEZE_ERROR_INVALID_DATA);
}
...
static uint32_t parse_firmware_version(uint8_t *data, uint8_t len)
{
    ...
    if (memcmp(l_data, data, len) == 0) {
        ...
        return BREEZE_ERROR_INVALID_DATA;
    }
}
 ...
 */
static uint32_t check_upgrade_fw_version(ali_ota_t *p_ota, uint8_t *p_data,
                                         uint8_t length)
{
...
static uint32_t send_bytes_received(ali_ota_t *p_ota)
{
    ...
    err_code =
      p_ota->tx_func(p_ota->p_tx_func_context, ALI_CMD_FW_BYTES_RECEIVED,
...
static uint32_t send_fwup_success(ali_ota_t *p_ota)
{
    ...
    p_ota->tx_buff[0] = 1;
    err_code          = p_ota->tx_func(p_ota->p_tx_func_context,
                              ALI_CMD_FW_UPDATE_PROCESS, p_ota->tx_buff, 1);
    if (err_code != BREEZE_SUCCESS) {
    ...
    switch (p_ota->state) {
        case ALI_OTA_STATE_UPGRADE_REPORT:
            if (ALI_OTA_FLASH_STORE_OK == t) {
                p_ota->state = (p_ota->feature_enable) ? ALI_OTA_STATE_IDLE
                                                       : ALI_OTA_STATE_OFF;
                (void)send_fwup_success(p_ota);
            } else {
                notify_error(p_ota, ALI_ERROR_SRC_OTA_SETTINGS_WRITE,
                             BREEZE_ERROR_FLASH_STORE_FAIL);
                return;
            }
            break;
        ...
        case ALI_OTA_STATE_WRITE_SETTINGS:
            if (ALI_OTA_FLASH_STORE_OK == t) {
                p_ota->state = (p_ota->bytes_recvd >= p_ota->rx_fw_size)
                                 ? ALI_OTA_STATE_FW_CHECK
                                 : ALI_OTA_STATE_RECEIVE;
                (void)send_bytes_received(p_ota);
            } else {
                notify_error(p_ota, ALI_ERROR_SRC_OTA_FW_RX_WR_SETTINGS_CB,
                             BREEZE_ERROR_FLASH_STORE_FAIL);
                return;
            }
            break;
    ...
    switch (p_ota->state) {
        case ALI_OTA_STATE_IDLE:
            if (ALI_OTA_FLASH_ERASE_OK == t) {
                ...
                notify_error(p_ota, ALI_ERROR_SRC_OTA_FLASH_ERASE,
                             BREEZE_ERROR_FLASH_ERASE_FAIL);
                return;
            }
            break;
            ...
            if (ALI_OTA_FLASH_STORE_OK == t) {
                ...
                if (update_bldr_settings) {
                    ...
                    if (err_code != ALI_OTA_SETTINGS_CODE_SUCCESS) {
                        notify_error(p_ota,
                                     ALI_ERROR_SRC_OTA_FW_RX_WRITE_SETTINGS,
                                     BREEZE_ERROR_SETTINGS_FAIL);
                        return;
                    }
                ...
                notify_error(p_ota, ALI_ERROR_SRC_OTA_FLASH_WRITE,
                             BREEZE_ERROR_FLASH_STORE_FAIL);
                return;
            }
            break;
 ...
 */
static void on_fw_upgrade_req(ali_ota_t *p_ota, uint8_t *p_data,
                              uint16_t length)
{
    ...
    if (err_code == BREEZE_SUCCESS || resume) {
        ...
        if (rx_fw_size != 0) {
            ...
            num_pages = align_to_page(p_ota->rx_fw_size - p_ota->bytes_recvd,
                                      ais_ota_get_page_size()) /
                        ais_ota_get_page_size();
            if (num_pages == 0) {
                /* Nothing to erase. */
...
static ali_ota_bin_info_t ali_ota_bin_info[] = {
    { ALI_OTA_BIN_TYPE_APP, ALI_OTA_BIN_TYPE_MAGIC_APP },
    { ALI_OTA_BIN_TYPE_KERNEL, ALI_OTA_BIN_TYPE_MAGIC_KERNEL },
    { ALI_OTA_BIN_TYPE_SINGLE, ALI_OTA_BIN_TYPE_MAGIC_SINGLE },
};

/**@brief Function for handling command @ref ALI_CMD_FW_DATA in state
 *        @ref ALI_OTA_STATE_RECEIVE.
 */
static void on_fw_data(ali_ota_t *p_ota, uint8_t *p_data, uint16_t length,
                       uint8_t num_frames)
{
    uint32_t        err_code;
    ...
    if (p_ota->bytes_recvd <= ALI_OTA_BIN_TYPE_INFO_OFFSET &&
        p_ota->bytes_recvd + length > ALI_OTA_BIN_TYPE_INFO_OFFSET) {
        uint32_t i,
          bin_magic = EXTRACT_U32(p_data + ALI_OTA_BIN_TYPE_INFO_OFFSET -
                                  p_ota->bytes_recvd);
        ali_ota_bin_type_t bin_type;

        LOGD("ble", "OTA bin file magic detected: 0x%08x.", bin_magic);
        for (i = 0; i < sizeof(ali_ota_bin_info) / sizeof(ali_ota_bin_info[0]);
             i++) {
            if (ali_ota_bin_info[i].magic == bin_magic) {
                break;
            }
        }

        if (i >= sizeof(ali_ota_bin_info) / sizeof(ali_ota_bin_info[0])) {
            notify_error(p_ota, ALI_ERROR_SRC_OTA_FW_DATA,
                         BREEZE_ERROR_INVALID_BIN_TYPE);
            return;
        }

        bin_type = ali_ota_bin_info[i].type;
        ...
        if (bin_type == ALI_OTA_BIN_TYPE_SINGLE ||
            ...
            notify_error(p_ota, ALI_ERROR_SRC_OTA_FW_DATA,
                         BREEZE_ERROR_BINS_UPGRADE_NOT_SUPPORTED);
            return;
        }

        LOGD("ble", "setting OTA bin type 0x%02x ...", bin_type);
        ais_ota_set_upgrade_bin_type_info(bin_type);
#else
        if (bin_type != ALI_OTA_BIN_TYPE_SINGLE ||
            ais_ota_check_if_bins_supported() == true) {
            notify_error(p_ota, ALI_ERROR_SRC_OTA_FW_DATA,
                         BREEZE_ERROR_BIN_UPGRADE_NOT_SUPPORTED);
            return;
        }
#endif
    }
    ...
    percent =
      p_ota->bytes_recvd * 100 / p_ota->rx_fw_size; /* Ensure no overflow */
...
static void on_xfer_finished(ali_ota_t *p_ota, uint8_t *p_data, uint16_t length)
{
    ...
    /* Check CRC here. */
    crc = crc16_compute((uint8_t const *)p_ota->bank_1_addr, p_ota->rx_fw_size,
                        NULL);
    LOGD("ble", "The calculated crc: 0x%x, the read crc: 0x%x", crc,
         p_ota->crc);
    if (crc == p_ota->crc) {
        int32_t img_crc;
        ...
        err_code =
          ais_ota_settings_write(/*bootloader_settings_event_handler*/ NULL);
        if (err_code != ALI_OTA_SETTINGS_CODE_SUCCESS) {
            notify_error(p_ota, ALI_ERROR_SRC_OTA_XFER_FINISHED,
                         BREEZE_ERROR_SETTINGS_FAIL);
            return;
        }
...
ret_code_t ali_ota_init(ali_ota_t *p_ota, ali_ota_init_t const *p_init)
{
    ...
    if (p_init->fw_version_len == 0 ||
        p_init->fw_version_len > ALI_OTA_MAX_FW_VERSION_LEN) {
        return BREEZE_ERROR_INVALID_LENGTH;
    }

    /* Check version string. */
    err_code =
      parse_firmware_version(p_init->p_fw_version, p_init->fw_version_len);
    VERIFY_SUCCESS(err_code);
...
void ali_ota_on_command(ali_ota_t *p_ota, uint8_t cmd, uint8_t *p_data,
                        uint16_t length, uint8_t num_frames)
{
    bool send_err = false;
    uint32_t err_code;
    ...
    switch (p_ota->state) {
        ...
        case ALI_OTA_STATE_RECEIVE:
            if (cmd == ALI_CMD_FW_DATA) { // cmd=0x2F
                ...
                if(g_page_erase_already <= g_page_erase_total){
                    err_code = ais_ota_flash_erase(
                    (uint32_t const *)(p_ota->bank_1_addr + (g_page_erase_already *ais_ota_get_page_size()) ), 1,  NULL);
                    if(err_code != ALI_OTA_FLASH_CODE_SUCCESS){
                       send_err = true;
                    } else{
                       g_page_erase_already++;
...
void ali_ota_on_auth(ali_ota_t *p_ota, bool is_authenticated)
{
    ...
    if (p_ota->state == ALI_OTA_STATE_OFF) {
        ...
        if (ais_ota_check_if_update_finished()) {
            LOG(
              "Image swapping performed before, let's notify fw upgrade done.");
            ais_ota_update_settings_after_update_finished();
            err_code = ais_ota_settings_write(NULL);
            if (err_code != ALI_OTA_SETTINGS_CODE_SUCCESS) {
                notify_error(p_ota, ALI_ERROR_SRC_OTA_NOTIFY_NEW_FW,
                             BREEZE_ERROR_SETTINGS_FAIL);
                return;
            }
        ...
        notify_error(p_ota, ALI_ERROR_SRC_OTA_AUTH_TIMING,
                     BREEZE_ERROR_INVALID_STATE);
    }
}
