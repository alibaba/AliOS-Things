...
static u8_t att_write_rsp(struct bt_conn *conn, u8_t op, u8_t rsp, u16_t handle,
...
static u8_t att_exec_write_rsp(struct bt_att *att, u8_t flags)
{
    ...
    while ((buf = net_buf_get(&att->prep_queue, K_NO_WAIT))) {
        ...
        if (!err && flags == BT_ATT_FLAG_EXEC) {
            err = att_write_rsp(conn, BT_ATT_OP_EXEC_WRITE_REQ, 0, data->handle,
                                data->offset, buf->data, buf->len);
            if (err) {
                /* Respond here since handle is set */
...
static const struct att_handler
{
    ...
    { BT_ATT_OP_ERROR_RSP, sizeof(struct bt_att_error_rsp), ATT_RESPONSE,
      att_error_rsp },
    { BT_ATT_OP_MTU_REQ, sizeof(struct bt_att_exchange_mtu_req), ATT_REQUEST,
      att_mtu_req },
    { BT_ATT_OP_MTU_RSP, sizeof(struct bt_att_exchange_mtu_rsp), ATT_RESPONSE,
      att_mtu_rsp },
    ...
    if (IS_ENABLED(CONFIG_BT_ATT_ENFORCE_FLOW)) {
        if (handler->type == ATT_REQUEST &&
            ...
            return;
                   ...
                   atomic_test_and_set_bit(att->flags, ATT_PENDING_CFM)) {
...
struct net_buf *bt_att_create_pdu(struct bt_conn *conn, u8_t op, size_t len)
{
    ...
    if (len + sizeof(op) > att->chan.tx.mtu) {
        BT_WARN("ATT MTU exceeded, max %u, wanted %zu", att->chan.tx.mtu,
                len + sizeof(op));
        return NULL;
    }
...
static void bt_att_encrypt_change(struct bt_l2cap_chan *chan, u8_t hci_status)
{
    ...
    BT_DBG("chan %p conn %p handle %u sec_level 0x%02x status 0x%02x", ch, conn,
