...
#define BT_SMP_AUTH_DEFAULT \
    (BT_SMP_AUTH_BONDING | BT_SMP_AUTH_SC | BT_SMP_AUTH_CT2)
#else
...
static u8_t get_io_capa(void)
{
    ...
    if (sc_supported && bt_auth->passkey_display && bt_auth->passkey_confirm) {
...
static struct net_buf *smp_create_pdu(struct bt_conn *conn, u8_t op, size_t len)
{
...
static void sc_derive_link_key(struct bt_smp *smp)
{
    ...
    if (atomic_test_bit(smp->flags, SMP_FLAG_CT2)) {
        /* constants as specified in Core Spec Vol.3 Part H 2.4.2.4 */
        static const u8_t salt[16] = { 0x31, 0x70, 0x6d, 0x74, 0x00, 0x00,
...
static void smp_br_derive_ltk(struct bt_smp_br *smp)
{
    ...
    if (atomic_test_bit(smp->flags, SMP_FLAG_CT2)) {
        /* constants as specified in Core Spec Vol.3 Part H 2.4.2.5 */
        static const u8_t salt[16] = { 0x32, 0x70, 0x6d, 0x74, 0x00, 0x00,
                                       0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                       0x00, 0x00, 0x00, 0x00 };
...
static void smp_br_distribute_keys(struct bt_smp_br *smp)
{
    ...
    if (smp->local_dist & BT_SMP_DIST_ID_KEY) {
        ...
        buf = smp_create_pdu(conn, BT_SMP_CMD_IDENT_INFO, sizeof(*id_info));
        if (!buf) {
    ...
    if (smp->local_dist & BT_SMP_DIST_SIGN) {
        ...
        buf = smp_create_pdu(conn, BT_SMP_CMD_SIGNING_INFO, sizeof(*info));
        if (!buf) {
...
static void bt_smp_br_recv(struct bt_l2cap_chan *chan, struct net_buf *buf)
{
    ...
    if (atomic_test_bit(smp->flags, SMP_FLAG_TIMEOUT)) {
        BT_WARN("SMP command (code 0x%02x) received after timeout", hdr->code);
        return;
    }
...
static void legacy_distribute_keys(struct bt_smp *smp)
{
    ...
    if (smp->local_dist & BT_SMP_DIST_ENC_KEY) {
        ...
        buf = smp_create_pdu(conn, BT_SMP_CMD_ENCRYPT_INFO, sizeof(*info));
        if (!buf) {
        ...
        buf = smp_create_pdu(conn, BT_SMP_CMD_MASTER_IDENT, sizeof(*ident));
        if (!buf) {
        ...
        if (atomic_test_bit(smp->flags, SMP_FLAG_BOND)) {
            ...
            memcpy(keys->slave_ltk.val, key, sizeof(keys->slave_ltk.val));
...
static u8_t smp_encrypt_info(struct bt_smp *smp, struct net_buf *buf)
{
    ...
    if (atomic_test_bit(smp->flags, SMP_FLAG_BOND)) {
        ...
        if (!keys) {
            BT_ERR("Unable to get keys for %s", bt_addr_le_str(&conn->le.dst));
            return BT_SMP_ERR_UNSPECIFIED;
        }
...
static bool sec_level_reachable(struct bt_conn *conn)
{
    switch (conn->required_sec_level) {
        ...
        case BT_SECURITY_FIPS:
            return get_io_capa() != BT_SMP_IO_NO_INPUT_OUTPUT && sc_supported;
        default:
            return false;
    }
}
...
int bt_smp_send_security_req(struct bt_conn *conn)
{
    ...
    req_buf = smp_create_pdu(conn, BT_SMP_CMD_SECURITY_REQUEST, sizeof(*req));
    if (!req_buf) {
        return -ENOBUFS;
    }
...
static u8_t smp_pairing_req(struct bt_smp *smp, struct net_buf *buf)
{
    ...
    if (smp->method == JUST_WORKS) {
        ...
        /* ask for consent if pairing is not due to sending SecReq*/
        if (!atomic_test_bit(smp->flags, SMP_FLAG_SEC_REQ) && bt_auth &&
            bt_auth->pairing_confirm) {
            atomic_set_bit(smp->flags, SMP_FLAG_USER);
            bt_auth->pairing_confirm(smp->chan.chan.conn);
            return 0;
...
static u8_t sc_smp_send_dhkey_check(struct bt_smp *smp, const u8_t *e)
{
    ...
    buf = smp_create_pdu(smp->chan.chan.conn, BT_SMP_DHKEY_CHECK, sizeof(*req));
    if (!buf) {
        return BT_SMP_ERR_UNSPECIFIED;
    }
    ...
    if (smp->chan.chan.conn->role == BT_HCI_ROLE_MASTER) {
        ...
        switch (smp->method) {
            case PASSKEY_CONFIRM:
                ...
                if (smp_g2(sc_public_key, smp->pkey, smp->prnd, smp->rrnd,
                           &passkey)) {
                    return BT_SMP_ERR_UNSPECIFIED;
                }
...
static u8_t smp_ident_addr_info(struct bt_smp *smp, struct net_buf *buf)
{
    ...
    if (!bt_addr_le_is_identity(&req->addr)) {
        BT_ERR("Invalid identity %s for %s", bt_addr_le_str(&req->addr),
...
static u8_t smp_dhkey_check(struct bt_smp *smp, struct net_buf *buf)
{
    ...
    if (IS_ENABLED(CONFIG_BT_CENTRAL) &&
        ...
        if (bt_conn_le_start_encryption(smp->chan.chan.conn, 0, 0, smp->tk,
                                        enc_size) < 0) {
            return BT_SMP_ERR_UNSPECIFIED;
        }
...
static void bt_smp_recv(struct bt_l2cap_chan *chan, struct net_buf *buf)
{
    ...
    if ((BT_SMP_CODE_VALID_CHK(smp->allowed_cmds)) &&
...
static void bt_smp_disconnected(struct bt_l2cap_chan *chan)
{
    ...
    if (keys) {
         ...
         */
        if (!keys->keys || atomic_test_bit(keys->flags, BT_KEYS_DEBUG)) {
            bt_keys_clear(keys);
        }
    }
...
static void bt_smp_encrypt_change(struct bt_l2cap_chan *chan, u8_t hci_status)
{
    ...
    BT_DBG("chan %p conn %p handle %u encrypt 0x%02x hci status 0x%02x", chan,
           conn, conn->handle, conn->encrypt, hci_status);
...
int bt_smp_sign_verify(struct bt_conn *conn, struct net_buf *buf)
{
    ...
    err =
      smp_sign_buf(keys->remote_csrk.val, buf->data, buf->len - sizeof(sig));
    if (err) {
