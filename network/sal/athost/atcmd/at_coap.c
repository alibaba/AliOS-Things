...
static int coap_sendreq(coap_sendreq_t *sendreq, bool confirmreq)
{
    ...
    message.p_payload     = sendreq->msg;
    message.payload_len   = strlen(sendreq->msg);
    if (confirmreq) {
    ...
    LOG("send msg: Len: %d, Payload: %s, path %s \r\n", message.payload_len,
        message.p_payload, sendreq->path);
    ...
    coap_set_devinfo(&devinfo);
    snprintf(path, IOTX_URI_MAX_LEN, "/topic/%s/%s/update/",
             (char *)devinfo.product_key, (char *)devinfo.device_name);

    ret = IOT_CoAP_SendMessage(p_context, sendreq->path, &message);
    if (ret >= 0) {
        if (confirmreq) {
            memcpy(&m_last_send_req, sendreq, sizeof(coap_sendreq_t));
        } else {
            memcpy(&m_last_send_req_noack, sendreq, sizeof(coap_sendreq_t));
        }
...
static void coap_context_init()
{
    ...
    p_context = IOT_CoAP_Init(&config);
}
    ...
    if ('?' == single) {
        ...
        if (offset + ONE_DIGIT_STR_LEN + strlen(m_last_send_req.path) +
            ...
            if (m_last_send_req.valid)
                offset += snprintf(
                  response + offset, MAX_ATCMD_COAP_MSG_RSP_LEN - offset,
                  "%d%s\"%s\"%s\"%s\"%s%d%s%d", m_last_send_req.method,
                  SEPARATOR_STR, m_last_send_req.path, SEPARATOR_STR,
                  m_last_send_req.msg, SEPARATOR_STR,
                  m_last_send_req.msg_format, SEPARATOR_STR,
                  m_last_send_req.fragment_id);
            else
                offset += snprintf(response + offset,
                                   MAX_ATCMD_COAP_MSG_RSP_LEN - offset, ",,,,");
        } else {
        ...
        if ('?' == single) {
            ...
            if (offset + strlen(TAG_METHOD_STR) + strlen(TAG_PATH_STR) +
                ...
                offset += snprintf(
                  response + offset, MAX_ATCMD_COAP_MSG_RSP_LEN - offset,
                  "%s%s%s%s%s%s%s%s%s", TAG_METHOD_STR, SEPARATOR_STR,
                  TAG_PATH_STR, SEPARATOR_STR, TAG_MESSAGE_STR, SEPARATOR_STR,
    ...
    if ('?' == single) {
        ...
        if (offset + ONE_DIGIT_STR_LEN + strlen(m_last_send_req_noack.path) +
            ...
            if (m_last_send_req_noack.valid)
                offset += snprintf(
                  response + offset, MAX_ATCMD_COAP_MSG_RSP_LEN - offset,
                  "%d%s\"%s\"%s\"%s\"%s%d%s%d", m_last_send_req_noack.method,
                  SEPARATOR_STR, m_last_send_req_noack.path, SEPARATOR_STR,
                  m_last_send_req_noack.msg, SEPARATOR_STR,
                  m_last_send_req_noack.msg_format, SEPARATOR_STR,
                  m_last_send_req_noack.fragment_id);
            else
                offset += snprintf(response + offset,
                                   MAX_ATCMD_COAP_MSG_RSP_LEN - offset, ",,,,");
        } else {
