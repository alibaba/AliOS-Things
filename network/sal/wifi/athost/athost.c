...
static void athost_uart_echo_off()
{
    char out[64] = {0};
...
int sal_wifi_start(sal_conn_t *c)
{
    ...
    if (strstr(out, AT_RSP_SUCCESS) != NULL &&
        TCP_SERVER == c->type) {
        if (aos_sem_is_valid(&g_link[link_id].sem_start)) {
            LOGD(TAG, "sem is going to be waked up: 0x%x", &g_link[link_id].sem_start);
            aos_sem_signal(&g_link[link_id].sem_start); // wakeup send task
...
static int athost_wifi_packet_input_cb_register(netconn_data_input_cb_t cb)
{
    if (cb) {
...
#ifdef SAL_SERVER
static int athost_client_status_notify(netconn_client_status_notify_t cb)
{
    if (cb) {
...
sal_op_t sal_op = {
    ...
    .register_netconn_data_input_cb = athost_wifi_packet_input_cb_register,
#ifdef SAL_SERVER
    .register_netconn_client_status_notify = athost_client_status_notify,
#endif
};
