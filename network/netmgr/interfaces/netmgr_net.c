...
static void netmgr_connect_fail_event(hal_net_module_t *m, int err, void *arg)
{}

static void netmgr_fatal_err_event(hal_net_module_t *m, void *arg)
{}

static void netmgr_disconnected_event(hal_net_module_t *m, void *arg)
{
    g_netmgr_cxt.disconnected_times++;
    aos_post_event(EV_NETWORK, CODE_ON_DISCONNECT, NULL);
}

static const hal_net_event_cb_t g_net_hal_event = {
...
{
    hal_net_module_t *module;

    module = hal_net_get_default_module();
    memset(&g_netmgr_cxt, 0, sizeof(g_netmgr_cxt));
    g_netmgr_cxt.ip_available = false;
    g_netmgr_cxt.hal_mod = module;
    hal_net_install_event(g_netmgr_cxt.hal_mod, &g_net_hal_event);
    return 0;
}
