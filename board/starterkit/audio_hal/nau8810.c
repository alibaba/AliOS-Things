...
static int drv_codec_nau8810_validate_id(void)
{
	...
	ret = nau8810_reg_read(NAU8810_REG_I2C_DEVICEID, &i2c_deviceid);
	if (ret != 0) {
		KIDS_A10_PRT("nau8810_reg_read return %d\n", ret);
		return ret;
	}
	if (i2c_deviceid != NAU8810_I2C_DEVICEID_VAL) {
		KIDS_A10_PRT("read NAU8810_REG_I2C_DEVICEID: %hu\n", i2c_deviceid);
		return -1;
	}
	
	ret = nau8810_reg_read(NAU8810_REG_ADDITIONID, &additionid);
	if (ret != 0) {
		KIDS_A10_PRT("nau8810_reg_read return %d\n", ret);
		return ret;
	}
	if (additionid != NAU8810_ADDITIONID_VAL) {
		KIDS_A10_PRT("read NAU8810_REG_ADDITIONID: %hu\n", additionid);
		return -1;
	}
