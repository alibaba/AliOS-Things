                                ...
                                uint16_t quantity, uint8_t *respond_buf, uint8_t *respond_count)
{
    ...
    status = mbmaster_main_type1_process(req_handler, FUNC_CODE_READ_COILS, slave_addr, start_addr, quantity, respond_buf, respond_count);
    return status;
}
...
{
    mb_status_t status = MB_SUCCESS;
    status = mbmaster_main_type1_process(req_handler, FUNC_CODE_READ_DISCRETE_INPUTS, slave_addr, start_addr, quantity, respond_buf, respond_count);
    return status;
}
                                       ...
                                       uint8_t *exception_code)
{
    ...
    status = mbmaster_main_type2_process(req_handler, FUNC_CODE_WRITE_SINGLE_COIL, slave_addr, coil_addr,
                                         coil_value, resp_addr, resp_value, exception_code);
    return status;
}
