 ...
 *  (used in recursion)
 */
static bt_u32_t search_uuid(struct bt_sdp_data_elem *elem, struct bt_uuid *uuid,
			 bool *found, u8_t nest_level)
{
	const u8_t *cur_elem;
	bt_u32_t seq_size, size;
	union {
		struct bt_uuid uuid;
 ...
 *  (used in recursion)
 */
static bt_u32_t copy_attribute(struct bt_sdp_data_elem *elem,
			    struct net_buf *buf, u8_t nest_level)
{
	const u8_t *cur_elem;
	bt_u32_t size, seq_size, total_size;
	...
	if ((elem->type & BT_SDP_TYPE_DESC_MASK) == BT_SDP_SEQ_UNSPEC ||
		...
		if (seq_size == 1U) {
			...
			net_buf_add_be32(buf, *((bt_u32_t *)elem->data));
		} else {
...
static u16_t create_attr_list(struct bt_sdp *sdp, struct bt_sdp_record *record,
			      bt_u32_t *filter, u8_t num_filters,
 ...
 *  @return 0 for success, or relevant error code
 */
static u16_t get_att_search_list(struct net_buf *buf, bt_u32_t *filter,
				 u8_t *num_filters)
{
	struct bt_sdp_data_elem data_elem;
	u16_t res;
...
{
	bt_u32_t *svc_rec_hdl = user_data;
...
static u16_t sdp_svc_att_req(struct bt_sdp *sdp, struct net_buf *buf,
			     u16_t tid)
{
	bt_u32_t filter[MAX_NUM_ATT_ID_FILTER];
	struct search_state state = {
...
{
	bt_u32_t handle = SDP_SERVICE_HANDLE_BASE;
	...
	*((bt_u32_t *)(service->attrs[0].val.data)) = handle;
	db = service;
