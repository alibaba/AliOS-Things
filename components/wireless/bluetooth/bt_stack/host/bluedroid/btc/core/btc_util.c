...
void uuid128_be_to_yoc_uuid(yoc_bt_uuid_t *u, uint8_t* uuid128)
{
    if (memcmp(base_uuid_be+4, uuid128 + 4, 12) != 0) {
        u->len = YOC_UUID_LEN_128;
        uint8_t *p_i = uuid128 + YOC_UUID_LEN_128 - 1;
        uint8_t *p_o = u->uuid.uuid128;
        uint8_t *p_end = p_o + YOC_UUID_LEN_128;
...
void uuid_to_string_legacy(bt_uuid_t *p_uuid, char *str)
{
    ...
    sprintf((char *)str, "%.8x-%.4x-%.4x-%.4x-%.8x%.4x",
            (unsigned int)ntohl(uuid0), ntohs(uuid1),
            ntohs(uuid2), ntohs(uuid3),
            (unsigned int)ntohl(uuid4), ntohs(uuid5));
    return;
}

yoc_bt_status_t btc_hci_to_yoc_status(uint8_t hci_status)
{
    yoc_bt_status_t yoc_status = YOC_BT_STATUS_FAIL;
    switch(hci_status) {
        case HCI_SUCCESS:
            yoc_status = YOC_BT_STATUS_SUCCESS;
            break;
        case HCI_ERR_HOST_TIMEOUT:
            yoc_status = YOC_BT_STATUS_TIMEOUT;
            break;
        case HCI_ERR_ILLEGAL_COMMAND:
            yoc_status = YOC_BT_STATUS_PENDING;
            break;
        case HCI_ERR_UNACCEPT_CONN_INTERVAL:
            yoc_status = YOC_BT_STATUS_UNACCEPT_CONN_INTERVAL;
            break;
        case HCI_ERR_PARAM_OUT_OF_RANGE:
            yoc_status = YOC_BT_STATUS_PARAM_OUT_OF_RANGE;
            break;
        case HCI_ERR_ILLEGAL_PARAMETER_FMT:
            yoc_status = YOC_BT_STATUS_ERR_ILLEGAL_PARAMETER_FMT;
            break;
        default:
            yoc_status = YOC_BT_STATUS_FAIL;
            break;
    }
...
yoc_bt_status_t btc_btm_status_to_yoc_status (uint8_t btm_status)
{
    yoc_bt_status_t yoc_status = YOC_BT_STATUS_FAIL;
    switch(btm_status){
        case BTM_SUCCESS:
            ...
            break;
        case BTM_BUSY:
            yoc_status = YOC_BT_STATUS_BUSY;
            break;
        case BTM_NO_RESOURCES:
            yoc_status = YOC_BT_STATUS_NOMEM;
            break;
        case BTM_ERR_PROCESSING:
            yoc_status = YOC_BT_STATUS_PENDING;
            break;
        case BTM_PEER_LE_DATA_LEN_UNSUPPORTED:
            yoc_status = YOC_BT_STATUS_PEER_LE_DATA_LEN_UNSUPPORTED;
            break;
        case BTM_CONTROL_LE_DATA_LEN_UNSUPPORTED:
            yoc_status = YOC_BT_STATUS_CONTROL_LE_DATA_LEN_UNSUPPORTED;
            break;
...
yoc_bt_status_t btc_bta_status_to_yoc_status (uint8_t bta_status)
{
    yoc_bt_status_t yoc_status = YOC_BT_STATUS_FAIL;
    switch(bta_status){
        case BTA_SUCCESS:
        ...
        case BTA_NO_RESOURCES:
            ...
            break;
        case BTA_WRONG_MODE:
            yoc_status = YOC_BT_STATUS_NOT_READY;
            break;
        case BTA_EIR_TOO_LARGE:
            yoc_status = YOC_BT_STATUS_EIR_TOO_LARGE;
            break;
        default:
            yoc_status = YOC_BT_STATUS_FAIL;
            break;
