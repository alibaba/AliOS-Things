        ...
        BD_NAME name)
{
    ...
    BTC_TRACE_DEBUG("add to storage: Remote device name:%s\n", name);
    ...
    ret &= btc_config_set_bin(bdstr, BTC_STORAGE_DEV_NAME_STR, name, sizeof(BD_NAME));
...
bt_status_t btc_storage_remove_bonded_device(bt_bdaddr_t *remote_bd_addr)
{
    ...
    if (btc_config_exist(bdstr, BTC_STORAGE_DEV_NAME_STR)) {
        ret &= btc_config_remove(bdstr, BTC_STORAGE_DEV_NAME_STR);
    }
    /* write bonded info immediately */
    btc_config_flush();
    btc_config_unlock();
...
**
*******************************************************************************/
bt_status_t btc_storage_get_bonded_bt_devices_name(bt_bdaddr_t *bond_dev, uint8_t *name, uint8_t length)
{
    bt_bdaddr_t bd_addr;
    BD_NAME bd_name;
    size_t name_length;

    BTC_TRACE_DEBUG("btc_storage_get_bonded_bt_devices_name\n");
    btc_config_lock();
    for (const btc_config_section_iter_t *iter = btc_config_section_begin(); iter != btc_config_section_end();
            iter = btc_config_section_next(iter)) {

        const char *sec_name = btc_config_section_name(iter);
        string_to_bdaddr(sec_name, &bd_addr);
        if (memcmp(bond_dev, &bd_addr, sizeof(bt_bdaddr_t)) == 0) {
            if (btc_config_exist(sec_name, BTC_STORAGE_DEV_NAME_STR)) {
                btc_config_get_bin(sec_name, BTC_STORAGE_DEV_NAME_STR, bd_name, &name_length);
                if (length > sizeof(BD_NAME)) {
                    strncpy(name, bd_name, sizeof(BD_NAME));
                } else {
                    strncpy(name, bd_name, length);
                }

                BTC_TRACE_DEBUG("btc_storage_get_bonded_bt_devices_name get %s\n", name);
                break;
            }
        }
    }
