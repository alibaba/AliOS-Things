...
{
    if (!connect_queue) {
        connect_queue = osi_list_new(osi_free_func);
        assert(connect_queue != NULL);
    }

    // Sanity check to make sure we're not leaking connection requests
    assert(osi_list_length(connect_queue) < MAX_REASONABLE_REQUESTS);

    for (const list_node_t *node = osi_list_begin(connect_queue); node != osi_list_end(connect_queue); node = osi_list_next(node)) {
        if (((connect_node_t *)osi_list_node(node))->uuid == p_param->uuid) {
    ...
    osi_list_append(connect_queue, p_node);
}

static void queue_int_advance(void)
{
    if (connect_queue && !osi_list_is_empty(connect_queue)) {
        osi_list_remove(connect_queue, osi_list_front(connect_queue));
    }
}
...
bt_status_t btc_queue_connect_next(void)
{
    ...
    connect_node_t *p_head = osi_list_front(connect_queue);
