...
static inline void a2dp_cmd_acknowledge(int cmd, int status)
{
    ...
    btc_a2d_cb_to_app(YOC_A2D_MEDIA_CTRL_ACK_EVT, &param);
}

void btc_a2dp_control_command_ack(int status)
{
    /* sanity check */
    if (btc_aa_ctrl_cb.a2dp_cmd_pending == YOC_A2D_MEDIA_CTRL_NONE) {
    ...
    btc_aa_ctrl_cb.a2dp_cmd_pending = YOC_A2D_MEDIA_CTRL_NONE;
...
void btc_a2dp_control_media_ctrl(yoc_a2d_media_ctrl_t ctrl)
{
    ...
    switch (ctrl) {
    case YOC_A2D_MEDIA_CTRL_CHECK_SRC_RDY:
#if BTC_AV_SRC_INCLUDED
        if (btc_a2dp_source_is_task_shutting_down()) {
            ...
            btc_a2dp_control_command_ack(YOC_A2D_MEDIA_CTRL_ACK_SUCCESS);
        } else {
            btc_a2dp_control_command_ack(YOC_A2D_MEDIA_CTRL_ACK_FAILURE);
        }
    ...
    case YOC_A2D_MEDIA_CTRL_SUSPEND:
        /* local suspend */
