...
static inline void btc_gattc_cb_to_app(yoc_gattc_cb_event_t event, yoc_gatt_if_t gattc_if, yoc_ble_gattc_cb_param_t *param)
{
    yoc_gattc_cb_t btc_gattc_cb = (yoc_gattc_cb_t )btc_profile_cb_get(BTC_PID_GATTC);
    if (btc_gattc_cb) {
...
static yoc_gatt_status_t btc_gattc_check_valid_param(int num, uint16_t offset)
{
    if (num == 0) {
        return YOC_GATT_NOT_FOUND;
    } else if (offset >= num) {
        return YOC_GATT_INVALID_OFFSET;
    }

    return YOC_GATT_OK;
}

static void btc_gattc_fill_gatt_db_conversion(uint16_t count, uint16_t num, yoc_gatt_db_attr_type_t type,
                                                                  uint16_t offset, void *result, btgatt_db_element_t *db)
{
    tBT_UUID bta_uuid = {0};
    uint16_t db_size = (count + offset > num) ? (num - offset) : count;
    switch(type) {
        case YOC_GATT_DB_PRIMARY_SERVICE:
        case YOC_GATT_DB_SECONDARY_SERVICE: {
            yoc_gattc_service_elem_t *svc_result = (yoc_gattc_service_elem_t *)result;
        ...
        case YOC_GATT_DB_CHARACTERISTIC: {
            yoc_gattc_char_elem_t *char_result = (yoc_gattc_char_elem_t *)result;
            for (int i = 0; i < db_size; i++) {
        ...
        case YOC_GATT_DB_DESCRIPTOR: {
            yoc_gattc_descr_elem_t *descr_result = (yoc_gattc_descr_elem_t *)result;
            for (int i = 0; i < db_size; i++) {
        ...
        case YOC_GATT_DB_INCLUDED_SERVICE: {
            yoc_gattc_incl_svc_elem_t *incl_result = (yoc_gattc_incl_svc_elem_t *)result;
            for (int i = 0; i < db_size; i++) {
...
yoc_gatt_status_t btc_ble_gattc_get_service(uint16_t conn_id, yoc_bt_uuid_t *svc_uuid,
                                                           yoc_gattc_service_elem_t *result,
                                                           uint16_t *count, uint16_t offset)
{
    ...
    if ((status = btc_gattc_check_valid_param(svc_num, offset)) != YOC_GATT_OK) {
        if (db) {
            osi_free(db);
        }
        if (bta_uuid) {
        ...
        *count = 0;
        ...
        btc_gattc_fill_gatt_db_conversion(*count, (uint16_t)svc_num, YOC_GATT_DB_PRIMARY_SERVICE, offset, (void *)result, db);
    }
...
yoc_gatt_status_t btc_ble_gattc_get_all_char(uint16_t conn_id,
                                             ...
                                             uint16_t *count, uint16_t offset)
{
    ...
    if ((status = btc_gattc_check_valid_param(char_num, offset)) != YOC_GATT_OK) {
        if (db) {
            osi_free(db);
        }
        *count = 0;
        ...
        btc_gattc_fill_gatt_db_conversion(*count, (uint16_t)char_num, YOC_GATT_DB_CHARACTERISTIC, offset, (void *)result, db);
    }
...
yoc_gatt_status_t btc_ble_gattc_get_all_descr(uint16_t conn_id,
                                              ...
                                              uint16_t *count, uint16_t offset)
{
    ...
    if ((status = btc_gattc_check_valid_param(descr_num, offset)) != YOC_GATT_OK) {
        if (db) {
            osi_free(db);
        }
        *count = 0;
        ...
        btc_gattc_fill_gatt_db_conversion(*count, (uint16_t)descr_num, YOC_GATT_DB_DESCRIPTOR, offset, (void *)result, db);
    }
...
yoc_gatt_status_t btc_ble_gattc_get_char_by_uuid(uint16_t conn_id,
                                                 uint16_t start_handle,
                                                 uint16_t end_handle,
                                                 ...
                                                 uint16_t *count)
{
    ...
    if ((status = btc_gattc_check_valid_param(char_num, 0)) != YOC_GATT_OK) {
        if (db) {
            osi_free(db);
        }
        *count = 0;
        ...
        btc_gattc_fill_gatt_db_conversion(*count, (uint16_t)char_num, YOC_GATT_DB_CHARACTERISTIC, 0, (void *)result, db);
    }
...
yoc_gatt_status_t btc_ble_gattc_get_descr_by_uuid(uint16_t conn_id,
                                                  uint16_t start_handle,
                                                  uint16_t end_handle,
                                                  yoc_bt_uuid_t char_uuid,
                                                  ...
                                                  uint16_t *count)
{
    ...
    if ((status = btc_gattc_check_valid_param(descr_num, 0)) != YOC_GATT_OK) {
        if (db) {
            osi_free(db);
        }
        *count = 0;
        ...
        btc_gattc_fill_gatt_db_conversion(*count, (uint16_t)descr_num, YOC_GATT_DB_DESCRIPTOR, 0, (void *)result, db);
    }
...
yoc_gatt_status_t btc_ble_gattc_get_descr_by_char_handle(uint16_t conn_id,
                                                         uint16_t char_handle,
                                                         yoc_bt_uuid_t descr_uuid,
                                                         yoc_gattc_descr_elem_t *result,
                                                         uint16_t *count)
...
yoc_gatt_status_t btc_ble_gattc_get_include_service(uint16_t conn_id,
                                                    uint16_t start_handle,
                                                    uint16_t end_handle,
                                                    ...
                                                    uint16_t *count)
{
    ...
    if ((status = btc_gattc_check_valid_param(incl_num, 0)) != YOC_GATT_OK) {
        if (db) {
            osi_free(db);
        }
        *count = 0;
        ...
        btc_gattc_fill_gatt_db_conversion(*count, (uint16_t)incl_num, YOC_GATT_DB_INCLUDED_SERVICE, 0, (void *)result, db);
    }
...
yoc_gatt_status_t btc_ble_gattc_get_attr_count(uint16_t conn_id,
...
yoc_gatt_status_t btc_ble_gattc_get_db(uint16_t conn_id, uint16_t start_handle, uint16_t end_handle,
...
static void btc_gattc_reg_for_notify(btc_ble_gattc_args_t *arg)
{
    ...
    yoc_ble_gattc_cb_param_t param;
    ...
    memset(&param, 0, sizeof(yoc_ble_gattc_cb_param_t));
    param.reg_for_notify.status = status;
    param.reg_for_notify.handle = arg->reg_for_notify.handle;
    btc_gattc_cb_to_app(YOC_GATTC_REG_FOR_NOTIFY_EVT, arg->reg_for_notify.gattc_if, &param);
}
...
static void btc_gattc_unreg_for_notify(btc_ble_gattc_args_t *arg)
{
    ...
    btc_gattc_cb_to_app(YOC_GATTC_UNREG_FOR_NOTIFY_EVT, arg->unreg_for_notify.gattc_if, &param);
}
...
{
    tBTA_GATTC *arg = (tBTA_GATTC *)(msg->arg);
    yoc_gatt_if_t gattc_if = 0;
    yoc_ble_gattc_cb_param_t param = {0};
    ...
    case BTA_GATTC_REG_EVT: {
        ...
        btc_gattc_cb_to_app(YOC_GATTC_REG_EVT, gattc_if, &param);
        break;
    }
    case BTA_GATTC_DEREG_EVT: {
        ...
        gattc_if = reg_oper->client_if;
        btc_gattc_cb_to_app(YOC_GATTC_UNREG_EVT, gattc_if, NULL);
        break;
    }
    case BTA_GATTC_READ_CHAR_EVT: {
        set_read_value(&gattc_if, &param, &arg->read);
        btc_gattc_cb_to_app(YOC_GATTC_READ_CHAR_EVT, gattc_if, &param);
        break;
    }
    ...
    case BTA_GATTC_PREP_WRITE_EVT: {
        ...
        uint32_t ret_evt = (msg->act == BTA_GATTC_WRITE_CHAR_EVT) ?
                           YOC_GATTC_WRITE_CHAR_EVT : YOC_GATTC_PREP_WRITE_EVT;
    ...
    case BTA_GATTC_EXEC_EVT: {
        ...
        btc_gattc_cb_to_app(YOC_GATTC_EXEC_EVT, gattc_if, &param);
        break;
    }
    ...
    case BTA_GATTC_SEARCH_CMPL_EVT: {
        ...
        btc_gattc_cb_to_app(YOC_GATTC_SEARCH_CMPL_EVT, gattc_if, &param);
        break;
    }
    case BTA_GATTC_SEARCH_RES_EVT: {
        ...
        btc_gattc_cb_to_app(YOC_GATTC_SEARCH_RES_EVT, gattc_if, &param);
        break;
    }
    case BTA_GATTC_READ_DESCR_EVT: {
        set_read_value(&gattc_if, &param, &arg->read);
        btc_gattc_cb_to_app(YOC_GATTC_READ_DESCR_EVT, gattc_if, &param);
        break;
    }
    case BTA_GATTC_READ_MULTIPLE_EVT: {
        set_read_value(&gattc_if, &param, &arg->read);
        btc_gattc_cb_to_app(YOC_GATTC_READ_MULTIPLE_EVT, gattc_if, &param);
        break;
    }
    case BTA_GATTC_WRITE_DESCR_EVT: {
        ...
        param.write.status = write->status;
        param.write.handle = write->handle;
        btc_gattc_cb_to_app(YOC_GATTC_WRITE_DESCR_EVT, gattc_if, &param);
        break;
    }
    case BTA_GATTC_NOTIF_EVT: {
        ...
        memcpy(param.notify.remote_bda, notify->bda, sizeof(yoc_bd_addr_t));
        param.notify.handle = notify->handle;
        param.notify.is_notify = (notify->is_notify == TRUE) ? true : false;
        param.notify.value_len = (notify->len > YOC_GATT_MAX_ATTR_LEN) ? \
                                 YOC_GATT_MAX_ATTR_LEN : notify->len;
        ...
        btc_gattc_cb_to_app(YOC_GATTC_NOTIFY_EVT, gattc_if, &param);
        break;
    }
    case BTA_GATTC_OPEN_EVT: {
        ...
        memcpy(param.open.remote_bda, open->remote_bda, sizeof(yoc_bd_addr_t));
        param.open.mtu = open->mtu;
        btc_gattc_cb_to_app(YOC_GATTC_OPEN_EVT, gattc_if, &param);
        break;
    }
    case BTA_GATTC_CONNECT_EVT: {
        ...
        memcpy(param.connect.remote_bda, connect->remote_bda, sizeof(yoc_bd_addr_t));
        ...
        btc_gattc_cb_to_app(YOC_GATTC_CONNECT_EVT, gattc_if, &param);
        break;
    }
    case BTA_GATTC_CLOSE_EVT: {
        ...
        memcpy(param.close.remote_bda, close->remote_bda, sizeof(yoc_bd_addr_t));
        param.close.reason = close->reason;
        btc_gattc_cb_to_app(YOC_GATTC_CLOSE_EVT, gattc_if, &param);
        break;
    }
    case BTA_GATTC_DISCONNECT_EVT: {
        ...
        memcpy(param.disconnect.remote_bda, disconnect->remote_bda, sizeof(yoc_bd_addr_t));
        btc_gattc_cb_to_app(YOC_GATTC_DISCONNECT_EVT, gattc_if, &param);
        break;
    }
    case BTA_GATTC_CFG_MTU_EVT: {
        ...
        btc_gattc_cb_to_app(YOC_GATTC_CFG_MTU_EVT, gattc_if, &param);
        break;
    }
    ...
    case BTA_GATTC_CONGEST_EVT: {
        ...
        btc_gattc_cb_to_app(YOC_GATTC_CONGEST_EVT, gattc_if, &param);
        break;
    }
    case BTA_GATTC_SRVC_CHG_EVT: {
        tBTA_GATTC_SERVICE_CHANGE *srvc_change = &arg->srvc_chg;
        gattc_if = BTC_GATT_GET_GATT_IF(srvc_change->conn_id);
        memcpy(param.srvc_chg.remote_bda, srvc_change->remote_bda, sizeof(yoc_bd_addr_t));
        btc_gattc_cb_to_app(YOC_GATTC_SRVC_CHG_EVT, gattc_if, &param);
        break;
    }
    case BTA_GATTC_QUEUE_FULL_EVT: {
        ...
        btc_gattc_cb_to_app(YOC_GATTC_QUEUE_FULL_EVT, gattc_if, &param);
        break;
    }
    case BTA_GATTC_ASSOC_EVT: {
        gattc_if = arg->set_assoc.client_if;
        param.set_assoc_cmp.status = arg->set_assoc.status;
        btc_gattc_cb_to_app(YOC_GATTC_SET_ASSOC_EVT, gattc_if, &param);
        break;
    }
    case BTA_GATTC_GET_ADDR_LIST_EVT: {
        ...
        btc_gattc_cb_to_app(YOC_GATTC_GET_ADDR_LIST_EVT, gattc_if, &param);
        break;
    }
    case BTA_GATTC_DIS_SRVC_CMPL_EVT:
        ...
        btc_gattc_cb_to_app(YOC_GATTC_DIS_SRVC_CMPL_EVT, gattc_if, &param);
        break;
    default:
