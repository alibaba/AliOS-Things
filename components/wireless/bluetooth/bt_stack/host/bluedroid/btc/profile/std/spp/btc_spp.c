...
static spp_slot_t *spp_malloc_slot(void)
{
    ...
    for (size_t i = 1; i <= BTA_JV_MAX_RFC_SR_SESSION; i++) {
        if (spp_local_param.spp_slots[i] == NULL) {
            ...
            spp_local_param.spp_slots[i]->list = osi_list_new(spp_osi_free);
            if (spp_local_param.spp_mode == YOC_SPP_MODE_VFS) {
                spp_local_param.spp_slots[i]->incoming_list = osi_list_new(spp_osi_free);
                if (yoc_vfs_register_fd(spp_local_param.spp_vfs_id, &spp_local_param.spp_slots[i]->fd) != BT_OK) {
                    osi_free(spp_local_param.spp_slots[i]);
                    return NULL;
                }
                spp_local_param.spp_slots[i]->ringbuf_read = xRingbufferCreate(YOC_SPP_RINGBUF_SIZE, RINGBUF_TYPE_BYTEBUF);
                spp_local_param.spp_slots[i]->ringbuf_write = xRingbufferCreate(YOC_SPP_RINGBUF_SIZE, RINGBUF_TYPE_BYTEBUF);
            }
...
static void spp_free_slot(spp_slot_t *slot)
{
    ...
    osi_list_free(slot->list);
    if (spp_local_param.spp_mode == YOC_SPP_MODE_VFS) {
        (void) yoc_vfs_unregister_fd(spp_local_param.spp_vfs_id, slot->fd);
        osi_list_free(slot->incoming_list);
...
static inline void btc_spp_cb_to_app(yoc_spp_cb_event_t event, yoc_spp_cb_param_t *param)
{
    yoc_spp_cb_t *btc_spp_cb = (yoc_spp_cb_t *)btc_profile_cb_get(BTC_PID_SPP);
    if (btc_spp_cb) {
...
{
    yoc_spp_cb_param_t param;
    param.start.status = YOC_SPP_FAILURE;
    btc_spp_cb_to_app(YOC_SPP_START_EVT, &param);
}

static void btc_disconnect_cb(uint32_t handle)
{
    yoc_spp_cb_param_t param;
    param.close.status = YOC_SPP_SUCCESS;
    param.close.handle = handle;
    param.close.async = FALSE;
    btc_spp_cb_to_app(YOC_SPP_CLOSE_EVT, &param);
}
...
static void *btc_spp_rfcomm_inter_cb(tBTA_JV_EVT event, tBTA_JV *p_data, void *user_data)
{
    ...
    case BTA_JV_RFCOMM_SRV_OPEN_EVT:
        ...
        memcpy(slot->addr, p_data->rfc_srv_open.rem_bda, BT_BD_ADDR_LEN);
        slot->connected = TRUE;
...
static void btc_spp_connect(btc_spp_args_t *arg)
{
    ...
    memcpy(slot->addr, arg->connect.peer_bd_addr, BT_BD_ADDR_LEN);
...
static void btc_spp_write(btc_spp_args_t *arg)
{
    ...
    if (spp_local_param.spp_mode == YOC_SPP_MODE_VFS) {
        ...
        osi_list_append(slot->list, arg->write.p_data);
...
void btc_spp_cb_handler(btc_msg_t *msg)
{
    ...
    case BTA_JV_ENABLE_EVT:
        ...
        btc_spp_cb_to_app(YOC_SPP_INIT_EVT, &param);
        break;
    case BTA_JV_DISCOVERY_COMP_EVT:
        ...
        btc_spp_cb_to_app(YOC_SPP_DISCOVERY_COMP_EVT, &param);
        break;
    case BTA_JV_RFCOMM_CL_INIT_EVT:
        ...
        btc_spp_cb_to_app(YOC_SPP_CL_INIT_EVT, &param);
        break;
    case BTA_JV_RFCOMM_OPEN_EVT:
        if (spp_local_param.spp_mode == YOC_SPP_MODE_VFS) {
            osi_mutex_lock(&spp_local_param.spp_slot_mutex, OSI_MUTEX_MAX_TIMEOUT);
        ...
        memcpy(param.open.rem_bda, p_data->rfc_open.rem_bda, BT_BD_ADDR_LEN);
        btc_spp_cb_to_app(YOC_SPP_OPEN_EVT, &param);
        break;
    ...
    case BTA_JV_RFCOMM_SRV_OPEN_EVT:
        ...
        memcpy(param.srv_open.rem_bda, p_data->rfc_srv_open.rem_bda, BT_BD_ADDR_LEN);
        btc_spp_cb_to_app(YOC_SPP_SRV_OPEN_EVT, &param);
        break;
    case BTA_JV_RFCOMM_WRITE_EVT:
        ...
        if (spp_local_param.spp_mode == YOC_SPP_MODE_CB){
            ...
            btc_spp_cb_to_app(YOC_SPP_WRITE_EVT, &param);
            osi_list_remove(slot->list, osi_list_front(slot->list));
        } else {
    ...
    case BTA_JV_RFCOMM_CONG_EVT:
        if (spp_local_param.spp_mode == YOC_SPP_MODE_CB) {
            ...
            btc_spp_cb_to_app(YOC_SPP_CONG_EVT, &param);
        } else {
            osi_mutex_lock(&spp_local_param.spp_slot_mutex, OSI_MUTEX_MAX_TIMEOUT);
        ...
        break;
    case BTA_JV_RFCOMM_DATA_IND_EVT:
        param.data_ind.status = YOC_SPP_SUCCESS;
        ...
        btc_spp_cb_to_app(YOC_SPP_DATA_IND_EVT, &param);
...
static void spp_delay_append(list_t *list, BT_HDR *p_buf){
    uint8_t count = 0;
    while (count++ < 100) {
        if (osi_list_length(list) < 200){
            osi_mutex_lock(&spp_local_param.spp_slot_mutex, OSI_MUTEX_MAX_TIMEOUT);
            osi_list_append(list, p_buf);
            osi_mutex_unlock(&spp_local_param.spp_slot_mutex);
            return;
        }
...
static ssize_t spp_vfs_write(int fd, const void * data, size_t size)
{
    ...
    yoc_spp_write(slot->rfc_handle, 0, NULL);
    osi_mutex_unlock(&spp_local_param.spp_slot_mutex);
    if (done){
        return size;
    } else {
...
static int spp_vfs_close(int fd)
{
    ...
    yoc_spp_disconnect(slot->rfc_handle);
    osi_mutex_unlock(&spp_local_param.spp_slot_mutex);
    return 0;
}
...
{
    while (!osi_list_is_empty(slot->incoming_list)) {
        BT_HDR *p_buf = osi_list_front(slot->incoming_list);
        BaseType_t done = xRingbufferSend(slot->ringbuf_read, p_buf->data + p_buf->offset, p_buf->len, 0);
        if (done) {
            osi_list_remove(slot->incoming_list, p_buf);
        } else {
            return false;
        }
    }
...
bt_err_t btc_spp_vfs_register(void)
{
    yoc_vfs_t vfs = {
        .flags = YOC_VFS_FLAG_DEFAULT,
        .write = spp_vfs_write,
        .open = NULL,
        .fstat = NULL,
        .close = spp_vfs_close,
    ...
    // file descriptors
    if (yoc_vfs_register_with_id(&vfs, NULL, &spp_local_param.spp_vfs_id) != BT_OK) {
        return BT_FAIL;
    }
