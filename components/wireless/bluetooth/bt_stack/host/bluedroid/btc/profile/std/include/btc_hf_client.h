...
typedef union {
    ...
    struct volume_update_args {
        yoc_hf_volume_control_target_t type;
        int volume;
    } volume_update;

    // BTC_HF_CLIENT_DIAL_EVT
    struct dial_args {
        char number[YOC_BT_HF_CLIENT_NUMBER_LEN + 1];
    } dial;
    ...
    struct send_chld_cmd_args {
        yoc_hf_chld_type_t type;
        int idx;
    } chld;

    // BTC_HF_CLIENT_SEND_BTRH_CMD_EVT
    struct send_btrh_cmd_args {
        yoc_hf_btrh_cmd_t cmd;
    } btrh;
    ...
    struct reg_data_callback {
        yoc_hf_client_incoming_data_cb_t recv;
...
typedef struct
{
    ...
    yoc_hf_client_connection_state_t   state;
    yoc_hf_vr_state_t                  vr_state;
...
typedef struct
{
    ...
    yoc_hf_client_incoming_data_cb_t    btc_hf_client_incoming_data_cb;
    yoc_hf_client_outgoing_data_cb_t    btc_hf_client_outgoing_data_cb;
