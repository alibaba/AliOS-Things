...
if (! hf_client_local_param.btc_hf_client_cb.initialized || \
    hf_client_local_param.btc_hf_client_cb.state != YOC_HF_CLIENT_CONNECTION_STATE_SLC_CONNECTED) { \
    return BT_STATUS_NOT_READY; \
} \
} while (0)

static inline void btc_hf_client_cb_to_app(yoc_hf_client_cb_event_t event, yoc_hf_client_cb_param_t *param)
{
    yoc_hf_client_cb_t btc_hf_client_callback = (yoc_hf_client_cb_t)btc_profile_cb_get(BTC_PID_HF_CLIENT);
...
{
    if (((hf_client_local_param.btc_hf_client_cb.state == YOC_HF_CLIENT_CONNECTION_STATE_CONNECTED) ||
            (hf_client_local_param.btc_hf_client_cb.state == YOC_HF_CLIENT_CONNECTION_STATE_SLC_CONNECTED))&&
...
void btc_hf_client_reg_data_cb(yoc_hf_client_incoming_data_cb_t recv,
...
bt_status_t btc_hf_client_init(void)
{
    ...
    data_path = YOC_SCO_DATA_PATH_HCI;
#else
    data_path = YOC_SCO_DATA_PATH_PCM;
#endif
    yoc_bredr_sco_datapath_set(data_path);
...
static bt_status_t connect_int( bt_bdaddr_t *bd_addr, uint16_t uuid )
{
    ...
    hf_client_local_param.btc_hf_client_cb.state = YOC_HF_CLIENT_CONNECTION_STATE_CONNECTING;
...
bt_status_t btc_hf_client_connect_audio( bt_bdaddr_t *bd_addr )
{
    ...
    if (is_connected(bd_addr))
    {
        ...
        do {
            yoc_hf_client_cb_param_t param;
            memset(&param, 0, sizeof(yoc_hf_client_cb_param_t));
            param.audio_stat.state = YOC_HF_CLIENT_AUDIO_STATE_CONNECTING;
            memcpy(param.audio_stat.remote_bda, &hf_client_local_param.btc_hf_client_cb.connected_bda, sizeof(yoc_bd_addr_t));
            btc_hf_client_cb_to_app(YOC_HF_CLIENT_AUDIO_STATE_EVT, &param);
        } while (0);
...
**
*******************************************************************************/
static bt_status_t btc_hf_client_volume_update(yoc_hf_volume_control_target_t type, int volume)
{
    CHECK_HF_CLIENT_SLC_CONNECTED();

    switch (type)
    {
        case YOC_HF_VOLUME_CONTROL_TARGET_SPK:
            BTA_HfClientSendAT(hf_client_local_param.btc_hf_client_cb.handle, BTA_HF_CLIENT_AT_CMD_VGS, volume, 0, NULL);
            break;
        case YOC_HF_VOLUME_CONTROL_TARGET_MIC:
...
static bt_status_t btc_hf_client_send_chld_cmd(yoc_hf_chld_type_t type, int idx)
{
    CHECK_HF_CLIENT_SLC_CONNECTED();

    switch (type)
    {
    case YOC_HF_CHLD_TYPE_REL:
    ...
    case YOC_HF_CHLD_TYPE_REL_ACC:
        ...
        return BT_STATUS_UNSUPPORTED;
    case YOC_HF_CHLD_TYPE_HOLD_ACC:
    ...
    case YOC_HF_CHLD_TYPE_MERGE:
        ...
        return BT_STATUS_UNSUPPORTED;
    case YOC_HF_CHLD_TYPE_MERGE_DETACH:
...
static void process_ind_evt(tBTA_HF_CLIENT_IND *ind)
{
    ...
    switch (ind->type)
    {
        case BTA_HF_CLIENT_IND_CALL:
            ...
            btc_hf_client_cb_to_app(YOC_HF_CLIENT_CIND_CALL_EVT, &param);
            break;

        case BTA_HF_CLIENT_IND_CALLSETUP:
            param.call_setup.status = ind->value;
            btc_hf_client_cb_to_app(YOC_HF_CLIENT_CIND_CALL_SETUP_EVT, &param);
            break;
        case BTA_HF_CLIENT_IND_CALLHELD:
            param.call_held.status = ind->value;
            btc_hf_client_cb_to_app(YOC_HF_CLIENT_CIND_CALL_HELD_EVT, &param);
            break;

        case BTA_HF_CLIENT_IND_SERVICE:
            param.service_availability.status = ind->value;
            btc_hf_client_cb_to_app(YOC_HF_CLIENT_CIND_SERVICE_AVAILABILITY_EVT, &param);
            break;

        case BTA_HF_CLIENT_IND_SIGNAL:
            param.signal_strength.value = ind->value;
            btc_hf_client_cb_to_app(YOC_HF_CLIENT_CIND_SIGNAL_STRENGTH_EVT, &param);
            break;

        case BTA_HF_CLIENT_IND_ROAM:
            param.roaming.status = ind->value;
            btc_hf_client_cb_to_app(YOC_HF_CLIENT_CIND_ROAMING_STATUS_EVT, &param);
            break;

        case BTA_HF_CLIENT_IND_BATTCH:
            param.battery_level.value = ind->value;
            btc_hf_client_cb_to_app(YOC_HF_CLIENT_CIND_BATTERY_LEVEL_EVT, &param);
            break;
    ...
    switch (event)
    {
        ...
        case BTA_HF_CLIENT_OPEN_EVT:
            if (p_data->open.status == BTA_HF_CLIENT_SUCCESS)
            {
                ...
                hf_client_local_param.btc_hf_client_cb.state = YOC_HF_CLIENT_CONNECTION_STATE_CONNECTED;
                hf_client_local_param.btc_hf_client_cb.peer_feat = 0;
                hf_client_local_param.btc_hf_client_cb.chld_feat = 0;
                //clear_phone_state();
            }
            else if (hf_client_local_param.btc_hf_client_cb.state == YOC_HF_CLIENT_CONNECTION_STATE_CONNECTING)
            {
                hf_client_local_param.btc_hf_client_cb.state = YOC_HF_CLIENT_CONNECTION_STATE_DISCONNECTED;
            }
            else
            {
            ...
            do {
                ...
                memcpy(param.conn_stat.remote_bda, &hf_client_local_param.btc_hf_client_cb.connected_bda,
                       sizeof(yoc_bd_addr_t));

                btc_hf_client_cb_to_app(YOC_HF_CLIENT_CONNECTION_STATE_EVT, &param);
            } while (0);

            if (hf_client_local_param.btc_hf_client_cb.state == YOC_HF_CLIENT_CONNECTION_STATE_DISCONNECTED)
        ...
        case BTA_HF_CLIENT_CONN_EVT:
            ...
            if (hf_client_local_param.btc_hf_client_cb.peer_feat & BTA_HF_CLIENT_PEER_INBAND)
            {
                do {
                    memset(&param, 0, sizeof(yoc_hf_client_cb_param_t));
                    param.bsir.state =  YOC_HF_CLIENT_IN_BAND_RINGTONE_PROVIDED;
                    btc_hf_client_cb_to_app(YOC_HF_CLIENT_BSIR_EVT, &param);
                } while (0);
            }
        ...
        case BTA_HF_CLIENT_MIC_EVT:
            do {
                memset(&param, 0, sizeof(yoc_hf_client_cb_param_t));
                param.volume_control.type = YOC_HF_VOLUME_CONTROL_TARGET_MIC;
                param.volume_control.volume = p_data->val.value;

                btc_hf_client_cb_to_app(YOC_HF_CLIENT_VOLUME_CONTROL_EVT, &param);
            } while (0);
            break;
        ...
        case BTA_HF_CLIENT_VOICE_REC_EVT:
            do {
                ...
                btc_hf_client_cb_to_app(YOC_HF_CLIENT_BVRA_EVT, &param);
            } while (0);
            break;
        case BTA_HF_CLIENT_OPERATOR_NAME_EVT:
            do {
                ...
                btc_hf_client_cb_to_app(YOC_HF_CLIENT_COPS_CURRENT_OPERATOR_EVT, &param);
            } while (0);
            break;
        case BTA_HF_CLIENT_CLIP_EVT:
            do {
                ...
                btc_hf_client_cb_to_app(YOC_HF_CLIENT_CLIP_EVT, &param);
            } while (0);
            break;
        case BTA_HF_CLIENT_BINP_EVT:
            do {
                ...
                btc_hf_client_cb_to_app(YOC_HF_CLIENT_BINP_EVT, &param);
            } while (0);
            break;
        case BTA_HF_CLIENT_CCWA_EVT:
            do {
                ...
                btc_hf_client_cb_to_app(YOC_HF_CLIENT_CCWA_EVT, &param);
            } while (0);
            break;
        case BTA_HF_CLIENT_AT_RESULT_EVT:
            do {
                ...
                btc_hf_client_cb_to_app(YOC_HF_CLIENT_AT_RESPONSE_EVT, &param);
            } while (0);
            break;
        case BTA_HF_CLIENT_CLCC_EVT:
            do {
                ...
                param.clcc.dir = p_data->clcc.inc ? YOC_HF_CURRENT_CALL_DIRECTION_INCOMING :
                    YOC_HF_CURRENT_CALL_DIRECTION_OUTGOING;
                param.clcc.status = p_data->clcc.status;
                param.clcc.mpty = p_data->clcc.mpty ? YOC_HF_CURRENT_CALL_MPTY_TYPE_MULTI :
                    YOC_HF_CURRENT_CALL_MPTY_TYPE_SINGLE;
                param.clcc.number = p_data->clcc.number_present ? p_data->clcc.number : NULL;
                btc_hf_client_cb_to_app(YOC_HF_CLIENT_CLCC_EVT, &param);
            } while (0);
            break;
        case BTA_HF_CLIENT_CNUM_EVT:
            do {
                ...
                if (p_data->cnum.service == 4) {
                    param.cnum.type = YOC_HF_SUBSCRIBER_SERVICE_TYPE_VOICE;
                } else if (p_data->cnum.service == 5) {
                    param.cnum.type = YOC_HF_SUBSCRIBER_SERVICE_TYPE_FAX;
                } else {
                    param.cnum.type = YOC_HF_SUBSCRIBER_SERVICE_TYPE_UNKNOWN;
                }
                btc_hf_client_cb_to_app(YOC_HF_CLIENT_CNUM_EVT, &param);
                break;
            } while (0);
            break;
        case BTA_HF_CLIENT_BTRH_EVT:
            if (p_data->val.value <= YOC_HF_BTRH_STATUS_REJECTED) {
                do {
                    memset(&param, 0, sizeof(yoc_hf_client_cb_param_t));
                    param.btrh.status = p_data->val.value;

                    btc_hf_client_cb_to_app(YOC_HF_CLIENT_BTRH_EVT, &param);
                } while (0);
            }
            break;
        case BTA_HF_CLIENT_BSIR_EVT:
            ...
            {
                param.bsir.state = YOC_HF_CLIENT_IN_BAND_RINGTONE_NOT_PROVIDED;
            }
            btc_hf_client_cb_to_app(YOC_HF_CLIENT_BSIR_EVT, &param);
            break;
        case BTA_HF_CLIENT_AUDIO_OPEN_EVT:
            do {
                memset(&param, 0, sizeof(yoc_hf_client_cb_param_t));
                param.audio_stat.state = YOC_HF_CLIENT_AUDIO_STATE_CONNECTED;
        ...
        case BTA_HF_CLIENT_AUDIO_CLOSE_EVT:
            do {
                memset(&param, 0, sizeof(yoc_hf_client_cb_param_t));
                param.audio_stat.state = YOC_HF_CLIENT_AUDIO_STATE_DISCONNECTED;
                memcpy(param.audio_stat.remote_bda, &hf_client_local_param.btc_hf_client_cb.connected_bda,
                       sizeof(yoc_bd_addr_t));
                btc_hf_client_cb_to_app(YOC_HF_CLIENT_AUDIO_STATE_EVT, &param);
            } while (0);
        ...
        case BTA_HF_CLIENT_RING_INDICATION:
            do {
                btc_hf_client_cb_to_app(YOC_HF_CLIENT_RING_IND_EVT, NULL);
            } while (0);
            break;
        default:
