...
static inline void btc_gap_ble_cb_to_app(yoc_gap_ble_cb_event_t event, yoc_ble_gap_cb_param_t *param)
{
    yoc_gap_ble_cb_t btc_gap_ble_cb = (yoc_gap_ble_cb_t)btc_profile_cb_get(BTC_PID_GAP_BLE);
    if (btc_gap_ble_cb) {
...
static void btc_to_bta_adv_data(yoc_ble_adv_data_t *p_adv_data, tBTA_BLE_ADV_DATA *bta_adv_data, uint32_t *data_mask)
{
    uint32_t mask;
    ...
    //     bta_adv_data->tx_power = yoc_ble_tx_power_get(YOC_BLE_PWR_TYPE_ADV);
    // }
...
{
    yoc_ble_gap_cb_param_t param;
    bt_status_t ret;
    btc_msg_t msg;

    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_GAP_BLE;
    msg.act = YOC_GAP_BLE_ADV_DATA_SET_COMPLETE_EVT;
...
static void btc_scan_rsp_data_callback(tBTA_STATUS status)
{
    ...
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_GAP_BLE;
    msg.act = YOC_GAP_BLE_SCAN_RSP_DATA_SET_COMPLETE_EVT;
...
static void btc_adv_data_raw_callback(tBTA_STATUS status)
{
    ...
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_GAP_BLE;
    msg.act = YOC_GAP_BLE_ADV_DATA_RAW_SET_COMPLETE_EVT;
...
static void btc_scan_rsp_data_raw_callback(tBTA_STATUS status)
{
    ...
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_GAP_BLE;
    msg.act = YOC_GAP_BLE_SCAN_RSP_DATA_RAW_SET_COMPLETE_EVT;
...
static void btc_ble_set_adv_data(yoc_ble_adv_data_t *adv_data,
...
static void btc_start_adv_callback(uint8_t status)
{
    ...
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_GAP_BLE;
    msg.act = YOC_GAP_BLE_ADV_START_COMPLETE_EVT;
    param.adv_start_cmpl.status = btc_hci_to_yoc_status(status);
...
static void btc_stop_adv_callback(uint8_t status)
{
    ...
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_GAP_BLE;
    msg.act = YOC_GAP_BLE_ADV_STOP_COMPLETE_EVT;
    param.adv_stop_cmpl.status = btc_hci_to_yoc_status(status);
...
void btc_update_duplicate_exceptional_list_callback(tBTA_STATUS status, uint8_t subcode, uint32_t length, uint8_t *device_info)
{
    ...
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_GAP_BLE;
    msg.act = YOC_GAP_BLE_UPDATE_DUPLICATE_EXCEPTIONAL_LIST_COMPLETE_EVT;
    ...
    ret = btc_transfer_context(&msg, &param, sizeof(yoc_ble_gap_cb_param_t), NULL);
...
static void btc_ble_start_advertising (yoc_ble_adv_params_t *ble_adv_params, tBTA_START_ADV_CMPL_CBACK start_adv_cback)
{
    tBLE_BD_ADDR peer_addr;
    yoc_bt_status_t status = YOC_BT_STATUS_SUCCESS;
    if (!BLE_ISVALID_PARAM(ble_adv_params->adv_int_min, BTM_BLE_ADV_INT_MIN, BTM_BLE_ADV_INT_MAX) ||
        ...
        status = YOC_BT_STATUS_PARM_INVALID;
    ...
    if(status != YOC_BT_STATUS_SUCCESS) {
        if(start_adv_cback) {
    ...
    memcpy(peer_addr.bda, ble_adv_params->peer_addr, BT_BD_ADDR_LEN);
...
static void btc_scan_params_callback(tGATT_IF gatt_if, tBTM_STATUS status)
{
    ...
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_GAP_BLE;
    msg.act = YOC_GAP_BLE_SCAN_PARAM_SET_COMPLETE_EVT;
...
static void btc_ble_set_scan_params(yoc_ble_scan_params_t *scan_params, tBLE_SCAN_PARAM_SETUP_CBACK scan_param_setup_cback)
{
    if (BLE_ISVALID_PARAM(scan_params->scan_interval, BTM_BLE_SCAN_INT_MIN, BTM_BLE_SCAN_INT_MAX) &&
        ...
        BTA_DmSetBleScanFilterParams(YOC_DEFAULT_GATT_IF,	/*client_if*/
        ...
        btc_scan_params_callback(YOC_DEFAULT_GATT_IF, BTM_ILLEGAL_VALUE);
    }
}
...
static void btc_search_callback(tBTA_DM_SEARCH_EVT event, tBTA_DM_SEARCH *p_data)
{
    ...
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_GAP_BLE;
    msg.act = YOC_GAP_BLE_SCAN_RESULT_EVT;
    ...
    btc_transfer_context(&msg, &param, sizeof(yoc_ble_gap_cb_param_t), NULL);
}
...
static void btc_start_scan_callback(uint8_t status)
{
    ...
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_GAP_BLE;
    msg.act = YOC_GAP_BLE_SCAN_START_COMPLETE_EVT;
...
static void btc_stop_scan_callback(tBTA_STATUS status)
{
    ...
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_GAP_BLE;
    msg.act = YOC_GAP_BLE_SCAN_STOP_COMPLETE_EVT;
...
void btc_update_conn_param_callback (UINT8 status, BD_ADDR bd_addr, tBTM_LE_UPDATE_CONN_PRAMS *update_conn_params)
{
    ...
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_GAP_BLE;
    msg.act = YOC_GAP_BLE_UPDATE_CONN_PARAMS_EVT;
    param.update_conn_params.status = btc_hci_to_yoc_status(status);
    ...
    memcpy(param.update_conn_params.bda, bd_addr, sizeof(yoc_bd_addr_t));
    ret = btc_transfer_context(&msg, &param,
                               sizeof(yoc_ble_gap_cb_param_t), NULL);
...
static void btc_set_pkt_length_callback(UINT8 status, tBTM_LE_SET_PKT_DATA_LENGTH_PARAMS *data_len_params)
{
    ...
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_GAP_BLE;
    msg.act = YOC_GAP_BLE_SET_PKT_LENGTH_COMPLETE_EVT;
    param.pkt_data_lenth_cmpl.status = btc_btm_status_to_yoc_status(status);
...
static void btc_add_whitelist_complete_callback(UINT8 status, tBTM_WL_OPERATION wl_opration)
{
    ...
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_GAP_BLE;
    msg.act = YOC_GAP_BLE_UPDATE_WHITELIST_COMPLETE_EVT;
    param.update_whitelist_cmpl.status = btc_hci_to_yoc_status(status);
...
static void btc_set_rand_addr_callback(UINT8 status)
{
    ...
    bt_status_t ret;
    btc_msg_t msg;
    param.set_rand_addr_cmpl.status = btc_btm_status_to_yoc_status(status); //todo status
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_GAP_BLE;
    msg.act = YOC_GAP_BLE_SET_STATIC_RAND_ADDR_EVT;
    ret = btc_transfer_context(&msg, &param,
                               sizeof(yoc_ble_gap_cb_param_t), NULL);
...
static void btc_set_local_privacy_callback(UINT8 status)
{
    ...
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_GAP_BLE;
    msg.act = YOC_GAP_BLE_SET_LOCAL_PRIVACY_COMPLETE_EVT;
    param.local_privacy_cmpl.status = btc_btm_status_to_yoc_status(status);
    ret = btc_transfer_context(&msg, &param,
                               sizeof(yoc_ble_gap_cb_param_t), NULL);
...
static void btc_read_ble_rssi_cmpl_callback(void *p_data)
{
    ...
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_GAP_BLE;
    msg.act = YOC_GAP_BLE_READ_RSSI_COMPLETE_EVT;
    param.read_rssi_cmpl.rssi = result->rssi;
    param.read_rssi_cmpl.status = btc_btm_status_to_yoc_status(result->status);
...
{
    yoc_ble_gap_cb_param_t *param = (yoc_ble_gap_cb_param_t *)msg->arg;

    if (msg->act < YOC_GAP_BLE_EVT_MAX) {
...
void btc_gap_ble_arg_deep_free(btc_msg_t *msg)
{
    ...
    switch (msg->act) {
    case BTC_GAP_BLE_ACT_CFG_ADV_DATA: {
        yoc_ble_adv_data_t *adv = &((btc_ble_gap_args_t *)msg->arg)->cfg_adv_data.adv_data;
...
void btc_gap_ble_call_handler(btc_msg_t *msg)
{
    ...
    case BTC_GAP_BLE_SET_SECURITY_PARAM_EVT: {
        ...
        switch(arg->set_security_param.param_type) {
            ...
            case YOC_BLE_SM_CLEAR_STATIC_PASSKEY: {
                BTA_DmBleSetStaticPasskey(false, 0);
                break;
            }
            case YOC_BLE_SM_ONLY_ACCEPT_SPECIFIED_SEC_AUTH: {
                uint8_t enable = 0;
                STREAM_TO_UINT8(enable, value);
                bta_dm_co_ble_set_accept_auth_enable(enable);
                break;
            }
            case YOC_BLE_SM_OOB_SUPPORT: {
                uint8_t enable = 0;
                STREAM_TO_UINT8(enable, value);
...
void btc_adv_list_init(void)
{
    ...
    adv_filter_list = osi_list_new(btc_adv_list_free);
}
...
void btc_adv_list_add_packet(void * data)
{
    ...
    btc_adv_list_lock();
    osi_list_prepend(adv_filter_list, data);
    btc_adv_list_unlock();
}
...
uint32_t btc_get_adv_list_length(void)
{
    ...
    btc_adv_list_lock();
    size_t length = osi_list_length(adv_filter_list);
    btc_adv_list_unlock();
...
void btc_adv_list_refresh(void)
{
    ...
    btc_adv_list_lock();
    osi_list_clear(adv_filter_list);
    btc_adv_list_unlock();
}
...
bool btc_check_adv_list(uint8_t * addr, uint8_t addr_type)
{
    ...
    btc_adv_list_lock();
    for (const list_node_t *node = osi_list_begin(adv_filter_list); node != osi_list_end(adv_filter_list); node = osi_list_next(node)) {
        btc_adv_packet_t *packet = (btc_adv_packet_t *)osi_list_node(node);
