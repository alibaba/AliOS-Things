...
#define YOC_SPP_MAX_SESSION     BTA_JV_MAX_RFC_SR_SESSION
#define YOC_SPP_SERVER_NAME_MAX 32

#define YOC_SPP_RINGBUF_SIZE 1000
...
typedef union {
    ...
    //BTC_SPP_ACT_CONNECT
    struct connect_arg {
        yoc_spp_sec_t sec_mask;
        yoc_spp_role_t role;
        UINT8 remote_scn;
        yoc_bd_addr_t peer_bd_addr;
    } connect;
    //BTC_SPP_ACT_DISCONNECT
    ...
    struct start_srv_arg {
        ...
        UINT8 local_scn;
        UINT8 max_session;
        char name[YOC_SPP_SERVER_NAME_MAX + 1];
    } start_srv;
    //BTC_SPP_ACT_WRITE
    struct write_arg {
        UINT32 handle;
