...
yoc_bt_uuid_t       bas_uuid = {LEN_UUID_16, {UUID_SERVCLASS_BATTERY}};
/******************************************************************************
** Function      bas_gatts_callback
**
** Description   battery service register callback function
*******************************************************************************/
static void bas_gatts_callback(yoc_gatts_evt_t event, tBTA_GATTS *p_data)
{
    switch (event) {
    case YOC_GATTS_REG_EVT: {
        yoc_gatt_status_t  status = p_data->reg_oper.status;
...
void bas_s_read_attr_value(tGATTS_DATA *p_data, UINT32 trans_id, UINT16 conn_id)
{
    ...
    UINT8               i;
    yoc_gatt_status_t   st = YOC_GATT_NOT_FOUND;
...
{
    yoc_ble_gatts_app_register(&bas_uuid, bas_gatts_callback);
...
void bas_init(tBTA_GATTS_IF gatt_if, UINT16 app_id)
{
    ...
    yoc_ble_gatts_create_srvc (gatt_if, &bas_uuid, battery_cb.inst_id ,
...
void bas_AddChar(UINT16 service_id, tBT_UUID *char_uuid)
{
    ...
    yoc_ble_gatts_add_char(service_id, char_uuid, BATTER_LEVEL_PERM, prop);
...
void bas_AddCharDescr(UINT16 service_id, UINT16 attr_id)
{
    ...
    if (ba_reg_info.ba_level_descr != 0) {
        if (ba_reg_info.ba_level_descr & BA_LEVEL_NOTIFY) {
            ...
            yoc_ble_gatts_add_char_descr(service_id, (GATT_PERM_READ | GATT_PERM_WRITE), &uuid);
            return;
        }
        ...
        if (ba_reg_info.ba_level_descr & BA_LEVEL_PRE_FMT) {
            ...
            yoc_ble_gatts_add_char_descr(service_id, GATT_PERM_READ, &uuid);
        ...
        yoc_ble_gatts_start_srvc(service_id);
    }
...
**
****************************************************************/
void bas_service_cmpl(UINT16 service_id, yoc_gatt_status_t status)
{
    if (status != YOC_GATT_OK) {
        battery_cb.inst_id --;
        yoc_ble_gatts_dele_srvc(service_id);
    }
...
void Battery_Rsp (UINT32 trans_id, UINT16 conn_id, UINT8 app_id,
                  yoc_gatt_status_t st, UINT8 event, tBA_RSP_DATA *p_rsp)
{
    tBA_INST *p_inst = &battery_cb.battery_inst[0];
    tGATTS_RSP  rsp;
    UINT8   *pp;
    ...
    if (p_inst->pending_evt == event) {
        ...
        case BA_READ_CLT_CFG_REQ:
            ...
            yoc_ble_gatts_send_rsp(conn_id, trans_id, st, &rsp);
        ...
        case BA_WRITE_CLT_CFG_REQ:
            yoc_ble_gatts_send_rsp(conn_id, trans_id, st, NULL);
...
void Battery_Notify (UINT16 conn_id, UINT8 app_id, BD_ADDR remote_bda, UINT8 battery_level)
{
    ...
    yoc_ble_gatts_hdl_val_indica(conn_id, p_inst->ba_level_hdl, 1, &battery_level, false);
