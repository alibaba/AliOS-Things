...
static inline void btc_gap_bt_cb_to_app(yoc_bt_gap_cb_event_t event, yoc_bt_gap_cb_param_t *param)
{
    yoc_bt_gap_cb_t cb = (yoc_bt_gap_cb_t)btc_profile_cb_get(BTC_PID_GAP_BT);
    if (cb) {
        cb(event, param);
    }
}

static void btc_bt_set_scan_mode(yoc_bt_connection_mode_t c_mode, yoc_bt_discovery_mode_t d_mode)
{
    tBTA_DM_DISC disc_mode;
    tBTA_DM_CONN conn_mode;

    switch (c_mode) {
    case YOC_BT_NON_CONNECTABLE:
        conn_mode = BTA_DM_NON_CONN;
        break;
    case YOC_BT_CONNECTABLE:
    ...
    switch (d_mode) {
    case YOC_BT_NON_DISCOVERABLE:
        disc_mode = BTA_DM_NON_DISC;
        break;
    case YOC_BT_LIMITED_DISCOVERABLE:
        disc_mode = BTA_DM_LIMITED_DISC;
        break;
    case YOC_BT_GENERAL_DISCOVERABLE:
...
static void btc_gap_bt_start_discovery(btc_gap_bt_args_t *arg)
{
    ...
    inq_params.mode = (arg->start_disc.mode == YOC_BT_INQ_MODE_GENERAL_INQUIRY) ?
...
{
    yoc_bt_uuid_t *uuid = &arg->get_rmt_srv_rcd.uuid;
...
static void btc_gap_bt_search_devices_evt(tBTA_DM_SEARCH_PARAM *p_data)
{
    ...
    case BTA_DM_DISC_RES_EVT: {
        ...
        if (bdname_len) {
            yoc_bt_gap_dev_prop_t prop[1];

            BTC_STORAGE_FILL_PROPERTY(&prop[0], YOC_BT_GAP_DEV_PROP_BDNAME, bdname_len + 1, p_data->p_data->disc_res.bd_name);
            ...
            btc_gap_bt_cb_to_app(YOC_BT_GAP_DISC_RES_EVT, &param);
        }
        break;
    }
    case BTA_DM_INQ_RES_EVT: {
        ...
        do {
            yoc_bt_gap_dev_prop_t prop[3];
            int num_prop = 0;

            memset(prop, 0, sizeof(prop));
            BTC_STORAGE_FILL_PROPERTY(&prop[0], YOC_BT_GAP_DEV_PROP_COD, sizeof(cod), &cod);
            num_prop++;

            BTC_STORAGE_FILL_PROPERTY(&prop[1], YOC_BT_GAP_DEV_PROP_RSSI, 1, &(p_data->p_data->inq_res.rssi));
            num_prop++;

            if (p_data->p_data->inq_res.p_eir) {
                BTC_STORAGE_FILL_PROPERTY(&prop[2], YOC_BT_GAP_DEV_PROP_EIR, HCI_EXT_INQ_RESPONSE_LEN, p_data->p_data->inq_res.p_eir);
                num_prop++;
            }
    ...
    case BTA_DM_DISC_CMPL_EVT: {
        yoc_bt_gap_cb_param_t param;
        param.disc_st_chg.state = YOC_BT_GAP_DISCOVERY_STOPPED;
        btc_gap_bt_cb_to_app(YOC_BT_GAP_DISC_STATE_CHANGED_EVT, &param);
        break;
    }
...
static void btc_gap_bt_search_service_record(char *p_param)
{
    ...
    case BTA_DM_DISC_RES_EVT: {
        ...
        if (p_data->p_data->disc_res.p_raw_data && p_data->p_data->disc_res.raw_data_size > 0) {
            param.rmt_srvc_rec.stat = YOC_BT_STATUS_SUCCESS;
            ...
            param.rmt_srvc_rec.stat = YOC_BT_STATUS_FAIL;
            // param.rmt_srvc_rec.raw_data_size = 0;
            // param.rmt_srvc_rec.raw_data = NULL;
        }
        btc_gap_bt_cb_to_app(YOC_BT_GAP_RMT_SRVC_REC_EVT, &param);
    }
    break;
    case BTA_DM_DISC_CMPL_EVT:
    default:
...
static void btc_gap_bt_search_services(char *p_param)
{
    ...
    case BTA_DM_DISC_RES_EVT: {
        yoc_bt_gap_cb_param_t param;
        yoc_bt_uuid_t *uuid_list = NULL;
        memcpy(param.rmt_srvcs.bda, p_data->p_data->disc_res.bd_addr, BD_ADDR_LEN);

        param.rmt_srvcs.stat = YOC_BT_STATUS_FAIL;
        if (p_data->p_data->disc_res.result == BTA_SUCCESS) {
            uuid_list = osi_malloc(sizeof(yoc_bt_uuid_t) * p_data->p_data->disc_res.num_uuids);
            if (uuid_list) {
                param.rmt_srvcs.stat = YOC_BT_STATUS_SUCCESS;
                ...
                yoc_bt_uuid_t *o_uu = uuid_list;
                for (int i = 0; i < p_data->p_data->disc_res.num_uuids; i++, i_uu += YOC_UUID_LEN_128, o_uu++) {
                    uuid128_be_to_yoc_uuid(o_uu, i_uu);
                }
            }
        }

        if (param.rmt_srvcs.stat == YOC_BT_STATUS_FAIL) {
            param.rmt_srvcs.num_uuids = 0;
            param.rmt_srvcs.uuid_list = NULL;
        }
        btc_gap_bt_cb_to_app(YOC_BT_GAP_RMT_SRVCS_EVT, &param);
...
{
    tBTA_UTL_COD p_cod;
    yoc_bt_cod_t *cod = &(arg->set_cod.cod);
...
static void btc_gap_bt_read_rssi_delta_cmpl_callback(void *p_data)
{
    ...
    param.read_rssi_delta.stat = btc_btm_status_to_yoc_status(result->status);
...
static bt_err_t btc_gap_bt_remove_bond_device(btc_gap_bt_args_t *arg)
{
    BD_ADDR bd_addr;
...
static bt_err_t btc_gap_bt_set_security_param(btc_gap_bt_args_t *arg)
{
    bt_err_t ret;
    switch(arg->set_security_param.param_type) {
    case YOC_BT_SP_IOCAP_MODE:{
        uint8_t iocap = 0;
...
static void btc_gap_bt_config_eir(btc_gap_bt_args_t *arg)
{
    ...
    yoc_bt_eir_data_t *eir_data = &arg->config_eir.eir_data;
...
void btc_gap_bt_busy_level_updated(uint8_t bl_flags)
{
    ...
    if (bl_flags == BTM_BL_INQUIRY_STARTED) {
        param.disc_st_chg.state = YOC_BT_GAP_DISCOVERY_STARTED;
        btc_gap_bt_cb_to_app(YOC_BT_GAP_DISC_STATE_CHANGED_EVT, &param);
...
void btc_gap_bt_cb_handler(btc_msg_t *msg)
{
    ...
    case BTC_GAP_BT_READ_RSSI_DELTA_EVT:{
        btc_gap_bt_cb_to_app(YOC_BT_GAP_READ_RSSI_DELTA_EVT, (yoc_bt_gap_cb_param_t *)msg->arg);
        break;
    }
    case BTC_GAP_BT_CONFIG_EIR_DATA_EVT: {
        btc_gap_bt_cb_to_app(YOC_BT_GAP_CONFIG_EIR_DATA_EVT, (yoc_bt_gap_cb_param_t *)msg->arg);
        break;
    }
    case BTC_GAP_BT_AUTH_CMPL_EVT:{
        btc_gap_bt_cb_to_app(YOC_BT_GAP_AUTH_CMPL_EVT, (yoc_bt_gap_cb_param_t *)msg->arg);
        break;
    }
    case BTC_GAP_BT_PIN_REQ_EVT:{
        btc_gap_bt_cb_to_app(YOC_BT_GAP_PIN_REQ_EVT, (yoc_bt_gap_cb_param_t *)msg->arg);
        break;
    }
#if (BT_SSP_INCLUDED == TRUE)
    case BTC_GAP_BT_CFM_REQ_EVT:{
        btc_gap_bt_cb_to_app(YOC_BT_GAP_CFM_REQ_EVT, (yoc_bt_gap_cb_param_t *)msg->arg);
        break;
    }
    case BTC_GAP_BT_KEY_NOTIF_EVT:{
        btc_gap_bt_cb_to_app(YOC_BT_GAP_KEY_NOTIF_EVT, (yoc_bt_gap_cb_param_t *)msg->arg);
        break;
    }
    case BTC_GAP_BT_KEY_REQ_EVT:{
        btc_gap_bt_cb_to_app(YOC_BT_GAP_KEY_REQ_EVT, (yoc_bt_gap_cb_param_t *)msg->arg);
        break;
    }
#endif ///BT_SSP_INCLUDED == TRUE
    default:
