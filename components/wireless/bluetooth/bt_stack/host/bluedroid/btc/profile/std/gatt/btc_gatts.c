...
static yoc_btc_creat_tab_t btc_creat_tab_env;
#else
yoc_btc_creat_tab_t *btc_creat_tab_env_ptr;
#endif

static yoc_gatt_status_t btc_gatts_check_valid_attr_tab(yoc_gatts_attr_db_t *gatts_attr_db,
                                                                          uint8_t max_nb_attr);

static inline void btc_gatts_cb_to_app(yoc_gatts_cb_event_t event, yoc_gatt_if_t gatts_if, yoc_ble_gatts_cb_param_t *param)
{
    yoc_gatts_cb_t btc_gatts_cb = (yoc_gatts_cb_t)btc_profile_cb_get(BTC_PID_GATTS);
    if (btc_gatts_cb) {
        btc_gatts_cb(event, gatts_if, param);
    }
}

static inline void btc_gatts_uuid_format_convert(yoc_bt_uuid_t* dest_uuid, uint16_t src_uuid_len, uint8_t* src_uuid_p)
{
    dest_uuid->len = src_uuid_len;
    if(src_uuid_len == YOC_UUID_LEN_16){
        dest_uuid->uuid.uuid16 = src_uuid_p[0] + (src_uuid_p[1]<<8);
    }
    else if(src_uuid_len == YOC_UUID_LEN_32){
        dest_uuid->uuid.uuid32 = src_uuid_p[0] + (src_uuid_p[1]<<8) + (src_uuid_p[2]<<16) + (src_uuid_p[3]<<24);
    }
    else if(src_uuid_len == YOC_UUID_LEN_128){
...
void btc_gatts_arg_deep_copy(btc_msg_t *msg, void *p_dest, void *p_src)
{
    ...
    case BTC_GATTS_ACT_SEND_RESPONSE: {
        if (src->send_rsp.rsp) {
            dst->send_rsp.rsp = (yoc_gatt_rsp_t *) osi_malloc(sizeof(yoc_gatt_rsp_t));
            if (dst->send_rsp.rsp) {
                memcpy(dst->send_rsp.rsp, src->send_rsp.rsp, sizeof(yoc_gatt_rsp_t));
            } else {
                BTC_TRACE_ERROR("%s %d no mem\n", __func__, msg->act);
            }
        }
    ...
    case BTC_GATTS_ACT_CREATE_ATTR_TAB: {
        ...
        if (src->create_attr_tab.gatts_attr_db && (num_attr > 0)) {
            dst->create_attr_tab.gatts_attr_db = (yoc_gatts_attr_db_t *) osi_malloc(sizeof(yoc_gatts_attr_db_t) * num_attr);
            if (dst->create_attr_tab.gatts_attr_db) {
                memcpy(dst->create_attr_tab.gatts_attr_db, src->create_attr_tab.gatts_attr_db,
                        sizeof(yoc_gatts_attr_db_t) * num_attr);
            } else {
                BTC_TRACE_ERROR("%s %d no mem\n",__func__, msg->act);
            }
        } else {
...
static void btc_gatts_act_create_attr_tab(yoc_gatts_attr_db_t *gatts_attr_db,
                                                        ...
                                                        uint8_t srvc_inst_id)
{
    ...
    param.add_attr_tab.status = YOC_GATT_OK;
    param.add_attr_tab.num_handle = max_nb_attr;

    if (param.add_attr_tab.status != YOC_GATT_OK) {
        btc_gatts_cb_to_app(YOC_GATTS_CREAT_ATTR_TAB_EVT, gatts_if, &param);
        //reset the env after sent the data to app
        memset(&btc_creat_tab_env, 0, sizeof(yoc_btc_creat_tab_t));
        return;
    }

    // Check the attribute table is valid or not
    if ((param.add_attr_tab.status = btc_gatts_check_valid_attr_tab(gatts_attr_db, max_nb_attr)) != YOC_GATT_OK) {
    ...
    for(int i = 0; i < max_nb_attr; i++){
        if(gatts_attr_db[i].att_desc.uuid_length == YOC_UUID_LEN_16){
        ...
        switch(uuid)
        {
            case YOC_GATT_UUID_PRI_SERVICE:{
                tBTA_GATT_SRVC_ID srvc_id;
                yoc_gatt_srvc_id_t        yoc_srvc_id;

                yoc_srvc_id.id.inst_id = srvc_inst_id;
                btc_gatts_uuid_format_convert(&yoc_srvc_id.id.uuid,gatts_attr_db[i].att_desc.length,
                                              gatts_attr_db[i].att_desc.value);

                btc_to_bta_srvc_id(&srvc_id, &yoc_srvc_id);
                if (btc_creat_tab_env.is_use_svc != true) {
                    ...
                    param.add_attr_tab.status = YOC_GATT_ERROR;
                    btc_gatts_cb_to_app(YOC_GATTS_CREAT_ATTR_TAB_EVT, gatts_if, &param);
                    //reset the env after sent the data to app
                    memset(&btc_creat_tab_env, 0, sizeof(yoc_btc_creat_tab_t));
                    return;
            ...
            case YOC_GATT_UUID_SEC_SERVICE:{
                tBTA_GATT_SRVC_ID srvc_id;
                yoc_gatt_srvc_id_t        yoc_srvc_id;
            ...
            case YOC_GATT_UUID_INCLUDE_SERVICE:{
                yoc_gatts_incl_svc_desc_t *incl_svc_desc = (yoc_gatts_incl_svc_desc_t *)gatts_attr_db[i].att_desc.value;
            ...
            case YOC_GATT_UUID_CHAR_DECLARE:{
                uint16_t svc_hal = 0;
                tBT_UUID bta_char_uuid;
                tGATT_ATTR_VAL attr_val;
                yoc_bt_uuid_t uuid_temp;
                tBTA_GATT_PERM perm;
            ...
            case YOC_GATT_UUID_CHAR_EXT_PROP:
            case YOC_GATT_UUID_CHAR_DESCRIPTION:
            case YOC_GATT_UUID_CHAR_CLIENT_CONFIG:
            case YOC_GATT_UUID_CHAR_SRVR_CONFIG:
            case YOC_GATT_UUID_CHAR_PRESENT_FORMAT:
            case YOC_GATT_UUID_CHAR_AGG_FORMAT:
            case YOC_GATT_UUID_CHAR_VALID_RANGE:
            case YOC_GATT_UUID_EXT_RPT_REF_DESCR:
            case YOC_GATT_UUID_RPT_REF_DESCR:{
    ...
    memcpy(&param.add_attr_tab.svc_uuid, &btc_creat_tab_env.svc_uuid, sizeof(yoc_bt_uuid_t));
        ...
        switch(uuid) {
            ...
            case YOC_GATT_UUID_SEC_SERVICE:
                if (++svc_num > 1) {
                    ...
                    return YOC_GATT_ERROR;
                }
                break;
            case YOC_GATT_UUID_INCLUDE_SERVICE: {
                yoc_gatts_incl_svc_desc_t *svc_desc = (yoc_gatts_incl_svc_desc_t *)gatts_attr_db[i].att_desc.value;
                if(svc_desc == NULL) {
            ...
            case YOC_GATT_UUID_CHAR_DECLARE:
                ...
                if(gatts_attr_db[i+1].att_desc.uuid_length != YOC_UUID_LEN_16 &&
                   gatts_attr_db[i+1].att_desc.uuid_length != YOC_UUID_LEN_32 &&
                   gatts_attr_db[i+1].att_desc.uuid_length != YOC_UUID_LEN_128) {
                    BTC_TRACE_ERROR("%s, The Charateristic uuid length = %d is invalid", __func__,\
                        gatts_attr_db[i+1].att_desc.uuid_length);
                    return YOC_GATT_INVALID_ATTR_LEN;
                }

                if(gatts_attr_db[i+1].att_desc.uuid_length == YOC_UUID_LEN_16) {
                    uuid = (gatts_attr_db[i+1].att_desc.uuid_p[1] << 8) + (gatts_attr_db[i+1].att_desc.uuid_p[0]);
                    if(uuid == YOC_GATT_UUID_CHAR_DECLARE || uuid == YOC_GATT_UUID_CHAR_EXT_PROP ||
                        uuid == YOC_GATT_UUID_CHAR_DESCRIPTION || uuid == YOC_GATT_UUID_CHAR_CLIENT_CONFIG ||
                        uuid == YOC_GATT_UUID_CHAR_SRVR_CONFIG || uuid == YOC_GATT_UUID_CHAR_PRESENT_FORMAT ||
                        uuid == YOC_GATT_UUID_CHAR_AGG_FORMAT || uuid == YOC_GATT_UUID_CHAR_VALID_RANGE ||
                        uuid == YOC_GATT_UUID_EXT_RPT_REF_DESCR || uuid == YOC_GATT_UUID_RPT_REF_DESCR) {
...
void btc_gatts_call_handler(btc_msg_t *msg)
{
    ...
    BTC_TRACE_DEBUG("%s, %d", __func__, msg->act);
   ...
   case BTC_GATTS_ACT_CREATE_ATTR_TAB:
    ...
    case BTC_GATTS_ACT_SEND_RESPONSE: {
        yoc_ble_gatts_cb_param_t param;
        yoc_gatt_rsp_t *p_rsp = arg->send_rsp.rsp;
        ...
        param.rsp.status = 0;
        btc_gatts_cb_to_app(YOC_GATTS_RESPONSE_EVT, BTC_GATT_GET_GATT_IF(arg->send_rsp.conn_id), &param);
        break;
    }
    case BTC_GATTS_ACT_SET_ATTR_VALUE:
...
void btc_gatts_cb_handler(btc_msg_t *msg)
{
    ...
    case BTA_GATTS_REG_EVT: {
        ...
        btc_gatts_cb_to_app(YOC_GATTS_REG_EVT, gatts_if, &param);
        break;
    }
    case BTA_GATTS_DEREG_EVT: {
        gatts_if = p_data->reg_oper.server_if;
        btc_gatts_cb_to_app(YOC_GATTS_UNREG_EVT, gatts_if, NULL);
        break;
    }
    case BTA_GATTS_READ_EVT: {
        ...
        memcpy(param.read.bda, p_data->req_data.remote_bda, BT_BD_ADDR_LEN);
        ...
        btc_gatts_cb_to_app(YOC_GATTS_READ_EVT, gatts_if, &param);
        break;
    }
    case BTA_GATTS_WRITE_EVT: {
        gatts_if = BTC_GATT_GET_GATT_IF(p_data->req_data.conn_id);
        ...
        memcpy(param.write.bda, p_data->req_data.remote_bda, BT_BD_ADDR_LEN);
        ...
        btc_gatts_cb_to_app(YOC_GATTS_WRITE_EVT, gatts_if, &param);
    ...
    case BTA_GATTS_EXEC_WRITE_EVT: {
        ...
        memcpy(param.exec_write.bda, p_data->req_data.remote_bda, BT_BD_ADDR_LEN);
        if (p_data->req_data.p_data == NULL) {
            break;
        }
        param.exec_write.exec_write_flag = p_data->req_data.p_data->exec_write;

        btc_gatts_cb_to_app(YOC_GATTS_EXEC_WRITE_EVT, gatts_if, &param);
        break;
    }
    case BTA_GATTS_MTU_EVT:
        gatts_if = BTC_GATT_GET_GATT_IF(p_data->req_data.conn_id);
        ...
        btc_gatts_cb_to_app(YOC_GATTS_MTU_EVT, gatts_if, &param);
        break;
    case BTA_GATTS_CONF_EVT:
        gatts_if = BTC_GATT_GET_GATT_IF(p_data->req_data.conn_id);
        ...
        if (p_data->req_data.status != YOC_GATT_OK && p_data->req_data.value){
            param.conf.len = p_data->req_data.data_len;
            param.conf.value = p_data->req_data.value;
        }else{
            param.conf.len = 0;
        }
        btc_gatts_cb_to_app(YOC_GATTS_CONF_EVT, gatts_if, &param);
        break;
    case BTA_GATTS_CREATE_EVT:
        ...
        btc_gatts_cb_to_app(YOC_GATTS_CREATE_EVT, gatts_if, &param);
        break;
    case BTA_GATTS_ADD_INCL_SRVC_EVT:
        ...
        btc_gatts_cb_to_app(YOC_GATTS_ADD_INCL_SRVC_EVT, gatts_if, &param);
        break;
    case BTA_GATTS_ADD_CHAR_EVT:
        gatts_if = p_data->add_result.server_if;
        ...
        btc_gatts_cb_to_app(YOC_GATTS_ADD_CHAR_EVT, gatts_if, &param);
        break;
    case BTA_GATTS_ADD_CHAR_DESCR_EVT:
        gatts_if = p_data->add_result.server_if;
        ...
        btc_gatts_cb_to_app(YOC_GATTS_ADD_CHAR_DESCR_EVT, gatts_if, &param);
        break;
    case BTA_GATTS_DELELTE_EVT:
        ...
        btc_gatts_cb_to_app(YOC_GATTS_DELETE_EVT, gatts_if, &param);
        break;
    case BTA_GATTS_START_EVT:
        ...
        btc_gatts_cb_to_app(YOC_GATTS_START_EVT, gatts_if, &param);
        break;
    case BTA_GATTS_STOP_EVT:
        ...
        btc_gatts_cb_to_app(YOC_GATTS_STOP_EVT, gatts_if, &param);
        break;
    case BTA_GATTS_CONNECT_EVT:
        gatts_if = p_data->conn.server_if;
        param.connect.conn_id = BTC_GATT_GET_CONN_ID(p_data->conn.conn_id);
        memcpy(param.connect.remote_bda, p_data->conn.remote_bda, BT_BD_ADDR_LEN);
        ...
        btc_gatts_cb_to_app(YOC_GATTS_CONNECT_EVT, gatts_if, &param);
        break;
    case BTA_GATTS_DISCONNECT_EVT:
        gatts_if = p_data->conn.server_if;
        param.disconnect.conn_id = BTC_GATT_GET_CONN_ID(p_data->conn.conn_id);
        param.disconnect.reason = p_data->conn.reason;
        memcpy(param.disconnect.remote_bda, p_data->conn.remote_bda, BT_BD_ADDR_LEN);

        btc_gatts_cb_to_app(YOC_GATTS_DISCONNECT_EVT, gatts_if, &param);
        break;
    ...
    case BTA_GATTS_SEND_SERVICE_CHANGE_EVT:
        ...
        btc_gatts_cb_to_app(YOC_GATTS_SEND_SERVICE_CHANGE_EVT, gatts_if, &param);
        break;
    ...
    case BTA_GATTS_CONGEST_EVT:
        ...
        btc_gatts_cb_to_app(YOC_GATTS_CONGEST_EVT, gatts_if, &param);
        break;
    case BTA_GATTS_SET_ATTR_VAL_EVT:
        ...
        btc_gatts_cb_to_app(YOC_GATTS_SET_ATTR_VAL_EVT, gatts_if, &param);
        break;
    default:
        // do nothing
        break;
