...
static void bta_hf_client_at_ryoc_timer_cback (TIMER_LIST_ENT *p_tle)
{
    if (p_tle) {
        bta_hf_client_cb.scb.at_cb.ryoc_timer_on = FALSE;
...
static void bta_hf_client_stop_at_ryoc_timer(void)
{
    if (bta_hf_client_cb.scb.at_cb.ryoc_timer_on) {
        bta_hf_client_cb.scb.at_cb.ryoc_timer_on = FALSE;
        bta_sys_stop_timer (&bta_hf_client_cb.scb.at_cb.ryoc_timer);
    }
}

static void bta_hf_client_free_at_ryoc_timer(void)
{
    bta_hf_client_cb.scb.at_cb.ryoc_timer_on = FALSE;
    bta_sys_free_timer (&bta_hf_client_cb.scb.at_cb.ryoc_timer);
}

static void bta_hf_client_start_at_ryoc_timer(void)
{
    if (bta_hf_client_cb.scb.at_cb.ryoc_timer_on) {
        bta_sys_stop_timer (&bta_hf_client_cb.scb.at_cb.ryoc_timer);
    }

    bta_hf_client_cb.scb.at_cb.ryoc_timer.p_cback = (TIMER_CBACK *)&bta_hf_client_at_ryoc_timer_cback;
    bta_sys_start_timer(&bta_hf_client_cb.scb.at_cb.ryoc_timer, 0, BTA_HF_CLIENT_AT_TIMEOUT);
    bta_hf_client_cb.scb.at_cb.ryoc_timer_on = TRUE;
}
...
static void bta_hf_client_send_at(tBTA_HF_CLIENT_AT_CMD cmd, char *buf, UINT16 buf_len)
{
    if ((bta_hf_client_cb.scb.at_cb.current_cmd == BTA_HF_CLIENT_AT_NONE ||
        ...
        bta_hf_client_start_at_ryoc_timer();
...
static void bta_hf_client_handle_ok(void)
{
    ...
    bta_hf_client_stop_at_ryoc_timer();
...
void bta_hf_client_at_reset(void)
{
    ...
    bta_hf_client_free_at_ryoc_timer();
