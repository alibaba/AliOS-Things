...
bool display_cache_service(void *data, void *context)
{
    ...
    if (p_cur_srvc->characteristics != NULL) {
        osi_list_foreach(p_cur_srvc->characteristics, display_cache_attribute, NULL);
    }
...
{
    APPL_TRACE_DEBUG("<================Start Server Cache =============>");
    osi_list_foreach(p_cache, display_cache_service, NULL);
    APPL_TRACE_DEBUG("<================End Server Cache =============>");
    APPL_TRACE_DEBUG(" ");
}
...
static void bta_gattc_display_explore_record(tBTA_GATTC_ATTR_REC *p_rec, UINT8 num_rec)
{
    ...
    APPL_TRACE_DEBUG("<================Start Explore Queue =============>");
    for (i = 0; i < num_rec; i ++, pp ++) {
        APPL_TRACE_DEBUG("\t rec[%d] uuid[0x%04x] s_handle[%d] e_handle[%d] is_primary[%d]",
                         i + 1, pp->uuid.uu.uuid16, pp->s_handle, pp->e_handle, pp->is_primary);
    }
    APPL_TRACE_DEBUG("<================ End Explore Queue =============>");
    APPL_TRACE_DEBUG(" ");
...
tBTA_GATT_STATUS bta_gattc_init_cache(tBTA_GATTC_SERV *p_srvc_cb)
{
    if (p_srvc_cb->p_srvc_cache != NULL) {
        osi_list_free(p_srvc_cb->p_srvc_cache);
...
static void characteristic_free(void *ptr)
{
  ...
  osi_list_free(p_char->descriptors);
  osi_free(p_char);
}
...
static void service_free(void *ptr)
{
  ...
  osi_list_free(srvc->characteristics);
  osi_list_free(srvc->included_svc);
  osi_free(srvc);
}
...
void bta_gattc_insert_sec_service_to_cache(list_t *services, tBTA_GATTC_SERVICE *p_new_srvc)
{
    ...
    //list is empty
    if (osi_list_is_empty(services)) {
        osi_list_append(services, p_new_srvc);
    } else {
        //check the first service
        list_node_t *sn = osi_list_begin(services);
                                                    ...
                                                    BOOLEAN is_primary)
{
    ...
    p_new_srvc->characteristics = osi_list_new(characteristic_free);
    p_new_srvc->included_svc = osi_list_new(bta_gattc_free);

    if (p_srvc_cb->p_srvc_cache == NULL) {
        p_srvc_cb->p_srvc_cache = osi_list_new(service_free);
    }

    if(is_primary) {
        osi_list_append(p_srvc_cb->p_srvc_cache, p_new_srvc);
    } else {
                                                    ...
                                                    UINT8 property)
{
    ...
    */
    ...
    characteristic->descriptors = osi_list_new(bta_gattc_free);
    memcpy(&characteristic->uuid, p_uuid, sizeof(tBT_UUID));

    characteristic->service = service;
    osi_list_append(service->characteristics, characteristic);
                                                    ...
                                                    tBTA_GATTC_ATTR_TYPE type)
{
    ...
    if (type == BTA_GATTC_ATTR_TYPE_INCL_SRVC) {
        ...
        osi_list_append(service->included_svc, isvc);
        ...
        if (service->characteristics == NULL ||
            osi_list_is_empty(service->characteristics)) {
        ...
        tBTA_GATTC_CHARACTERISTIC *char_node = osi_list_back(service->characteristics);

        descriptor->characteristic = char_node;
        osi_list_append(char_node->descriptors, descriptor);
    }
    return BTA_GATT_OK;
}
...
void bta_gattc_update_include_service(const list_t *services) {
    if (!services || osi_list_is_empty(services)) {
        return;
    }
    for (list_node_t *sn = osi_list_begin(services); sn != osi_list_end(services); sn = osi_list_next(sn)) {
        tBTA_GATTC_SERVICE *service = osi_list_node(sn);
        if(!service && osi_list_is_empty(service->included_svc)) break;
        for (list_node_t *sn = osi_list_begin(service->included_svc); sn != osi_list_end(service->included_svc); sn = osi_list_next(sn)) {
            ...
            if(include_service && !include_service->included_service) {
                ...
                if(!include_service->included_service) {
                    //not match, free it
                    osi_list_remove(service->included_svc, include_service);
...
static void bta_gattc_char_dscpt_disc_cmpl(UINT16 conn_id, tBTA_GATTC_SERV *p_srvc_cb)
{
    ...
    {
#if (defined BTA_GATT_DEBUG && BTA_GATT_DEBUG == TRUE)
        APPL_TRACE_DEBUG("all char has been explored");
#endif
        p_srvc_cb->cur_srvc_idx ++;
        bta_gattc_explore_srvc (conn_id, p_srvc_cb);
    }
...
void bta_gattc_search_service(tBTA_GATTC_CLCB *p_clcb, tBT_UUID *p_uuid)
{
    ...
    if (!p_clcb->p_srcb->p_srvc_cache || osi_list_is_empty(p_clcb->p_srcb->p_srvc_cache)) {
        return;
    }

    for (list_node_t *sn = osi_list_begin(p_clcb->p_srcb->p_srvc_cache);
...
list_t* bta_gattc_get_services_srcb(tBTA_GATTC_SERV *p_srcb) {
    if (!p_srcb || !p_srcb->p_srvc_cache || osi_list_is_empty(p_srcb->p_srvc_cache)) {
        return NULL;
    }
...
tBTA_GATTC_SERVICE*  bta_gattc_find_matching_service(const list_t *services, UINT16 handle) {
    ...
    for (list_node_t *sn = osi_list_begin(services);
         sn != osi_list_end(services); sn = osi_list_next(sn)) {
        tBTA_GATTC_SERVICE *service = osi_list_node(sn);
...
tBTA_GATTC_CHARACTERISTIC*  bta_gattc_get_characteristic_srcb(tBTA_GATTC_SERV *p_srcb, UINT16 handle)
{
    ...
    for (list_node_t *cn = osi_list_begin(service->characteristics);
         cn != osi_list_end(service->characteristics); cn = osi_list_next(cn)) {
        tBTA_GATTC_CHARACTERISTIC *p_char = osi_list_node(cn);
                                     ...
                                     int *count)
{
    ...
    size_t db_size = osi_list_length(svc);
                                                      ...
                                                      int *count)
{
    ...
    tBTA_GATTC_SERV *p_srcb = p_clcb->p_srcb;
    if (!p_srcb->p_srvc_cache || osi_list_is_empty(p_srcb->p_srvc_cache)) {
    ...
    btgatt_db_element_t *curr_db_attr = buffer;
    db_size = 0;
    for (list_node_t *sn = osi_list_begin(p_srcb->p_srvc_cache);
         sn != osi_list_end(p_srcb->p_srvc_cache); sn = osi_list_next(sn)) {
        tBTA_GATTC_SERVICE *p_cur_srvc = osi_list_node(sn);
        ...
        if (op == GATT_OP_GET_INCLUDE_SVC) {
            if (!p_cur_srvc->included_svc || osi_list_is_empty(p_cur_srvc->included_svc))
                continue;

            for (list_node_t *isn = osi_list_begin(p_cur_srvc->included_svc);
                 isn != osi_list_end(p_cur_srvc->included_svc); isn = osi_list_next(isn)) {
                tBTA_GATTC_INCLUDED_SVC *p_isvc = osi_list_node(isn);
        ...
        if (!p_cur_srvc->characteristics || osi_list_is_empty(p_cur_srvc->characteristics)) {
            continue;
        }

        for (list_node_t *cn = osi_list_begin(p_cur_srvc->characteristics);
             cn != osi_list_end(p_cur_srvc->characteristics); cn = osi_list_next(cn)) {
            tBTA_GATTC_CHARACTERISTIC *p_char = osi_list_node(cn);
            ...
            if (!p_char->descriptors || osi_list_is_empty(p_char->descriptors)) {
                continue;
            }
                                              ...
                                              UINT16 start_handle, UINT16 end_handle)
{
    ...
    size_t db_size = 0;
    UINT16 svc_length = osi_list_length(services) - 1;

    for (list_node_t *sn = osi_list_begin(services);
         sn != osi_list_end(services); sn = osi_list_next(sn), svc_length--) {
        tBTA_GATTC_SERVICE *p_cur_srvc = osi_list_node(sn);
                                            ...
                                            UINT16 start_handle, UINT16 end_handle, UINT16 char_handle, int *count)
{
    ...
    if (type == BTGATT_DB_DESCRIPTOR) {
        if (char_handle == BTA_GATTC_INVALID_HANDLE) {
            ...
            *count = char_db ? osi_list_length(char_db->descriptors) : 0;
            return;
        }
    }
                                       ...
                                       int *count)
{
    ...
    if (!p_srvc_cb->p_srvc_cache || osi_list_is_empty(p_srvc_cb->p_srvc_cache)) {
        *count = 0;
        *db = NULL;
        return;
    }
    ...
    for (list_node_t *sn = osi_list_begin(p_srvc_cb->p_srvc_cache);
         sn != osi_list_end(p_srvc_cb->p_srvc_cache); sn = osi_list_next(sn)) {
        tBTA_GATTC_SERVICE *p_cur_srvc = osi_list_node(sn);
...
void bta_gattc_cache_save(tBTA_GATTC_SERV *p_srvc_cb, UINT16 conn_id)
{
    ...
    for (list_node_t *sn = osi_list_begin(p_srvc_cb->p_srvc_cache);
        ...
        for (list_node_t *an = osi_list_begin(p_cur_srvc->included_svc);
             an != osi_list_end(p_cur_srvc->included_svc); an = osi_list_next(an)) {
            tBTA_GATTC_INCLUDED_SVC *p_isvc = osi_list_node(an);
