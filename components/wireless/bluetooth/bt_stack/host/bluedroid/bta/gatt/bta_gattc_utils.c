    ...
    for (i_clcb = 0; i_clcb < BTA_GATTC_CLCB_MAX; i_clcb++) {
        if (!bta_gattc_cb.clcb[i_clcb].in_use) {
            ...
            if (p_clcb->p_cmd_list == NULL) {
                p_clcb->p_cmd_list = osi_list_new(osi_free_func);
            }
    ...
    if (p_clcb) {
        ...
        if ( p_srcb->num_clcb == 0) {
            ...
            if (p_srcb->p_srvc_cache) {
                osi_list_free(p_srcb->p_srvc_cache);
        ...
        osi_list_clear(p_clcb->p_cmd_list);
...
tBTA_GATTC_SERV *bta_gattc_srcb_alloc(BD_ADDR bda)
{
    ...
    if (p_tcb != NULL)
    {
        if (p_tcb->p_srvc_cache != NULL) {
            osi_list_free(p_tcb->p_srvc_cache);
        }
...
static BOOLEAN bta_gattc_has_prepare_command_in_queue(tBTA_GATTC_CLCB *p_clcb)
{
    ...
    for(list_node_t *sn = osi_list_begin(p_clcb->p_cmd_list);
        sn != osi_list_end(p_clcb->p_cmd_list); sn = osi_list_next(sn)) {

        tBTA_GATTC_DATA *cmd_data = (tBTA_GATTC_DATA *)osi_list_node(sn);
...
BOOLEAN bta_gattc_enqueue(tBTA_GATTC_CLCB *p_clcb, tBTA_GATTC_DATA *p_data)
{
    ...
    if (p_clcb->p_q_cmd == NULL) {
        ...
        if (osi_list_length(p_clcb->p_cmd_list) >= GATTC_COMMAND_QUEUE_SIZE_MAX) {
        ...
        osi_list_append(p_clcb->p_cmd_list, (void *)cmd_data);
        return FALSE;
    }
