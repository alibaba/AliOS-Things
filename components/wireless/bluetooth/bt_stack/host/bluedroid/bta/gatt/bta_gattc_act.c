...
void bta_gattc_disc_cmpl(tBTA_GATTC_CLCB *p_clcb, tBTA_GATTC_DATA *p_data)
{
    ...
    if (p_clcb->status != GATT_SUCCESS) {
        ...
        if (p_clcb->p_srcb && p_clcb->p_srcb->p_srvc_cache) {
            osi_list_free(p_clcb->p_srcb->p_srvc_cache);
...
{
    if (!bta_gattc_enqueue(p_clcb, p_data)) {
        APPL_TRACE_DEBUG("%s, bta_gattc_enqueue failed", __func__);
        return;
    }
    ...
    /* write fail */
    if (status != BTA_GATT_OK) {
        APPL_TRACE_ERROR("%s, GATTC_Write failed %d", __func__, status);
        /* Dequeue the data, if it was enqueued */
        if (p_clcb->p_q_cmd == p_data) {
            p_clcb->p_q_cmd = NULL;
            bta_gattc_pop_command_to_send(p_clcb);
...
static void bta_gattc_pop_command_to_send(tBTA_GATTC_CLCB *p_clcb)
{
    if (!osi_list_is_empty(p_clcb->p_cmd_list)) {
        list_node_t *node = osi_list_begin(p_clcb->p_cmd_list);
        tBTA_GATTC_DATA *p_data = (tBTA_GATTC_DATA *)osi_list_node(node);
        if (p_data != NULL) {
            /* execute pending operation of link block still present */
            if (l2cu_find_lcb_by_bd_addr(p_clcb->p_srcb->server_bda, BT_TRANSPORT_LE) != NULL) {
                // The data to be sent to the gattc state machine for processing
                if(bta_gattc_sm_execute(p_clcb, p_data->hdr.event, p_data)) {
                    osi_list_remove(p_clcb->p_cmd_list, (void *)p_data);
                }
...
void bta_gattc_free_command_data(tBTA_GATTC_CLCB *p_clcb)
{
    ...
    if (!osi_list_is_empty(p_clcb->p_cmd_list)) {
        ...
        for (list_node_t *node = osi_list_begin(p_clcb->p_cmd_list); node != osi_list_end(p_clcb->p_cmd_list);
