        ...
        for (xx = 0; xx < BTA_AV_NUM_STRS; xx++) {
            if (bta_av_cb.p_scb[xx] == NULL) {
                ...
                if (p_ret) {
                    ...
                    p_ret->hdi  = xx;
                    p_ret->a2d_list = osi_list_new(NULL);
...
BOOLEAN bta_av_link_role_ok(tBTA_AV_SCB *p_scb, UINT8 bits)
{
    ...
    if (BTM_GetRole(p_scb->peer_addr, &role) == BTM_SUCCESS) {
        BT_LOG_INFO("%s hndl:x%x role:%d conn_audio:x%x bits:%d features:x%x\n",
        ...
        for (i = 0; i < BTA_AV_NUM_STRS; i++) {
            ...
            if ( (p_scb->hdi != i) && /* not the original channel */
                ...
                if (p_new) {
                    ...
                    osi_list_append(p_scbi->a2d_list, p_new);
                    if (osi_list_length(p_scbi->a2d_list) >  p_bta_av_cfg->audio_mqs) {
                        // Drop the oldest packet
                        bta_av_co_audio_drop(p_scbi->hndl);
                        BT_HDR *p_buf = osi_list_front(p_scbi->a2d_list);
                        osi_list_remove(p_scbi->a2d_list, p_buf);
                        osi_free(p_buf);
                    }
                }
            }
