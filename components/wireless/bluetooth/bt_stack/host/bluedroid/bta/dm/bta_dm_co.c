...
**
** @return          - YOC_BT_STATUS_SUCCESS : success
**                  - other  : failed
**
*******************************************************************************/
bt_err_t bta_dm_co_bt_set_io_cap(UINT8 bt_io_cap)
{
    bt_err_t ret = YOC_BT_STATUS_SUCCESS;
#if (BT_SSP_INCLUDED == TRUE)
    if(bt_io_cap < BTM_IO_CAP_MAX ) {
        bte_bt_appl_cfg.bt_io_cap = bt_io_cap;
        btm_cb.devcb.loc_io_caps = bt_io_cap;
        ret = YOC_BT_STATUS_SUCCESS;
    } else {
                          ...
                          tBTA_LE_KEY_TYPE  *p_ryoc_key )
{
    ...
    *p_auth_req = bte_appl_cfg.ble_auth_req | (bte_appl_cfg.ble_auth_req & BTA_LE_AUTH_REQ_MITM) | ((*p_auth_req) & BTA_LE_AUTH_REQ_MITM);
    ...
    if (bte_appl_cfg.ble_ryoc_key <= BTM_BLE_RESPONDER_KEY_SIZE) {
        *p_ryoc_key = bte_appl_cfg.ble_ryoc_key;
    }
...
void bta_dm_co_ble_set_rsp_key_req(UINT8 rsp_key)
{
   ...
   bte_appl_cfg.ble_ryoc_key &= rsp_key;
#endif  ///SMP_INCLUDED == TRUE
}
