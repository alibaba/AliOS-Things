...
void avdt_scb_hdl_write_req_frag(tAVDT_SCB *p_scb, tAVDT_SCB_EVT *p_data)
{
    ...
    if (! fixed_queue_is_empty(p_scb->frag_q)) {
        ...
        const list_node_t *node = osi_list_begin(list);
        if (node != osi_list_end(list)) {
            BT_HDR *p_frag = (BT_HDR *)osi_list_node(node);
        ...
        for ( ; node != osi_list_end(list); node = osi_list_next(node)) {
            BT_HDR *p_frag = (BT_HDR *)osi_list_node(node);
