...
void gatt_set_srv_chg(void)
{
    ...
    for (const list_node_t *node = osi_list_begin(list); node != osi_list_end(list);
         node = osi_list_next(node)) {
        GATT_TRACE_DEBUG ("found a srv_chg clt");

        tGATTS_SRV_CHG *p_buf = (tGATTS_SRV_CHG *)osi_list_node(node);
...
tGATTS_PENDING_NEW_SRV_START *gatt_sr_is_new_srv_chg(tBT_UUID *p_app_uuid128, tBT_UUID *p_svc_uuid, UINT16 svc_inst)
{
    ...
    for (const list_node_t *node = osi_list_begin(list); node != osi_list_end(list);
         node = osi_list_next(node)) {
        p_buf = (tGATTS_PENDING_NEW_SRV_START *)osi_list_node(node);
...
BOOLEAN gatt_is_srv_chg_ind_pending (tGATT_TCB *p_tcb)
{
    ...
    if (p_tcb->indicate_handle == gatt_cb.handle_of_h_r) {
        ...
        for (const list_node_t *node = osi_list_begin(list);
             node != osi_list_end(list);
             node = osi_list_next(node)) {
            tGATT_VALUE *p_buf = (tGATT_VALUE *)osi_list_node(node);
...
tGATTS_SRV_CHG *gatt_is_bda_in_the_srv_chg_clt_list (BD_ADDR bda)
{
    ...
    for (const list_node_t *node = osi_list_begin(list); node != osi_list_end(list);
         node = osi_list_next(node)) {
        p_buf = (tGATTS_SRV_CHG *)osi_list_node(node);
