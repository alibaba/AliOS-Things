...
static BOOLEAN retransmit_i_frames (tL2C_CCB *p_ccb, UINT8 tx_seq)
{
    ...
    if (! fixed_queue_is_empty(p_ccb->fcrb.waiting_for_ack_q)) {
        ...
        node_ack = osi_list_begin(list_ack);
    }
    if (tx_seq != L2C_FCR_RETX_ALL_PKTS) {
        /* If sending only one, the sequence number tells us which one. Look for it.
        */
        if (list_ack != NULL) {
            for ( ; node_ack != osi_list_end(list_ack); node_ack = osi_list_next(node_ack)) {
                p_buf = (BT_HDR *)osi_list_node(node_ack);
    ...
    if (list_ack != NULL) {
        while (node_ack != osi_list_end(list_ack))
        {
            p_buf = (BT_HDR *)osi_list_node(node_ack);
            node_ack = osi_list_next(node_ack);
...
static void l2c_fcr_collect_ack_delay (tL2C_CCB *p_ccb, UINT8 num_bufs_acked)
{
    ...
    if (list != NULL) {
        for (const list_node_t *node = osi_list_begin(list), xx = 0;
             (node != osi_list_end(list)) && (xx < num_bufs_acked);
             node = osi_list_next(node), xx++) {
            p_buf = osi_list_node(node);
