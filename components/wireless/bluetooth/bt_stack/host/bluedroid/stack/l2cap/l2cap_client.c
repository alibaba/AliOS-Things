...
l2cap_client_t *l2cap_client_new(const l2cap_client_callbacks_t *callbacks, void *context)
{
    ...
    if (!l2cap_clients) {
        l2cap_clients = osi_list_new(NULL);
        if (!l2cap_clients) {
...
void l2cap_client_free(l2cap_client_t *client)
{
    ...
    osi_list_remove(l2cap_clients, client);
    l2cap_client_disconnect(client);
    osi_list_free(client->outbound_fragments);
    osi_free(client);
}
...
void l2cap_client_disconnect(l2cap_client_t *client)
{
    ...
    for (const list_node_t *node = osi_list_begin(client->outbound_fragments); node != osi_list_end(client->outbound_fragments); node = osi_list_next(node)) {
        osi_free(osi_list_node(node));
    }

    osi_list_clear(client->outbound_fragments);
}
...
static void fragment_packet(l2cap_client_t *client, buffer_t *packet)
{
    ...
    for (;;) {
        if (bt_packet->len <= client->remote_mtu) {
            if (bt_packet->len > 0) {
                osi_list_append(client->outbound_fragments, bt_packet);
            } else {
                osi_free(bt_packet);
            }
            break;
        ...
        osi_list_append(client->outbound_fragments, fragment);
...
static void dispatch_fragments(l2cap_client_t *client)
{
    ...
    while (!osi_list_is_empty(client->outbound_fragments)) {
        BT_HDR *packet = (BT_HDR *)osi_list_front(client->outbound_fragments);
        osi_list_remove(client->outbound_fragments, packet);
...
static l2cap_client_t *find(uint16_t local_channel_id)
{
    ...
    for (const list_node_t *node = osi_list_begin(l2cap_clients); node != osi_list_end(l2cap_clients); node = osi_list_next(node)) {
        l2cap_client_t *client = (l2cap_client_t *)osi_list_node(node);
