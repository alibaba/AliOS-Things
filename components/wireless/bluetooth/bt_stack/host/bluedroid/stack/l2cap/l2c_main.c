        ...
        if ((p_lcb = l2cu_find_lcb_by_handle (handle)) == NULL) {
            ...
            if ((p_msg->layer_specific == 0) && (rcv_cid == L2CAP_SIGNALLING_CID)
                ...
                L2CAP_TRACE_WARNING ("L2CAP - holding ACL for unknown handle:%d ls:%d"
                                     ...
                                     rcv_cid, cmd_code, osi_list_length(l2cb.rcv_pending_q));
                p_msg->layer_specific = 2;
                osi_list_append(l2cb.rcv_pending_q, p_msg);

                if (osi_list_length(l2cb.rcv_pending_q) == 1) {
                ...
                L2CAP_TRACE_ERROR ("L2CAP - rcvd ACL for unknown handle:%d ls:%d cid:%d"
                                   ...
                                   cmd_code, osi_list_length(l2cb.rcv_pending_q));
            }
            osi_free (p_msg);
            return;
        }
    ...
    while (TRUE) {
        ...
        case L2CAP_CMD_INFO_RSP:
            ...
            if ( (info_type == L2CAP_EXTENDED_FEATURES_INFO_TYPE)
                    && (result == L2CAP_INFO_RYOC_RESULT_SUCCESS) ) {
            ...
            if (info_type == L2CAP_FIXED_CHANNELS_INFO_TYPE) {
                if (result == L2CAP_INFO_RYOC_RESULT_SUCCESS) {
...
{
    if (osi_list_is_empty(l2cb.rcv_pending_q)) {
        return;
    }
    ...
    for (const list_node_t *node = osi_list_begin(l2cb.rcv_pending_q);
            node != osi_list_end(l2cb.rcv_pending_q);)  {
        BT_HDR *p_buf = osi_list_node(node);
        node = osi_list_next(node);
        if (!timed_out || (!p_buf->layer_specific) || (--p_buf->layer_specific == 0)) {
            osi_list_remove(l2cb.rcv_pending_q, p_buf);
...
void l2c_init (void)
{
    ...
    l2cb.rcv_pending_q = osi_list_new(NULL);
...
void l2c_free(void)
{
    osi_list_free(l2cb.rcv_pending_q);
