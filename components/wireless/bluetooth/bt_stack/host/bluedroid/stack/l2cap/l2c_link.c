        ...
        if (p_lcb->ccb_queue.p_first_ccb != NULL || p_lcb->p_pending_ccb) {
            ...
            if (p_lcb->transport == BT_TRANSPORT_LE) {
                ...
                while (!osi_list_is_empty(p_lcb->link_xmit_data_q)) {
                    p_buf = osi_list_front(p_lcb->link_xmit_data_q);
...
void l2c_link_adjust_allocation (void)
{
    ...
    for (yy = 0, p_lcb = &l2cb.lcb_pool[0]; yy < MAX_L2CAP_LINKS; yy++, p_lcb++) {
        if (p_lcb->in_use) {
            ...
            if ( (p_lcb->link_state == LST_CONNECTED)
                    && (!osi_list_is_empty(p_lcb->link_xmit_data_q))
...
BOOLEAN l2c_link_check_power_mode (tL2C_LCB *p_lcb)
{
     ...
     */
    if (osi_list_is_empty(p_lcb->link_xmit_data_q)) {
        for (p_ccb = p_lcb->ccb_queue.p_first_ccb; p_ccb; p_ccb = p_ccb->p_next_ccb) {
...
void l2c_link_check_send_pkts (tL2C_LCB *p_lcb, tL2C_CCB *p_ccb, BT_HDR *p_buf)
{
    ...
    if (p_buf) {
        ...
        osi_list_append(p_lcb->link_xmit_data_q, p_buf);
    ...
    */
    if ( (p_lcb == NULL) || (p_lcb->link_xmit_quota == 0) ) {
        ...
        for (xx = 0; xx < MAX_L2CAP_LINKS; xx++, p_lcb++) {
            ...
            if (!osi_list_is_empty(p_lcb->link_xmit_data_q)) {
                p_buf = (BT_HDR *)osi_list_front(p_lcb->link_xmit_data_q);
                osi_list_remove(p_lcb->link_xmit_data_q, p_buf);
        ...
        /* this link may have sent anything but some other link sent packets so  */
        /* so we may need a timer to kick off this link's transmissions.         */
        if ( (!osi_list_is_empty(p_lcb->link_xmit_data_q)) && (p_lcb->sent_not_acked < p_lcb->link_xmit_quota) ) {
            btu_start_timer (&p_lcb->timer_entry, BTU_TTYPE_L2CAP_LINK, L2CAP_LINK_FLOW_CONTROL_TOUT);
        }
    }
...
void l2c_link_segments_xmitted (BT_HDR *p_msg)
{
    ...
    if (p_lcb->link_state == LST_CONNECTED) {
        ...
        osi_list_prepend(p_lcb->link_xmit_data_q, p_msg);
