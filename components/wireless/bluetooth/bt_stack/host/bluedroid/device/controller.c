...
static void start_up(void)
{
...
#ifdef BT_HOST_BLE_FORCED   //for some chip not support ble command
    controller_param.ble_supported = 1;
#endif
...
#ifdef BT_HOST_BLE_FORCED  //for some chip not support ble command
        {
            memset(controller_param.ble_supported_states, 0xff, sizeof(controller_param.ble_supported_states));
        }
#endif
    ...
    BT_LOG_DEBUG("controller start up end!!!");
