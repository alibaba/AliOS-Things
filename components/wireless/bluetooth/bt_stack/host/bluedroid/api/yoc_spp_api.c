...
bt_err_t yoc_spp_init(yoc_spp_mode_t mode)
{
    ...
    arg.init.mode = mode;
    return (btc_transfer_context(&msg, &arg, sizeof(btc_spp_args_t), NULL) == BT_STATUS_SUCCESS ? BT_OK : BT_FAIL);
}
...
bt_err_t yoc_spp_start_discovery(yoc_bd_addr_t bd_addr)
{
    ...
    memcpy(arg.start_discovery.bd_addr, bd_addr, BT_BD_ADDR_LEN);
    arg.start_discovery.num_uuid = 1;
    arg.start_discovery.p_uuid_list = &sdp_uuid;

    return (btc_transfer_context(&msg, &arg, sizeof(btc_spp_args_t), btc_spp_arg_deep_copy) == BT_STATUS_SUCCESS ? BT_OK : BT_FAIL);
}
                          ...
                          yoc_spp_role_t role, uint8_t remote_scn, yoc_bd_addr_t peer_bd_addr)
{
    ...
    memcpy(arg.connect.peer_bd_addr, peer_bd_addr, BT_BD_ADDR_LEN);
                            ...
                            yoc_spp_role_t role, uint8_t local_scn, const char *name)
{
    ...
    if (strlen(name) > YOC_SPP_SERVER_NAME_MAX) {
    ...
    arg.start_srv.max_session = YOC_SPP_MAX_SESSION;
