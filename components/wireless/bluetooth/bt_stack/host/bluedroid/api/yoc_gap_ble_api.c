...
bt_err_t yoc_ble_gap_register_callback(yoc_gap_ble_cb_t callback)
{
    ...
    return (btc_profile_cb_set(BTC_PID_GAP_BLE, callback) == 0 ? BT_OK : BT_FAIL);
}
...
bt_err_t yoc_ble_gap_config_adv_data(yoc_ble_adv_data_t *adv_data)
{
    ...
    return (btc_transfer_context(&msg, &arg, sizeof(btc_ble_gap_args_t), btc_gap_ble_arg_deep_copy) == BT_STATUS_SUCCESS ? BT_OK : BT_FAIL);
...
bt_err_t yoc_ble_gap_set_scan_params(yoc_ble_scan_params_t *scan_params)
{
    ...
    return (btc_transfer_context(&msg, &arg, sizeof(btc_ble_gap_args_t), NULL) == BT_STATUS_SUCCESS ? BT_OK : BT_FAIL);
}
...
bt_err_t yoc_ble_gap_set_pkt_data_len(yoc_bd_addr_t remote_device, uint16_t tx_data_length)
{
    ...
    memcpy(arg.set_pkt_data_len.remote_device, remote_device, BT_BD_ADDR_LEN);
...
bt_err_t yoc_ble_gap_set_rand_addr(yoc_bd_addr_t rand_addr)
{
    ...
    memcpy(arg.set_rand_addr.rand_addr, rand_addr, BT_BD_ADDR_LEN);
...
bt_err_t yoc_ble_gap_config_local_icon (uint16_t icon)
{
    ...
    case YOC_BLE_APPEARANCE_OUTDOOR_SPORTS_LOCATION_POD_AND_NAV:
        ...
        ret = (btc_transfer_context(&msg, &arg, sizeof(btc_ble_gap_args_t), NULL) == BT_STATUS_SUCCESS ? BT_OK : BT_FAIL);
        break;
    default:
        ret = BT_ERR_INVALID_ARG;
        break;
    }
    return ret;
}
                                                                 ...
                                                                 uint16_t slave_latency, uint16_t supervision_tout)
{
    ...
    if (YOC_BLE_IS_VALID_PARAM(min_conn_int, YOC_BLE_CONN_INT_MIN, YOC_BLE_CONN_INT_MAX) &&
        ...
        BT_LOG_ERROR("%s,invalid connection params:min_int = %d, max_int = %d, latency = %d, timeout = %d",\
...
{
    if(yoc_bluedroid_get_status() != (BT_BLUEDROID_STATUS_ENABLED)) {
        BT_LOG_ERROR("%s, bluedroid status error", __func__);
        return BT_FAIL;
    }
...
uint8_t *yoc_ble_resolve_adv_data( uint8_t *adv_data, uint8_t type, uint8_t *length)
{
    if (((type < YOC_BLE_AD_TYPE_FLAG) || (type > YOC_BLE_AD_TYPE_128SERVICE_DATA)) &&
        ...
        BT_LOG_ERROR("the eir type not define, type = %x\n", type);
        return NULL;
    }

    if (adv_data == NULL) {
        BT_LOG_ERROR("Invalid p_eir data.\n");
        return NULL;
    }
...
bt_err_t yoc_ble_gap_add_duplicate_scan_exceptional_device(yoc_ble_duplicate_exceptional_info_type_t type, yoc_duplicate_info_t device_info)
{
    ...
    if (!device_info && type <= YOC_BLE_DUPLICATE_SCAN_EXCEPTIONAL_INFO_MESH_LINK_ID) {
        return BT_ERR_INVALID_SIZE;
    }
    msg.sig = BTC_SIG_API_CALL;
    msg.pid = BTC_PID_GAP_BLE;
    msg.act = BTC_GAP_BLE_UPDATE_DUPLICATE_SCAN_EXCEPTIONAL_LIST;
    arg.update_duplicate_exceptional_list.subcode = YOC_BLE_DUPLICATE_EXCEPTIONAL_LIST_ADD;
    arg.update_duplicate_exceptional_list.info_type = type;
    if (device_info) {
        memcpy(arg.update_duplicate_exceptional_list.device_info, device_info, sizeof(yoc_bd_addr_t));
    }
...
bt_err_t yoc_ble_gap_remove_duplicate_scan_exceptional_device(yoc_ble_duplicate_exceptional_info_type_t type, yoc_duplicate_info_t device_info)
{
    ...
    msg.pid = BTC_PID_GAP_BLE;
    msg.act = BTC_GAP_BLE_UPDATE_DUPLICATE_SCAN_EXCEPTIONAL_LIST;
    arg.update_duplicate_exceptional_list.subcode = YOC_BLE_DUPLICATE_EXCEPTIONAL_LIST_REMOVE;
    arg.update_duplicate_exceptional_list.info_type = type;
    if (device_info) {
        memcpy(arg.update_duplicate_exceptional_list.device_info, device_info, sizeof(yoc_bd_addr_t));
    }
...
bt_err_t yoc_ble_gap_clean_duplicate_scan_exceptional_list(yoc_duplicate_scan_exceptional_list_type_t list_type)
{
    ...
    msg.pid = BTC_PID_GAP_BLE;
    msg.act = BTC_GAP_BLE_UPDATE_DUPLICATE_SCAN_EXCEPTIONAL_LIST;
    arg.update_duplicate_exceptional_list.subcode = YOC_BLE_DUPLICATE_EXCEPTIONAL_LIST_CLEAN;
...
bt_err_t yoc_ble_set_encryption(yoc_bd_addr_t bd_addr, yoc_ble_sec_act_t sec_act)
{
    ...
    memcpy(arg.set_encryption.bd_addr, bd_addr, BT_BD_ADDR_LEN);
...
bt_err_t yoc_ble_gap_security_rsp(yoc_bd_addr_t bd_addr, bool accept)
{
    ...
    memcpy(arg.sec_rsp.bd_addr, bd_addr, BT_BD_ADDR_LEN);
...
bt_err_t yoc_ble_passkey_reply(yoc_bd_addr_t bd_addr, bool accept, uint32_t passkey)
{
    ...
    memcpy(arg.enc_passkey_replay.bd_addr, bd_addr, BT_BD_ADDR_LEN);
...
bt_err_t yoc_ble_confirm_reply(yoc_bd_addr_t bd_addr, bool accept)
{
    ...
    memcpy(arg.enc_comfirm_replay.bd_addr, bd_addr, BT_BD_ADDR_LEN);
...
bt_err_t yoc_ble_remove_bond_device(yoc_bd_addr_t bd_addr)
{
    ...
    memcpy(arg.remove_bond_device.bd_addr, bd_addr, BT_BD_ADDR_LEN);
...
bt_err_t yoc_ble_oob_req_reply(yoc_bd_addr_t bd_addr, uint8_t *TK, uint8_t len)
{
    ...
    memcpy(arg.oob_req_reply.bd_addr, bd_addr, BT_BD_ADDR_LEN);
...
bt_err_t yoc_ble_gap_disconnect(yoc_bd_addr_t remote_device)
{
    ...
    memcpy(arg.disconnect.remote_device, remote_device, BT_BD_ADDR_LEN);
