...
struct osi_thread {
  hal_task_t thread_handle;                  /*!< Store the thread object */
...
static void osi_thread_stop(osi_thread_t *thread)
{
    ...
    //join
    ret = osi_thread_join(thread, 10000000); //wait 1000ms
    if (ret != 0) {
        OSI_TRACE_ERROR("osi_thread_stop failed :%x", thread);
    }
...
osi_thread_t *osi_thread_create(const char *name, size_t stack_size, int priority, osi_thread_core_t core, uint8_t work_queue_num)
{
    ...
    OSI_TRACE_ERROR("osi_thread_create :%s", name);
    if (stack_size <= 0 ||
    ...
    if (hal_os_thread_new(&thread->thread_handle, name, osi_thread_run,
...
bool osi_thread_post(osi_thread_t *thread, osi_thread_func_t func, void *context, int queue_idx, uint32_t timeout)
{
    ...
    if (fixed_queue_enqueue(thread->work_queues[queue_idx], item, timeout) == false) {
        osi_free(item);
        OSI_TRACE_ERROR("osi_thread_post failed\n");
        return false;
    }
