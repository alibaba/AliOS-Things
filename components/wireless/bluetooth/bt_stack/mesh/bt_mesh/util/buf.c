...
//no need to alloc buffer for hci cmd/tx and acl tx
#ifndef assert
#define assert(x) if (!x) {printf("assert failed at %s %d\n", __FILE__, __LINE__);}
#endif
#else
...
#endif
{
	...
	NET_BUF_DBG("%s():%d: fifo %pp timeout %d", func, line, fifo, timeout);
	key = irq_lock();
