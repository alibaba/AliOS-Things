            ...
            bool update)
{
    ...
    #if 0
    buf[0] = (uint8_t)((app_idx>>8)&0xff);
    buf[1] = (uint8_t)(app_idx&0xff);
    memcpy(&buf[2],val,16);
    ais_set_para(ALI_MESH_PARA_APPKEY,buf,18);
    #endif
    /*--add by fengcai 20190403--*/
...
static void vndmod_app_bind(u16_t elem_addr,u16_t key_app_idx)

{
    ...
    BT_DBG("start elem for addr %04x\n",elem_addr);
    elem = bt_mesh_elem_find(elem_addr);
    if(!elem){
        BT_DBG("can not find ele for addr %d\n",elem_addr);
        return;
    }
    mod = bt_mesh_model_find_vnd(elem, CONFIG_MESH_VENDOR_COMPANY_ID, CONFIG_MESH_VENDOR_MODEL_SRV);
    status = mod_bind(mod, key_app_idx);

    BT_DBG(">>>vnd model app bind 0x%02x <<<\n", status);
}

u16_t group_list[] = {0xc000, 0xcfff};

s8_t genie_mesh_setup(void)
{
    struct bt_mesh_elem *elem;
    struct bt_mesh_model *mod;
    u8_t elem_cnt, i, j, k, l, status;
    u16_t primary_addr;
    u16_t key_app_idx = 0;
    ...
    BT_DBG("start elem %d primary addr %04x\n",elem_cnt,primary_addr);
    for(i = 0; i < elem_cnt; i++){
        ...
        for (j = 0; j < elem->model_count; j++) {
            ...
            status = mod_bind(mod, key_app_idx);
            BT_DBG(">>>model app bind 0x%02x %04x <<<\n", status, elem->models[j].id);
            for (k = 0; k < ARRAY_SIZE(mod->groups); k++) {
                l = 0;
                if (mod->groups[k] == BT_MESH_ADDR_UNASSIGNED) {
                    if(l < sizeof(group_list)) {
                        mod->groups[k] = group_list[l];
                        l++;
                    }
                }
            }
        ...
        for (j = 0; j < elem->vnd_model_count; j++) {
            ...
            status = mod_bind(mod, key_app_idx);
            BT_DBG(">>>vnd model app bind 0x%02x %04x %04x <<<\n", status,\
                    elem->vnd_models[j].vnd.company,elem->vnd_models[j].vnd.id);
            for (k = 0; k < ARRAY_SIZE(mod->groups); k++) {
                l = 0;
                if (mod->groups[k] == BT_MESH_ADDR_UNASSIGNED) {
                    if(l < sizeof(group_list)) {
...
#endif
//    BT_ERR("%s not implmented", __func__);
    return BT_MESH_BEACON_DISABLED;
}
...
#endif
    return (BT_MESH_TRANSMIT(3, 70));
}

u8_t bt_mesh_net_transmit_get_segment(void)
{
#if 0
    if (conf) {
        return conf->net_transmit;
...
#endif
#ifdef ADV_ONE_SHOT
    return (BT_MESH_TRANSMIT(0, 10));
#else
    return (BT_MESH_TRANSMIT(2, 20));
#endif
}
