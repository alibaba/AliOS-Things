...
#define SEG_RETRANSMIT_TIMEOUT      K_MSEC(120)

/* Number of retransmit attempts (after the initial transmit) per segment */
#define SEG_RETRANSMIT_ATTEMPTS     5
		    ...
		    const struct bt_mesh_send_cb *cb, void *cb_data)
{
	...
	BT_DBG("send_seg: src 0x%04x dst 0x%04x app_idx 0x%04x aszmic %u sdu_len %u",
...
void bt_mesh_app_key_set_manual(u16_t net_idx, u16_t app_idx, uint8_t key_val[16], uint8_t aid)
{
    ...
    if (app_idx >= ARRAY_SIZE(bt_mesh.app_keys)) {
        BT_ERR("Error: app idx exceeds limit. NOT set!");
        return;
    }
...
static bool is_replay(struct bt_mesh_net_rx *rx)
{
	...
	for (i = 0; i < ARRAY_SIZE(bt_mesh.rpl); i++) {
		...
		if (rpl->src == rx->ctx.addr) {
			...
			if ((!rx->old_iv && rpl->old_iv) ||
                ...
                BT_DBG("RPL is old, old_seq:0x%x,new_seq:0x%x,old_iv:0x%x,new_iv:0x%x",
                    __func__,rpl->seq,rx->seq,rpl->old_iv,rx->old_iv);
				return true;
			}
        }       
        if (!rpl->src) {
    ...
    /*RPL is full ,need to replace it */
    {
		bt_mesh.rpl[bt_mesh.rpl_index].src = rx->ctx.addr;
		bt_mesh.rpl[bt_mesh.rpl_index].seq = rx->seq;
		bt_mesh.rpl[bt_mesh.rpl_index].old_iv = rx->old_iv;

        bt_mesh.rpl_index++;
        if(bt_mesh.rpl_index == CONFIG_BT_MESH_CRPL-1)
        {
            bt_mesh.rpl_index = 0;
        }
        BT_INFO("RPL is full!");
...
found_rx:
	...
	* Don't change rx->seq for friend queue
	*/
	if (!net_rx->friend_match){
...
int bt_mesh_trans_recv(struct net_buf_simple *buf, struct bt_mesh_net_rx *rx)
{
	...
	BT_DBG("src 0x%04x dst 0x%04x seq 0x%08x friend_match %u, local_match %d",
	       rx->ctx.addr, rx->dst, rx->seq, rx->friend_match, rx->local_match);
	 ...
	 * 
	 * Delete ENOENT condition as per MESH/NODE/FRND/LPN/BI-02-C.
	 */
	if (IS_ENABLED(CONFIG_BT_MESH_LOW_POWER)  && (bt_mesh_lpn_timer() ||
