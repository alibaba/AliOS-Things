...
struct bt_mesh_health_srv *p_health_srv;

static u8_t fast_period_div = 0;
...
static int health_pub_update(struct bt_mesh_model *mod)
{
	 ...
	 * Bugfix for MESH/HM/CFS/BV-01-C MESH/HM/CFS/BV-02-C
	 * Lower tester sets the health publish period to 8s, and expect no fault status 
	 * every 8s and fault status every 2s as Fast Period Divisor described in spec
	 * 4.4.3.2.1
	 * 
	 **/
	pub->period_div = count ? fast_period_div : 0;
...
static void attention_off_cb(struct k_work *work)
{
...
int bt_mesh_health_srv_init(struct bt_mesh_model *model, bool primary)
{
	...
	k_delayed_work_init(&srv->attn_timer, attention_off_cb);
