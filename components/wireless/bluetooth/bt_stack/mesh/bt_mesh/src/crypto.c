...
#if 1   //ndef CONFIG_MESH_STACK_ALONE
			       ...
			       u8_t *out_msg, size_t mic_size)
{
	...
	BT_DBG("msg (len %d) %s", msg_len, bt_hex(msg, msg_len));
	BT_DBG("aad_len %d mic_size %d", aad_len, mic_size);
...
int bt_mesh_aes_cmac_local(const uint8_t key[16], struct bt_mesh_sg *sg,
...
const uint8_t *bt_mesh_pub_key_get_local(void)
{
    return g_local_ecc.pk;
}

int bt_mesh_pub_key_gen_local(struct bt_mesh_pub_key_cb *cb)
{
    u8_t pk[64];
    ...
    do {
        ...
        rc = uECC_make_key(pk, g_local_ecc.private_key, &curve_secp256r1);
    ...
    } while (0); //(memcmp(g_local_ecc.private_key, debug_private_key, 32) == 0);

    sys_memcpy_swap(g_local_ecc.pk, pk, 32);
    sys_memcpy_swap(&g_local_ecc.pk[32], &pk[32], 32);

    memcpy(g_local_ecc.private_key2, g_local_ecc.private_key, 32);
    if (cb) {
        if (cb->func)
            cb->func(g_local_ecc.pk);
    }

    return 0;
}

int bt_mesh_dh_key_gen_local(const uint8_t remote_pk[64], bt_mesh_dh_key_cb_t cb)
{
    int ret;
    u8_t pk[64];
    u8_t dhkey[32];
//    memcpy(g_local_ecc.private_key, g_local_ecc.private_key2, 32);

#if 1
    sys_memcpy_swap(pk, remote_pk, 32);
    sys_memcpy_swap(&pk[32], &remote_pk[32], 32);
#else
    memcpy(pk, remote_pk, 64);
#endif
    ret = uECC_valid_public_key(pk, &curve_secp256r1);
    if (ret < 0) {
        BT_ERR("public key is not valid (ret %d)", ret);
        ret = TC_CRYPTO_FAIL;
    } else {
        ret = uECC_shared_secret(pk, g_local_ecc.private_key, dhkey,
                                 &curve_secp256r1);
    }

    if (ret == TC_CRYPTO_SUCCESS) {
        sys_memcpy_swap(g_local_ecc.dhkey, dhkey, 32);
        cb(g_local_ecc.dhkey);
        return 0;
    } else {
        return -1;
    }
...
void prng_random_get(u8_t *random, u8_t size)
{
    int32_t i;
    int32_t time;
    ...
    for (i = 0; i < (size&0xfffffffc); i+=4) {
        time = k_now_ms();
        memcpy(random+i, &time, 4);
    }

    if (i < size) {
        time = k_now_ms();
        memcpy(random+i, &time, size-i);
    }
}
...
static int prng_reseed(struct tc_hmac_prng_struct *h)
{
    ...
    for (i = 0; i < (sizeof(seed) / 8); i++) {
        prng_random_get(&random[0], 8);
        memcpy(&seed[i * 8], random, 8);
    }
...
int prng_init(void)
{
    ...
    ret = tc_hmac_prng_init(&prng, &random[0], sizeof(random));
    if (ret == TC_CRYPTO_FAIL) {
...
int bt_mesh_random_local(void *buf, size_t len)
{
    int ret;
...
{
    return !bt_mesh_random_local(dst, len);
}
