                ...
                const u8_t key[16])
{
 ...
 void bt_mesh_rpl_delete(u16_t addr)
 {
    int i;
    ...
    if (rpl->src == addr) {
            ...
            BT_DBG("%s, reset the rpl item #0x%x\n",__func__, addr);
            return;
        }      
    }
    BT_DBG("%s,Unable to find the rpl item 0x%x\n",__func__, addr);
 }
...
#if 0 //Ethan: prevent from build error
    genie_event(GENIE_EVT_SDK_SEQ_UPDATE, NULL);
#endif
    if (ctl) {
        net_buf_simple_push_u8(buf, tx->ctx->send_ttl | 0x80);
    } else {
        net_buf_simple_push_u8(buf, tx->ctx->send_ttl & 0x7F);
    }
             ...
             const struct bt_mesh_send_cb *cb, void *cb_data)
{
    ...
    BT_DBG("src 0x%04x dst 0x%04x len %u headroom %x tailroom %x",
    ...
    if (err) {
        BT_ERR("bt_mesh_net_encode err: 0x%x", err);
        goto done;
    }
