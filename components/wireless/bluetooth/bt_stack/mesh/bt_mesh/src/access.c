...
static bool model_has_key(struct bt_mesh_model *mod, u16_t key)
{
    ...
    for (i = 0; i < ARRAY_SIZE(mod->keys); i++) {
        //BT_DBG("i >%d key1:%d key2:%d", i,mod->keys[i],key);
        if (mod->keys[i] == key) {
            return true;
        }
    }
...
extern void bt_mesh_access_msg_handler(unsigned int opcode, unsigned short src, unsigned short dst,
                                                 unsigned char* buf, unsigned short len, char rssi,
                                                 unsigned char ttl, unsigned char appKeyIdx, unsigned char netKeyIdx);
void bt_mesh_model_recv(struct bt_mesh_net_rx *rx, struct net_buf_simple *buf)
{
...
#ifdef MESH_DEBUG_RX   //Lancelot
    MESH_MSG_RX("SRC: 0x%02X", rx->ctx.addr);
    MESH_MSG_RX("DST: 0x%02X", rx->dst);
    MESH_MSG_RX("OPCODE: 0x%04X", opcode);
    MESH_MSG_RX("Payload size: %d", buf->len);
    MESH_MSG_RX_BUFF(buf->data, buf->len);
#endif
    ...
    for (i = 0; i < dev_comp->elem_count; i++) {
        ...
        bt_mesh_access_msg_handler(opcode, rx->ctx.addr, rx->dst, buf->data, buf->len, rx->rssi, rx->ctx.recv_ttl, rx->ctx.app_idx, rx->ctx.net_idx);
    }
}
...
int bt_mesh_model_direct_send(struct bt_mesh_msg_ctx *ctx, s32_t src,
             u8_t* data, s32_t data_len)
{
    struct bt_mesh_net_tx tx;
    struct net_buf_simple *msg = NET_BUF_SIMPLE(BT_MESH_TX_SDU_MAX);
    int32_t len;

    if (data_len > BT_MESH_TX_SDU_MAX) {
        BT_ERR("%s, too large pdu length = %d", __func__, data_len);
        return -1;
    }
    ...
    if(tx.ctx->send_ttl & 0x80) {
        BT_DBG("%s, ttl: 0x%x, 1.2s activation packet.", __func__, tx.ctx->send_ttl);
        tx.ctx->send_ttl &= 0x7f;
        tx.xmit |= 0x80;
    }
...
#if 1
    if (ctx->send_rel || msg->len > 11) {
        tx.xmit = bt_mesh_net_transmit_get_segment();
    }
#endif

    if (!tx.sub) {
        BT_ERR("No matching subnet for NetKey Index 0x%04x\n", ctx->net_idx);
        return 0;
    }

    return bt_mesh_trans_send(&tx, msg, NULL, NULL);
}
#endif
               ...
               const struct bt_mesh_send_cb *cb, void *cb_data)
{
    ...
    BT_DBG("%s tx.sub = %x", __func__, tx.sub);
    if (!tx.sub) {
        BT_ERR("No available subnet found");
        return -EINVAL;
    }
...
u16_t bt_mesh_model_get_appkey_id(struct bt_mesh_elem *elem,
                                  u16_t company, u16_t id)
{
    return 0;
}
...
u8_t app_key_set(u16_t net_idx, u16_t app_idx, const u8_t val[16],
            bool update)
{
