...
arm_status
arm_convolve_HWC_q7_RGB_uai(const q7_t * Im_in,
    ...
    for (i = 0; i < ch_im_out; i++)
    {
        for (j = 0; j < dim_im_out; j++)
        {
            for (k = 0; k < dim_im_out; k++)
            {
                ...
                Im_out[i + (j * dim_im_out + k) * ch_im_out] = uai_quant_int8(conv_out, kernel_scale[i], bias[i], bias_scale[i], act_scale, shift);
            }
        }
    }
