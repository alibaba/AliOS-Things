...
int net_download_work(net_cache_t *nc)
{
	...
	if (ret == 0) {
		...
		char temp_suffix[len];
		memset(temp_suffix, 0, len);
		temp_suffix[0] = '_';
		temp_suffix[1] = '1';
		memcpy(&temp_suffix[2], ptr, strlen(ptr) + 1);
		memcpy(ptr, temp_suffix, len);
		M_LOGD("update file path: %s\n", filepath);
		#endif
	}
	...
	if (nc->head_data_size > 0) {
		ret = os_fwrite(nc->buffer, nc->head_data_size, 1, fp);
		if (os_ferror(fp) || ret != 1) {
	...
	while (load_size > 0) {
		...
		os_mutex_lock(nc->lock, OS_WAIT_FOREVER);
		ret = os_fwrite(nc->buffer, rd_ret, 1, fp);
		if (os_ferror(fp) || ret != 1) {
