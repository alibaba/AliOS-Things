 ...
 * @param[in]  member  the name of the variable within the struct.
 */
#define aos_list_entry(ptr, type, member) aos_container_of(ptr, type, member)
...
#define dlist_for_each_entry_reverse(pos, head, member, type) \
        for (pos = aos_list_entry((head)->prev, type, member);    \
            &pos->member != (head);                           \
            pos = aos_list_entry(pos->member.prev, type, member))
