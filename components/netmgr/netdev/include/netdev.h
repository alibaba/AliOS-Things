...
#define NETDEV_FLAG_DHCPD              0x200U
...
struct netdev_ping_resp
{
    ...
    uint64_t ticks;                           /* response time, unit us */
    void *user_data;                             /* user-specific data */
};
...
struct netdev_ops
{
    ...
    int (*set_default_netif)(struct netdev *netdev);
    ...
    int (*set_dhcpd)(struct netdev *netdev, bool is_enabled);
...
#define netdev_is_dhcpd_enabled(netdev) (((netdev)->flags & NETDEV_FLAG_DHCPD) ? (uint8_t)1 : (uint8_t)0)
