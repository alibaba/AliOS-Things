...
static int lfs_bd_read(lfs_t *lfs,
    ...
    if (off+size > lfs->cfg->block_size) {
        LFS_DEBUG("%s %d, off+size: %d", __func__, __LINE__, off+size);
    ...
    while (size > 0) {
        ...
        if (block >= lfs->cfg->block_count)  {
            LFS_DEBUG("ASSERT: %s %d, block: %d", __func__, __LINE__, block);
            return LFS_ERR_CORRUPT;
        }
        ...
        if (err > 0) {
            LFS_DEBUG("%s %d, err: %d", __func__, __LINE__, err);
            return LFS_ERR_CORRUPT;
        }
        LFS_ASSERT(err <= 0);
        if (err) {
...
static int lfs_bd_flush(lfs_t *lfs,
    ...
    if (pcache->block != LFS_BLOCK_NULL && pcache->block != LFS_BLOCK_INLINE) {
        if (pcache->block >= lfs->cfg->block_count) {
            LFS_DEBUG("ASSERT: %s %d, pcache->block: %d exceed cfg count: %d",
                      __func__, __LINE__, pcache->block, lfs->cfg->block_count);
            return LFS_ERR_CORRUPT;
        }
        ...
        if (err > 0)  {
            LFS_DEBUG("ASSERT: %s %d, err: %d", __func__, __LINE__, err);
            return LFS_ERR_CORRUPT;
        }
        LFS_ASSERT(err <= 0);
        if (err) {
...
static int lfs_bd_erase(lfs_t *lfs, lfs_block_t block) {
    ...
    if (err > 0) {
        LFS_DEBUG("%s assert, err: %d", __func__, err);
        return LFS_ERR_CORRUPT;
    }
    LFS_ASSERT(err <= 0);
    return err;
...
static int insert_into_cfg_list(lfs_t *lfs, struct lfs_cfg_list_s **out)
{
    struct lfs_cfg_list_s *p, *cfg;
...
int lfs_file_open(lfs_t *lfs, lfs_file_t *file,
    ...
    err = lfs_file_opencfg(lfs, file, path, flags, &defaults);
#else
    static struct lfs_cfg_info_s _info[2];
...
int lfs_fstat(lfs_t *lfs, lfs_file_t *file, struct lfs_info *info) {
    LFS_TRACE("lfs_fstat(%p, %p, %p)", (void*)lfs, file, (void*)info);
