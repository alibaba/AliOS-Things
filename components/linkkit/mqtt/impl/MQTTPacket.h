...
#define MQTT_HEADER_SET_TYPE(head, type) \
    do {                                 \
        head |= ((type << 4) & 0xF0);    \
    } while (0)
#define MQTT_HEADER_SET_DUP(head, dup) \
    do {                               \
        head |= ((dup << 3) & 0x08);   \
    } while (0)
#define MQTT_HEADER_SET_QOS(head, qos) \
    do {                               \
        head |= ((qos << 1) & 0x06);   \
    } while (0)
#define MQTT_HEADER_SET_RETAIN(head, retain) \
    do {                                     \
        head |= (retain & 0x01);             \
    } while (0)
...
int MQTTDeserialize_ack(unsigned char *packettype, unsigned char *dup,
                        unsigned short *packetid, unsigned char *buf,
                        int buflen);
