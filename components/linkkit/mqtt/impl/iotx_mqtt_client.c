...
#else
    memset(pClient->list_pub_wait_ack, 0,
           sizeof(iotx_mc_pub_info_t) * IOTX_MC_REPUB_NUM_MAX);
#endif
}
...
static void iotx_mc_pub_wait_list_deinit(iotx_mc_client_t *pClient)
{
    ...
    list_for_each_entry_safe(node, next_node, &pClient->list_pub_wait_ack,
                             linked_list, iotx_mc_pub_info_t)
    {
...
static int iotx_mc_set_connect_params(iotx_mc_client_t *pClient,
                                      MQTTPacket_connectData *pConnectParams)
{
    ...
    memcpy(pClient->connect_data.will.struct_id, pConnectParams->will.struct_id,
           4);
    pClient->connect_data.will.struct_version =
        pConnectParams->will.struct_version;
    ...
    if (pConnectParams->keepAliveInterval <
        ...
        pClient->connect_data.keepAliveInterval =
            CONFIG_MQTT_KEEPALIVE_INTERVAL;
    } else if (pConnectParams->keepAliveInterval >
        ...
        pClient->connect_data.keepAliveInterval =
            pConnectParams->keepAliveInterval;
    }
...
static void iotx_mc_set_client_state(iotx_mc_client_t *pClient,
                                     iotx_mc_state_t newState)
{
...
static int iotx_mc_init(iotx_mc_client_t *pClient,
                        iotx_mqtt_param_t *pInitParams)
{
    int rc = FAIL_RETURN;
    ...
    rc = iotx_net_init(&pClient->ipstack, pInitParams->host, pInitParams->port,
                       pInitParams->pub_key);
...
static int _alloc_recv_buffer(iotx_mc_client_t *c, int len)
{
    ...
    if (c->buf_read != NULL) { /* do realloc */
        ...
        memcpy(temp, c->buf_read,
               c->buf_size_read < tmp_len ? c->buf_size_read : tmp_len);
        mqtt_free(c->buf_read);
        c->buf_read = temp;
    } else {
...
int MQTTConnect(iotx_mc_client_t *pClient)
{
    ...
    len = MQTTSerialize_connect((unsigned char *)pClient->buf_send,
                                pClient->buf_size_send, pConnectParams);
    if (len <= 0) {
    ...
    if ((iotx_mc_send_packet(pClient, pClient->buf_send, len, &connectTimer)) !=
        SUCCESS_RETURN) {
...
static int _handle_event(iotx_mqtt_event_handle_pt handle, iotx_mc_client_t *c,
...
static int iotx_mc_read_packet(iotx_mc_client_t *c, iotx_time_t *timer,
                               unsigned int *packet_type)
{
    ...
    left_t = iotx_time_left(timer);
    left_t = (left_t == 0) ? 1 : left_t;
    rc = iotx_mc_decode_packet(c, &rem_len, left_t);
    if (rc < 0) {
    ...
    if ((rem_len > 0) && ((rem_len + len) > c->buf_size_read)) {
        ...
        mqtt_err(
            "mqtt read buffer is too short, mqttReadBufLen : %u, remainDataLen "
            ": %d",
        ...
        *packet_type = 0;
        ...
        do {
            needReadLen =
                (rem_len > c->buf_size_read) ? c->buf_size_read : rem_len;
            printf("read len:%d\n", needReadLen);
            if (c->ipstack.read(&c->ipstack, c->buf_read, needReadLen,
                                left_t) != needReadLen) {
...
#define RETRY_INTV_PERIOD (2000)
    ...
    mqtt_info(
        "connect params: MQTTVersion=%d, clientID=%s, keepAliveInterval=%d, "
        "username=%s",
    ...
    printf(
        "connect params: %s:%d - MQTTVersion=%d, clientID=%s, "
        "keepAliveInterval=%d, username=%s, password:%s\r\n",
        pClient->ipstack.pHostAddress, pClient->ipstack.port,
        pClient->connect_data.MQTTVersion,
        pClient->connect_data.clientID.cstring,
        pClient->connect_data.keepAliveInterval,
        pClient->connect_data.username.cstring,
    ...
    do {
        ...
        if (rc <= MQTT_CONNACK_NOT_AUTHORIZED_ERROR &&
            rc >= MQTT_CONANCK_UNACCEPTABLE_PROTOCOL_VERSION_ERROR) {
        ...
        if (SUCCESS_RETURN != rc) {
            ...
            mqtt_warning("tried [%d/%d] times CONN, waiting for %d ms...",
                         try_count, RETRY_TIME_LIMIT - 1, RETRY_INTV_PERIOD);
    ...
    utils_time_countdown_ms(&pClient->next_ping_time,
...
#if !WITH_MQTT_ONLY_QOS0
static int iotx_mc_push_pubInfo_to(iotx_mc_client_t *c, int len,
                                   unsigned short msgId,
                                   iotx_mc_pub_info_t **node)
{
#ifdef PLATFORM_HAS_DYNMEM
    int list_number;
    ...
    if ((len < 0) || (len > c->buf_size_send)) {
        ...
        if (len >= c->buf_size_send) {
            mqtt_err(
                "IOTX_MC_TX_MAX_LEN is too short, len: %d, IOTX_MC_TX_MAX_LEN: "
                "%d",
                len, IOTX_MC_TX_MAX_LEN);
        }
#endif
        return FAIL_RETURN;
    }
    ...
    if (list_number >= IOTX_MC_REPUB_NUM_MAX) {
        mqtt_err("more than %u elements in republish list. List overflow!",
                 list_number);
        return FAIL_RETURN;
    }

    repubInfo =
        (iotx_mc_pub_info_t *)mqtt_malloc(sizeof(iotx_mc_pub_info_t) + len);
    if (NULL == repubInfo) {
...
static int iotx_mc_mask_pubInfo_from(iotx_mc_client_t *c, uint16_t msgId)
{
    ...
    list_for_each_entry(node, &c->list_pub_wait_ack, linked_list,
                        iotx_mc_pub_info_t)
    {
        if (node->msg_id == msgId) {
            node->node_state =
                IOTX_MC_NODE_STATE_INVALID; /* mark as invalid node */
        }
    }
    ...
    for (idx = 0; idx < IOTX_MC_REPUB_NUM_MAX; idx++) {
        if (c->list_pub_wait_ack[idx].used &&
            c->list_pub_wait_ack[idx].msg_id == msgId) {
            c->list_pub_wait_ack[idx].node_state =
                IOTX_MC_NODE_STATE_INVALID; /* mark as invalid node */
        }
    }
#endif
...
static int MQTTPubInfoProc(iotx_mc_client_t *pClient)
{
                             ...
                             linked_list, iotx_mc_pub_info_t)
    {
        ...
        if (utils_time_spend(&node->pub_start_time) <=
            (pClient->request_timeout_ms * 2)) {
            continue;
        }
    ...
    for (idx = 0; idx < IOTX_MC_REPUB_NUM_MAX; idx++) {
        ...
        if (IOTX_MC_NODE_STATE_INVALID ==
            pClient->list_pub_wait_ack[idx].node_state) {
            memset(&pClient->list_pub_wait_ack[idx], 0,
                   sizeof(iotx_mc_pub_info_t));
            continue;
        }
        ...
        /* check the request if timeout or not */
        if (utils_time_spend(&pClient->list_pub_wait_ack[idx].pub_start_time) <=
            (pClient->request_timeout_ms * 2)) {
            continue;
        }

        /* If wait ACK timeout, republish */
        rc = MQTTRePublish(pClient, (char *)pClient->list_pub_wait_ack[idx].buf,
                           pClient->list_pub_wait_ack[idx].len);
...
static void _iotx_mqtt_event_handle_sub(void *pcontext, void *pclient,
                                        iotx_mqtt_event_msg_pt msg)
{
...
#ifdef PLATFORM_HAS_DYNMEM
    list_for_each_entry_safe(node, next, &client->list_sub_sync_ack,
                             linked_list, mqtt_sub_sync_node_t)
    {
...
static int iotx_mc_handle_recv_SUBACK(iotx_mc_client_t *c)
{
    ...
    rc = MQTTDeserialize_suback(&mypacketid, MUTLI_SUBSCIRBE_MAX, &count,
                                grantedQoS, (unsigned char *)c->buf_read,
                                c->buf_size_read);

    if (rc < 0) {
        mqtt_err("Sub ack packet error, rc = MQTTDeserialize_suback() = %d",
                 rc);
...
static int iotx_mc_get_zip_topic(const char *path, int len, char outbuf[],
                                 int outlen)
{
    ...
    memcpy(outbuf, comp_data,
           outlen > MQTT_ZIP_PATH_DEFAULT_LEN ? MQTT_ZIP_PATH_DEFAULT_LEN
                                              : outlen);
    return 0;
}
#endif
...
static void iotx_mc_deliver_message(iotx_mc_client_t *c, MQTTString *topicName,
                                    iotx_mqtt_topic_info_pt topic_msg)
{
    ...
    iotx_mc_get_zip_topic(net_topic, net_topic_len, md5_topic_data,
                          MQTT_ZIP_PATH_DEFAULT_LEN);
...
#ifdef PLATFORM_HAS_DYNMEM
    list_for_each_entry(node, &c->list_sub_handle, linked_list,
    ...
    for (idx = 0; idx < IOTX_MC_SUBHANDLE_LIST_MAX_LEN; idx++) {
        if ((c->list_sub_handle[idx].used == 1) &&
            (MQTTPacket_equals(compare_topic,
                               (char *)c->list_sub_handle[idx].topic_filter) ||
             iotx_mc_is_topic_matched(
                 (char *)c->list_sub_handle[idx].topic_filter, topicName))) {
            mqtt_debug("topic be matched");
...
static int MQTTPuback(iotx_mc_client_t *c, unsigned int msgId,
                      enum msgTypes type)
{
    int rc = 0;
    int len = 0;
    ...
    if (type == PUBACK) {
        ...
        len = MQTTSerialize_ack((unsigned char *)c->buf_send, c->buf_size_send,
                                PUBACK, 0, msgId);
...
static int iotx_mc_handle_recv_PUBLISH(iotx_mc_client_t *c)
{
    ...
    if (1 != MQTTDeserialize_publish(
                 (unsigned char *)&topic_msg.dup, (int *)&qos,
                 (unsigned char *)&topic_msg.retain,
                 (unsigned short *)&topic_msg.packet_id, &topicName,
                 (unsigned char **)&topic_msg.payload, (int *)&payload_len,
                 (unsigned char *)c->buf_read, c->buf_size_read)) {
    ...
    mqtt_info("Downstream Topic: '%.*s'", topicName.lenstring.len,
    ...
    mqtt_debug("%20s : %.*s", "Topic Name", topicName.lenstring.len,
               topicName.lenstring.data);
...
{
    unsigned short mypacketid =
        0; /* should be the same as the packetid above */
    if (!c) {
        return FAIL_RETURN;
    }

    if (MQTTDeserialize_unsuback(&mypacketid, (unsigned char *)c->buf_read,
                                 c->buf_size_read) != 1) {
...
static int MQTTKeepalive(iotx_mc_client_t *pClient)
{
    ...
    len = MQTTSerialize_pingreq((unsigned char *)pClient->buf_send,
...
static int iotx_mc_attempt_reconnect(iotx_mc_client_t *pClient)
{
    ...
    mqtt_info(
        "reconnect params: MQTTVersion=%d, clientID=%s, keepAliveInterval=%d, "
        "username=%s",
        pClient->connect_data.MQTTVersion,
        pClient->connect_data.clientID.cstring,
        pClient->connect_data.keepAliveInterval,
...
static int iotx_mc_handle_reconnect(iotx_mc_client_t *pClient)
{
    ...
    if (!utils_time_is_expired(
            &(pClient->reconnect_param.reconnect_next_time))) {
    ...
    */
    ...
    if (SUCCESS_RETURN == rc) {
        ...
        */
        ...
        /* e.g. init currentReconnectWaitInterval=1s, reconnect failed, then
         * 2s..4s..8s */
        if (IOTX_MC_RECONNECT_INTERVAL_MAX_MS >
            pClient->reconnect_param.reconnect_time_interval_ms) {
            pClient->reconnect_param.reconnect_time_interval_ms *= 2;
        } else {
            pClient->reconnect_param.reconnect_time_interval_ms =
                IOTX_MC_RECONNECT_INTERVAL_MAX_MS;
        }
    }
    /*
        _conn_info_dynamic_reload_clear(pClient);
...
static void iotx_mc_reconnect_callback(iotx_mc_client_t *pClient)
{
    ...
    if (NULL != pClient->handle_event.h_fp) {
        ...
        pClient->handle_event.h_fp(pClient->handle_event.pcontext, pClient,
                                   &msg);
    }
}
...
static void iotx_mc_keepalive(iotx_mc_client_t *pClient)
{
    ...
    do {
        ...
        /* If network suddenly interrupted, stop pinging packet, try to
         * reconnect network immediately */
        if (IOTX_MC_STATE_DISCONNECTED == currentState) {
            ...
            pClient->ipstack.disconnect(&pClient->ipstack);
            iotx_mc_set_client_state(pClient,
                                     IOTX_MC_STATE_DISCONNECTED_RECONNECTING);
            break;
        }
...
static int iotx_mc_check_handle_is_identical_ex(
    ...
    iotx_mc_topic_handle_t *messageHandler2)
{
    ...
    if (0 != strncmp(messageHandlers1->topic_filter,
                     messageHandler2->topic_filter, topicNameLen)) {
        return 1;
    }
#else
    ...
    if (messageHandlers1->topic_type == TOPIC_NAME_TYPE) {
        ...
        for (i = 0; i < MQTT_ZIP_PATH_DEFAULT_LEN; i++) {
            if (messageHandler2->topic_filter[i] !=
                messageHandlers1->topic_filter[i]) {
                return 1;
            }
        }
    } else {
...
static int iotx_mc_check_handle_is_identical(
    iotx_mc_topic_handle_t *messageHandlers1,
    iotx_mc_topic_handle_t *messageHandler2)
{
    if (iotx_mc_check_handle_is_identical_ex(messageHandlers1,
                                             messageHandler2) != 0) {
        return 1;
    }
...
static int MQTTSubscribe(iotx_mc_client_t *c, const char *topicFilter,
                         ...
                         void *pcontext)
{
    ...
    /*iotx_mc_topic_handle_t handler = {topicFilter, {messageHandler,
     * pcontext}};*/
...
#else
    if (strstr(topicFilter, "/+") != NULL ||
        strstr(topicFilter, "/#") != NULL) {
#ifdef PLATFORM_HAS_DYNMEM
        handler->topic_filter = mqtt_malloc(strlen(topicFilter) + 1);
        if (NULL == handler->topic_filter) {
            mqtt_free(handler);
        ...
        if (iotx_mc_get_zip_topic(topicFilter, strlen(topicFilter),
                                  (char *)handler->topic_filter,
    ...
    if (qos == IOTX_MQTT_QOS3_SUB_LOCAL) {
        ...
        for (idx = 0; idx < IOTX_MC_SUBHANDLE_LIST_MAX_LEN; idx++) {
            ...
            if (&c->list_sub_handle[idx] != handler &&
                0 == iotx_mc_check_handle_is_identical(&c->list_sub_handle[idx],
                                                       handler)) {
                mqtt_warning("dup sub,topic = %s", topicFilter);
                dup = 1;
            }
    ...
    len = MQTTSerialize_subscribe((unsigned char *)c->buf_send,
                                  c->buf_size_send, 0, (unsigned short)msgId, 1,
                                  &topic, (int *)&qos);
    if (len <= 0) {
#ifdef PLATFORM_HAS_DYNMEM
        mqtt_free(handler->topic_filter);
    ...
    if ((iotx_mc_send_packet(c, c->buf_send, len, &timer)) !=
        SUCCESS_RETURN) { /* send the subscribe packet */
...
static int iotx_mc_get_next_packetid(iotx_mc_client_t *c)
{
    ...
    HAL_MutexLock(c->lock_generic);
    c->packet_id =
        (c->packet_id == IOTX_MC_PACKET_ID_MAX) ? 1 : c->packet_id + 1;
    id = c->packet_id;
    HAL_MutexUnlock(c->lock_generic);
...
static int MQTTUnsubscribe(iotx_mc_client_t *c, const char *topicFilter,
                           unsigned int msgId)
{
    ...
    len = MQTTSerialize_unsubscribe(
             (unsigned char *)c->buf_send, c->buf_size_send, 0,
             (unsigned short)msgId, 1, &topic);
    if (len <= 0) {
#ifdef PLATFORM_HAS_DYNMEM
        mqtt_free(handler->topic_filter);
        mqtt_free(handler);
    ...
    HAL_MutexLock(c->lock_generic);
#ifdef PLATFORM_HAS_DYNMEM
    list_for_each_entry_safe(node, next, &c->list_sub_handle, linked_list,
                             iotx_mc_topic_handle_t)
    {
    ...
    for (idx = 0; idx < IOTX_MC_SUBHANDLE_LIST_MAX_LEN; idx++) {
        if ((c->list_sub_handle[idx].used == 1) &&
            (MQTTPacket_equals(&cur_topic,
                               (char *)c->list_sub_handle[idx].topic_filter) ||
             iotx_mc_is_topic_matched(
                 (char *)c->list_sub_handle[idx].topic_filter, &cur_topic))) {
            mqtt_debug("topic be matched");
...
int MQTTPublish(iotx_mc_client_t *c, const char *topicName,
                iotx_mqtt_topic_info_pt topic_msg)

{
    ...
    len = MQTTSerialize_publish(
        (unsigned char *)c->buf_send, c->buf_size_send, 0, topic_msg->qos,
        topic_msg->retain, topic_msg->packet_id, topic,
        (unsigned char *)topic_msg->payload, topic_msg->payload_len);
    if (len <= 0) {
        mqtt_err(
            "MQTTSerialize_publish is error, len=%d, buf_size_send=%u, "
            "payloadlen=%u",
            len, c->buf_size_send, topic_msg->payload_len);
        _reset_send_buffer(c);
        HAL_MutexUnlock(c->lock_write_buf);
        HAL_MutexUnlock(c->lock_list_pub);
        return MQTT_PUBLISH_PACKET_ERROR;
    ...
    if (topic_msg->qos > IOTX_MQTT_QOS0) {
        /* push into list */
        if (SUCCESS_RETURN !=
            iotx_mc_push_pubInfo_to(c, len, topic_msg->packet_id, &node)) {
...
#if !WITH_MQTT_ONLY_QOS0
        if (topic_msg->qos > IOTX_MQTT_QOS0) {
            /* If not even successfully sent to IP stack, meaningless to wait
             * QOS1 ack, give up waiting */
#ifdef PLATFORM_HAS_DYNMEM
            list_del(&node->linked_list);
            mqtt_free(node);
#else
...
static int MQTTDisconnect(iotx_mc_client_t *c)
{
    ...
    int len = 0;
    iotx_time_t
        timer; /* we might wait for incomplete incoming publishes to complete */
    ...
    len = MQTTSerialize_disconnect((unsigned char *)c->buf_send,
                                   c->buf_size_send);
    ...
    if (len > 0) {
        rc = iotx_mc_send_packet(c, c->buf_send, len,
                                 &timer); /* send the disconnect packet */
    }
    _reset_send_buffer(c);
    HAL_MutexUnlock(c->lock_write_buf);
    return rc;
...
void *wrapper_mqtt_init(iotx_mqtt_param_t *mqtt_params)
{
    ...
    if (NULL == pclient) {
        mqtt_err("IOTX_MC_CLIENT_MAX_COUNT too short: %d",
                 IOTX_MC_CLIENT_MAX_COUNT);
        return NULL;
    }
#endif
...
int wrapper_mqtt_connect(void *client)
{
    ...
    /* Establish TCP or TLS connection */
    do {
        mqtt_debug("calling TCP or TLS connect HAL for [%d/%d] iteration",
                   retry_cnt, retry_max);
        ...
        if (SUCCESS_RETURN != rc) {
            ...
            mqtt_debug(
                "rc = pClient->ipstack.connect() = %d, success @ [%d/%d] "
                "iteration",
...
#ifdef PLATFORM_HAS_DYNMEM
    list_for_each_entry_safe(node, next, &pClient->list_sub_handle, linked_list,
                             iotx_mc_topic_handle_t)
    {
        list_del(&node->linked_list);
        mqtt_free(node->topic_filter);
...
#else
    memset(pClient->list_sub_handle, 0,
           sizeof(iotx_mc_topic_handle_t) * IOTX_MC_SUBHANDLE_LIST_MAX_LEN);
#endif
...
int wrapper_mqtt_check_state(void *client)
{
    ...
    if (iotx_mc_get_client_state((iotx_mc_client_t *)client) ==
...
int wrapper_mqtt_subscribe(void *client, const char *topicFilter,
                           iotx_mqtt_qos_t qos,
                           ...
                           void *pcontext)
{
    ...
    if (NULL == client || NULL == topicFilter || strlen(topicFilter) == 0 ||
        !topic_handle_func) {
    ...
    if (!wrapper_mqtt_check_state(c)) {
        mqtt_err("mqtt client state is error,state = %d",
...
int wrapper_mqtt_subscribe_sync(
    void *c, const char *topic_filter, iotx_mqtt_qos_t qos,
    ...
    int timeout_ms)
{
    ...
    if (qos > IOTX_MQTT_QOS3_SUB_LOCAL) {
        mqtt_warning("Invalid qos(%d) out of [%d, %d], using %d", qos,
    ...
    do {
...
#endif
        if (ret < 0) {
            ret = wrapper_mqtt_subscribe(client, topic_filter, qos,
                                         topic_handle_func, pcontext);
...
#endif
#ifdef PLATFORM_HAS_DYNMEM
            node = (mqtt_sub_sync_node_t *)mqtt_malloc(
                sizeof(mqtt_sub_sync_node_t));
#else
            for (idx = 0; idx < IOTX_MC_SUBSYNC_LIST_MAX_LEN; idx++) {
                if (client->list_sub_sync_ack[idx].used == 0) {
                    memset(&client->list_sub_sync_ack[idx], 0,
                           sizeof(mqtt_sub_sync_node_t));
        ...
        for (idx = 0; idx < IOTX_MC_SUBSYNC_LIST_MAX_LEN; idx++) {
            ...
            if (client->list_sub_sync_ack[idx].packet_id == ret) {
                mqtt_debug("client->list_sub_sync_ack[%d].ack_type=%d cnt=%d",
                           idx, client->list_sub_sync_ack[idx].ack_type, cnt++);
                if (client->list_sub_sync_ack[idx].ack_type ==
                    ...
                    HAL_MutexUnlock(client->lock_generic);
                    return ret;
                } else if (client->list_sub_sync_ack[idx].ack_type ==
...
int wrapper_mqtt_publish(void *client, const char *topicName,
                         iotx_mqtt_topic_info_pt topic_msg)
{
    uint16_t msg_id = 0;
    int rc = FAIL_RETURN;
    iotx_mc_client_t *c = (iotx_mc_client_t *)client;
    if (c == NULL || topicName == NULL || topic_msg == NULL ||
        topic_msg->payload == NULL) {
        return NULL_VALUE_ERROR;
    }
...
#ifdef ASYNC_PROTOCOL_STACK
int wrapper_mqtt_nwk_event_handler(void *client, iotx_mqtt_nwk_event_t event,
                                   iotx_mqtt_nwk_param_t *param)
{
    int rc = FAIL_RETURN;
    iotx_mc_client_t *pClient = (iotx_mc_client_t *)client;
