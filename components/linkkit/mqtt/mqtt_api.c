...
static int _iotx_dynamic_register(iotx_http_region_types_t region,
...
static int _iotx_preauth(iotx_mqtt_region_types_t region,
                         ...
                         iotx_sign_mqtt_t *preauth_out)
{
    ...
    memcpy(device_id + strlen(device_id), meta->device_name,
           strlen(meta->device_name));

    /* setup sign_string */
    res = _iotx_generate_sign_string(device_id, meta->device_name,
                                     meta->product_key, meta->device_secret,
                                     sign_string);
    if (res < SUCCESS_RETURN) {
        return res;
    }
...
void *IOT_MQTT_Construct(iotx_mqtt_param_t *pInitParams)
{
    ...
    if (g_mqtt_client != NULL) {
        mqtt_err(
            "Already exist default MQTT connection, won't proceed another one");
    ...
    if (meta_info.product_key[0] == '\0' ||
        meta_info.product_key[IOTX_PRODUCT_KEY_LEN] != '\0') {
        mqtt_err("Invalid product key, abort!");
        return NULL;
    }
    if (meta_info.device_name[0] == '\0' ||
        meta_info.device_name[IOTX_DEVICE_NAME_LEN] != '\0') {
    ...
    if (dynamic) {
        ...
        if (meta_info.product_secret[0] == '\0' ||
            meta_info.product_secret[IOTX_PRODUCT_SECRET_LEN] != '\0') {
        ...
        HAL_GetDeviceSecret(meta_info.device_secret);
        if (meta_info.device_secret[0] == '\0' ||
            meta_info.device_secret[IOTX_DEVICE_SECRET_LEN] != '\0') {
    ...
    ret = _iotx_preauth(region, &meta_info,
                        (iotx_sign_mqtt_t *)&g_default_sign); /* type convert */
    if (ret < SUCCESS_RETURN) {
    ...
    memcpy(device_id + strlen(device_id), ".", strlen("."));
    memcpy(device_id + strlen(device_id), meta_info.device_name,
           strlen(meta_info.device_name));

    if (_sign_get_clientid(
            g_default_sign.clientid, device_id,
            (pInitParams != NULL) ? pInitParams->customize_info : NULL,
            enalbe_itls) != SUCCESS_RETURN) {
        return NULL;
    }
    ...
    if (pInitParams != NULL) {
        if (pInitParams->host && strlen(pInitParams->host)) {
            ...
            mqtt_warning("Using default hostname: '%s'",
                         g_default_sign.hostname);
        ...
        if (pInitParams->client_id && strlen(pInitParams->client_id)) {
            ...
            mqtt_warning("Using default client_id: %s",
        ...
        if (pInitParams->request_timeout_ms < CONFIG_MQTT_REQ_TIMEOUT_MIN ||
            ...
            mqtt_warning(
                "Using default request_timeout_ms: %d, configured value(%d) "
                "out of [%d, %d]",
                mqtt_params.request_timeout_ms, pInitParams->request_timeout_ms,
                CONFIG_MQTT_REQ_TIMEOUT_MIN, CONFIG_MQTT_REQ_TIMEOUT_MAX);
        } else {
            mqtt_params.request_timeout_ms = pInitParams->request_timeout_ms;
        }

        if (pInitParams->clean_session == 0 ||
            pInitParams->clean_session == 1) {
            mqtt_params.clean_session = pInitParams->clean_session;
        }

        if (pInitParams->keepalive_interval_ms <
                CONFIG_MQTT_KEEPALIVE_INTERVAL_MIN * 1000 ||
            pInitParams->keepalive_interval_ms >
                CONFIG_MQTT_KEEPALIVE_INTERVAL_MAX * 1000) {
            mqtt_warning(
                "Using default keepalive_interval_ms: %d, configured value(%d) "
                "out of [%d, %d]",
        ...
        if (!pInitParams->read_buf_size) {
            mqtt_warning("Using default read_buf_size: %d",
        ...
        if (!pInitParams->write_buf_size) {
            mqtt_warning("Using default write_buf_size: %d",
        ...
        if (pInitParams->handle_event.pcontext != NULL) {
            mqtt_params.handle_event.pcontext =
                pInitParams->handle_event.pcontext;
        }
    } else {
        mqtt_warning("Using default port: [%d]", g_default_sign.port);
        mqtt_params.port = g_default_sign.port;
    ...
    /* report device info */
    ret = iotx_report_devinfo(pclient);
    if (SUCCESS_RETURN != ret) {
                       ...
                       void *pcontext)
{
    ...
    return wrapper_mqtt_subscribe(client, topic_filter, qos, topic_handle_func,
                                  pcontext);
}
                            ...
                            void *pcontext, int timeout_ms)
{
    ...
    return wrapper_mqtt_subscribe_sync(client, topic_filter, qos,
                                       topic_handle_func, pcontext, timeout_ms);
}
...
int IOT_MQTT_Publish_Simple(void *handle, const char *topic_name, int qos,
                            void *data, int len)
{
...
int IOT_MQTT_Nwk_Event_Handler(void *handle, iotx_mqtt_nwk_event_t event,
