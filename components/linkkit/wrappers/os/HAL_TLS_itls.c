...
#define ITLS_READ_HEADER_TIMEOUT  30
#define ITLS_READ_PAYLOAD_TIMEOUT 20000
...
static int _TLSConnectNetwork(TLSDataParams_t *pTlsData, const char *addr,
                              const char *port, const char *product_key,
                              const char *product_secret)
{
...
#if defined(_PLATFORM_IS_LINUX_)
    ret = mbedtls_net_connect_timeout(&(pTlsData->fd), addr, port,
    ...
    ret = mbedtls_ssl_conf_auth_extra(&(pTlsData->conf), product_key,
                                      strlen(product_key));
    if (ret != 0) {
    ...
    ret = mbedtls_ssl_conf_auth_token(&(pTlsData->conf), product_secret,
    ...
    ret = mbedtls_ssl_conf_max_frag_len(
             &(pTlsData->conf), MBEDTLS_SSL_MAX_FRAG_LEN_1024);
    if (ret != 0) {
        platform_err(
    ...
    platform_info(
        "=========================== iTLS handshake used time(usec): %d\n",
                             ...
                             int timeout_ms)
{
    ...
    if (len > 1) { /* len > 1 indicates that trying to read payload */
        ...
        timeout_ms =
            ITLS_READ_HEADER_TIMEOUT; /* wait for at least 30ms for header type
                                         or packet length */
    }
    ...
    gettimeofday(&tv2, NULL);
    platform_info(
        "=========================== iTLS receive data(%d bytes) used "
        "time(usec): %d\n",
        readLen,
        (int)((tv2.tv_sec - tv1.tv_sec) * 1000000 +
              (tv2.tv_usec - tv1.tv_usec)));
                            ...
                            size_t ca_crt_len)
{
    ...
    if (0 != _TLSConnectNetwork(pTlsData, host, port_str, _product_key,
                                _product_secret)) {
