...
#ifdef HAL_TLS_DNS_ENHANCE
static char *g_fixed_list[3] = { "106.15.83.29", "106.15.100.2",
                                 "139.196.135.135" };
#endif
...
static int mbedtls_net_connect_timeout_backup(mbedtls_net_context *ctx,
                                              const char *host,
                                              ...
                                              unsigned int timeout)
{
    ...
    while (dns_retry++ < 8) {
        ...
        if (ret != 0) {
            platform_info(
                "getaddrinfo error[%d], res: %d, host: %s, port: %s\n",
                              ...
                              size_t client_pwd_len)
{
    ...
    do {
        ret = mbedtls_net_connect(&(pTlsData->fd), addr, port,
        ...
        ret = mbedtls_net_connect_timeout_backup(&(pTlsData->fd), addr, port,
                                                 MBEDTLS_NET_PROTO_TCP,
        ...
        if (ret == MBEDTLS_ERR_NET_UNKNOWN_HOST) {
            und_update_statis(UND_STATIS_NETWORK_FAIL_IDX,
    ...
    platform_err(
        "SKIPPING mbedtls_ssl_set_hostname() when ON_PRE or ON_DAILY defined!");
#else
    ...
    do {
        ...
        if (saved_session == NULL) {
            ...
            ret = memcmp(new_session->ticket, saved_session->ticket,
                         new_session->ticket_len);
        }
        if (ret != 0) {
            unsigned char *save_buf = HAL_Malloc(TLS_MAX_SESSION_BUF);
            if (save_buf == NULL) {
            ...
            ret = ssl_serialize_session(new_session, save_buf,
                                        TLS_MAX_SESSION_BUF, &real_session_len);
            printf(
                "mbedtls_ssl_get_session_session return 0x%04x real_len=%d\r\n",
                ret, (int)real_session_len);
            if (ret == 0) {
                char key_buf[KEY_MAX_LEN] = { 0 };
                HAL_Snprintf(key_buf, KEY_MAX_LEN - 1, KV_SESSION_KEY_FMT,
                             addr);
                ret =
                    HAL_Kv_Set(key_buf, (void *)save_buf, real_session_len, 1);
                if (ret < 0) {
