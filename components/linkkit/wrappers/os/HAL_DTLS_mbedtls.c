...
static void *_DTLSCalloc_wrapper(size_t n, size_t size)
{
    ...
    /* DTLS_TRC("INFO -- mbedtls malloc: %p %d  total used: %d  max used:
...
void _DTLSFree_wrapper(void *ptr)
{
    ...
    /* DTLS_TRC("INFO mbedtls free: %p %d  total used: %d  max used: %d\r\n",
                       ptr, mem_info->size, mbedtls_mem_used,
...
static void _DTLSLog_wrapper(void *p_ctx, int level, const char *p_file,
                             int line, const char *p_str)
{
                                       ...
                                       coap_dtls_options_t *p_options)
{
    ...
    if (result == 0) {
        ...
        if (NULL != saved_session) {
            result = mbedtls_ssl_set_session(&p_dtls_session->context,
                                             saved_session);
...
#endif
    /*    mbedtls_platform_set_calloc_free(_DTLSCalloc_wrapper,
     * _DTLSFree_wrapper);*/
    ...
    */
    ...
    if (NULL != p_dtls_session) {
        ...
        result = mbedtls_ctr_drbg_seed(
            &p_dtls_session->ctr_drbg, mbedtls_entropy_func,
            &p_dtls_session->entropy, (const unsigned char *)"IoTx",
            strlen("IoTx"));
        if (0 != result) {
...
#if defined(MBEDTLS_DEBUG_C)
        mbedtls_ssl_conf_rng(&p_dtls_session->conf, mbedtls_ctr_drbg_random,
        ...
        result = _DTLSVerifyOptions_set(
            p_dtls_session, p_options->p_ca_cert_pem, p_options->p_host);
