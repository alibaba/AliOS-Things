...
static char *g_dns_server_list[DNS_SERVER_COUNT] = { "223.5.5.5", "223.6.6.6",
                                                     "8.8.8.8" };
    ...
    /* qname */
    {
        ...
        do {
            if (domain[qname_idx] == '.' || qname_idx == strlen(domain)) {
                ...
                memcpy(&buffer[idx], &domain[section_start],
                       (qname_idx - section_start));
...
int dns_response_message(uint8_t buffer[1024], uint32_t buffer_len,
                         char *ip[DNS_RESULT_COUNT])
{
    int res = 0;
...
int dns_resolve(char dns[16], char *domain, char *ip[DNS_RESULT_COUNT])
{
    ...
    if (FD_ISSET(sock_fd, &send_recv_sets)) {
        res = sendto(sock_fd, (void *)send_message, (size_t)idx, 0,
    ...
    if (FD_ISSET(sock_fd, &send_recv_sets)) {
        ...
        if ((res = recvfrom(sock_fd, (void *)recv_message, 1024, 0,
