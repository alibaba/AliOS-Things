...
intptr_t HAL_UDP_create_without_connect(const char *host, unsigned short port)
{
    ...
    ret = setsockopt(socket_id, IPPROTO_IP, IP_RECVPKTINFO, &flag,
                          sizeof(flag));
#else  /* IP_RECVPKTINFO */
    ret = setsockopt(socket_id, IPPROTO_IP, IP_PKTINFO, &flag,
                          sizeof(flag));
...
intptr_t HAL_UDP_create(_IN_ char *host, _IN_ unsigned short port)
{
    ...
    for (ainfo = res; ainfo != NULL; ainfo = ainfo->ai_next) {
        if (AF_INET == ainfo->ai_family) {
            ...
            socket_id = socket(ainfo->ai_family, ainfo->ai_socktype,
                               ainfo->ai_protocol);
            if (socket_id < 0) {
...
int HAL_UDP_read(_IN_ intptr_t p_socket, _OU_ unsigned char *p_data,
...
int HAL_UDP_recvfrom(intptr_t sockfd, struct _network_addr_t *p_remote,
...
int HAL_UDP_sendto(intptr_t sockfd, struct _network_addr_t *p_remote,
...
int HAL_UDP_joinmulticast(intptr_t sockfd, char *p_group)
{
    ...
    int loop = 1;
    socket_id = (int)sockfd;
    err = setsockopt(socket_id, IPPROTO_IP, IP_MULTICAST_LOOP, &loop,
                     sizeof(loop));
    if (err < 0) {
    ...
    err = setsockopt(socket_id, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq,
                     sizeof(mreq));
    if (err < 0) {
