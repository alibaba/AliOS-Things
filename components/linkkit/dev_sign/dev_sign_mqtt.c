...
#ifdef SIGN_CIPHER_HMAC_SHA256
    { "signmethod", "hmacsha256" },
#elif defined(SIGN_CIPHER_HMAC_SHA1)
    { "signmethod", "hmacsha1" },
#else
#error "SIGN CIPHER is not defined"
#endif
    { "lan", "C" },
                       ...
                       const char *custom_kv, uint8_t enable_itls)
{
    ...
    for (i = 0; i < (sizeof(clientid_kv) / (sizeof(clientid_kv[0]))); i++) {
        if ((strlen(clientid_string) + strlen(clientid_kv[i][0]) +
             strlen(clientid_kv[i][1]) + 2) >= DEV_SIGN_CLIENT_ID_MAXLEN) {
            return FAIL_RETURN;
        }

        memcpy(clientid_string + strlen(clientid_string), clientid_kv[i][0],
               strlen(clientid_kv[i][0]));
        memcpy(clientid_string + strlen(clientid_string), "=", 1);
        memcpy(clientid_string + strlen(clientid_string), clientid_kv[i][1],
               strlen(clientid_kv[i][1]));
        memcpy(clientid_string + strlen(clientid_string), ",", 1);
    }

    if (custom_kv != NULL) {
        if ((strlen(clientid_string) + strlen(custom_kv) + 1) >=
            DEV_SIGN_CLIENT_ID_MAXLEN) {
            return FAIL_RETURN;
        }
        memcpy(clientid_string + strlen(clientid_string), custom_kv,
               strlen(custom_kv));
        memcpy(clientid_string + strlen(clientid_string), ",", 1);
    }
                               ...
                               const char *device_secret, char *sign_string)
{
    ...
    signsource_len = SIGN_FMT_LEN + strlen(device_id) + strlen(device_name) +
                     strlen(product_key) + strlen(TIMESTAMP_VALUE);
    ...
    memcpy(signsource + strlen(signsource), TIMESTAMP_VALUE,
           strlen(TIMESTAMP_VALUE));

    utils_hmac_sha256((uint8_t *)signsource, strlen(signsource),
                               ...
                               const char *device_secret, char *sign_string)
{
    ...
    memcpy(signsource + strlen(signsource), TIMESTAMP_VALUE,
           strlen(TIMESTAMP_VALUE));
    utils_hmac_sha1((uint8_t *)signsource, strlen(signsource), sign_hex,
                    (uint8_t *)device_secret, strlen(device_secret));
    snprintf(sign_string, 41, "%s", sign_hex);
...
int32_t IOT_Sign_MQTT(iotx_mqtt_region_types_t region,
                      iotx_dev_meta_info_t *meta, iotx_sign_mqtt_t *signout)
{
    uint16_t length = 0;
