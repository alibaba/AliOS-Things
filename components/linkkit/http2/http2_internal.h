...
typedef void (*on_user_header_callback)(int32_t stream_id, int cat,
...
typedef void (*on_user_stream_close_callback)(int32_t stream_id,
                                              uint32_t error_code);

typedef void (*on_user_frame_send_callback)(int32_t stream_id, int type,
                                            uint8_t flags);

typedef void (*on_user_frame_recv_callback)(int32_t stream_id, int type,
                                            uint8_t flags);
 ...
 */
extern http2_connection_t *iotx_http2_client_connect(void *pclient, char *url,
                                                     int port);
 ...
 */
extern int iotx_http2_client_send(http2_connection_t *conn,
                                  http2_data *h2_data);
/**
 ...
 * @return         The result. 0 is ok.
 */
extern int iotx_http2_client_recv(http2_connection_t *conn, char *data,
                                  int data_len, int *len, int timeout);
/**
 * @brief          the http2 client connect.
 * @param[in]      handler: http2 client connection handler.
 * @return         The result. 0 is ok.
...
/**
 * @brief          the http2 client receive windows size packet to update
 * window.
 * @param[in]      handler: http2 client connection handler.
 * @return         The result. 0 is ok.
 */
