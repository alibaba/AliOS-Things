...
typedef struct {
    ...
    char *channel_id; /* string return by server to identify a specific stream
                         channel, different from stream identifier which is a
                         field in http2 frame */
...
static int _set_device_info(device_conn_info_t *device_info)
{
    ...
    strncpy(g_device_info.product_key, device_info->product_key,
            IOTX_PRODUCT_KEY_LEN);
    strncpy(g_device_info.device_name, device_info->device_name,
            IOTX_DEVICE_NAME_LEN);
    strncpy(g_device_info.device_secret, device_info->device_secret,
...
static int http2_nv_copy(http2_header *nva, int start, http2_header *nva_copy,
                         int num)
{
    int i, j;
...
static void file_upload_gen_string(char *str, int type, char *para1, int para2)
{
    ...
    case CID_STRING_ENUM:
        {
            if (type == PATH_CREATE_STR_ENUM) {
                sprintf(str, "/message/pub_with_resp/sys/%s/%s/thing/%s/create",
                        g_device_info.product_key, g_device_info.device_name,
                        para1);
                ...
                sprintf(str, "clientId%sdeviceName%sproductKey%s", para1,
                        g_device_info.device_name, g_device_info.product_key);
            } else {
                sprintf(str, "%s.%s", g_device_info.product_key,
    ...
    case REAL_SIGN_STR_ENUM:
        {
            utils_hmac_sha1(para1, strlen(para1), str,
                            g_device_info.device_secret,
                            strlen(g_device_info.device_secret));
            break;
        }
    default:
        {
...
static int http2_stream_node_search(stream_handle_t *handle,
                                    ...
                                    http2_stream_node_t **p_node)
{
    ...
    *p_node = NULL;
    ...
    list_for_each_entry(search_node, &handle->stream_list, list,
                        http2_stream_node_t)
    {
...
static void on_stream_header(int32_t stream_id, int cat, const uint8_t *name,
                             uint32_t namelen, const uint8_t *value,
                             uint32_t valuelen, uint8_t flags)
{
...
#ifdef FS_ENABLED
            else if (strncmp((char *)name, "x-file-upload-id", (int)namelen) ==
                     0) {
                fs_rsp_header_val_t *rsp_data =
                    (fs_rsp_header_val_t *)user_data;
                memcpy(rsp_data->fs_upload_id, value, valuelen);
            } else if (strncmp((char *)name, "x-next-append-position",
                               (int)namelen) == 0) {
                fs_rsp_header_val_t *rsp_data =
                    (fs_rsp_header_val_t *)user_data;
                rsp_data->fs_offset = atoi((char *)value);
            } else if (strncmp((char *)name, "x-response-status",
                               (int)namelen) == 0) {
                strncpy(node->status_code, (char *)value,
                        sizeof(node->status_code) - 1);
                if (++node->rcv_hd_cnt == 2) {
                    HAL_SemaphorePost(node->semaphore);
                }
            }
    ...
    if (g_stream_handle->cbs && g_stream_handle->cbs->on_stream_header_cb) {
        g_stream_handle->cbs->on_stream_header_cb(stream_id, channel_id, cat,
                                                  name, namelen, value,
                                                  valuelen, flags, user_data);
    }
}

static void on_stream_chunk_recv(int32_t stream_id, const uint8_t *data,
                                 uint32_t len, uint8_t flags)
{
    ...
    if (g_stream_handle->cbs && g_stream_handle->cbs->on_stream_chunk_recv_cb) {
        g_stream_handle->cbs->on_stream_chunk_recv_cb(
            stream_id, channel_id, data, len, flags, user_data);
    }
}
...
static void on_stream_close(int32_t stream_id, uint32_t error_code)
{
    ...
    if (g_stream_handle->cbs && g_stream_handle->cbs->on_stream_close_cb) {
        g_stream_handle->cbs->on_stream_close_cb(stream_id, channel_id,
...
static void on_stream_frame_send(int32_t stream_id, int type, uint8_t flags)
{
    ...
    if (g_stream_handle->cbs && g_stream_handle->cbs->on_stream_frame_send_cb) {
        g_stream_handle->cbs->on_stream_frame_send_cb(stream_id, channel_id,
                                                      type, flags, user_data);
    }
}
...
static void on_stream_frame_recv(int32_t stream_id, int type, uint8_t flags)
{
    ...
    if (g_stream_handle->cbs && g_stream_handle->cbs->on_stream_frame_recv_cb) {
        g_stream_handle->cbs->on_stream_frame_recv_cb(stream_id, channel_id,
                                                      type, flags, user_data);
    }
}
...
static int reconnect(stream_handle_t *handle)
{
    ...
    conn =
        iotx_http2_client_connect_with_cb((void *)&g_client, buf, port, &my_cb);
    if (conn == NULL) {
        return -1;
    }
...
static int http2_stream_node_insert(stream_handle_t *handle, unsigned int id,
                                    void *user_data,
                                    http2_stream_node_t **p_node)
{
    http2_stream_node_t *node = NULL;
    ...
    node =
        (http2_stream_node_t *)HTTP2_STREAM_MALLOC(sizeof(http2_stream_node_t));
    if (node == NULL) {
        return FAIL_RETURN;
    }
...
void *IOT_HTTP2_Connect(device_conn_info_t *conn_info,
                        http2_stream_cb_t *user_cb)
{
    ...
    ret = HAL_ThreadCreate(&stream_handle->rw_thread, http2_io, stream_handle,
                           &thread_parms, NULL);
    if (ret != 0) {
...
int IOT_HTTP2_Stream_Open(void *hd, stream_data_info_t *info,
                          header_ext_info_t *header)
{
    ...
    {
        const http2_header static_header[] = {
            ...
            MAKE_HEADER_CS("x-auth-param-product-key",
                           g_device_info.product_key),
            MAKE_HEADER_CS("x-auth-param-device-name",
                           g_device_info.device_name),
        ...
        nva = (http2_header *)HTTP2_STREAM_MALLOC(sizeof(http2_header) *
                                                  header_num);
        if (nva == NULL) {
        ...
        header_count =
            http2_nv_copy(nva, 0, (http2_header *)static_header,
                          sizeof(static_header) / sizeof(static_header[0]));
        if (header != NULL) {
            header_count = http2_nv_copy(
                nva, header_count, (http2_header *)header->nva, header->num);
        }
        ...
        http2_stream_node_insert(handle, h2_data.stream_id, info->user_data,
                                 &node);
...
int IOT_HTTP2_Stream_Send_Message(void *hd, const char *identify,
                                  char *channel_id, char *data,
                                  uint32_t data_len, header_ext_info_t *header)
{
    ...
    while (windows_size < data_len) {
        ...
        windows_size =
            iotx_http2_get_available_window_size(handle->http2_connect);
    }
...
int IOT_HTTP2_Stream_Send(void *hd, stream_data_info_t *info,
                          header_ext_info_t *header)
{
    int rv = 0;
    http2_data h2_data;
    ...
    if (info->send_len == 0) { /* first send,need header */
        ...
        HAL_Snprintf(version, sizeof(version), "%d", get_version_int());
        {
            ...
            if (header != NULL) {
                header_count =
                    http2_nv_copy(nva, header_count,
                                  (http2_header *)header->nva, header->num);
            }
            memset(&h2_data, 0, sizeof(h2_data));
            h2_data.header = (http2_header *)nva;
            h2_data.header_count = header_count;
            ...
            if (info->packet_len + info->send_len ==
                info->stream_len) { /* last frame */
                h2_data.flag = 1;
            } else {
                h2_data.flag = 0;
            }
            ...
            HAL_MutexLock(handle->mutex);
            rv =
                iotx_http2_client_send((void *)handle->http2_connect, &h2_data);
            http2_stream_node_insert(handle, h2_data.stream_id, info->user_data,
                                     &node);
            HTTP2_STREAM_FREE(nva);
        }
    ...
    if (h2_data.flag == 1) {
        ...
        if (rv < 0 || memcmp(node->status_code, "200", 3)) {
            h2_err(
                "semaphore wait overtime or status code "
                "error,h2_data.stream_id %d\n",
                h2_data.stream_id);
            HAL_MutexLock(handle->mutex);
            http2_stream_node_remove(handle, node->stream_id);
            HAL_MutexUnlock(handle->mutex);
...
int IOT_HTTP2_Stream_Query(void *hd, stream_data_info_t *info,
                           header_ext_info_t *header)
{
    int rv = 0;
    http2_data h2_data;
...
#ifdef FS_ENABLED
int IOT_HTTP2_FS_Close(void *hd, stream_data_info_t *info,
                       header_ext_info_t *header)
{
    int rv = 0;
    http2_data h2_data;
    ...
    HAL_MutexLock(handle->mutex);
    list_for_each_entry_safe(node, next, &handle->stream_list, list,
                             http2_stream_node_t)
    {
        if (info->h2_stream_id == node->stream_id) {
            h2_info("stream_node found:stream_id= %d, Delete It",
                    node->stream_id);
        ...
        if ((node->channel_id != NULL) && (stream_id != NULL) &&
            (len == strlen(node->channel_id) &&
             !strncmp(node->channel_id, stream_id, len))) {
            list_del((list_head_t *)&node->list);
            HTTP2_STREAM_FREE(node->channel_id);
            HAL_SemaphoreDestroy(node->semaphore);
            HTTP2_STREAM_FREE(node);
