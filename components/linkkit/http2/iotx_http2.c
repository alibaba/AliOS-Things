...
static int http2_nv_copy_nghttp2_nv(nghttp2_nv *nva, int start,
                                    http2_header *nva_copy, int end);
                             ...
                             size_t length, int flags, void *user_data)
{
    ...
    NGHTTP2_DBG(
        "send_callback data len %d, session->remote_window_size=%d!\r\n",
        (int)length, session->remote_window_size);
...
/**
 * @brief      The implementation of nghttp2_recv_callback type. Here we read
 * |data| from the network and write them in |buf|. The capacity of |buf| is
 * |length| bytes. Returns the number of bytes stored in |buf|. See the
 * documentation of nghttp2_recv_callback for the details. To set this callback
 * to :type:`nghttp2_session_callbacks`, use
 ...
 * @param[in]   user_data: The |user_data| pointer is the third argument passed
 * in to the call to `nghttp2_session_client_new()` or
 * `nghttp2_session_server_new()`
 * @return      The implementation of this function must return 0 if it
                                  ...
                                  const nghttp2_frame *frame, void *user_data)
{
    ...
    if (connection->cbs && connection->cbs->on_user_frame_send_cb) {
        connection->cbs->on_user_frame_send_cb(frame->hd.stream_id,
...
/**
 * @brief       Callback function invoked by `nghttp2_session_recv()` and
 * `nghttp2_session_mem_recv()` when a frame is received. If frame is HEADERS or
 * PUSH_PROMISE, the ``nva`` and ``nvlen``member of their data structure are
 * always
 *              ``NULL`` and 0 respectively.  The header name/value pairs are
 * emitted via:type:`nghttp2_on_header_callback` To set this callback to
 * :type:`nghttp2_session_callbacks`,
 * use`nghttp2_session_callbacks_set_on_frame_send_callback()`. For HEADERS,
 * PUSH_PROMISE and DATA frames, this callback may be called after stream is
 * closed (see:type: `nghttp2_on_stream_close_callback`).  The application
 * should check that stream is still alive using its own stream management or
 * :func:`nghttp2_session_get_stream_user_data()`. Only HEADERS and DATA frame
 * can signal the end of incoming data. If ``frame->hd.flags &
 * NGHTTP2_FLAG_END_STREAM`` is nonzero, the|frame| is the last frame from the
 * remote peer in this stream. This callback won't be called for CONTINUATION
 * frames. HEADERS/PUSH_PROMISE + CONTINUATIONs are treated as single frame.
 * @param[in]   session: nghttp2 session.
 * @param[in]   frame: nghttp2 frame.
 * @param[in]   user_data: The |user_data| pointer is the third argument passed
 * in to the call to `nghttp2_session_client_new()` or
                                  ...
                                  const nghttp2_frame *frame, void *user_data)
{
    ...
    if (connection->cbs && connection->cbs->on_user_frame_recv_cb) {
        connection->cbs->on_user_frame_recv_cb(frame->hd.stream_id,
                                               frame->hd.type, frame->hd.flags);
    }
    return 0;
}
 ...
 *              We use this function to know if the response is fully received.
 * Since we just fetch 1 resource in this program, after the response is
 * received, we submit GOAWAY and close the session.
 * @param[in]   session: nghttp2 session.
 * @param[in]   stream_id: stream id.
 * @param[in]   error_code: The reason of closure.
 *              Usually one of :enum:`nghttp2_error_code`, but that is not
 * guaranteed.  The stream_user_data, which was specified in
 ...
 * immediately return :enum: `NGHTTP2_ERR_CALLBACK_FAILURE`.
 */
static int on_h2_stream_close_callback(nghttp2_session *session,
...
/**
 * @brief        Callback function invoked when a chunk of data in DATA frame is
 * received. The implementation of nghttp2_on_data_chunk_recv_callback type. We
                                       ...
                                       size_t len, void *user_data)
{
    ...
    NGHTTP2_DBG(
        "[INFO] C <----------- S (DATA chunk) stream_id [%d] :: %lu bytes\n",
...
/**
 * @brief       Callback function invoked when a header name/value pair is
 * received. The implementation of nghttp2_on_data_chunk_recv_callback type. We
 * use this function to print the received response body.
 * @param[in]   session: nghttp2 session.
 * @param[in]   frame: nghttp2 frame.
    ...
    case NGHTTP2_HEADERS:
        ...
        if (frame->headers.cat == NGHTTP2_HCAT_RESPONSE) {
            ...
            if (connection->cbs && connection->cbs->on_user_header_cb) {
                connection->cbs->on_user_header_cb(
                    frame->hd.stream_id, (int)frame->headers.cat, name, namelen,
...
/**
 * @brief        Called when nghttp2 library gets started to receive header
 * block.
 * @param[in]    session: nghttp2 session.
 * @param[in]    frame: nghttp2 frame.
 * @param[in]    user_data: The |user_data| pointer is the third argument passed
 ...
 *                  nghttp2 API offers many callback functions, but most of them
 * are optional. The send_callback is always required. Since we use
 * nghttp2_session_recv(), the recv_callback is also required.
...
int iotx_http2_client_send(http2_connection_t *conn, http2_data *h2_data)
{
    ...
    if (header != NULL && header_count != 0) {
        nva = (nghttp2_nv *)HTTP2_STREAM_MALLOC(sizeof(nghttp2_nv) *
                                                header_count);
        if (nva == NULL) {
            return -1;
        }
        nva_size =
            http2_nv_copy_nghttp2_nv(nva, nva_size, header, header_count);
    }
    /*upload to server*/
    if (data != NULL && len != 0) {
        ...
        if (nva_size != 0) {
            rv = nghttp2_submit_request(conn->session, NULL, nva, nva_size,
                                        &data_prd, NULL);
        ...
        rv = nghttp2_submit_request(conn->session, NULL, nva, nva_size, NULL,
                                    NULL);
        h2_data->stream_id = rv;
    }
...
int iotx_http2_client_recv(http2_connection_t *conn, char *data, int data_len,
                           int *len, int timeout)
{
    int rv = 0;
    int read_flag = 0;
 ...
 */
http2_connection_t *iotx_http2_client_connect(void *pclient, char *url,
                                              int port)
{
    http2_connection_t *connection;
    ...
    ret = http2_client_conn((httpclient_t *)pclient, url, port);
    if (ret != 0) {
    ...
    rv = nghttp2_session_client_new((nghttp2_session **)&connection->session,
 ...
 * @return         http2 client connection handler.
 */
http2_connection_t *iotx_http2_client_connect_with_cb(void *pclient, char *url,
                                                      int port,
                                                      http2_user_cb_t *cb)
{
    http2_connection_t *connection;
...
int iotx_http2_reset_stream(http2_connection_t *connection, int32_t stream_id)
{
    ...
    if (!nghttp2_session_get_stream_local_close(connection->session,
                                                stream_id)) {
        rv = nghttp2_submit_rst_stream(connection->session, 0, stream_id,
                                       NGHTTP2_NO_ERROR);
    }
    if (rv < 0) {
        return rv;
