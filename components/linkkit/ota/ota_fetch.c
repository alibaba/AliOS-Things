...
void *ofc_Init(char *url, int offset)
{
    ...
    if (protocol_end != NULL) {
        ...
        if ((strlen("http") == protocol_len) &&
            (memcmp("http", url, protocol_len) == 0)) {
            OTA_LOG_INFO("protocol: http");
            port = 80;
        } else if ((strlen("https") == protocol_len) &&
                   (memcmp("https", url, protocol_len) == 0)) {
    ...
    if (port_start != NULL) {
        ...
        if (port_end != NULL) {
            OTA_LOG_INFO("port exist: %.*s", (int)(port_end - port_start - 1),
                         port_start + 1);
    ...
    wrapper_http_setopt(h_odc->http_handle, IOTX_HTTPOPT_METHOD,
                        (void *)&method);
    wrapper_http_setopt(h_odc->http_handle, IOTX_HTTPOPT_HEADER,
                        (void *)header);
...
static int _ota_fetch_callback(char *ptr, int length, int total_length,
                               void *userdata)
{
...
int32_t ofc_Fetch(void *handle, char *buf, uint32_t buf_len, uint32_t timeout_s)
{
    ...
    wrapper_http_setopt(h_odc->http_handle, IOTX_HTTPOPT_TIMEOUT,
                        (void *)&http_timeout_s);
    wrapper_http_setopt(h_odc->http_handle, IOTX_HTTPOPT_RECVCALLBACK,
                        (void *)_ota_fetch_callback);
    wrapper_http_setopt(h_odc->http_handle, IOTX_HTTPOPT_RECVMAXLEN,
                        (void *)&http_recv_maxlen);
    wrapper_http_setopt(h_odc->http_handle, IOTX_HTTPOPT_RECVCONTEXT,
                        (void *)&response);
