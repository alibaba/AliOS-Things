...
static int otamqtt_GenTopicName(char *buf, size_t buf_len,
                                ...
                                const char *device_name)
{
    ...
    ret = HAL_Snprintf(buf, buf_len, "/ota/device/%s/%s/%s", ota_topic_type,
...
static int otamqtt_Publish(otamqtt_Struct_pt handle, const char *topic_type,
                           int qos, const char *msg)
{
    int ret;
     ...
     */
    ret = otamqtt_GenTopicName(topic_name, OTA_MQTT_TOPIC_LEN, topic_type,
                               handle->product_key, handle->device_name);
    if (ret < 0) {
...
static int otamqtt_publish_full_topic(otamqtt_Struct_pt handle,
...
/* decode JSON string to get firmware information, like firmware version, URL,
 * file size, MD5. */
/* return NONE */
static void otamqtt_UpgrageCb(void *pcontext, void *pclient,
                              iotx_mqtt_event_msg_pt msg)
{
    ...
    OTA_LOG_DEBUG("len=%u, topic_msg=%.*s", topic_info->payload_len,
                  topic_info->payload_len, (char *)topic_info->payload);
    ...
    if (NULL != strstr(topic_info->ptopic, "/ota/device/request")) {
        ...
        if (NULL != strstr(topic_info->payload, "url")) {
            ...
            if (NULL != handle->cb) {
                handle->cb(handle->context, topic_info->payload,
                           topic_info->payload_len,
                           IOTX_OTA_TOPIC_TYPE_DEVICE_REQUEST);
            }
        }
        ...
        if (NULL != handle->cb) {
            handle->cb(handle->context, topic_info->payload,
                       topic_info->payload_len, IOTX_OTA_TOPIC_TYPE_CONFIG_GET);
        }
    } else if (NULL != strstr(topic_info->ptopic, "/thing/config/push")) {
        OTA_LOG_DEBUG("receive config push");
        if (NULL != handle->cb) {
            if (0 != handle->cb(handle->context, topic_info->payload,
                                topic_info->payload_len,
                                IOTX_OTA_TOPIC_TYPE_CONFIG_PUSH)) {
                ...
                pvalue =
                    otalib_JsonValueOf(topic_info->payload,
                                       topic_info->payload_len, "id", &val_len);

                HAL_Snprintf(topic, OTA_MQTT_TOPIC_LEN,
                             "/sys/%s/%s/thing/config/push_reply",
                             handle->product_key, handle->device_name);
                HAL_Snprintf(message, OTA_MQTT_TOPIC_LEN,
                             "\"id\":%.*s,\"code\":\"%d\",\"data\":{}", val_len,
                             pvalue, 400);
...
void *osc_Init(const char *product_key, const char *device_name,
               void *ch_signal, ota_cb_fpt cb, void *context)
{
    int ret;
    otamqtt_Struct_pt h_osc = NULL;

    h_osc = OTA_MALLOC(sizeof(otamqtt_Struct_t));
    if (h_osc == NULL) {
     ...
     * "/ota/device/request/$(product_key)/$(device_name)" */
    ret = otamqtt_GenTopicName(h_osc->topic_request, OTA_MQTT_TOPIC_LEN,
                               "request", product_key, device_name);
    if (ret < 0) {
    ...
    ret =
        IOT_MQTT_Subscribe(ch_signal, h_osc->topic_request,
                           IOTX_MQTT_QOS3_SUB_LOCAL, otamqtt_UpgrageCb, h_osc);
#else
    ret = IOT_MQTT_Subscribe(ch_signal, h_osc->topic_request, IOTX_MQTT_QOS1,
                             otamqtt_UpgrageCb, h_osc);
#endif
    if (ret < 0) {
    ...
    /* subscribe the OTA topic:
     * "/ota/device/upgrade/$(product_key)/$(device_name)" */
    ret = otamqtt_GenTopicName(h_osc->topic_upgrade, OTA_MQTT_TOPIC_LEN,
                               "upgrade", product_key, device_name);
    if (ret < 0) {
...
#ifdef MQTT_AUTO_SUBSCRIBE
    ret =
        IOT_MQTT_Subscribe(ch_signal, h_osc->topic_upgrade,
                           IOTX_MQTT_QOS3_SUB_LOCAL, otamqtt_UpgrageCb, h_osc);
#else
    ret = IOT_MQTT_Subscribe(ch_signal, h_osc->topic_upgrade, IOTX_MQTT_QOS1,
                             otamqtt_UpgrageCb, h_osc);
#endif
    if (ret < 0) {
        OTA_LOG_ERROR("mqtt subscribe failed");
    ...
    ret = HAL_Snprintf(h_osc->topic_config_get, OTA_MQTT_TOPIC_LEN,
                       "/sys/%s/%s/thing/config/get_reply", product_key,
                       device_name);
    if (ret < 0) {
...
#ifdef MQTT_AUTO_SUBSCRIBE
    ret =
        IOT_MQTT_Subscribe(ch_signal, h_osc->topic_config_get,
                           IOTX_MQTT_QOS3_SUB_LOCAL, otamqtt_UpgrageCb, h_osc);
#else
    ret = IOT_MQTT_Subscribe(ch_signal, h_osc->topic_config_get, IOTX_MQTT_QOS0,
                             otamqtt_UpgrageCb, h_osc);
#endif
    if (ret < 0) {
        OTA_LOG_ERROR("mqtt subscribe failed");
    ...
    ret =
        HAL_Snprintf(h_osc->topic_config_push, OTA_MQTT_TOPIC_LEN,
                     "/sys/%s/%s/thing/config/push", product_key, device_name);
    if (ret < 0) {
        OTA_LOG_ERROR("generate topic name of config get failed");
        goto do_exit;
    }
...
#ifdef MQTT_AUTO_SUBSCRIBE
    ret =
        IOT_MQTT_Subscribe(ch_signal, h_osc->topic_config_push,
                           IOTX_MQTT_QOS3_SUB_LOCAL, otamqtt_UpgrageCb, h_osc);
#else
    ret = IOT_MQTT_Subscribe(ch_signal, h_osc->topic_config_push,
                             IOTX_MQTT_QOS0, otamqtt_UpgrageCb, h_osc);
#endif
    if (ret < 0) {
        OTA_LOG_ERROR("mqtt subscribe failed");
        goto do_exit;
...
/* request the config */
int osc_RequestConfig(void *handle, const char *topic_name,
                      iotx_mqtt_topic_info_pt topic_msg)
{
