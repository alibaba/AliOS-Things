...
static void otacoap_response_handler(void *arg, void *p_response)
{
    ...
    if ((NULL != h_osc_coap) && (NULL != p_payload)) {
        h_osc_coap->cb(h_osc_coap->context, (const char *)p_payload,
                       (uint32_t)len);
    }
}
                                ...
                                const char *device_name)
{
    ...
    ret = OTA_SNPRINTF(buf, buf_len, "/topic/ota/device/%s/%s/%s",
                       ota_topic_type, product_key, device_name);
...
static int otacoap_Publish(otacoap_Struct_pt handle, const char *topic_type,
                           const char *msg)
{
    int ret;
    ...
    ret = IOT_CoAP_SendMessage(handle->coap, (char *)uri, &message);
    if (ret != IOTX_SUCCESS) {
               ...
               void *ch_signal, ota_cb_fpt cb, void *context)
{
    ...
    h_osc = OTA_MALLOC(sizeof(otacoap_Struct_t));
    if (h_osc == NULL) {
