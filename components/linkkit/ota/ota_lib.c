...
const char *otalib_JsonValueOf(const char *json, uint32_t json_len,
                               const char *key, uint32_t *val_len)
{
    int length;
    const char *val;
    val = json_get_value_by_name((char *)json, json_len, (char *)key, &length,
                                 NULL);
    if (NULL != val) {
...
int otalib_GetFirmwareFixlenPara(const char *json_doc, size_t json_doc_len,
                                 const char *key, char *dest, size_t dest_len)
{
    const char *pvalue;
    uint32_t val_len;

    pvalue = otalib_JsonValueOf(json_doc, json_doc_len, key, &val_len);
    if (pvalue == NULL) {
...
/* 0, successful; -1, failed */
int otalib_GetFirmwareVarlenPara(const char *json_doc, size_t json_doc_len,
...
int otalib_GetParams(const char *json_doc, uint32_t json_len, char **url,
                     char **version, char *md5, uint32_t *file_size)
{
#define OTA_FILESIZE_STR_LEN (16)
    ...
    /* get version */
    if (0 !=
        otalib_GetFirmwareVarlenPara(json_doc, json_len, "version", version)) {
    ...
    /* get file size */
    if (0 != otalib_GetFirmwareFixlenPara(json_doc, json_len, "size",
                                          file_size_str,
                                          OTA_FILESIZE_STR_LEN)) {
...
int otalib_GetConfigParams(const char *json_doc, uint32_t json_len,
                           char **configId, uint32_t *configSize, char **sign,
...
#define OTA_FILESIZE_STR_LEN (16)
    ...
    /* get configId */
    if (0 != otalib_GetFirmwareVarlenPara(json_doc, json_len, "configId",
                                          configId)) {
    ...
    /* get configSize */
    if (0 != otalib_GetFirmwareFixlenPara(json_doc, json_len, "configSize",
                                          file_size_str,
                                          OTA_FILESIZE_STR_LEN)) {
        OTA_LOG_ERROR("get value of size key failed");
        return -1;
    ...
    *configSize = atoi(file_size_str);
    ...
    /* get signMethod */
    if (0 != otalib_GetFirmwareVarlenPara(json_doc, json_len, "signMethod",
                                          signMethod)) {
    ...
    /* get getType */
    if (0 !=
        otalib_GetFirmwareVarlenPara(json_doc, json_len, "getType", getType)) {
...
/* 0, successful; -1, failed */
int otalib_GenInfoMsg(char *buf, size_t buf_len, uint32_t id,
                      const char *version)
{
    int ret;
    ret = HAL_Snprintf(buf, buf_len,
                       "{\"id\":%d,\"params\":{\"version\":\"%s\"}}", id,
                       version);
...
/* and then copy to @buf. */
/* 0, successful; -1, failed */
int otalib_GenReportMsg(char *buf, size_t buf_len, uint32_t id, int progress,
