...
static int ota_callback(void *pcontext, const char *msg, uint32_t msg_len,
                        iotx_ota_topic_types_t type)
{
    const char *pvalue;
    uint32_t val_len;
    ...
    case IOTX_OTA_TOPIC_TYPE_DEVICE_UPGRATE:
        {
            ...
            if (!((strlen("success") == val_len) &&
                  (0 == strncmp(pvalue, "success", val_len)))) {
            ...
            if (0 != otalib_GetParams(pvalue, val_len, &h_ota->purl,
                                      &h_ota->version, h_ota->md5sum,
                                      &h_ota->size_file)) {
            ...
            h_ota->ch_fetch =
                             ofc_Init(h_ota->purl, h_ota->size_fetched);
    ...
    case IOTX_OTA_TOPIC_TYPE_CONFIG_GET:
        {
            ...
            /* check whether is positive message */
            if (!((strlen("200") == val_len) &&
                  (0 == strncmp(pvalue, "200", val_len)))) {
                OTA_LOG_ERROR("fail state of json doc of OTA");
                return -1;
            }
            ...
            if (0 != otalib_GetConfigParams(
                         pvalue, val_len, &h_ota->configId, &h_ota->configSize,
                         &h_ota->sign, &h_ota->signMethod, &h_ota->cota_url,
                         &h_ota->getType)) {
                   ...
                   void *ch_signal)
{
    ...
    h_ota = OTA_MALLOC(sizeof(OTA_Struct_t));
    if (h_ota == NULL) {
    ...
    h_ota->ch_signal =
        osc_Init(product_key, device_name, ch_signal, ota_callback, h_ota);
...
#define MSG_INFORM_LEN (128)
    ...
    msg_informed = OTA_MALLOC(MSG_INFORM_LEN);
    if (msg_informed == NULL) {
...
#define MSG_REQUEST_LEN (128)
    ...
    msg_informed = OTA_MALLOC(MSG_REQUEST_LEN);
    if (msg_informed == NULL) {
        OTA_LOG_ERROR("allocate for msg_informed failed");
        h_ota->err = IOT_OTAE_NOMEM;
        return -1;
...
int IOT_OTA_ReportProgress(void *handle, IOT_OTA_Progress_t progress,
                           const char *msg)
{
#define MSG_REPORT_LEN (256)
    ...
    msg_reported = OTA_MALLOC(MSG_REPORT_LEN);
    if (msg_reported == NULL) {
        OTA_LOG_ERROR("allocate for msg_reported failed");
        h_ota->err = IOT_OTAE_NOMEM;
        return -1;
    }

    ret = otalib_GenReportMsg(msg_reported, MSG_REPORT_LEN, h_ota->id, progress,
                              msg);
    if (0 != ret) {
...
int iotx_ota_get_config(void *handle, const char *configScope,
                        const char *getType, const char *attributeKeys)
{
#define MSG_REPORT_LEN (256)
    ...
    msg_get = OTA_MALLOC(MSG_REPORT_LEN);
    if (msg_get == NULL) {
        OTA_LOG_ERROR("allocate for msg_reported failed");
        h_ota->err = IOT_OTAE_NOMEM;
        return -1;
    ...
    if (0 > HAL_Snprintf(topic, OTA_MQTT_TOPIC_LEN,
                         "/sys/%s/%s/thing/config/get", h_ota->product_key,
                         h_ota->device_name)) {
        goto do_exit;
    };

    if (0 > HAL_Snprintf(msg_get, MSG_REPORT_LEN,
                         "{\"id\" : "
                         "%d,\"version\":\"1.0\",\"params\":{\"configScope\":"
                         "\"%s\",\"getType\":\"%s\",\"attributeKeys\":\"%s\"},"
                         "\"method\":\"thing.config.get\"}",
                         h_ota->id, configScope, getType, attributeKeys)) {
        goto do_exit;
    };
...
int IOT_OTA_IsFetching(void *handle)
{
    ...
    return (IOT_OTAS_PUSHED == h_ota->state ||
            IOT_OTAS_FETCHING == h_ota->state);
}
                       ...
                       uint32_t timeout_s)
{
    ...
    if (ret < 0) {
        ...
        IOT_OTA_ReportProgress(h_ota, IOT_OTAP_FETCH_PERCENTAGE_MIN,
                               "Enter in downloading state");
    }
...
int IOT_OTA_Ioctl(void *handle, IOT_OTA_CmdType_t type, void *buf, int buf_len)
{
    ...
    case IOT_OTAG_CHECK_FIRMWARE:
        if ((4 != buf_len) || (0 != ((unsigned long)buf & 0x3))) {
            ...
            h_ota->err = IOT_OTAE_INVALID_STATE;
            OTA_LOG_ERROR(
                "Firmware can be checked in IOT_OTAS_FETCHED state only");
            return -1;
        } else {
            char md5_str[33];
    ...
    case IOT_OTAG_CHECK_CONFIG:
        if ((4 != buf_len) || (0 != ((unsigned long)buf & 0x3))) {
            ...
            h_ota->err = IOT_OTAE_INVALID_STATE;
            OTA_LOG_ERROR(
                "Config can be checked in IOT_OTAS_FETCHED state only");
            return -1;
        } else {
            if (0 ==
                strncmp(h_ota->signMethod, "Md5", strlen(h_ota->signMethod))) {
                char md5_str[33];
                otalib_MD5Finalize(h_ota->md5, md5_str);
            ...
            if (0 == strncmp(h_ota->signMethod, "Sha256",
                             strlen(h_ota->signMethod))) {
                char sha256_str[65];
