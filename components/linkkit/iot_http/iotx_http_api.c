...
#define HTTP_LITE_JSON_VALUE_OF(key, src) \
    LITE_json_value_of(key, src, 0x1234, "http.api")
...
#define IOTX_HTTP_ONLINE_SERVER_URL \
    "https://iot-as-http.cn-shanghai.aliyuncs.com"
...
#define IOTX_HTTP_HEADER_CONTENT_TYPE_STR \
...
#define IOTX_HTTP_UPSTREAM_HEADER_STR \
    ...
    "%s" IOTX_HTTP_HEADER_END_STR IOTX_HTTP_HEADER_CONTENT_TYPE_STR
...
/*
  ...
  The /auth interface is used to get access token before any data transform
  which need to be requested just one time. The device must call /auth to get
  the token before transfer any data. Every data transfer need to be with access
  token. You need to request for a new token if the current token have been
  expired. Token can be cached in local devices, which has 48 hours term of
  validity. POST /auth HTTP/1.1 Host: iot-as-http.cn-shanghai.aliyuncs.com
  Content-Type: application/json
  body:
  {"version":"default","clientId":"xxxxx","signmethod":"hmacsha1","sign":"xxxxxxxxxx","productKey":"xxxxxx","deviceName":"xxxxxxx","timestamp":"xxxxxxx"}
*/

static int iotx_calc_sign(const char *p_device_secret, const char *p_msg,
                          char *sign)
{
    http_info("| method: %s", IOTX_SHA_METHOD);
    utils_hmac_sha1(p_msg, strlen(p_msg), sign, p_device_secret,
...
{
    LITE_snprintf(buf, IOTX_HTTP_URL_LEN_MAX, "%s/%s",
                  IOTX_HTTP_ONLINE_SERVER_URL, IOTX_HTTP_AUTH_STR);
...
{
    LITE_snprintf(buf, IOTX_HTTP_URL_LEN_MAX, "%s%s",
                  IOTX_HTTP_ONLINE_SERVER_URL, topic_path);
    http_info("construct_full_http_upstream_url is %s", buf);
    return 0;
}

static int http_report_func(void *handle, const char *topic_name, int req_ack,
                            void *data, int len)
{
...
static void *verify_iotx_http_context(void *handle)
{
    ...
    if (NULL == iotx_http_context || NULL == iotx_http_context_bak ||
...
static int _http_recv_callback(char *ptr, int length, int total_length,
                               void *userdata)
{
...
void *IOT_HTTP_Init(iotx_http_param_t *pInitParams)
{
    ...
    */
    ...
    iotx_http_context->p_devinfo =
        (iotx_device_info_t *)HTTP_API_MALLOC(sizeof(iotx_device_info_t));
    ...
    memset(iotx_http_context->p_devinfo, 0x00, sizeof(iotx_device_info_t));
    strncpy(iotx_http_context->p_devinfo->device_id, p_devinfo->device_id,
            strlen(p_devinfo->device_id));
    strncpy(iotx_http_context->p_devinfo->product_key, p_devinfo->product_key,
            strlen(p_devinfo->product_key));
    strncpy(iotx_http_context->p_devinfo->device_secret,
            p_devinfo->device_secret, strlen(p_devinfo->device_secret));
    strncpy(iotx_http_context->p_devinfo->device_name, p_devinfo->device_name,
            strlen(p_devinfo->device_name));
...
void IOT_HTTP_DeInit(void **handle)
{
    ...
    iotx_http_context = verify_iotx_http_context(*handle);
...
int IOT_HTTP_DeviceNameAuth(void *handle)
{
    ...
    /* "sign": "xxxxxxxxxxxxx",//必填，signmethod(deviceSecret,content), content
     * = 将所有提交给服务器的参数（version,sign,signmethod除外）,
     * 按照字母顺序排序, 然后将参数值依次拼接，无拼接符号 */
    ...
    */
    ...
    iotx_calc_sign(iotx_http_context->p_devinfo->device_secret, p_msg_unsign,
                   sign);

    /* to save stack memory*/
    len = calc_snprintf_string_length(
        IOTX_HTTP_AUTH_DEVICENAME_STR, "default",
        iotx_http_context->p_devinfo->device_id, IOTX_SHA_METHOD, sign,
        iotx_http_context->p_devinfo->product_key,
        iotx_http_context->p_devinfo->device_name, timestamp);
    ...
    len = HAL_Snprintf(req_payload, len + 1, IOTX_HTTP_AUTH_DEVICENAME_STR,
                       "default", iotx_http_context->p_devinfo->device_id,
                       IOTX_SHA_METHOD, sign,
                       iotx_http_context->p_devinfo->product_key,
                       iotx_http_context->p_devinfo->device_name, timestamp);
    ...
    /*
    Test Code
    iotx_http_context->p_auth_token =
    "eyJ0eXBlIjoiSldUIiwiYWxnIjoiaG1hY3NoYTEifQ.eyJleHBpcmUiOjE1MDQ3ODE4MzQ5MDAsInRva2VuIjoiM2EyZTRmYzMyNjk5NDE0Y2E3MDFjNzIzNzI1YjIyNDgifQ.e87AFhkvNKiqF5xdgm1P47f9DwY";
    ...
    */
    ...
    wrapper_http_setopt(http_handle, IOTX_HTTPOPT_TIMEOUT,
                        (void *)&http_timeout_ms);
    wrapper_http_setopt(http_handle, IOTX_HTTPOPT_RECVCALLBACK,
                        (void *)_http_recv_callback);
    wrapper_http_setopt(http_handle, IOTX_HTTPOPT_RECVMAXLEN,
                        (void *)&http_recv_maxlen);
    wrapper_http_setopt(http_handle, IOTX_HTTPOPT_RECVCONTEXT,
                        (void *)&recv_message);
    ...
    body:
    {
      ...
      "info": {
        "token":
    "eyJ0eXBlIjoiSldUIiwiYWxnIjoiaG1hY3NoYTEifQ.eyJleHBpcmUiOjE1MDI1MzE1MDc0NzcsInRva2VuIjoiODA0ZmFjYTBiZTE3NGUxNjliZjY0ODVlNWNiNDg3MTkifQ.OjMwu29F0CY2YR_6oOyiOLXz0c8"
      }
    }
    */
...
int IOT_HTTP_SendMessage(void *handle, iotx_http_message_param_t *msg_param)
{
    ...
    */
    ...
    msg_param->request_payload_len =
        msg_param->request_payload_len > payload_len
            ? payload_len
            : msg_param->request_payload_len;
    ...
    len = strlen(IOTX_HTTP_HEADER_PASSWORD_STR) +
          strlen(iotx_http_context->p_auth_token) +
          strlen(IOTX_HTTP_HEADER_KEEPALIVE_STR) +
          strlen(IOTX_HTTP_HEADER_END_STR) +
          strlen(IOTX_HTTP_HEADER_CONTENT_TYPE_STR);
    http_header = HTTP_API_MALLOC(len + 1);
    if (NULL == http_header) {
        http_err("Allocate memory for httpc->header failed");
        goto do_exit;
    }
    LITE_snprintf(http_header, len + 1, IOTX_HTTP_UPSTREAM_HEADER_STR,
                  iotx_http_context->p_auth_token);
    ...
    /* Send Request and Get Response */
    ret = wrapper_http_perform(http_handle, msg_param->request_payload,
                               msg_param->request_payload_len);
    if (ret < 0) {
        http_err("wrapper_http_perform error");
    ...
    */
    ...
    /* info.messageId */
    pvalue =
        HTTP_LITE_JSON_VALUE_OF("info.messageId", msg_param->response_payload);
    if (NULL == pvalue) {
...
void IOT_HTTP_Disconnect(void *handle)
{
    ...
    verify_iotx_http_context(handle);
}
