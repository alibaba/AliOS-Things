...
static int _parse_dynreg_value(char *payload, char *key, int *pos, int *start,
                               int *end)
{
    int idx = 0;
    /* printf("=====%s\n",&payload[*pos]); */
...
static int _calc_dynreg_sign(char product_key[IOTX_PRODUCT_KEY_LEN],
                             ...
                             char sign[DYNREG_SIGN_LENGTH])
{
    ...
    sign_source_len = strlen(dynamic_register_sign_fmt) + strlen(device_name) +
                      strlen(product_key) + strlen(random) + 1;
    ...
    HAL_Snprintf((char *)sign_source, sign_source_len,
                 dynamic_register_sign_fmt, device_name, product_key, random);

    utils_hmac_sha256(sign_source, strlen((const char *)sign_source),
                      (uint8_t *)product_secret, strlen(product_secret),
                      signnum);
...
static int _recv_callback(char *ptr, int length, int total_length,
                          void *userdata)
{
...
static int _fetch_dynreg_http_resp(char *request_payload,
                                   char *response_payload,
                                   iotx_http_region_types_t region,
                                   char device_secret[IOTX_DEVICE_SECRET_LEN])
{
    ...
    res = wrapper_http_perform(http_handle, request_payload,
    ...
    dynreg_info("Dynamic Register Code: %.*s", end - start + 1,
                &response_payload[start]);
    ...
    /*  dynreg_info("value: %.*s\n",data_end - data_start +
     * 1,&response_payload[data_start]); */

    _parse_dynreg_result(&response_payload[data_start + 1], "deviceSecret",
                         &start, &end);
    dynreg_info("Dynamic Register Device Secret: %.*s", end - start + 1,
                &response_payload[data_start + 1 + start]);

    if (end - start + 1 > IOTX_DEVICE_SECRET_LEN) {
        return FAIL_RETURN;
    }

    memcpy(device_secret, &response_payload[data_start + 1 + start],
           end - start + 1);

    return SUCCESS_RETURN;
}

int32_t _http_dynamic_register(iotx_http_region_types_t region,
                               iotx_dev_meta_info_t *meta)
{
    int res = 0, dynamic_register_request_len = 0;
    char sign[DYNREG_SIGN_LENGTH] = { 0 };
    char random[DYNREG_RANDOM_KEY_LENGTH + 1] = { 0 };
    const char *dynamic_register_format =
        "productKey=%s&deviceName=%s&random=%s&sign=%s&signMethod=%s";
    ...
    /* Calcute Signature */
    res = _calc_dynreg_sign(meta->product_key, meta->product_secret,
                            meta->device_name, random, sign);
    if (res != SUCCESS_RETURN) {
    ...
    dynamic_register_request_len =
        strlen(dynamic_register_format) + strlen(meta->product_key) +
        strlen(meta->device_name) + strlen(random) + strlen(sign) +
        strlen(DYNREG_SIGN_METHOD_HMACSHA256) + 1;
    ...
    HAL_Snprintf(dynamic_register_request, dynamic_register_request_len,
                 dynamic_register_format, meta->product_key, meta->device_name,
                 random, sign, DYNREG_SIGN_METHOD_HMACSHA256);
    ...
    res = _fetch_dynreg_http_resp(dynamic_register_request,
                                  dynamic_register_response, region,
                                  meta->device_secret);
...
#ifdef MQTT_DYNAMIC_REGISTER
static int _mqtt_dynreg_sign_password(iotx_dev_meta_info_t *meta_info,
                                      iotx_sign_mqtt_t *signout, char *rand)
{
    ...
    signsource_len = strlen(sign_fmt) + strlen(meta_info->device_name) + 1 +
                     strlen(meta_info->product_key) + strlen(rand) + 1;
    ...
    memcpy(signsource + strlen(signsource), meta_info->device_name,
           strlen(meta_info->device_name));
    memcpy(signsource + strlen(signsource), "productKey", strlen("productKey"));
    memcpy(signsource + strlen(signsource), meta_info->product_key,
           strlen(meta_info->product_key));
    memcpy(signsource + strlen(signsource), "random", strlen("random"));
    memcpy(signsource + strlen(signsource), rand, strlen(rand));

    utils_hmac_sha256((const uint8_t *)signsource, strlen(signsource),
                      (uint8_t *)meta_info->product_secret,
...
static int32_t _mqtt_dynreg_sign_clientid(iotx_dev_meta_info_t *meta_info,
                                          iotx_sign_mqtt_t *signout, char *rand)
{
    ...
    clientid_len = strlen(meta_info->product_key) + 1 +
                   strlen(meta_info->device_name) + strlen(clientid1) +
                   strlen(rand) + strlen(clientid2) + 1;
    if (clientid_len >= DEV_SIGN_CLIENT_ID_MAXLEN) {
        return ERROR_DEV_SIGN_CLIENT_ID_TOO_SHORT;
    }
    memset(signout->clientid, 0, clientid_len);
    memcpy(signout->clientid, meta_info->product_key,
           strlen(meta_info->product_key));
    memcpy(signout->clientid + strlen(signout->clientid), ".", strlen("."));
    memcpy(signout->clientid + strlen(signout->clientid),
           meta_info->device_name, strlen(meta_info->device_name));
    memcpy(signout->clientid + strlen(signout->clientid), clientid1,
           strlen(clientid1));
    memcpy(signout->clientid + strlen(signout->clientid), rand, strlen(rand));
    memcpy(signout->clientid + strlen(signout->clientid), clientid2,
           strlen(clientid2));

    return SUCCESS_RETURN;
}

void _mqtt_dynreg_topic_handle(void *pcontext, void *pclient,
    ...
    case IOTX_MQTT_EVENT_PUBLISH_RECEIVED:
        {
            ...
            /* parse secret */
            res = infra_json_value((char *)topic_info->payload,
                                   topic_info->payload_len, "deviceSecret",
                                   strlen("deviceSecret"), &device_secret,
                                   &device_secret_len);
            if (res == SUCCESS_RETURN) {
                memcpy(ds, device_secret + 1, device_secret_len - 2);
                memcpy(device_secret + 1 + 5, asterisk, strlen(asterisk));
                dynreg_info("Topic  : %.*s", topic_info->topic_len,
                            topic_info->ptopic);
                dynreg_info("Payload: %.*s", topic_info->payload_len,
                            topic_info->payload);
            }
        }
        break;
...
int32_t _mqtt_dynamic_register(iotx_http_region_types_t region,
                               iotx_dev_meta_info_t *meta)
{
    void *pClient = NULL;
    ...
    if (IOTX_HTTP_REGION_CUSTOM == region) {
        ...
        length =
            strlen(meta->product_key) + strlen(g_infra_mqtt_domain[region]) + 2;
        if (length >= DEV_SIGN_HOSTNAME_MAXLEN) {
            return ERROR_DEV_SIGN_HOST_NAME_TOO_SHORT;
        }
        memset(signout.hostname, 0, DEV_SIGN_HOSTNAME_MAXLEN);
        ...
        memcpy(signout.hostname + strlen(signout.hostname),
               g_infra_mqtt_domain[region],
               strlen(g_infra_mqtt_domain[region]));
    }
    ...
    memcpy(signout.username + strlen(signout.username), meta->product_key,
           strlen(meta->product_key));
    ...
    res = wrapper_mqtt_subscribe(pClient, "/ext/register", IOTX_MQTT_QOS1,
                                 _mqtt_dynreg_topic_handle, device_secret);
    if (res < 0) {
        wrapper_mqtt_release(&pClient);
        return FAIL_RETURN;
    }
...
int32_t IOT_Dynamic_Register(iotx_http_region_types_t region,
                             iotx_dev_meta_info_t *meta)
{
#ifdef MQTT_DYNAMIC_REGISTER
