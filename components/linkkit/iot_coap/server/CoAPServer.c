...
CoAPContext *CoAPServer_init()
{
    ...
    if (NULL == g_context) {
        ...
        HAL_ThreadCreate(&g_coap_thread, CoAPServer_yield, (void *)g_context,
...
int CoAPServerResp_send(CoAPContext *context, NetworkAddr *remote,
                        ...
                        const char *paths, CoAPSendMsgHandler callback,
                        unsigned short *msgid, char qos)
{
    int ret = COAP_SUCCESS;
    ...
    CoAPMessageType_set(
        &response, qos == 0 ? COAP_MESSAGE_TYPE_NON : COAP_MESSAGE_TYPE_CON);
