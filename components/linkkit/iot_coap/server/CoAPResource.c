...
int CoAPResource_deinit(CoAPContext *context)
{
    ...
    HAL_MutexLock(ctx->resource.list_mutex);
    list_for_each_entry_safe(node, next, &ctx->resource.list, reslist,
                             CoAPResource)
    {
        ...
        infra_hex2str((unsigned char *)node->path, COAP_MAX_PATH_CHECKSUM_LEN,
                      tmpbuf);
...
CoAPResource *CoAPResource_create(const char *path, path_type_t path_type,
                                  ...
                                  CoAPRecvMsgHandler callback)
{
    ...
    if (path_type == PATH_NORMAL) {
        ...
        CoAPPathMD5_sum(path, strlen(path), resource->path,
                        COAP_PATH_DEFAULT_SUM_LEN);
    } else {
                          ...
                          unsigned int maxage, CoAPRecvMsgHandler callback)
{
    ...
    if (strstr(path, "/#") != NULL) {
        ...
        CoAPPathMD5_sum(path, strlen(path), path_calc,
                        COAP_PATH_DEFAULT_SUM_LEN);
    }
    ...
    if (0 == exist) {
        newnode = CoAPResource_create(path, type, permission, ctype, maxage,
                                      callback);
        if (NULL != newnode) {
            COAP_DEBUG("CoAPResource_register, context:%p, new node", ctx);
            list_add_tail(&newnode->reslist, &ctx->resource.list);
            ctx->resource.count++;
            COAP_DEBUG("Register new resource %s success, count: %d", path,
                       ctx->resource.count);
        } else {
