...
int CoAPObsServer_deinit(CoAPContext *context)
{
    ...
    HAL_MutexLock(ctx->obsserver.list_mutex);
    list_for_each_entry_safe(node, next, &ctx->obsserver.list, obslist,
                             CoapObserver)
    {
        list_del(&node->obslist);
        COAP_DEBUG("Delete %s:%d from observe server", node->remote.addr,
                   node->remote.port);
        coap_free(node);
    }
    ctx->obsserver.count = 0;
...
int CoAPObsServer_add(CoAPContext *context, const char *path,
    ...
    if (NULL != resource && COAP_SUCCESS == ret && 0 == observe) {
        ...
        list_for_each_entry(node, &ctx->obsserver.list, obslist, CoapObserver)
        {
            if ((node->p_resource_of_interest == resource) &&
                ...
                (0 ==
                 memcmp(node->remote.addr, remote->addr, NETWORK_ADDR_LEN))) {
                COAP_DEBUG("The observe client %s:%d already exist,update it",
        ...
        if (ctx->obsserver.count >= ctx->obsserver.maxcount) {
            HAL_MutexUnlock(ctx->obsserver.list_mutex);
            coap_free(obs);
            COAP_INFO("Cur have %d observer, max allow %d",
                      ctx->obsserver.count, ctx->obsserver.maxcount);
            return COAP_ERROR_DATA_SIZE;
        } else {
            list_add_tail(&obs->obslist, &ctx->obsserver.list);
            ctx->obsserver.count++;
            COAP_DEBUG("Create a observe node, cur have %d nodes",
                       ctx->obsserver.count);
            HAL_MutexUnlock(ctx->obsserver.list_mutex);
            return COAP_SUCCESS;
        }
...
int CoAPObsServer_notify(CoAPContext *context, const char *path,
                         unsigned char *payload, unsigned short payloadlen,
                         CoAPDataEncrypt handler)
{
    ...
    if (NULL != resource) {
        ...
        list_for_each_entry(node, &ctx->obsserver.list, obslist, CoapObserver)
        {
            if (node->p_resource_of_interest == resource) {
                ...
                CoAPUintOption_add(&message, COAP_OPTION_OBSERVE,
                                   node->observer_sequence_num++);
                CoAPUintOption_add(&message, COAP_OPTION_CONTENT_FORMAT,
                                   node->ctype);
                CoAPUintOption_add(&message, COAP_OPTION_MAXAGE,
                                   resource->maxage);
                COAP_DEBUG("Send notify message path %s to remote %s:%d ", path,
                           node->remote.addr, node->remote.port);
                ...
                if (NULL != handler) {
                    src.len = payloadlen;
                    src.data = payload;
                    ret = handler(context, path, &node->remote, &message, &src,
                                  &dest);
                    if (COAP_SUCCESS == ret) {
...
int CoAPObsClient_deinit(CoAPContext *context)
{
    ...
    HAL_MutexLock(ctx->obsclient.list_mutex);
    list_for_each_entry_safe(node, next, &ctx->obsclient.list, obslist,
                             CoAPObservable)
    {
        list_del(&node->obslist);
        coap_free(node);
...
int CoAPObsClient_add(CoAPContext *context, CoAPMessage *message,
                      NetworkAddr *remote, CoAPSendNode *sendnode)
{
    CoAPObservable *node = NULL, *next = NULL;
    CoAPIntContext *ctx = (CoAPIntContext *)context;
    ...
    if (COAP_SUCCESS == CoAPOption_present(message, COAP_OPTION_OBSERVE)) {
        ...
        if (NULL == sendnode) { /* Not the first response */
            ...
            HAL_MutexLock(ctx->obsclient.list_mutex);
            list_for_each_entry(node, &ctx->obsclient.list, obslist,
                                CoAPObservable)
            {
                if (0 != node->tokenlen &&
                    node->tokenlen == message->header.tokenlen &&
                    0 == memcmp(node->token, message->token, node->tokenlen)) {
                    CoAPUintOption_get(message, COAP_OPTION_MAXAGE,
                                       &node->max_age);
                    if (NULL != node->callback) {
                        COAP_DEBUG("Call the observe client callback %p",
                                   node->callback);
                        node->callback(ctx, COAP_REQUEST_SUCCESS,
                                       node->userdata, remote, message);
                    } else {
            ...
            if (!found && ctx->obsclient.count < ctx->obsclient.maxcount) {
                ...
                if (NULL != newnode) {
                    ...
                    memcpy(newnode->token, message->token,
                           message->header.tokenlen);
                ...
                COAP_INFO("Cur have %d obsclient, max allow %d",
                          ctx->obsclient.count, ctx->obsclient.maxcount);
            }
            HAL_MutexUnlock(ctx->obsclient.list_mutex);
        }
    } else {
...
int CoAPObsClient_delete(CoAPContext *context, CoAPMessage *message)
{
    ...
    if (COAP_MSG_CODE_GET == message->header.code) {
        if (COAP_SUCCESS == CoAPOption_present(message, COAP_OPTION_OBSERVE)) {
            ret = CoAPUintOption_get(message, COAP_OPTION_OBSERVE,
                                     &observe_option);
            if (COAP_SUCCESS == ret && 1 == observe_option) {
                HAL_MutexLock(ctx->obsclient.list_mutex);
                list_for_each_entry_safe(node, next, &ctx->obsclient.list,
                                         obslist, CoAPObservable)
                {
                    if (0 != node->tokenlen &&
                        node->tokenlen == message->header.tokenlen &&
                        0 == memcmp(node->token, message->token,
                                    node->tokenlen)) {
                        list_del(&node->obslist);
                        ctx->obsclient.count--;
                        coap_free(node);
