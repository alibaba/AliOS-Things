...
int CoAPNetwork_read(NetworkContext *p_context, NetworkAddr *p_remote,
...
#endif
        len = HAL_UDP_recvfrom(network->fd, p_remote, p_data, datalen,
                               timeout_ms);
...
int CoAPNetwork_write(NetworkContext *p_context, NetworkAddr *p_remote,
...
#endif
        len =
            HAL_UDP_sendto(network->fd, p_remote, p_data, datalen, timeout_ms);
#ifdef COAP_DTLS_SUPPORT
    }
#endif
    return len;
...
NetworkContext *CoAPNetwork_init(const NetworkInit *p_param)
{
    ...
    if (COAP_NETWORK_DTLS == network->type) {
        ...
        network->fd =
            (intptr_t)HAL_UDP_create_without_connect(NULL, network->port);
