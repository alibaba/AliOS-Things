...
{
    memcpy(msg->token, buf,
           msg->header.tokenlen > COAP_MSG_MAX_TOKEN_LEN
               ? COAP_MSG_MAX_TOKEN_LEN
               : msg->header.tokenlen);
    return msg->header.tokenlen;
}

#define COAP_OPT(o, e, step)  do {               \
    if ((e) < step) {                            \
 ...
 */
#define COAP_OPT_CHECK(o, e, step) \
    do {                           \
        COAP_OPT(o, e, step);      \
        if ((e) < 1)               \
            return -1;             \
...
static int CoAPDeserialize_Option(CoAPMsgOption *option, unsigned char *buf,
                                  int left, unsigned short *predeltas)
{
...
int CoAPDeserialize_Options(CoAPMessage *msg, unsigned char *buf, int buflen)
{
    ...
    while (left > 0 && (0xFF != *ptr) && index < COAP_MSG_MAX_OPTION_NUM) {
        len =
            CoAPDeserialize_Option(&msg->options[index], ptr, left, &optdeltas);
        if (len < 0) {
            return len;
        }
        msg->optcount += 1;
