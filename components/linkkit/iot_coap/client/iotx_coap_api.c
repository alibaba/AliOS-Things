...
#define IOTX_AUTH_DEVICENAME_STR                                        \
    "{\"productKey\":\"%s\",\"deviceName\":\"%s\",\"clientId\":\"%s\"," \
    "\"sign\":\"%s\"}"
#define IOTX_AUTH_DEVICENAME_STR_WITH_SEQ                               \
    "{\"productKey\":\"%s\",\"deviceName\":\"%s\",\"clientId\":\"%s\"," \
    "\"sign\":\"%s\",\"seq\":\"%d\"}"

#define IOTX_COAP_ONLINE_DTLS_SERVER_URL \
    "coaps://%s.coap.cn-shanghai.link.aliyuncs.com:5684"
#define IOTX_COAP_ONLINE_PSK_SERVER_URL \
    "coap-psk://%s.coap.cn-shanghai.link.aliyuncs.com:5682"
...
int iotx_calc_sign(const char *p_device_secret, const char *p_client_id,
                   const char *p_device_name, const char *p_product_key,
                   char sign[IOTX_SIGN_LENGTH])
{
    char *p_msg = NULL;
    ...
    HAL_Snprintf(p_msg, IOTX_SIGN_SOURCE_LEN, IOTX_SIGN_SRC_STR, p_client_id,
                 p_device_name, p_product_key);
    utils_hmac_md5(p_msg, strlen(p_msg), sign, p_device_secret,
                   strlen(p_device_secret));
...
int iotx_calc_sign_with_seq(const char *p_device_secret,
                            const char *p_client_id, const char *p_device_name,
                            const char *p_product_key, unsigned int seq,
                            char sign[IOTX_SIGN_LENGTH])
{
    char *p_msg = NULL;
    ...
    HAL_Snprintf(p_msg, IOTX_SIGN_SOURCE_LEN, IOTX_SIGN_SRC_STR_WITH_SEQ,
                 p_client_id, p_device_name, p_product_key, seq);
...
static int iotx_parse_auth_from_json(char *p_str, iotx_coap_t *p_iotx_coap)
{
    ...
    memset(&node, 0x00, sizeof(lite_cjson_t));
    ret =
        lite_cjson_object_item(&root, "seqOffset", strlen("seqOffset"), &node);
    if (-1 == ret) {
        return IOTX_ERR_AUTH_FAILED;
    }
...
static void iotx_device_name_auth_callback(void *user, void *p_message)
{
    ...
    case COAP_MSG_CODE_205_CONTENT:
        {
            if (COAP_ENDPOINT_PSK == p_iotx_coap->p_coap_ctx->network.ep_type) {
                ret_code = iotx_parse_auth_from_json((char *)message->payload,
                                                     p_iotx_coap);
            } else {
                ret_code = iotx_get_token_from_json(
                    (char *)message->payload, p_iotx_coap->p_auth_token,
                    p_iotx_coap->auth_token_len);
            }
    ...
    case COAP_MSG_CODE_500_INTERNAL_SERVER_ERROR:
        {
            COAP_INFO(
                "CoAP internal server error, authenticate failed, will retry "
                "it");
            HAL_SleepMs(1000);
...
static unsigned int iotx_get_coap_token(iotx_coap_t *p_iotx_coap,
                                        unsigned char *p_encoded_data)
{
...
int iotx_get_well_known(iotx_coap_context_t *p_context)
{
    ...
    CoAPStrOption_add(&message, COAP_OPTION_URI_PATH,
                      (unsigned char *)".well-known", strlen(".well-known"));
    CoAPStrOption_add(&message, COAP_OPTION_URI_PATH, (unsigned char *)"core",
                      strlen("core"));
...
static void iotx_coap_report_rsphdl(void *arg, void *p_response)
{
    ...
    if (p_payload_len > 0) {
        COAP_DEBUG("Report response: CoAP msg = '%.*s'", p_payload_len,
                   p_payload);
    } else {
...
static int coap_report_func(void *handle, const char *topic_name, int req_ack,
                            void *data, int len)
{
...
int iotx_aes_cbc_encrypt(const unsigned char *src, int len,
                         const unsigned char *key, void *out)
{
    ...
    p_Aes128_t aes_e_h = infra_aes128_init((unsigned char *)key,
                                           (unsigned char *)iv, AES_ENCRYPTION);
    if (len1) {
    ...
    if (!ret && pad) {
        ...
        ret = infra_aes128_cbc_encrypt(aes_e_h, buf, 1,
                                       (unsigned char *)out + len1);
    }
...
int iotx_aes_cbc_decrypt(const unsigned char *src, int len,
                         const unsigned char *key, void *out)
{
    char *iv = "543yhjy97ae7fyfg";
    ...
    if (ret == 0) {
        ...
        if (aes_d_h) {
            ret = infra_aes128_cbc_decrypt(aes_d_h, src + offset, 1,
                                           out_c + offset);
        } else {
...
static int iotx_aes_cfb_encrypt(const unsigned char *src, int len,
                                const unsigned char *key, void *out)
{
    int ret = -1;
    char *iv = "543yhjy97ae7fyfg";
...
int iotx_aes_cfb_decrypt(const unsigned char *src, int len,
                         const unsigned char *key, void *out)
{
    int ret = -1;
    char *iv = "543yhjy97ae7fyfg";

    p_Aes128_t aes_d_h = infra_aes128_init((unsigned char *)key,
                                           (unsigned char *)iv, AES_ENCRYPTION);
...
int IOT_CoAP_DeviceNameAuth(iotx_coap_context_t *p_context)
{
    ...
    p_iotx_coap = (iotx_coap_t *)p_context;
    if (NULL == p_iotx_coap ||
        (NULL != p_iotx_coap && (NULL == p_iotx_coap->p_auth_token ||
                                 NULL == p_iotx_coap->p_coap_ctx ||
                                 0 == p_iotx_coap->auth_token_len))) {
    ...
    CoAPStrOption_add(&message, COAP_OPTION_URI_PATH,
                      (unsigned char *)IOTX_AUTH_STR, strlen(IOTX_AUTH_STR));
    ...
    if (COAP_ENDPOINT_PSK == p_iotx_coap->p_coap_ctx->network.ep_type) {
        iotx_calc_sign_with_seq(p_iotx_coap->p_devinfo->device_secret,
                                p_iotx_coap->p_devinfo->device_id,
                                p_iotx_coap->p_devinfo->device_name,
                                p_iotx_coap->p_devinfo->product_key,
                                p_iotx_coap->seq, sign);
        HAL_Snprintf((char *)p_payload, COAP_MSG_MAX_PDU_LEN,
                     IOTX_AUTH_DEVICENAME_STR_WITH_SEQ,
                     p_iotx_coap->p_devinfo->product_key,
                     p_iotx_coap->p_devinfo->device_name,
                     p_iotx_coap->p_devinfo->device_id, sign, p_iotx_coap->seq);

    } else {
        iotx_calc_sign(p_iotx_coap->p_devinfo->device_secret,
                       p_iotx_coap->p_devinfo->device_id,
                       p_iotx_coap->p_devinfo->device_name,
                       p_iotx_coap->p_devinfo->product_key, sign);
        HAL_Snprintf((char *)p_payload, COAP_MSG_MAX_PDU_LEN,
                     IOTX_AUTH_DEVICENAME_STR,
                     p_iotx_coap->p_devinfo->product_key,
                     p_iotx_coap->p_devinfo->device_name,
                     p_iotx_coap->p_devinfo->device_id, sign);
    }
    ...
    if (COAP_SUCCESS != ret) {
        COAP_DEBUG("Send authentication message to server failed, ret = %d",
                   ret);
...
int IOT_CoAP_SendMessage(iotx_coap_context_t *p_context, char *p_path,
                         iotx_message_t *p_message)
{
    ...
    /* as this function only support POST request message, type ACK and RST
     * shall be considered error parameters */
    if (p_message->msg_type != IOTX_MESSAGE_CON &&
        p_message->msg_type != IOTX_MESSAGE_NON) {
        return IOTX_ERR_INVALID_PARAM;
    }
    ...
    if (p_iotx_coap->is_authed) {
        ...
        if (IOTX_CONTENT_TYPE_CBOR == p_message->content_type) {
            CoAPUintOption_add(&message, COAP_OPTION_CONTENT_FORMAT,
                               COAP_CT_APP_CBOR);
            CoAPUintOption_add(&message, COAP_OPTION_ACCEPT,
        ...
        if (COAP_ENDPOINT_PSK == p_iotx_coap->p_coap_ctx->network.ep_type) {
            ...
            HAL_Snprintf((char *)buff, sizeof(buff) - 1, "%d",
                         p_iotx_coap->seq++);
            len = iotx_aes_cbc_encrypt(buff, strlen((char *)buff),
                                       p_iotx_coap->key, seq);
            if (0 < len) {
            ...
            len = iotx_aes_cbc_encrypt(p_message->p_payload,
                                       p_message->payload_len, p_iotx_coap->key,
                                       payload);
            if (0 == len) {
                coap_free(payload);
                payload = NULL;
            ...
            CoAPMessagePayload_set(&message, p_message->p_payload,
        ...
        COAP_ERR("The client [%s/%s] still un-authorized yet, return %d",
                 p_iotx_coap->p_devinfo->product_key,
                 p_iotx_coap->p_devinfo->device_name, IOTX_ERR_NOT_AUTHED);
        return IOTX_ERR_NOT_AUTHED;
    }
}

int IOT_CoAP_GetMessagePayload(void *p_message, unsigned char **pp_payload,
                               int *p_len)
{
    ...
    if (COAP_ENDPOINT_PSK == p_iotx_coap->p_coap_ctx->network.ep_type) {
        ...
        len = iotx_aes_cbc_decrypt(message->payload, message->payloadlen,
                                   p_iotx_coap->key, payload);
        if (len > 0) {
...
int IOT_CoAP_GetMessageCode(void *p_message, iotx_coap_resp_code_t *p_resp_code)
{
    ...
    if (NULL == p_message || NULL == p_resp_code) {
        COAP_ERR("Invalid paramter p_message %p, p_resp_code %p", p_message,
                 p_resp_code);
        return IOTX_ERR_INVALID_PARAM;
    }
    message = (Cloud_CoAPMessage *)p_message;
...
iotx_coap_context_t *IOT_CoAP_Init(iotx_coap_config_t *p_config)
{
    ...
    if (NULL != p_config->p_devinfo) {
        memset(p_iotx_coap->p_devinfo, 0x00, sizeof(iotx_coap_device_info_t));
        strncpy(p_iotx_coap->p_devinfo->device_id,
                p_config->p_devinfo->device_id,
                strlen(p_config->p_devinfo->device_id));
        strncpy(p_iotx_coap->p_devinfo->product_key,
                p_config->p_devinfo->product_key,
                strlen(p_config->p_devinfo->product_key));
        strncpy(p_iotx_coap->p_devinfo->device_secret,
                p_config->p_devinfo->device_secret,
                strlen(p_config->p_devinfo->device_secret));
        strncpy(p_iotx_coap->p_devinfo->device_name,
                p_config->p_devinfo->device_name,
    ...
    if (NULL != p_config->p_url) {
        ...
        HAL_Snprintf(url, sizeof(url), IOTX_COAP_ONLINE_PSK_SERVER_URL,
                     p_iotx_coap->p_devinfo->product_key);
        param.url = url;
