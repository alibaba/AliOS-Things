...
#define Cloud_CoAPRstMsg(header) (header.type == COAP_MESSAGE_TYPE_RST)

#define Cloud_CoAPCONRespMsg(header)               \
    ((header.code == COAP_MSG_CODE_205_CONTENT) && \
     (header.type == COAP_MESSAGE_TYPE_CON))

#define Cloud_CoAPReqMsg(header)   ((1 <= header.code) && (32 > header.code))
...
unsigned short Cloud_CoAPMessageId_gen(Cloud_CoAPContext *context)
{
    ...
    msg_id =
        ((COAP_MAX_MESSAGE_ID == context->message_id) ? 1
                                                      : context->message_id++);
    return msg_id;
}

int Cloud_CoAPMessageHandler_set(Cloud_CoAPMessage *message,
...
static int Cloud_CoAPMessageList_add(Cloud_CoAPContext *context,
                                     Cloud_CoAPMessage *message, int len)
{
...
int Cloud_CoAPMessage_send(Cloud_CoAPContext *context,
                           Cloud_CoAPMessage *message)
{
    ...
    msglen =
        CoAPSerialize_Message(message, context->sendbuf, COAP_MSG_MAX_PDU_LEN);
    COAP_DEBUG("----The message length %d-----", msglen);

    ret = Cloud_CoAPNetwork_write(&context->network, context->sendbuf,
                                  (unsigned int)msglen);
    if (COAP_SUCCESS == ret) {
        if (Cloud_CoAPReqMsg(message->header) ||
            Cloud_CoAPCONRespMsg(message->header)) {
...
static int Cloud_CoAPAckMessage_handle(Cloud_CoAPContext *context,
                                       Cloud_CoAPMessage *message)
{
    Cloud_CoAPSendNode *node = NULL;
...
static int Cloud_CoAPAckMessage_send(Cloud_CoAPContext *context,
...
static int Cloud_CoAPRespMessage_handle(Cloud_CoAPContext *context,
                                        Cloud_CoAPMessage *message)
{
    Cloud_CoAPSendNode *node = NULL;
                        ...
                        Cloud_CoAPSendNode)
    {
        if (0 != node->tokenlen && node->tokenlen == message->header.tokenlen &&
            ...
            iotx_facility_json_print((const char *)message->payload,
                                     LOG_INFO_LEVEL, '<');
#endif
...
int Cloud_CoAPMessage_recv(Cloud_CoAPContext *context, unsigned int timeout,
                           int readcount)
{
    int len = 0;
    int count = readcount;
        ...
        if (NULL != node) {
            if (node->timeout == 0) {
                if (node->retrans_count < COAP_MAX_RETRY_COUNT &&
                    ...
                    ret = Cloud_CoAPNetwork_write(&context->network,
