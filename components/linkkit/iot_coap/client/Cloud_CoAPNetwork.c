...
unsigned int Cloud_CoAPNetworkDTLS_read(void *p_session, unsigned char *p_data,
                                         ...
                                         unsigned int *p_datalen)
{
    ...
    if (NULL != p_session) {
        err_code =
            HAL_DTLSSession_write((DTLSContext *)p_session, p_data, p_datalen);
        if (DTLS_SUCCESS == err_code) {
            return COAP_SUCCESS;
        } else {
...
void *Cloud_CoAPNetworkDTLS_createSession(char *p_host, unsigned short port,
                           ...
                           unsigned int datalen, unsigned int timeout)
{
    ...
    if (COAP_ENDPOINT_DTLS == network->ep_type) {
        ...
        Cloud_CoAPNetworkDTLS_read(network->context, data, (unsigned int *)&len,
                                   timeout);
    } else {
#endif
        memset(data, 0x00, datalen);
        len = HAL_UDP_readTimeout((intptr_t)network->context, data,
...
unsigned int Cloud_CoAPNetwork_init(const coap_network_init_t *p_param,
                                    coap_network_t *p_network)
{
    ...
    if (COAP_ENDPOINT_DTLS == p_param->ep_type) {
        p_network->context = Cloud_CoAPNetworkDTLS_createSession(
            p_param->p_host, p_param->port, p_param->p_ca_cert_pem);
    ...
    if (COAP_ENDPOINT_NOSEC == p_param->ep_type ||
        ...
        /*Create udp socket*/
        p_network->context =
            (void *)HAL_UDP_create(p_param->p_host, p_param->port);
