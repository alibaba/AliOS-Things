...
unsigned int Cloud_CoAPUri_parse(char *p_uri,
                                 coap_endpoint_type *p_endpoint_type,
                                 char host[COAP_DEFAULT_HOST_LEN],
                                 unsigned short *port)
{
    int len = 0;
    char *p = NULL, *q = NULL;
...
Cloud_CoAPContext *Cloud_CoAPContext_create(Cloud_CoAPInitParam *param)
{
    ...
    if (NULL != param->url) {
        ret = Cloud_CoAPUri_parse(param->url, &network_param.ep_type, host,
                                  &network_param.port);
    }
