 ...
 */
static int get_ssid_passwd_from_w(uint8_t *in, int total_len, uint8_t *src,
                                  uint8_t *bssid)
{
    uint8_t tmp_ssid[ZC_MAX_SSID_LEN + 1] = { 0 },
                                       tmp_passwd[ZC_MAX_PASSWD_LEN + 1] = {
                                           0
                                       };
    ...
    char *magic_p_w_d =
        "zl&ws"; /* FIXME: Maybe it will be dangerous when opening source. */
...
#define MAC_LOCAL_ADMINISTERED_BIT (0x02)
    ...
    case P2P_ENCODE_TYPE_ENCRYPT:
        {
            ...
            decode_chinese(tmp_passwd, passwd_len, passwd_cipher,
                           &passwd_cipher_len, 7);
            passwd_len = passwd_cipher_len;
            memset(tmp_passwd, 0, ZC_MAX_PASSWD_LEN);
            aes_decrypt_string((char *)passwd_cipher, (char *)tmp_passwd,
                               passwd_len, 1, awss_get_encrypt_type(), 0, NULL);
            ...
            if (is_utf8((const char *)tmp_passwd, passwd_len) == 0) {
                ...
                memset(zc_pre_ssid, 0, sizeof(zconfig_data->android_pre_ssid));
                memset(zc_android_ssid, 0, sizeof(zconfig_data->android_ssid));
                memset(zc_android_bssid, 0,
                       sizeof(zconfig_data->android_bssid));

                awss_warn("p2p decrypt passwd content err\r\n");
                awss_event_post(IOTX_AWSS_PASSWD_ERR);
                AWSS_UPDATE_STATIS(AWSS_STATIS_WPS_IDX,
    ...
    if (passwd_len && !memcmp(tmp_passwd, magic_p_w_d, passwd_len)) {
        ...
        strncpy((char *)zc_pre_ssid, (char const *)tmp_ssid,
                ZC_MAX_SSID_LEN - 1);
        return GOT_CHN_LOCK;
    }
    /*
...
#endif
            {
                if (memcmp(bssid, zero_mac, ETH_ALEN) &&
                    memcmp(bssid, br_mac, ETH_ALEN)) {
                ...
                if (ap_info) {
                    if (ap_info->ssid[0] ==
                        '\0') { /* hide ssid, MUST not truncate */
                        strncpy((char *)zc_android_ssid,
                                (const char *)best_ssid, ZC_MAX_SSID_LEN - 1);
...
#endif
                    if (time_elapsed_ms_since(start_time) >
                        awss_get_channel_scan_interval_ms() * (13 + 3) * 2) {
                    start_time = 0;
                    strncpy((char *)zc_android_ssid, (const char *)best_ssid,
                            ZC_MAX_SSID_LEN - 1);
                }
                ...
                strncpy((char *)zc_ssid, (const char *)zc_android_ssid,
                        ZC_MAX_SSID_LEN - 1);
                               ...
                               struct parser_res *res, signed char rssi)
{
    ...
    /* get wps ie */
    wps_ie = (const uint8_t *)cfg80211_find_vendor_ie(
        WLAN_OUI_WPS, WLAN_OUI_TYPE_WPS, mgmt_header + ieoffset,
        len - ieoffset);
    if (wps_ie == NULL) {
        return ALINK_INVALID;
    }
    /* get wps name in wps ie */
    wps_ie = (const uint8_t *)get_device_name_attr_from_wps((uint8_t *)wps_ie,
                                                            &attr_len);
    if (wps_ie == NULL) {
        return ALINK_INVALID;
    }
