...
static int awss_80211_frame_handler(char *buf, int length,
                                    ...
                                    signed char rssi)
{
    ...
    awss_trace("bssid = %02x %02x %02x %02x %02x %02x", bssid[0], bssid[1],
...
static int try_get_real_ssid(uint32_t timeout, uint8_t *bissd_in,
                             char *ssid_out)
{
...
static int awss_dev_ap_switchap_resp(void *context, int result, void *userdata,
                                     void *remote, void *message)
{
    if (result == 2) { /* success */
...
static void start_connect_ap()
{
    ...
    awss_cmp_coap_cancel_packet(info->msgid);
    AWSS_UPDATE_STATIS(AWSS_STATIS_CONN_ROUTER_IDX,
    ...
    if (is_str_asii(info->ssid) == 0 &&
        bssid_is_valid(
            info->bssid)) { /*try to get real ssid from mngmt frame */
...
int wifimgr_process_dev_ap_switchap_request(void *ctx, void *resource,
...
#define AWSS_DEV_AP_SWITCHA_RSP_LEN (512)
    ...
    do {
        ...
        if (!ssid_found) {
            ...
            if (str && (str_len < PLATFORM_MAX_SSID_LEN * 2 - 1)) {
                ...
                HAL_Snprintf(msg, AWSS_DEV_AP_SWITCHA_RSP_LEN, AWSS_ACK_FMT,
                             req_msg_id, -1, "\"ssid error\"");
                success = 0;
                break;
            }
        }
        ...
        if (str && str_len == RANDOM_MAX_LEN * 2) {
            utils_str_to_hex(str, str_len, (unsigned char *)random,
                             RANDOM_MAX_LEN);
        } else {
            HAL_Snprintf(msg, AWSS_DEV_AP_SWITCHA_RSP_LEN, AWSS_ACK_FMT,
                         req_msg_id, -4, "\"random len error\"");
            success = 0;
            break;
        }
        ...
        if (str &&
            str_len == RANDOM_MAX_LEN * 2) { /* token len equal to random len */
            utils_str_to_hex(str, str_len, (unsigned char *)token,
                             RANDOM_MAX_LEN);
            *token_found = 1;
        }
        ...
        if (str_len < (PLATFORM_MAX_PASSWD_LEN * 2) - 1) {
            ...
            aes_decrypt_string(encoded, passwd, str_len, 0,
                               awss_get_encrypt_type(), 1,
                               random); /* 64bytes=2x32bytes */
        } else {
            HAL_Snprintf(msg, AWSS_DEV_AP_SWITCHA_RSP_LEN, AWSS_ACK_FMT,
                         req_msg_id, -3, "\"passwd len error\"");
            success = 0;
            AWSS_UPDATE_STATIS(AWSS_STATIS_DAP_IDX,
                               AWSS_STATIS_TYPE_PASSWD_ERR);
        }

        if (success && is_utf8(passwd, strlen(passwd)) == 0) {
            HAL_Snprintf(msg, AWSS_DEV_AP_SWITCHA_RSP_LEN, AWSS_ACK_FMT,
                         req_msg_id, -3, "\"passwd content error\"");
            success = 0;
            AWSS_UPDATE_STATIS(AWSS_STATIS_DAP_IDX,
                               AWSS_STATIS_TYPE_PASSWD_ERR);
        }
    ...
    if (success == 1) {
        ...
        dev_info[0] = '{';
        awss_build_dev_info(*token_found == 1 ? AWSS_NOTIFY_TYPE_MAX
                                              : AWSS_NOTIFY_DEV_BIND_TOKEN,
                            dev_info + 1, AWSS_DEV_AP_SWITCHA_RSP_LEN - 1);
        dev_info[strlen(dev_info)] = '}';
        dev_info[AWSS_DEV_AP_SWITCHA_RSP_LEN - 1] = '\0';
        HAL_Snprintf(msg, AWSS_DEV_AP_SWITCHA_RSP_LEN, AWSS_ACK_FMT, req_msg_id,
                     200, dev_info);
    }
    awss_build_topic((const char *)TOPIC_AWSS_DEV_AP_SWITCHAP, topic,
                     TOPIC_LEN_MAX);
    result =
        awss_cmp_coap_send_resp(msg, strlen(msg), remote, topic, request,
                                awss_dev_ap_switchap_resp, &(info->msgid), 1);
