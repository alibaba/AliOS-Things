...
int awss_start_connectap_monitor()
{
    ...
    connectap_monitor_timer =
        HAL_Timer_Create("connap_monitor", awss_connectap_monitor, NULL);
...
int process_get_device_info(void *ctx, void *resource, void *remote,
                            void *request, char is_mcast, int type)
{
    char *buf = NULL;
    char *dev_info = NULL;
    int len = 0, id_len = 0;
    ...
    if (type == AWSS_NOTIFY_DEV_RAND_SIGN) {
        topic_fmt = is_mcast ? TOPIC_AWSS_GETDEVICEINFO_MCAST
                             : TOPIC_AWSS_GETDEVICEINFO_UCAST;
    } else if (type == AWSS_NOTIFY_SUCCESS) {
        topic_fmt = is_mcast ? TOPIC_AWSS_GET_CONNECTAP_INFO_MCAST
                             : TOPIC_AWSS_GET_CONNECTAP_INFO_UCAST;
    } else {
        goto DEV_INFO_ERR;
    }
...
int awss_process_mcast_get_connectap_info(void *ctx, void *resource,
...
int awss_process_ucast_get_connectap_info(void *ctx, void *resource,
                                          void *remote, void *request)
{
    if (g_awss_connectap_info_avaliable == 0) {
        return -1;
