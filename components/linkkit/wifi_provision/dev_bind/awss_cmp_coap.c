...
void *awss_cpy_coap_ctx(void *request, void *remote, char mcast)
{
    struct coap_session_ctx_t *ctx =
        awss_zalloc(sizeof(struct coap_session_ctx_t));
    if (ctx == NULL) {
        goto CPY_CTX_FAIL;
    }
...
int awss_cmp_coap_register_cb(char *topic, void *cb)
{
    ...
    CoAPServer_register(g_coap_ctx, (const char *)topic,
...
int awss_cmp_coap_send(void *buf, uint32_t len, void *sa, const char *uri,
                       void *cb, uint16_t *msgid)
{
    if (g_coap_ctx == NULL) {
        g_coap_ctx = (void *)CoAPServer_init();
    } else {
    ...
    return CoAPServerMultiCast_send(g_coap_ctx, (NetworkAddr *)sa, uri,
                                    (uint8_t *)buf, (uint16_t)len,
                                    (CoAPSendMsgHandler)cb, msgid);
}

int awss_cmp_coap_send_resp(void *buf, uint32_t len, void *sa, const char *uri,
                            void *req, void *cb, uint16_t *msgid, char qos)
{
    if (g_coap_ctx == NULL) {
        g_coap_ctx = (void *)CoAPServer_init();
    }

    return CoAPServerResp_send(g_coap_ctx, (NetworkAddr *)sa, (uint8_t *)buf,
                               (uint16_t)len, req, uri, (CoAPSendMsgHandler)cb,
                               msgid, qos);
}

int awss_cmp_coap_ob_send(void *buf, uint32_t len, void *sa, const char *uri,
                          void *cb)
{
    if (g_coap_ctx == NULL) {
        g_coap_ctx = (void *)CoAPServer_init();
    ...
    return CoAPObsServer_notify(g_coap_ctx, uri, (uint8_t *)buf, (uint16_t)len,
                                cb);
}
...
#ifdef WIFI_PROVISION_ENABLED
#if defined(AWSS_SUPPORT_AHA)
    { AWSS_LC_INIT_PAP, TOPIC_AWSS_SWITCHAP,
      wifimgr_process_switch_ap_request },
    { AWSS_LC_INIT_ROUTER | AWSS_LC_INIT_PAP, TOPIC_AWSS_GETDEVICEINFO_MCAST,
      wifimgr_process_mcast_get_device_info },
    { AWSS_LC_INIT_ROUTER | AWSS_LC_INIT_PAP, TOPIC_AWSS_GETDEVICEINFO_UCAST,
      wifimgr_process_ucast_get_device_info },
#endif
#ifdef AWSS_SUPPORT_DEV_AP
    { AWSS_LC_INIT_DEV_AP, TOPIC_AWSS_DEV_AP_SWITCHAP,
      wifimgr_process_dev_ap_switchap_request },
#endif
    { AWSS_LC_INIT_SUC, TOPIC_AWSS_GET_CONNECTAP_INFO_MCAST,
      awss_process_mcast_get_connectap_info },
    { AWSS_LC_INIT_SUC, TOPIC_AWSS_GET_CONNECTAP_INFO_UCAST,
...
#endif
#endif
    { AWSS_LC_INIT_SUC | AWSS_LC_INIT_BIND, TOPIC_GETDEVICEINFO_MCAST,
      online_mcast_get_device_info },
    { AWSS_LC_INIT_SUC | AWSS_LC_INIT_BIND, TOPIC_GETDEVICEINFO_UCAST,
...
int awss_cmp_local_init(int init_stage)
{
    ...
    for (i = 0; i < sizeof(awss_local_couple) / sizeof(awss_local_couple[0]);
         i++) {
