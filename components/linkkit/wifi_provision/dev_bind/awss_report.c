...
volatile char awss_report_token_suc = 0; // avoid be optimized by the compiler
volatile char awss_report_token_cnt = 0; // avoid be optimized by the compiler
...
#define AWSS_TIMEOUT       "timeout"
    ...
    awss_info = json_get_value_by_name(payload, payload_len, AWSS_JSON_PARAM,
                                       &awss_info_len, NULL);
     ...
     */
    elem =
        json_get_value_by_name(awss_info, awss_info_len, AWSS_SSID, &len, NULL);
    ...
    len = 0;
    elem = json_get_value_by_name(awss_info, awss_info_len, AWSS_PASSWD, &len,
                                  NULL);
    ...
    elem = json_get_value_by_name(awss_info, awss_info_len, AWSS_BSSID, &len,
                                  NULL);
    ...
    if (elem != NULL && len == AWSS_BSSID_STR_LEN) {
        ...
        while (i < OS_ETH_ALEN) {
             ...
             */
            if ((uint32_t)((unsigned long)bssid_str - (unsigned long)elem) >
                AWSS_BSSID_STR_LEN) {
                memset(switchap_bssid, 0, sizeof(switchap_bssid));
                break;
            }
    ...
    len = 0;
    elem = json_get_value_by_name(awss_info, awss_info_len, AWSS_SWITCH_MODE,
                                  &len, NULL);
    if (elem != NULL && (elem[0] == '0' || elem[0] == 0)) {
        elem = json_get_value_by_name(awss_info, awss_info_len, AWSS_TIMEOUT,
                                      &len, NULL);
        if (elem) {
...
static int awss_switch_ap_online()
{
    HAL_Awss_Connect_Ap(WLAN_CONNECTION_TIMEOUT_MS, switchap_ssid,
    ...
    reboot_timer = HAL_Timer_Create("rb_timer",
                                    (void (*)(void *))awss_reboot_system, NULL);
...
#define REPORT_TOKEN_PARAM_LEN (64)
    ...
    do {
        ...
        for (i = 0; i < sizeof(aes_random);
             i++) /* check aes_random is initialed or not */
        ...
        utils_hex_to_str(aes_random, RANDOM_MAX_LEN, token_str,
                         sizeof(token_str) - 1);
        HAL_Snprintf(param, REPORT_TOKEN_PARAM_LEN - 1, "{\"token\":\"%s\"}",
                     token_str);
        awss_build_packet(AWSS_CMP_PKT_TYPE_REQ, id_str, ILOP_VER,
                          METHOD_MATCH_REPORT, param, 0, packet, &packet_len);
    } while (0);
