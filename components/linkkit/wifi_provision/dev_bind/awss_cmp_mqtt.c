...
int awss_cmp_mqtt_register_cb(char *topic, void *cb)
{
    ...
    return IOT_MQTT_Subscribe(NULL, topic, IOTX_MQTT_QOS3_SUB_LOCAL,
                              (iotx_mqtt_event_handle_func_fpt)cb, NULL);
#else
    return IOT_MQTT_Subscribe(NULL, topic, IOTX_MQTT_QOS0,
                              (iotx_mqtt_event_handle_func_fpt)cb, NULL);
#endif
}
...
{
    return IOT_MQTT_Publish_Simple(NULL, topic, qos, data,
                                   len); /* IOTX_MQTT_QOS1 or IOTX_MQTT_QOS1 */
}
...
int awss_cmp_online_init()
{
    ...
    for (i = 0; i < sizeof(awss_online_couple) / sizeof(awss_online_couple[0]);
         i++) {
        int res = -1;
