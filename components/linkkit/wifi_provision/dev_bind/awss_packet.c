...
#define AWSS_DEV_RAND_SIGN_FMT \
    ",\"random\":\"%s\",\"signMethod\":%d,\"sign\":\"%s\""
#define AWSS_DEV_BIND_TOKEN_FMT \
    ",\"token\":\"%s\",\"remainTime\":%d,\"type\":%d"
#define AWSS_SUCCESS_FMT ",\"type\":%d"
#define AWSS_DEV_INFO_FMT                                                    \
    "\"awssVer\":%s,\"productKey\":\"%s\",\"deviceName\":\"%s\",\"mac\":\"%" \
    "s\",\"ip\":\"%s\",\"cipherType\":%d"
...
static void *awss_get_dev_info(void *dev_info, int len)
{
    ...
    HAL_Snprintf(dev_info, len - 1, AWSS_DEV_INFO_FMT, AWSS_VER, pk, dev_name,
                 mac_str, ip_str, bind_get_encrypt_type());
...
void *awss_build_dev_info(int type, void *dev_info, int info_len)
{
    ...
    len += HAL_Snprintf((char *)dev_info + len, info_len - len - 1, "%s",
                        (char *)awss_get_dev_info(buf, DEV_INFO_LEN_MAX));
    HAL_Free(buf);
    buf = NULL;
    ...
    case AWSS_NOTIFY_DEV_BIND_TOKEN:
        {
            ...
            utils_hex_to_str(aes_random, RANDOM_MAX_LEN, rand_str,
                             sizeof(rand_str));
            len += HAL_Snprintf((char *)dev_info + len, info_len - len - 1,
                                AWSS_DEV_BIND_TOKEN_FMT, rand_str,
    ...
    case AWSS_NOTIFY_DEV_RAND_SIGN:
        {
            ...
            char rand_str[(RANDOM_MAX_LEN << 1) + 1] = { 0 };
            {
                ...
                utils_hex_to_str(sign, DEV_SIGN_SIZE, sign_str,
                                 sizeof(sign_str));
            }
            utils_hex_to_str(aes_random, RANDOM_MAX_LEN, rand_str,
                             sizeof(rand_str));
            len += HAL_Snprintf((char *)dev_info + len, info_len - len - 1,
                                AWSS_DEV_RAND_SIGN_FMT, rand_str, 0, sign_str);
            break;
        }
#endif
    default:
...
int awss_build_packet(int type, void *id, void *ver, void *method, void *data,
                      int code, void *packet, int *packet_len)
{
    int len;
    ...
    if (type == AWSS_CMP_PKT_TYPE_REQ) {
        ...
        len = HAL_Snprintf(packet, len - 1, AWSS_REQ_FMT, (char *)id,
                           (char *)ver, (char *)method, (char *)data);
        return 0;
    } else if (type == AWSS_CMP_PKT_TYPE_RSP) {
        len = HAL_Snprintf(packet, len - 1, AWSS_ACK_FMT, (char *)id, code,
                           (char *)data);
        return 0;
    }
    return -1;
