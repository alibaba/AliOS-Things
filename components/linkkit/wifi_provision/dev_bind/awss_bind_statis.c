...
int awss_bind_report_statis(const char *module)
{
    ...
    do {
        ...
        len += HAL_Snprintf(log_buf + len, log_buf_len - len, elem_fmt,
                            "SyncToken", DB_TMAX, DB_TMIN, DB_TMEAN, DB_CNT,
                            DB_SUC);

        HAL_Snprintf(log_content, AWSS_STATIS_DB_BUF_LEN, AWSS_STATIS_FMT,
                     (uint32_t)HAL_UptimeMs(), "BIND_TRACE",
                     module == NULL ? "default" : module,
                     awss_statis_db_trace_id, log_buf);
...
void awss_bind_disp_statis()
{
    ...
    awss_debug("SyncToken      \t%u\t%u\t%u\t%u\t%u\t", DB_TMAX, DB_TMIN,
               DB_TMEAN, DB_CNT, DB_SUC);
    awss_debug(
        "---------------------------------------------------------------------"
        "-");
