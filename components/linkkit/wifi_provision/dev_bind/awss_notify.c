...
static int awss_dev_bind_notify_resp(void *context, int result, void *userdata,
                                     void *remote, void *message);
int awss_dev_bind_notify();
#endif

#ifdef WIFI_PROVISION_ENABLED
static int awss_devinfo_notify_resp(void *context, int result, void *userdata,
                                    void *remote, void *message);
static int awss_suc_notify_resp(void *context, int result, void *userdata,
                                void *remote, void *message);
...
#ifndef DEV_BIND_DISABLE_NOTIFY
    { AWSS_NOTIFY_DEV_BIND_TOKEN, METHOD_DEV_INFO_NOTIFY, TOPIC_NOTIFY,
      awss_dev_bind_notify_resp },
#endif
#ifdef WIFI_PROVISION_ENABLED
    { AWSS_NOTIFY_DEV_RAND_SIGN, METHOD_AWSS_DEV_INFO_NOTIFY, TOPIC_AWSS_NOTIFY,
      awss_devinfo_notify_resp },
    { AWSS_NOTIFY_SUCCESS, METHOD_AWSS_CONNECTAP_NOTIFY,
...
static int awss_notify_response(int type, int result, void *message)
{
    ...
    do {
        ...
        payload = awss_cmp_get_coap_payload(message, &len);
        if (payload == NULL ||
            len > 0x40 || len == 0) {
            return 0;
        }
 ...
 * @param mask_level 0 ~ 4(255), 1 ~ 3(255), 2 ~ 2(255), 3 ~ 1(255), others ~
 * invalid,
 */
    ...
    if (ip[0] != '\0' && mask_level != 0) {
        ...
        if (mask_level == 1) {
            if (i + strlen("255.255.255") < 16) {
                memcpy(bcast_addr->host + strlen(bcast_addr->host),
                       "255.255.255", strlen("255.255.255"));
                return 0;
            }
        } else if (mask_level == 2) {
            if (i + strlen("255.255") < 16) {
                memcpy(bcast_addr->host + strlen(bcast_addr->host), "255.255",
                       strlen("255.255"));
                return 0;
            }
        } else if (mask_level == 3) {
            if (i + strlen("255") < 16) {
                memcpy(bcast_addr->host + strlen(bcast_addr->host), "255",
                       strlen("255"));
                return 0;
            }
        }
...
int awss_notify_dev_info(int type, int count)
{
    ...
    do {
        ...
        HAL_Snprintf(buf, DEV_INFO_LEN_MAX - 1, AWSS_DEV_NOTIFY_FMT,
                     ++g_notify_id, method, dev_info);

        awss_info("topic:%s\n", topic);
        awss_debug("payload:%s\n", buf);
        for (i = 0; i < count; i++) {
            int ret = awss_cmp_coap_send(buf, strlen(buf), &notify_sa, topic,
                                         cb, &g_notify_msg_id[type]);
...
static int awss_process_get_devinfo()
{
    ...
    do {
        ...
        HAL_Snprintf(buf, DEV_INFO_LEN_MAX - 1, AWSS_ACK_FMT, req_msg_id, 200,
                     dev_info);
        HAL_Free(dev_info);
...
static int __awss_dev_bind_notify()
{
    ...
    if (awss_report_token_suc == 0) {
        if (dev_bind_notify_timer == NULL) {
            dev_bind_notify_timer = HAL_Timer_Create(
                "dev_bind", (void (*)(void *))__awss_dev_bind_notify, NULL);
        }
    ...
    do {
        ...
        if (dev_bind_cnt < AWSS_NOTIFY_CNT_MAX &&
            ...
            if (dev_bind_notify_timer == NULL) {
                dev_bind_notify_timer = HAL_Timer_Create(
                    "dev_bind", (void (*)(void *))awss_dev_bind_notify, NULL);
            }
            HAL_Timer_Stop(dev_bind_notify_timer);
    ...
    do {
        ...
        if (suc_cnt++ < AWSS_NOTIFY_CNT_MAX &&
            ...
            if (success_notify_timer == NULL) {
                success_notify_timer = HAL_Timer_Create(
                    "awss_suc", (void (*)(void *))__awss_suc_notify, NULL);
            }
        ...
        if (devinfo_cnt++ < AWSS_NOTIFY_CNT_MAX &&
            ...
            if (devinfo_notify_timer == NULL) {
                devinfo_notify_timer = HAL_Timer_Create(
                    "devinfo", (void (*)(void *))__awss_devinfo_notify, NULL);
            }
