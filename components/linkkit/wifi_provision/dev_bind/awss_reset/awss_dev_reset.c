...
#define AWSS_RESET_REQ_FMT \
    "{\"id\":%s, \"version\":\"1.0\", \"method\":\"%s\", \"params\":%s}"
...
int awss_report_reset_to_cloud()
{
    ...
    do {
        ...
        HAL_Snprintf(id_str, AWSS_RESET_MSG_ID_LEN, "\"%u\"",
                     awss_report_reset_id++);
        final_len = HAL_Snprintf(packet, packet_len, AWSS_RESET_REQ_FMT, id_str,
                                 METHOD_RESET_REPORT, "{}");
    } while (0);
