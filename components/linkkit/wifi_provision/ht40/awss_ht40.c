...
int ht40_scanning_hint_frame(uint8_t filter, signed char rssi, uint32_t length,
                             uint8_t channel)
{
    ...
    for (i = 1; i >= 0; i--)
        for (j = 3; j >= 0; j--)
            for (k = 0; k < 8; k++) {
                if (zconfig_hint_frame[k] + zconfig_fixed_offset[j][0] +
                        i * 2 ==
                    length) {
                    hint_pos = i * 32 + j * 8 + k;
                    awss_trace(
                        ...
                        "group:%d, %s\r\n",
                        filter, rssi, length, i, j, k, next_loop ? "DUP" : "");
                    if (!next_loop) {
                        channel_locked = ht40_lock_channel(channel, filter);
                        if (channel_locked)
                            zconfig_set_state(STATE_CHN_LOCKED_BY_BR, tods,
                                              channel);
...
int ht40_get_qos_auth_group_info(uint32_t length)
{
    ...
    int continues = 0, max_continues = 0, max_continues_pos = 0,
        second_continues = 0;
    int tods = 1;
    int i, j, k;
    ...
    for (i = 1; i >= 0; i--)
        for (j = 3; j >= 0; j--)
            for (count = 0, continues = 0, k = 0; k < 8; k++) {
                int pos = i * 32 + j * 8 + k;
    ...
    awss_debug(
        "max_cont:%d, sec_cont:%d, max_count:%d, max_cont_pos:%d, "
        "max_count_pos:%d\r\n",
        max_continues, second_continues, max_count, max_count_pos,
        max_continues_pos);

    if (max_continues > second_continues
    ...
    if (ht40_state == STATE_RCV_IN_PROGRESS) {
        ...
        if (is_data_frame(len)) {
            ...
            /*
               ...
               2) not equal:    pkg_score cut down & give warning &
               try_to_replace score now < last: 1) data equal:    score_uplimit
               up??? 2) not equal:    goto pos_unsync
               */
