...
int wifimgr_process_switch_ap_request(void *ctx, void *resource, void *remote,
                                      void *request)
{
    ...
    do {
        HAL_Snprintf(msg, sizeof(msg) - 1, AWSS_ACK_FMT, req_msg_id, 200,
                     "\"success\"");
        ...
        if (!ssid_found) {
            ...
            if (str && (str_len < PLATFORM_MAX_SSID_LEN * 2 - 1)) {
                ...
                HAL_Snprintf(msg, sizeof(msg) - 1, AWSS_ACK_FMT, req_msg_id, -1,
                             "\"ssid error\"");
                success = 0;
                break;
            }
        ...
        if (!str) {
            success = 0;
            HAL_Snprintf(msg, sizeof(msg) - 1, AWSS_ACK_FMT, req_msg_id, -4,
        ...
        if (0 == enc_lvl) {
            if (str_len < PLATFORM_MAX_PASSWD_LEN) {
                ...
                HAL_Snprintf(msg, sizeof(msg) - 1, AWSS_ACK_FMT, req_msg_id, -2,
                             "\"passwd len error\"");
                success = 0;
            }
        } else {
            if (str_len < (PLATFORM_MAX_PASSWD_LEN * 2) - 1) {
                ...
                aes_decrypt_string(encoded, passwd, str_len, 0,
                                   ...
                                   (const char *)aes_random);
            } else {
                HAL_Snprintf(msg, sizeof(msg) - 1, AWSS_ACK_FMT, req_msg_id, -3,
                             "\"passwd len error\"");
                AWSS_UPDATE_STATIS(AWSS_STATIS_PAP_IDX,
    ...
    if (0 != awss_connect(ssid, passwd, (uint8_t *)bssid, ETH_ALEN,
                          token_found == 1 ? token : NULL,
