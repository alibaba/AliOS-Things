...
int zconfig_recv_completed(uint8_t tods)
{
    ...
    /*
        ...
        index:        0x100        0x180    0x200        0x280        0x300
       0x380
    */
    len = pkg_len(1) &
          PAYLOAD_BITS_MASK; /* total len, include len(1B) & crc(2B) */
...
#define SSID_AUTO_COMPLETE_SCORE (score_max + 1)
    ...
    if (zc_ssid[0] != '\0' && (flag & SSID_EXIST_MASK) &&
        pkg_score(2) < SSID_AUTO_COMPLETE_SCORE && pkg_score(3) > score_mid &&
        ...
        if (!(flag & SSID_ENCODE_MASK)) { /* ASCLL ssid */
            ...
            for (i = 5; i < ssid_len + 5; i++) {
                pkg_len(i) = (zc_ssid[i - 5] - 32) |
                             (0x100 + 0x80 * ((i - 1) % GROUP_NUMBER));
            ...
            for (i = 5; i < buf_len + 5; i++) {
                pkg_len(i) =
                    buf[i - 5] | (0x100 + 0x80 * ((i - 1) % GROUP_NUMBER));
                pkg_score(i) = SSID_AUTO_COMPLETE_SCORE;
            }
            HAL_Free(buf);
        }
...
int zconfig_get_ssid_passwd(uint8_t tods)
{
    ...
    if (passwd_encrypt == PASSWD_ENCRYPT_CIPHER ||
        passwd_encrypt == PASSWD_ENCRYPT_OPEN) {
    ...
    if (flag & SSID_EXIST_MASK) { /* ssid exist */
        ...
        if (zc_ssid[0] == '\0' || zc_ssid_auto_complete_disable) {
            ...
            if (!strncmp((const char *)tmp, (char *)zc_ssid,
                         ZC_MAX_SSID_LEN - 1)) {
                awss_trace("SSID1: [%s]\r\n", zc_ssid);
            } else {
...
#endif
            ssid_len = strlen((const char *)ap->ssid) > ssid_len
                           ? ssid_len
                           : strlen((const char *)ap->ssid);
            if (is_utf8((const char *)ap->ssid, ssid_len) == 0) {
                strncpy((char *)zc_ssid, (const char *)ap->ssid,
                        ZC_MAX_SSID_LEN - 1);
            }
        } while (0);
#endif
    ...
    if (passwd_encrypt > PASSWD_ENCRYPT_CIPHER) {
        ...
        aes_decrypt_string((char *)tmp, (char *)zc_passwd, passwd_len, 1,
...
int package_cmp(uint8_t *package, uint8_t *src, uint8_t *dst, uint8_t tods,
                uint16_t len)
{
...
int package_save(uint8_t *package, uint8_t *src, uint8_t *dst, uint8_t tods,
                 uint16_t len)
{
    struct package *pkg = (struct package *)package;
...
const uint16_t zconfig_hint_frame[] = {
    /* GROUP_FRAME is not used, gourp 0 - 7 */
    START_FRAME,     GROUP_FRAME + 1, GROUP_FRAME + 2,
    GROUP_FRAME + 3, GROUP_FRAME + 4, GROUP_FRAME + 5,
    GROUP_FRAME + 6, GROUP_FRAME + 7, 0 /* NULL terminated */
};
...
found_match:
        ...
        if (memcmp(zc_src_mac, src, ETH_ALEN)) { /* case 1,2 */
            ...
            if (memcmp(zc_bssid, bssid, ETH_ALEN)) { /* case 4 */
                if (tods) {
                    ...
                    if (zconfig_data->data[0].state_machine ==
                        STATE_CHN_LOCKED_BY_BR) {
                        zconfig_data->data[0].state_machine =
                            STATE_CHN_SCANNING;
                    }
                    awss_warn("%c WDS! bssid:" MAC_FORMAT
                              " -> bssid:" MAC_FORMAT "\r\n",
    ...
    zc_frame_offset =
        zconfig_fixed_offset[encry][0]; /* delta, len(80211) - len(8023) */
    ...
    do {
        ...
        if (ap_info) { /* save ssid */
            strncpy((char *)zc_ssid, (const char *)ap_info->ssid,
                    ZC_MAX_SSID_LEN - 1);
        }
    } while (0);
#endif
                       ...
                       uint8_t index_now, uint8_t index_last, uint8_t tods)
{
    ...
    example: 1
     ...
     8+4 2bf 0  d15 cc:fa:00:c8:cf:d2 > ff:ff:ff:ff:ff:ff //两个包index_delta=1,
    sn_delta=7

    example: 2
     8+0, 3e1,  9a5
     8+1, 13f,  9a7
             group_sn=9a7, sn=9ab-9a7, pos=e-9, len=3ce        //here,
    index_delta=5, sn_delta=4 group_sn=9a7, sn=9ac-9ab, pos=f-9, len=454
    ...
    */
    ...
    if (sn_compare(sn_now, group_sn) <= 0 ||
        sn_compare(sn_now, zc_prev_sn) <= 0) {
        return score_min;
                    ...
                    int last_group_pos, int group_pos)
{
    ...
    int max_match = 0, match_group = -1, match_end = GROUP_NUMBER,
        match_score = 0;
...
retry:
        ...
        for (match = 0, score = score_max, j = 1; j <= GROUP_NUMBER; j++) {
            ...
            if (empty_match) {
                ...
                if (pkg_len(i + j) == tmp_len(j)) {
                    match++;
                    score =
                        (score > pkg_score(i + j)) ? pkg_score(i + j) : score;
    ...
    if (group_pos != -1) {                    /* 根据后位置确定 */
        ...
        if (!max_match || empty_match) { /* case 3 */
            ...
            reason = 3;
            goto replace;
            /* can not del goto, cause guess_pos has higher priority than empty
             * match */
        }
    }
    /* 前位置 有效性难以判断，忽略 */
        ...
        for (i = final_pos + 1, j = 1; i <= final_pos + match_end; i++, j++) {
            ...
            if (pkg_score(i) < match_score && tmp_score(j)) {
                ...
                pkg_score(i) = (match_score > tmp_score(j) - 1)
                                   ? (match_score - (tmp_score(j) - 1))
                                   : match_score; /*TODO*/
                awss_trace("\t%d+%d [%d] %c %-3x\r\n", final_pos, j,
                           pkg_score(i), flag_tods(tods), tmp_len(j));
...
int awss_ieee80211_smartconfig_process(uint8_t *ieee80211, int len,
                                       int link_type, struct parser_res *res,
                                       signed char rssi)
{
...
int awss_recv_callback_smartconfig(struct parser_res *res)
{
    ...
    awss_flow(
        "len=%d, %c, sn=%d, enc=%d, chn=%d, src=%02x%02x%02x%02x%02x%02x\r\n",
        len, flag_tods(tods), sn, encry_type, channel, src[0], src[1], src[2],
        src[3], src[4], src[5]);
    /*
    ...
    if (zc_state == STATE_CHN_LOCKED_BY_P2P ||
        ...
        if (is_hint_frame(encry_type, len, bssid, src, channel, tods, sn)) {
            if (statis == 0) {
                statis = 1;
                AWSS_UPDATE_STATIS(AWSS_STATIS_SM_IDX,
            ...
            awss_trace(
                ...
                src[0], src[1], src[2], src[3], src[4], src[5], bssid[0],
            ...
            if (ap_info) {
                if (ap_info->ssid[0] != 0x00 && ap_info->ssid[0] != 0xFF) {
                    strncpy((char *)zc_android_ssid,
                            (const char *)ap_info->ssid, ZC_MAX_SSID_LEN - 1);
                }
        ...
        if (ret == 0) {
            awss_debug("drop: %3x == %3x\r\n", sn,
                       zc_prev_sn); /* log level, too many retry pkg */
            goto drop;
        } else if (ret < 0 && !timeout) { /* if timeout, goto pos_unsync */
            awss_debug("drop: %3x < %3x\r\n", sn,
                       zc_prev_sn); /* TODO: better not drop */
        ...
        pos_unsync:
            if (zc_pos_unsync) { /* tmp save */
                ...
                if (zc_pos_unsync == 1) {
                    ...
                    tmp_score(index) =
                        (sn - zc_prev_sn); /* TODO: index? last_tmp_score */
                }
                zc_pos_unsync++; /* unsync pkg counter */
                awss_trace("\tX+%d [%d] %-3x %c %-3x\r\n", index,
                           tmp_score(index), sn, flag_tods(tods), len);
                goto update_sn; /* FIXME: update prev_sn or not? */
            }

            /* assert(sn > zc_prev_sn && pos > zc_cur_pos) */
            score = get_data_score(zc_group_sn, sn, zc_prev_sn, pos, zc_cur_pos,
                                   tods);
            if (score == score_min) { /* better not drop any pkg here */
                awss_trace(
                    "\t drop: group_sn:%x, sn:%x-%x=%x, pos:%d-%d, len:%x\r\n",
                    zc_group_sn, sn, zc_prev_sn, sn_minus(sn, zc_group_sn), pos,
                    zc_cur_pos, len);
                goto update_sn;
            } else {
            ...
            if (is_start_frame(len) || is_group_frame(len)) {
                ...
                awss_trace("%d+%d [%d] %-3x %c %-3x\r\n", group, 0,
                           zc_score_uplimit, sn, flag_tods(tods), len);
                ...
                /* zc_replace may happen in try_to_sync_pos(), so goto
                 * is_recv_completed */
        ...
        for (tods = 0; tods < 2; tods++) {
            ...
            if (score > pkg_score(pos)) {
                ...
                if (!equal) {               /* data not equal */
                    ...
                    tods = res->tods;
                    goto pos_unsync;
                } else if (zc_score_uplimit >= score_mid &&
                           pkg_score(pos) - score < 10) { /* data equal */
