...
/*
    ...
    1) 数据包，目的地址为广播地址
    2) 长度>40的管理帧
    厂家需要自行切换信道，切信道时间按照自身平台需要进行调整，建议取值范围[100ms
   - 300ms] 其中，6号信道需作为固定信道放在信道列表里！！！ input: pkt_data:
   80211 wireless raw package, include data frame & management frame pkt_length:
   radio_hdr + 80211 hdr + payload, without fcs(4B) return: 见enum
   _PKG_TYPE_结构体说明
*/
 ...
 */
int zconfig_set_apinfo(uint8_t *ssid, uint8_t *bssid, uint8_t channel,
