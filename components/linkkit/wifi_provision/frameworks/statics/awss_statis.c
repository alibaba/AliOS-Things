...
{
    const char *elem_fmt =
        "[%s max:%u min:%u mean:%u cnt:%u suc:%u crc-err:%u pw-err:%u],";
    ...
    if (awss_build_topic(TOPIC_POST_STATIS, statis_topic, TOPIC_LEN_MAX) ==
        NULL) {
    ...
    if (DROUTE_CNT > 0) {
        len += HAL_Snprintf(log_buf + len, log_buf_len - len, elem_fmt,
                            "ConnRouter", DROUTE_TMAX, DROUTE_TMIN,
                            DROUTE_TMEAN, DROUTE_CNT, DROUTE_SUC, 0, 0);
    }

#ifdef AWSS_SUPPORT_SMARTCONFIG
    if (SM_CNT > 0) {
        len += HAL_Snprintf(log_buf + len, log_buf_len - len, elem_fmt,
                            "Smartconfig", SM_TMAX, SM_TMIN, SM_TMEAN, SM_CNT,
                            SM_SUC, SM_CRC_ERR, SM_PW_ERR);
    }
#ifdef AWSS_SUPPORT_SMARTCONFIG_WPS
    if (WPS_CNT > 0) {
        len += HAL_Snprintf(log_buf + len, log_buf_len - len, elem_fmt,
                            "Smartconfig-wps", 0, 0, 0, WPS_CNT, WPS_SUC,
                            WPS_CRC_ERR, WPS_PW_ERR);
    }
#endif
#endif
#ifdef AWSS_SUPPORT_AHA
    if (PAP_CNT > 0) {
        len += HAL_Snprintf(log_buf + len, log_buf_len - len, elem_fmt, "Aha",
                            PAP_TMAX, PAP_TMIN, PAP_TMEAN, PAP_CNT, PAP_SUC, 0,
                            PAP_PW_ERR);
    }
#endif
#ifndef AWSS_DISABLE_ENROLLEE
    if (ZC_CNT > 0) {
        len += HAL_Snprintf(log_buf + len, log_buf_len - len, elem_fmt,
                            "Zconfig", 0, 0, 0, ZC_CNT, ZC_SUC, 0, ZC_PW_ERR);
    }
#endif
#ifdef AWSS_SUPPORT_DEV_AP
    if (DAP_CNT > 0) {
        len += HAL_Snprintf(log_buf + len, log_buf_len - len, elem_fmt,
                            "Dev-ap", DAP_TMAX, DAP_TMIN, DAP_TMEAN, DAP_CNT,
                            DAP_SUC, 0, DAP_PW_ERR);
    }
#endif
    ...
    do {
        ...
        HAL_Snprintf(log_content, AWSS_STATIS_BUF_LEN, AWSS_STATIS_FMT,
                     (uint32_t)HAL_UptimeMs(), "AWSS_TRACE",
                     module == NULL ? "default" : module, awss_statis_trace_id,
                     log_buf);

        HAL_Snprintf(id_str, sizeof(id_str), "%u", ++awss_statis_report_id);

        awss_build_packet(AWSS_CMP_PKT_TYPE_REQ, id_str, ILOP_VER,
                          METHOD_LOG_POST, log_content, 0, log_buf,
                          &log_buf_len);
...
void awss_disp_statis()
{
    ...
    awss_debug("ConnRouter      \t%u\t%u\t%u\t%u\t%u\t%u\t%u\t", DROUTE_TMAX,
               DROUTE_TMIN, DROUTE_TMEAN, DROUTE_CNT, DROUTE_SUC, 0, 0);
#ifdef AWSS_SUPPORT_SMARTCONFIG
    awss_debug("Smartconfig     \t%u\t%u\t%u\t%u\t%u\t%u\t%u\t", SM_TMAX,
               SM_TMIN, SM_TMEAN, SM_CNT, SM_SUC, SM_CRC_ERR, SM_PW_ERR);
#ifdef AWSS_SUPPORT_SMARTCONFIG_WPS
    awss_debug("Smartconfig-wps \t%u\t%u\t%u\t%u\t%u\t%u\t%u\t", 0, 0, 0,
               WPS_CNT, WPS_SUC, WPS_CRC_ERR, WPS_PW_ERR);
#endif
#endif
#ifdef AWSS_SUPPORT_AHA
    awss_debug("Aha             \t%u\t%u\t%u\t%u\t%u\t%u\t%u\t", PAP_TMAX,
               PAP_TMIN, PAP_TMEAN, PAP_CNT, PAP_SUC, 0, PAP_PW_ERR);
#endif
#ifndef AWSS_DISABLE_ENROLLEE
    awss_debug("Zconfig         \t%u\t%u\t%u\t%u\t%u\t%u\t%u\t", 0, 0, 0,
               ZC_CNT, ZC_SUC, 0, ZC_PW_ERR);
#endif
#ifdef AWSS_SUPPORT_DEV_AP
    awss_debug("Dev-ap          \t%u\t%u\t%u\t%u\t%u\t%u\t%u\t", DAP_TMAX,
               DAP_TMIN, DAP_TMEAN, DAP_CNT, DAP_SUC, 0, DAP_PW_ERR);
#endif
    awss_debug(
        "----------------------------------------------------------------------"
        "--------");
