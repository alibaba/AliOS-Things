...
int aes_decrypt_string(char *cipher, char *plain, int len, int cipher_hex,
                       int sec_lvl, char cbc, const char *rand)
{
    char res = 0;
    char decrypt = 1;
    ...
    if (decrypt) {
        ...
        if (cbc) { /* AP */
             ...
             */
            res = infra_aes128_cbc_decrypt(aes, decoded,
                                           len / AES128_KEY_LEN / 2, plain);
        } else { /* smartconfig */
            /*
             * smartconfig/wps, zconfig
             */
