 ...
 */
void encode_chinese(uint8_t *in, uint8_t in_len, uint8_t *out, uint8_t *out_len,
                    uint8_t bits)
{
    if (bits == 0 || bits > 7) {
        return;
...
/* x bit -> 8bit */
void decode_chinese(uint8_t *in, uint8_t in_len, uint8_t *out, uint8_t *out_len,
                    uint8_t bits)
{
...
uint8_t zconfig_callback_over(uint8_t *ssid, uint8_t *passwd, uint8_t *bssid,
                              uint8_t *token)
{
    uint8_t auth = ZC_AUTH_TYPE_INVALID, encry = ZC_ENC_TYPE_INVALID,
            channel = 0;
    ...
    zconfig_got_ssid_passwd_callback(ssid, passwd, bssid, token, auth, encry,
                                     channel);
...
void zconfig_set_state(uint8_t state, uint8_t tods, uint8_t channel)
{
    ...
    case STATE_CHN_LOCKED_BY_MCAST:
        ...
        break;
    case STATE_RCV_DONE:
        /* prevent main_thread_func to free zconfig_data until curent task is
         * finished. */
                          ...
                          int link_type, int with_fcs, signed char rssi)
{
    ...
    pkt_type =
        ieee80211_data_extract(pkt_data, pkt_length, link_type, &res, rssi);
...
void zconfig_init()
{
    ...
    zconfig_data =
        (struct zconfig_data *)awss_zalloc(sizeof(struct zconfig_data));
