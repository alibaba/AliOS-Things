...
int __awss_start(void)
{
    ...
    ret = aws_get_ssid_passwd(&ssid[0], &passwd[0], &bssid[0], &token[0], NULL,
                              NULL, NULL);
    if (!ret) {
    ...
    do {
        ...
        ret = awss_connect(ssid, passwd, bssid, ETH_ALEN,
                           find_token != 0 ? token : NULL,
        ...
        if (!ret) {
            ...
            printf("awss connect ssid:%s success\r\n", ssid);
            ...
            if (awss_notify_needed == 0) {
                ...
                awss_devinfo_notify();
                AWSS_UPDATE_STATIS(AWSS_STATIS_ROUTE_IDX,
