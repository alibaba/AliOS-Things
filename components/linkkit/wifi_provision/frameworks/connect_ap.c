...
int awss_complete_token(char passwd[HAL_MAX_PASSWD_LEN], uint8_t *bssid,
                        uint8_t bssid_len, uint8_t *token_in, uint8_t token_len,
                        uint8_t token_out[AWSS_TOKEN_LEN])
{
                 ...
                 uint8_t token_len)
{
    ...
    /*need to complete the token*/
    ret = awss_complete_token(passwd, bssid, bssid_len, token, token_len,
                              final_token);
    ...
    return HAL_Awss_Connect_Ap(WLAN_CONNECTION_TIMEOUT_MS, ssid, passwd, 0, 0,
                               has_bssid ? final_bssid : NULL, 0);
}
