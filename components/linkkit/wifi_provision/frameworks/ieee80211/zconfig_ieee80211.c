...
/**
 * ieee80211_is_data_present - check if type is IEEE80211_FTYPE_DATA and only
 * data
 * @fc: frame control bytes in little-endian byteorder
 */
...
{
    uint16_t tmp =
        fc & os_htole16(IEEE80211_FCTL_MOREFRAGS | IEEE80211_FCTL_ORDER);
 ...
 * the given data.
 */
const uint8_t *cfg80211_find_vendor_ie(uint32_t oui, uint8_t oui_type,
...
int ieee80211_get_ssid(uint8_t *beacon_frame, uint16_t frame_len, uint8_t *ssid)
{
    uint16_t ieoffset =
        offsetof(struct ieee80211_mgmt,
                 u.beacon.variable); /* same as u.probe_resp.variable */
    const uint8_t *ptr = cfg80211_find_ie(
        WLAN_EID_SSID, beacon_frame + ieoffset, frame_len - ieoffset);
    if (ptr) {
...
int cfg80211_parse_wpa2_info(const uint8_t *rsn_ie, int rsn_ie_len,
                             uint8_t *group_cipher, uint8_t *pairwise_cipher,
                             uint8_t *is_8021x)
{
    int i, ret = 0;
                             ...
                             uint8_t *group_cipher_type)
{
    ...
    uint8_t is_privacy =
        !!(mgmt->u.beacon.capab_info & WLAN_CAPABILITY_PRIVACY);
    ...
    if (tmp && tmp[1]) {
        ret = cfg80211_parse_wpa2_info(tmp, tmp[1] + 2, &group_cipher,
                                       &pairwise_cipher, &is80211X);
        if (is80211X)
        ...
        tmp = cfg80211_find_vendor_ie(WLAN_OUI_MICROSOFT,
                                      WLAN_OUI_TYPE_MICROSOFT_WPA, ie, ielen);
        if (tmp) {
            ret = cfg80211_parse_wpa_info(tmp, tmp[1] + 2, &group_cipher,
                                          &pairwise_cipher, &is80211X);
            if (is80211X)
...
#define check_ieee80211_buf_alignment(buf_addr, len)           \
    do {                                                       \
        if (((unsigned long)(buf_addr) & 0x1) && len > 0) {      \
            uint8_t *word_align_addr =                         \
                (uint8_t *)((unsigned long)(buf_addr) & ~0x1); \
...
struct awss_protocol_couple_type awss_protocol_couple_array[] = {
    ...
    { ALINK_HT_CTRL, awss_ieee80211_ht_ctrl_process,
      awss_recv_callback_ht_ctrl },
#endif
#ifdef AWSS_SUPPORT_APLIST
    { ALINK_APLIST, awss_ieee80211_aplist_process, NULL },
#endif
#ifdef AWSS_SUPPORT_AHA
    { ALINK_DEFAULT_SSID, awss_ieee80211_aha_process,
      awss_recv_callback_aha_ssid },
#endif
#ifndef AWSS_DISABLE_ENROLLEE
    { ALINK_ZERO_CONFIG, awss_ieee80211_zconfig_process,
      awss_recv_callback_zconfig },
#endif
#ifdef AWSS_SUPPORT_SMARTCONFIG_WPS
    { ALINK_WPS, awss_ieee80211_wps_process, awss_recv_callback_wps },
#endif
#ifdef AWSS_SUPPORT_SMARTCONFIG
    { ALINK_BROADCAST, awss_ieee80211_smartconfig_process,
      awss_recv_callback_smartconfig },
#endif
#ifdef AWSS_SUPPORT_SMARTCONFIG_MCAST
    { ALINK_BROADCAST, awss_ieee80211_mcast_smartconfig_process,
      awss_recv_callback_mcast_smartconfig },
#endif
                           ...
                           struct parser_res *res, signed char rssi)
{
    ...
    hdr = (struct ieee80211_hdr *)zconfig_remove_link_header(&in, &len,
                                                             link_type);
    if (len <= 0)
        goto drop;
    fc = hdr->frame_control;

    for (i = 0; i < sizeof(awss_protocol_couple_array) /
                        sizeof(awss_protocol_couple_array[0]);
         i++) {
        awss_protocol_process_func_type protocol_func =
    ...
    if (alink_type != ALINK_HT_CTRL) {
         ...
         * header IEEE 802.11 address fields: ToDS FromDS Addr1 Addr2 Addr3
         * Addr4 0     0   DA    SA    BSSID n/a 0     1   DA    BSSID SA    n/a
         *   1     0   BSSID SA    DA    n/a
         *   1     1   RA    TA    DA    SA
         */
    ...
    do {
        awss_protocol_finish_func_type finish_func =
