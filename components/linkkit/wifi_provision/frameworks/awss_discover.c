...
static uint16_t create_mng_frame(uint8_t *p_buffer, uint8_t type,
                                 uint8_t dst[6], simulate_ap_t *p_ap)
{
    uint16_t len = 0;
    uint8_t t_i;
...
static int get_ability(char *src, uint8_t mac[6])
{
    ...
    mac_dst_bits = (mac_org_bits & 0x3f) | (((mac_org_bits >> 6) & 0x3f) << 8) |
                   (((mac_org_bits >> 12) & 0x3f) << 16) |
                   (((mac_org_bits >> 18) & 0x3f) << 24);
 ...
 * @note None.
 */
int aws_discover_callback(uint8_t *buffer, int length, int link_type,
