...
{
    if (g_user_press && time_elapsed_ms_since(config_press_start_timestamp) >
                            g_config_press_timeout_ms) {
        awss_press_timeout();
    }
}
...
static char awss_aha_connect_to_router()
{
    ...
    for (iter = 0; iter < count; iter++) {
        ...
        if (HAL_Sys_Net_Is_Ready() && strlen(ssid) > 0 &&
            strcmp(ssid, DEFAULT_SSID)) { /* not AHA */
            dest_ap = 1;
            break;
        }
        if (awss_stopped) {
