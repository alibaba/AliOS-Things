...
#endif
    uint8_t security;    /* securation type, per product(3) or device(4) or
                            manufacture(5) */
...
#define ENROLLEE_IE_FIX_LEN                                          \
    (sizeof(struct ieee80211_enrollee_alibaba_ie) + RANDOM_MAX_LEN + \
     ENROLLEE_SIGN_SIZE)
#define REGISTRAR_IE_FIX_LEN (sizeof(struct ieee80211_registrar_alibaba_ie))
#define ENROLLEE_INFO_HDR_SIZE \
    (ENROLLEE_IE_FIX_LEN - 6 + MAX_DEV_NAME_LEN + 1 + MAX_PK_LEN + 1)
...
struct enrollee_info {
    ...
    uint8_t
        security; /* encryption per product(3) or device(4) or manufacture(5) */
    uint8_t sign_method; /* 0:hmacsha1, 1:hmacsha256 */
    uint8_t sign_len;
 ...
 * ENR_CHECKIN_ENABLE   --consumer-->   ENR_CHECKIN_ONGOING -->
 * ENR_CHECKIN_END/ENR_FREE *any state*      --consumer-->   ENR_FREE
 */
enum enrollee_state {
    ENR_FREE = 0,
    ENR_IN_QUEUE,
