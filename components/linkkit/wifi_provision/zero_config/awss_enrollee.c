...
const uint8_t probe_req_frame[ZC_PROBE_LEN] = {
    ...
    0x8C, 0x92, 0x98, 0xA4,             /* supported rates */
...
void awss_init_enrollee_info(void) /* void enrollee_raw_frame_init(void) */
{
    ...
    for (int i = 0; i < len; i++) {
        printf("0x%x ", *(unsigned char *)(text + i));
    }
    printf("\r\n");
    ...
    for (int i = 0; i < SHA1_DIGEST_SIZE; i++) {
        printf("0x%x ", *(unsigned char *)(sign + i));
    }
    printf("\r\n");
    ...
    g_product_key =
        &enrollee_frame[len]; /* pointer to pk len, see decrypt func */
    ...
    enrollee_frame[len++] =
        0; /* signature method, 0: hmacsha1, 1: hmacsha256 */
    ...
    for (int i = 0; i < 6; i++) {
        printf("0x%x ", *(unsigned char *)(enrollee_frame + SA_POS + i));
    }
...
static int decrypt_ssid_passwd(uint8_t *ie, uint8_t ie_len,
                               ...
                               uint8_t out_bssid[ETH_ALEN])
{
    uint8_t tmp_ssid[OS_MAX_SSID_LEN + 1] = { 0 },
                                       tmp_passwd[OS_MAX_PASSWD_LEN + 1] = {
                                           0
                                       };
    uint8_t *p_dev_name_sign = NULL, *p_ssid = NULL, *p_passwd = NULL,
            *p_bssid = NULL;

    /* ignore ie hdr: 221, len, oui[3], type(0xAB) */
#define REGISTRAR_IE_HDR (6)
    ie += REGISTRAR_IE_HDR;
    printf("ethan registrar(devtype/ver=%d\r\n", ie[0]);
    ...
    if (!g_dev_sign ||
        memcmp(g_dev_sign, p_dev_name_sign + 1, p_dev_name_sign[0])) {
        p_dev_name_sign[p_dev_name_sign[0]] = '\0';
        awss_debug("dev_name not match, expect:");
        printf("dev_name not match, expect\r\n");
        if (g_dev_sign)
    ...
    printf("ethan registrar(frametype/ver=%d\r\n", ie[0]);
    ...
    if (p_passwd[0] >= OS_MAX_PASSWD_LEN) {
        ...
        printf("registrar(passwdlen=%d invalid!\r\n", p_passwd[0]);
        return -1;
    }
    ...
    aes_decrypt_string((char *)p_passwd + 1, (char *)tmp_passwd, p_passwd[0], 1,
                       awss_get_encrypt_type(), 0,
                       (const char *)aes_random); /* aes128 cfb */
    if (is_utf8((const char *)tmp_passwd, p_passwd[0]) != 1) {
        awss_debug("registrar(passwd invalid!");
        printf("registrar(passwd invalid!\r\n");
        AWSS_UPDATE_STATIS(AWSS_STATIS_ZCONFIG_IDX,
                           AWSS_STATIS_TYPE_PASSWD_ERR);
        return -1;
    }
    awss_debug("ssid:%s\r\n", tmp_ssid);
    printf("ssid:%s, tmp_passwd:%s\r\n", tmp_ssid, tmp_passwd);
    strncpy((char *)out_passwd, (const char *)tmp_passwd,
            OS_MAX_PASSWD_LEN - 1);
                                   ...
                                   struct parser_res *res, signed char rssi)
{
    ...
    /*
     * we don't process zconfig used by enrollee until user press configure
     * button
     */
    ...
    registrar_ie = (const uint8_t *)cfg80211_find_vendor_ie(
        WLAN_OUI_ALIBABA, WLAN_OUI_TYPE_REGISTRAR, mgmt_header + ieoffset,
        len - ieoffset);
