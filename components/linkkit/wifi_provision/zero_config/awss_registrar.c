...
#define AWSS_REPORT_PARAM_FMT                                                  \
    "{\"awssVer\":%s,\"type\":0,\"ssid\":\"%s\",\"bssid\":\"%s\",\"rssi\":%d," \
    "\"payload\":[\"%s\"]}"

static void awss_wifi_mgnt_frame_callback(uint8_t *buffer, int length,
...
static int enrollee_enable_somebody_checkin(char *key, char *dev_name,
                                            int timeout);
...
void registrar_schedule()
{
    ...
    if (last_open) { /* need to close */
        HAL_Wifi_Enable_Mgmt_Frame_Filter(
            FRAME_BEACON_MASK | FRAME_PROBE_REQ_MASK, (uint8_t *)alibaba_oui,
            NULL);
        schedule_duration = REGISTRAR_WORK_TIME * idle_duty;
    } else {
        HAL_Wifi_Enable_Mgmt_Frame_Filter(
            FRAME_BEACON_MASK | FRAME_PROBE_REQ_MASK, (uint8_t *)alibaba_oui,
            awss_wifi_mgnt_frame_callback);
...
int online_dev_bind_monitor(void *ctx, void *resource, void *remote,
                            void *request)
{
    uint8_t i;
    char *payload = NULL;
    ...
    dev_info = json_get_value_by_name(payload, payload_len, AWSS_JSON_PARAM,
                                      &dev_info_len, NULL);
    ...
    if (awss_enrollee_get_dev_info(dev_info, dev_info_len, key, dev_name, NULL,
                                   NULL) < 0) {
        goto CONNECTAP_MONITOR_END;
    }
    ...
    for (i = 0; i < MAX_ENROLLEE_NUM; i++) {
        ...
        if (strlen(dev_name) == enrollee_info[i].dev_name_len &&
            0 == memcmp(dev_name, enrollee_info[i].dev_name,
                        enrollee_info[i].dev_name_len) &&
...
#define CHECK_IN_RSP_LEN (64)
...

    {
        ...
        id = json_get_value_by_name(payload, payload_len, AWSS_JSON_ID, &len,
                                    NULL);
        memcpy(id_str, id, len > MSG_REQ_ID_LEN - 1 ? MSG_REQ_ID_LEN - 1 : len);
        awss_build_packet(AWSS_CMP_PKT_TYPE_RSP, id_str, ILOP_VER,
                          METHOD_EVENT_ZC_CHECKIN, "{}", 200, packet,
                          &packet_len);
    }
...
static int enrollee_enable_somebody_cipher(char *key, char *dev_name,
                                           char *cipher)
{
    int i;
    ...
    if (strlen(key) > MAX_PK_LEN || strlen(dev_name) > MAX_DEV_NAME_LEN) {
        goto out;
    }
                                            ...
                                            int timeout)
{
    ...
    for (i = 0; i < MAX_ENROLLEE_NUM; i++) {
        ...
        if (strlen(dev_name) == enrollee_info[i].dev_name_len &&
            ...
            enrollee_info[i].checkin_timeout =
                timeout <= 0 ? REGISTRAR_TIMEOUT : timeout;
...
#define AWSS_DEV_CIPHER_FMT                                        \
    "{\"awssVer\":%s,\"productKey\":\"%s\",\"deviceName\":\"%s\"," \
    "\"cipherType\":%d, \"random\":\"%s\"}"
...

    {
        ...
        utils_hex_to_str(enrollee_info[i].random, RANDOM_MAX_LEN, rand_str,
                         sizeof(rand_str));
        HAL_Snprintf(id, MSG_REQ_ID_LEN - 1, "\"%u\"", registrar_id++);
        HAL_Snprintf(param, AWSS_REPORT_PKT_LEN - 1, AWSS_DEV_CIPHER_FMT,
                     AWSS_VER, enrollee_info[i].pk, enrollee_info[i].dev_name,
                     enrollee_info[i].security, rand_str);
        awss_build_packet(AWSS_CMP_PKT_TYPE_REQ, id, ILOP_VER,
                          METHOD_EVENT_ZC_CIPHER, param, 0, packet,
                          &packet_len);
        HAL_Free(param);
    }
...
void awss_get_cipher_reply(void *pcontext, void *pclient, void *msg)
{
    ...
    dev_info = json_get_value_by_name(payload, payload_len, AWSS_JSON_DEV_LIST,
                                      &dev_info_len, NULL);
    if (dev_info == NULL || dev_info_len == 0) {
        goto CIPHER_ERR;
    }

    if (awss_enrollee_get_dev_info(dev_info, dev_info_len, key, dev_name,
                                   cipher, NULL) < 0) {
        goto CIPHER_ERR;
    }
...
ongoing:
    ...
    if (time_elapsed_ms_since(enrollee_info[i].checkin_timestamp) >
        enrollee_info[i].checkin_timeout * 1000) {
        memset(&enrollee_info[i], 0, sizeof(enrollee_info[0]));
        awss_debug("enrollee[%d] state %d->%d", i, enrollee_info[i].state,
                   ENR_CHECKIN_END);
        enrollee_info[i].state =
            ENR_CHECKIN_END; /* FIXME: remove this state? */
        awss_debug("enrollee[%d] state %d->%d", i, enrollee_info[i].state,
                   ENR_FREE);
        enrollee_info[i].state = ENR_FREE;
...
int awss_report_set_interval(char *key, char *dev_name, int interval)
{
    ...
    for (i = 0; i < MAX_ENROLLEE_NUM; i++) {
        ...
        if (strlen(dev_name) == enrollee_info[i].dev_name_len &&
            ...
            strlen(key) == enrollee_info[i].pk_len &&
            0 == memcmp(key, enrollee_info[i].pk, enrollee_info[i].pk_len)) {
            enrollee_info[i].interval =
                interval <= 0 ? REGISTRAR_TIMEOUT : interval;
            enrollee_checkin();
            return 0; /* match */
        }
    }
                                      ...
                                      char *cipher, int *timeout)
{
    ...
    elem =
        json_get_value_by_name(payload, payload_len, AWSS_JSON_PK, &len, NULL);
    ...
    len = 0;
    elem = json_get_value_by_name(payload, payload_len, AWSS_JSON_DEV_NAME,
                                  &len, NULL);
    ...
    len = 0;
    elem = json_get_value_by_name(payload, payload_len, AWSS_JSON_PERIOD, &len,
                                  NULL);
    if (elem && timeout) {
        *timeout = atoi(elem);
    }
    ...
    len = 0;
    elem = json_get_value_by_name(payload, payload_len, AWSS_JSON_CIPHER, &len,
                                  NULL);
...
void awss_report_enrollee_reply(void *pcontext, void *pclient, void *msg)
{
    ...
    dev_list = json_get_value_by_name(payload, payload_len, AWSS_JSON_DEV_LIST,
                                      &dev_list_len, NULL);
    if (dev_list == NULL) {
        goto REPORT_REPLY_FAIL;
    }

    json_array_for_each_entry(dev_list, dev_list_len, str_pos, entry, entry_len,
                              type)
    {
        memset(dev_name, 0, MAX_DEV_NAME_LEN + 1);
        memset(key, 0, MAX_PK_LEN + 1);
        if (awss_enrollee_get_dev_info(entry, entry_len, key, dev_name, NULL,
                                       &interval) < 0) {
            continue;
        }
...
int awss_report_enrollee(uint8_t *payload, int payload_len, signed char rssi)
{
...

    {
        ...
        sprintf(bssid_str, "%02X:%02X:%02X:%02X:%02X:%02X", bssid[0], bssid[1],
        ...
        payload_str[payload_len * 2] =
            '\0'; /* sprintf not add '\0' in the end of string in qcom */

        HAL_Snprintf(id, MSG_REQ_ID_LEN - 1, "\"%u\"", registrar_id++);

        HAL_Snprintf(param, AWSS_REPORT_PKT_LEN - 1, AWSS_REPORT_PARAM_FMT,
                     AWSS_VER, ssid, bssid_str, rssi > 0 ? rssi - 256 : rssi,
                     payload_str);
        HAL_Free(payload_str);
        awss_build_packet(AWSS_CMP_PKT_TYPE_REQ, id, ILOP_VER,
                          METHOD_EVENT_ZC_ENROLLEE, param, 0, packet,
                          &packet_len);
        HAL_Free(param);
    }
...
static void enrollee_report(void)
{
    ...
    for (i = 0; i < MAX_ENROLLEE_NUM; i++) {
        ...
        case ENR_FOUND:
            {
                if (time_elapsed_ms_since(enrollee->report_timestamp) >
        ...
        case ENR_IN_QUEUE:
            {
                ...
                uint16_t payload_len =
                    1 + enrollee->dev_name_len + 1 + enrollee->pk_len + 1 +
                    enrollee->rand_len + 3 + enrollee->sign_len;
                ...
                memcpy(&payload[idx], enrollee->dev_name,
                       enrollee->dev_name_len);
...
int enrollee_put(struct enrollee_info *in)
{
    ...
    do {
        ...
        /* reduce stack used */
        if (in == NULL ||
            !HAL_Sys_Net_Is_Ready()) { /* not ready to work as registerar */
            return -1;
        }
#if defined(AWSS_SUPPORT_AHA)
        HAL_Wifi_Get_Ap_Info(ssid, NULL, NULL);
    ...
    for (i = 0; i < MAX_ENROLLEE_NUM; i++) {
        if (enrollee_info[i].state) {
            if (in->dev_name_len == enrollee_info[i].dev_name_len &&
                0 == memcmp(in->dev_name, enrollee_info[i].dev_name,
                            enrollee_info[i].dev_name_len) &&
                in->pk_len == enrollee_info[i].pk_len &&
                0 == memcmp(in->pk, enrollee_info[i].pk,
                            enrollee_info[i].pk_len)) {
                if (enrollee_info[i].state == ENR_FOUND &&
                    time_elapsed_ms_since(enrollee_info[i].report_timestamp) >
                        enrollee_info[i].interval * 1000) {
                    enrollee_report();
                }
                if (enrollee_info[i].state !=
                    ENR_IN_QUEUE) { /* already reported */
                    if ((0 != memcmp(&(enrollee_info[i].random[0]),
                                     &(in->random[0]), RANDOM_MAX_LEN)) &&
                        (ENR_CHECKIN_ONGOING == enrollee_info[i].state)) {
                        enrollee_info[i].state = ENR_CHECKIN_ENABLE;
                        memset(&(enrollee_info[i].random[0]), 0,
                               RANDOM_MAX_LEN);
                        memcpy(&(enrollee_info[i].random[0]), &(in->random[0]),
                               RANDOM_MAX_LEN);
                        memset(&(enrollee_info[i].sign), 0,
                               enrollee_info[i].sign_len);
                        enrollee_info[i].sign_len = in->sign_len;
                        memcpy(&(enrollee_info[i].sign), &(in->sign),
                               in->sign_len);
                        awss_trace("fix diff random, %s, %s", in->dev_name,
                                   enrollee_info[i].dev_name);
                    } /* Fix device name not match issue.
                      It happens only when the enrollee reboots during
                      wifi-provision provison process, thus the random and sign
                      changes. The registar is not aware of that, keeping on
                      broadcasting wifi-provision info encrypted with previous
                      random and sign. To fix this issue, the registar has to
                      regenerate wifi-provision info when it detectes enrollee's
                      random changes. */

                    return 1;
                }
                memcpy(&enrollee_info[i], in, ENROLLEE_INFO_HDR_SIZE);
                enrollee_info[i].rssi =
                    (2 * enrollee_info[i].rssi + in->rssi) / 3;
                return 1; /* wait for report */
            }
        } else if (enrollee_info[i].state == ENR_FREE &&
                   empty_slot >= MAX_ENROLLEE_NUM) {
            empty_slot = i;
        }
    }
    ...
    awss_debug("new enrollee[%d] dev_name:%s time:%x", empty_slot, in->dev_name,
               os_get_time_ms());
    enrollee_report();
...
extern const uint8_t *cfg80211_find_vendor_ie(uint32_t oui, uint8_t oui_type,
 ...
 * @note None.
 */
void awss_wifi_mgnt_frame_callback(uint8_t *buffer, int length,
                                   signed char rssi, int buffer_type)
{
...
#define MGMT_HDR_LEN    (24)
    ...
    case MGMT_BEACON:
        /* awss_trace("beacon"); */
        buffer += MGMT_HDR_LEN +
                  12; /* hdr(24) + 12(timestamp, beacon_interval, cap) */
...
static void registrar_raw_frame_init(struct enrollee_info *enr)
{
    ...
    {
        p_Aes128_t aes = infra_aes128_init(&enr->key[0], enr->random,
                                           PLATFORM_AES_ENCRYPTION);
        infra_aes128_cfb_encrypt(aes, (uint8_t *)passwd, passwd_len,
