...
int iotx_dm_subscribe(_IN_ int devid)
{
    ...
    _dm_api_lock();
    res = dm_mgr_search_device_by_devid(devid, product_key, device_name,
                                        device_secret);
    if (res < SUCCESS_RETURN) {
        _dm_api_unlock();
        return res;
...
#endif
    while (CONFIG_DISPATCH_QUEUE_MAXLEN == 0 ||
           count++ < CONFIG_DISPATCH_QUEUE_MAXLEN) {
...
int iotx_dm_subscribe_user_topic(char *topic, void *user_callback)
{
    ...
    res = dm_utils_service_name((char *)DM_URI_SYS_PREFIX, topic, product_key,
                                device_name, &uri);
...

    {
        ...
        int local_sub = 0;
        do {
            res = dm_client_subscribe(uri, dm_client_user_sub_request,
                                      &local_sub);
...
int iotx_dm_post_rawdata(_IN_ int devid, _IN_ char *payload,
...
int iotx_dm_property_desired_get(_IN_ int devid, _IN_ char *payload,
                                 _IN_ int payload_len)
{
    int res = 0;
    ...
    res =
        dm_mgr_upstream_thing_property_desired_get(devid, payload, payload_len);
    if (res < SUCCESS_RETURN) {
        _dm_api_unlock();
        return FAIL_RETURN;
    }
...
int iotx_dm_property_desired_delete(_IN_ int devid, _IN_ char *payload,
                                    _IN_ int payload_len)
{
    int res = 0;
    ...
    res = dm_mgr_upstream_thing_property_desired_delete(devid, payload,
                                                        payload_len);
    if (res < SUCCESS_RETURN) {
        _dm_api_unlock();
        return FAIL_RETURN;
...
int iotx_dm_post_property(_IN_ int devid, _IN_ char *payload,
                          _IN_ int payload_len)
{
    int res = 0;
...
int iotx_dm_post_event(_IN_ int devid, _IN_ char *identifier,
                       _IN_ int identifier_len, _IN_ char *payload,
                       _IN_ int payload_len)
{
    ...
    res = dm_mgr_upstream_thing_event_post(devid, identifier, identifier_len,
...
int iotx_dm_send_service_response(_IN_ int devid, _IN_ char *msgid,
                                  _IN_ int msgid_len,
                                  ...
                                  _IN_ int payload_len, void *ctx)
{
    ...
    dm_log_debug("Current Service Response Payload, Length: %d, Payload: %.*s",
                 payload_len, payload_len, payload);

    res = dm_mgr_upstream_thing_service_response(devid, msgid, msgid_len, code,
...
int iotx_dm_send_property_get_response(_IN_ int devid, _IN_ char *msgid,
                                       _IN_ int msgid_len,
                                       _IN_ iotx_dm_error_code_t code,
                                       ...
                                       void *ctx)
{
    ...
    dm_log_debug(
        "Current Property Get Response Payload, Length: %d, Payload: %.*s",
        payload_len, payload_len, payload);

    res = dm_mgr_upstream_thing_property_get_response(
        devid, msgid, msgid_len, code, payload, payload_len, ctx);

    _dm_api_unlock();
    return res;
}

int iotx_dm_deviceinfo_update(_IN_ int devid, _IN_ char *payload,
                              _IN_ int payload_len)
{
    int res = 0;
...
int iotx_dm_deviceinfo_delete(_IN_ int devid, _IN_ char *payload,
                              _IN_ int payload_len)
{
    int res = 0;
...
int iotx_dm_send_rrpc_response(_IN_ int devid, _IN_ char *msgid,
                               _IN_ int msgid_len,
                               _IN_ iotx_dm_error_code_t code,
                               ...
                               _IN_ char *payload, _IN_ int payload_len)
{
    ...
    res = dm_mgr_upstream_rrpc_response(devid, msgid, msgid_len, code, rrpcid,
                                        rrpcid_len, payload, payload_len);
...
int iotx_dm_cota_get_config(_IN_ const char *config_scope, const char *get_type,
                         ...
                         _OU_ int *devid)
{
    ...
    if (product_key == NULL || device_name == NULL ||
        ...
        (strlen(device_name) >= IOTX_DEVICE_NAME_LEN + 1) || devid == NULL) {
        return DM_INVALID_PARAMETER;
    }
                          ...
                          _OU_ int *devid)
{
    ...
    _dm_api_lock();
    res =
        dm_mgr_device_create(IOTX_DM_DEVICE_SUBDEV, product_key, product_secret,
                             device_name, device_secret, devid);
    if (res != SUCCESS_RETURN) {
        _dm_api_unlock();
        return FAIL_RETURN;
    }
...
int iotx_dm_subdev_register(_IN_ int devid)
{
    ...
    if ((strlen(search_node->device_secret) > 0) &&
        (strlen(search_node->device_secret) < IOTX_DEVICE_SECRET_LEN + 1)) {
        _dm_api_unlock();
        return SUCCESS_RETURN;
    }
...
int iotx_dm_get_device_avail_status(_IN_ int devid,
...
int iotx_dm_firmware_version_update(_IN_ int devid, _IN_ char *payload,
                                    _IN_ int payload_len)
{
    int res = 0;
    ...
    _dm_api_lock();
    res = dm_mgr_upstream_thing_firmware_version_update(devid, payload,
                                                        payload_len);
    if (res < SUCCESS_RETURN) {
        _dm_api_unlock();
        return FAIL_RETURN;
...
int iotx_dm_send_firmware_version(int devid, const char *version)
{
    ...
    int ret = HAL_Snprintf(msg, FIRMWARE_VERSION_MSG_LEN,
                           "{\"id\":\"%d\",\"params\":{\"version\":\"%s\"}}",
                           iotx_report_id(), version);
    if (ret <= 0) {
...
int iotx_dm_deprecated_set_tsl(_IN_ int devid, _IN_ iotx_dm_tsl_source_t source,
                               _IN_ const char *tsl, _IN_ int tsl_len)
{
    int res = 0;
    ...
    if (source == IOTX_DM_TSL_SOURCE_LOCAL) {
        ...
        res = dm_mgr_deprecated_set_tsl(devid, IOTX_DM_TSL_TYPE_ALINK, tsl,
                                        tsl_len);
        if (res != SUCCESS_RETURN) {
            _dm_api_unlock();
            return FAIL_RETURN;
...
int iotx_dm_deprecated_set_property_value(_IN_ int devid, _IN_ char *key,
                                          ...
                                          _IN_ int value_len)
{
    ...
    _dm_api_lock();
    res = dm_mgr_deprecated_set_property_value(devid, key, key_len, value,
                                               value_len);
    if (res != SUCCESS_RETURN) {
        _dm_api_unlock();
        return FAIL_RETURN;
...
int iotx_dm_deprecated_get_property_value(_IN_ int devid, _IN_ char *key,
                                          _IN_ int key_len, _IN_ void *value)
{
    int res = 0;
...
int iotx_dm_deprecated_set_event_output_value(_IN_ int devid, _IN_ char *key,
                                              _IN_ int key_len,
                                              _IN_ void *value,
                                              _IN_ int value_len)
{
    ...
    _dm_api_lock();
    res = dm_mgr_deprecated_set_event_output_value(devid, key, key_len, value,
                                                   value_len);
    if (res != SUCCESS_RETURN) {
        _dm_api_unlock();
        return FAIL_RETURN;
...
int iotx_dm_deprecated_get_event_output_value(_IN_ int devid, _IN_ char *key,
                                              _IN_ int key_len,
                                              _IN_ void *value)
{
    int res = 0;
...
int iotx_dm_deprecated_get_service_input_value(_IN_ int devid, _IN_ char *key,
                                               _IN_ int key_len,
                                               _IN_ void *value)
{
    int res = 0;
...
int iotx_dm_deprecated_set_service_output_value(_IN_ int devid, _IN_ char *key,
                                                _IN_ int key_len,
                                                _IN_ void *value,
                                                _IN_ int value_len)
{
    ...
    _dm_api_lock();
    res = dm_mgr_deprecated_set_service_output_value(devid, key, key_len, value,
                                                     value_len);
    if (res != SUCCESS_RETURN) {
        _dm_api_unlock();
        return FAIL_RETURN;
...
int iotx_dm_deprecated_get_service_output_value(_IN_ int devid, _IN_ char *key,
                                                _IN_ int key_len,
                                                _IN_ void *value)
{
    int res = 0;
    ...
    _dm_api_lock();
    res =
        dm_mgr_deprecated_get_service_output_value(devid, key, key_len, value);
    if (res != SUCCESS_RETURN) {
        _dm_api_unlock();
        return FAIL_RETURN;
    }
...
static int _iotx_dm_deprecated_post_property_add(_IN_ void *handle,
                                                 _IN_ char *identifier,
                                                 _IN_ int identifier_len)
{
    ...
    res = dm_mgr_deprecated_assemble_property(
        dapi_property->devid, identifier, identifier_len, dapi_property->lite);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
...
int iotx_dm_deprecated_post_property_add(_IN_ void *handle,
                                         _IN_ char *identifier,
                                         _IN_ int identifier_len)
{
    ...
    if (identifier != IOTX_DM_POST_PROPERTY_ALL) {
        ...
        ret = _iotx_dm_deprecated_post_property_add(handle, identifier,
                                                    identifier_len);
    ...
    for (index = 0; index < number; index++) {
        ...
        res = dm_mgr_deprecated_get_property_by_index(dapi_property->devid,
                                                      index, &property_refer);
        if (res != SUCCESS_RETURN) {
            continue;
        }

        res = dm_mgr_deprecated_get_property_identifier(property_refer,
                                                        &identifier_refer);
        if (res != SUCCESS_RETURN) {
            continue;
        }

        res = _iotx_dm_deprecated_post_property_add(handle, identifier_refer,
                                                    strlen(identifier_refer));
        if (res != SUCCESS_RETURN) {
            ret = FAIL_RETURN;
        }
    }
...
int iotx_dm_deprecated_post_property_end(_IN_ void **handle)
{
    ...
    dm_log_debug("Current Property Post Payload, Length: %d, Payload: %s",
                 strlen(payload), payload);

    res = dm_mgr_upstream_thing_property_post(dapi_property->devid, payload,
                                              strlen(payload));
                                  ...
                                  _IN_ int identifier_len)
{
    ...
    res = dm_mgr_deprecated_assemble_event_output(devid, identifier,
                                                  identifier_len, lite);
    if (res != SUCCESS_RETURN) {
    ...
    dm_log_debug("Current Event Post Payload, Length: %d, Payload: %s",
                 strlen(payload), payload);
...
int iotx_dm_deprecated_legacy_set_property_value(_IN_ int devid, _IN_ char *key,
                                                 _IN_ int key_len,
                                                 _IN_ void *value,
                                                 _IN_ char *value_str)
{
    ...
    case DM_SHW_DATA_TYPE_BOOL:
        {
            int value_int =
                (value == NULL) ? (atoi(value_str)) : (*(int *)value);
            res = dm_mgr_deprecated_set_property_value(devid, key, key_len,
                                                       &value_int, sizeof(int));
        }
        break;
    case DM_SHW_DATA_TYPE_FLOAT:
        {
            float value_float =
                (value == NULL) ? (atof(value_str)) : (*(float *)value);
            res = dm_mgr_deprecated_set_property_value(
                devid, key, key_len, &value_float, sizeof(float));
        }
        break;
    case DM_SHW_DATA_TYPE_DOUBLE:
        {
            double value_double =
                (value == NULL) ? (atof(value_str)) : (*(double *)value);
            res = dm_mgr_deprecated_set_property_value(
                devid, key, key_len, &value_double, sizeof(double));
        }
        break;
    ...
    case DM_SHW_DATA_TYPE_DATE:
        {
            ...
            res = dm_mgr_deprecated_set_property_value(
                devid, key, key_len, value_string, strlen(value_string));
        }
        break;
    default:
        {
...
int iotx_dm_deprecated_legacy_get_property_value(_IN_ int devid, _IN_ char *key,
                                                 _IN_ int key_len,
                                                 _IN_ void *value,
                                                 _IN_ char **value_str)
{
    ...
    case DM_SHW_DATA_TYPE_BOOL:
        {
            int value_int = 0;
            res = dm_mgr_deprecated_get_property_value(devid, key, key_len,
                                                       (void *)&value_int);
            if (res == SUCCESS_RETURN) {
                if (value) {
                    *(int *)value = value_int;
...
int iotx_dm_deprecated_legacy_set_event_output_value(_IN_ int devid,
                                                     _IN_ char *key,
                                                     _IN_ int key_len,
                                                     _IN_ void *value,
                                                     _IN_ char *value_str)
    ...
    case DM_SHW_DATA_TYPE_BOOL:
        {
            int value_int =
                (value == NULL) ? (atoi(value_str)) : (*(int *)value);
            res = dm_mgr_deprecated_set_event_output_value(
                devid, key, key_len, &value_int, sizeof(int));
        }
        break;
    case DM_SHW_DATA_TYPE_FLOAT:
        {
...
int iotx_dm_deprecated_legacy_get_event_output_value(_IN_ int devid,
                                                     _IN_ char *key,
                                                     _IN_ int key_len,
                                                     _IN_ void *value,
                                                     _IN_ char **value_str)
{
    ...
    case DM_SHW_DATA_TYPE_BOOL:
        {
            int value_int = 0;
            res = dm_mgr_deprecated_get_event_output_value(devid, key, key_len,
                                                           (void *)&value_int);
            if (res == SUCCESS_RETURN) {
                if (value) {
                    *(int *)value = value_int;
    ...
    case DM_SHW_DATA_TYPE_FLOAT:
        {
            float value_float = 0;
            res = dm_mgr_deprecated_get_event_output_value(
                devid, key, key_len, (void *)&value_float);
            if (res == SUCCESS_RETURN) {
                if (value) {
                    *(float *)value = value_float;
                }
    ...
    case DM_SHW_DATA_TYPE_DOUBLE:
        {
            double value_double = 0;
            res = dm_mgr_deprecated_get_event_output_value(
                devid, key, key_len, (void *)&value_double);
            if (res == SUCCESS_RETURN) {
                if (value) {
                    *(double *)value = value_double;
                }
    ...
    case DM_SHW_DATA_TYPE_DATE:
        {
            char *value_string = NULL;
            res = dm_mgr_deprecated_get_event_output_value(
                devid, key, key_len, (void *)&value_string);
            if (res == SUCCESS_RETURN) {
                if (value) {
                    memcpy(value, value_string, strlen(value_string));
                }
...
int iotx_dm_deprecated_legacy_get_service_input_value(_IN_ int devid,
                                                      _IN_ char *key,
                                                      _IN_ int key_len,
                                                      _IN_ void *value,
                                                      _IN_ char **value_str)
{
    ...
    case DM_SHW_DATA_TYPE_BOOL:
        {
            int value_int = 0;
            res = dm_mgr_deprecated_get_service_input_value(devid, key, key_len,
                                                            (void *)&value_int);
            if (res == SUCCESS_RETURN) {
                if (value) {
                    *(int *)value = value_int;
    ...
    case DM_SHW_DATA_TYPE_FLOAT:
        {
            float value_float = 0;
            res = dm_mgr_deprecated_get_service_input_value(
                devid, key, key_len, (void *)&value_float);
            if (res == SUCCESS_RETURN) {
                if (value) {
                    *(float *)value = value_float;
...
int iotx_dm_deprecated_legacy_set_service_output_value(_IN_ int devid,
                                                       _IN_ char *key,
                                                       _IN_ int key_len,
                                                       _IN_ void *value,
                                                       _IN_ char *value_str)
    ...
    case DM_SHW_DATA_TYPE_BOOL:
        {
            int value_int =
                (value == NULL) ? (atoi(value_str)) : (*(int *)value);
            res = dm_mgr_deprecated_set_service_output_value(
                devid, key, key_len, &value_int, sizeof(int));
        }
        break;
    case DM_SHW_DATA_TYPE_FLOAT:
...
int iotx_dm_deprecated_legacy_get_service_output_value(_IN_ int devid,
                                                       _IN_ char *key,
                                                       _IN_ int key_len,
                                                       _IN_ void *value,
                                                       _IN_ char **value_str)
{
    ...
    case DM_SHW_DATA_TYPE_BOOL:
        {
            int value_int = 0;
            res = dm_mgr_deprecated_get_service_output_value(
                devid, key, key_len, (void *)&value_int);
            if (res == SUCCESS_RETURN) {
                if (value) {
                    *(int *)value = value_int;
                }
...
int iotx_dm_deprecated_legacy_get_pkdn_by_devid(
...
int iotx_dm_deprecated_legacy_get_devid_by_pkdn(
...
int iotx_dm_deprecated_legacy_get_thingid_by_devid(_IN_ int devid,
                                                   _OU_ void **thing_id)
{
    int res = 0;
...
int iotx_dm_deprecated_legacy_get_devid_by_thingid(_IN_ void *thing_id,
                                                   _OU_ int *devid)
{
    int res = 0;
...
int iotx_dm_deprecated_legacy_get_pkdn_ptr_by_devid(_IN_ int devid,
...
int iotx_dm_deprecated_legacy_send_service_response(
    ...
    _IN_ int payload_len)
{
    ...
    res = dm_mgr_deprecated_upstream_thing_service_response(
        devid, msgid, code, identifier, identifier_len, payload, payload_len);

    _dm_api_unlock();
    return res;
}

int iotx_dm_deprecated_send_service_response(_IN_ int devid, _IN_ int msgid,
                                             _IN_ iotx_dm_error_code_t code,
                                             _IN_ char *identifier,
                                             _IN_ int identifier_len)
{
    ...
    res = dm_mgr_deprecated_assemble_service_output(devid, identifier,
                                                    identifier_len, lite);
    if (res != SUCCESS_RETURN) {
        lite_cjson_delete(lite);
        _dm_api_unlock();
    ...
    res = dm_mgr_deprecated_upstream_thing_service_response(
        devid, msgid, code, identifier, identifier_len, payload,
        strlen(payload));
...
#ifdef DEVICE_MODEL_GATEWAY
int iotx_dm_deprecated_subdev_register(
    _IN_ int devid, _IN_ char device_secret[IOTX_DEVICE_SECRET_LEN + 1])
{
    ...
    if ((device_secret != NULL) && (strlen(device_secret) > 0) &&
        ...
        memcpy(search_node->device_secret, device_secret,
