...
const char DM_URI_DEV_CORE_SERVICE_DEV_NOTIFY[] DM_READ_ONLY =
    "/dev/core/service/dev/notify";
...
static int _dm_server_dev_notify(void *handle)
{
    ...
    payload_len = strlen(DM_MSG_REQUEST) + 10 + strlen(DM_MSG_VERSION) +
                  data_len + strlen(ALCS_NOTIFY_METHOD) + 1;
    ...
    dm_log_debug("notify path:%s; payload = %s",
                 DM_URI_DEV_CORE_SERVICE_DEV_NOTIFY, payload);

    for (i = 0; i < 2; i++) {
        ret = CoAPServerMultiCast_send(
            g_coap_ctx, &notify_sa, DM_URI_DEV_CORE_SERVICE_DEV_NOTIFY,
            (uint8_t *)payload, (uint16_t)payload_len, NULL, NULL);
    }
    DM_free(payload);
    DM_free(data);
    return ret;
...
int dm_server_send(char *uri, unsigned char *payload, int payload_len,
                   void *context)
{
    int res = 0;
    dm_server_ctx_t *ctx = dm_server_get_ctx();
    iotx_alcs_msg_t alcs_msg;
    dm_server_alcs_context_t *alcs_context =
        (dm_server_alcs_context_t *)context;
    ...
    alcs_msg.msg_code =
        (alcs_context && alcs_context->token_len && alcs_context->token)
            ? ITOX_ALCS_COAP_MSG_CODE_205_CONTENT
    ...
    if (alcs_context == NULL) {
        res = iotx_alcs_observe_notify(ctx->conn_handle, alcs_msg.uri,
                                       alcs_msg.payload_len, alcs_msg.payload);
        dm_log_info("Send Observe Notify Result %d", res);
    } else if (alcs_context->ip && alcs_context->port &&
               NULL == alcs_context->token) {
        res = iotx_alcs_send(ctx->conn_handle, &alcs_msg);
        dm_log_info("Send Result %d", res);
    } else if (alcs_context->ip && alcs_context->port &&
               alcs_context->token_len && alcs_context->token) {
        res = iotx_alcs_send_Response(ctx->conn_handle, &alcs_msg,
                                      (uint8_t)alcs_context->token_len,
...
int dm_server_add_device(char product_key[IOTX_PRODUCT_KEY_LEN + 1],
                         char device_name[IOTX_DEVICE_NAME_LEN + 1])
{
    int res = 0;
    dm_server_ctx_t *ctx = dm_server_get_ctx();

    res = iotx_alcs_add_sub_device(ctx->conn_handle, (const char *)product_key,
                                   (const char *)device_name);
    dm_log_info("Add Device Result: %d, Product Key: %s, Device Name: %s", res,
                product_key, device_name);

    return res;
}

int dm_server_del_device(char product_key[IOTX_PRODUCT_KEY_LEN + 1],
                         char device_name[IOTX_DEVICE_NAME_LEN + 1])
{
    int res = 0;
    dm_server_ctx_t *ctx = dm_server_get_ctx();

    res = iotx_alcs_remove_sub_device(
        ctx->conn_handle, (const char *)product_key, (const char *)device_name);
    dm_log_info("Del Device Result: %d, Product Key: %s, Device Name: %s", res,
                product_key, device_name);
