...
int dm_msg_cache_deinit(void)
{
    ...
    list_for_each_entry_safe(node, next, &ctx->dmc_list, linked_list,
                             dm_msg_cache_node_t)
    {
...
int dm_msg_cache_insert(int msgid, int devid, iotx_dm_event_types_t type,
                        char *data)
{
    dm_msg_cache_ctx_t *ctx = _dm_msg_cache_get_ctx();
    dm_msg_cache_node_t *node = NULL;
...
int dm_msg_cache_search(_IN_ int msgid, _OU_ dm_msg_cache_node_t **node)
{
    ...
    _dm_msg_cache_mutex_lock();
    list_for_each_entry(search_node, &ctx->dmc_list, linked_list,
                        dm_msg_cache_node_t)
    {
...
void dm_msg_cache_tick(void)
{
                             ...
                             dm_msg_cache_node_t)
    {
        ...
        if (current_time - node->ctime >= DM_MSG_CACHE_TIMEOUT_MS_DEFAULT) {
            ...
            dm_msg_send_msg_timeout_to_user(node->msgid, node->devid,
                                            node->response_type);
            list_del(&node->linked_list);
            if (node->data) {
                DM_free(node->data);
