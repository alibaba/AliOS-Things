...
int dm_client_subscribe(char *uri, iotx_cm_data_handle_cb callback,
                        void *context)
{
    int res = 0;
    uint8_t local_sub = 0;
                      ...
                      iotx_cm_data_handle_cb callback)
{
    ...
    res = iotx_cm_pub(ctx->fd, &pub_param, (const char *)pub_uri,
...
void dm_client_user_sub_request(int fd, const char *topic, const char *payload,
