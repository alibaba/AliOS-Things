...
static int _impl_copy(_IN_ void *input, _IN_ int input_len, _OU_ void **output,
                      _IN_ int output_len)
{
...
static int _linkkit_service_list_insert(iotx_service_req_type_t type,
                                        char *msgid, char *rrpcid,
                                        int rrpcid_len, void *alcs_ctx,
                                        void **p_node)
{
    ...
    insert_node = (iotx_service_ctx_node_t *)IMPL_LINKKIT_MALLOC(
        sizeof(iotx_service_ctx_node_t));
    if (insert_node == NULL) {
...
static int _linkkit_service_list_search(iotx_service_ctx_node_t *node)
{
    ...
    _linkkit_service_list_mutex_lock();
    list_for_each_entry(search_node, &ctx->downstream_service_list, linked_list,
                        iotx_service_ctx_node_t)
    {
        if (search_node == node) {
            _linkkit_service_list_mutex_unlock();
...
static int _linkkit_service_list_destroy(void)
{
    ...
    _linkkit_service_list_mutex_lock();
    list_for_each_entry_safe(search_node, temp, &ctx->downstream_service_list,
                             linked_list, iotx_service_ctx_node_t)
    {
...
void iotx_linkkit_service_list_overtime_handle(void)
{
                             ...
                             linked_list, iotx_service_ctx_node_t)
    {
        ...
        if (current_time - search_node->ctime >=
            ...
            dm_log_warning("service request timeout, msgid = %d",
                           search_node->msgid);
            /* TODO: notify user? */
...
static int _iotx_linkkit_upstream_sync_callback_list_insert(
    int msgid, void *semaphore,
    iotx_linkkit_upstream_sync_callback_node_t **node)
{
    iotx_linkkit_ctx_t *ctx = _iotx_linkkit_get_ctx();
    iotx_linkkit_upstream_sync_callback_node_t *search_node = NULL;

    list_for_each_entry(search_node, &ctx->upstream_sync_callback_list,
                        linked_list, iotx_linkkit_upstream_sync_callback_node_t)
    {
    ...
    search_node =
        IMPL_LINKKIT_MALLOC(sizeof(iotx_linkkit_upstream_sync_callback_node_t));
    if (search_node == NULL) {
...
static int _iotx_linkkit_upstream_sync_callback_list_search(
    int msgid, iotx_linkkit_upstream_sync_callback_node_t **node)
{
    iotx_linkkit_ctx_t *ctx = _iotx_linkkit_get_ctx();
    iotx_linkkit_upstream_sync_callback_node_t *search_node = NULL;
...
static void _iotx_linkkit_upstream_sync_callback_list_destroy(void)
{
    ...
    list_for_each_entry_safe(search_node, next_node,
                             &ctx->upstream_sync_callback_list, linked_list,
                             iotx_linkkit_upstream_sync_callback_node_t)
    {
        list_del(&search_node->linked_list);
        HAL_SemaphoreDestroy(search_node->semaphore);
...
static void _iotx_linkkit_upstream_callback_remove(int msgid, int code)
{
    ...
    if (res == SUCCESS_RETURN) {
        sync_node->code = (code == IOTX_DM_ERR_CODE_SUCCESS) ? (SUCCESS_RETURN)
                                                             : (FAIL_RETURN);
...
static void _iotx_linkkit_event_callback(iotx_dm_event_types_t type,
                                         char *payload)
{
    int res = 0;
    void *callback;
    ...
    lite_cjson_t lite, lite_item_id, lite_item_devid, lite_item_serviceid,
        lite_item_payload, lite_item_ctx;
    lite_cjson_t lite_item_code, lite_item_eventid, lite_item_utc,
        lite_item_rrpcid, lite_item_topo;
    lite_cjson_t lite_item_pk, lite_item_time;
    lite_cjson_t lite_item_version, lite_item_configid, lite_item_configsize,
        lite_item_gettype, lite_item_sign, lite_item_signmethod, lite_item_url,
        lite_item_data, lite_item_message;

    dm_log_info("Receive Message Type: %d", type);
    if (payload) {
        dm_log_info("Receive Message: %s", payload);
        res =
            dm_utils_json_parse(payload, strlen(payload), cJSON_Invalid, &lite);
        if (res != SUCCESS_RETURN) {
            return;
        }
#ifdef LOG_REPORT_TO_CLOUD
...
#endif
        dm_utils_json_object_item(&lite, IOTX_LINKKIT_KEY_ID,
                                  strlen(IOTX_LINKKIT_KEY_ID), cJSON_Invalid,
                                  &lite_item_id);
        dm_utils_json_object_item(&lite, IOTX_LINKKIT_KEY_DEVID,
                                  strlen(IOTX_LINKKIT_KEY_DEVID), cJSON_Invalid,
                                  &lite_item_devid);
        dm_utils_json_object_item(&lite, IOTX_LINKKIT_KEY_SERVICEID,
                                  strlen(IOTX_LINKKIT_KEY_SERVICEID),
                                  cJSON_Invalid, &lite_item_serviceid);
        dm_utils_json_object_item(&lite, IOTX_LINKKIT_KEY_PAYLOAD,
                                  strlen(IOTX_LINKKIT_KEY_PAYLOAD),
                                  cJSON_Invalid, &lite_item_payload);
        dm_utils_json_object_item(&lite, IOTX_LINKKIT_KEY_CTX,
                                  strlen(IOTX_LINKKIT_KEY_CTX), cJSON_Invalid,
                                  &lite_item_ctx);
        dm_utils_json_object_item(&lite, IOTX_LINKKIT_KEY_CODE,
                                  strlen(IOTX_LINKKIT_KEY_CODE), cJSON_Invalid,
                                  &lite_item_code);
        dm_utils_json_object_item(&lite, IOTX_LINKKIT_KEY_EVENTID,
                                  strlen(IOTX_LINKKIT_KEY_EVENTID),
                                  cJSON_Invalid, &lite_item_eventid);
        dm_utils_json_object_item(&lite, IOTX_LINKKIT_KEY_UTC,
                                  strlen(IOTX_LINKKIT_KEY_UTC), cJSON_Invalid,
                                  &lite_item_utc);
        dm_utils_json_object_item(&lite, IOTX_LINKKIT_KEY_RRPCID,
                                  strlen(IOTX_LINKKIT_KEY_RRPCID),
                                  cJSON_Invalid, &lite_item_rrpcid);
        dm_utils_json_object_item(&lite, IOTX_LINKKIT_KEY_TOPO,
                                  strlen(IOTX_LINKKIT_KEY_TOPO), cJSON_Invalid,
                                  &lite_item_topo);
        dm_utils_json_object_item(&lite, IOTX_LINKKIT_KEY_PRODUCT_KEY,
                                  strlen(IOTX_LINKKIT_KEY_PRODUCT_KEY),
                                  cJSON_Invalid, &lite_item_pk);
        dm_utils_json_object_item(&lite, IOTX_LINKKIT_KEY_TIME,
                                  strlen(IOTX_LINKKIT_KEY_TIME), cJSON_Invalid,
                                  &lite_item_time);
        dm_utils_json_object_item(&lite, IOTX_LINKKIT_KEY_VERSION,
                                  strlen(IOTX_LINKKIT_KEY_VERSION),
                                  cJSON_Invalid, &lite_item_version);
        dm_utils_json_object_item(&lite, IOTX_LINKKIT_KEY_CONFIG_ID,
                                  strlen(IOTX_LINKKIT_KEY_CONFIG_ID),
                                  cJSON_Invalid, &lite_item_configid);
        dm_utils_json_object_item(&lite, IOTX_LINKKIT_KEY_CONFIG_SIZE,
                                  strlen(IOTX_LINKKIT_KEY_CONFIG_SIZE),
                                  cJSON_Invalid, &lite_item_configsize);
        dm_utils_json_object_item(&lite, IOTX_LINKKIT_KEY_GET_TYPE,
                                  strlen(IOTX_LINKKIT_KEY_GET_TYPE),
                                  cJSON_Invalid, &lite_item_gettype);
        dm_utils_json_object_item(&lite, IOTX_LINKKIT_KEY_SIGN,
                                  strlen(IOTX_LINKKIT_KEY_SIGN), cJSON_Invalid,
                                  &lite_item_sign);
        dm_utils_json_object_item(&lite, IOTX_LINKKIT_KEY_SIGN_METHOD,
                                  strlen(IOTX_LINKKIT_KEY_SIGN_METHOD),
                                  cJSON_Invalid, &lite_item_signmethod);
        dm_utils_json_object_item(&lite, IOTX_LINKKIT_KEY_URL,
                                  strlen(IOTX_LINKKIT_KEY_URL), cJSON_Invalid,
                                  &lite_item_url);
        dm_utils_json_object_item(&lite, IOTX_LINKKIT_KEY_DATA,
                                  strlen(IOTX_LINKKIT_KEY_DATA), cJSON_Invalid,
                                  &lite_item_data);
        dm_utils_json_object_item(&lite, IOTX_LINKKIT_KEY_MESSAGE,
                                  strlen(IOTX_LINKKIT_KEY_MESSAGE),
                                  cJSON_Invalid, &lite_item_message);
    }
    ...
    case IOTX_DM_EVENT_MODEL_DOWN_RAW:
        {
            ...
            dm_log_debug("Current Devid: %d", lite_item_devid.value_int);
            dm_log_debug("Current Raw Data: %.*s",
                         lite_item_payload.value_length,
                         lite_item_payload.value);
            ...
            LITE_hexstr_convert(lite_item_payload.value,
                                lite_item_payload.value_length, raw_data,
                                raw_data_len);
    ...
    case IOTX_DM_EVENT_THING_SERVICE_REQUEST:
        {
            ...
            if (payload == NULL || lite_item_id.type != cJSON_String ||
            ...
            dm_log_debug("Current Id: %.*s", lite_item_id.value_length,
                         lite_item_id.value);
            dm_log_debug("Current Devid: %d", lite_item_devid.value_int);
            dm_log_debug("Current ServiceID: %.*s",
                         lite_item_serviceid.value_length,
                         lite_item_serviceid.value);
            dm_log_debug("Current Payload: %.*s",
                         lite_item_payload.value_length,
                         lite_item_payload.value);
            dm_log_debug("Current Ctx: %.*s", lite_item_ctx.value_length,
                         lite_item_ctx.value);

            LITE_hexstr_convert(lite_item_ctx.value, lite_item_ctx.value_length,
            ...
            memcpy(request, lite_item_payload.value,
                   lite_item_payload.value_length);

            callback = iotx_event_callback(ITE_SERVICE_REQUEST);
            if (callback) {
                res =
                    ((int (*)(const int, const char *, const int, const char *,
                              const int, char **, int *))callback)(
                        lite_item_devid.value_int, lite_item_serviceid.value,
                        lite_item_serviceid.value_length, request,
                        lite_item_payload.value_length, &response,
                        &response_len);
                if (response != NULL && response_len > 0) {
                    /* service response exist */
                    iotx_dm_error_code_t code =
                        (res == 0) ? (IOTX_DM_ERR_CODE_SUCCESS)
                                   : (IOTX_DM_ERR_CODE_REQUEST_ERROR);
                    iotx_dm_send_service_response(
                        lite_item_devid.value_int, lite_item_id.value,
                        lite_item_id.value_length, code,
                        lite_item_serviceid.value,
                        lite_item_serviceid.value_length, response,
                        response_len, property_get_ctx);
                    HAL_Free(response);
                }
            }
            ...
            if (callback) {
                ...
                _linkkit_service_list_insert(IOTX_SERVICE_REQ_TYPE_GENERAL,
                                             lite_item_id.value, NULL, 0,
                                             property_get_ctx, &service_ctx);
                if (service_ctx != NULL) {
                    res = ((int (*)(int, const char *, int, const char *, int,
                                    const char *, int, void *))callback)(
                        lite_item_devid.value_int, lite_item_serviceid.value,
                        lite_item_serviceid.value_length, lite_item_id.value,
                        lite_item_id.value_length, request,
    ...
    case IOTX_DM_EVENT_PROPERTY_SET:
        {
            ...
            property_payload =
                IMPL_LINKKIT_MALLOC(lite_item_payload.value_length + 1);
            if (property_payload == NULL) {
                dm_log_err("No Enough Memory");
                return;
            }
            memset(property_payload, 0, lite_item_payload.value_length + 1);
            memcpy(property_payload, lite_item_payload.value,
                   lite_item_payload.value_length);
#ifdef LOG_REPORT_TO_CLOUD
            if (SUCCESS_RETURN ==
                check_target_msg(msg_id.value, msg_id.value_length)) {
                report_sample = 1;
            ...
            if (callback) {
                ((int (*)(const int, const char *, const int))callback)(
                    lite_item_devid.value_int, property_payload,
                    lite_item_payload.value_length);
            }
#ifdef LOG_REPORT_TO_CLOUD
            if (1 == report_sample) {
    ...
    case IOTX_DM_EVENT_PROPERTY_DESIRED_GET_REPLY:
        {
            ...
            dm_utils_json_object_item(&lite, IOTX_LINKKIT_KEY_DATA,
                                      strlen(IOTX_LINKKIT_KEY_DATA),
                                      cJSON_Invalid, &lite_item_data);
            if (payload == NULL || lite_item_data.type != cJSON_Object) {
                return;
            }
            dm_log_debug("Current Data: %.*s", lite_item_data.value_length,
                         lite_item_data.value);

            property_data =
                IMPL_LINKKIT_MALLOC(lite_item_data.value_length + 1);
            if (property_data == NULL) {
                dm_log_err("No Enough Memory");
                return;
            }
            memset(property_data, 0, lite_item_data.value_length + 1);
            memcpy(property_data, lite_item_data.value,
                   lite_item_data.value_length);

            callback = iotx_event_callback(ITE_PROPERTY_DESIRED_GET_REPLY);
            if (callback) {
                ((int (*)(const char *, const int))callback)(
                    property_data, lite_item_data.value_length);
            }
    ...
    case IOTX_DM_EVENT_PROPERTY_GET:
        {
            ...
            LITE_hexstr_convert(lite_item_ctx.value, lite_item_ctx.value_length,
                                ...
                                sizeof(uintptr_t));
            property_get_ctx = (void *)property_get_ctx_num;
            dm_log_debug("property_get_ctx_num: %0x016llX",
                         (unsigned int)property_get_ctx_num);
            ...
            if (callback) {
                res = ((int (*)(const int, const char *, const int, char **,
                                int *))callback)(
                    lite_item_devid.value_int, request,
                ...
                if (response != NULL && response_len > 0) {
                    ...
                    iotx_dm_send_property_get_response(
                        lite_item_devid.value_int, lite_item_id.value,
                        lite_item_id.value_length, code, response, response_len,
                        property_get_ctx);
                    HAL_Free(response);
                }
            }
    ...
    case IOTX_DM_EVENT_DEVICEINFO_DELETE_REPLY:
        {
            ...
            if (payload == NULL || lite_item_id.type != cJSON_Number ||
            ...
            if (lite_item_payload.type == cJSON_Object &&
                lite_item_payload.value_length > 0) {
                user_payload =
                    IMPL_LINKKIT_MALLOC(lite_item_payload.value_length + 1);
                ...
                memcpy(user_payload, lite_item_payload.value,
                       lite_item_payload.value_length);
            ...
            if (callback) {
                ((int (*)(const int, const int, const int, const char *,
                          const int))callback)(
                    lite_item_devid.value_int, lite_item_id.value_int,
                    lite_item_code.value_int, user_payload,
                    user_payload_length);
            }
    ...
    case IOTX_DM_EVENT_EVENT_SPECIFIC_POST_REPLY:
        {
            ...
            if (payload == NULL || lite_item_id.type != cJSON_Number ||
                lite_item_code.type != cJSON_Number ||
                lite_item_devid.type != cJSON_Number ||
                lite_item_eventid.type != cJSON_String ||
                lite_item_payload.type != cJSON_String) {
                return;
            }
            ...
            dm_log_debug("Current Code: %d", lite_item_code.value_int);
            dm_log_debug("Current Devid: %d", lite_item_devid.value_int);
            dm_log_debug("Current EventID: %.*s",
                         lite_item_eventid.value_length,
                         lite_item_eventid.value);
            dm_log_debug("Current Message: %.*s",
                         lite_item_payload.value_length,
                         lite_item_payload.value);

            user_eventid =
                IMPL_LINKKIT_MALLOC(lite_item_eventid.value_length + 1);
            if (user_eventid == NULL) {
                dm_log_err("Not Enough Memory");
                return;
            }
            memset(user_eventid, 0, lite_item_eventid.value_length + 1);
            memcpy(user_eventid, lite_item_eventid.value,
                   lite_item_eventid.value_length);
            ...
            if (callback) {
                ((int (*)(const int, const int, const int, const char *,
                          const int, const char *, const int))callback)(
                    lite_item_devid.value_int, lite_item_id.value_int,
                    lite_item_code.value_int, user_eventid,
                    lite_item_eventid.value_length, user_payload,
                    lite_item_payload.value_length);
            }
    ...
    case IOTX_DM_EVENT_NTP_RESPONSE:
        {
            ...
            dm_log_debug("Current UTC: %.*s", lite_item_utc.value_length,
                         lite_item_utc.value);
            ...
            memcpy(utc_payload, lite_item_utc.value,
                   lite_item_utc.value_length);
    ...
    case IOTX_DM_EVENT_RRPC_REQUEST:
        {
            ...
            if (payload == NULL || lite_item_id.type != cJSON_String ||
                lite_item_devid.type != cJSON_Number ||
                lite_item_serviceid.type != cJSON_String ||
                lite_item_rrpcid.type != cJSON_String ||
                lite_item_payload.type != cJSON_Object) {
                return;
            }
            ...
            dm_log_debug("Current ServiceID: %.*s",
                         lite_item_serviceid.value_length,
                         lite_item_serviceid.value);
            dm_log_debug("Current RRPC ID: %.*s", lite_item_rrpcid.value_length,
            ...
            memcpy(rrpc_request, lite_item_payload.value,
                   lite_item_payload.value_length);
            ...
            if (callback) {
                res =
                    ((int (*)(const int, const char *, const int, const char *,
                              const int, char **, int *))callback)(
                        lite_item_devid.value_int, lite_item_serviceid.value,
                        lite_item_serviceid.value_length, rrpc_request,
                        lite_item_payload.value_length, &rrpc_response,
                        &rrpc_response_len);
                if (rrpc_response != NULL && rrpc_response_len > 0) {
                    ...
                    iotx_dm_send_rrpc_response(
                        lite_item_devid.value_int, lite_item_id.value,
                        lite_item_id.value_length, code, lite_item_rrpcid.value,
                        lite_item_rrpcid.value_length, rrpc_response,
                        rrpc_response_len);
            ...
            if (callback) {
                void *service_ctx = NULL;
                _linkkit_service_list_insert(
                    IOTX_SERVICE_REQ_TYPE_RRPC, lite_item_id.value,
                    lite_item_rrpcid.value, lite_item_rrpcid.value_length, NULL,
                    &service_ctx);
                if (service_ctx != NULL) {
                    res = ((int (*)(int, const char *, int, const char *, int,
                                    const char *, int, void *))callback)(
                        lite_item_devid.value_int, lite_item_serviceid.value,
                        lite_item_serviceid.value_length, lite_item_id.value,
                        lite_item_id.value_length, rrpc_request,
                        lite_item_payload.value_length, service_ctx);
                }
            }
    ...
    case IOTX_DM_EVENT_FOTA_NEW_FIRMWARE:
        {
            ...
            dm_log_debug("Current Firmware Version: %.*s",
                         lite_item_version.value_length,
                         lite_item_version.value);
            ...
            memcpy(version, lite_item_version.value,
                   lite_item_version.value_length);
    ...
    case IOTX_DM_EVENT_COTA_NEW_CONFIG:
        {
            char *config_id = NULL, *get_type = NULL, *sign = NULL,
                 *sign_method = NULL, *url = NULL;

            if (payload == NULL || lite_item_configid.type != cJSON_String ||
                lite_item_configsize.type != cJSON_Number ||
                lite_item_gettype.type != cJSON_String ||
                lite_item_sign.type != cJSON_String ||
                lite_item_signmethod.type != cJSON_String ||
                lite_item_url.type != cJSON_String) {
                return;
            }

            dm_log_debug("Current Config ID: %.*s",
                         lite_item_configid.value_length,
                         lite_item_configid.value);
            dm_log_debug("Current Config Size: %d",
                         lite_item_configsize.value_int);
            dm_log_debug("Current Get Type: %.*s",
                         lite_item_gettype.value_length,
                         lite_item_gettype.value);
            dm_log_debug("Current Sign: %.*s", lite_item_sign.value_length,
                         lite_item_sign.value);
            dm_log_debug("Current Sign Method: %.*s",
                         lite_item_signmethod.value_length,
                         lite_item_signmethod.value);
            dm_log_debug("Current URL: %.*s", lite_item_url.value_length,
                         lite_item_url.value);

            _impl_copy(lite_item_configid.value,
                       lite_item_configid.value_length, (void **)&config_id,
                       lite_item_configid.value_length + 1);
            _impl_copy(lite_item_gettype.value, lite_item_gettype.value_length,
                       (void **)&get_type, lite_item_gettype.value_length + 1);
            _impl_copy(lite_item_sign.value, lite_item_sign.value_length,
                       (void **)&sign, lite_item_sign.value_length + 1);
            _impl_copy(lite_item_signmethod.value,
                       lite_item_signmethod.value_length, (void **)&sign_method,
                       lite_item_signmethod.value_length + 1);
            _impl_copy(lite_item_url.value, lite_item_url.value_length,
                       (void **)&url, lite_item_url.value_length + 1);

            if (config_id == NULL || get_type == NULL || sign == NULL ||
                sign_method == NULL || url == NULL) {
                if (config_id) {
            ...
            if (callback) {
                ((int (*)(const int, const char *, int, const char *,
                          const char *, const char *, const char *))callback)(
                    0, config_id, lite_item_configsize.value_int, get_type,
                    sign, sign_method, url);
            }
    ...
    case IOTX_DM_EVENT_CLOUD_ERROR:
        {
            ...
            err_detail =
                IMPL_LINKKIT_MALLOC(lite_item_message.value_length + 1);
            if (err_detail == NULL) {
                dm_log_err("Not Enough Memory");
            ...
            memcpy(err_detail, lite_item_message.value,
                   lite_item_message.value_length);

            callback = iotx_event_callback(ITE_CLOUD_ERROR);
            if (callback) {
                ((int (*)(int, const char *, const char *))callback)(
                    lite_item_code.value_int, err_data, err_detail);
            }
            IMPL_LINKKIT_FREE(err_data);
    ...
    case IOTX_DM_EVENT_TOPO_GET_REPLY:
        {
            ...
            if (payload == NULL || lite_item_id.type != cJSON_Number ||
                lite_item_devid.type != cJSON_Number ||
                lite_item_code.type != cJSON_Number ||
                lite_item_topo.type != cJSON_Array) {
                return;
            }
            dm_log_debug("Current Id: %d", lite_item_id.value_int);
            dm_log_debug("Current Devid: %d", lite_item_devid.value_int);
            dm_log_debug("Current Code: %d", lite_item_code.value_int);
            dm_log_debug("Current Topo List: %.*s", lite_item_topo.value_length,
                         lite_item_topo.value);
            ...
            memcpy(topo_list, lite_item_topo.value,
                   lite_item_topo.value_length);
            ...
            if (callback) {
                ((int (*)(const int, const int, const int, const char *,
                          const int))callback)(
                    lite_item_devid.value_int, lite_item_id.value_int,
                    lite_item_code.value_int, topo_list,
                    lite_item_topo.value_length);
            }
    ...
    case IOTX_DM_EVENT_COMBINE_LOGOUT_REPLY:
        {
            ...
            _iotx_linkkit_upstream_callback_remove(lite_item_id.value_int,
    ...
    case IOTX_DM_EVENT_GATEWAY_PERMIT:
        {
            ...
            if (lite_item_pk.type == cJSON_String) {
                dm_log_debug("Current Product Key: %.*s",
                             lite_item_pk.value_length, lite_item_pk.value);
                product_key =
                    IMPL_LINKKIT_MALLOC(lite_item_pk.value_length + 1);
                if (product_key == NULL) {
                    dm_log_err("Not Enough Memory");
                    return;
                }
                memset(product_key, 0, lite_item_pk.value_length + 1);
                memcpy(product_key, lite_item_pk.value,
                       lite_item_pk.value_length);
            }

            callback = iotx_event_callback(ITE_PERMIT_JOIN);
            if (callback) {
                ((int (*)(const char *, int))callback)(
                    (const char *)product_key,
                    (const int)lite_item_time.value_int);
            }
...
static int _iotx_linkkit_slave_open(iotx_linkkit_dev_meta_info_t *meta_info)
{
    ...
    res = iotx_dm_subdev_create(
        meta_info->product_key, meta_info->product_secret,
        meta_info->device_name, meta_info->device_secret, &devid);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
...
static int _iotx_linkkit_slave_connect(int devid)
{
    ...
    res = iotx_dm_get_opt(DM_OPT_PROXY_PRODUCT_REGISTER,
                          (void *)&proxy_product_register);
    if (res < SUCCESS_RETURN) {
    ...
    if (res > SUCCESS_RETURN) {
        ...
        _iotx_linkkit_upstream_mutex_lock();
        res = _iotx_linkkit_upstream_sync_callback_list_insert(msgid, semaphore,
                                                               &node);
        if (res != SUCCESS_RETURN) {
        ...
        res =
            HAL_SemaphoreWait(semaphore, IOTX_LINKKIT_SYNC_DEFAULT_TIMEOUT_MS);
        if (res < SUCCESS_RETURN) {
            _iotx_linkkit_upstream_mutex_lock();
...
int IOT_Linkkit_Open(iotx_linkkit_dev_type_t dev_type,
...
int IOT_Linkkit_Report(int devid, iotx_linkkit_msg_type_t msg_type,
    ...
    case ITM_MSG_PROPERTY_DESIRED_GET:
        {
            ...
            res = iotx_dm_property_desired_get(devid, (char *)payload,
                                               payload_len);
        }
        break;
    case ITM_MSG_PROPERTY_DESIRED_DELETE:
        {
            ...
            res = iotx_dm_property_desired_delete(devid, (char *)payload,
                                                  payload_len);
        }
        break;
#endif
    case ITM_MSG_DEVICEINFO_UPDATE:
        {
            ...
            res =
                iotx_dm_deviceinfo_update(devid, (char *)payload, payload_len);
        }
        break;
    case ITM_MSG_DEVICEINFO_DELETE:
        {
            ...
            res =
                iotx_dm_deviceinfo_delete(devid, (char *)payload, payload_len);
        }
        break;
#endif
...
int IOT_Linkkit_Query(int devid, iotx_linkkit_msg_type_t msg_type,
                      unsigned char *payload, int payload_len)
{
    int res = 0;
    iotx_linkkit_ctx_t *ctx = _iotx_linkkit_get_ctx();
    ...
    case ITM_MSG_REQUEST_FOTA_IMAGE:
        {
            res =
                iotx_dm_fota_request_image((const char *)payload, payload_len);
        }
        break;
    default:
        {
...
int IOT_Linkkit_TriggerEvent(int devid, char *eventid, int eventid_len,
                             char *payload, int payload_len)
{
    ...
    if (devid < 0 || eventid == NULL || eventid_len <= 0 || payload == NULL ||
        payload_len <= 0) {
        dm_log_err("Invalid Parameter");
        return FAIL_RETURN;
    }
...
int IOT_Linkkit_AnswerService(int devid, char *serviceid, int serviceid_len,
                              char *payload, int payload_len,
                              void *p_service_ctx)
{
#if !defined(DEVICE_MODEL_RAWDATA_SOLO)
    ...
    char msgid[11] = { 0 };
    iotx_service_ctx_node_t *service_ctx =
        (iotx_service_ctx_node_t *)p_service_ctx;
    ...
    if (service_ctx->type == IOTX_SERVICE_REQ_TYPE_RRPC) {
        ...
        res = iotx_dm_send_rrpc_response(devid, msgid, strlen(msgid),
                                         IOTX_DM_ERR_CODE_SUCCESS, rrpc_id,
                                         strlen(rrpc_id), payload, payload_len);
    } else if (service_ctx->type ==
               IOTX_SERVICE_REQ_TYPE_GENERAL) { /* alcs/normal service response
                                                 */
        void *alcs_ctx = service_ctx->service_meta.general.alcs_ctx;
        res = iotx_dm_send_service_response(
            devid, msgid, strlen(msgid), IOTX_DM_ERR_CODE_SUCCESS, serviceid,
            serviceid_len, payload, payload_len, alcs_ctx);
    }
...
#ifdef DEVICE_MODEL_GATEWAY
int iot_linkkit_subdev_query_id(char product_key[IOTX_PRODUCT_KEY_LEN + 1],
