...
void dm_ipc_deinit(void)
{
    ...
    list_for_each_entry_safe(del_node, next_node, &ctx->msg_list.message_list,
                             linked_list, dm_ipc_msg_node_t)
    {
        /* Free Message */
...
int dm_ipc_msg_insert(void *data)
{
    ...
    _dm_ipc_lock();
    dm_log_debug("dm msg list size: %d, max size: %d", ctx->msg_list.size,
                 ctx->msg_list.max_size);
...
int dm_ipc_msg_next(void **data)
{
    ...
    node = list_first_entry(&ctx->msg_list.message_list, dm_ipc_msg_node_t,
                            linked_list);
