...
int dm_utils_copy_direct(_IN_ void *input, _IN_ int input_len,
                         _OU_ void **output, _IN_ int output_len)
{
...
int dm_utils_copy(_IN_ void *input, _IN_ int input_len, _OU_ void **output,
                  _IN_ int output_len)
{
    if (input == NULL || output == NULL || *output != NULL) {
        return DM_INVALID_PARAMETER;
...
int dm_utils_strarr_index(_IN_ char *input, _IN_ int input_len,
                          _OU_ int *partial_input_len,
                          _OU_ int *array_input_len, _OU_ int *array_index)
{
    int index = 0;
    int deep = 0;
    ...
    if (bracket_pre && bracket_suf &&
        ((bracket_suf - input + 1) == input_len)) {
...
int dm_utils_hex_to_str(_IN_ unsigned char *input, _IN_ int input_len,
                        _OU_ char **output)
{
...
int dm_utils_str_to_hex(_IN_ char *input, _IN_ int input_len,
                        _OU_ unsigned char **output, _OU_ int *output_len)
{
    int index = 0;
    char iter_char = 0;
...
int dm_utils_memtok(_IN_ char *input, _IN_ int input_len, _IN_ char delimiter,
                    _IN_ int index, _OU_ int *offset)
{
    int item_index = 0;
    int count = 0;
...
int dm_utils_replace_char(_IN_ char *input, _IN_ int input_len, _IN_ char src,
                          _IN_ char dest)
{
    int index = 0;
...
int dm_utils_service_name(_IN_ const char *prefix, _IN_ const char *name,
                          _IN_ char product_key[IOTX_PRODUCT_KEY_LEN + 1],
                          _IN_ char device_name[IOTX_DEVICE_NAME_LEN + 1],
                          _OU_ char **service_name)
{
    int prefix_len = (prefix == NULL) ? (0) : (strlen(prefix));
    int name_len = (name == NULL) ? (0) : (strlen(name));
    int service_name_len = 0;
    if ((prefix == NULL && name == NULL) || product_key == NULL ||
        device_name == NULL || service_name == NULL || *service_name != NULL) {
        return DM_INVALID_PARAMETER;
    }

    service_name_len =
        prefix_len + name_len + strlen(product_key) + strlen(device_name) + 1;
    *service_name = DM_malloc(service_name_len);
    ...
    if (prefix != NULL) {
        HAL_Snprintf(*service_name, service_name_len, prefix, product_key,
                     device_name);
    }
...
int dm_utils_uri_add_prefix(_IN_ const char *prefix, _IN_ char *uri,
                            _OU_ char **new_uri)
{
    int new_uri_len = 0;
...
int dm_utils_json_parse(_IN_ const char *payload, _IN_ int payload_len,
                        _IN_ int type, _OU_ lite_cjson_t *lite)
{
    int res = 0;
...
int dm_utils_json_object_item(_IN_ lite_cjson_t *lite, _IN_ const char *key,
                              _IN_ int key_len, _IN_ int type,
                              _OU_ lite_cjson_t *lite_item)
{
    int res = 0;

    if (lite == NULL || lite->type != cJSON_Object || key == NULL ||
        key_len <= 0 || type < 0 || lite_item == NULL) {
        return DM_INVALID_PARAMETER;
    }
    ...
    if (res != SUCCESS_RETURN) {
        /* dm_log_err(DM_UTILS_LOG_JSON_PARSE_FAILED, lite->value_length,
         * lite->value); */
        memset(lite_item, 0, sizeof(lite_cjson_t));
        return FAIL_RETURN;
    }
