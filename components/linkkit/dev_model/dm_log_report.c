...
const char THING_LOG_POST_PARAMS_HEAD[] = "\"%.*s %s %ld ";
const char THING_LOG_POST_PARAMS_BODY[] = "%s %ld ";
const char THING_LOG_POST_PARAMS_END[] = "%s %ld\",";
...
unsigned int push_log(const char *input_log, int input_log_size)
{
    ...
    len = current_log_pos - g_log_poll;
...
void add_log_header()
{
    const char *subprefix =
        "{\"template\": \"traceContext logContent\",\"contents\":[";
...
void parse_msg_id(_IN_ char *payload, _IN_ int payload_len,
...
{
    lite_cjson_t lite, lite_sample_count, lite_sample_interval,
        lite_sample_target;
    const char *c1 = "Count";
    ...
    dm_log_info("switch count is %d, interval is %d, target is %.*s\n",
                sample_count, sample_interval, sample_target_len,
                sample_target);
...
void send_permance_info(char *input, int input_len, char *comments,
                        int report_format)
{
#define LOCAL_POST_LEN (150)
    ...
    case 0:
        ...
        HAL_Snprintf(data, sizeof(data), format, input_len, input, comments,
                     (long)HAL_UptimeMs());
        break;
    case 1:
        format = THING_LOG_POST_PARAMS_BODY;
        HAL_Snprintf(data, sizeof(data), format, comments,
                     (long)HAL_UptimeMs());
        break;
    case 2:
...
void get_msgid(char *payload, int is_cloud)
{
    ...
    if (SUCCESS_RETURN !=
        set_target_msg(request.id.value, request.id.value_length)) {
        return;
    }
    ...
    if (1 == is_cloud) {
        send_permance_info(request.id.value, request.id.value_length, "1_cloud",
                           0);
    } else if (0 == is_cloud) {
        send_permance_info(request.id.value, request.id.value_length, "1_alcs",
                           0);
    }
}
#endif
