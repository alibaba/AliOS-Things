...
const char DM_MSG_SEND_MSG_TIMEOUT_FMT[] DM_READ_ONLY =
    "{\"id\":%d,\"code\":%d,\"devid\":%d}";
int dm_msg_send_msg_timeout_to_user(int msg_id, int devid,
                                    iotx_dm_event_types_t type)
{
    ...
    message_len =
        strlen(DM_MSG_SEND_MSG_TIMEOUT_FMT) + DM_UTILS_UINT32_STRLEN * 3 + 1;
    message = DM_malloc(message_len + 1);
    if (message == NULL) {
        return DM_MEMORY_NOT_ENOUGH;
    }
    memset(message, 0, message_len);
    HAL_Snprintf(message, message_len, DM_MSG_SEND_MSG_TIMEOUT_FMT, msg_id,
                 IOTX_DM_ERR_CODE_TIMEOUT, devid);
                                ...
                                _IN_ char *payload, _IN_ int payload_len)
{
    ...
    if (product_key == NULL || device_name == NULL ||
        ...
        (strlen(device_name) >= IOTX_DEVICE_NAME_LEN) || payload == NULL ||
        payload_len <= 0) {
    ...
    if (g_user_topic_callback) {
        ((int (*)(const int, const unsigned char *,
                  const int))g_user_topic_callback)(devid, request,
                                                    payload_len);
    }
    DM_free(request);
...
int dm_msg_uri_parse_pkdn(_IN_ char *uri, _IN_ int uri_len, _IN_ int start_deli,
                          _IN_ int end_deli,
                          _OU_ char product_key[IOTX_PRODUCT_KEY_LEN + 1],
                          _OU_ char device_name[IOTX_DEVICE_NAME_LEN + 1])
{
    int res = 0, start = 0, end = 0, slice = 0;

    if (uri == NULL || uri_len <= 0 || product_key == NULL ||
        device_name == NULL ||
    ...
    res = dm_utils_memtok(uri, uri_len, DM_URI_SERVICE_DELIMITER, start_deli,
                          &start);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }

    res = dm_utils_memtok(uri, uri_len, DM_URI_SERVICE_DELIMITER,
                          start_deli + 1, &slice);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
    ...
    res =
        dm_utils_memtok(uri, uri_len, DM_URI_SERVICE_DELIMITER, end_deli, &end);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }

    /* dm_log_debug("URI Product Key: %.*s, Device Name: %.*s", slice - start -
       1, uri + start + 1, end - slice - 1, uri + slice + 1); */
...
int dm_msg_request_parse(_IN_ char *payload, _IN_ int payload_len,
                         _OU_ dm_msg_request_payload_t *request)
{
    lite_cjson_t lite;
    ...
    if (dm_utils_json_parse(payload, payload_len, cJSON_Object, &lite) !=
            SUCCESS_RETURN ||
        dm_utils_json_object_item(&lite, DM_MSG_KEY_ID, strlen(DM_MSG_KEY_ID),
                                  cJSON_String,
                                  &request->id) != SUCCESS_RETURN ||
        dm_utils_json_object_item(&lite, DM_MSG_KEY_VERSION,
                                  strlen(DM_MSG_KEY_VERSION), cJSON_String,
                                  &request->version) != SUCCESS_RETURN ||
        dm_utils_json_object_item(&lite, DM_MSG_KEY_METHOD,
                                  strlen(DM_MSG_KEY_METHOD), cJSON_String,
                                  &request->method) != SUCCESS_RETURN ||
        dm_utils_json_object_item(&lite, DM_MSG_KEY_PARAMS,
                                  strlen(DM_MSG_KEY_PARAMS), cJSON_Invalid,
                                  &request->params) != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }

    dm_log_debug("Current Request Message ID: %.*s", request->id.value_length,
                 request->id.value);
    dm_log_debug("Current Request Message Version: %.*s",
                 request->version.value_length, request->version.value);
    dm_log_debug("Current Request Message Method: %.*s",
                 request->method.value_length, request->method.value);
    dm_log_debug("Current Request Message Params: %.*s",
                 request->params.value_length, request->params.value);

    return SUCCESS_RETURN;
}

int dm_msg_response_parse(_IN_ char *payload, _IN_ int payload_len,
                          _OU_ dm_msg_response_payload_t *response)
{
    ...
    if (dm_utils_json_parse(payload, payload_len, cJSON_Object, &lite) !=
        ...
        dm_utils_json_object_item(&lite, DM_MSG_KEY_ID, strlen(DM_MSG_KEY_ID),
                                  cJSON_String,
                                  &response->id) != SUCCESS_RETURN ||
        dm_utils_json_object_item(&lite, DM_MSG_KEY_CODE,
                                  strlen(DM_MSG_KEY_CODE), cJSON_Number,
                                  &response->code) != SUCCESS_RETURN ||
        dm_utils_json_object_item(&lite, DM_MSG_KEY_DATA,
                                  strlen(DM_MSG_KEY_DATA), cJSON_Invalid,
                                  &response->data) != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }

    dm_log_debug("Current Request Message ID: %.*s", response->id.value_length,
                 response->id.value);
    dm_log_debug("Current Request Message Code: %d", response->code.value_int);
    dm_log_debug("Current Request Message Data: %.*s",
                 response->data.value_length, response->data.value);

    memset(&lite_message, 0, sizeof(lite_cjson_t));
    if (dm_utils_json_object_item(&lite, DM_MSG_KEY_MESSAGE,
                                  strlen(DM_MSG_KEY_MESSAGE), cJSON_Invalid,
                                  &response->message) == SUCCESS_RETURN) {
        dm_log_debug("Current Request Message Desc: %.*s",
...
const char DM_MSG_REQUEST[] DM_READ_ONLY =
    "{\"id\":\"%d\",\"version\":\"%s\",\"params\":%.*s,\"method\":\"%s\"}";
int dm_msg_request(dm_msg_dest_type_t type, _IN_ dm_msg_request_t *request)
{
    ...
    /* Request URI */
    res =
        dm_utils_service_name(request->service_prefix, request->service_name,
                              request->product_key, request->device_name, &uri);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }

    payload_len = strlen(DM_MSG_REQUEST) + 10 + strlen(DM_MSG_VERSION) +
                  request->params_len + strlen(request->method) + 1;
    ...
    HAL_Snprintf(payload, payload_len, DM_MSG_REQUEST, request->msgid,
                 DM_MSG_VERSION, request->params_len, request->params,
                 request->method);
...
const char DM_MSG_RESPONSE_WITH_DATA[] DM_READ_ONLY =
    "{\"id\":\"%.*s\",\"code\":%d,\"data\":%.*s}";
int dm_msg_response(dm_msg_dest_type_t type,
                    _IN_ dm_msg_request_payload_t *request,
                    _IN_ dm_msg_response_t *response, _IN_ char *data,
                    _IN_ int data_len, _IN_ void *user_data)
{
    int res = 0, payload_len = 0;
    ...
    /* Response URI */
    res = dm_utils_service_name(response->service_prefix,
                                response->service_name, response->product_key,
                                response->device_name, &uri);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }

    /* Response Payload */
    payload_len = strlen(DM_MSG_RESPONSE_WITH_DATA) + request->id.value_length +
                  DM_UTILS_UINT32_STRLEN + data_len + 1;
    payload = DM_malloc(payload_len);
    if (payload == NULL) {
        DM_free(uri);
        return DM_MEMORY_NOT_ENOUGH;
    ...
    if (type & DM_MSG_DEST_LOCAL) {
        ...
        do {
            ...
            dm_server_send(uri, (unsigned char *)payload, strlen(payload),
                           user_data);
        } while (0);
    }
#endif
...
const char DM_MSG_THING_MODEL_DOWN_FMT[] DM_READ_ONLY =
    "{\"devid\":%d,\"payload\":\"%.*s\"}";
                                ...
                                _IN_ char *payload, _IN_ int payload_len)
{
    ...
    if (product_key == NULL || device_name == NULL ||
        (strlen(product_key) >= IOTX_PRODUCT_KEY_LEN + 1) ||
        (strlen(device_name) >= IOTX_DEVICE_NAME_LEN + 1) || payload == NULL ||
        payload_len <= 0) {
        return DM_INVALID_PARAMETER;
    }
    ...
    message_len = strlen(DM_MSG_THING_MODEL_DOWN_FMT) + DM_UTILS_UINT32_STRLEN +
                  strlen(hexstr) + 1;
    ...
    memset(message, 0, message_len);
    HAL_Snprintf(message, message_len, DM_MSG_THING_MODEL_DOWN_FMT, devid,
                 strlen(hexstr), hexstr);
    DM_free(hexstr);
...
const char DM_MSG_THING_MODEL_UP_RAW_REPLY_FMT[] DM_READ_ONLY =
    "{\"devid\":%d,\"payload\":\"%.*s\"}";
int dm_msg_thing_model_up_raw_reply(
    _IN_ char product_key[IOTX_PRODUCT_KEY_LEN + 1],
    _IN_ char device_name[IOTX_DEVICE_NAME_LEN + 1], char *payload,
    int payload_len)
{
    int res = 0, devid = 0, message_len = 0;
    char *hexstr = NULL, *message = NULL;
...
const char DM_MSG_PROPERTY_SET_FMT[] DM_READ_ONLY =
    "{\"devid\":%d,\"payload\":%.*s,\"msgid\":%.*s}";
#else
const char DM_MSG_PROPERTY_SET_FMT[] DM_READ_ONLY =
    "{\"devid\":%d,\"payload\":%.*s}";
#endif
int dm_msg_property_set(int devid, dm_msg_request_payload_t *request)
{
    ...
    message_len = strlen(DM_MSG_PROPERTY_SET_FMT) + DM_UTILS_UINT32_STRLEN +
    ...
    memset(message, 0, message_len);
#ifdef LOG_REPORT_TO_CLOUD
    HAL_Snprintf(message, message_len, DM_MSG_PROPERTY_SET_FMT, devid,
                 request->params.value_length, request->params.value,
...
int dm_msg_property_get(_IN_ int devid, _IN_ dm_msg_request_payload_t *request,
                        _IN_ void *ctx)
{
    int res = 0, message_len = 0;
    ...
    infra_hex2str((unsigned char *)&ctx_addr_num, sizeof(uintptr_t),
                  ctx_addr_str);
    /* dm_log_debug("ctx_addr_str: %s", ctx_addr_str); */

    message_len = strlen(DM_MSG_THING_PROPERTY_GET_FMT) +
                  request->id.value_length + DM_UTILS_UINT32_STRLEN +
    ...
    memset(message, 0, message_len);
    HAL_Snprintf(message, message_len, DM_MSG_THING_PROPERTY_GET_FMT,
                 request->id.value_length, request->id.value, devid,
                 request->params.value_length, request->params.value,
                 ctx_addr_str);
...
const char DM_MSG_SERVICE_REQUEST_FMT[] DM_READ_ONLY =
    ...
    "\"ctx\":\"%s\"}";
int dm_msg_thing_service_request(
    _IN_ char product_key[IOTX_PRODUCT_KEY_LEN + 1],
    _IN_ char device_name[IOTX_DEVICE_NAME_LEN + 1], char *identifier,
    int identifier_len, dm_msg_request_payload_t *request, _IN_ void *ctx)
{
    int res = 0, devid = 0, message_len = 0;
    char *message = NULL;
    uintptr_t ctx_addr_num = (uintptr_t)ctx;
    ...
    message_len = strlen(DM_MSG_SERVICE_REQUEST_FMT) +
                  request->id.value_length + DM_UTILS_UINT32_STRLEN +
                  identifier_len + request->params.value_length +
                  strlen(ctx_addr_str) + 1;
    message = DM_malloc(message_len);
    if (message == NULL) {
        DM_free(ctx_addr_str);
        return DM_MEMORY_NOT_ENOUGH;
    ...
    memset(message, 0, message_len);
    HAL_Snprintf(message, message_len, DM_MSG_SERVICE_REQUEST_FMT,
                 request->id.value_length, request->id.value, devid,
                 identifier_len, identifier, request->params.value_length,
                 request->params.value, ctx_addr_str);
                        ...
                        dm_msg_request_payload_t *request)
{
    ...
    /* Get Service ID */
    res = dm_utils_memtok(request->method.value, request->method.value_length,
                          '.', 2, &service_offset);
    if (res != SUCCESS_RETURN ||
        service_offset >= request->method.value_length - 1) {
        return FAIL_RETURN;
    }
    ...
    message_len = strlen(DM_MSG_EVENT_RRPC_REQUEST_FMT) +
                  request->id.value_length + DM_UTILS_UINT32_STRLEN +
                  serviceid_len + rrpcid_len + request->params.value_length + 1;
    message = DM_malloc(message_len);
    if (message == NULL) {
        return DM_MEMORY_NOT_ENOUGH;
    }
    memset(message, 0, message_len);
    HAL_Snprintf(message, message_len, DM_MSG_EVENT_RRPC_REQUEST_FMT,
                 request->id.value_length, request->id.value, devid,
...
int dm_msg_thing_event_property_post_reply(dm_msg_response_payload_t *response)
{
    ...
    if ((strlen("success") == response->message.value_length) &&
        (memcmp("success", response->message.value,
                response->message.value_length) == 0)) {
        ...
        if (response->message.type == cJSON_String) {
            ...
            str_payload[0] = '\"';
            memcpy(str_payload + 1, response->message.value,
                   response->message.value_length);
    ...
    message_len = strlen(DM_MSG_EVENT_PROPERTY_POST_REPLY_FMT) +
                  DM_UTILS_UINT32_STRLEN * 3 + payload_len + 1;
    message = DM_malloc(message_len);
    if (message == NULL) {
        DM_free(str_payload);
        return DM_MEMORY_NOT_ENOUGH;
    ...
    memset(message, 0, message_len);
    HAL_Snprintf(message, message_len, DM_MSG_EVENT_PROPERTY_POST_REPLY_FMT, id,
                 response->code.value_int, devid, payload_len, payload);
    DM_free(str_payload);

    res =
        _dm_msg_send_to_user(IOTX_DM_EVENT_EVENT_PROPERTY_POST_REPLY, message);
    if (res != SUCCESS_RETURN) {
        DM_free(message);
        return FAIL_RETURN;
    }
...
const char DM_MSG_EVENT_SPECIFIC_POST_REPLY_FMT[] DM_READ_ONLY =
    "{\"id\":%d,\"code\":%d,\"devid\":%d,\"eventid\":\"%.*s\",\"payload\":\"%.*"
    "s\"}";
int dm_msg_thing_event_post_reply(_IN_ char *identifier,
                                  ...
                                  _IN_ dm_msg_response_payload_t *response)
{
    ...
    message_len = strlen(DM_MSG_EVENT_SPECIFIC_POST_REPLY_FMT) +
                  DM_UTILS_UINT32_STRLEN * 3 + strlen(identifier) +
                  response->message.value_length + 1;
    message = DM_malloc(message_len);
    if (message == NULL) {
        return DM_MEMORY_NOT_ENOUGH;
    }
    memset(message, 0, message_len);
    HAL_Snprintf(message, message_len, DM_MSG_EVENT_SPECIFIC_POST_REPLY_FMT, id,
                 response->code.value_int, devid, identifier_len, identifier,
                 response->message.value_length, response->message.value);

    res =
        _dm_msg_send_to_user(IOTX_DM_EVENT_EVENT_SPECIFIC_POST_REPLY, message);
    if (res != SUCCESS_RETURN) {
        DM_free(message);
        return FAIL_RETURN;
    }
...
const char DM_MSG_EVENT_PROPERTY_DESIRED_GET_REPLY_FMT[] DM_READ_ONLY =
    "{\"id\":%d,\"code\":%d,\"data\":%.*s}";
int dm_msg_thing_property_desired_get_reply(dm_msg_response_payload_t *response)
{
    ...
    message_len = strlen(DM_MSG_EVENT_PROPERTY_DESIRED_GET_REPLY_FMT) +
                  DM_UTILS_UINT32_STRLEN * 2 + 1 + response->data.value_length;
    message = DM_malloc(message_len);
    if (message == NULL) {
        return DM_MEMORY_NOT_ENOUGH;
    }
    memset(message, 0, message_len);
    HAL_Snprintf(message, message_len,
                 DM_MSG_EVENT_PROPERTY_DESIRED_GET_REPLY_FMT, id,
                 response->code.value_int, response->data.value_length,
                 response->data.value);

    res =
        _dm_msg_send_to_user(IOTX_DM_EVENT_PROPERTY_DESIRED_GET_REPLY, message);
    if (res != SUCCESS_RETURN) {
        DM_free(message);
        return FAIL_RETURN;
    }
...
int dm_msg_thing_property_desired_delete_reply(
    dm_msg_response_payload_t *response)
{
    ...
    message_len = strlen(DM_MSG_EVENT_PROPERTY_DESIRED_DELETE_REPLY_FMT) +
                  DM_UTILS_UINT32_STRLEN * 3 + 1 + response->data.value_length;
    message = DM_malloc(message_len);
    if (message == NULL) {
        return DM_MEMORY_NOT_ENOUGH;
    }
    memset(message, 0, message_len);
    HAL_Snprintf(message, message_len,
                 DM_MSG_EVENT_PROPERTY_DESIRED_DELETE_REPLY_FMT, id,
                 response->code.value_int, response->data.value_length,
                 response->data.value, devid);

    res = _dm_msg_send_to_user(IOTX_DM_EVENT_PROPERTY_DESIRED_DELETE_REPLY,
                               message);
    if (res != SUCCESS_RETURN) {
        DM_free(message);
        return FAIL_RETURN;
...
const char DM_MSG_EVENT_DEVICEINFO_UPDATE_REPLY_FMT[] DM_READ_ONLY =
    "{\"id\":%d,\"code\":%d,\"devid\":%d}";
int dm_msg_thing_deviceinfo_update_reply(dm_msg_response_payload_t *response)
{
    ...
    message_len = strlen(DM_MSG_EVENT_DEVICEINFO_UPDATE_REPLY_FMT) +
                  DM_UTILS_UINT32_STRLEN * 3 + 1;
    message = DM_malloc(message_len);
    if (message == NULL) {
        return DM_MEMORY_NOT_ENOUGH;
    }
    memset(message, 0, message_len);
    HAL_Snprintf(message, message_len, DM_MSG_EVENT_DEVICEINFO_UPDATE_REPLY_FMT,
                 id, response->code.value_int, devid);
...
const char DM_MSG_EVENT_DEVICEINFO_DELETE_REPLY_FMT[] DM_READ_ONLY =
    "{\"id\":%d,\"code\":%d,\"devid\":%d}";
int dm_msg_thing_deviceinfo_delete_reply(dm_msg_response_payload_t *response)
{
    ...
    message_len = strlen(DM_MSG_EVENT_DEVICEINFO_DELETE_REPLY_FMT) +
                  DM_UTILS_UINT32_STRLEN * 3 + 1;
    message = DM_malloc(message_len);
    if (message == NULL) {
        return DM_MEMORY_NOT_ENOUGH;
    ...
    memset(message, 0, message_len);
    HAL_Snprintf(message, message_len, DM_MSG_EVENT_DEVICEINFO_DELETE_REPLY_FMT,
                 id, response->code.value_int, devid);
...
int dm_msg_thing_dsltemplate_get_reply(dm_msg_response_payload_t *response)
{
    ...
    dm_mgr_deprecated_set_tsl(devid, IOTX_DM_TSL_TYPE_ALINK,
                              (const char *)response->data.value,
...
int dm_msg_ntp_response(char *payload, int payload_len)
{
    ...
    if (dm_utils_json_parse(payload, payload_len, cJSON_Object, &lite) !=
            SUCCESS_RETURN ||
        dm_utils_json_object_item(&lite, serverSendTime, strlen(serverSendTime),
                                  cJSON_String, &lite_item_server_send_time) !=
            SUCCESS_RETURN) {
        return FAIL_RETURN;
    }

    /* dm_log_debug("NTP Time In String: %.*s",
     * lite_item_server_send_time.value_length,
     * lite_item_server_send_time.value); */

    /* Send Message To User */
    message_len = strlen(DM_MSG_THING_NTP_RESPONSE_FMT) +
                  DM_UTILS_UINT32_STRLEN +
                  lite_item_server_send_time.value_length + 1;
    message = DM_malloc(message_len);
    if (message == NULL) {
        return DM_MEMORY_NOT_ENOUGH;
    }
    memset(message, 0, message_len);
    HAL_Snprintf(message, message_len, DM_MSG_THING_NTP_RESPONSE_FMT,
                 lite_item_server_send_time.value_length,
                 lite_item_server_send_time.value);
...
const char DM_MSG_TOPO_ADD_NOTIFY_USER_PAYLOAD[] DM_READ_ONLY =
    "{\"result\":%d,\"devid\":%d,\"product_key\":\"%s\",\"device_name\":\"%"
    "s\"}";
int dm_msg_topo_add_notify(_IN_ char *payload, _IN_ int payload_len)
{
    ...
    for (index = 0; index < lite.size; index++) {
        ...
        res = lite_cjson_object_item(&lite_item, DM_MSG_KEY_PRODUCT_KEY,
                                     strlen(DM_MSG_KEY_PRODUCT_KEY),
                                     &lite_item_pk);
        if (res != SUCCESS_RETURN) {
            ret = FAIL_RETURN;
            continue;
        ...
        res = lite_cjson_object_item(&lite_item, DM_MSG_KEY_DEVICE_NAME,
                                     strlen(DM_MSG_KEY_DEVICE_NAME),
                                     &lite_item_dn);
        if (res != SUCCESS_RETURN) {
            ret = FAIL_RETURN;
            continue;
        ...
        res = dm_mgr_device_create(IOTX_DM_DEVICE_SUBDEV, product_key, "",
                                   device_name, NULL, &devid);
        if (res != SUCCESS_RETURN) {
            ret = FAIL_RETURN;
        }
        ...
        memset(message, 0, message_len);
        HAL_Snprintf(message, message_len, DM_MSG_TOPO_ADD_NOTIFY_USER_PAYLOAD,
                     res, devid, product_key, device_name);
                         ...
                         _IN_ char device_name[IOTX_DEVICE_NAME_LEN + 1])
{
    ...
    message_len =
        strlen(DM_MSG_EVENT_THING_DISABLE_FMT) + DM_UTILS_UINT32_STRLEN + 1;
    message = DM_malloc(message_len);
    if (message == NULL) {
        return DM_MEMORY_NOT_ENOUGH;
    }
                        ...
                        _IN_ char device_name[IOTX_DEVICE_NAME_LEN + 1])
{
    ...
    message_len =
        strlen(DM_MSG_EVENT_THING_ENABLE_FMT) + DM_UTILS_UINT32_STRLEN + 1;
    message = DM_malloc(message_len);
    if (message == NULL) {
        return DM_MEMORY_NOT_ENOUGH;
    }
                        ...
                        _IN_ char device_name[IOTX_DEVICE_NAME_LEN + 1])
{
    ...
    message_len = strlen(DM_MSG_EVENT_THING_DELETE_FMT) + strlen(product_key) +
    ...
    memset(message, 0, message_len);
    HAL_Snprintf(message, message_len, DM_MSG_EVENT_THING_DELETE_FMT, res,
...
const char DM_MSG_EVENT_SUBDEV_REGISTER_REPLY_FMT[] DM_READ_ONLY =
    "{\"id\":%d,\"code\":%d,\"devid\":%d}";
int dm_msg_thing_sub_register_reply(dm_msg_response_payload_t *response)
{
    ...
    if (response->code.value_int != IOTX_DM_ERR_CODE_SUCCESS) {
        ...
        message_len = strlen(DM_MSG_EVENT_SUBDEV_REGISTER_REPLY_FMT) +
                      DM_UTILS_UINT32_STRLEN * 2 + 1;
        message = DM_malloc(message_len);
        if (message == NULL) {
        ...
        memset(message, 0, message_len);
        HAL_Snprintf(message, message_len,
                     DM_MSG_EVENT_SUBDEV_REGISTER_REPLY_FMT, atoi(temp_id),
                     response->code.value_int, 0);

        res =
            _dm_msg_send_to_user(IOTX_DM_EVENT_SUBDEV_REGISTER_REPLY, message);
        if (res != SUCCESS_RETURN) {
            DM_free(message);
        }
    }

    res = lite_cjson_parse(response->data.value, response->data.value_length,
                           &lite);
    if (res != SUCCESS_RETURN || !lite_cjson_is_array(&lite)) {
        return DM_JSON_PARSE_FAILED;
    }
    ...
    for (index = 0; index < lite.size; index++) {
        ...
        /* dm_log_debug("Current Product Key: %.*s", lite_item_pk.value_length,
         * lite_item_pk.value); */
        ...
        /* dm_log_debug("Current Device Name: %.*s", lite_item_dn.value_length,
         * lite_item_dn.value); */

        /* Device Secret */
        res = lite_cjson_object_item(&lite_item, DM_MSG_KEY_DEVICE_SECRET,
                                     strlen(DM_MSG_KEY_DEVICE_SECRET),
                                     &lite_item_ds);
        if (res != SUCCESS_RETURN || !lite_cjson_is_string(&lite_item_ds)) {
            continue;
        }
        /* dm_log_debug("Current Device Secret: %.*s",
         * lite_item_ds.value_length, lite_item_ds.value); */
        ...
        memcpy(temp_id, response->id.value,
               (response->id.value_length > DM_UTILS_UINT32_STRLEN)
                   ? DM_UTILS_UINT32_STRLEN
                   : response->id.value_length);
        message_len = strlen(DM_MSG_EVENT_SUBDEV_REGISTER_REPLY_FMT) +
                      DM_UTILS_UINT32_STRLEN * 2 + 1;
        message = DM_malloc(message_len);
        if (message == NULL) {
...
const char DM_MSG_EVENT_PROXY_PRODUCT_REGISTER_REPLY_FMT[] DM_READ_ONLY =
    "{\"id\":%d,\"code\":%d,\"devid\":%d}";

static void dm_msg_thing_proxy_product_register_reply_successes(
    dm_msg_response_payload_t *response, lite_cjson_t *lite)
{
    int res = 0, index = 0, message_len = 0, devid = 0;
    char *message = NULL;
    ...
    for (index; index < lite->size; index++) {
        ...
        /* Send Message To User */
        memcpy(temp_id, response->id.value, response->id.value_length);
        message_len = strlen(DM_MSG_EVENT_PROXY_PRODUCT_REGISTER_REPLY_FMT) +
                      DM_UTILS_UINT32_STRLEN * 2 + 1;
        message = DM_malloc(message_len);
        if (message == NULL) {
            dm_log_warning("Memory Not Enough");
        ...
        memset(message, 0, message_len);
        HAL_Snprintf(message, message_len,
                     DM_MSG_EVENT_PROXY_PRODUCT_REGISTER_REPLY_FMT,
                     atoi(temp_id), response->code.value_int, devid);

        res = _dm_msg_send_to_user(IOTX_DM_EVENT_PROXY_PRODUCT_REGISTER_REPLY,
                                   message);
        if (res != SUCCESS_RETURN) {
            DM_free(message);
        }
...
static void dm_msg_thing_proxy_product_register_reply_failures(
    dm_msg_response_payload_t *response, lite_cjson_t *lite)
{
    int res = 0, index = 0, message_len = 0, devid = 0;
    char *message = NULL;
    lite_cjson_t lite_item, lite_item_pk, lite_item_dn, lite_item_ed,
        lite_item_code;
    char product_key[IOTX_PRODUCT_KEY_LEN + 1] = { 0 };
    char device_name[IOTX_DEVICE_NAME_LEN + 1] = { 0 };
    char temp_id[DM_UTILS_UINT32_STRLEN] = { 0 };
    ...
    for (index; index < lite->size; index++) {
        ...
        /* Error Detail */
        res = lite_cjson_object_item(&lite_item, DM_MSG_KEY_ERRORDETAIL,
                                     strlen(DM_MSG_KEY_ERRORDETAIL),
                                     &lite_item_ed);
        ...
        /* Error Detail Code */
        res = lite_cjson_object_item(&lite_item_ed, DM_MSG_KEY_CODE,
                                     strlen(DM_MSG_KEY_CODE), &lite_item_code);
        ...
        HAL_Snprintf(message, message_len,
                     DM_MSG_EVENT_PROXY_PRODUCT_REGISTER_REPLY_FMT,
                     atoi(temp_id), lite_item_code.value_int, devid);
...
int dm_msg_thing_proxy_product_register_reply(
    dm_msg_response_payload_t *response)
{
    int res = 0, index = 0, message_len = 0, devid = 0;
    lite_cjson_t lite, lite_failures, lite_successes, lite_item, lite_item_pk,
        lite_item_dn, lite_item_ds;
    char *message = NULL;
    ...
    /* handle successes */
    res = lite_cjson_object_item(&lite, DM_MSG_KEY_SUCCESSES,
                                 strlen(DM_MSG_KEY_SUCCESSES), &lite_successes);
    if (res != SUCCESS_RETURN || !lite_cjson_is_array(&lite_successes)) {
        return DM_JSON_PARSE_FAILED;
    }

    dm_msg_thing_proxy_product_register_reply_successes(response,
                                                        &lite_successes);

    /* handle failures */
    res = lite_cjson_object_item(&lite, DM_MSG_KEY_FAILURES,
                                 strlen(DM_MSG_KEY_FAILURES), &lite_failures);
    if (res != SUCCESS_RETURN || !lite_cjson_is_array(&lite_failures)) {
        return DM_JSON_PARSE_FAILED;
    }

    dm_msg_thing_proxy_product_register_reply_failures(response,
                                                       &lite_failures);

    return 0;
}

const char DM_MSG_EVENT_SUBDEV_UNREGISTER_REPLY_FMT[] DM_READ_ONLY =
    "{\"id\":%d,\"code\":%d,\"devid\":%d}";
int dm_msg_thing_sub_unregister_reply(dm_msg_response_payload_t *response)
{
    ...
    message_len = strlen(DM_MSG_EVENT_SUBDEV_UNREGISTER_REPLY_FMT) +
                  DM_UTILS_UINT32_STRLEN * 3 + 1;
    message = DM_malloc(message_len);
    if (message == NULL) {
        return DM_MEMORY_NOT_ENOUGH;
    ...
    memset(message, 0, message_len);
    HAL_Snprintf(message, message_len, DM_MSG_EVENT_SUBDEV_UNREGISTER_REPLY_FMT,
                 id, response->code.value_int, devid);
...
const char DM_MSG_EVENT_THING_TOPO_ADD_REPLY_FMT[] DM_READ_ONLY =
    "{\"id\":%d,\"code\":%d,\"devid\":%d}";
int dm_msg_thing_topo_add_reply(dm_msg_response_payload_t *response)
{
    ...
    message_len = strlen(DM_MSG_EVENT_THING_TOPO_ADD_REPLY_FMT) +
                  DM_UTILS_UINT32_STRLEN * 3 + 1;
    message = DM_malloc(message_len);
    if (message == NULL) {
        return DM_MEMORY_NOT_ENOUGH;
    ...
    memset(message, 0, message_len);
    HAL_Snprintf(message, message_len, DM_MSG_EVENT_THING_TOPO_ADD_REPLY_FMT,
                 id, response->code.value_int, devid);
...
const char DM_MSG_EVENT_THING_TOPO_DELETE_REPLY_FMT[] DM_READ_ONLY =
    "{\"id\":%d,\"code\":%d,\"devid\":%d}";
int dm_msg_thing_topo_delete_reply(dm_msg_response_payload_t *response)
{
    ...
    message_len = strlen(DM_MSG_EVENT_THING_TOPO_DELETE_REPLY_FMT) +
                  DM_UTILS_UINT32_STRLEN * 3 + 1;
    message = DM_malloc(message_len);
    if (message == NULL) {
        return DM_MEMORY_NOT_ENOUGH;
    ...
    memset(message, 0, message_len);
    HAL_Snprintf(message, message_len, DM_MSG_EVENT_THING_TOPO_DELETE_REPLY_FMT,
                 id, response->code.value_int, devid);
...
const char DM_MSG_TOPO_GET_REPLY_FMT[] DM_READ_ONLY =
    "{\"id\":%d,\"code\":%d,\"devid\":%d,\"topo\":%.*s}";
int dm_msg_topo_get_reply(dm_msg_response_payload_t *response)
{
    ...
    message_len = strlen(DM_MSG_TOPO_GET_REPLY_FMT) +
                  DM_UTILS_UINT32_STRLEN * 3 + response->data.value_length + 1;
    message = DM_malloc(message_len);
    if (message == NULL) {
        return DM_MEMORY_NOT_ENOUGH;
    }
    memset(message, 0, message_len);
    HAL_Snprintf(message, message_len, DM_MSG_TOPO_GET_REPLY_FMT, id,
                 response->code.value_int, IOTX_DM_LOCAL_NODE_DEVID,
                 response->data.value_length, response->data.value);
...
const char DM_MSG_EVENT_COMBINE_LOGIN_REPLY_FMT[] DM_READ_ONLY =
    "{\"id\":%d,\"code\":%d,\"devid\":%d}";
int dm_msg_combine_login_reply(dm_msg_response_payload_t *response)
{
    ...
    /* Parse Product Key */
    res = lite_cjson_object_item(&lite, DM_MSG_KEY_PRODUCT_KEY,
                                 strlen(DM_MSG_KEY_PRODUCT_KEY), &lite_item_pk);
    if (res != SUCCESS_RETURN || !lite_cjson_is_string(&lite_item_pk) ||
    ...
    /* Parse Device Name */
    res = lite_cjson_object_item(&lite, DM_MSG_KEY_DEVICE_NAME,
                                 strlen(DM_MSG_KEY_DEVICE_NAME), &lite_item_dn);
    if (res != SUCCESS_RETURN || !lite_cjson_is_string(&lite_item_dn) ||
        lite_item_dn.value_length >= IOTX_DEVICE_NAME_LEN + 1) {
        return DM_JSON_PARSE_FAILED;
    }
    ...
    message_len = strlen(DM_MSG_EVENT_COMBINE_LOGIN_REPLY_FMT) +
                  DM_UTILS_UINT32_STRLEN * 3 + 1;
    message = DM_malloc(message_len);
    if (message == NULL) {
        return DM_MEMORY_NOT_ENOUGH;
    ...
    memset(message, 0, message_len);
    HAL_Snprintf(message, message_len, DM_MSG_EVENT_COMBINE_LOGIN_REPLY_FMT,
                 atoi(temp_id), response->code.value_int, devid);
...
const char DM_MSG_EVENT_COMBINE_LOGOUT_REPLY_FMT[] DM_READ_ONLY =
    "{\"id\":%d,\"code\":%d,\"devid\":%d}";
int dm_msg_combine_logout_reply(dm_msg_response_payload_t *response)
{
    ...
    message_len = strlen(DM_MSG_EVENT_COMBINE_LOGOUT_REPLY_FMT) +
                  DM_UTILS_UINT32_STRLEN * 3 + 1;
    message = DM_malloc(message_len);
    if (message == NULL) {
        return DM_MEMORY_NOT_ENOUGH;
    ...
    memset(message, 0, message_len);
    HAL_Snprintf(message, message_len, DM_MSG_EVENT_COMBINE_LOGOUT_REPLY_FMT,
                 atoi(temp_id), response->code.value_int, devid);
...
const char DM_MSG_DEV_CORE_SERVICE_DEV[] DM_READ_ONLY =
    "{\"devices\":{\"addr\":\"%s\",\"port\":%d,\"pal\":\"linkkit-ica\","
    "\"profile\":%s}}";
int dm_msg_dev_core_service_dev(char **payload, int *payload_len)
{
    ...
    for (index = 0; index < dm_mgr_device_number(); index++) {
        ...
        res = dm_mgr_search_device_by_devid(search_devid, product_key,
    ...
    *payload_len = strlen(DM_MSG_DEV_CORE_SERVICE_DEV) + strlen(ip_addr) +
                   DM_UTILS_UINT16_STRLEN + strlen(device_array) + 1;
    *payload = DM_malloc(*payload_len);
    if (*payload == NULL) {
    ...
    memset(*payload, 0, *payload_len);
    HAL_Snprintf(*payload, *payload_len, DM_MSG_DEV_CORE_SERVICE_DEV, ip_addr,
                 port, device_array);
...
#ifdef DEVICE_MODEL_GATEWAY
const char DM_MSG_THING_SUB_REGISTER_METHOD[] DM_READ_ONLY =
    "thing.sub.register";
const char DM_MSG_THING_SUB_REGISTER_PARAMS[] DM_READ_ONLY =
                              ...
                              _OU_ dm_msg_request_t *request)
{
    ...
    params_len = strlen(DM_MSG_THING_SUB_REGISTER_PARAMS) +
                 strlen(product_key) + strlen(device_name) + 1;
    ...
    HAL_Snprintf(params, params_len, DM_MSG_THING_SUB_REGISTER_PARAMS,
...
const char DM_MSG_THING_PROXY_PRODUCT_REGISTER_SIGN_SOURCE[] DM_READ_ONLY =
    "deviceName%sproductKey%srandom%s";
const char DM_MSG_THING_PROXY_PRODUCT_REGISTER_METHOD[] DM_READ_ONLY =
    "thing.proxy.provisioning.product_register";
const char DM_MSG_THING_PROXY_PRODUCT_REGISTER_PARAMS[] DM_READ_ONLY =
    "{\"proxieds\":[{\"productKey\":\"%s\",\"deviceName\":\"%s\","
    "\"signMethod\":\"%s\",\"sign\":\"%s\",\"random\":\"%s\"}]}";
int dm_msg_thing_proxy_product_register(
    _IN_ char product_key[IOTX_PRODUCT_KEY_LEN + 1],
    ...
    _OU_ dm_msg_request_t *request)
{
    ...
    if (request == NULL || product_key == NULL || product_secret == NULL ||
        device_name == NULL ||
        (strlen(product_key) >= IOTX_PRODUCT_KEY_LEN + 1) ||
    ...
    /* Random */
    HAL_Snprintf(random, DM_UTILS_UINT64_STRLEN, "%llu",
                 (unsigned long long)HAL_UptimeMs());

    /* Sign */
    sign_source_len = strlen(DM_MSG_THING_PROXY_PRODUCT_REGISTER_SIGN_SOURCE) +
                      strlen(device_name) + strlen(product_key) +
                      strlen(random) + 1;
    ...
    HAL_Snprintf(sign_source, sign_source_len,
                 DM_MSG_THING_PROXY_PRODUCT_REGISTER_SIGN_SOURCE, device_name,
                 product_key, random);
    utils_hmac_sha256((uint8_t *)sign_source, strlen(sign_source),
                      (uint8_t *)product_secret, strlen(product_secret), sign);
    ...
    params_len = strlen(DM_MSG_THING_PROXY_PRODUCT_REGISTER_PARAMS) +
                 strlen(product_key) + strlen(device_name) +
    ...
    memset(params, 0, params_len);
    HAL_Snprintf(params, params_len, DM_MSG_THING_PROXY_PRODUCT_REGISTER_PARAMS,
                 product_key, device_name, sign_method, sign_str, random);
...
const char DM_MSG_THING_SUB_UNREGISTER_METHOD[] DM_READ_ONLY =
    "thing.sub.unregister";
const char DM_MSG_THING_SUB_UNREGISTER_PARAMS[] DM_READ_ONLY =
    "[{\"productKey\":\"%s\",\"deviceName\":\"%s\"}]";
                                ...
                                _OU_ dm_msg_request_t *request)
{
    ...
    params_len = strlen(DM_MSG_THING_SUB_UNREGISTER_PARAMS) +
                 strlen(product_key) + strlen(device_name) + 1;
    params = DM_malloc(params_len);
    if (params == NULL) {
        return DM_MEMORY_NOT_ENOUGH;
    ...
    memset(params, 0, params_len);
    HAL_Snprintf(params, params_len, DM_MSG_THING_SUB_UNREGISTER_PARAMS,
                 product_key, device_name);
...
const char DM_MSG_THING_TOPO_ADD_SIGN_SOURCE[] DM_READ_ONLY =
...
const char DM_MSG_THING_TOPO_ADD_PARAMS[] DM_READ_ONLY =
    "[{\"productKey\":\"%s\",\"deviceName\":\"%s\",\"signmethod\":\"%s\","
    "\"sign\":\"%s\",\"timestamp\":\"%s\",\"clientId\":\"%s\"}]";
int dm_msg_thing_topo_add(_IN_ char product_key[IOTX_PRODUCT_KEY_LEN + 1],
                          _IN_ char device_name[IOTX_DEVICE_NAME_LEN + 1],
                          _IN_ char device_secret[IOTX_DEVICE_SECRET_LEN + 1],
                          _OU_ dm_msg_request_t *request)
{
    char *params = NULL;
    int params_len = 0;
    char timestamp[DM_UTILS_UINT64_STRLEN] = { 0 };
    char client_id[IOTX_PRODUCT_KEY_LEN + 1 + IOTX_DEVICE_NAME_LEN + 1 + 1] = {
        0
    };
    char *sign_source = NULL;
    int sign_source_len = 0;
    ...
    /* TimeStamp */
    HAL_Snprintf(timestamp, DM_UTILS_UINT64_STRLEN, "%llu",
                 (unsigned long long)HAL_UptimeMs());
    /* dm_log_debug("Time Stamp: %s", timestamp); */

    /* Client ID */
    HAL_Snprintf(client_id,
                 IOTX_PRODUCT_KEY_LEN + 1 + IOTX_DEVICE_NAME_LEN + 1 + 1,
                 "%s.%s", product_key, device_name);

    /* Sign */
    sign_source_len = strlen(DM_MSG_THING_TOPO_ADD_SIGN_SOURCE) +
                      strlen(client_id) + strlen(device_name) +
                      strlen(product_key) + strlen(timestamp) + 1;
    sign_source = DM_malloc(sign_source_len);
    if (sign_source == NULL) {
        return DM_MEMORY_NOT_ENOUGH;
    }
    memset(sign_source, 0, sign_source_len);
    HAL_Snprintf(sign_source, sign_source_len,
                 DM_MSG_THING_TOPO_ADD_SIGN_SOURCE, client_id, device_name,
                 product_key, timestamp);

    /* dm_log_debug("Sign Srouce: %s", sign_source); */
    utils_hmac_sha256((uint8_t *)sign_source, strlen(sign_source),
                      (uint8_t *)device_secret, strlen(device_secret), sign);
    infra_hex2str(sign, 32, sign_str);
    ...
    params_len = strlen(DM_MSG_THING_TOPO_ADD_PARAMS) + strlen(product_key) +
                 strlen(device_name) + strlen(sign_method) + strlen(sign_str) +
                 strlen(timestamp) + strlen(client_id) + 1;
    params = DM_malloc(params_len);
    ...
    memset(params, 0, params_len);
    HAL_Snprintf(params, params_len, DM_MSG_THING_TOPO_ADD_PARAMS, product_key,
                 device_name, sign_method, sign_str, timestamp, client_id);
...
const char DM_MSG_THING_TOPO_DELETE_PARAMS[] DM_READ_ONLY =
    "[{\"productKey\":\"%s\",\"deviceName\":\"%s\"}]";
                             ...
                             _OU_ dm_msg_request_t *request)
{
    ...
    params_len = strlen(DM_MSG_THING_TOPO_DELETE_PARAMS) + strlen(product_key) +
                 strlen(device_name) + 1;
    params = DM_malloc(params_len);
    if (params == NULL) {
        return DM_MEMORY_NOT_ENOUGH;
    ...
    memset(params, 0, params_len);
    HAL_Snprintf(params, params_len, DM_MSG_THING_TOPO_DELETE_PARAMS,
                 product_key, device_name);
...
int dm_msg_thing_topo_get(_OU_ dm_msg_request_t *request)
{
    ...
    memset(params, 0, params_len);
    memcpy(params, DM_MSG_THING_TOPO_GET_PARAMS,
           strlen(DM_MSG_THING_TOPO_GET_PARAMS));
...
const char DM_MSG_THING_LIST_FOUND_PARAMS[] DM_READ_ONLY =
    "[{\"productKey\":\"%s\",\"deviceName\":\"%s\"}]";
                            ...
                            _OU_ dm_msg_request_t *request)
{
    ...
    if (product_key == NULL || device_name == NULL ||
        (strlen(product_key) >= IOTX_PRODUCT_KEY_LEN + 1) ||
        (strlen(device_name) >= IOTX_DEVICE_NAME_LEN + 1) || request == NULL) {
        return DM_INVALID_PARAMETER;
    }

    /* Params */
    request->method = (char *)DM_MSG_THING_LIST_FOUND_METHOD;
    params_len = strlen(DM_MSG_THING_LIST_FOUND_PARAMS) + strlen(product_key) +
                 strlen(device_name) + 1;
    params = DM_malloc(params_len);
    if (params == NULL) {
        return DM_MEMORY_NOT_ENOUGH;
    ...
    memset(params, 0, params_len);
    HAL_Snprintf(params, params_len, DM_MSG_THING_LIST_FOUND_PARAMS,
                 product_key, device_name);
...
const char DM_MSG_COMBINE_LOGIN_SIGN_SOURCE[] DM_READ_ONLY =
    "clientId%sdeviceName%sproductKey%stimestamp%s";
                         ...
                         _OU_ dm_msg_request_t *request)
{
    ...
    int params_len = 0;
    char timestamp[DM_UTILS_UINT64_STRLEN] = { 0 };
    char client_id[IOTX_PRODUCT_KEY_LEN + 1 + IOTX_DEVICE_NAME_LEN + 25] = {
        0
    };
    char *sign_source = NULL;
    int sign_source_len = 0;
    ...
    /* Client ID */
    HAL_Snprintf(
        client_id, IOTX_PRODUCT_KEY_LEN + 1 + IOTX_DEVICE_NAME_LEN + 25,
        "%s.%s|_ss=1,_v=sdk-c-" IOTX_SDK_VERSION "|", product_key, device_name);

    /* Sign */
    sign_source_len = strlen(DM_MSG_COMBINE_LOGIN_SIGN_SOURCE) +
                      strlen(client_id) + strlen(device_name) +
                      strlen(product_key) + strlen(timestamp) + 1;
    sign_source = DM_malloc(sign_source_len);
    if (sign_source == NULL) {
    ...
    memset(sign_source, 0, sign_source_len);
    HAL_Snprintf(sign_source, sign_source_len, DM_MSG_COMBINE_LOGIN_SIGN_SOURCE,
    ...
    params_len = strlen(DM_MSG_COMBINE_LOGIN_PARAMS) + strlen(product_key) +
                 strlen(device_name) + strlen(sign_method) + strlen(sign_str) +
                 strlen(timestamp) + strlen(client_id) + 1;
    params = DM_malloc(params_len);
    ...
    memset(params, 0, params_len);
    HAL_Snprintf(params, params_len, DM_MSG_COMBINE_LOGIN_PARAMS, product_key,
                 device_name, client_id, timestamp, sign_method, sign_str,
                 "true");
...
const char DM_MSG_COMBINE_LOGOUT_PARAMS[] DM_READ_ONLY =
    "{\"productKey\":\"%s\",\"deviceName\":\"%s\"}";
                          ...
                          _OU_ dm_msg_request_t *request)
{
    ...
    params_len = strlen(DM_MSG_COMBINE_LOGOUT_PARAMS) + strlen(product_key) +
                 strlen(device_name) + 1;
    params = DM_malloc(params_len);
    ...
    memset(params, 0, params_len);
    HAL_Snprintf(params, params_len, DM_MSG_COMBINE_LOGOUT_PARAMS, product_key,
                 device_name);
...
typedef int (*dm_get_shadow_data)(_IN_ int devid, _IN_ char *key,
                                  _IN_ int key_len, _OU_ void **data);
typedef int (*dm_set_shadow_data)(_IN_ int devid, _IN_ char *key,
                                  _IN_ int key_len, _IN_ void *value,
                                  _IN_ int value_len);

static int _dm_msg_set_number(dm_msg_set_type_t type, int devid, char *key,
                              lite_cjson_t *root);
static int _dm_msg_set_string(dm_msg_set_type_t type, int devid, char *key,
                              lite_cjson_t *root);
static int _dm_msg_set_object(dm_msg_set_type_t type, int devid, char *key,
                              lite_cjson_t *root);
static int _dm_msg_set_array(dm_msg_set_type_t type, int devid, char *key,
                             lite_cjson_t *root);
                              ...
                              lite_cjson_t *root)
{
    ...
    dm_get_shadow_data get_shadow_data_func =
        (type == DM_MSG_PROPERTY_SET)
            ? (dm_mgr_deprecated_get_property_data)
            : (dm_mgr_deprecated_get_service_input_data);
    dm_set_shadow_data set_shadow_data_func =
        (type == DM_MSG_PROPERTY_SET)
            ? (dm_mgr_deprecated_set_property_value)
    ...
    case DM_SHW_DATA_TYPE_BOOL:
        {
            res = set_shadow_data_func(devid, key, strlen(key),
                                       &root->value_int, 0);
        }
        break;
    case DM_SHW_DATA_TYPE_FLOAT:
        {
            ...
            res =
                set_shadow_data_func(devid, key, strlen(key), &value_float, 0);
        }
        break;
                              ...
                              lite_cjson_t *root)
{
    ...
    case DM_SHW_DATA_TYPE_DATE:
        {
            res = set_shadow_data_func(devid, key, strlen(key), root->value,
                                       root->value_length);
        }
        break;
    default:
                              ...
                              lite_cjson_t *root)
{
    ...
    for (index = 0; index < root->size; index++) {
        res = lite_cjson_object_item_by_index(root, index, &lite_item_key,
                                              &lite_item_value);
        if (res != SUCCESS_RETURN) {
            continue;
        }
        ...
        new_key_len = ((key == NULL) ? (0) : (strlen(key) + 1)) +
                      lite_item_key.value_length + 1;
        ...
        memcpy(new_key + strlen(new_key), lite_item_key.value,
                             ...
                             lite_cjson_t *root)
{
    ...
    for (index = 0; index < root->size; index++) {
        ...
        /* dm_log_debug("Current Value: %.*s", lite_item_value.value_length,
         * lite_item_value.value); */
        ...
        /*                         Original Key              '['         Index
         * ']'*/
        new_key_len = ((key == NULL) ? (0) : (strlen(key) + 1)) + 1 +
                      strlen(ascii_index) + 1 + 1;
        new_key = DM_malloc(new_key_len);
        if (new_key == NULL) {
            DM_free(ascii_index);
            return DM_MEMORY_NOT_ENOUGH;
...
int dm_msg_property_set(int devid, dm_msg_request_payload_t *request)
{
    ...
    /* Parse Root */
    memset(&lite, 0, sizeof(lite_cjson_t));
    res = lite_cjson_parse(request->params.value, request->params.value_length,
                           &lite);
    if (res != SUCCESS_RETURN ||
        (!lite_cjson_is_object(&lite) && !lite_cjson_is_array(&lite))) {
        return DM_JSON_PARSE_FAILED;
    }
    ...
    for (index = 0; index < lite.size; index++) {
        ...
        res = lite_cjson_object_item_by_index(&lite, index, &lite_item_key,
                                              &lite_item_value);
        if (res != SUCCESS_RETURN) {
            continue;
        }
                        ...
                        _IN_ char **payload, _IN_ int *payload_len)
{
    ...
    if (devid < 0 || request == NULL || payload == NULL || *payload != NULL ||
        payload_len == NULL) {
        return DM_INVALID_PARAMETER;
    }
    ...
    for (index = 0; index < lite.size; index++) {
        ...
        res = dm_mgr_deprecated_assemble_property(
            devid, lite_item.value, lite_item.value_length, lite_cjson_item);
        if (res != SUCCESS_RETURN) {
            lite_cjson_delete(lite_cjson_item);
            return FAIL_RETURN;
        }
    ...
    int identifier_len, dm_msg_request_payload_t *request, _IN_ void *ctx)
{
...
#ifdef DEVICE_MODEL_GATEWAY
    message_len = strlen(DM_MSG_SERVICE_REQUEST_FMT) +
                  DM_UTILS_UINT32_STRLEN * 2 + identifier_len +
                  request->params.value_length + 1;
    message = DM_malloc(message_len);
    if (message == NULL) {
        return DM_MEMORY_NOT_ENOUGH;
    ...
    memset(message, 0, message_len);
    HAL_Snprintf(message, message_len, DM_MSG_SERVICE_REQUEST_FMT, id, devid,
                 identifier_len, identifier, request->params.value_length,
    ...
    message_len = strlen(DM_MSG_SERVICE_REQUEST_FMT) +
                  DM_UTILS_UINT32_STRLEN * 2 + identifier_len + 1;
    message = DM_malloc(message_len);
    if (message == NULL) {
        return DM_MEMORY_NOT_ENOUGH;
    }
