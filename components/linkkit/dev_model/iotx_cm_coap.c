...
#define IOTX_COAP_SERVER_URI \
    "coaps://pre.coap.cn-shanghai.link.aliyuncs.com:5684"
...
#ifdef ON_PRE
#define IOTX_COAP_SERVER_URI \
    "coap-psk://pre.coap.cn-shanghai.link.aliyuncs.com:5682"
#else /* online */
#define IOTX_COAP_SERVER_URI \
...
static struct list_head g_coap_response_list =
    LIST_HEAD_INIT(g_coap_response_list);
...
static int _coap_publish(iotx_cm_ext_params_t *params, const char *topic,
...
iotx_cm_connection_t *iotx_cm_open_coap(iotx_cm_init_param_t *params)
{
    ...
    deviceinfo =
        (iotx_coap_device_info_t *)cm_malloc(sizeof(iotx_coap_device_info_t));
    if (deviceinfo == NULL) {
...
static int iotx_set_devinfo(iotx_coap_device_info_t *p_devinfo)
{
    ...
    HAL_Snprintf(p_devinfo->device_id,
                 IOTX_PRODUCT_KEY_LEN + IOTX_DEVICE_NAME_LEN + 2, "%s.%s",
                 p_devinfo->product_key, p_devinfo->device_name);
    p_devinfo->device_id[IOTX_PRODUCT_KEY_LEN + IOTX_DEVICE_NAME_LEN + 1] =
        '\0';
    /**< end*/
    ...
    do {
        ...
        if (ret == 0) {
            ...
            if (_coap_conncection->event_handler) {
                _coap_conncection->event_handler(_coap_conncection->fd, &event,
                                                 (void *)_coap_conncection);
            }
            return 0;
        }
...
static void _coap_response_default(void *p_arg, void *p_message)
{
    ...
    list_for_each_entry_safe(node, next, &g_coap_response_list, linked_list,
                             coap_response_node_t)
    {
        if (node->token_num == token) {
            ...
            cm_free(node);
            HAL_MutexUnlock(
                _coap_conncection->list_lock); /* do not lock while callback */

            recieve_cb(_coap_conncection->fd, topic, (const char *)p_payload,
                       len, context);
...
static int _coap_publish(iotx_cm_ext_params_t *ext, const char *topic,
                         const char *payload, unsigned int payload_len)
{
    ...
    token =
        IOT_CoAP_GetCurToken((iotx_coap_context_t *)_coap_conncection->context);
    ret =
        IOT_CoAP_SendMessage((iotx_coap_context_t *)_coap_conncection->context,
                             (char *)topic, &message);
