 ...
 *
 * @param params, linkkit initialize parameters, return from
 ...
 *
 * @param initParams, linkkit initialize parameters, see linkkit_params_t for
 * more detail.
 *
 * @return 0 when success, < 0 when fail.
 */
...
typedef struct {
     ...
     *
     * @param in, properties to be get, in JSON array format, terminated by
     * NULL.
     * @param out, output buffer fill by user, in json format, terminated by
     * NULL.
     * @param out_len, out buffer length.
     * @param ctx, user private data passed by linkkit_gateway_start() or
     ...
     *
     * @param in, properties to be set, in JSON object format, terminated by
     * NULL.
     * @param ctx, user private data passed by linkkit_gateway_start() or
     * linkkit_gateway_subdev_create()
     *
     ...
     *
     * @param identifier, service identifier, available services define in TSL
     * file.
     * @param in, service input data, in JSON object format, terminated by NULL.
     * @param out, service output, this buffer will be filled by user, in json
     * format, terminated by NULL.
     * @param out_len, out buffer length.
     * @param ctx, user private data passed by linkkit_gateway_start() or
     * linkkit_gateway_subdev_create().
     *
     * @return 0 when success, -1 when fail.
     */
    int (*call_service)(char *identifier, char *in, char *out, int out_len,
                        void *ctx);
     ...
     * @param out, output data to cloud, allocated by linkkit fill by user, no
     * need to be free.
     * @param out_len, out buffer length.
     * @param ctx, user private data passed by linkkit_gateway_start() or
     * linkkit_gateway_subdev_create().
     ...
     */
    int (*down_rawdata)(const void *in, int in_len, void *out, int out_len,
                        void *ctx);

    /**
     * @brief return data from cloud when calling
 ...
 *
 * @param devid, device id return from linkkit_gateway_start() or
 * linkkit_gateway_subdev_create().
 * @param identifier, event identifier, see tsl file for more detail.
 * @param event, event data, in JSON format.
 * @param timeout_ms, transmission timeout, in milliseconds. when timeout_ms is
 * 0, wait no response.
 *
 * @return 0 when success, -1 when fail.
 */
 ...
 * @param timeout_ms, transmission timeout, in milliseconds. when timeout_ms is
 * 0, wait no response.
 * @param func, callback function when success(retval > 0), timeout(retval = 0)
 * or failed(retval < 0).
 ...
 *
 * @param callback_fp, user callback which register to fota. (NULL for
 * unregister)
 *
 * @return 0 when success, -1 when fail.
 */
 ...
 * @param devid, device id return from linkkit_gateway_start() or
 * linkkit_gateway_subdev_create().
 * @param extinfos, group of extend info to be deleted, attrValue in
 * linkkit_extinfo_t will be ignore.
 * @param nb_extinfos, number of extend infos in extinfos.
 * @param timeout_ms, transmission timeout, in milliseconds. when timeout_ms is
 * 0, wait no response.
 *
