...
static being_deprecated linkkit_solo_legacy_ctx_t *_linkkit_solo_legacy_get_ctx(
    void)
{
...
{
    linkkit_solo_legacy_ctx_t *linkkit_solo_ctx =
        _linkkit_solo_legacy_get_ctx();
...
static int _linkkit_solo_upstream_callback_list_insert(
    int msgid, handle_post_cb_fp_t callback)
{
    int count = 0;
    linkkit_solo_legacy_ctx_t *linkkit_solo_ctx =
        _linkkit_solo_legacy_get_ctx();
    ...
    list_for_each_entry(search_node, &linkkit_solo_ctx->callback_list,
                        linked_list, linkkit_solo_upstream_callback_node_t)
    {
        count++;
    ...
    search_node =
        IMPL_SOLO_MALLOC(sizeof(linkkit_solo_upstream_callback_node_t));
    if (search_node == NULL) {
        return FAIL_RETURN;
    }
...
static int _linkkit_solo_upstream_callback_list_search(
    int msgid, linkkit_solo_upstream_callback_node_t **node)
{
    linkkit_solo_legacy_ctx_t *linkkit_solo_ctx =
        _linkkit_solo_legacy_get_ctx();
    linkkit_solo_upstream_callback_node_t *search_node = NULL;
...
static int _linkkit_solo_upstream_callback_list_destroy(void)
{
    ...
    list_for_each_entry_safe(search_node, next_node,
                             &linkkit_solo_ctx->callback_list, linked_list,
...
static void _linkkit_solo_event_callback(iotx_dm_event_types_t type,
                                         char *payload)
{
    int res = 0;
    linkkit_solo_legacy_ctx_t *linkkit_solo_ctx =
    ...
    lite_cjson_t lite, lite_item_id, lite_item_code, lite_item_devid,
        lite_item_payload, lite_item_serviceid;
    lite_cjson_t lite_item_propertyid, lite_item_eventid, lite_item_configid,
        lite_item_configsize, lite_item_gettype;
    ...
    if (payload) {
        ...
        dm_utils_json_object_item(&lite, LINKKIT_SOLO_LEGACY_KEY_ID,
                                  strlen(LINKKIT_SOLO_LEGACY_KEY_ID),
                                  cJSON_Invalid, &lite_item_id);
        dm_utils_json_object_item(&lite, LINKKIT_SOLO_LEGACY_KEY_CODE,
                                  strlen(LINKKIT_SOLO_LEGACY_KEY_CODE),
                                  cJSON_Invalid, &lite_item_code);
        dm_utils_json_object_item(&lite, LINKKIT_SOLO_LEGACY_KEY_DEVID,
                                  strlen(LINKKIT_SOLO_LEGACY_KEY_DEVID),
                                  cJSON_Invalid, &lite_item_devid);
        dm_utils_json_object_item(&lite, LINKKIT_SOLO_LEGACY_KEY_PAYLOAD,
                                  strlen(LINKKIT_SOLO_LEGACY_KEY_PAYLOAD),
                                  cJSON_Invalid, &lite_item_payload);
        dm_utils_json_object_item(&lite, LINKKIT_SOLO_LEGACY_KEY_SERVICEID,
                                  strlen(LINKKIT_SOLO_LEGACY_KEY_SERVICEID),
                                  cJSON_Invalid, &lite_item_serviceid);
        dm_utils_json_object_item(&lite, LINKKIT_SOLO_LEGACY_KEY_PROPERTYID,
                                  strlen(LINKKIT_SOLO_LEGACY_KEY_PROPERTYID),
                                  cJSON_Invalid, &lite_item_propertyid);
        dm_utils_json_object_item(&lite, LINKKIT_SOLO_LEGACY_KEY_EVENTID,
                                  strlen(LINKKIT_SOLO_LEGACY_KEY_EVENTID),
                                  cJSON_Invalid, &lite_item_eventid);
        dm_utils_json_object_item(&lite, LINKKIT_SOLO_LEGACY_KEY_CONFIG_ID,
                                  strlen(LINKKIT_SOLO_LEGACY_KEY_CONFIG_ID),
                                  cJSON_Invalid, &lite_item_configid);
        dm_utils_json_object_item(&lite, LINKKIT_SOLO_LEGACY_KEY_CONFIG_SIZE,
                                  strlen(LINKKIT_SOLO_LEGACY_KEY_CONFIG_SIZE),
                                  cJSON_Invalid, &lite_item_configsize);
        dm_utils_json_object_item(&lite, LINKKIT_SOLO_LEGACY_KEY_GET_TYPE,
                                  strlen(LINKKIT_SOLO_LEGACY_KEY_GET_TYPE),
                                  cJSON_Invalid, &lite_item_gettype);
        dm_utils_json_object_item(&lite, LINKKIT_SOLO_LEGACY_KEY_SIGN,
                                  strlen(LINKKIT_SOLO_LEGACY_KEY_SIGN),
                                  cJSON_Invalid, &lite_item_sign);
        dm_utils_json_object_item(&lite, LINKKIT_SOLO_LEGACY_KEY_SIGN_METHOD,
                                  strlen(LINKKIT_SOLO_LEGACY_KEY_SIGN_METHOD),
                                  cJSON_Invalid, &lite_item_signmethod);
        dm_utils_json_object_item(&lite, LINKKIT_SOLO_LEGACY_KEY_URL,
                                  strlen(LINKKIT_SOLO_LEGACY_KEY_URL),
                                  cJSON_Invalid, &lite_item_url);
        dm_utils_json_object_item(&lite, LINKKIT_SOLO_LEGACY_KEY_VERSION,
                                  strlen(LINKKIT_SOLO_LEGACY_KEY_VERSION),
    ...
    case IOTX_DM_EVENT_CLOUD_CONNECTED:
        {
            if (linkkit_solo_ctx->user_callback->on_connect) {
                linkkit_solo_ctx->user_callback->on_connect(
    ...
    case IOTX_DM_EVENT_CLOUD_DISCONNECT:
        {
            if (linkkit_solo_ctx->user_callback->on_disconnect) {
                linkkit_solo_ctx->user_callback->on_disconnect(
                    linkkit_solo_ctx->user_context);
            }
        }
        break;
    case IOTX_DM_EVENT_MODEL_DOWN_RAW:
        {
            ...
            impl_solo_debug("Current Raw Data: %.*s",
                            lite_item_payload.value_length,
                            lite_item_payload.value);

            res = iotx_dm_deprecated_legacy_get_thingid_by_devid(
                lite_item_devid.value_int, &thing_id);
            if (res != SUCCESS_RETURN) {
                return;
            }

            res = dm_utils_str_to_hex(lite_item_payload.value,
            ...
            if (linkkit_solo_ctx->user_callback->raw_data_arrived) {
                linkkit_solo_ctx->user_callback->raw_data_arrived(
                    thing_id, raw_data, raw_data_len,
                    linkkit_solo_ctx->user_context);
            }
    ...
    case IOTX_DM_EVENT_THING_SERVICE_REQUEST:
        {
            ...
            impl_solo_debug("Current Devid: %d", lite_item_devid.value_int);
            impl_solo_debug("Current ServiceID: %.*s",
            ...
            memcpy(service, lite_item_serviceid.value,
                   lite_item_serviceid.value_length);

            if (linkkit_solo_ctx->user_callback->thing_call_service) {
                linkkit_solo_ctx->user_callback->thing_call_service(
                    thing_id, (const char *)service, lite_item_id.value_int,
                    linkkit_solo_ctx->user_context);
            }
    ...
    case IOTX_DM_EVENT_LEGACY_THING_CREATED:
        {
            ...
            if (linkkit_solo_ctx->user_callback->thing_create) {
                linkkit_solo_ctx->user_callback->thing_create(
                    thing_id, linkkit_solo_ctx->user_context);
            }
        }
        break;
    case IOTX_DM_EVENT_THING_DISABLE:
        {
            ...
            if (linkkit_solo_ctx->user_callback->thing_disable) {
                linkkit_solo_ctx->user_callback->thing_disable(
                    thing_id, linkkit_solo_ctx->user_context);
            }
        }
        break;
    case IOTX_DM_EVENT_THING_ENABLE:
        {
            ...
            if (linkkit_solo_ctx->user_callback->thing_enable) {
                linkkit_solo_ctx->user_callback->thing_enable(
                    thing_id, linkkit_solo_ctx->user_context);
            }
        }
        break;
    case IOTX_DM_EVENT_PROPERTY_SET:
        {
            ...
            if (payload == NULL || lite_item_devid.type != cJSON_Number ||
                lite_item_propertyid.type != cJSON_String) {
                return;
            }

            impl_solo_debug("Current Devid: %d", lite_item_devid.value_int);
            impl_solo_debug("Current PropertyID: %.*s",
                            lite_item_propertyid.value_length,
                            lite_item_propertyid.value);
            ...
            propertyid =
                IMPL_SOLO_MALLOC(lite_item_propertyid.value_length + 1);
            if (propertyid == NULL) {
                return;
            }
            memset(propertyid, 0, lite_item_propertyid.value_length + 1);
            memcpy(propertyid, lite_item_propertyid.value,
                   lite_item_propertyid.value_length);

            if (linkkit_solo_ctx->user_callback->thing_prop_changed) {
                linkkit_solo_ctx->user_callback->thing_prop_changed(
                    thing_id, propertyid, linkkit_solo_ctx->user_context);
            }
    ...
    case IOTX_DM_EVENT_EVENT_PROPERTY_POST_REPLY:
        {
            ...
            res = _linkkit_solo_upstream_callback_list_search(
                lite_item_id.value_int, &node);
            if (res == SUCCESS_RETURN) {
            ...
            if (res == SUCCESS_RETURN) {
                if (callback) {
                    callback(thing_id, lite_item_id.value_int,
                             lite_item_code.value_int, NULL,
                             linkkit_solo_ctx->user_context);
                }
                _linkkit_solo_upstream_mutex_lock();
    ...
    case IOTX_DM_EVENT_EVENT_SPECIFIC_POST_REPLY:
        {
            ...
            impl_solo_debug("Current Code: %d", lite_item_code.value_int);
            impl_solo_debug("Current Devid: %d", lite_item_devid.value_int);
            impl_solo_debug("Current EventID: %.*s",
    ...
    case IOTX_DM_EVENT_COTA_NEW_CONFIG:
        {
            ...
            impl_solo_debug("Current Config ID: %.*s",
                            lite_item_configid.value_length,
                            lite_item_configid.value);
            impl_solo_debug("Current Config Size: %d",
                            lite_item_configsize.value_int);
            impl_solo_debug("Current Get Type: %.*s",
                            lite_item_gettype.value_length,
                            lite_item_gettype.value);
            impl_solo_debug("Current Sign: %.*s", lite_item_sign.value_length,
                            lite_item_sign.value);
            impl_solo_debug("Current Sign Method: %.*s",
                            lite_item_signmethod.value_length,
                            lite_item_signmethod.value);
            impl_solo_debug("Current URL: %.*s", lite_item_url.value_length,
                            lite_item_url.value);
            ...
            if (linkkit_solo_ctx->cota_callback) {
                linkkit_solo_ctx->cota_callback(
                    service_cota_callback_type_new_version_detected, config_id,
                    lite_item_configsize.value_int, get_type, sign, sign_method,
                    url);
            }
    ...
    case IOTX_DM_EVENT_FOTA_NEW_FIRMWARE:
        {
            ...
            impl_solo_debug("Current Firmware Version: %.*s",
                            lite_item_version.value_length,
                            lite_item_version.value);

            _impl_copy(lite_item_version.value, lite_item_version.value_length,
                       (void **)&version, lite_item_version.value_length + 1);
            if (version == NULL) {
                return;
            }

            if (linkkit_solo_ctx->fota_callback) {
                linkkit_solo_ctx->fota_callback(
                    service_fota_callback_type_new_version_detected, version);
            }
                                   ...
                                   void *user_context)
{
    ...
    if (get_tsl_from_cloud != 0) {
        res = iotx_dm_deprecated_set_tsl(IOTX_DM_LOCAL_NODE_DEVID,
                                         IOTX_DM_TSL_SOURCE_CLOUD, NULL, 0);
        if (res < SUCCESS_RETURN) {
            HAL_MutexDestroy(linkkit_solo_ctx->mutex);
            HAL_MutexDestroy(linkkit_solo_ctx->upstream_mutex);
            iotx_dm_close();
...
void *linkkit_set_tsl(const char *tsl, int tsl_len)
{
    ...
    _linkkit_solo_mutex_lock();
    res = iotx_dm_deprecated_set_tsl(IOTX_DM_LOCAL_NODE_DEVID,
                                     IOTX_DM_TSL_SOURCE_LOCAL, tsl, tsl_len);
    if (res != SUCCESS_RETURN) {
        _linkkit_solo_mutex_unlock();
        return NULL;
    }
    ...
    res = iotx_dm_deprecated_legacy_get_thingid_by_devid(
        IOTX_DM_LOCAL_NODE_DEVID, &thing_id);
    if (res != SUCCESS_RETURN) {
        _linkkit_solo_mutex_unlock();
        return NULL;
    }
...
int being_deprecated linkkit_set_value(linkkit_method_set_t method_set,
                                       const void *thing_id,
                                       ...
                                       const void *value, const char *value_str)
{
    ...
    if (thing_id == NULL || identifier == NULL ||
        (value == NULL && value_str == NULL)) {
        impl_solo_err("Invalid Parameter");
        return FAIL_RETURN;
    }
    ...
    _linkkit_solo_mutex_lock();
    res = iotx_dm_deprecated_legacy_get_devid_by_thingid((void *)thing_id,
                                                         &devid);
    if (res != SUCCESS_RETURN) {
        _linkkit_solo_mutex_unlock();
        return FAIL_RETURN;
    ...
    case linkkit_method_set_property_value:
        {
            res = iotx_dm_deprecated_legacy_set_property_value(
                devid, (char *)identifier, strlen(identifier), (void *)value,
                (char *)value_str);
        }
        break;
    case linkkit_method_set_event_output_value:
        {
            res = iotx_dm_deprecated_legacy_set_event_output_value(
                devid, (char *)identifier, strlen(identifier), (void *)value,
                (char *)value_str);
        }
        break;
    case linkkit_method_set_service_output_value:
        {
            res = iotx_dm_deprecated_legacy_set_service_output_value(
                devid, (char *)identifier, strlen(identifier), (void *)value,
                (char *)value_str);
        }
        break;
...
int being_deprecated linkkit_get_value(linkkit_method_get_t method_get,
                                       const void *thing_id,
                                       const char *identifier, void *value,
                                       char **value_str)
{
    int res = 0, devid = 0;
    linkkit_solo_legacy_ctx_t *linkkit_solo_ctx =
    ...
    case linkkit_method_get_property_value:
        {
            res = iotx_dm_deprecated_legacy_get_property_value(
                devid, (char *)identifier, strlen(identifier), value,
                value_str);
        }
        break;
    ...
    case linkkit_method_get_service_input_value:
        {
            res = iotx_dm_deprecated_legacy_get_service_input_value(
                devid, (char *)identifier, strlen(identifier), value,
                value_str);
        }
        break;
    case linkkit_method_get_service_output_value:
        {
            res = iotx_dm_deprecated_legacy_get_service_output_value(
                devid, (char *)identifier, strlen(identifier), value,
                value_str);
        }
        break;
...
int being_deprecated linkkit_answer_service(const void *thing_id,
                                            ...
                                            int response_id, int code)
{
    ...
    res = iotx_dm_deprecated_send_service_response(
        devid, response_id, (iotx_dm_error_code_t)code,
...
int being_deprecated linkkit_invoke_raw_service(const void *thing_id,
                                                int is_up_raw, void *raw_data,
                                                int raw_data_length)
{
    int res = 0, devid = 0;
    linkkit_solo_legacy_ctx_t *linkkit_solo_ctx =
...
int being_deprecated linkkit_trigger_extended_info_operate(
    const void *thing_id, const char *params,
    linkkit_extended_info_operate_t linkkit_extended_info_operation)
{
    ...
    case linkkit_extended_info_operate_update:
        {
            res = iotx_dm_deviceinfo_update(devid, (char *)params,
                                            strlen(params));
        }
        break;
    case linkkit_extended_info_operate_delete:
        {
            res = iotx_dm_deviceinfo_delete(devid, (char *)params,
                                            strlen(params));
        }
        break;
    default:
...
int being_deprecated linkkit_trigger_event(const void *thing_id,
                                           ...
                                           handle_post_cb_fp_t cb)
{
    ...
    res = iotx_dm_deprecated_post_event(devid, (char *)event_identifier,
                                        strlen((char *)event_identifier));
    if (res < SUCCESS_RETURN) {
        _linkkit_solo_mutex_unlock();
        return FAIL_RETURN;
    }
...
int being_deprecated linkkit_post_property(const void *thing_id,
                                           const char *property_identifier,
                                           handle_post_cb_fp_t cb)
{
    int res = 0, devid = 0, msgid = 0, property_identifier_len = 0,
        post_property_reply = 0;
    ...
    property_identifier_len =
        (property_identifier) ? (strlen((char *)property_identifier)) : (0);
    res = iotx_dm_deprecated_post_property_add(
        property_handle, (char *)property_identifier, property_identifier_len);
    if (res != SUCCESS_RETURN) {
    ...
    res =
        iotx_dm_get_opt(linkkit_opt_property_post_reply, &post_property_reply);
...
int being_deprecated
linkkit_cota_init(handle_service_cota_callback_fp_t callback_fp)
{
    linkkit_solo_legacy_ctx_t *linkkit_solo_ctx =
        _linkkit_solo_legacy_get_ctx();
...
int being_deprecated linkkit_invoke_cota_service(void *data_buf,
                                                 int data_buf_length)
{
    int res = 0;
    linkkit_solo_legacy_ctx_t *linkkit_solo_ctx =
...
int being_deprecated linkkit_invoke_cota_get_config(const char *config_scope,
                                                    const char *get_type,
...
int being_deprecated
linkkit_fota_init(handle_service_fota_callback_fp_t callback_fp)
{
    linkkit_solo_legacy_ctx_t *linkkit_solo_ctx =
        _linkkit_solo_legacy_get_ctx();
...
int being_deprecated linkkit_invoke_fota_service(void *data_buf,
                                                 int data_buf_length)
{
    int res = 0;
    linkkit_solo_legacy_ctx_t *linkkit_solo_ctx =
