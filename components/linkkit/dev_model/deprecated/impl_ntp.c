...
#define SERVER_TX_TIME "serverSendTime"
     ...
     */
    elem = json_get_value_by_name(payload, payload_len, SERVER_TX_TIME, &len,
                                  NULL);
    ...
    elem = json_get_value_by_name(payload, payload_len, SERVER_RX_TIME, &len,
                                  NULL);
    if (elem == NULL || len <= 0 || len > NTP_TIME_STR_MAX_LEN) {
        goto NTP_FAIL;
    }
    ...
    elem =
        json_get_value_by_name(payload, payload_len, DEV_TX_TIME, &len, NULL);
    if (elem == NULL || len <= 0 || len > NTP_TIME_STR_MAX_LEN) {
        goto NTP_FAIL;
    }
    /*
...
int linkkit_ntp_time_request(void (*ntp_reply)(const char *ntp_offset_time_ms))
{
    ...
    do {
        ...
        ret = IOT_MQTT_Subscribe_Sync(
            NULL, topic, IOTX_MQTT_QOS3_SUB_LOCAL,
            (iotx_mqtt_event_handle_func_fpt)linkkit_ntp_time_reply, NULL,
            1000);
#else
        ret = IOT_MQTT_Subscribe_Sync(
            NULL, topic, IOTX_MQTT_QOS0,
            (iotx_mqtt_event_handle_func_fpt)linkkit_ntp_time_reply, NULL,
            1000);
#endif
        if (ret < 0) {
    ...
    final_len = HAL_Snprintf(packet, packet_len, "{\"deviceSendTime\":\"%u\"}",
                             (unsigned int)(HAL_UptimeMs()));

    log_debug("[ntp]", "report ntp:%s\r\n", packet);

    ret =
        IOT_MQTT_Publish_Simple(NULL, topic, IOTX_MQTT_QOS0, packet, final_len);
