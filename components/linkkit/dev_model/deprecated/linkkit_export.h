...
typedef void (*handle_post_cb_fp_t)(const void *thing_id, int respons_id,
                                    int code, const char *response_message,
                                    void *ctx);
typedef void (*handle_subdev_cb_fp_t)(const void *thing_id, int code,
                                      const char *response_message, int success,
                                      void *ctx);
...
typedef struct _linkkit_ops {
    ...
    int (*on_connect)(
        void *ctx,
        int cloud); /* true: cloud connection; false: local connection */
    int (*on_disconnect)(
        void *ctx,
        int cloud); /* true: cloud connection; false: local connection */
#else
    int (*on_connect)(
        void *ctx); /* true: cloud connection; false: local connection */
    int (*on_disconnect)(
        void *ctx); /* true: cloud connection; false: local connection */
#endif
    int (*raw_data_arrived)(const void *thing_id, const void *data, int len,
                            void *ctx);
    ...
    int (*thing_call_service)(const void *thing_id, const char *service,
                              int request_id, void *ctx);
    int (*thing_prop_changed)(const void *thing_id, const char *property,
                              void *ctx);
    int (*linkit_data_arrived)(const void *thing_id, const void *data, int len,
                               void *ctx);
} linkkit_ops_t;
...
/**
 * @brief start linkkit routines, and install callback funstions(async type for
 * cloud connecting).
 *
 ...
 * @param get_tsl_from_cloud, config if device need to get tsl from cloud(!0) or
 * local(0), if local selected, must invoke linkkit_set_tsl to tell tsl to dm
 ...
 * type, use '.' as delimeter like "identifier1.ientifier2" to point to specific
 * item. value and value_str could not be NULL at the same time; if value and
 * value_str both as not NULL, value shall be used and value_str will be
 * ignored. if value is NULL, value_str not NULL, value_str will be used. in
 * brief, value will be used if not NULL, value_str will be used only if value
 * is NULL.
 *
 ...
 * @param value. The value to be set, data type decided by data type of property
 * as follows: int: int*, float: float*, double: double*, text: char*, enum:
 * int*, date: char*, bool: int*
 *
 ...
 * output type, use '.' as delimeter like "identifier1.ientifier2" to point to
 * specific item. value and value_str could not be NULL at the same time; if
 * value and value_str both as not NULL, value shall be used and value_str will
 * be ignored. if value is NULL, value_str not NULL, value_str will be used. in
 * brief, value will be used if not NULL, value_str will be used only if value
 * is NULL.
 ...
 * @param value. The variable to store value, data type decided by data type of
 * property as follows: int: int*, float: float*, double: double*, text: char**,
 * enum: int*, date: char**, bool: int*
 *
 * @param value_str, value to get in string format. If success, memory of
 * *value_str will be allocated, user should free the memory.
 *
 ...
 * @param service_identifier, service identifier to answer, user should get this
 * identifier from handle_dm_callback_fp_t type callback report that
 * "dm_callback_type_service_requested" happened, use this function to generate
 * response to the service sender.
 * @param response_id, id value in response payload. its value is from
 * "dm_callback_type_service_requested" type callback function. use the same id
 * as the request to send response as the same communication session.
 * @param code, code value in response payload. for example, 200 when service
 * successfully executed, 400 when not successfully executed.
 *
 * @return 0 when success, -1 when fail.
 */
...
/**
 * @brief answer a down raw service when a raw service requested by cloud, or
 * invoke a up raw service to cloud.
 *
 * @param thing_id, pointer to thing object.
 ...
 * "event.property.post", if set, post specified property, if NULL, post all.
...
typedef void (*handle_service_cota_callback_fp_t)(
    service_cota_callback_type_t callback_type, const char *configid,
    uint32_t configsize, const char *gettype, const char *sign,
    const char *signmethod, const char *cota_url);
 ...
 *
 * @param data_buf, data buf that used to do ota. ota service will use this
 * buffer to download bin.
...
typedef void (*handle_service_fota_callback_fp_t)(
    service_fota_callback_type_t callback_type, const char *version);
 ...
 *                      when cloud returns ntp reply, sdk would trigger the
 * callback function
 *
 * @return 0 when success, -1 when fail.
 */
int being_deprecated
linkkit_ntp_time_request(void (*ntp_reply_cb)(const char *ntp_offset_time_ms));
