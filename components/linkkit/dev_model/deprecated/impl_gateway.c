...
{
    linkkit_gateway_legacy_ctx_t *linkkit_gateway_ctx =
        _linkkit_gateway_legacy_get_ctx();
...
static int _linkkit_gateway_callback_list_insert(int devid,
                                                 ...
                                                 void *context)
{
    ...
    list_for_each_entry(search_node, &linkkit_gateway_ctx->dev_callback_list,
                        linked_list, linkkit_gateway_dev_callback_node_t)
    {
...
static int _linkkit_gateway_callback_list_search(
    int devid, linkkit_gateway_dev_callback_node_t **node)
{
    linkkit_gateway_legacy_ctx_t *linkkit_gateway_ctx =
        _linkkit_gateway_legacy_get_ctx();
    linkkit_gateway_dev_callback_node_t *search_node = NULL;
...
static void _linkkit_gateway_callback_list_destroy(void)
{
    ...
    list_for_each_entry_safe(search_node, next_node,
                             &linkkit_gateway_ctx->dev_callback_list,
                             linked_list, linkkit_gateway_dev_callback_node_t)
    {
        list_del(&search_node->linked_list);
        IMPL_GATEWAY_FREE(search_node);
...
static int _linkkit_gateway_upstream_sync_callback_list_insert(
    ...
    linkkit_gateway_upstream_sync_callback_node_t **node)
{
    ...
    list_for_each_entry(
        search_node, &linkkit_gateway_ctx->upstream_sync_callback_list,
        linked_list, linkkit_gateway_upstream_sync_callback_node_t)
    {
    ...
    search_node = IMPL_GATEWAY_MALLOC(
        sizeof(linkkit_gateway_upstream_sync_callback_node_t));
    if (search_node == NULL) {
        return DM_MEMORY_NOT_ENOUGH;
    }
    memset(search_node, 0,
           sizeof(linkkit_gateway_upstream_sync_callback_node_t));
    search_node->msgid = msgid;
    search_node->semaphore = semaphore;
    INIT_LIST_HEAD(&search_node->linked_list);

    list_add(&search_node->linked_list,
             &linkkit_gateway_ctx->upstream_sync_callback_list);
...
static int _linkkit_gateway_upstream_sync_callback_list_search(
    int msgid, linkkit_gateway_upstream_sync_callback_node_t **node)
{
    linkkit_gateway_legacy_ctx_t *linkkit_gateway_ctx =
        _linkkit_gateway_legacy_get_ctx();
    linkkit_gateway_upstream_sync_callback_node_t *search_node = NULL;
...
static int _linkkit_gateway_upstream_async_callback_list_insert(
    int msgid, int timeout_ms, linkkit_gateway_upstream_async_callback callback,
    void *context)
{
    linkkit_gateway_legacy_ctx_t *linkkit_gateway_ctx =
        _linkkit_gateway_legacy_get_ctx();
    ...
    list_for_each_entry(
        search_node, &linkkit_gateway_ctx->upstream_async_callback_list,
        linked_list, linkkit_gateway_upstream_async_callback_node_t)
    {
        if (search_node->msgid == msgid) {
    ...
    node = IMPL_GATEWAY_MALLOC(
        sizeof(linkkit_gateway_upstream_async_callback_node_t));
    if (node == NULL) {
        return DM_MEMORY_NOT_ENOUGH;
    }
    ...
    list_add(&node->linked_list,
             &linkkit_gateway_ctx->upstream_async_callback_list);
...
static int _linkkit_gateway_upstream_async_callback_list_search(
    int msgid, linkkit_gateway_upstream_async_callback_node_t **node)
{
    linkkit_gateway_legacy_ctx_t *linkkit_gateway_ctx =
        _linkkit_gateway_legacy_get_ctx();
    linkkit_gateway_upstream_async_callback_node_t *search_node = NULL;
...
static void _linkkit_gateway_upstream_callback_remove(int msgid, int code)
{
    ...
    res =
        _linkkit_gateway_upstream_sync_callback_list_search(msgid, &sync_node);
    if (res != SUCCESS_RETURN) {
        linkkit_gateway_upstream_async_callback_node_t *node = NULL;
        res =
            _linkkit_gateway_upstream_async_callback_list_search(msgid, &node);
        if (res == SUCCESS_RETURN) {
            ...
            if (current_time - node->timestamp_ms > node->timeout_ms) {
                ...
                if (node->callback) {
                    int return_value = (code == IOTX_DM_ERR_CODE_SUCCESS)
                                           ? (SUCCESS_RETURN)
                                           : (FAIL_RETURN);
                    impl_gateway_info("Async Message %d Result: %d", msgid,
                                      return_value);
...
int linkkit_gateway_setopt(linkkit_params_t *params, int option, void *value,
                           int value_len)
{
    linkkit_gateway_legacy_ctx_t *linkkit_gateway_ctx =
        _linkkit_gateway_legacy_get_ctx();
...
int linkkit_gateway_set_event_callback(linkkit_params_t *params,
                                       int (*event_cb)(linkkit_event_t *ev,
                                                       void *ctx),
                                       void *ctx)
{
    linkkit_gateway_legacy_ctx_t *linkkit_gateway_ctx =
...
int linkkit_gateway_init(linkkit_params_t *initParams)
{
    ...
    if (initParams->maxMsgSize < 256 || initParams->maxMsgQueueSize < 1 ||
        initParams->threadPoolSize < 1 || initParams->threadStackSize < 1024) {
        return FAIL_RETURN;
    }
...
static void _linkkit_gateway_event_callback(iotx_dm_event_types_t type,
                                            char *payload)
{
    linkkit_gateway_legacy_ctx_t *linkkit_gateway_ctx =
        _linkkit_gateway_legacy_get_ctx();
    ...
    case IOTX_DM_EVENT_CLOUD_CONNECTED:
        {
            if (linkkit_gateway_ctx->init_params.event_cb) {
                ...
                linkkit_gateway_ctx->init_params.event_cb(
                    &event, linkkit_gateway_ctx->init_params.ctx);
            }
        }
        break;
    ...
    case IOTX_DM_EVENT_SUBDEV_REGISTER_REPLY:
        {
            ...
            /* Parse Message ID */
            res = lite_cjson_object_item(&lite, LINKKIT_GATEWAY_LEGACY_KEY_ID,
                                         strlen(LINKKIT_GATEWAY_LEGACY_KEY_ID),
                                         &lite_item_id);
            ...
            /* Parse Message Code */
            res = lite_cjson_object_item(
                &lite, LINKKIT_GATEWAY_LEGACY_KEY_CODE,
                strlen(LINKKIT_GATEWAY_LEGACY_KEY_CODE), &lite_item_code);
            if (res != SUCCESS_RETURN ||
            ...
            /* Parse Devid */
            res = lite_cjson_object_item(
                &lite, LINKKIT_GATEWAY_LEGACY_KEY_DEVID,
                strlen(LINKKIT_GATEWAY_LEGACY_KEY_DEVID), &lite_item_devid);
            if (res != SUCCESS_RETURN ||
    ...
    case IOTX_DM_EVENT_PROPERTY_SET:
        {
            ...
            res = lite_cjson_object_item(
                &lite, LINKKIT_GATEWAY_LEGACY_KEY_PAYLOAD,
                strlen(LINKKIT_GATEWAY_LEGACY_KEY_PAYLOAD), &lite_item_payload);
            if (res != SUCCESS_RETURN) {
                return;
            }
            impl_gateway_debug("Current Payload: %.*",
            ...
            memcpy(params, lite_item_payload.value,
                   lite_item_payload.value_length);

            _linkkit_gateway_upstream_mutex_lock();
            res = _linkkit_gateway_callback_list_search(
                lite_item_devid.value_int, &node);
            _linkkit_gateway_upstream_mutex_unlock();
            if (res == SUCCESS_RETURN) {
    ...
    case IOTX_DM_EVENT_GATEWAY_PERMIT:
        {
            ...
            res = lite_cjson_object_item(
                &lite, LINKKIT_GATEWAY_LEGACY_KEY_PRODUCT_KEY,
                strlen(LINKKIT_GATEWAY_LEGACY_KEY_PRODUCT_KEY), &lite_item_pk);
            if (res != SUCCESS_RETURN ||
                lite_item_pk.value_length >= IOTX_PRODUCT_KEY_LEN + 1) {
                return;
            }
            impl_gateway_debug("Current Product Key: %.*s",
                               lite_item_pk.value_length, lite_item_pk.value);

            /* Parse Timeout */
            memset(&lite_item_timeout, 0, sizeof(lite_cjson_t));
            res = lite_cjson_object_item(
                &lite, LINKKIT_GATEWAY_LEGACY_KEY_TIME,
                strlen(LINKKIT_GATEWAY_LEGACY_KEY_TIME), &lite_item_timeout);
            if (res != SUCCESS_RETURN) {
                return;
            }
            impl_gateway_debug("Current Timeout: %d",
                               lite_item_timeout.value_int);
            ...
            if (linkkit_gateway_ctx->init_params.event_cb) {
                ...
                event.event_data.subdev_permited.timeoutSec =
                    lite_item_timeout.value_int;
                linkkit_gateway_ctx->init_params.event_cb(
                    &event, linkkit_gateway_ctx->init_params.ctx);
            }
    ...
    case IOTX_DM_EVENT_THING_SERVICE_REQUEST:
        {
            int res = 0;
            linkkit_gateway_dev_callback_node_t *node = NULL;
            lite_cjson_t lite, lite_item_id, lite_item_devid,
                lite_item_serviceid, lite_item_paylaod;
            ...
            res = lite_cjson_object_item(
                &lite, LINKKIT_GATEWAY_LEGACY_KEY_SERVICEID,
                strlen(LINKKIT_GATEWAY_LEGACY_KEY_SERVICEID),
                &lite_item_serviceid);
            if (res != SUCCESS_RETURN) {
                return;
            }
            impl_gateway_debug("Current ServiceID: %.*s",
            ...
            res = lite_cjson_object_item(
                &lite, LINKKIT_GATEWAY_LEGACY_KEY_PAYLOAD,
                strlen(LINKKIT_GATEWAY_LEGACY_KEY_PAYLOAD), &lite_item_paylaod);
            if (res != SUCCESS_RETURN) {
                return;
            }
            impl_gateway_debug("Current Payload: %.*s",
                               lite_item_paylaod.value_length,
                               lite_item_paylaod.value);

            identifier =
                IMPL_GATEWAY_MALLOC(lite_item_serviceid.value_length + 1);
            if (identifier == NULL) {
                return;
            }
            memset(identifier, 0, lite_item_serviceid.value_length + 1);
            memcpy(identifier, lite_item_serviceid.value,
                   lite_item_serviceid.value_length);
            ...
            memcpy(input, lite_item_paylaod.value,
                   lite_item_paylaod.value_length);

            output = IMPL_GATEWAY_MALLOC(
                linkkit_gateway_ctx->init_params.maxMsgSize + 1);
            if (output == NULL) {
                IMPL_GATEWAY_FREE(identifier);
            ...
            if (res == SUCCESS_RETURN) {
                if (node->callback->call_service) {
                    res = node->callback->call_service(
                        identifier, input, output,
                        linkkit_gateway_ctx->init_params.maxMsgSize,
                        node->callback_ctx);
                    if (res == SUCCESS_RETURN) {
                        iotx_dm_deprecated_legacy_send_service_response(
                            lite_item_devid.value_int, lite_item_id.value_int,
                            200, lite_item_serviceid.value,
                            lite_item_serviceid.value_length, output,
                            strlen(output));
                    } else {
                        iotx_dm_deprecated_legacy_send_service_response(
                            lite_item_devid.value_int, lite_item_id.value_int,
                            202, lite_item_serviceid.value,
                            lite_item_serviceid.value_length, "{}",
                            strlen("{}"));
                    }
                }
    ...
    case IOTX_DM_EVENT_MODEL_DOWN_RAW:
        {
            ...
            res = lite_cjson_object_item(
                &lite, LINKKIT_GATEWAY_LEGACY_KEY_PAYLOAD,
                strlen(LINKKIT_GATEWAY_LEGACY_KEY_PAYLOAD), &lite_item_rawdata);
            if (res != SUCCESS_RETURN) {
                return;
            }
            impl_gateway_debug("Current Raw Data: %.*s",
                               lite_item_rawdata.value_length,
                               lite_item_rawdata.value);
            ...
            if (res == SUCCESS_RETURN) {
                if (node->callback->down_rawdata) {
                    res = node->callback->down_rawdata(
                        lite_item_rawdata.value, lite_item_rawdata.value_length,
                        output, linkkit_gateway_ctx->init_params.maxMsgSize,
                        node->callback_ctx);
                    if (res > 0) {
                        iotx_dm_post_rawdata(lite_item_devid.value_int, output,
                                             res);
                    }
                }
            }
    ...
    case IOTX_DM_EVENT_MODEL_UP_RAW_REPLY:
        {
            ...
            if (res == SUCCESS_RETURN) {
                if (node->callback->post_rawdata_reply) {
                    node->callback->post_rawdata_reply(
                        lite_item_rawdata.value, lite_item_rawdata.value_length,
                        node->callback_ctx);
                }
            }
    ...
    case IOTX_DM_EVENT_EVENT_SPECIFIC_POST_REPLY:
        {
            ...
            res = lite_cjson_object_item(
                &lite, LINKKIT_GATEWAY_LEGACY_KEY_EVENTID,
                strlen(LINKKIT_GATEWAY_LEGACY_KEY_EVENTID), &lite_item_eventid);
            if (res != SUCCESS_RETURN) {
                return;
            }
            impl_gateway_debug("Current EventID: %.*s",
            ...
            memcpy(eventid, lite_item_eventid.value,
                   lite_item_eventid.value_length);
    ...
    case IOTX_DM_EVENT_FOTA_NEW_FIRMWARE:
        {
            ...
            res = lite_cjson_object_item(
                &lite, LINKKIT_GATEWAY_LEGACY_KEY_VERSION,
                strlen(LINKKIT_GATEWAY_LEGACY_KEY_VERSION), &lite_item_version);
            if (res != SUCCESS_RETURN ||
                !lite_cjson_is_string(&lite_item_version)) {
                return;
            }
            impl_gateway_debug("Current Firmware Version: %.*s",
            ...
            if (linkkit_gateway_ctx->fota_callback) {
                linkkit_gateway_ctx->fota_callback(
...
{
    linkkit_gateway_legacy_ctx_t *linkkit_gateway_ctx =
        (linkkit_gateway_legacy_ctx_t *)params;
...
int linkkit_gateway_start(linkkit_cbs_t *cbs, void *ctx)
{
    ...
    res = HAL_ThreadCreate(&linkkit_gateway_ctx->dispatch_thread,
                           _linkkit_gateway_dispatch, linkkit_gateway_ctx, NULL,
                           &stack_used);
    if (res != SUCCESS_RETURN) {
        HAL_MutexDestroy(linkkit_gateway_ctx->mutex);
        HAL_MutexDestroy(linkkit_gateway_ctx->upstream_mutex);
    ...
    /* Insert Gateway Callback And Callback Context Into Device Callback Linkked
     * List */
    INIT_LIST_HEAD(&linkkit_gateway_ctx->dev_callback_list);

    res = _linkkit_gateway_callback_list_insert(IOTX_DM_LOCAL_NODE_DEVID, cbs,
                                                ctx);
    if (res != SUCCESS_RETURN) {
        HAL_MutexDestroy(linkkit_gateway_ctx->mutex);
        HAL_MutexDestroy(linkkit_gateway_ctx->upstream_mutex);
    ...
    _linkkit_gateway_upstream_mutex_lock();
    res =
        _linkkit_gateway_callback_list_search(IOTX_DM_LOCAL_NODE_DEVID, &node);
    _linkkit_gateway_upstream_mutex_unlock();
    if (res == SUCCESS_RETURN) {
        if (node->callback->register_complete) {
            node->callback->register_complete(node->callback_ctx);
...
int linkkit_gateway_subdev_register(char *productKey, char *deviceName,
                                    char *deviceSecret)
{
    ...
    _linkkit_gateway_mutex_lock();
    res = iotx_dm_deprecated_legacy_get_devid_by_pkdn(productKey, deviceName,
                                                      &devid);
    if (res != SUCCESS_RETURN) {
        _linkkit_gateway_mutex_unlock();
        return FAIL_RETURN;
    ...
    _linkkit_gateway_upstream_mutex_lock();
    res = _linkkit_gateway_upstream_sync_callback_list_insert(msgid, semaphore,
                                                              &node);
    if (res != SUCCESS_RETURN) {
    ...
    res = HAL_SemaphoreWait(semaphore,
                            LINKKIT_GATEWAY_LEGACY_SYNC_DEFAULT_TIMEOUT_MS);
    if (res < SUCCESS_RETURN) {
        _linkkit_gateway_upstream_mutex_lock();
    ...
    if (res > SUCCESS_RETURN) {
        ...
        _linkkit_gateway_upstream_mutex_lock();
        res = _linkkit_gateway_upstream_sync_callback_list_insert(
            msgid, semaphore, &node);
        if (res != SUCCESS_RETURN) {
            HAL_SemaphoreDestroy(semaphore);
            _linkkit_gateway_upstream_mutex_unlock();
            _linkkit_gateway_mutex_unlock();
    ...
    if (res == SUCCESS_RETURN) {
        if (dev_callback_node->callback->register_complete) {
            dev_callback_node->callback->register_complete(
                dev_callback_node->callback_ctx);
        }
    }
...
int linkkit_gateway_subdev_create(char *productKey, char *deviceName,
                                  linkkit_cbs_t *cbs, void *ctx)
{
    int res = 0, devid = 0;
    linkkit_gateway_legacy_ctx_t *linkkit_gateway_ctx =
        _linkkit_gateway_legacy_get_ctx();
...
int linkkit_gateway_get_devinfo(int devid, linkkit_devinfo_t *devinfo)
{
    ...
    res = iotx_dm_deprecated_legacy_get_pkdn_ptr_by_devid(
        devid, &(devinfo->productKey), &(devinfo->deviceName));
    if (res != SUCCESS_RETURN) {
        _linkkit_gateway_mutex_unlock();
        return FAIL_RETURN;
    }
...
int linkkit_gateway_trigger_event_json_sync(int devid, char *identifier,
                                            char *event, int timeout_ms)
{
    ...
    if (timeout_ms == 0 || event_reply_value == 0) {
        res = iotx_dm_post_event(devid, identifier, strlen(identifier), event,
                                 strlen(event));
        if (res < SUCCESS_RETURN) {
            _linkkit_gateway_mutex_unlock();
            return FAIL_RETURN;
...
int linkkit_gateway_trigger_event_json(int devid, char *identifier, char *event,
                                       int timeout_ms,
                                       ...
                                       void *ctx)
{
    ...
    _linkkit_gateway_upstream_mutex_lock();
    res = _linkkit_gateway_upstream_async_callback_list_insert(res, timeout_ms,
                                                               func, ctx);
    if (res != SUCCESS_RETURN) {
        _linkkit_gateway_upstream_mutex_unlock();
        _linkkit_gateway_mutex_unlock();
...
int linkkit_gateway_post_property_json_sync(int devid, char *property,
                                            int timeout_ms)
{
...
int linkkit_gateway_post_property_json(int devid, char *property,
                                       int timeout_ms,
                                       void (*func)(int retval, void *ctx),
                                       void *ctx)
{
...
int linkkit_gateway_post_extinfos(int devid, linkkit_extinfo_t *extinfos,
                                  int nb_extinfos, int timeout_ms)
{
    ...
    for (index = 0; index < nb_extinfos; index++) {
        if (extinfos[index].attrKey == NULL ||
            extinfos[index].attrValue == NULL) {
        ...
        lite_cjson_add_string_to_object(lite_array_item, "attrKey",
                                        extinfos[index].attrKey);
        lite_cjson_add_string_to_object(lite_array_item, "attrValue",
                                        extinfos[index].attrValue);
...
int linkkit_gateway_delete_extinfos(int devid, linkkit_extinfo_t *extinfos,
                                    int nb_extinfos, int timeout_ms)
{
    int res = 0, index = 0, msgid = 0, code = 0;
    linkkit_gateway_upstream_sync_callback_node_t *node = NULL;
