...
static int _dm_mgr_search_dev_by_devid(_IN_ int devid,
                                       _OU_ dm_mgr_dev_node_t **node)
{
    dm_mgr_ctx *ctx = _dm_mgr_get_ctx();
...
static int _dm_mgr_search_dev_by_pkdn(
    ...
    _OU_ dm_mgr_dev_node_t **node)
{
                        ...
                        dm_mgr_dev_node_t)
    {
        if ((strlen(search_node->product_key) == strlen(product_key)) &&
            (memcmp(search_node->product_key, product_key,
                    strlen(product_key)) == 0) &&
            (strlen(search_node->device_name) == strlen(device_name)) &&
            (memcmp(search_node->device_name, device_name,
                    strlen(device_name)) == 0)) {
            /* dm_log_debug("Device Found, Product Key: %s, Device Name: %s",
    ...
    dm_log_debug("Device Not Found, Product Key: %s, Device Name: %s",
                 product_key, device_name);
    return FAIL_RETURN;
}

static int _dm_mgr_insert_dev(_IN_ int devid, _IN_ int dev_type,
...
static void _dm_mgr_destroy_devlist(void)
{
    ...
    list_for_each_entry_safe(del_node, next_node, &ctx->dev_list, linked_list,
                             dm_mgr_dev_node_t)
    {
...
int dm_mgr_init(void)
{
    ...
    res = _dm_mgr_insert_dev(IOTX_DM_LOCAL_NODE_DEVID, IOTX_DM_DEVICE_TYPE,
                             product_key, device_name);
    if (res != SUCCESS_RETURN) {
        goto ERROR;
    }
...
int dm_mgr_device_create(_IN_ int dev_type,
                         _IN_ char product_key[IOTX_PRODUCT_KEY_LEN + 1],
...
int dm_mgr_get_next_devid(_IN_ int devid, _OU_ int *devid_next)
{
    ...
    list_for_each_entry(next_node, &ctx->dev_list, linked_list,
                        dm_mgr_dev_node_t)
    {
...
int dm_mgr_search_device_by_devid(
    _IN_ int devid, _OU_ char product_key[IOTX_PRODUCT_KEY_LEN + 1],
    _OU_ char device_name[IOTX_DEVICE_NAME_LEN + 1],
    _OU_ char device_secret[IOTX_DEVICE_SECRET_LEN + 1])
{
    int res = 0;
    dm_mgr_dev_node_t *node = NULL;
...
int dm_mgr_search_device_by_pkdn(
    _IN_ char product_key[IOTX_PRODUCT_KEY_LEN + 1],
...
int dm_mgr_set_device_secret(
    _IN_ int devid, _IN_ char device_secret[IOTX_DEVICE_SECRET_LEN + 1])
{
    int res = 0;
    dm_mgr_dev_node_t *node = NULL;
...
int dm_mgr_upstream_thing_sub_register(_IN_ int devid)
{
    ...
    res = dm_msg_thing_sub_register(node->product_key, node->device_name,
                                    &request);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
    ...
    dm_msg_cache_insert(request.msgid, request.devid,
                        IOTX_DM_EVENT_SUBDEV_REGISTER_REPLY, NULL);
#endif
...
int dm_mgr_upstream_thing_proxy_product_register(_IN_ int devid)
{
    ...
    /* Get Params And Method */
    res = dm_msg_thing_proxy_product_register(
        node->product_key, node->product_secret, node->device_name, &request);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
...
#if !defined(DM_MESSAGE_CACHE_DISABLED)
    dm_msg_cache_insert(request.msgid, request.devid,
                        IOTX_DM_EVENT_PROXY_PRODUCT_REGISTER_REPLY, NULL);
#endif
    /* Send Message To Cloud */
    res = dm_msg_request(DM_MSG_DEST_CLOUD, &request);
#if !defined(DM_MESSAGE_CACHE_DISABLED)
...
int dm_mgr_upstream_thing_sub_unregister(_IN_ int devid)
{
    ...
    /* Get Params And Method */
    res = dm_msg_thing_sub_unregister(node->product_key, node->device_name,
                                      &request);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
...
#if !defined(DM_MESSAGE_CACHE_DISABLED)
    dm_msg_cache_insert(request.msgid, request.devid,
                        IOTX_DM_EVENT_SUBDEV_UNREGISTER_REPLY, NULL);
#endif
...
int dm_mgr_upstream_thing_topo_add(_IN_ int devid)
{
    ...
    /* Get Params And Method */
    res = dm_msg_thing_topo_add(node->product_key, node->device_name,
                                node->device_secret, &request);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
...
#if !defined(DM_MESSAGE_CACHE_DISABLED)
    dm_msg_cache_insert(request.msgid, request.devid,
                        IOTX_DM_EVENT_TOPO_ADD_REPLY, NULL);
#endif
    /* Send Message To Cloud */
    res = dm_msg_request(DM_MSG_DEST_CLOUD, &request);
#if !defined(DM_MESSAGE_CACHE_DISABLED)
...
int dm_mgr_upstream_thing_topo_delete(_IN_ int devid)
{
    ...
    /* Get Params And Method */
    res = dm_msg_thing_topo_delete(node->product_key, node->device_name,
                                   &request);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
...
#if !defined(DM_MESSAGE_CACHE_DISABLED)
    dm_msg_cache_insert(request.msgid, request.devid,
                        IOTX_DM_EVENT_TOPO_DELETE_REPLY, NULL);
#endif
    /* Send Message To Cloud */
    res = dm_msg_request(DM_MSG_DEST_CLOUD, &request);
#if !defined(DM_MESSAGE_CACHE_DISABLED)
...
int dm_mgr_upstream_thing_topo_get(void)
{
    ...
    res = _dm_mgr_search_dev_by_pkdn(request.product_key, request.device_name,
                                     &node);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
...
#if !defined(DM_MESSAGE_CACHE_DISABLED)
    dm_msg_cache_insert(request.msgid, request.devid,
                        IOTX_DM_EVENT_TOPO_GET_REPLY, NULL);
#endif
    /* Send Message To Cloud */
    res = dm_msg_request(DM_MSG_DEST_CLOUD, &request);
#if !defined(DM_MESSAGE_CACHE_DISABLED)
...
int dm_mgr_upstream_thing_list_found(_IN_ int devid)
{
    ...
    /* Get Params And Method */
    res =
        dm_msg_thing_list_found(node->product_key, node->device_name, &request);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
...
#if !defined(DM_MESSAGE_CACHE_DISABLED)
    dm_msg_cache_insert(request.msgid, request.devid,
                        IOTX_DM_EVENT_TOPO_ADD_NOTIFY_REPLY, NULL);
#endif
    /* Send Message To Cloud */
    res = dm_msg_request(DM_MSG_DEST_CLOUD, &request);
#if !defined(DM_MESSAGE_CACHE_DISABLED)
...
int dm_mgr_upstream_combine_login(_IN_ int devid)
{
    ...
    /* Get Params And Method */
    res = dm_msg_combine_login(node->product_key, node->device_name,
                               node->device_secret, &request);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
...
#if !defined(DM_MESSAGE_CACHE_DISABLED)
    dm_msg_cache_insert(request.msgid, request.devid,
                        IOTX_DM_EVENT_COMBINE_LOGIN_REPLY, NULL);
#endif
    /* Send Message To Cloud */
    res = dm_msg_request(DM_MSG_DEST_CLOUD, &request);
#if !defined(DM_MESSAGE_CACHE_DISABLED)
...
#if !defined(DM_MESSAGE_CACHE_DISABLED)
    dm_msg_cache_insert(request.msgid, request.devid,
                        IOTX_DM_EVENT_COMBINE_LOGOUT_REPLY, NULL);
#endif
...
int dm_mgr_upstream_thing_firmware_version_update(_IN_ int devid,
                                                  _IN_ char *payload,
...
int dm_mgr_upstream_thing_model_up_raw(_IN_ int devid, _IN_ char *payload,
                                       _IN_ int payload_len)
{
    int res = 0, res1 = 0;
    dm_mgr_dev_node_t *node = NULL;
    ...
    res = dm_client_publish(uri, (unsigned char *)payload, payload_len,
                            dm_client_thing_model_up_raw_reply);
#ifdef ALCS_ENABLED
...
static int _dm_mgr_upstream_request_assemble(
    _IN_ int msgid, _IN_ int devid, _IN_ const char *service_prefix,
    _IN_ const char *service_name, _IN_ char *params, _IN_ int params_len,
    _IN_ char *method, _OU_ dm_msg_request_t *request)
{
    int res = 0;
    dm_mgr_dev_node_t *node = NULL;
...
int dm_mgr_upstream_thing_property_desired_get(_IN_ int devid,
                                               _IN_ char *payload,
                                               _IN_ int payload_len)
{
    int res = 0;
    ...
    memset(&request, 0, sizeof(dm_msg_request_t));
    res = _dm_mgr_upstream_request_assemble(
        iotx_report_id(), devid, DM_URI_SYS_PREFIX,
        DM_URI_THING_PROPERTY_DESIRED_GET, payload, payload_len,
        "thing.property.desired.get", &request);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
...
#if !defined(DM_MESSAGE_CACHE_DISABLED)
    res = dm_opt_get(DM_OPT_DOWNSTREAM_EVENT_PROPERTY_DESIRED_GET_REPLY,
                     &prop_desired_get_reply);
    if (res == SUCCESS_RETURN && prop_desired_get_reply) {
        dm_msg_cache_insert(request.msgid, request.devid,
                            IOTX_DM_EVENT_PROPERTY_DESIRED_GET_REPLY, NULL);
    }
#endif
...
int dm_mgr_upstream_thing_property_desired_delete(_IN_ int devid,
                                                  _IN_ char *payload,
                                                  _IN_ int payload_len)
{
    int res = 0;
    ...
    res = _dm_mgr_upstream_request_assemble(
        iotx_report_id(), devid, DM_URI_SYS_PREFIX,
        DM_URI_THING_PROPERTY_DESIRED_DELETE, payload, payload_len,
        "thing.property.desired.delete", &request);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
...
#if !defined(DM_MESSAGE_CACHE_DISABLED)
    res = dm_opt_get(DM_OPT_DOWNSTREAM_EVENT_PROPERTY_DESIRED_DELETE_REPLY,
                     &prop_desired_delete_reply);
    if (res == SUCCESS_RETURN && prop_desired_delete_reply) {
        dm_msg_cache_insert(request.msgid, request.devid,
                            IOTX_DM_EVENT_PROPERTY_DESIRED_DELETE_REPLY, NULL);
    }
#endif
    /* Send Message To Cloud */
    res = dm_msg_request(DM_MSG_DEST_CLOUD, &request);
...
int dm_mgr_upstream_thing_property_post(_IN_ int devid, _IN_ char *payload,
                                        _IN_ int payload_len)
{
    int res = 0;
    dm_msg_request_t request;
    ...
    res = _dm_mgr_upstream_request_assemble(
        iotx_report_id(), devid, DM_URI_SYS_PREFIX,
        DM_URI_THING_EVENT_PROPERTY_POST, payload, payload_len,
        "thing.event.property.post", &request);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
    ...
    if (res == SUCCESS_RETURN && prop_post_reply) {
        dm_msg_cache_insert(request.msgid, request.devid,
                            IOTX_DM_EVENT_EVENT_PROPERTY_POST_REPLY, NULL);
    }
#endif
    /* Send Message To Cloud */
...
int dm_mgr_upstream_thing_log_post(_IN_ int devid, _IN_ char *payload,
                                   _IN_ int payload_len, int force_upload)
{
    int res = 0;
    dm_msg_request_t request;
    ...
    if (0 == force_upload) {
        ...
        if (log_size + payload_len < OVERFLOW_LEN) {
            ...
            /* it should NOT happen; it means that it is too late to upload log
             * files */
            reset_log_poll();
    ...
    memset(&request, 0, sizeof(dm_msg_request_t));
    res = _dm_mgr_upstream_request_assemble(
        iotx_report_id(), devid, DM_URI_SYS_PREFIX, DM_URI_THING_LOG_POST,
...
int dm_mgr_upstream_thing_event_post(_IN_ int devid, _IN_ char *identifier,
                                     _IN_ int identifier_len, _IN_ char *method,
                                     _IN_ char *payload, _IN_ int payload_len)
{
    ...
    HAL_Snprintf(service_name, service_name_len, DM_URI_THING_EVENT_POST,
                 identifier_len, identifier);

    memset(&request, 0, sizeof(dm_msg_request_t));
    res = _dm_mgr_upstream_request_assemble(
        iotx_report_id(), devid, DM_URI_SYS_PREFIX, service_name, payload,
        payload_len, method, &request);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
...
int dm_mgr_upstream_thing_deviceinfo_update(_IN_ int devid, _IN_ char *payload,
                                            _IN_ int payload_len)
{
    int res = 0;
    dm_msg_request_t request;
    ...
    res = _dm_mgr_upstream_request_assemble(
        iotx_report_id(), devid, DM_URI_SYS_PREFIX,
        DM_URI_THING_DEVICEINFO_UPDATE, payload, payload_len,
        "thing.deviceinfo.update", &request);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
...
#if !defined(DM_MESSAGE_CACHE_DISABLED)
    dm_msg_cache_insert(request.msgid, request.devid,
                        IOTX_DM_EVENT_DEVICEINFO_UPDATE_REPLY, NULL);
#endif
    /* Send Message To Cloud */
    res = dm_msg_request(DM_MSG_DEST_CLOUD, &request);
#if !defined(DM_MESSAGE_CACHE_DISABLED)
...
int dm_mgr_upstream_thing_deviceinfo_delete(_IN_ int devid, _IN_ char *payload,
                                            _IN_ int payload_len)
{
    int res = 0;
    dm_msg_request_t request;
    ...
    res = _dm_mgr_upstream_request_assemble(
        iotx_report_id(), devid, DM_URI_SYS_PREFIX,
        DM_URI_THING_DEVICEINFO_DELETE, payload, payload_len,
        "thing.deviceinfo.delete", &request);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
...
#if !defined(DM_MESSAGE_CACHE_DISABLED)
    dm_msg_cache_insert(request.msgid, request.devid,
                        IOTX_DM_EVENT_DEVICEINFO_DELETE_REPLY, NULL);
#endif
    /* Send Message To Cloud */
    res = dm_msg_request(DM_MSG_DEST_CLOUD, &request);
#if !defined(DM_MESSAGE_CACHE_DISABLED)
...
int dm_mgr_upstream_thing_dsltemplate_get(_IN_ int devid)
{
    ...
    res = _dm_mgr_upstream_request_assemble(
        iotx_report_id(), devid, DM_URI_SYS_PREFIX,
        DM_URI_THING_DSLTEMPLATE_GET, params, params_len,
        "thing.dsltemplate.get", &request);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
#if !defined(DM_MESSAGE_CACHE_DISABLED)
    dm_msg_cache_insert(request.msgid, request.devid,
                        IOTX_DM_EVENT_DSLTEMPLATE_GET_REPLY, NULL);
#endif
...
int dm_mgr_upstream_thing_dynamictsl_get(_IN_ int devid)
{
    ...
    memset(&request, 0, sizeof(dm_msg_request_t));
    res = _dm_mgr_upstream_request_assemble(
        iotx_report_id(), devid, DM_URI_SYS_PREFIX, DM_URI_THING_DYNAMICTSL_GET,
...
int dm_mgr_upstream_ntp_request(void)
{
    ...
    res = dm_client_publish(uri, (unsigned char *)ntp_request_fmt,
                            strlen(ntp_request_fmt), dm_client_ntp_response);
    if (res != SUCCESS_RETURN) {
...
static int _dm_mgr_upstream_response_assemble(
    _IN_ int devid, _IN_ char *msgid, _IN_ int msgid_len,
    _IN_ const char *prefix, _IN_ const char *service_name, _IN_ int code,
...
int dm_mgr_upstream_thing_service_response(_IN_ int devid, _IN_ char *msgid,
                                           _IN_ int msgid_len,
                                           _IN_ iotx_dm_error_code_t code,
                                           _IN_ char *identifier,
                                           _IN_ int identifier_len,
                                           _IN_ char *payload,
                                           _IN_ int payload_len, void *ctx)
{
    int res = 0, service_name_len = 0;
    char *service_name = NULL;
    ...
    if (devid < 0 || msgid == NULL || msgid_len <= 0 || identifier == NULL ||
        identifier_len <= 0 || payload == NULL || payload_len <= 0) {
        return DM_INVALID_PARAMETER;
    }

    /* Service Name */
    service_name_len =
        strlen(DM_URI_THING_SERVICE_RESPONSE) + identifier_len + 1;
    service_name = DM_malloc(service_name_len);
    if (service_name == NULL) {
        return DM_MEMORY_NOT_ENOUGH;
    }
    memset(service_name, 0, service_name_len);
    HAL_Snprintf(service_name, service_name_len, DM_URI_THING_SERVICE_RESPONSE,
                 identifier_len, identifier);

    res = _dm_mgr_upstream_response_assemble(devid, msgid, msgid_len,
                                             DM_URI_SYS_PREFIX, service_name,
    ...
    if (ctx != NULL) {
        dm_msg_response(DM_MSG_DEST_LOCAL, &request, &response, payload,
                        payload_len, ctx);
    } else {
        dm_msg_response(DM_MSG_DEST_CLOUD, &request, &response, payload,
                        payload_len, ctx);
    }
...
int dm_mgr_upstream_thing_property_get_response(
    _IN_ int devid, _IN_ char *msgid, _IN_ int msgid_len,
    _IN_ iotx_dm_error_code_t code, _IN_ char *payload, _IN_ int payload_len,
    _IN_ void *ctx)
{
    int res = 0;
    dm_msg_request_payload_t request;
    ...
    if (devid < 0 || msgid == NULL || msgid_len <= 0 || payload == NULL ||
        payload_len <= 0) {
        return DM_INVALID_PARAMETER;
    }
    ...
    res = _dm_mgr_upstream_response_assemble(
        devid, msgid, msgid_len, DM_URI_SYS_PREFIX, reply_service_name, code,
        &request, &response);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
    dm_log_debug("Current Service Name: %s", reply_service_name);
    dm_msg_response(reply_msg_type, &request, &response, payload, payload_len,
                    ctx);
...
int dm_mgr_upstream_rrpc_response(_IN_ int devid, _IN_ char *msgid,
                                  _IN_ int msgid_len,
                                  _IN_ iotx_dm_error_code_t code,
                                  _IN_ char *rrpcid, _IN_ int rrpcid_len,
                                  _IN_ char *payload, _IN_ int payload_len)
{
    int res = 0, service_name_len = 0;
    ...
    if (devid < 0 || msgid == NULL || msgid_len <= 0 || rrpcid == NULL ||
        rrpcid_len <= 0 || payload == NULL || payload_len <= 0) {
        return DM_INVALID_PARAMETER;
    }
    ...
    memset(service_name, 0, service_name_len);
    HAL_Snprintf(service_name, service_name_len, rrpc_response_service_name,
                 rrpcid_len, rrpcid);
    ...
    dm_log_debug("Current Service Name: %s", service_name);
    dm_msg_response(DM_MSG_DEST_ALL, &request, &response, payload, payload_len,
                    NULL);
...
#ifdef DEPRECATED_LINKKIT
int dm_mgr_deprecated_set_tsl_source(_IN_ int devid,
                                     _IN_ iotx_dm_tsl_source_t tsl_source)
{
    int res = 0;
    dm_mgr_dev_node_t *node = NULL;
...
int dm_mgr_deprecated_get_tsl_source(_IN_ int devid,
                                     _IN_ iotx_dm_tsl_source_t *tsl_source)
{
    int res = 0;
    dm_mgr_dev_node_t *node = NULL;
...
static int dm_mgr_deprecated_search_devid_by_node(_IN_ dm_mgr_dev_node_t *node,
                                                  _OU_ int *devid)
{
    dm_mgr_ctx *ctx = _dm_mgr_get_ctx();
    dm_mgr_dev_node_t *search_node = NULL;
...
int dm_mgr_deprecated_search_devid_by_device_node(_IN_ void *node,
                                                  _OU_ int *devid)
{
    int res = 0;
    ...
    res = dm_mgr_deprecated_search_devid_by_node((dm_mgr_dev_node_t *)node,
                                                 devid);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
...
int dm_mgr_deprecated_set_tsl(int devid, iotx_dm_tsl_type_t tsl_type,
...
int dm_mgr_deprecated_get_property_data(_IN_ int devid, _IN_ char *key,
...
int dm_mgr_deprecated_get_service_input_data(_IN_ int devid, _IN_ char *key,
                                             _IN_ int key_len, _OU_ void **data)
{
    int res = 0;
    dm_mgr_dev_node_t *node = NULL;
    ...
    res = dm_shw_get_service_input_output_data(
        DM_SHW_DATA_TARGET_SERVICE_INPUT_DATA, node->dev_shadow, key, key_len,
        data);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
...
int dm_mgr_deprecated_get_service_output_data(_IN_ int devid, _IN_ char *key,
                                              _IN_ int key_len,
                                              _OU_ void **data)
{
    int res = 0;
    ...
    res = dm_shw_get_service_input_output_data(
        DM_SHW_DATA_TARGET_SERVICE_OUTPUT_DATA, node->dev_shadow, key, key_len,
        data);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
...
int dm_mgr_deprecated_get_event_output_data(_IN_ int devid, _IN_ char *key,
                                            _IN_ int key_len, _OU_ void **data)
{
    int res = 0;
    dm_mgr_dev_node_t *node = NULL;
...
int dm_mgr_deprecated_get_data_type(_IN_ void *data,
                                    _OU_ dm_shw_data_type_e *type)
{
...
int dm_mgr_deprecated_get_property_by_index(_IN_ int devid, _IN_ int index,
                                            _OU_ void **property)
{
    int res = 0;
    dm_mgr_dev_node_t *node = NULL;
...
int dm_mgr_deprecated_get_service_by_index(_IN_ int devid, _IN_ int index,
                                           _OU_ void **service)
{
    int res = 0;
    dm_mgr_dev_node_t *node = NULL;
...
int dm_mgr_deprecated_get_event_by_index(_IN_ int devid, _IN_ int index,
                                         _OU_ void **event)
{
    int res = 0;
    dm_mgr_dev_node_t *node = NULL;
...
int dm_mgr_deprecated_get_service_by_identifier(_IN_ int devid,
                                                _IN_ char *identifier,
                                                _OU_ void **service)
{
    int res = 0;
    ...
    if (devid < 0 || identifier == NULL || service == NULL ||
        *service != NULL) {
        return DM_INVALID_PARAMETER;
    }
    res = _dm_mgr_search_dev_by_devid(devid, &node);
    ...
    return dm_shw_get_service_by_identifier(node->dev_shadow, identifier,
                                            service);
}

int dm_mgr_deprecated_get_event_by_identifier(_IN_ int devid,
                                              _IN_ char *identifier,
                                              _OU_ void **event)
{
    int res = 0;
...
int dm_mgr_deprecated_get_property_identifier(_IN_ void *property,
                                              _OU_ char **identifier)
{
...
int dm_mgr_deprecated_set_property_value(_IN_ int devid, _IN_ char *key,
                                         ...
                                         _IN_ int value_len)
{
    ...
    res = dm_shw_set_property_value(node->dev_shadow, key, key_len, value,
                                    value_len);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
...
int dm_mgr_deprecated_get_property_value(_IN_ int devid, _IN_ char *key,
                                         _IN_ int key_len, _IN_ void *value)
{
    int res = 0;
    dm_mgr_dev_node_t *node = NULL;
...
int dm_mgr_deprecated_set_event_output_value(_IN_ int devid, _IN_ char *key,
                                             _IN_ int key_len, _IN_ void *value,
                                             _IN_ int value_len)
{
    int res = 0;
    ...
    res = dm_shw_set_event_output_value(node->dev_shadow, key, key_len, value,
                                        value_len);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
...
int dm_mgr_deprecated_get_event_output_value(_IN_ int devid, _IN_ char *key,
                                             _IN_ int key_len, _IN_ void *value)
{
    int res = 0;
    dm_mgr_dev_node_t *node = NULL;
...
int dm_mgr_deprecated_set_service_input_value(_IN_ int devid, _IN_ char *key,
                                              _IN_ int key_len,
                                              _IN_ void *value,
                                              _IN_ int value_len)
{
    ...
    res = dm_shw_set_service_input_output_value(
        DM_SHW_DATA_TARGET_SERVICE_INPUT_DATA, node->dev_shadow, key, key_len,
        value, value_len);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
...
int dm_mgr_deprecated_get_service_input_value(_IN_ int devid, _IN_ char *key,
                                              _IN_ int key_len,
                                              _IN_ void *value)
{
    int res = 0;
    ...
    res = dm_shw_get_service_input_output_value(
        DM_SHW_DATA_TARGET_SERVICE_INPUT_DATA, node->dev_shadow, key, key_len,
        value);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
...
int dm_mgr_deprecated_set_service_output_value(_IN_ int devid, _IN_ char *key,
                                               _IN_ int key_len,
                                               _IN_ void *value,
                                               _IN_ int value_len)
{
...
int dm_mgr_deprecated_get_service_output_value(_IN_ int devid, _IN_ char *key,
                                               _IN_ int key_len,
                                               _IN_ void *value)
{
    int res = 0;
...
int dm_mgr_deprecated_assemble_property(_IN_ int devid, _IN_ char *identifier,
                                        _IN_ int identifier_len,
                                        _IN_ lite_cjson_item_t *lite)
{
    ...
    res = dm_shw_assemble_property(node->dev_shadow, identifier, identifier_len,
                                   lite);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
...
int dm_mgr_deprecated_assemble_event_output(_IN_ int devid,
                                            _IN_ char *identifier,
                                            _IN_ int identifier_len,
                                            _IN_ lite_cjson_item_t *lite)
{
    ...
    res = dm_shw_assemble_event_output(node->dev_shadow, identifier,
                                       identifier_len, lite);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
...
int dm_mgr_deprecated_assemble_service_output(_IN_ int devid,
                                              _IN_ char *identifier,
                                              _IN_ int identifier_len,
                                              _IN_ lite_cjson_item_t *lite)
{
    ...
    res = dm_shw_assemble_service_output(node->dev_shadow, identifier,
                                         identifier_len, lite);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
...
int dm_mgr_deprecated_upstream_thing_service_response(
    _IN_ int devid, _IN_ int msgid, _IN_ iotx_dm_error_code_t code,
    _IN_ char *identifier, _IN_ int identifier_len, _IN_ char *payload,
    _IN_ int payload_len)
{
    int res = 0, service_name_len = 0;
    ...
    res = _dm_mgr_upstream_response_assemble(
        devid, msgid_str, strlen(msgid_str), DM_URI_SYS_PREFIX, service_name,
        code, &request, &response);
    if (res != SUCCESS_RETURN) {
        return FAIL_RETURN;
    }
