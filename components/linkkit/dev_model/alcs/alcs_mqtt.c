...
static alcs_mqtt_status_e __alcs_mqtt_publish(char *topic, int qos, void *data,
                                              int len)
{
    return (IOT_MQTT_Publish_Simple(NULL, topic, qos, data, len) < 0)
                                                    ...
                                                    int code, char *data)
{
    ...
    msg_pub = ALCS_ADAPTER_malloc(msg_len);
    if (msg_pub == NULL) {
        return ALCS_MQTT_STATUS_ERROR;
    }

    HAL_Snprintf(msg_pub, msg_len, ALCS_MQTT_THING_LAN_PREFIX_RESPONSE_FMT, id,
                 code, data);
    ...
    uint16_t secret_len)
{
    ...
    if (pk == NULL || pk_len >= IOTX_PRODUCT_KEY_LEN + 1 || dn == NULL ||
        dn_len >= IOTX_DEVICE_NAME_LEN + 1 || prefix == NULL ||
        secret == NULL) {
                                                ...
                                                char *prefix, char *secret)
{
    ...
    if (pk == NULL || strlen(pk) >= IOTX_PRODUCT_KEY_LEN + 1 || dn == NULL ||
        strlen(dn) >= IOTX_DEVICE_NAME_LEN + 1 || prefix == NULL ||
        secret == NULL) {
        COAP_ERR("Invalid Parameter");
        return ALCS_MQTT_STATUS_ERROR;
    }
                                               ...
                                               const char *dn, uint16_t dn_len)
{
    ...
    if (pk == NULL || strlen(pk) >= IOTX_PRODUCT_KEY_LEN + 1 || dn == NULL ||
        strlen(dn) >= IOTX_DEVICE_NAME_LEN + 1) {
        COAP_ERR("Invalid Parameter");
        return ALCS_MQTT_STATUS_ERROR;
    }
...
static void __alcs_mqtt_subscribe_callback(void *pcontext, void *pclient,
                                           iotx_mqtt_event_msg_pt msg)
{
    ...
    HAL_Snprintf(topic_compare, ALCS_MQTT_TOPIC_MAX_LEN,
    ...
    if ((strlen(topic_compare) == topic_len) &&
        ...
        if (NULL != param && 0 != param_len) {
            ...
            if (NULL != alcs_mqtt_ctx->coap_ctx && prefix) {
                ...
                mpk_len = strlen(mpk);
                mdn_len = strlen(mdn);
                if (alcs_mqtt_prefix_secret_load(mpk, mpk_len, mdn, mdn_len,
                                                 mprefix, msecret) ==
                    ALCS_MQTT_STATUS_SUCCESS) {
                    char back1 = prefix[prefix_len];
                    prefix[prefix_len] = 0;
                    alcs_add_svr_key(alcs_mqtt_ctx->coap_ctx, prefix, msecret,
                                     FROMCLOUDSVR);
                    prefix[prefix_len] = back1;
                    if (__alcs_mqtt_prefix_secret_save(
                            mpk, mpk_len, mdn, mdn_len, prefix, prefix_len,
                            msecret,
    ...
    memset(topic_compare, 0, ALCS_MQTT_TOPIC_MAX_LEN);
    HAL_Snprintf(
        topic_compare, ALCS_MQTT_TOPIC_MAX_LEN,
    ...
    if ((strlen(topic_compare) == topic_len) &&
        ...
        if (NULL != param && 0 != param_len) {
            char reply_topic[ALCS_MQTT_TOPIC_MAX_LEN] = { 0 };
            blacklist = json_get_value_by_name(param, param_len,
                                               ALCS_MQTT_JSON_KEY_BLACK,
                                               &blacklist_len, NULL);
                                  ...
                                  char *device_name)
{
    ...
    if (handle == NULL || product_key == NULL ||
        strlen(product_key) > IOTX_PRODUCT_KEY_LEN || device_name == NULL ||
    ...
    HAL_Snprintf(topic, ALCS_MQTT_TOPIC_MAX_LEN,
                 ALCS_MQTT_PREFIX ALCS_MQTT_THING_LAN_PREFIX_GET_REPLY_FMT,
...
alcs_mqtt_status_e alcs_mqtt_prefixkey_update(void *ctx)
{
    ...
    if (alcs_mqtt_prefix_secret_load(product_key, prodkey_len, device_name,
                                     devname_len, prefix,
                                               ...
                                               const char *device_name)
{
    ...
    HAL_Snprintf(msg_pub, msg_len, ALCS_MQTT_THING_ALCS_SUBDEV_REQUEST, id,
                 (int)strlen(product_key), product_key,
                 (int)strlen(device_name), device_name);
