...
static localsetup_status __alcs_localsetup_kv_set(const char *key,
...
static localsetup_status __alcs_localsetup_kv_get(const char *key, void *buffer,
                                                  int *buffer_len)
{
    int rc = -1;

    rc = HAL_Kv_Get(key, buffer, buffer_len);
    if (rc != 0) {
        COAP_WRN("HAL_Kv_Get('%s') = %d (!= 0), return %d", key, rc,
                 ALCS_LOCALSETUP_ERROR);
        return ALCS_LOCALSETUP_ERROR;
    }
                                    ...
                                    char key_md5_hexstr[33])
{
    ...
    if (pk == NULL || pk_len >= IOTX_PRODUCT_KEY_LEN + 1 || dn == NULL ||
        dn_len >= IOTX_DEVICE_NAME_LEN + 1) {
        COAP_ERR("Invalid Parameter");
        return ALCS_LOCALSETUP_ERROR;
    }

    /* Calculate Key */
    HAL_Snprintf(key_source, sizeof(key_source), "%.*s%.*s", pk_len, pk, dn_len,
                 dn);
...
static localsetup_status __alcs_localsetup_ac_as_save(
    ...
    uint16_t secret_len)
{
    ...
    if (ALCS_LOCALSETUP_SUCCESS !=
        __alcs_localsetup_kv_set(key_md5_hexstr, value,
                                 prefix_len + secret_len + 3, 1)) {
                                             ...
                                             char *secret, int secret_len)
{
    ...
    /* Get Value */
    if (ALCS_LOCALSETUP_SUCCESS !=
        __alcs_localsetup_kv_get(key_md5_hexstr, value, &value_len)) {
...
static localsetup_status __alcs_localsetup_ac_as_del(const char *pk,
                                                     uint16_t pk_len,
                                                     const char *dn,
                                                     uint16_t dn_len)
{
...
static void alcs_service_cb_setup(CoAPContext *context, const char *paths,
                                  NetworkAddr *remote, CoAPMessage *message)
{
    ...
    do {
        ...
        id = json_get_value_by_name((char *)message->payload,
                                    message->payloadlen, "id", &idlen,
                                    (int *)NULL);
        p = json_get_value_by_name((char *)message->payload,
                                   message->payloadlen, "params", &len,
                                   (int *)NULL);
        if (!p || !len) {
        ...
        json_array_for_each_entry(p, len, str_pos, entry, entry_len, type)
        {
            ...
            ac = json_get_value_by_name(entry, entry_len, "authCode", &aclen,
                                        (int *)NULL);
            as = json_get_value_by_name(entry, entry_len, "authSecret", &aslen,
                                        (int *)NULL);
            pk = json_get_value_by_name(entry, entry_len, "productKey", &pklen,
                                        (int *)NULL);
            dn = json_get_value_by_name(entry, entry_len, "deviceName", &dnlen,
                                        (int *)NULL);
            break;
        ...
        __alcs_localsetup_ac_as_save(pk, pklen, dn, dnlen, ac, aclen, as,
                                     aslen);

        alcs_add_svr_key(g_coap_handle, ac, as, LOCALSETUP);

        restore_json_str_last_char(ac, aclen, acBack);
        restore_json_str_last_char(as, aslen, asBack) success = 1;

    } while (0);

    if (success) {
        HAL_Snprintf(payload, sizeof(payload), "{\"id\":\"%.*s\",\"code\":200}",
                     idlen, id ? id : "");
    } else {
        HAL_Snprintf(payload, sizeof(payload),
                     "{\"id\":\"%.*s\",\"code\":400,\"msg\":\"%s\"}", idlen,
                     id ? id : "", err_msg);
    ...
    if (message) {
        iotx_alcs_send_Response(g_adapter_handle, &rsp_msg,
                                message->header.tokenlen, message->token);
    }
}

static void alcs_localsetup_register_resource(void *adapter_handle, char *pk,
                                              char *dn)
{
    iotx_alcs_res_t alcs_res;
    char uri[IOTX_PRODUCT_KEY_LEN + IOTX_DEVICE_NAME_LEN + 24];

    if (adapter_handle == NULL || pk == NULL ||
        strlen(pk) > IOTX_PRODUCT_KEY_LEN || dn == NULL ||
        strlen(dn) > IOTX_DEVICE_NAME_LEN) {
        return;
    }
...
void alcs_localsetup_init(void *adapter_handle, void *coap_handler, char *pk,
                          char *dn)
{
    char prefix[10];
    char secret[64];
    ...
    if (alcs_localsetup_ac_as_load(pk, strlen(pk), dn, strlen(dn), prefix,
                                   sizeof(prefix), secret,
