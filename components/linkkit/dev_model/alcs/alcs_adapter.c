...
static int _iotx_alcs_send_list_search_and_remove(
    iotx_alcs_adapter_t *adapter, CoAPMessage *message,
    iotx_alcs_send_msg_t **send_msg)
{
    iotx_alcs_send_msg_t *node = NULL;
    iotx_alcs_send_msg_t *next = NULL;

    list_for_each_entry_safe(node, next, &adapter->alcs_send_list, linked_list,
                             iotx_alcs_send_msg_t)
    {
    ...
    case COAP_REQUEST_SUCCESS:
        {
            ...
            res = _iotx_alcs_send_list_search_and_remove(adapter, message,
                                                         &send_msg);
            ...
            adapter->alcs_event_handle->h_fp(
                adapter->alcs_event_handle->pcontext, (void *)adapter, &event);
...
void iotx_alcs_coap_adapter_event_notifier(unsigned int event,
                                           NetworkAddr *remote, void *message)
{
    COAP_INFO(
        "ALCS Coap Event: %d, Remote Device Address: %s, Remote Device Port: "
        "%d",
...
static void _iotx_alcs_adapter_subdev_list_destroy(iotx_alcs_adapter_t *adapter)
{
    ...
    list_for_each_entry_safe(node, next, &adapter->alcs_subdev_list,
                             linked_list, iotx_alcs_subdev_item_t)
    {
        list_del(&node->linked_list);
        ALCS_free(node);
    }
...
int iotx_alcs_adapter_init(iotx_alcs_adapter_t *adapter,
                           iotx_alcs_param_t *param)
{
    int res;
    ...
    adapter->alcs_event_handle =
        (iotx_alcs_event_handle_t *)ALCS_ADAPTER_malloc(
            sizeof(iotx_alcs_event_handle_t));
    if (adapter->alcs_event_handle == NULL) {
        iotx_alcs_adapter_deinit();
        COAP_ERR("ALCS Event Handle Init Failed");
        return FAIL_RETURN;
    }
    memcpy(adapter->alcs_event_handle, param->handle_event,
           sizeof(iotx_alcs_event_handle_t));
...
static int _iotx_alcs_subdev_list_search(const char *pk, const char *dn,
                                         iotx_alcs_subdev_item_t **subdev_item)
{
    ...
    list_for_each_entry(node, &adapter->alcs_subdev_list, linked_list,
...
int iotx_alcs_subdev_update_stage(iotx_alcs_subdev_item_t *item)
{
    ...
    HAL_MutexLock(adapter->mutex);
    res = _iotx_alcs_subdev_list_search(item->product_key, item->device_name,
                                        &subdev_item);
        ...
        if (node->stage == IOTX_ALCS_SUBDEV_DISCONNCET_CLOUD) {
            if (((time_now > node->retry_ms) &&
                 (time_now - node->retry_ms >=
                  IOTX_ALCS_SUBDEV_RETRY_INTERVAL_MS)) ||
                ((time_now <= node->retry_ms) &&
                 ((0xFFFFFFFFFFFFFFFF - node->retry_ms) + time_now >=
                  IOTX_ALCS_SUBDEV_RETRY_INTERVAL_MS))) {
                /* Get Prefix And Secret From Cloud */
                alcs_mqtt_subdev_prefix_get(node->product_key,
                                            node->device_name);
...
void *iotx_alcs_construct(iotx_alcs_param_t *params)
{
    ...
    if (params == NULL || params->group == NULL || strlen(params->group) == 0) {
        return NULL;
    }
...
int iotx_alcs_cloud_init(void *handle)
{
    ...
    if (alcs_mqtt_init(adapter->coap_ctx, product_key, device_name) !=
...
static void alcs_heartbeat(void *handle)
{
    ...
    if (adapter->role & IOTX_ALCS_ROLE_SERVER &&
        adapter->alcs_server_auth_timer_func != NULL) {
        adapter->alcs_server_auth_timer_func(adapter->coap_ctx);
    }

    if (adapter->role & IOTX_ALCS_ROLE_CLIENT &&
        adapter->alcs_client_auth_timer_func != NULL) {
...
int iotx_alcs_send(void *handle, iotx_alcs_msg_t *msg)
{
    ...
    if (adapter == NULL || adapter->coap_ctx || msg == NULL ||
        msg->payload == NULL || msg->ip == NULL || strlen(msg->ip) == 0 ||
    ...
    alcs_msg_init(adapter->coap_ctx, &coap_msg, msg->msg_code, msg->msg_type, 0,
                  &coap_payload, (void *)adapter);
    ...
    res = alcs_sendmsg_secure(adapter->coap_ctx, &devKey, &coap_msg, 2,
                              iotx_alcs_coap_adapter_send_msg_handle);
    ...
    alcs_send_msg = (iotx_alcs_send_msg_t *)ALCS_ADAPTER_malloc(
        sizeof(iotx_alcs_send_msg_t));
    ...
    alcs_send_msg->token =
        (uint8_t *)ALCS_ADAPTER_malloc(coap_msg.header.tokenlen + 1);
                            ...
                            uint8_t token_len, uint8_t *token)
{
    ...
    if (adapter == NULL || adapter->coap_ctx == NULL || msg == NULL ||
        msg->payload == NULL || msg->ip == NULL || strlen(msg->ip) == 0 ||
        msg->uri == NULL || strlen(msg->uri) == 0) {
        return NULL_VALUE_ERROR;
    }
    ...
    if (alcs_resource_need_auth(adapter->coap_ctx, msg->uri)) {
        res = alcs_sendrsp_secure(adapter->coap_ctx, &devKey, &coap_msg, 0, 0,
                                  &token_payload);
    } else {
        res = alcs_sendrsp(adapter->coap_ctx, &network_addr, &coap_msg, 0, 0,
                           &token_payload);
    }
...
int iotx_alcs_register_resource(void *handle, iotx_alcs_res_t *resource)
{
    ...
    if (adapter == NULL || adapter->coap_ctx == NULL || resource->uri == NULL ||
        strlen(resource->uri) == 0) {
        return NULL_VALUE_ERROR;
    }
    ...
    needAuth =
        resource
            ->need_auth; /* strcmp (resource->uri, "/dev/core/service/dev"); */

    res = alcs_resource_register(
        adapter->coap_ctx, productKey, deviceName, resource->uri,
        resource->msg_perm, resource->msg_ct, resource->maxage, needAuth,
...
int iotx_alcs_add_sub_device(void *handle, const char *pk, const char *dn)
{
    ...
    if (handle == NULL || pk == NULL ||
        strlen(pk) >= IOTX_PRODUCT_KEY_LEN + 1 || dn == NULL ||
    ...
    subdev_item = (iotx_alcs_subdev_item_t *)ALCS_ADAPTER_malloc(
    ...
    alcs_localsetup_add_sub_device(adapter, subdev_item->product_key,
                                   subdev_item->device_name);

    /* Get Prefix And Secret From KV */
    res = alcs_mqtt_prefix_secret_load(pk, strlen(pk), dn, strlen(dn), prefix,
                                       secret);
    if (res == SUCCESS_RETURN) {
