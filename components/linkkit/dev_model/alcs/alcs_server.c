...
void utils_hmac_sha1_base64(const char *msg, int msg_len, const char *key,
                            int key_len, char *digest, int *digest_len)
{
    char buf[20];
    uint32_t outlen;
    utils_hmac_sha1_hex(msg, msg_len, buf, key, key_len);

    utils_base64encode((unsigned char *)buf, 20, *digest_len,
                       (unsigned char *)digest, &outlen);
    *digest_len = outlen;
}

void alcs_rec_auth_select(CoAPContext *ctx, const char *paths,
                          NetworkAddr *from, CoAPMessage *resMsg)
{
    int seqlen, datalen;
    char *seq, *data;
    char *targetKey = "";
    ...
    do {
        if (!req_payload_parser((const char *)resMsg->payload,
                                resMsg->payloadlen, &seq, &seqlen, &data,
                                &datalen)) {
            break;
        }
        lst = get_list(ctx);
        ...
        accesskeys =
            json_get_value_by_name(data, datalen, "accessKeys", &keylen, NULL);
        ...
        json_array_for_each_entry(accesskeys, keylen, str_pos, entry, entry_len,
                                  type)
        {
            svr_key_item *node = NULL, *next = NULL;
            svr_group_item *gnode = NULL, *gnext = NULL;
            COAP_DEBUG("entry:%.*s", entry_len, entry);
            list_for_each_entry_safe(node, next, &lst->lst_svr, lst,
                                     svr_key_item)
            {
            ...
            list_for_each_entry_safe(gnode, gnext, &lst->lst_svr_group, lst,
                                     svr_group_item)
            {
    ...
    HAL_Snprintf(keybuf, sizeof(keybuf), "\"accessKey\":\"%.*s\"", targetLen,
                 targetKey);

    HAL_Snprintf(payloadbuf, sizeof(payloadbuf), RES_FORMAT, seqlen, seq,
                 targetKey ? 200 : COAP_MSG_CODE_401_UNAUTHORIZED, keybuf);
    payload.data = (unsigned char *)payloadbuf;
    payload.len = strlen(payloadbuf);

    alcs_msg_init(ctx, &msg, COAP_MSG_CODE_205_CONTENT, COAP_MESSAGE_TYPE_ACK,
                  0, &payload, NULL);
    alcs_sendrsp(ctx, from, &msg, 1, resMsg->header.msgid, &token);
}

svr_key_info *is_legal_key(CoAPContext *ctx, const char *keyprefix,
                           int prefixlen, const char *keyseq, int seqlen,
                           int *res_code)
{
    auth_list *lst = get_list(ctx);
    COAP_INFO("islegal prefix:%.*s, seq:%.*s", prefixlen, keyprefix, seqlen,
              keyseq);
        ...
        if (list_empty(&lst->lst_svr)) {
            ...
            *res_code = ALCS_AUTH_AUTHLISTEMPTY;
                                     ...
                                     svr_key_item)
            {
                ...
                if (strlen(node->keyInfo.keyprefix) == prefixlen &&
                    strncmp(keyprefix, node->keyInfo.keyprefix, prefixlen) ==
                        0) {
                                     ...
                                     svr_group_item)
            {
                ...
                if (strlen(gnode->keyInfo.keyprefix) == prefixlen &&
                    strncmp(keyprefix, gnode->keyInfo.keyprefix, prefixlen) ==
                        0) {
                    *res_code = ALCS_AUTH_OK;
                    HAL_MutexUnlock(lst->list_mutex);
...
void alcs_rec_auth(CoAPContext *ctx, const char *paths, NetworkAddr *from,
                   CoAPMessage *resMsg)
{
    int seqlen, datalen;
    char *seq, *data;
    ...
    CoAPLenString payload;
    CoAPLenString token;
    COAP_INFO("receive data:%.*s, from:%s", resMsg->payloadlen, resMsg->payload,
              from->addr);
    ...
    do {
        ...
        accesskey =
            json_get_value_by_name(data, datalen, "accessKey", &tmplen, NULL);
        ...
        item = is_legal_key(ctx, keyprefix, KEYPREFIX_LEN, keyseq, KEYSEQ_LEN,
                            &res_code);
        if (!item) {
        ...
        utils_hmac_sha1_base64(accesskey, tmplen, item->secret,
                               strlen(item->secret), accessToken, &tokenlen);

        COAP_INFO("accessToken:%.*s", tokenlen, accessToken);
        randomkey = json_get_value_by_name(data, datalen, "randomKey",
                                           &randomkeylen, NULL);
        ...
        utils_hmac_sha1_base64(randomkey, randomkeylen, accessToken, tokenlen,
                               buf, &calc_sign_len);

        COAP_INFO("calc randomKey:%.*s,token:%.*s,sign:%.*s", randomkeylen,
                  randomkey, tokenlen, accessToken, calc_sign_len, buf);

        sign = json_get_value_by_name(data, datalen, "sign", &tmplen, NULL);
        if (!sign || tmplen != calc_sign_len ||
            strncmp(sign, buf, calc_sign_len)) {
        ...
        if (!session) {
            ...
            CoAPPathMD5_sum(path, strlen(path), session->pk_dn,
                            PK_DN_CHECKSUM_LEN);

            memcpy(&session->addr, from, sizeof(NetworkAddr));
            COAP_INFO("new session, addr:%s, port:%d", session->addr.addr,
                      session->addr.port);
        ...
        HAL_Snprintf(buf, sizeof(buf), "%.*s%s", randomkeylen, randomkey,
                     session->randomKey);
        utils_hmac_sha1_hex(buf, strlen(buf), session->sessionKey, accessToken,
                            tokenlen);

        /*calc sign, save in buf*/
        calc_sign_len = sizeof(buf);
        utils_hmac_sha1_base64(session->randomKey, RANDOMKEY_LEN, accessToken,
                               tokenlen, buf, &calc_sign_len);
        HAL_Snprintf(body, sizeof(body),
                     "\"sign\":\"%.*s\",\"randomKey\":\"%s\",\"sessionId\":%d,"
                     "\"expire\":86400",
                     calc_sign_len, buf, session->randomKey,
                     session->sessionId);
    ...
    payload.len = strlen(payloadbuf);
    payload.data = (unsigned char *)payloadbuf;
    alcs_msg_init(ctx, &message, COAP_MSG_CODE_205_CONTENT,
                  COAP_MESSAGE_TYPE_ACK, 0, &payload, NULL);
    token.len = resMsg->header.tokenlen;
    token.data = resMsg->token;
    alcs_sendrsp(ctx, from, &message, 1, resMsg->header.msgid, &token);
}

static int alcs_remove_low_priority_key(CoAPContext *ctx,
...
static int add_svr_key(CoAPContext *ctx, const char *keyprefix,
                       const char *secret, bool isGroup,
                       ServerKeyPriority priority)
{
    auth_list *lst = get_list(ctx);
    svr_key_item *node = NULL, *next = NULL;
    ...
    if (!lst || lst->svr_count >= KEY_MAXCOUNT ||
        strlen(keyprefix) != KEYPREFIX_LEN) {
...
int alcs_add_svr_key(CoAPContext *ctx, const char *keyprefix,
                     const char *secret, ServerKeyPriority priority)
{
...
void send_err_rsp(CoAPContext *ctx, NetworkAddr *addr, int code,
                  CoAPMessage *request)
{
    CoAPMessage sendMsg;
    CoAPLenString payload = { 0 };
    CoAPLenString token;
    alcs_msg_init(ctx, &sendMsg, code, COAP_MESSAGE_TYPE_ACK, 0, &payload,
                  NULL);
...
void call_cb(CoAPContext *context, const char *path, NetworkAddr *remote,
             CoAPMessage *message, const char *key, char *buf,
             CoAPRecvMsgHandler cb)
{
    ...
    if (key && buf) {
        int len = alcs_decrypt((const char *)message->payload,
                               message->payloadlen, key, buf);
...
static secure_resource_cb_item *get_resource_by_path(const char *path)
{
    ...
    list_for_each_entry_safe(node, next, &secure_resource_cb_head, lst,
...
void recv_msg_handler(CoAPContext *context, const char *path,
                      NetworkAddr *remote, CoAPMessage *message)
{
    ...
    if (CoAPUintOption_get(message, COAP_OPTION_OBSERVE, &obsVal) ==
        COAP_SUCCESS) {
        if (obsVal == 0) {
    ...
    if (message->payloadlen < 256) {
        char buf[256];
        call_cb(context, path, remote, message, session->sessionKey, buf,
                node->cb);
    } else {
...
int alcs_resource_register_secure(CoAPContext *context, const char *pk,
                                  const char *dn, const char *path,
                                  ...
                                  CoAPRecvMsgHandler callback)
{
    ...
    item =
        (secure_resource_cb_item *)coap_malloc(sizeof(secure_resource_cb_item));
    if (item == NULL) {
        return -1;
    }
    ...
    list_for_each_entry_safe(node, next_node, &secure_resource_cb_head, lst,
                             secure_resource_cb_item)
    {
        ...
        } else if (item->path_type == PATH_FILTER &&
                   node->path_type == PATH_FILTER) {
    ...
    return CoAPResource_register(context, path, permission, ctype, maxage,
                                 &recv_msg_handler);
}
...
void alcs_resource_cb_deinit(void)
{
    ...
    list_for_each_entry(del_item, &secure_resource_cb_head, lst,
                        secure_resource_cb_item)
    {
        ...
        coap_free(del_item);
        del_item =
            list_entry(&secure_resource_cb_head, secure_resource_cb_item, lst);
    }
}
...
void alcs_auth_list_deinit(void)
{
    ...
    list_for_each_entry_safe(del_item, next_item, &auth_list_ctx->lst_svr, lst,
                             svr_key_item)
    {
        list_del(&del_item->lst);
    ...
    list_for_each_entry_safe(del_session_item, next_session_item,
                             &dev_lst->lst_svr_sessions, lst, session_item)
    {
...
void alcs_rec_heart_beat(CoAPContext *ctx, const char *path,
                         NetworkAddr *remote, CoAPMessage *request)
{
    ...
    if (!req_payload_parser((const char *)request->payload, request->payloadlen,
                            &seq, &seqlen, &data, &datalen)) {
        /* do nothing */
    }

    if (session) {
        HAL_Snprintf(databuf, sizeof(databuf), "\"delayTime\":%d",
                     default_heart_expire / 1000);
        HAL_Snprintf(payloadbuf, sizeof(payloadbuf), RES_FORMAT, seqlen, seq,
                     200, databuf);
    } else {
        HAL_Snprintf(payloadbuf, sizeof(payloadbuf), RES_FORMAT, seqlen, seq,
    ...
    payload.data = (unsigned char *)payloadbuf;
    payload.len = strlen(payloadbuf);
    alcs_msg_init(ctx, &msg, COAP_MSG_CODE_205_CONTENT, COAP_MESSAGE_TYPE_CON,
                  0, &payload, NULL);
    if (session) {
...
int observe_data_encrypt(CoAPContext *ctx, const char *path, NetworkAddr *from,
                         CoAPMessage *message, CoAPLenString *src,
                         CoAPLenString *dest)
{
    secure_resource_cb_item *node = get_resource_by_path(path);
    struct list_head *sessions;
    ...
    if (session) {
        ...
        alcs_encrypt((const char *)src->data, src->len, session->sessionKey,
                     dest->data);
