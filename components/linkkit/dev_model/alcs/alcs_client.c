             ...
             NetworkAddr *remote, CoAPMessage *message)
{
    ...
    if (COAP_RECV_RESP_TIMEOUT == result) {
        ...
        do {
            ...
            int keylen;
            char *accessKey = json_get_value_by_name(data, datalen, "accessKey",
                                                     &keylen, NULL);
            if (!accessKey || !keylen) {
                break;
            }
    ...
    if (!session) {
        ...
        if (msg.code == 200) {
            do {
                ...
                char buf[32];
                HAL_Snprintf(buf, sizeof(buf), "%s%.*s", session->randomKey,
                             tmplen, tmp);
                utils_hmac_sha1_hex(buf, strlen(buf), session->sessionKey,
            ...
            void *user_data, AuthHandler handler)
{
    ...
    utils_hmac_sha1_base64(session->randomKey, RANDOMKEY_LEN,
