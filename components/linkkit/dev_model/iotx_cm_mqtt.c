...
static int _mqtt_publish(iotx_cm_ext_params_t *params, const char *topic,
...
iotx_cm_connection_t *iotx_cm_open_mqtt(iotx_cm_init_param_t *params)
{
    ...
    _mqtt_conncection =
        (iotx_cm_connection_t *)cm_malloc(sizeof(iotx_cm_connection_t));
    if (_mqtt_conncection == NULL) {
    ...
    case IOTX_MQTT_EVENT_DISCONNECT:
        {
            ...
            if (_mqtt_conncection->event_handler) {
                _mqtt_conncection->event_handler(_mqtt_conncection->fd, &event,
                                                 _mqtt_conncection->cb_data);
            }
        }
        break;
    ...
    case IOTX_MQTT_EVENT_PUBLISH_RECEIVED:
        {
            ...
            iotx_cm_data_handle_cb topic_handle_func =
                (iotx_cm_data_handle_cb)pcontext;
#ifndef DEVICE_MODEL_ALINK2
            char *topic = NULL;
#endif
            if (topic_handle_func == NULL) {
                cm_warning("bypass %d bytes on [%.*s]", topic_info->payload_len,
                           topic_info->topic_len, topic_info->ptopic);
                return;
            }
#ifdef DEVICE_MODEL_ALINK2
            topic_handle_func(_mqtt_conncection->fd, topic_info->ptopic,
                              topic_info->topic_len, topic_info->payload,
            ...
            topic_handle_func(_mqtt_conncection->fd, topic, topic_info->payload,
                              topic_info->payload_len, NULL);
...
static int _mqtt_connect(uint32_t timeout)
{
    ...
    if (g_sdk_impl_ctx.mqtt_customzie_info[0] != '\0') {
        ((iotx_mqtt_param_t *)_mqtt_conncection->open_params)->customize_info =
            g_sdk_impl_ctx.mqtt_customzie_info;
    }
    if (g_sdk_impl_ctx.mqtt_port_num != 0) {
        ((iotx_mqtt_param_t *)_mqtt_conncection->open_params)->port =
            g_sdk_impl_ctx.mqtt_port_num;
    }

    do {
        pclient = IOT_MQTT_Construct(
            (iotx_mqtt_param_t *)_mqtt_conncection->open_params);
        if (pclient != NULL) {
            iotx_cm_event_msg_t event;
...
static int _mqtt_publish(iotx_cm_ext_params_t *ext, const char *topic,
                         const char *payload, unsigned int payload_len)
{
    int qos = 0;
    ...
    return IOT_MQTT_Publish_Simple(_mqtt_conncection->context, topic, qos,
                     ...
                     iotx_cm_data_handle_cb topic_handle_func, void *pcontext)
{
    ...
    if (_mqtt_conncection == NULL || topic == NULL ||
    ...
    if (sync != 0) {
        ret = IOT_MQTT_Subscribe_Sync(_mqtt_conncection->context, topic, qos,
                                      iotx_cloud_conn_mqtt_event_handle,
                                      (void *)topic_handle_func, timeout);
    } else {
        ret = IOT_MQTT_Subscribe(_mqtt_conncection->context, topic, qos,
                                 iotx_cloud_conn_mqtt_event_handle,
