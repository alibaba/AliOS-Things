 ...
 * @retval  0 : sucess
 * @note
 *      if awss_stop is called before exit of awss_start, awss and notify will
 * stop.
 */
int awss_stop(void);
 ...
 * @retval  0 : sucess
 * @note
 *      check reset flag in perisistent storage, if device failed to report
 * reset message last time, retry it.
 */
 ...
 * @retval  0 : sucess
 * @note
 *      device will save reset flag if device dosen't connect cloud, device will
 * fails to send reset to cloud. when connection between device and cloud is
 * ready, device will retry to report reset to cloud.
 */
 ...
 */
int awss_connect(char ssid[HAL_MAX_SSID_LEN], char passwd[HAL_MAX_PASSWD_LEN],
                 uint8_t *bssid, uint8_t bssid_len, uint8_t *token,
                 uint8_t token_len);
