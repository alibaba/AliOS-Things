...
#define LITE_calloc(num, size, ...) \
    LITE_malloc_internal(__func__, __LINE__, (num * size), ##__VA_ARGS__)
#define LITE_malloc(size, ...) \
    LITE_malloc_internal(__func__, __LINE__, size, ##__VA_ARGS__)
#define LITE_realloc(ptr, size, ...) \
    LITE_realloc_internal(__func__, __LINE__, ptr, size, ##__VA_ARGS__)
#define LITE_free(ptr)                                                       \
    do {                                                                     \
        if (!ptr) {                                                          \
            log_warning("utils", "%s == NULL! LITE_free(%s) aborted.", #ptr, \
                        #ptr);                                               \
            break;                                                           \
        }                                                                    \
                                                                             \
