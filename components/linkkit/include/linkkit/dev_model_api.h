 ...
 *
 * @param dev_type. type of device which will be created. see
 * iotx_linkkit_dev_type_t
 * @param meta_info. The product key, product secret, device name and device
 * secret of new device.
 *
 ...
 *        for slave device, send message to cloud for register new device and
 * add topo with master device
 *
...
/**
 * @brief try to receive message from cloud and dispatch these message to user
 * event callback
 *
 ...
 *        for master device, disconnect with aliyun server and release all local
 * resources. for slave device, send message to cloud for delete topo with
 * master device and unregister itself, then release device's resources.
 *
 * @param devid. device identifier.
 *
