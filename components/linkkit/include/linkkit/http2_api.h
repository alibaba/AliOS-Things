...
typedef void (*on_stream_header_callback)(uint32_t stream_id, char *channel_id,
                                          int cat, const uint8_t *name,
                                          uint32_t namelen,
                                          const uint8_t *value,
                                          uint32_t valuelen, uint8_t flags,
                                          void *user_data);

typedef void (*on_stream_chunk_recv_callback)(uint32_t stream_id,
                                              char *channel_id,
                                              const uint8_t *data, uint32_t len,
                                              uint8_t flags, void *user_data);

typedef void (*on_stream_close_callback)(uint32_t stream_id, char *channel_id,
                                         uint32_t error_code, void *user_data);

typedef void (*on_stream_frame_send_callback)(uint32_t stream_id,
                                              char *channel_id, int type,
                                              uint8_t flags, void *user_data);

typedef void (*on_stream_frame_recv_callback)(uint32_t stream_id,
                                              char *channel_id, int type,
                                              uint8_t flags, void *user_data);
...
typedef struct {
    ...
    char *
        channel_id;  /* string return by server to identify a specific stream
                     channel,  different from stream identifier which is a field
                     in HTTP2 frame */
...
int IOT_HTTP2_Stream_Open(void *handle, stream_data_info_t *info,
                          header_ext_info_t *header);
int IOT_HTTP2_Stream_Send(void *handle, stream_data_info_t *info,
                          header_ext_info_t *header);
int IOT_HTTP2_Stream_Query(void *handle, stream_data_info_t *info,
                           header_ext_info_t *header);
int IOT_HTTP2_Stream_Close(void *handle, stream_data_info_t *info);
int IOT_HTTP2_Stream_Send_Message(void *handle, const char *identify,
