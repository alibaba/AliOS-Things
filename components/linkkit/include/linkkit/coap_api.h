...
typedef enum {
    ...
    IOTX_COAP_RESP_CODE_UNAUTHORIZED =
        0x81, /* Mapping to 4.01, Token is invalid or expire*/
    IOTX_COAP_RESP_CODE_NOT_FOUND =
        0x84, /* Mapping to 4.04, Path or uri is not found*/
    IOTX_COAP_RESP_CODE_URL_TOO_LONG =
        0x8E, /* Mapping to 4.14, The request url is too long*/
    IOTX_COAP_RESP_CODE_INTERNAL_SERVER_ERROR =
        0xA0, /* Mapping to 5.00, Internal server error*/

} iotx_coap_resp_code_t;

/* Callback function to notify the application events.*/
typedef void (*iotx_event_handle_t)(void *context, iotx_coap_event_t event,
                                    void *p_data);
 ...
 *
 * @param  [in] p_message: Pointer to the message to get the payload. Should not
 *be NULL.
 ...
 * @retval IOTX_ERR_INVALID_PARAM    : Can't get the payload due to invalid
 *parameter.
 * @see iotx_ret_code_t.
 **/
 ...
 *
 * @param [in] p_message: Pointer to the message to add the address information
...
int IOT_CoAP_GetMessageCode(void *p_message,
                            iotx_coap_resp_code_t *p_resp_code);
