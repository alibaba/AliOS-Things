...
typedef enum {
    ...
    /* No ACK to the specific subscribe which specify by packet-id be received
     * in timeout period */
    IOTX_MQTT_EVENT_SUBCRIBE_TIMEOUT = 4,

    /* A failed ACK to the specific subscribe which specify by packet-id be
     * received*/
    ...
    /* No ACK to the specific unsubscribe which specify by packet-id be received
     * in timeout period */
    IOTX_MQTT_EVENT_UNSUBCRIBE_TIMEOUT = 7,

    /* A failed ACK to the specific unsubscribe which specify by packet-id be
     * received*/
    ...
    /* No ACK to the specific publish which specify by packet-id be received in
     * timeout period */
    IOTX_MQTT_EVENT_PUBLISH_TIMEOUT = 10,

    /* A failed ACK to the specific publish which specify by packet-id be
     * received*/
    ...
    /* MQTT packet buffer overflow which the remaining space less than to
     * receive byte */
...
typedef struct {
    ...
    /*
     * Specify the detail event information. @msg means different to different
     * event types:
     *
     ...
     *      Its data type is @iotx_mqtt_topic_info_pt and see detail at the
     * declare of this type.
     *
     * */
    void *msg;
 ...
 * @return none
 */
typedef void (*iotx_mqtt_event_handle_func_fpt)(void *pcontext, void *pclient,
...
typedef struct {
    ...
    uint32_t request_timeout_ms;    /* Specify timeout of a MQTT request in
                                       millisecond */
    uint32_t keepalive_interval_ms; /* Specify MQTT keep-alive interval in
                                       millisecond */
 ...
 * @param [in] pcontext: specify context. When call 'topic_handle_func', it will
 be passed back.
 *
 * @retval -1  : Subscribe failed.
 * @retval >=0 : Subscribe successful.
          The value is a unique ID of this request.
          The ID will be passed back when callback
 'iotx_mqtt_param_t:handle_event'.
 * @see None.
 */
...
/**
 * @brief Only used in async network stack and FEATURE_ASYNC_PROTOCOL_STACK must
 * be selected
 *
 * @param [in] handle: specify the MQTT client.
...
/* Default maximum length of topic name in byte when PLATFORM_HAS_DYNMEM is not
 * set */
