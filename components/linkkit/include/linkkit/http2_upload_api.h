...
typedef struct {
    const char *file_path; /* file path, filename must be ASCII string and
                              strlen < 2014 */
    uint32_t part_len; /* maximum content len of one http2 request, must be in
                          range of 100KB ~ 100MB */
    const char
        *upload_id; /* a specific id used to indicate one upload session, only
                       required when UPLOAD_FILE_OPT_BIT_RESUME option set */
    uint32_t upload_len; /* used to indicate the upload length, only required
                            when UPLOAD_FILE_OPT_BIT_SPECIFIC_LEN option set */
    uint32_t
        opt_bit_map; /* option bit map, support UPLOAD_FILE_OPT_BIT_OVERWRITE,
                        UPLOAD_FILE_OPT_BIT_RESUME and
                        UPLOAD_FILE_OPT_BIT_SPECIFIC_LEN */
...
/* gerneral callback function, this callback will be invoke when http2
 * disconnected */
...
/* callback function type define, this callback will be invoke when upload
 * completed */
typedef void (*http2_upload_completed_cb_t)(const char *file_path, int result,
                                            void *user_data);

/* callback funciton type define, this callback will be invoke when upload_id
 * received */
typedef void (*http2_upload_id_received_cb_t)(const char *file_path,
