 ...
 *
 * @param[in]   uart         the UART interface
 * @param[out]  data         pointer to the buffer which will store incoming
 * data
...
int HAL_AT_MQTT_Subscribe(const char *topic, int qos,
                          unsigned int *mqtt_packet_id, int *mqtt_status,
                          int timeout_ms);
int HAL_AT_MQTT_Unsubscribe(const char *topic, unsigned int *mqtt_packet_id,
                            int *mqtt_status);
int HAL_AT_MQTT_Publish(const char *topic, int qos, const char *message,
