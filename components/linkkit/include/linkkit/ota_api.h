...
/**
 * @brief Deinitialize OTA module specified by the 'handle', and release the
 * related resource. You must call this interface to release resource if reboot
 * is not invoked after downloading.
 *
 ...
 *        NOTE: If you want to download more faster, the bigger 'buf' should be
 * given.
 *
 * @param [in] handle: specify the OTA module.
 ...
 * @retval (0, len] : The length of data be downloaded in 'timeout_s' timeout
 * period in bytes.
 * @see None.
 */
 ...
 *        By this interface, you can get information like state, size of file,
 md5 of file, etc.
 *
 ...
 * @return
 ...
 uint32_t, and 'buf_len' should be 4. 2) When type is IOT_OTAG_FILE_SIZE, 'buf'
 should be pointer of uint32_t, and 'buf_len' should be 4. 3) When type is
 IOT_OTAG_MD5SUM, 'buf' should be a buffer, and 'buf_len' should be 33. 4) When
 type is IOT_OTAG_VERSION, 'buf' should be a buffer, and 'buf_len' should be
 OTA_VERSION_LEN_MAX. 5) When type is IOT_OTAG_CHECK_FIRMWARE, 'buf' should be
 pointer of uint32_t, and 'buf_len' should be 4. 0, firmware is invalid; 1,
 firmware is valid.
  @endverbatim
 *
 * @retval   0 : Successful.
