...
typedef struct {
    ...
    int length;
    int offset;
    int depth; /* How deeply nested (in arrays/objects) is the input at the
                  current offset. */
} parse_buffer;
...
static int parse_value(lite_cjson_t *const item,
                       parse_buffer * const input_buffer);
static int parse_string(lite_cjson_t *const item,
                        parse_buffer * const input_buffer);
static int parse_array(lite_cjson_t *const item,
                       parse_buffer * const input_buffer);
static int parse_object(lite_cjson_t *const item,
                        parse_buffer * const input_buffer);
 ...
 */
static int parse_number(lite_cjson_t *const item,
                        parse_buffer *const input_buffer)
{
    double number = 0;
                       ...
                       parse_buffer *const input_buffer)
{
    ...
    do {
         ...
         */
        /* item->size+1, current_item.type, current_item.value_length,
         * current_item.value_length, current_item.value); */
                        ...
                        parse_buffer *const input_buffer)
{
    ...
    do {
        ...
        /* printf("Current Object [Index: %d], [Key Length: %d, Key Value:
         * %.*s], [Value Length: %d, Value: %.*s]\n", */
                        ...
                        parse_buffer *const input_buffer)
{
...

    {
        ...
        int skipped_bytes = 0;
        while (
            ((int)(input_end - input_buffer->content) < input_buffer->length) &&
            (*input_end != '\"')) {
            /* is escape sequence */
            if (input_end[0] == '\\') {
                if ((int)(input_end + 1 - input_buffer->content) >=
                    input_buffer->length) {
        ...
        if (((int)(input_end - input_buffer->content) >=
             input_buffer->length) ||
            (*input_end != '\"')) {
            /* printf("end error\n"); */
...
static int parse_value(lite_cjson_t *const lite,
                       parse_buffer *const input_buffer)
{
    if ((input_buffer == NULL) || (input_buffer->content == NULL)) {
        return -1; /* no input */
...
int lite_cjson_parse(const char *src, int src_len, lite_cjson_t *lite)
{
    ...
    if (parse_value(lite, buffer_skip_whitespace(skip_utf8_bom(&buffer))) !=
        0) {
...
int lite_cjson_array_item(lite_cjson_t *lite, int index,
                          lite_cjson_t *lite_item)
{
    ...
    if (!lite || lite->type != cJSON_Array || !lite->value || index < 0 ||
        index >= lite->size || !lite_item) {
        return -1;
    }
    ...
    if (can_access_at_index(p_buffer, 0) &&
        (buffer_at_offset(p_buffer)[0] == ']')) {
        /* empty array */
        return -1;
    }
    ...
    do {
        ...
        /* printf("current item index: %d, type: %d, length: %d, value: %.*s\n",
         */
        /* iter_index+1, current_item.type, current_item.value_length,
         * current_item.value_length, current_item.value); */
        ...
        iter_index++;
    } while (can_access_at_index(p_buffer, 0) &&
             (buffer_at_offset(p_buffer)[0] == ','));

    return -1;
}

static int _lite_cjson_object_item(lite_cjson_t *lite, const char *key,
                                   int key_len, lite_cjson_t *lite_item)
{
    parse_buffer buffer;
    parse_buffer *p_buffer = &buffer;
    lite_cjson_t current_item_key;
    ...
    if (!lite || lite->type != cJSON_Object || !lite->value ||
        lite->size == 0 || !key || key_len <= 0 || !lite_item) {
        return -1;
    };
    ...
    do {
         ...
         * %.*s], [Value Length: %d, Value: %.*s]\n", */
        /* index + 1,
         * current_item_key.value_length,current_item_key.value_length,current_item_key.value,
         */
        /* current_item_value.value_length,current_item_value.value_length,current_item_value.value);
         */
        index++;
...
static int _lite_cjson_key_array_index(const char *key, int key_len,
                                       int *partial_key_len, int *array_key_len,
                                       int *array_index)
{
                           ...
                           lite_cjson_t *lite_item)
{
    ...
    do {
        ...
        if (delim != NULL) {
            ...
            /* printf("key: %s, last key: %s, key len: %d, last key len:
             * %d\n",key, key_iter, key_len, key_iter_len); */
        }

        if (_lite_cjson_key_array_index(key_iter, key_iter_len,
                                        &partial_key_len, &array_key_len,
                                        &array_index) == 0) {
            /* printf("partial_key_len: %d, array_key_len: %d, array_index:
             * %d\n", partial_key_len, array_key_len, array_index); */

            res = _lite_cjson_object_item(&lite_iter, key_iter, partial_key_len,
                                          &lite_prev);
            ...
            res = _lite_cjson_object_item(&lite_iter, key_iter, key_iter_len,
                                          &lite_prev);
            if (res) {
                return -1;
            }
...
int lite_cjson_object_item_by_index(lite_cjson_t *lite, int index,
                                    ...
                                    lite_cjson_t *lite_item_value)
{
    ...
    if (!lite || lite->type != cJSON_Object || !lite->value ||
        lite->size == 0 || index < 0 || index >= lite->size) {
        return -1;
    };
    ...
    do {
        ...
        if (item_index == index) {
            ...
            if (lite_item_value) {
                memcpy(lite_item_value, &current_item_value,
                       sizeof(lite_cjson_t));
            }
            return 0;
        }
...
static cJSON_bool print_value(const lite_cjson_item_t *const item,
...
static cJSON_bool print_number(const lite_cjson_item_t *const item,
                               printbuffer * const output_buffer)
{
    unsigned char *output_pointer = NULL;
    ...
    if ((d * 0) != 0) {
        ...
        if ((sscanf((char *)number_buffer, "%f", &test_float) != 1) ||
            ((double)test_float != d)) {
...
static cJSON_bool print_string(const lite_cjson_item_t *const item,
                               printbuffer * const p)
{
...
static cJSON_bool print_array(const lite_cjson_item_t *const item,
                              printbuffer * const output_buffer)
{
    unsigned char *output_pointer = NULL;
    size_t length = 0;
...
static cJSON_bool print_object(const lite_cjson_item_t *const item,
                               printbuffer * const output_buffer)
{
    unsigned char *output_pointer = NULL;
    size_t length = 0;
...
static unsigned char *print(const lite_cjson_item_t *const item,
                            cJSON_bool format,
...
static cJSON_bool add_item_to_array(lite_cjson_item_t *array,
                                    lite_cjson_item_t *item)
{
...
void lite_cjson_add_item_to_array(lite_cjson_item_t *array,
                                  lite_cjson_item_t *item)
{
...
static cJSON_bool add_item_to_object(lite_cjson_item_t *const object,
                                     const char *const string,
                                     lite_cjson_item_t *const item,
                                     const internal_hooks *const hooks,
...
void lite_cjson_add_item_to_object(lite_cjson_item_t *object,
                                   const char *string, lite_cjson_item_t *item)
{
...
{
    lite_cjson_item_t *node =
        (lite_cjson_item_t *)hooks->allocate(sizeof(lite_cjson_item_t));
    if (node) {
...
lite_cjson_item_t *lite_cjson_create_doubleArray(const double *numbers,
                                                 int count)
{
    size_t i = 0;
    lite_cjson_item_t *n = NULL;
...
lite_cjson_item_t *lite_cjson_create_stringArray(const char **strings,
                                                 int count)
{
    size_t i = 0;
    lite_cjson_item_t *n = NULL;
