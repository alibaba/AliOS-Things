...
static int _mem_cmp_max_used(list_head_t *a, list_head_t *b)
{
    ...
    return (temp_a->mem_statis.bytes_max_in_use <
            temp_b->mem_statis.bytes_max_in_use);
}
...
static void _mem_sort_module_pos(list_head_t *head)
{
    ...
    for (end = head->prev; end != head;
         end = end->prev) { /* list_for_each_prev */
...
void *_create_mem_table(char *module_name, struct list_head *list_head)
{
    ...
    memcpy(pos->mem_statis.module_name, module_name,
           (len >= sizeof(pos->mem_statis.module_name))
               ? (sizeof(pos->mem_statis.module_name) - 1)
               : len);
...
int _del_mem_table(char *module_name, struct list_head *list_head)
{
    ...
    module_mem_t *table_pos =
        (module_mem_t *)_find_mem_table(module_name, list_head);
    if (table_pos) {
        list_for_each_entry_safe(calling_stack_pos, tmp,
                                 &table_pos->mem_statis.calling_stack.func_head,
                                 func_head, calling_stack_t)
        {
...
int _count_malloc_internal(const char *f, const int l,
                           OS_malloc_record *os_malloc_pos, va_list ap)
{
    int ret = -1;
    ...
    if (!pos) {
        pos = (module_mem_t *)_create_mem_table(module_name, &mem_module_statis);
        if (pos == NULL) {
    ...
    if (!strcmp(pos->mem_statis.module_name, "unknown")) {
        list_for_each_entry_safe(call_pos, call_tmp,
                                 &pos->mem_statis.calling_stack.func_head,
                                 func_head, calling_stack_t)
        {
    ...
    pos->mem_statis.bytes_max_in_use =
        (pos->mem_statis.bytes_max_in_use > pos->mem_statis.bytes_total_in_use)
            ? pos->mem_statis.bytes_max_in_use
            : pos->mem_statis.bytes_total_in_use;
    pos->mem_statis.bytes_max_allocated =
        (pos->mem_statis.bytes_max_allocated >= os_malloc_pos->buflen)
            ? pos->mem_statis.bytes_max_allocated
            : os_malloc_pos->buflen;

    pos->mem_statis.iterations_in_use += 1;
    pos->mem_statis.iterations_max_in_use =
        (pos->mem_statis.iterations_in_use >
         pos->mem_statis.iterations_max_in_use)
            ? pos->mem_statis.iterations_in_use
            : pos->mem_statis.iterations_max_in_use;
    ret = 0;
...
void *LITE_malloc_internal(const char *f, const int l, int size, ...)
{
    ...
    bytes_max_in_use = (bytes_max_in_use > bytes_total_in_use)
                           ? bytes_max_in_use
                           : bytes_total_in_use;
    bytes_max_allocated =
        (bytes_max_allocated >= size) ? bytes_max_allocated : size;

#if defined(WITH_TOTAL_COST_WARNING)
    if (bytes_total_in_use > WITH_TOTAL_COST_WARNING) {
        utils_debug(" ");
        utils_debug("==== PRETTY HIGH TOTAL IN USE: %d BYTES ====",
                    bytes_total_in_use);
    ...
    iterations_in_use += 1;
    iterations_max_in_use = (iterations_in_use > iterations_max_in_use)
                                ? iterations_in_use
    ...
    if (size > WITH_ALLOC_WARNING_THRESHOLD) {
        ...
        log_warning("utils", "large allocating @ %s(%d) for %04d bytes!", f, l,
                    size);
        LITE_printf("\r\n");
#if defined(__UBUNTU_SDK_DEMO__)
...
void LITE_dump_malloc_free_stats(int level)
{
    ...
    LITE_printf("\r\n");
    LITE_printf(
        "|               |  max_in_use          |  max_allocated   |  "
        "total_allocated      |  total_free\r\n");
    LITE_printf(
        "|---------------|----------------------|------------------|-----------"
        "------------|----------------------\r\n");
    list_for_each_entry_safe(module_pos, tmp, &mem_module_statis, list,
                             module_mem_t)
    {
        if (module_pos) {
            LITE_printf(
                "| %-13s | %6d bytes / %-5d |    %6d bytes  | %6d bytes / %-5d "
                " | %6d bytes / %-5d     \r\n",
            ...
            /*
               ...
               module_pos->mem_statis.bytes_total_allocated); LITE_printf(".
               bytes_total_freed:        %d\r\n",
               module_pos->mem_statis.bytes_total_freed); LITE_printf(".
               bytes_total_in_use:       %d\r\n",
               module_pos->mem_statis.bytes_total_in_use); LITE_printf(".
               bytes_max_allocated:      %d\r\n",
               module_pos->mem_statis.bytes_max_allocated); LITE_printf(".
               bytes_max_in_use:         %d\r\n",
               module_pos->mem_statis.bytes_max_in_use); LITE_printf(".
               iterations_allocated:     %d\r\n",
               module_pos->mem_statis.iterations_allocated); LITE_printf(".
               iterations_freed:         %d\r\n",
               module_pos->mem_statis.iterations_freed); LITE_printf(".
               iterations_in_use:        %d\r\n",
               module_pos->mem_statis.iterations_in_use); LITE_printf(".
               iterations_max_in_use:    %d\r\n",
               module_pos->mem_statis.iterations_max_in_use);
                        LITE_printf("---------------------------------------------------\r\n");
            */
    ...
    if (unknown_mod) {
        ...
        list_for_each_entry_safe(
            call_pos, tmp, &unknown_mod->mem_statis.calling_stack.func_head,
            func_head, calling_stack_t)
        {
    ...
    if (LITE_get_loglevel() == level) {
        ...
        {
            if (pos->buf) {
                LITE_printf("%4d. %-24s Ln:%-5d @ %p: %4d bytes [", ++cnt,
                            pos->func, pos->line, pos->buf, pos->buflen);
