...
char *json_get_next_object(int type, char *str, char *str_end, char **key,
                           ...
                           int *val_type)
{
    ...
    while (p_cPos && *p_cPos && p_cPos < str_end && iValueType > JNONE) {
        if (iValueType == JBOOLEAN) {
            ...
            if ((*p_cPos < '0' || *p_cPos > '9') && (*p_cPos != '.') &&
                (*p_cPos != '+') && (*p_cPos != '-') && ((*p_cPos != 'e')) &&
                (*p_cPos != 'E')) {
                          ...
                          void *p_CBData)
{
    ...
    json_object_for_each_kv(p_cJsonStr, iStrLen, pos, key, klen, val, vlen,
                            vtype)
    {
        if (key && klen && val && vlen) {
            ret = JSON_RESULT_OK;
            if (JSON_PARSE_FINISH ==
                pfnCB(key, klen, val, vlen, vtype, p_CBData)) {
                break;
            }
        }
    }
                              ...
                              int iValueLen, int iValueType, void *p_CBData)
{
    ...
    if ((iNameLen == p_stNameValue->nLen) &&
        !strncmp(p_cName, p_stNameValue->pN, p_stNameValue->nLen)) {
...
char *json_get_value_by_name_len(char *p_cJsonStr, int iStrLen, char *p_cName,
                                 int p_cNameLen, int *p_iValueLen,
                                 int *p_iValueType)
{
    JSON_NV stNV;
...
char *LITE_json_value_of(char *key, char *src, ...)
{
    ...
    do {
        ...
        if (delim != NULL) {
            ...
            value = json_get_value_by_name_len(src_iter, src_iter_len, key_next,
                                               key_len, &value_len, 0);
            if (value == NULL) {
                return NULL;
            }
                                       ...
                                       ...)
{
    ...
    json_object_for_each_kv(src, src_len, pos, key, klen, val, vlen, vtype)
    {
        if (key && klen && val && vlen) {
            ...
            if (module_name) {
                entry->key = LITE_format_nstring(
                    mlen, "%s%.*s", magic, module_name, prefix, klen, key);
            } else {
                entry->key =
                    LITE_format_nstring(mlen, "%s%.*s", prefix, klen, key);
            }
            if (NULL == entry->key) {
                jparser_free(entry);
                return NULL;
...
static list_head_t *_LITE_json_keys_of_ext(int type, char *src, int src_len,
                                           char *prefix, ...)
{
    static LIST_HEAD(keylist);
    char *module_name = NULL;
    ...
    if (JOBJECT == type) {
        json_object_for_each_kv(src, src_len, pos, key, klen, val, vlen, vtype)
        {
            if (key && klen && val && vlen) {
                ...
                if (JOBJECT == vtype) {
                    ...
                    if (module_name) {
                        iter_pre =
                            LITE_format_nstring(mlen, "%s%.*s.", magic,
                                                module_name, prefix, klen, key);
                    } else {
                        iter_pre = LITE_format_nstring(mlen, "%s%.*s.", prefix,
                                                       klen, key);
                    }
                    if (NULL == iter_pre) {
                        return NULL;
                    ...
                    _LITE_json_keys_of_ext(vtype, val, vlen, iter_pre, magic,
                                           module_name);
                    jparser_free(iter_pre);
        ...
        json_array_for_each_entry(src, src_len, pos, val, vlen, vtype)
        {
            if (val && vlen) {
                ...
                if (module_name) {
                    entry->key = LITE_format_nstring(
                        mlen, "%s[%d]", magic, module_name, prefix, count);
                } else {
                    entry->key =
                        LITE_format_nstring(mlen, "%s[%d]", prefix, count);
                }
                if (NULL == entry->key) {
                    jparser_free(entry);
                    return NULL;
...
static char *_json_value_by_arrname(char *src, int src_len, const char *key,
...
list_head_t *LITE_json_keys_of_ext(char *src, char *prefix, ...)
{
    ...
    return _LITE_json_keys_of_ext(JOBJECT, src, strlen(src), prefix, magic,
                                  module_name);
}

static char *_LITE_json_value_of_ext(char *key, char *src, int src_len,
                                     int *val_len)
{
    char *value = NULL;
    char *delim = NULL;
    ...
    do {
        ...
        if (delim != NULL) {
            ...
            if (0 == contain_arr(key_next, key_len, &arr_pre)) {
                ...
                if (obj_key_len) {
                    value = json_get_value_by_name_len(src_iter, src_iter_len,
                                                       key_next, obj_key_len,
                                                       &value_len, 0);
                } else {
                    value = src_iter;
                }
            } else {
                value = json_get_value_by_name_len(
                    src_iter, src_iter_len, key_next, key_len, &value_len, 0);
            }
            ...
            if (key_type == JARRAY) {
                value = _json_value_by_arrname(
                                 value, value_len, arr_pre,
                                 key_len - obj_key_len, &value_len);
                if (value == NULL) {
                    return NULL;
                }
            }
    ...
    if (key_type == JARRAY) {
        value = _json_value_by_arrname(value, value_len, arr_pre,
                                       key_len - obj_key_len,
                                       &value_len);
        if (value == NULL) {
            return NULL;
