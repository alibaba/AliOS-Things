...
{
    volatile unsigned char *p = (unsigned char *)v; // avoid be optimized by compiler
                         ...
                         unsigned int keybits)
{
    ...
    case 256:
        ...
        break;
    default:
        return INFRA_ERR_AES_INVALID_KEY_LENGTH;
    }
                         ...
                         unsigned int keybits)
{
    ...
    /* Also checks keybits */
    ret = infra_aes_setkey_enc(&cty, key, keybits);
    if (ret != 0)
        goto exit;
 ...
 */
int infra_aes_crypt_ecb(infra_aes_context *ctx, int mode,
 ...
 */
int infra_aes_crypt_cbc(infra_aes_context *ctx, int mode, size_t length,
...
#if defined(INFRA_PADLOCK_C) && defined(INFRA_HAVE_X86)
    if (aes_padlock_ace) {
        if (infra_aes_padlock_xcryptcbc(ctx, mode, length, iv, input, output) ==
            0)
            return 0;
 ...
 */
int infra_aes_crypt_cfb128(infra_aes_context *ctx, int mode, size_t length,
...
p_Aes128_t infra_aes128_init(const uint8_t *key, const uint8_t *iv,
                             AES_DIR_t dir)
{
    int ret = 0;
...
int infra_aes128_cbc_decrypt(p_Aes128_t aes, const void *src, size_t blockNum,
                             void *dst)
{
    int i = 0;
    int ret = -1;
...
#if defined(INFRA_AES_BUILDIN)
        ret = infra_aes_crypt_cbc(&p_aes128->ctx, INFRA_AES_DECRYPT,
                                  AES_BLOCK_SIZE, p_aes128->iv, src, dst);
#else
        ret = mbedtls_aes_crypt_cbc(&p_aes128->ctx, MBEDTLS_AES_DECRYPT,
                                    AES_BLOCK_SIZE, p_aes128->iv, src, dst);
#endif
...
int infra_aes128_cfb_decrypt(p_Aes128_t aes, const void *src, size_t length,
                             void *dst)
{
    size_t offset = 0;
    int ret = -1;
...
int infra_aes128_cfb_encrypt(p_Aes128_t aes, const void *src, size_t length,
                             void *dst)
{
    size_t offset = 0;
    int ret = -1;
...
int infra_aes128_cbc_encrypt(p_Aes128_t aes, const void *src, size_t blockNum,
                             void *dst)
{
    int i = 0;
    int ret = ret;
    ...
    for (i = 0; i < blockNum; ++i) {
#if defined(INFRA_AES_BUILDIN)
        ret = infra_aes_crypt_cbc(&p_aes128->ctx, INFRA_AES_ENCRYPT,
                                  AES_BLOCK_SIZE, p_aes128->iv, src, dst);
#else
        ret = mbedtls_aes_crypt_cbc(&p_aes128->ctx, MBEDTLS_AES_ENCRYPT,
                                    AES_BLOCK_SIZE, p_aes128->iv, src, dst);
#endif
        src = (unsigned char *)src + 16;
        dst = (unsigned char *)dst + 16;
