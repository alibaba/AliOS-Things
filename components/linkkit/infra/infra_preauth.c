...
int _preauth_assemble_auth_req_string(const iotx_dev_meta_info_t *dev_meta,
                                      ...
                                      char *request_buff, uint32_t buff_len)
{
    uint8_t i = 0;
    const char *kv[][2] = {
        { "productKey", NULL },           { "deviceName", NULL },
        { "signmethod", "hmacsha256" },   { "sign", NULL },
        { "version", "default" },         { "clientId", NULL },
        { "timestamp", "2524608000000" }, { "resources", "mqtt" }
    };

    if (dev_meta == NULL || sign == NULL || device_id == NULL ||
        request_buff == NULL) {
        return FAIL_RETURN;
    }
...
static int _preauth_get_string_value(char *p_string, char *value_buff,
                                     uint32_t buff_len)
{
    char *p = p_string;
    char *p_start = NULL;
...
static int _preauth_parse_auth_rsp_string(char *json_string,
                                          uint32_t string_len,
                                          iotx_sign_mqtt_t *output)
{
    int res = FAIL_RETURN;
    char *p = json_string;
    ...
    while (p < (json_string + string_len)) {
        ...
        if (strlen("code") == len && !memcmp(p_start, "code", len)) {
            ...
            res = _preauth_get_string_value(p, output->username,
                                            PREAUTH_IOT_ID_MAXLEN);
            if (res < SUCCESS_RETURN) {
                return res;
            }
        } else if (strlen("iotToken") == len &&
                   !memcmp(p_start, "iotToken", len)) {
            res = _preauth_get_string_value(p, output->password,
                                            PREAUTH_IOT_TOKEN_MAXLEN);
            if (res < SUCCESS_RETURN) {
                return res;
            }
        } else if (strlen("host") == len && !memcmp(p_start, "host", len)) {
            res = _preauth_get_string_value(p, output->hostname,
                                            PREAUTH_IOT_HOST_MAXLEN);
            if (res < SUCCESS_RETURN) {
                return res;
            }
...
static int _preauth_recv_callback(char *ptr, int length, int total_length,
                                  void *userdata)
{
                                ...
                                iotx_sign_mqtt_t *preauth_output)
{
    ...
    memcpy(http_url + strlen(http_url), g_infra_http_domain[region],
           strlen(g_infra_http_domain[region]));
    memcpy(http_url + strlen(http_url), http_url_frag, sizeof(http_url_frag));

    _preauth_assemble_auth_req_string(dev_meta, sign, device_id, request_buff,
                                      sizeof(request_buff));
...
#endif
    res = _preauth_parse_auth_rsp_string(response_buff, strlen(response_buff),
                                         preauth_output);
