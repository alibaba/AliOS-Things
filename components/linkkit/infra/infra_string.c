...
char *infra_strtok(char *str, const char *delim)
{
    ...
    if (pos == NULL || delim == NULL || strlen(pos) <= strlen(delim)) {
        return NULL;
    }
...
void LITE_hexstr_convert(char *input, int input_len, unsigned char *output,
                         int output_len)
{
    int i = 0;
    uint8_t ch0, ch1;
...
void LITE_hexbuf_convert(unsigned char *digest, char *out, int in_len,
                         int uppercase)
{
...
int32_t infra_json_value(const char *input, uint32_t input_len, const char *key,
                         uint32_t key_len, char **value, uint32_t *value_len)
{
    uint32_t idx = 0;
    ...
    for (idx = 0; idx < input_len; idx++) {
        ...
        if (memcmp(&input[idx], key, key_len) == 0) {
            ...
            if ((idx + 4 >= input_len) || (input[idx + 1] != ':')) {
                return -1;
            }
            idx += 2;
