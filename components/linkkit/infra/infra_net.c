...
static int read_ssl(utils_network_pt pNetwork, char *buffer, uint32_t len,
                    uint32_t timeout_ms)
{
    if (NULL == pNetwork) {
...
static int write_ssl(utils_network_pt pNetwork, const char *buffer,
...
static int connect_ssl(utils_network_pt pNetwork)
{
    ...
    pNetwork->handle = (intptr_t)AT_SSL_Establish(
                  pNetwork->pHostAddress, pNetwork->port, pNetwork->ca_crt,
...
static int connect_ssl(utils_network_pt pNetwork)
{
    ...
    pNetwork->handle = (intptr_t)HAL_SSL_Establish(
                  pNetwork->pHostAddress, pNetwork->port, pNetwork->ca_crt,
                  pNetwork->ca_crt_len + 1);
    if (pNetwork->handle != 0) {
        return 0;
...
#include "at_tcp.h"
/*** TCP connection ***/
static int read_tcp(utils_network_pt pNetwork, char *buffer, uint32_t len,
                    uint32_t timeout_ms)
{
    return AT_TCP_Read(pNetwork->handle, buffer, len, timeout_ms);
}

static int write_tcp(utils_network_pt pNetwork, const char *buffer,
                     uint32_t len, uint32_t timeout_ms)
{
...
static int connect_tcp(utils_network_pt pNetwork)
{
    ...
    pNetwork->handle =
        HAL_TCP_Establish(pNetwork->pHostAddress, pNetwork->port);
    if (pNetwork->handle == (uintptr_t)(-1)) {
        return -1;
    }
...
/****** network interface ******/
int utils_net_read(utils_network_pt pNetwork, char *buffer, uint32_t len,
                   uint32_t timeout_ms)
{
    int ret = 0;
#if defined(SUPPORT_TLS)
...
int utils_net_write(utils_network_pt pNetwork, const char *buffer, uint32_t len,
                    uint32_t timeout_ms)
{
    int ret = 0;
#if defined(SUPPORT_TLS)
...
int iotx_net_init(utils_network_pt pNetwork, const char *host, uint16_t port,
                  const char *ca_crt)
{
    if (!pNetwork || !host) {
