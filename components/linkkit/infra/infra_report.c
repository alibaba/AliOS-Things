...
const char *DEVICE_INFO_UPDATE_FMT =
    "{\"id\":\"%d\",\"version\":\"1.0\",\"params\":["
    "{\"attrKey\":\"SYS_LP_SDK_VERSION\",\"attrValue\":\"%s\",\"domain\":"
    "\"SYSTEM\"},"
...
int iotx_report_devinfo(void *pclient)
{
    ...
    ret = HAL_Snprintf(topic_name, IOTX_URI_MAX_LEN,
                       "/sys/%s/%s/thing/deviceinfo/update", product_key,
                       device_name);
    if (ret <= 0) {
    ...
    msg_len =
        strlen(DEVICE_INFO_UPDATE_FMT) + 10 + strlen(IOTX_SDK_VERSION) + 1;
    ...
    ret = HAL_Snprintf(msg, msg_len, DEVICE_INFO_UPDATE_FMT, iotx_report_id(),
                       IOTX_SDK_VERSION);
    if (ret <= 0) {
...
int iotx_report_firmware_version(void *pclient)
{
    ...
    ret = HAL_Snprintf(topic_name, IOTX_URI_MAX_LEN, "/ota/device/inform/%s/%s",
    ...
    ret = HAL_Snprintf(msg, FIRMWARE_VERSION_MSG_LEN,
