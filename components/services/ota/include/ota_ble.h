...
#ifdef EN_LONG_MTU
#define BLE_RECVD_PDU                   BLE_LONG_MTU_SIZE
#else
#define BLE_RECVD_PDU                   (16)
#endif

#define OTA_BLE_WAIT_TIME_MS            (500)
#define OTA_BLE_CMD_SIZE                (BLE_RECVD_PDU * 16 + 8)
#define OTA_BLE_CAHCH_CMD_NUMB          (2)
...
typedef struct _ota_ble_receive_data_{
    ...
    unsigned short len;
    unsigned char rec_buf[OTA_BLE_CMD_SIZE];
} ota_ble_receive_t;
...
typedef struct _ota_ble_global_dat{
    ...
    unsigned char file_type;                   /**< file type:image or picture */
    unsigned short ota_type;                   /**< image ota type:xz/diff/normal */
    unsigned int slaver_image_size;            /**< slaver image size */
    unsigned short slaver_image_crc16;         /**< slaver image crc */
    unsigned short crc;                        /**< image crc */
...
int ota_ble_mcu_upgrade(unsigned int partition_offset, unsigned char file_type, unsigned int size, 
                         char *ver, unsigned short file_crc16);
#endif /*OTA_BLE_IN_H*/