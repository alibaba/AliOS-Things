...
const ulog_man_handler_service_t ulog_man_handler_service[] =
{
    ...
    { "loglevel",{"up", "none"}, on_update_level2cfg_file},
    ...
    if ((log_text != NULL) && log_len > 0) {
        ...
        if ((str[0] == '<') && ((text_info = strchr(str, '>')) != NULL)) {
            ...
            if (check_pass_pop_out(ulog_session_std, severity)) {
                if (log_get_mutex()) {
                    printf("%s", (&((char*)log_text)[LOG_PREFIX_LEN]));
                    if (((char*)log_text)[LOG_PREFIX_LEN + strlen(&((char *)log_text)[LOG_PREFIX_LEN]) - 1] != '\n') {
                        printf("\r\n");
                    }
                    log_release_mutex();
                }
...
#else
                pop_out_on_fs(&((char*)log_text)[strlen(log_text) + 1], log_len - (strlen(log_text) + 1));
#endif
            }
#endif
...
#if ULOG_POP_FS_ENABLE
        } else if (0 == strncmp(str, ULOG_FILE_NOTIFY, strlen(ULOG_FILE_NOTIFY))) {
            ulog_file_sync_schedule();
...
void ulog_async_init()
{
    ...
    if (0 == aos_task_new_ext(&ulog_routine,
        ...
        RHINO_CONFIG_USER_PRI_MAX - 20, 1)) {
