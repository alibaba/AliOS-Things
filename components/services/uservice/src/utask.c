...
utask_t *utask_new(const char *name, size_t stack_size, int queue_count, int prio)
{
    ...
    utask_t *task = aos_zalloc(sizeof(utask_t));
    ...
    if (aos_queue_new(&task->queue , queue_buffer_size, sizeof(rpc_t)) != 0)
        goto out0;
    ...
    if (aos_task_new_ext(&task->task, name ? name : "utask", utask_entry, task, stack_size, prio, 1) != 0)
        goto out3;
