...
#ifndef HTTPCLIENT_SSL_ENABLE
#define HTTPCLIENT_SSL_ENABLE 1
#endif
...
#endif
#ifdef HTTPCLIENT_SOCK_PARAM
    http_sock_param_t sock_param;   /**< socket param */
#endif
    bool keep_alive;                /**< keep alive request header */
} httpclient_t;
...
typedef struct {
    ...
    char *response_buf_dyn;      /**< Dynamic buffer to store the response body data. Allocate by httpclient */
...
/**
 * @brief            This function executes a GET request on a given URL. It blocks until completion.
 *                   It gets result from response_buf_dyn which is dynamic allocated according to content length
 * @param[in]        client is a pointer to the #httpclient_t.
 * @param[in]        url is the URL to run the request.
 * @param[in, out]   client_data is a pointer to the #httpclient_data_t instance to collect the data returned by the request.
 * @return           Please refer to #HTTPCLIENT_RESULT.
 ...
 *                   httpclient_t client = {0};
 *                   httpclient_data_t client_data = {0};
 *                   httpclient_get_dyn(&client, url, &client_data);
 *                   printf("Data received: %s\r\n", client_data.response_buf);
 * @endcode
 */
 ...
 *                   client_data.post_buf_len = strlen(post_data);  //Sets the post data length.
 *                   client_data.post_content_type = content_type;  //Sets the content type.
 *                   httpclient_post_dyn(&client, url, &client_data);
 *                   printf("Data received: %s\r\n", client_data.response_buf);
 * @endcode
 */
 ...
 *                   client_data.post_buf_len = strlen(put_data);  //Sets the put data length.
 *                   client_data.post_content_type = content_type;  //Sets the content type.
 *                   httpclient_put_dyn(&client, url, &client_data);
 *                   printf("Data received: %s\r\n", client_data.response_buf);
 * @endcode
 */
 ...
 *                   httpclient_t client = {0};
 *                   httpclient_data_t client_data = {0};
 *                   httpclient_delete_dyn(&client, url, &client_data);
 *                   printf("Data received: %s\r\n", client_data.response_buf);
 * @endcode
 */
