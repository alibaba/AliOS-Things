...
#ifdef ENABLE_DEINIT_LOCK
#include <aos/kernel.h>
static aos_hdl_t deinit_lock = {0};
static void hal_init_lock()
{
    static int init = 0;
    if (init == 0) {
        aos_mutex_new(&deinit_lock);
        init = 1;
    }
}
        ...
        uint32_t timeout_ms, int *fd_out)
{
    ...
    if (res < 0) {
        ...
        printf("%s success to establish tcp, fd=%d\n", aos_task_name(), *fd_out);
        res = STATE_SUCCESS;
    }
...
static int32_t _core_sysdep_network_udp_recv(core_network_handle_t *network_handle, uint8_t *buffer, uint32_t len,
...
#ifdef ENABLE_DEINIT_LOCK
    aos_mutex_lock(&deinit_lock, AOS_WAIT_FOREVER);
#endif
    if (handle == NULL || *handle == NULL) {
#ifdef ENABLE_DEINIT_LOCK
        aos_mutex_unlock(&deinit_lock);
#endif
        return STATE_PORT_INPUT_NULL_POINTER;
    }
