...
void net_daemon_thread(void *arg)
{
  ...
  do {
    ...
    if (success_count == 0) {
        ...
        LOGW(TAG, "==> Attention! Fail to ping router! <===\n");
        ping_fail_count++;
        sys_msleep(POLL_NET_STATUS_PERIOD_FAST_MS);
    } else {
        LOGI(TAG, "==> ping router success! %"U32_F" ms <===\n", elapse_time);
...
u32_t get_poll_ping_success_count(void)
{
    return ping_success_count;
}

u32_t get_poll_ping_fail_count(void)
{
    return ping_fail_count;
}

float get_poll_ping_avg_elapse_ms(void)
{
    return avg_elapse_time_ms;
}
