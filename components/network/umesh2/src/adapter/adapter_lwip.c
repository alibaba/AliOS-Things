...
bool umesh_is_mcast_subscribed(const ur_ip6_addr_t *addr)
{
    ...
    log_hex("----------self mcast---------", ip6_addr, 16);
    return !memcmp(addr->m8, ip6_addr, sizeof(ur_ip6_addr_t));
}
                                     ...
                                     const ip6_addr_t *dest)
{
    ...
    log_hex("----IPV6 addr ", dest->addr, 16);
    log_hex("----IPV6 IN", p->payload, p->len);
...
int umesh_adapter_interface_up(void)
{
    ...
    //#endif
    log_d("umesh_adapter_interface_up seccess!");
    return ret;
}
