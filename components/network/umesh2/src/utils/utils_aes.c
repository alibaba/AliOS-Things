...
#ifdef UTILS_AES_BUILDIN
#if !defined(UTILS_CONFIG_FILE)
    #include "utils_aes_config.h"
#else
    #include UTILS_CONFIG_FILE
#endif

/* Implementation that should never be optimized out by the compiler */
static void utils_aes_zeroize(void *v, size_t n)
{
...
void utils_aes_free(utils_aes_context *ctx)
{
    ...
    utils_aes_zeroize(ctx, sizeof(utils_aes_context));
}
...
#if defined(UTILS_PADLOCK_C) && defined(UTILS_PADLOCK_ALIGN16)
    if (aes_padlock_ace == -1) {
        aes_padlock_ace = utils_aes_padlock_has_support(UTILS_PADLOCK_ACE);
    }

    if (aes_padlock_ace) {
        ctx->rk = RK = UTILS_PADLOCK_ALIGN16(ctx->buf);
    } else
#endif
        ctx->rk = RK = ctx->buf;

#if defined(UTILS_AESNI_C) && defined(UTILS_HAVE_X86_64)
    if (utils_aesni_has_support(UTILS_AESNI_AES)) {
        return (utils_aesni_setkey_enc((unsigned char *) ctx->rk, key, keybits));
    }
#endif
...
#if defined(UTILS_AESNI_C) && defined(UTILS_HAVE_X86_64)
    if (utils_aesni_has_support(UTILS_AESNI_AES)) {
        utils_aesni_inverse_key((unsigned char *) ctx->rk,
...
static int utils_aes_internal_aes_encrypt(utils_aes_context *ctx,
...
static int utils_aes_internal_aes_decrypt(utils_aes_context *ctx,
        const unsigned char input[16],
        unsigned char output[16])
{
    int i;
                        ...
                        unsigned char output[16])
{
#if defined(UTILS_AESNI_C) && defined(UTILS_HAVE_X86_64)
    if (utils_aesni_has_support(UTILS_AESNI_AES)) {
        return (utils_aesni_crypt_ecb(ctx, mode, input, output));
    }
#endif

#if defined(UTILS_PADLOCK_C) && defined(UTILS_HAVE_X86)
    if (aes_padlock_ace) {
        if (utils_aes_padlock_xcryptecb(ctx, mode, input, output) == 0) {
            return (0);
        }
    ...
    if (mode == UTILS_AES_ENCRYPT) {
        return (utils_aes_internal_aes_encrypt(ctx, input, output));
    } else {
        return (utils_aes_internal_aes_decrypt(ctx, input, output));
    }
}
...
#if defined(UTILS_PADLOCK_C) && defined(UTILS_HAVE_X86)
    if (aes_padlock_ace) {
        if (utils_aes_padlock_xcryptcbc(ctx, mode, length, iv, input, output) == 0) {
            return (0);
        }
                           ...
                           unsigned char *output)
{
    ...
    if (mode == UTILS_AES_DECRYPT) {
        while (length--) {
            if (n == 0) {
                utils_aes_crypt_ecb(ctx, UTILS_AES_ENCRYPT, iv, iv);
            }
...
#if defined(UTILS_AES_BUILDIN)
    utils_aes_init(&p_aes128->ctx);
    if (dir == AES_ENCRYPTION) {
        ret = utils_aes_setkey_enc(&p_aes128->ctx, key, 128);
    } else {
        ret = utils_aes_setkey_dec(&p_aes128->ctx, key, 128);
    }
#else
...
#if defined(UTILS_AES_BUILDIN)
    utils_aes_free(&((platform_aes_t *)aes)->ctx);
#else
...
#if defined(UTILS_AES_BUILDIN)
        ret = utils_aes_crypt_cbc(&p_aes128->ctx, UTILS_AES_DECRYPT, AES_BLOCK_SIZE,
                                  p_aes128->iv, src, dst);
#else
...
#if defined(UTILS_AES_BUILDIN)
    ret = utils_aes_setkey_enc(&p_aes128->ctx, p_aes128->key, 128);
    ret = utils_aes_crypt_cfb128(&p_aes128->ctx, UTILS_AES_DECRYPT, length,
...
#if defined(UTILS_AES_BUILDIN)
    ret = utils_aes_crypt_cfb128(&p_aes128->ctx, UTILS_AES_ENCRYPT, length,
                                 &offset, p_aes128->iv, src, dst);
#else
            ...
            void *dst)
{
    ...
    for (i = 0; i < blockNum; ++i) {
#if defined(UTILS_AES_BUILDIN)
        ret = utils_aes_crypt_cbc(&p_aes128->ctx, UTILS_AES_ENCRYPT, AES_BLOCK_SIZE,
                                  p_aes128->iv, src, dst);
#else
