...
extern int netmgr_wifi_set_ip_mode(netmgr_wifi_ip_mode_t mode);
extern int netmgr_wifi_set_static_ip_stat(const char* ip_addr, const char* mask, const char* gw, const char* dns_server);
extern netmgr_wifi_ip_mode_t netmgr_wifi_get_ip_mode(void);
...
static int add_hdl_info(netmgr_hdl_t hdl, char* file_name, netmgr_type_t type)
{
    hdl_info_t* cur;
...
int netmgr_add_dev(const char* name)
{
    ...
    if(fd >= 0) {
        ...
        netmgr_wifi_init((netmgr_hdl_t)fd);
        return 0;
    } else {
