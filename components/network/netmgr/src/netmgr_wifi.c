...
#include <fcntl.h>
#if ((RHINO_CONFIG_USER_SPACE > 0) && (AOS_COMP_NETMGR_IN_SERVICE == 0))
#include "callback.h"
#include "res.h"
#endif /* AOS_CONFIG_USER_SPACE > 0 */
...
#define NETMGR_WIFI_TEMP_CONF    "/data/temp.conf"
...
#if 0
#define NETMGR_WIFI_LOGE(level, ...)      be_error(level, ##__VA_ARGS__)
#define NETMGR_WIFI_LOGI(level, ...)      be_info(level, ##__VA_ARGS__)
#define NETMGR_WIFI_LOGD(level, ...)      be_debug(level, ##__VA_ARGS__)
#else
...
extern int aos_sem_is_valid(aos_sem_t *sem);
...
static bool g_is_sntp_enabled = false;
...
static bool g_is_auto_save_ap_config = true;
...
static int wifi_queue_init()
{
    ...
    if (aos_queue_new(&g_msg_queue , size,
    ...
    while (true) {
        if(g_is_recvmsg_result_got == false) {
         ...
         if (size != sizeof(netmgr_wifi_msg_t)) {
            NETMGR_WIFI_LOGE("%s:%d Error msg queue recv: msg size %u is not valid\n", __func__, __LINE__, size);
            goto exit;
         }
...
static int wifi_recvmsg_task()
{
    ...
    if(g_is_recvmsg_result_got == false) {
        ...
        aos_task_new_ext(&recvmsg_task, "wifi_recvmsg_task", (task_entry_t)wifi_recvmsg_handle, NULL, 4*1024, NETMGR_WIFI_RECVMSG_PRI, 1);
        ...
        if((result != NULL)
          ...
          && (result->ap_list != NULL)) {
            ...
            if(g_scan_result.ap_list == NULL) {
                if(aos_sem_is_valid((aos_hdl_t *)&g_scan_result_sem) == 1) {
...
static void wifi_sntp_task(void)
{
    ...
    if(g_is_sntp_task_running == false) {
        ...
        aos_task_new_ext(&sntp_task, "sntp_task", (task_entry_t)wifi_sntp_handle, NULL, 4*1024, NETMGR_WIFI_SNTP_PRI, 1);
    } else {
...
static void tcpip_dhcpc_cb(struct netif *net_if) {
    ...
    if (netif_is_up(net_if) && (dhcp->state == DHCP_STATE_BOUND) && !ip_addr_isany(&net_if->ip_addr)) {
        ...
        NETMGR_WIFI_LOGI(" IPv4 Gateway     : %hhu.%hhu.%hhu.%hhu\r\n\r\n",
               ...
               ((u8_t *)&net_if->gw.addr)[2],
               ((u8_t *)&net_if->gw.addr)[3]);
        event_publish(EVENT_WIFI_DHCP_SUCCESS, NULL);
...
static void reconnect_task_handle(void* arg)
{
    ...
    while(1) {
        ...
        if(conn->state == CONN_STATE_DISCONNECTED) {
            ...
            memcpy(params.ssid, saved_ap_conf->config[0].ssid, sizeof(params.ssid));
            memcpy(params.pwd, saved_ap_conf->config[0].pwd, sizeof(params.pwd));
...
static int wifi_auto_reconnect_task()
{
    ...
    aos_task_new_ext(&reconnect_task, "reconnect_task", (task_entry_t)reconnect_task_handle, NULL, 8*1024, NETMGR_WIFI_RECONNECT_PRI, 1);
    return 0;
}
		...
		const char* obj,int* line, netmgr_wifi_ap_info_t* config)
{
    ...
    if((pos_start=strstr(pos,obj)) != NULL) {
        ...
        if(NULL == pos_start) {
            NETMGR_WIFI_LOGE("Line %d:Invalid value line.", *line);
            return -1;
        }
        pos_start ++;
...
static char* get_config_line(char* s, int size, FILE * stream, int* line, char** _pos)
{
    char *pos, *end, *sstart;
...
static int gets_read(int fd, char *buffer, int max_size)
{
    char *p_b = buffer;
    char * p_buf = p_b;
    int nCnt = 0;
...
static char* get_config_line(char* s, int size, int fd, int* line, char** _pos)
{
    char *pos, *end, *sstart;
    while (gets_read(fd, s, size)) {
        (*line)++;
        s[size - 1] = '\0';
        if (!newline_terminated(s, size)) {
        /*
...
int get_wifi_config(const char *name, netmgr_wifi_ap_config_t* saved_ap_conf, WIFI_CONFIG_READ_MODE mode)
{
#ifdef USE_FILE_IO
    FILE *fp = NULL;
#else
        ...
        if(strstr(pos, "network={") != NULL) {
            ...
            while(get_config_line(buf, sizeof(buf), fd, &line, &pos)) {
                ...
                for(j = 0; j < CONFIG_ELEMENT_NUM; j++) {
                    ...
                    NETMGR_WIFI_LOGD("%s:%d %s, len=%d", __func__, __LINE__, wificonfigsymbol[j], len);
                    if(strncmp(pos, wificonfigsymbol[j], len) != 0 || pos[len] != '=')
                        continue;
                    if(read_config_value(pos, wificonfigsymbol[j], &line, &ap_info)) {
            ...
            NETMGR_WIFI_LOGD("%s:%d read ssid=%s\n", __func__, __LINE__, ap_info.ssid);
...
static int update_wifi_config(netmgr_conn_t* conn, const char *name)
{
    ...
    temp_fp = fopen(NETMGR_WIFI_TEMP_CONF,"a");
    if(NULL == temp_fp) {
...
static void wifi_status_change(netmgr_conn_t *conn, int event)
{
    ...
    else if(event == EVENT_NETMGR_WIFI_DHCP_SUCCESS) {
...
static void handle_wifi_event(netmgr_conn_t* conn, int event)
{
    ...
    if(event >= EVENT_NETMGR_WIFI_DISCONNECTED && event < EVENT_NETMGR_MAX) {
        ...
        while(node != NULL) {
            ...
            if((event == EVENT_NETMGR_WIFI_DISCONNECTED) ||
                (event == EVENT_NETMGR_WIFI_DHCP_SUCCESS)) {
...
static void wifi_indicate_event_handle(void* hdl, int wifi_event, void* arg)
{
    ...
    switch(wifi_event)
    {
        ...
        case EVENT_WIFI_CONN_TIMEOUT:
            ...
            break;
        case EVENT_WIFI_DHCP_SUCCESS:
            event = EVENT_NETMGR_WIFI_DHCP_SUCCESS;
            NETMGR_WIFI_LOGD("NETMGR_WIFI_DHCP_SUCCESS");
            break;
...
static bool sntp_start(void)
{
    ...
    while(1) {
        ...
        if (0 == sntp_get_time(&m_sntp_arg, &ntp_time)) {
            NETMGR_WIFI_LOGI("[sntp] OK: sec %lld usec %lld\n", ntp_time.tv_sec, ntp_time.tv_usec);
            return true;
...
int netmgr_wifi_init(netmgr_hdl_t hdl)
{
    ...
    event_subscribe(EVENT_WIFI_DHCP_SUCCESS, wifi_stat_chg_cb, NULL);
...
static int wifi_start_scan(netmgr_conn_t* conn, netmgr_wifi_ap_config_t* saved_ap_conf)
{
    ...
    ap_list_t ap_list[MAX_AP_CONFIG_NUM * 2 + 1];
    int ap_num;
    int i;
    int ret;
...
int netmgr_wifi_connect(netmgr_hdl_t hdl, netmgr_wifi_connect_params_t *params)
{
    ...
    if(ssid != NULL) {
        ...
        memset(saved_ap_conf, 0, sizeof(netmgr_wifi_ap_config_t));
        NETMGR_WIFI_LOGI("%s:%d ssid=%s pwd=%s,ssid len = %d, pwd len = %d\n", __func__, __LINE__, ssid, pwd, (sizeof(saved_ap_conf->config[0].ssid) - 1), sizeof(saved_ap_conf->config[0].pwd));
    ...
    while(1) {
        ...
        if(conn->state == CONN_STATE_NETWORK_CONNECTED) {
            ...
            /* Reset retry to 0 */
            retry = 0;
            if((netmgra_wifi_ssid_format_t)saved_ap_conf->config[i].ssid_format == NETMGR_WIFI_SSID_FORMAT_UTF8) {
...
int netmgr_wifi_disconnect(netmgr_hdl_t hdl)
{
    ...
    if(conn != NULL) {
        ...
        NETMGR_WIFI_LOGE("%s:%d g_wifi_conn_info is null. will not do disconnect\n", __func__, __LINE__);
        return -1;
    }
...
void wifi_handle_cmd(int argc, char **argv)
{
    ...
    if (strcmp(rtype, "-i") == 0) {
...
#else
        while(gets_read(fd, buf, sizeof(buf))) {
#endif
            buf[sizeof(buf) - 1] = '\0';
        ...
        if(argc > 1) {
            ...
            fd = open(NETMGR_WIFI_CONF, O_RDWR | O_CREAT | O_TRUNC, 0666);
            if(fd < 0) {
                NETMGR_WIFI_LOGE("%s:%d open %s failed:%s\n", __func__, __LINE__, NETMGR_WIFI_CONF,strerror(errno));
                return ;
            }
