...
#define G_RESP_DATA_LEN 400
static        char g_response_data[G_RESP_DATA_LEN];
int activation_get_report_msg(char* report_msg, int len)
{
    ...
    aos_version_str_get(kernel_version, sizeof(kernel_version) - 1);
    if((strstr(kernel_version, SYSINFO_KERNEL_VERSION_PREFIX)) != NULL) {
...
#define RESP_RESULT_LEN     6
#define RESP_MSG_LEN        18
int activation_parse_data(char *response_data)
{
    char response_result[RESP_RESULT_LEN];
    char response_msg[RESP_MSG_LEN];
    char* result_start;
    char* result_end;
    int len;
    ...
    if(((result_start = strstr(response_data, ACTIVATION_RESPONSE_RESULT_START)) != NULL)
        && ((result_end = strstr(result_start, ACTIVATION_RESPONSE_RESULT_END)) != NULL)) {
            ...
            len = len < (RESP_RESULT_LEN - 1) ? len : (RESP_RESULT_LEN - 1);
        ...
        && ((result_end = strstr(result_start, ACTIVATION_RESPONSE_MESSAGE_END)) != NULL)) {
            ...
            len = len < (RESP_MSG_LEN - 1) ? len : (RESP_MSG_LEN - 1);
...
int activation_report(void)
{
    ...
    if((ret != -1) && (ret != 0)) {
        g_response_data[G_RESP_DATA_LEN - 1] = '\0';
