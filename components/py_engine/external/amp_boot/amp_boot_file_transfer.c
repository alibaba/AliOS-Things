...
{
    amp_file_transfer_status_set(AMP_FILE_TRANSFER_STATUS_NORMAL);
...
int pyamp_ymodem_save_transfer_file(uint8_t *buf, int32_t buf_len, AMP_FILE_RECEVIE_FLAG flag)
{
    ...
    if(flag == YMODEM_RECEVIE_HEADER) {
        ...
        // set status install
        amp_file_transfer_status_set(AMP_FILE_TRANSFER_STATUS_INSTALL);
    } else {
...
{
    amp_file_transfer_status_set(AMP_FILE_TRANSFER_STATUS_UPDATING);
    pyamp_ymodem_save_transfer_file(buf, size, flag);
}

void amp_file_transfer_status_set(AMP_FILE_TRANSFER_STATUS mode)
{
