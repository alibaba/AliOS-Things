...
#define MBEDTLS_SET_CALLOC_FREE_ENABLED (0)
 ...
 *
 *  大多数情况下, 就保持它如下的设置即可
 *  只有少数时候, SDK的用户关心对接层代码的ROM尺寸,
 * 并且也没有选择用TLS连接服务器 那时才会出现, 将 CORE_SYSDEP_MBEDTLS_ENABLED
 * 宏定义关闭的改动, 以减小对接尺寸
 *
...
#if MBEDTLS_SET_CALLOC_FREE_ENABLED
...
static void *_core_mbedtls_calloc(size_t n, size_t size) {
  ...
  /* printf("INFO -- mbedtls malloc: %p %d  total used: %d  max used: %d\r\n",
                     buf, (int)size, g_mbedtls_total_mem_used,
     g_mbedtls_max_mem_used); */
...
static void _core_mbedtls_free(void *ptr) {
  ...
  mem_info =
      (mbedtls_mem_info_t *)((unsigned char *)ptr - sizeof(mbedtls_mem_info_t));
  ...
  /* printf("INFO -- mbedtls free: %p %d  total used: %d  max used: %d\r\n",
                     ptr, mem_info->size, g_mbedtls_total_mem_used,
     g_mbedtls_max_mem_used); */
...
void *core_sysdep_malloc(uint32_t size, char *name) { return aos_malloc(size); }

void core_sysdep_free(void *ptr) { aos_free(ptr); }
...
void core_sysdep_sleep(uint64_t time_ms) { aos_msleep(time_ms); }
...
int32_t core_sysdep_network_setopt(void *handle,
                                   core_sysdep_network_option_t option,
                                   void *data) {
...
static int32_t _core_sysdep_network_connect(char *host, uint16_t port,
                                            int family, int socktype,
                                            int protocol, uint32_t timeout_ms,
                                            int *fd_out) {
...
static int32_t
_core_sysdep_network_tcp_establish(core_network_handle_t *network_handle) {
  int32_t res = STATE_SUCCESS;

  // printf("establish tcp connection with server(host='%s', port=[%u])\n",
  //        network_handle->host, network_handle->port);

  // res = _core_sysdep_network_connect(
  //     network_handle->host, network_handle->port, AF_UNSPEC, SOCK_STREAM,
  //     IPPROTO_TCP, network_handle->connect_timeout_ms, &network_handle->fd);
  // if (res == STATE_PORT_NETWORK_DNS_FAILED &&
  //     strlen(network_handle->backup_ip) > 0) {
  //   printf("using backup ip: %s\n", network_handle->backup_ip);
  //   res = _core_sysdep_network_connect(
  //       network_handle->backup_ip, network_handle->port, AF_UNSPEC, SOCK_STREAM,
  //       IPPROTO_TCP, network_handle->connect_timeout_ms, &network_handle->fd);
  // }
...
static int32_t _core_sysdep_network_udp_server_establish(
...
static int _mbedtls_random(void *handle, unsigned char *output,
                           size_t output_len) {
...
static void _mbedtls_debug(void *ctx, int level, const char *file, int line,
                           const char *str) {
  ((void)level);
  if (NULL != ctx) {
    printf("%s\n", str);
...
static int32_t
_core_sysdep_network_mbedtls_establish(core_network_handle_t *network_handle) {
  int32_t res = 0;
  char port_str[6] = {0};
  ...
  if (network_handle->cred->max_tls_fragment <= 512) {
    res = mbedtls_ssl_conf_max_frag_len(&network_handle->mbedtls.ssl_config,
  ...
  if (network_handle->socket_type == CORE_SYSDEP_SOCKET_TCP_CLIENT) {
    res = _core_sysdep_network_connect(
        network_handle->host, network_handle->port, AF_UNSPEC, SOCK_STREAM,
        IPPROTO_TCP, network_handle->connect_timeout_ms,
        &network_handle->mbedtls.net_ctx.fd);
  } else if (network_handle->socket_type == CORE_SYSDEP_SOCKET_UDP_CLIENT) {
    res = _core_sysdep_network_connect(
        network_handle->host, network_handle->port, AF_UNSPEC, SOCK_DGRAM,
        IPPROTO_UDP, network_handle->connect_timeout_ms,
        &network_handle->mbedtls.net_ctx.fd);
  }
  ...
  if (network_handle->socket_type == CORE_SYSDEP_SOCKET_TCP_CLIENT) {
    res = mbedtls_ssl_config_defaults(
        &network_handle->mbedtls.ssl_config, MBEDTLS_SSL_IS_CLIENT,
  ...
  mbedtls_ssl_conf_max_version(&network_handle->mbedtls.ssl_config,
                               MBEDTLS_SSL_MAJOR_VERSION_3,
                               MBEDTLS_SSL_MINOR_VERSION_3);
  mbedtls_ssl_conf_min_version(&network_handle->mbedtls.ssl_config,
                               MBEDTLS_SSL_MAJOR_VERSION_3,
                               MBEDTLS_SSL_MINOR_VERSION_3);
  // mbedtls_ssl_conf_handshake_timeout(&network_handle->mbedtls.ssl_config,(MBEDTLS_SSL_DTLS_TIMEOUT_DFL_MIN
  // * 2),
  //                             (MBEDTLS_SSL_DTLS_TIMEOUT_DFL_MIN * 2 * 4));
  mbedtls_ssl_conf_rng(&network_handle->mbedtls.ssl_config, _mbedtls_random,
                       NULL);
  mbedtls_ssl_conf_dbg(&network_handle->mbedtls.ssl_config, _mbedtls_debug,
                       stdout);

  if (network_handle->cred->option == AIOT_SYSDEP_NETWORK_CRED_SVRCERT_CA) {
    if (network_handle->cred->x509_server_cert == NULL &&
        network_handle->cred->x509_server_cert_len == 0) {
    ...
    res = mbedtls_x509_crt_parse(
        &network_handle->mbedtls.x509_server_cert,
        (const unsigned char *)network_handle->cred->x509_server_cert,
        (size_t)network_handle->cred->x509_server_cert_len + 1);
    if (res < 0) {
    ...
    if (network_handle->cred->x509_client_cert != NULL &&
        network_handle->cred->x509_client_cert_len > 0 &&
        network_handle->cred->x509_client_privkey != NULL &&
        network_handle->cred->x509_client_privkey_len > 0) {
      mbedtls_x509_crt_init(&network_handle->mbedtls.x509_client_cert);
      mbedtls_pk_init(&network_handle->mbedtls.x509_client_pk);
      res = mbedtls_x509_crt_parse(
          &network_handle->mbedtls.x509_client_cert,
          (const unsigned char *)network_handle->cred->x509_client_cert,
          (size_t)network_handle->cred->x509_client_cert_len + 1);
      if (res < 0) {
      ...
      res = mbedtls_ssl_conf_own_cert(&network_handle->mbedtls.ssl_config,
                                      &network_handle->mbedtls.x509_client_cert,
                                      &network_handle->mbedtls.x509_client_pk);
      if (res < 0) {
    ...
    mbedtls_ssl_conf_ca_chain(&network_handle->mbedtls.ssl_config,
                              &network_handle->mbedtls.x509_server_cert, NULL);
  } else if (network_handle->cred->option ==
             AIOT_SYSDEP_NETWORK_CRED_SVRCERT_PSK) {
    static const int ciphersuites[1] = {MBEDTLS_TLS_PSK_WITH_AES_128_CBC_SHA};
    res =
        mbedtls_ssl_conf_psk(&network_handle->mbedtls.ssl_config,
                             (const unsigned char *)network_handle->psk.psk,
                             (size_t)strlen(network_handle->psk.psk),
                             (const unsigned char *)network_handle->psk.psk_id,
                             (size_t)strlen(network_handle->psk.psk_id));
    if (res < 0) {
      printf("mbedtls_ssl_conf_psk error, res = -0x%04X\n", (unsigned int)-res);
      return STATE_PORT_TLS_CONFIG_PSK_FAILED;
    }
    mbedtls_ssl_conf_ciphersuites(&network_handle->mbedtls.ssl_config,
                                  ciphersuites);
  } else {
  ...
  res = mbedtls_ssl_setup(&network_handle->mbedtls.ssl_ctx,
                          &network_handle->mbedtls.ssl_config);
  if (res < 0) {
  ...
  if (_host_is_ip(network_handle->host) == 0) {
    res = mbedtls_ssl_set_hostname(&network_handle->mbedtls.ssl_ctx,
                                   network_handle->host);
    if (res < 0) {
  ...
  if (network_handle->socket_type == CORE_SYSDEP_SOCKET_UDP_CLIENT) {
    mbedtls_ssl_set_timer_cb(&network_handle->mbedtls.ssl_ctx,
                             (void *)&network_handle->mbedtls.timer_delay_ctx,
                             mbedtls_timing_set_delay,
                             mbedtls_timing_get_delay);
  }

  mbedtls_ssl_set_bio(&network_handle->mbedtls.ssl_ctx,
                      &network_handle->mbedtls.net_ctx, mbedtls_net_send,
                      mbedtls_net_recv, mbedtls_net_recv_timeout);
  mbedtls_ssl_conf_read_timeout(&network_handle->mbedtls.ssl_config,
                                network_handle->connect_timeout_ms);
  ...
  printf("success to establish mbedtls connection, fd = %d(cost %d bytes in "
         "total, max used %d bytes)\n",
         (int)network_handle->mbedtls.net_ctx.fd, g_mbedtls_total_mem_used,
         g_mbedtls_max_mem_used);
...
static int32_t
_core_sysdep_network_tcp_recv(core_network_handle_t *network_handle,
  ...
  //       recv_res =
  //           recv(network_handle->fd, buffer + recv_bytes, len - recv_bytes, 0);
  //       if (recv_res == 0) {
...
static int32_t
_core_sysdep_network_udp_recv(core_network_handle_t *network_handle,
                              uint8_t *buffer, uint32_t len,
  ...
  // res = recvfrom(network_handle->fd, buffer, len, 0,
  //                (struct sockaddr *)&cliaddr, &addr_len);
  // if (res >= 0) {
  //   if (NULL != addr) {
  //     addr->port = ntohs(cliaddr.sin_port);
  //     strncpy((char *)addr->addr, inet_ntoa(cliaddr.sin_addr),
  //             sizeof(addr->addr));
  //   }
...
#ifdef CORE_SYSDEP_MBEDTLS_ENABLED
static int32_t
_core_sysdep_network_mbedtls_recv(core_network_handle_t *network_handle,
                                  uint8_t *buffer, uint32_t len,
                                  uint32_t timeout_ms) {
  int res = 0;
  int32_t recv_bytes = 0;
  ...
  mbedtls_ssl_conf_read_timeout(&network_handle->mbedtls.ssl_config,
                                timeout_ms);
  do {
    res = mbedtls_ssl_read(&network_handle->mbedtls.ssl_ctx,
                           buffer + recv_bytes, len - recv_bytes);
    if (res < 0) {
...
int32_t core_sysdep_network_recv(void *handle, uint8_t *buffer, uint32_t len,
                                 uint32_t timeout_ms,
  ...
  if (network_handle->socket_type == CORE_SYSDEP_SOCKET_TCP_CLIENT) {
    if (network_handle->cred == NULL) {
      return _core_sysdep_network_tcp_recv(network_handle, buffer, len,
                                           timeout_ms);
    } else {
      if (network_handle->cred->option == AIOT_SYSDEP_NETWORK_CRED_NONE) {
        return _core_sysdep_network_tcp_recv(network_handle, buffer, len,
        ...
        return _core_sysdep_network_mbedtls_recv(network_handle, buffer, len,
                                                 timeout_ms);
      }
#endif
    }
    ...
    return _core_sysdep_network_udp_recv(network_handle, buffer, len,
                                         timeout_ms, addr);
  }
...
int32_t _core_sysdep_network_tcp_send(core_network_handle_t *network_handle,
                                      uint8_t *buffer, uint32_t len,
                                      uint32_t timeout_ms) {
  int res = 0;
...
//         send_res =
//             send(network_handle->fd, buffer + send_bytes, len - send_bytes, 0);
//         if (send_res == 0) {
...
int32_t _core_sysdep_network_udp_send(core_network_handle_t *network_handle,
                                      uint8_t *buffer, uint32_t len,
                                      uint32_t timeout_ms,
                                      core_sysdep_addr_t *addr) {
  int res = 0;
  ...
  // res = sendto(network_handle->fd, buffer, len, 0, (struct sockaddr *)&cliaddr,
...
#ifdef CORE_SYSDEP_MBEDTLS_ENABLED
int32_t _core_sysdep_network_mbedtls_send(core_network_handle_t *network_handle,
                                          uint8_t *buffer, uint32_t len,
                                          uint32_t timeout_ms) {
  int32_t res = 0;
  int32_t send_bytes = 0;
  ...
  res = setsockopt(network_handle->mbedtls.net_ctx.fd, SOL_SOCKET, SO_SNDTIMEO,
  ...
  do {
    ...
    res = mbedtls_ssl_write(&network_handle->mbedtls.ssl_ctx,
                            buffer + send_bytes, len - send_bytes);
    if (res < 0) {
      if (res != MBEDTLS_ERR_SSL_WANT_READ &&
          res != MBEDTLS_ERR_SSL_WANT_WRITE) {
        if (send_bytes == 0) {
...
int32_t core_sysdep_network_send(void *handle, uint8_t *buffer, uint32_t len,
                                 uint32_t timeout_ms,
                                 core_sysdep_addr_t *addr) {
//   core_network_handle_t *network_handle = (core_network_handle_t *)handle;
...
//   if (network_handle->socket_type == CORE_SYSDEP_SOCKET_TCP_CLIENT) {
//     if (network_handle->cred == NULL) {
//       return _core_sysdep_network_tcp_send(network_handle, buffer, len,
//                                            timeout_ms);
//     } else {
//       if (network_handle->cred->option == AIOT_SYSDEP_NETWORK_CRED_NONE) {
//         return _core_sysdep_network_tcp_send(network_handle, buffer, len,
...
// #ifdef CORE_SYSDEP_MBEDTLS_ENABLED
//       else {
//         return _core_sysdep_network_mbedtls_send(network_handle, buffer, len,
//                                                  timeout_ms);
//       }
// #endif
//     }
...
//     }
//   } else if (network_handle->socket_type == CORE_SYSDEP_SOCKET_UDP_SERVER) {
//     return _core_sysdep_network_udp_send(network_handle, buffer, len,
//                                          timeout_ms, addr);
//   }
...
static void
_core_sysdep_network_tcp_disconnect(core_network_handle_t *network_handle) {
...
#ifdef CORE_SYSDEP_MBEDTLS_ENABLED
static void
_core_sysdep_network_mbedtls_disconnect(core_network_handle_t *network_handle) {
...
int32_t core_sysdep_network_deinit(void **handle) {
  ...
  if ((network_handle->socket_type == CORE_SYSDEP_SOCKET_TCP_CLIENT ||
       network_handle->socket_type == CORE_SYSDEP_SOCKET_UDP_CLIENT) &&
...
void core_sysdep_rand(uint8_t *output, uint32_t output_len) {
  ...
  aos_srand((unsigned int)(time.tv_sec * 1000 + time.tv_usec / 1000) +
            aos_rand());
