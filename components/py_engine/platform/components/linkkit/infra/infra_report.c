...
#else
    #define VERSION_DEBUG(fmt, ...)  LOGD("report", fmt, ##__VA_ARGS__)
    #define VERSION_ERR(fmt, ...)    LOGE("report", fmt, ##__VA_ARGS__)
#endif
...
int iotx_report_devinfo(void *pclient)
{
    ...
    msg_len = strlen(DEVICE_INFO_UPDATE_FMT) + 10 + strlen(IOTX_AOS_VERSION) + 1;
