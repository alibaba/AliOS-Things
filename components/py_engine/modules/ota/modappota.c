...
static void ota_install_notify(void *pdata)
{
    ...
    if(mp_obj_is_fun(ota_on_upgrade)){
        callback_to_python(ota_on_upgrade, mp_obj_new_int(ota_package_info->res));
    }else{
...
static void ota_install_handler(void *pdata)
{
    ...
    res = ota_install_pyapp(&customer_ota_ctx, ota_package_info->store_path, ota_package_info->length, ota_package_info->install_path);
    if (res < 0) {
...
static void ota_verify_notify(void *pdata)
{
    ...
    if(mp_obj_is_fun(ota_on_verify)){
        callback_to_python(ota_on_verify, mp_obj_new_int(ota_package_info->res));
    }else{
...
static void ota_download_notify(void *pdata)
{
    ...
    if(mp_obj_is_fun(ota_on_download)){
        callback_to_python(ota_on_download, mp_obj_new_int(ota_package_info->res));
    }else{
...
static mp_obj_t ota_report(mp_obj_t data)
{
    ...
    iot_device_handle = MP_OBJ_TO_PTR(mp_obj_dict_get(data,index));
...
static void ota_trigger_notify(void *pdata)
{
    ...
    mp_obj_dict_store(MP_OBJ_FROM_PTR(dict),mp_obj_new_str("length",6),mp_obj_new_int(ota_package_info->length));
    mp_obj_dict_store(MP_OBJ_FROM_PTR(dict),mp_obj_new_str("module_name",11),mp_obj_new_str(ota_package_info->module_name, strlen(ota_package_info->module_name)));
    mp_obj_dict_store(MP_OBJ_FROM_PTR(dict),mp_obj_new_str("version",7),mp_obj_new_str(ota_package_info->version, strlen(ota_package_info->version)));
    mp_obj_dict_store(MP_OBJ_FROM_PTR(dict),mp_obj_new_str("url",3),mp_obj_new_str(ota_package_info->url, strlen(ota_package_info->url)));
    mp_obj_dict_store(MP_OBJ_FROM_PTR(dict),mp_obj_new_str("hash",4),mp_obj_new_str(ota_package_info->hash, strlen(ota_package_info->hash)));
    ...
    mp_obj_dict_store(MP_OBJ_FROM_PTR(dict),mp_obj_new_str("hash_type",9),mp_obj_new_str(hash_type, strlen(hash_type)));

    amp_debug(MOD_STR,"module_name is %s  ; length is %d",ota_package_info->module_name,strlen(ota_package_info->module_name));
    amp_debug(MOD_STR,"version is %s ;  length is %d",ota_package_info->version,strlen(ota_package_info->version));
...
static int32_t customer_upgrade_cb(void *pctx, char *ver, char *module_name, void *args)
{
    ...
    if (strncmp(module_name, "system", strlen(module_name)) == 0) {
        ...
        py_app_version_get(current_ver);
...
static mp_obj_t ota_init(mp_obj_t data)
{
    ...
    */
    ...
    ota_register_trigger_msg_cb(&customer_ota_ctx, (void *)customer_upgrade_cb, 1);
