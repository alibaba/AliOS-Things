...
static aos_sem_t g_iot_connect_sem = NULL;
...
static void subcribe_cb(void *handle, const aiot_mqtt_recv_t *packet, void *userdata)
{
    ...
    case AIOT_MQTTRECV_PUB:
        ...
        mp_obj_dict_store(MP_OBJ_FROM_PTR(ret_dict),
                            mp_obj_new_str("topic", 5),
                            mp_obj_new_str(packet->data.pub.topic,  packet->data.pub.topic_len));
        mp_obj_dict_store(
            MP_OBJ_FROM_PTR(ret_dict), mp_obj_new_str("payload", 7),
            mp_obj_new_str(packet->data.pub.payload, packet->data.pub.payload_len));
        if (mp_obj_is_fun(iot_device_handle->callback[ON_SUBCRIBE])) {
            callback_to_python(iot_device_handle->callback[ON_SUBCRIBE], ret_dict);
        } else {
            amp_error(MOD_STR,
    ...
    if (param->dm_recv) {
        ...
        case AIOT_DMRECV_PROPERTY_SET:
            ...
            mp_obj_dict_store(MP_OBJ_FROM_PTR(dict),
                              mp_obj_new_str("msg_id", 6),
                              mp_obj_new_int(param->msg_id));
            mp_obj_dict_store(MP_OBJ_FROM_PTR(dict),
                              mp_obj_new_str("params_len", 10),
                              mp_obj_new_int(param->params_len));
            mp_obj_dict_store(MP_OBJ_FROM_PTR(dict),
                              mp_obj_new_str("params", 6),
                              mp_obj_new_str(param->params, param->params_len));
            if (mp_obj_is_fun(param->iot_device_handle->callback[ON_PROPS])) {
                callback_to_python(param->iot_device_handle->callback[ON_PROPS],
                                   dict);
            } else {
                amp_error(MOD_STR,
                          "param->iot_device_handle->cb_on_connected is "
                          "not function");
            }
        ...
        case AIOT_DMRECV_ASYNC_SERVICE_INVOKE:
            ...
            mp_obj_dict_store(MP_OBJ_FROM_PTR(dict),
                              ...
                              mp_obj_new_int(param->msg_id));
            mp_obj_dict_store(
                MP_OBJ_FROM_PTR(dict), mp_obj_new_str("service_id", 6),
            ...
            mp_obj_dict_store(MP_OBJ_FROM_PTR(dict),
                              mp_obj_new_str("params", 6),
                              mp_obj_new_str(param->params, param->params_len));
            if (mp_obj_is_fun(param->iot_device_handle->callback[ON_SERVICE])) {
                callback_to_python(
                    param->iot_device_handle->callback[ON_SERVICE], dict);
            } else {
                amp_error(MOD_STR,
                          "param->iot_device_handle->cb_on_connected is "
                          "not function");
        ...
        case AIOT_DMRECV_RAW_DATA:
            ...
            mp_obj_dict_store(MP_OBJ_FROM_PTR(dict), mp_obj_new_str("params", 6), mp_obj_new_bytearray(param->params_len, param->params));
            if (mp_obj_is_fun(param->iot_device_handle->callback[ON_RAWDATA]))
                callback_to_python(param->iot_device_handle->callback[ON_RAWDATA], dict);
        ...
        case AIOT_MQTTEVT_DISCONNECT:
            ...
            mp_obj_dict_store(dict, mp_obj_new_str("code", 4),
                            mp_obj_new_int(param->ret_code));
            if (mp_obj_is_fun(param->iot_device_handle->callback[param->cb_type])) {
                callback_to_python(param->iot_device_handle->callback[param->cb_type],
                                dict);
            } else {
                amp_error(MOD_STR,
                        "param->iot_device_handle->cb_on_connected is "
...
/* 用户数据接收处理回调函数 */
static void aiot_app_dm_recv_handler(void *dm_handle,
                                     const aiot_dm_recv_t *recv, void *userdata)
{
    iot_device_handle_t *iot_device_handle = (iot_device_handle_t *)userdata;
    ...
    case AIOT_DMRECV_PROPERTY_SET:
        {
            amp_error(MOD_STR, "msg_id = %ld, params = %.*s",
    ...
    case AIOT_DMRECV_ASYNC_SERVICE_INVOKE:
        {
            amp_error(MOD_STR, "msg_id = %ld, service_id = %s, params = %.*s",
            ...
            param->service_id =
                __amp_strdup(recv->data.async_service_invoke.service_id);
            param->params =
                __amp_strdup(recv->data.async_service_invoke.params);
            param->cb_type = AIOT_DEV_JSCALLBACK_ONPROPS_REF ;
    ...
    case AIOT_DMRECV_RAW_SYNC_SERVICE_INVOKE:
        {
            amp_error(MOD_STR, "raw sync service rrpc_id = %s, data_len = %d",
...
static void aiot_mqtt_message_cb(iot_mqtt_message_t *message, void *userdata)
{
    ...
    amp_error(MOD_STR, "message->option is %d  ", message->option);

    if (message->option == AIOT_MQTTOPT_EVENT_HANDLER) {
        amp_error(MOD_STR, "message->event.type is %d  ", message->event.type);

        switch (message->event.type) {
        case AIOT_MQTTEVT_CONNECT:
            aos_sem_signal(&g_iot_connect_sem);
            aos_free(param);
            return;
        ...
        amp_debug(MOD_STR, "message->recv.type is %d  ", message->recv.type);
...
static void aiot_device_connect(void *pdata)
{
    ...
    amp_error(MOD_STR, "start mqtt connect task");
    ...
    res = pyamp_aiot_mqtt_client_start(&iot_device_handle->mqtt_handle,
                                       keepaliveSec, userdata);
    ...
    /* 配置MQTT实例句柄 */
    aiot_dm_setopt(iot_device_handle->dm_handle, AIOT_DMOPT_MQTT_HANDLE,
                   iot_device_handle->mqtt_handle);
    /* 配置消息接收处理回调函数 */
    aiot_dm_setopt(iot_device_handle->dm_handle, AIOT_DMOPT_RECV_HANDLER,
                   (void *)aiot_app_dm_recv_handler);
    /* 配置回调函数参数 */
    aiot_dm_setopt(iot_device_handle->dm_handle, AIOT_DMOPT_USERDATA,
                   iot_device_handle);
    ...
    aos_sem_wait(&g_iot_connect_sem, AOS_WAIT_FOREVER);
...
static mp_obj_t aiot_postRaw(mp_obj_t self_in, mp_obj_t data)
{
    ...
    if (!mp_obj_is_dict_or_ordereddict(data)) {
        LOGE(MOD_STR , "%s  data type error,param type must be dict \r\n", __func__);
    ...
    if (!bytearray) {
        LOGE(MOD_STR , "%s  params data type error,param data type must be bytearray \r\n", __func__);
        return mp_obj_new_int(-1);
    }
    ...
    if (res < STATE_SUCCESS)
        LOGE(MOD_STR, "property post failed, res:%d", res);
...
static mp_obj_t aiot_create_device(mp_obj_t self_in, mp_obj_t data)
{
    ...
    amp_info(MOD_STR,
             "productKey=%s deviceName=%s deviceSecret=%s keepaliveSec=%d\n",
    ...
    aos_kv_set(AMP_CUSTOMER_DEVICESECRET, deviceSecret, IOTX_DEVICE_SECRET_LEN,
               1);

    self->iot_device_handle->keepaliveSec = keepaliveSec;

    res = aos_task_new_ext(&iot_device_task, "amp aiot device task",
                           aiot_device_connect, self->iot_device_handle,
...
static mp_obj_t aiot_postEvent(mp_obj_t self_in, mp_obj_t data)
{
    ...
    res = pyamp_aiot_app_send_event_post(
        self->iot_device_handle->dm_handle, event_id, event_payload);
    if (res < STATE_SUCCESS) {
...
static mp_obj_t aiot_postProps(mp_obj_t self_in, mp_obj_t data)
{
    ...
    res = pyamp_aiot_app_send_property_post(
        self->iot_device_handle->dm_handle, payload_str);
    if (res < STATE_SUCCESS) {
        amp_error(MOD_STR, "property post failed, res:%d", res);
    }
...
static mp_obj_t aiot_publish(mp_obj_t self_in, mp_obj_t data)
{
    ...
    amp_error(MOD_STR, "publish topic: %s, payload: %s, payload_len is %d , qos is: %d", topic,
              payload, payload_len, qos);

    res = aiot_mqtt_pub(self->iot_device_handle->mqtt_handle, topic,
                        payload, payload_len, qos);
    if (res < STATE_SUCCESS) {
...
static mp_obj_t aiot_subscribe(mp_obj_t self_in, mp_obj_t data)
{
    ...
    res = aiot_mqtt_sub(self->iot_device_handle->mqtt_handle, topic,
                        subcribe_cb, (uint8_t)qos, self->iot_device_handle);
    if (res < STATE_SUCCESS) {
        amp_error(MOD_STR, "aiot app mqtt subscribe failed\n");
        goto out;
    } else {
        amp_error(MOD_STR, "subcribe topic: %s  succeed \r\n", topic);
    }
 ...
 **************************************************************************************/
static mp_obj_t aiot_close(mp_obj_t self_in)
{
    int res = -1;
    ...
    res = pyamp_aiot_mqtt_client_stop(
        &self->iot_device_handle->mqtt_handle);
    if (res < STATE_SUCCESS) {
...
MP_DEFINE_CONST_FUN_OBJ_1(native_aiot_close, aiot_close);
...
static mp_obj_t aiot_dynreg(mp_obj_t self_in, mp_obj_t data, mp_obj_t cb)
{
    ...
    aos_kv_set(AMP_CUSTOMER_PRODUCTKEY, productKey, IOTX_PRODUCT_KEY_LEN, 1);
    aos_kv_set(AMP_CUSTOMER_DEVICENAME, deviceName, IOTX_DEVICE_NAME_LEN, 1);
    aos_kv_set(AMP_CUSTOMER_PRODUCTSECRET, productSecret,
               IOTX_PRODUCT_SECRET_LEN, 1);
...
STATIC const mp_rom_map_elem_t linkkit_device_locals_dict_table[] = {
    ...
    { MP_OBJ_NEW_QSTR(MP_QSTR_unsubscribe),
      MP_ROM_PTR(&native_aiot_unsubscribe) },
    ...
    { MP_OBJ_NEW_QSTR(MP_QSTR_end), MP_ROM_PTR(&native_aiot_close) },
...
STATIC MP_DEFINE_CONST_DICT(linksdk_device_locals_dict,
                            linkkit_device_locals_dict_table);

STATIC void linksdk_device_print(const mp_print_t *print, mp_obj_t self_in,
                                 mp_print_kind_t kind)
{
}

STATIC mp_obj_t linksdk_device_make_new(const mp_obj_type_t *type,
                                        ...
                                        const mp_obj_t *args)
{
    ...
    iot_device_handle->callback = m_new(mp_obj_t, ON_CB_MAX);
    aos_sem_new(&g_iot_connect_sem, 0);
