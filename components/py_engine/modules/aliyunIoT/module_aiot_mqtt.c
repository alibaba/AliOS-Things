 ...
 * 且无对应用户回调处理时被调用 */
void pyamp_aiot_app_mqtt_recv_handler(void *handle,
    ...
    case AIOT_MQTTRECV_PUB:
        {
            ...
            if (udata && udata->callback) {
                ...
                message.recv.topic = __amp_strdup(packet->data.pub.topic,
                                                  packet->data.pub.topic_len);
                message.recv.payload = __amp_strdup(
                    packet->data.pub.payload, packet->data.pub.payload_len);
...
void pyamp_aiot_app_mqtt_event_handler(void *handle,
                                     ...
                                     iot_mqtt_userdata_t *userdata)
{
    ...
    aiot_mqtt_setopt(mqtt_handle, AIOT_MQTTOPT_RECV_HANDLER,
                     (void *)pyamp_aiot_app_mqtt_recv_handler);
    /* 配置MQTT事件回调函数 */
    aiot_mqtt_setopt(mqtt_handle, AIOT_MQTTOPT_EVENT_HANDLER,
                     (void *)pyamp_aiot_app_mqtt_event_handler);
