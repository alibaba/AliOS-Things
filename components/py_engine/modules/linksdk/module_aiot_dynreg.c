...
void pyamp_aiot_app_dynreg_recv_handler(void *handle, const aiot_dynreg_recv_t *packet, void *userdata)
{
    ...
    else if (packet->type == AIOT_DYNREGRECV_DEVICE_INFO)
    {
        ...
        if(mp_obj_is_fun(cb)){
             callback_to_python(cb, mp_obj_new_str(packet->data.device_info.device_secret,strlen(packet->data.device_info.device_secret)));
        }else{
...
int32_t pyamp_aiot_dynreg_http(mp_obj_t cb)
{
    ...
    if (0 != res || (device_secret[0] == '\0' || device_secret[IOTX_DEVICE_SECRET_LEN] != '\0'))
    {
