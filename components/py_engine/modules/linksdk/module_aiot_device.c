...
static void aiot_device_notify(void *pdata)
{
    ...
    if (param->dm_recv) {
        ...
        case AIOT_DMRECV_PROPERTY_SET:
            ...
            if(mp_obj_is_fun(param->iot_device_handle->cb_on_property_set)){
                callback_to_python(param->iot_device_handle->cb_on_property_set, dict);
            }else{
        ...
        case AIOT_MQTTEVT_DISCONNECT:
            ...
            mp_obj_dict_store(MP_OBJ_FROM_PTR(dict),mp_obj_new_str("handle",6),MP_OBJ_FROM_PTR(param->iot_device_handle));
            if(mp_obj_is_fun(param->iot_device_handle->cb_on_connected)){
                callback_to_python(param->iot_device_handle->cb_on_connected, dict);
            }else{
                amp_error(MOD_STR,"param->iot_device_handle->cb_on_connected is not function");
            }
            break;
        ...
        case AIOT_MQTTRECV_PUB:
            ...
            mp_obj_dict_store(MP_OBJ_FROM_PTR(dict),mp_obj_new_str("code",4),mp_obj_new_int(param->ret_code));
            mp_obj_dict_store(MP_OBJ_FROM_PTR(dict),mp_obj_new_str("handle",6),MP_OBJ_FROM_PTR(param->iot_device_handle));
            mp_obj_dict_store(MP_OBJ_FROM_PTR(dict),mp_obj_new_str("topic",5),mp_obj_new_str(param->topic, param->topic_len));
            mp_obj_dict_store(MP_OBJ_FROM_PTR(dict),mp_obj_new_str("payload",7),mp_obj_new_str(param->payload,param->payload_len));
            /*to do*/
...
static void aiot_mqtt_message_cb(iot_mqtt_message_t *message, void *userdata)
{
    ...
    if (message->option == AIOT_MQTTOPT_EVENT_HANDLER) {
        ...
        amp_error(MOD_STR, "message->recv.type is %d  ",message->recv.type);
...
static mp_obj_t aiot_create_device(mp_obj_t self_in, mp_obj_t data)
{
    ...
    self->device->iot_device_handle->keepaliveSec = keepaliveSec;


    res = aos_task_new_ext(&iot_device_task, "amp aiot device task", aiot_device_connect, self->device->iot_device_handle, 1024 * 10, AOS_DEFAULT_APP_PRI);
    if (res != STATE_SUCCESS) {
...
static mp_obj_t aiot_postProps(mp_obj_t self_in, mp_obj_t data)
{
    ...
    if(self->device->iot_device_handle->dm_handle == NULL){
...
static mp_obj_t aiot_publish(mp_obj_t self_in, mp_obj_t data)
{
    ...
    res = aiot_mqtt_pub(self->device->iot_device_handle->mqtt_handle, topic, payload, payload_len, qos);
    if (res < STATE_SUCCESS) {
...
static mp_obj_t aiot_unsubscribe(mp_obj_t self_in, mp_obj_t data)
{
    ...
    res = aiot_mqtt_unsub(self->device->iot_device_handle->mqtt_handle, topic);
    if (res < STATE_SUCCESS) {
...
static mp_obj_t aiot_subscribe(mp_obj_t self_in, mp_obj_t data)
{
    ...
    res = aiot_mqtt_sub(self->device->iot_device_handle->mqtt_handle, topic, NULL, (uint8_t)qos, NULL);
    if (res < STATE_SUCCESS) {
...
STATIC mp_obj_t aiot_register_cb(mp_obj_t self_in,mp_obj_t id,mp_obj_t func) {
    ...
    switch(callback_id){
        case ITE_CONNECT_SUCC:
            self->device->iot_device_handle->cb_on_connected = func;

            break;
        case ITE_DISCONNECTED:
            break;
        case ITE_SERVICE_REQUEST:
            self->device->iot_device_handle->cb_on_async_service = func;

            break;
        case ITE_PROPERTY_SET:
            self->device->iot_device_handle->cb_on_property_set = func;
            break;
        case ITE_REPORT_REPLY:
            self->device->iot_device_handle->cb_on_report_reply = func;
            break;
        case ITE_TRIGGER_EVENT_REPLY:
            self->device->iot_device_handle->cb_on_event_reply =  func;
            break;
...
STATIC void linksdk_device_print(const mp_print_t *print, mp_obj_t self_in,
    ...
    mp_printf(print, "py_cb_ref is =%d, ", self->device->py_cb_ref);
}

STATIC mp_obj_t linksdk_device_make_new(const mp_obj_type_t *type, size_t n_args,
    ...
    device_obj->device->iot_device_handle  = iot_device_handle ;
