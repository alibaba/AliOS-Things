...
STATIC mp_obj_t aiagent_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *args)
{
...
void aiagent_print(const mp_print_t *print, mp_obj_t self_in, mp_print_kind_t kind)
{
...
STATIC mp_obj_t aiagent_infer(size_t n_args, const mp_obj_t *args)
{
    ...
    AIAGENT_CHECK_PARAMS();
    self->callback = (mp_obj_get_type(args[3]) == &mp_type_NoneType) ? NULL : args[3];
    self->engine_obj->src1 = mp_obj_is_str(args[1]) ? (char *) mp_obj_str_get_str(args[1]) : NULL;
    self->engine_obj->src1 = mp_obj_is_str(args[2]) ? (char *) mp_obj_str_get_str(args[2]) : NULL;
