...
int32_t pyamp_httpc_construct_header(char *buf, uint16_t buf_size,
...
static void task_http_request_func(void *arg)
{
    ...
    for (i = 0; i < http_header_index; i++) {
        pyamp_httpc_construct_header(customer_header, HTTP_HEADER_SIZE,
                                     param->http_header[i].name,
    ...
    if (http_method == HTTP_GET) {
        ...
        ret = httpclient_post(
            &client,
            "https://www.ixigua.com/tlb/comment/article/v5/tab_comments/",
            &client_data);
        if (ret >= 0) {
...
static duk_ret_t native_http_download(duk_context *ctx)
{
    ...
    aos_task_new_ext(&http_task, "amp http download task",
                     task_http_download_func, http_param, 1024 * 8,
                     ADDON_TSK_PRIORRITY);
...
static duk_ret_t native_http_request(duk_context *ctx)
{
    ...
    if (duk_get_prop_string(ctx, 0, "headers")) {
        ...
        while (duk_next(ctx, -1, 1)) {
            ...
            if (!duk_is_string(ctx, -2) || !duk_is_string(ctx, -1)) {
                amp_debug(MOD_STR, "get header failed, index is: %d",
                          http_header_index);
                break;
            }
            http_param->http_header[http_header_index].name =
                duk_to_string(ctx, -2);
            http_param->http_header[http_header_index].data =
                duk_to_string(ctx, -1);
            http_header_index++;
            duk_pop_2(ctx);
        }
    ...
    for (i = 0; i < http_header_index; i++) {
        amp_debug(MOD_STR, "headers: %s:%s", http_param->http_header[i].name,
    ...
    aos_task_new_ext(&http_task, "amp http task", task_http_request_func,
                     http_param, 1024 * 8, ADDON_TSK_PRIORRITY);
