...
static mp_obj_t on_service_request ;
    ...
    switch (recv->type) {
        ...
        case AIOT_DMRECV_PROPERTY_SET: {
            ...
            if(mp_obj_is_fun(on_property_set)){
                ...
                printf(" **********on_property_set is function ******** \r\n");
...
//     if(retry_count  < RETRY_NUM  && status != 1 ){
...
//             retry_count++;
//             if(retry_count == 0 ){
//                 printf("IOT_Linkkit_Open failed for %d times !\n", RETRY_NUM);
//             }
//             printf("IOT_Linkkit_Open failed! retry  %d  times after %d ms\n",(retry_count+1) ,2000);
//             HAL_SleepMs(2000);
...
STATIC mp_obj_t post_property(mp_obj_t data) {
    ...
    //size_t ret = user_post_property(payload,len);
...
STATIC mp_obj_t post_event(mp_obj_t eventid,mp_obj_t data) {
    ...
    //size_t ret = user_post_event(event_id,id_len,payload,payload_len);
...
STATIC mp_obj_t linkkit_yield(mp_obj_t time) {
    // size_t timeout = mp_obj_get_int(time);
