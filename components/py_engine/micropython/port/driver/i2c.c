...
out:
	if (0 != ret) {
        LOG_E("%s: amp_hal_i2c_init failed ret = %d;\n", __func__, ret);
...
STATIC mp_obj_t obj_read(size_t n_args, const mp_obj_t *args)
{
    ...
    ret = amp_hal_i2c_master_recv(i2c_device, i2c_device->config.dev_addr,
                                  bufinfo.buf, bufinfo.len, I2C_TIMEOUT);
    if (ret !=0 )
    {
        LOG_E("amp_hal_i2c_master_recv failed\n");
    }
    LOG_D("%s:out\n", __func__);
...
STATIC mp_obj_t obj_write(mp_obj_t self_in, mp_obj_t write_buf)
{
    ...
    ret = amp_hal_i2c_master_send(i2c_device, i2c_device->config.dev_addr,
                                  src.buf, src.len, I2C_TIMEOUT);
    if (ret != 0)
	{
		LOG_E("%s: amp_hal_i2c_master_send failed;\n", __func__);
        //return mp_const_none;
	}
    LOG_D("%s:out\n", __func__);
...
STATIC mp_obj_t obj_readReg(size_t n_args, const mp_obj_t *args)
{
    ...
    if (ret !=0 )
    {
        LOG_E("amp_hal_i2c_mem_read failed\n");
    }
    LOG_D("%s:out\n", __func__);
...
STATIC mp_obj_t obj_writeReg(size_t n_args, const mp_obj_t *args)
{
    ...
    if (ret != 0)
	{
        LOG_E("%s:amp_hal_i2c_mem_write failed\n", __func__);
	}
    LOG_D("%s:out\n", __func__);
