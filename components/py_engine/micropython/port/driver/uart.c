...
out:
	if (0 != ret) {
        LOG_E("%s: amp_hal_uart_init failed ret = %d;\n", __func__, ret);
...
STATIC mp_obj_t obj_close(size_t n_args, const mp_obj_t *args)
{
    ...
    if (ret != 0)
    {
        LOG_E("amp_hal_uart_finalize failed\n");
    }
...
STATIC mp_obj_t obj_read(size_t n_args, const mp_obj_t *args)
{
    ...
    ret = amp_hal_uart_recv_II(uart_device, bufinfo.buf, bufinfo.len, &recvsize, 0);
    if (recvsize <= 0)
    {
        LOG_E("%s:amp_hal_uart_recv_II failed\n", __func__);
    }
...
STATIC mp_obj_t obj_write(size_t n_args, const mp_obj_t *args)
{
    ...
    ret  = amp_hal_uart_send(uart_device, bufinfo.buf, bufinfo.len, UART_TIMEOUT);
    if (ret == -1)
    {
        LOG_E("amp_hal_uart_send failed\n");
    }
    LOG_D("%s:out\n", __func__);
