...
out:
	if (0 != ret) {
        LOG_E("%s: amp_hal_spi_init failed ret = %d;\n", __func__, ret);
...
STATIC mp_obj_t obj_close(size_t n_args, const mp_obj_t *args)
{
    ...
    if (ret != 0) {
		LOG_E("%s: amp_hal_spi_finalize failed;\n", __func__);
    }
...
STATIC mp_obj_t obj_read(size_t n_args, const mp_obj_t *args)
{
    ...
    ret = amp_hal_spi_recv(spi_device, bufinfo.buf, bufinfo.len, SPI_TIMEOUT);
    if (ret == -1)
    {
		LOG_E("%s: amp_hal_spi_recv failed;\n", __func__);
    }
    LOG_D("%s:out\n", __func__);
...
STATIC mp_obj_t obj_write(size_t n_args, const mp_obj_t *args)
{
    ...
    ret = amp_hal_spi_send(spi_device, bufinfo.buf, bufinfo.len, SPI_TIMEOUT);
    if (ret == -1)
    {
		LOG_E("%s: amp_hal_spi_send failed;\n", __func__);
    }
    LOG_D("%s:out\n", __func__);
...
STATIC mp_obj_t obj_sendRecv(size_t n_args, const mp_obj_t *args)
{
    ...
    ret = amp_hal_spi_send_and_recv(spi_device, write_bufinfo.buf,
           write_bufinfo.len, read_bufinfo.buf, read_bufinfo.len, SPI_TIMEOUT);
    if (ret == -1)
    {
        LOG_E("amp_hal_spi_send_recv failed\n");
    }
    LOG_D("%s:out\n", __func__);
