...
STATIC mp_obj_t obj_close(size_t n_args, const mp_obj_t *args)
{
    ...
    ret = amp_hal_rtc_finalize(&(driver_obj->rtc_dev));
    if (ret != 0)
    {
        LOG_E("amp_hal_rtc_finalize fail!");
    }
    LOG_D("%s:out\n", __func__);
...
STATIC mp_obj_t obj_getTime(size_t n_args, const mp_obj_t *args)
{
    ...
    ret = amp_hal_rtc_get_time(&(driver_obj->rtc_dev), &rtcTime);
    if (ret != 0)
    {
        LOG_E("amp_hal_rtc_get_time failed\n");
        return mp_const_none;
    }
    ...
    mp_obj_dict_store(MP_OBJ_FROM_PTR(dict),mp_obj_new_str("month",4),mp_obj_new_int((uint32_t)rtcTime.month));
    mp_obj_dict_store(MP_OBJ_FROM_PTR(dict),mp_obj_new_str("date",4),mp_obj_new_int((uint32_t)rtcTime.date));
    mp_obj_dict_store(MP_OBJ_FROM_PTR(dict),mp_obj_new_str("hr",4),mp_obj_new_int((uint32_t)rtcTime.hr));
    mp_obj_dict_store(MP_OBJ_FROM_PTR(dict),mp_obj_new_str("min",4),mp_obj_new_int((uint32_t)rtcTime.min));
    mp_obj_dict_store(MP_OBJ_FROM_PTR(dict),mp_obj_new_str("sec",4),mp_obj_new_int((uint32_t)rtcTime.sec));
...
STATIC mp_obj_t obj_setTime(size_t n_args, const mp_obj_t *args)
{
    ...
    ret = amp_hal_rtc_set_time(&(driver_obj->rtc_dev), &rtcTime);
    if (ret != 0)
    {
        LOG_E("amp_hal_rtc_get_time failed\n");
    }
