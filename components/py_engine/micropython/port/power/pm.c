...
void pm_obj_print(const mp_print_t *print, mp_obj_t self_in, mp_print_kind_t kind)
{
    LOG_D("entern %s;\n", __func__);
    mp_pm_obj_t *self = MP_OBJ_TO_PTR(self_in);
    mp_printf(print, "ModuleName(%s)", self->ModuleName);
}

STATIC mp_obj_t pm_obj_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *args)
{
    LOG_D("entern  %s;\n", __func__);
    mp_pm_obj_t* driver_obj = m_new_obj(mp_pm_obj_t);
    if (!driver_obj) {
        mp_raise_OSError(ENOMEM);
    }

    driver_obj->Base.type = &power_pm_type;
    driver_obj->ModuleName = "pm";
...
STATIC mp_obj_t obj_open(size_t n_args, const mp_obj_t *args)
{
    ...
    mp_pm_obj_t* driver_obj = (mp_pm_obj_t *)self;
    if (driver_obj == NULL)
    {
...
const mp_obj_type_t power_pm_type = {
    .base = {&mp_type_type},
    .name = MP_QSTR_PM,
    .print = pm_obj_print,
    .make_new = pm_obj_make_new,
    .locals_dict = (mp_obj_dict_t *)&pm_locals_dict,
};
