...
void aiot_app_dynreg_recv_handler(void *handle, const aiot_dynreg_recv_t *packet, void *userdata)
{
    ...
    else if (packet->type == AIOT_DYNREGRECV_DEVICE_INFO)
    {
        ...
        amp_kv_set(AMP_CUSTOMER_DEVICESECRET, packet->data.device_info.device_secret, strlen(packet->data.device_info.device_secret), 1);
...
int32_t aiot_dynreg_http(int js_cb_ref)
{
    ...
    amp_kv_get(AMP_CUSTOMER_PRODUCTSECRET, product_secret, &productsecret_len);
