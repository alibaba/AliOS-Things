...
void aiot_app_mqtt_recv_handler(void *handle, const aiot_mqtt_recv_t *packet, void *userdata)
{
    ...
    //             duk_push_object(ctx);
    //             {
    //                 duk_push_int(ctx, AIOT_MQTT_MESSAGE);
    //                 duk_put_prop_string(ctx, -2, "code");
    //                 duk_push_string(ctx, packet->data.pub.topic);
    //                 duk_put_prop_string(ctx, -2, "topic");
    //                 duk_push_string(ctx, packet->data.pub.payload);
...
void aiot_app_mqtt_event_handler(void *handle, const aiot_mqtt_event_t *event, void *userdata)
{
    ...
    //         duk_push_object(ctx);
    //         {
    //             duk_push_int(ctx, AIOT_MQTT_CONNECT);
    //             duk_put_prop_string(ctx, -2, "code");
    //         }
    //     }
    //     break;
...
int32_t aiot_mqtt_client_start(void **handle, int keepaliveSec, int js_cb_ref)
{
    ...
    // amp_kv_get(AMP_CUSTOMER_PRODUCTKEY, product_key, &productkey_len);
    // amp_kv_get(AMP_CUSTOMER_DEVICENAME, device_name, &devicename_len);
    // amp_kv_get(AMP_CUSTOMER_DEVICESECRET, device_secret, &devicesecret_len);
    ...
    // int stack_used;
    // amp_os_thread_param_t mqtt_process_task_params = {0};

    // mqtt_process_task_params.priority = amp_get_default_task_priority();
    // mqtt_process_task_params.stack_size = 1024 * 4;
    // mqtt_process_task_params.name = "mqtt_process";

    // if (amp_thread_create(&mqtt_process_task, aiot_app_mqtt_process_thread, mqtt_handle, &mqtt_process_task_params, &stack_used) != 0) {
    ...
    // void *mqtt_rec_task;
    // amp_os_thread_param_t mqtt_recv_task_params = {0};
    // mqtt_recv_task_params.priority = amp_get_default_task_priority();
    // mqtt_recv_task_params.stack_size = 1024 * 4;
    // mqtt_recv_task_params.name = "mqtt_rec";

    // if (amp_thread_create(&mqtt_rec_task, aiot_app_mqtt_recv_thread, mqtt_handle, &mqtt_recv_task_params, &stack_used) != 0) {
