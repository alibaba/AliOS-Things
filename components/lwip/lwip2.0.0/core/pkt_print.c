...
/* pkt info stats */
struct pkt_stats_data g_stats_data = {0};
...
void lwip_pkt_print(char* note_ptr, void *buf, void* net)
{
    ...
    if(strcmp(note_ptr, "LwIP_send") == 0) {
        direction = DIR_OUTGOING;
    } else if(strcmp(note_ptr, "LwIP_recv") == 0) {
                 ...
                 switch( dbg_pkt_info.trans )
                 {
                    case TL_ICMP:
                    {
                       ...
                       if(direction == DIR_OUTGOING) {
                           g_stats_data.tx_packets ++;
                           g_stats_data.tx_bytes += ip4_ptr->len;
                           g_stats_data.tx_other_packets ++;
                           g_stats_data.tx_other_bytes += ip4_ptr->len;
                       } else if(direction == DIR_INCOMING) {
                           g_stats_data.rx_packets ++;
                           g_stats_data.rx_bytes += ip4_ptr->len;
                           g_stats_data.rx_other_packets ++;
                           g_stats_data.rx_other_bytes += ip4_ptr->len;
                       }
                    }
                    break;
                    case TL_IGMP:
                    ...
                    case TL_TCP:
                    {
                       ...
                       if(direction == DIR_OUTGOING) {
                           g_stats_data.tx_packets ++;
                           g_stats_data.tx_bytes += ip4_ptr->len;
                           g_stats_data.tx_tcp_packets ++;
                           g_stats_data.tx_tcp_bytes += ip4_ptr->len;
                       } else if(direction == DIR_INCOMING) {
                           g_stats_data.rx_packets ++;
                           g_stats_data.rx_bytes += ip4_ptr->len;
                           g_stats_data.rx_tcp_packets ++;
                           g_stats_data.rx_tcp_bytes += ip4_ptr->len;
                       }
                    }
                    break;
                    case TL_UDP:
                    {
                       ...
                       if(direction == DIR_OUTGOING) {
                           g_stats_data.tx_packets ++;
                           g_stats_data.tx_bytes += ip4_ptr->len;
                           g_stats_data.tx_udp_packets ++;
                           g_stats_data.tx_udp_bytes += ip4_ptr->len;
                       } else if(direction == DIR_INCOMING) {
                           g_stats_data.rx_packets ++;
                           g_stats_data.rx_bytes += ip4_ptr->len;
                           g_stats_data.rx_udp_packets ++;
                           g_stats_data.rx_udp_bytes += ip4_ptr->len;
                       }
                    }
                    break;
                    default:
...
void lwip_pkt_stats(pkt_stats_data *data) {
    if(data != NULL) {
        memcpy(data, &g_stats_data, sizeof(g_stats_data));
    }
}
...
void pktprint_command( char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv )
{
    ...
    if ( strcmp( argv[1], "-h" ) == 0 ) {
        ...
        LWIP_DEBUGF(PKTPRINT_DEBUG, ("rx_bytes    rx_packets    \
tx_bytes    tx_packets    rx_tcp_bytes    rx_tcp_packets    rx_udp_bytes   rx_udp_packets    rx_other_bytes    rx_other_packets    tx_tcp_bytes   tx_tcp_packets    tx_udp_bytes    tx_udp_packets    tx_other_bytes    tx_other_packets\n"));
        LWIP_DEBUGF(PKTPRINT_DEBUG, ("%lld    %lld    %lld    %lld    %lld    %lld    %lld    %lld    %lld    %lld    %lld    %lld    %lld    %lld    %lld    %lld\n",
                g_stats_data.rx_bytes, g_stats_data.rx_packets, g_stats_data.tx_bytes, g_stats_data.tx_packets,
                g_stats_data.rx_tcp_bytes, g_stats_data.rx_tcp_packets, g_stats_data.rx_udp_bytes,
                g_stats_data.rx_udp_packets, g_stats_data.rx_other_bytes, g_stats_data.rx_other_packets,
                g_stats_data.tx_tcp_bytes, g_stats_data.tx_tcp_packets, g_stats_data.tx_udp_bytes,
                g_stats_data.tx_udp_packets, g_stats_data.tx_other_bytes, g_stats_data.tx_other_packets));
    }
    else {
            LWIP_DEBUGF(PKTPRINT_DEBUG, ("Invalid command, please use pktprint -h\n"));
    }
