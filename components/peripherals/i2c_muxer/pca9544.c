...
int pca9544_write_reg(uint16_t dev_addr, uint8_t regval)
{
    ...
    ret = hal_i2c_master_send(i2c, dev_addr, &regval, sizeof(regval), timeout);
    if(ret)
    {
        ...
        uint16_t size,  uint8_t* data, uint16_t len)
{
    ...
    ret = hal_i2c_mem_write(i2c, dev_addr, mem_addr, mem_addr_size, &data[0], len, timeout);
    if(ret)
    {
        ...
        uint16_t size,  uint8_t* data, uint16_t len)
{
    ...
    ret = hal_i2c_mem_read(i2c, dev_addr, mem_addr, mem_addr_size, &data[0], len, timeout);
    if(ret)
    {
