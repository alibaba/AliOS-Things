...
uint8_t MPU_Write_Byte(uint8_t reg, uint8_t data)
{
	...
	write_buf[0] = reg;
	write_buf[1] = data;
    hal_i2c_master_send(&i2c_dev, i2c_dev.config.dev_addr, write_buf, 2, 1000);
	return 0;
}
...
void MPU_Write_Len(uint8_t reg, uint8_t len,uint8_t *buf)
{
	...
	if((len > 100) || (buf == NULL))	return;

	write_buf[0] = reg;
	memcpy(&write_buf[1],buf,len);
    hal_i2c_master_send(&i2c_dev, i2c_dev.config.dev_addr, write_buf, len + 1, 1000);
}
...
void MPU_Read_Len(uint8_t reg, uint8_t len, uint8_t *buf)
{
    hal_i2c_master_send(&i2c_dev, i2c_dev.config.dev_addr, &reg, 1, 1000);
    aos_msleep(2);
    hal_i2c_master_recv(&i2c_dev, i2c_dev.config.dev_addr, buf, len, 1000);
}
...
uint8_t MPU_Init(void)
{
    ...
    if(device_id == MPU_DEV_ID)//器件ID正确
    {
