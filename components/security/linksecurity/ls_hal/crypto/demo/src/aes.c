...
#if !defined(CONFIG_AES_ROM_TABLES_NO)
/*
 * Forward S-box
 */
...
#endif /* CONFIG_AES_ROM_TABLES_NO */

void impl_aes_init(impl_aes_context *ctx)
{
    memset(ctx, 0, sizeof(impl_aes_context));
}
...
void impl_aes_free(impl_aes_context *ctx)
{
    ...
    zeroize(ctx, sizeof(impl_aes_context));
}
                      ...
                      unsigned int keybits)
{
    ...
    /* Also checks keybits */
    if ((ret = impl_aes_setkey_enc(&cty, key, keybits)) != 0)
        goto exit;
 ...
 */
int impl_aes_crypt_ecb(impl_aes_context *ctx, int mode,
                     const unsigned char input[16],
                     unsigned char       output[16])
{
    if (mode == HAL_AES_ENCRYPT)
        impl_aes_encrypt(ctx, input, output);
    else
        impl_aes_decrypt(ctx, input, output);
    return (0);
}
 ...
 */
int impl_aes_crypt_cbc(impl_aes_context *ctx, int mode, size_t length,
                     ...
                     unsigned char *output)
{
    ...
    if (mode == HAL_AES_DECRYPT) {
        while (length > 0) {
            memcpy(temp, input, 16);
            impl_aes_crypt_ecb(ctx, mode, input, output);
        ...
        while (length > 0) {
            ...
            impl_aes_crypt_ecb(ctx, mode, output, output);
            memcpy(iv, output, 16);
 ...
 */
int impl_aes_crypt_cfb128(impl_aes_context *ctx, int mode, size_t length,
                        ...
                        const unsigned char *input, unsigned char *output)
{
    ...
    if (mode == HAL_AES_DECRYPT) {
        while (length--) {
            if (n == 0)
                impl_aes_crypt_ecb(ctx, HAL_AES_ENCRYPT, iv, iv);
 ...
 */
int impl_aes_crypt_cfb8(impl_aes_context *ctx, int mode, size_t length,
                      unsigned char iv[16], const unsigned char *input,
                      unsigned char *output)
{
    unsigned char c;
 ...
 */
int impl_aes_crypt_ctr(impl_aes_context *ctx, size_t length,
                     ...
                     const unsigned char *input, unsigned char *output)
{
    ...
    while (length--) {
        if (n == 0) {
            impl_aes_crypt_ecb(ctx, HAL_AES_ENCRYPT, nonce_counter,
                             stream_block);
