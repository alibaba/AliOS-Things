...
void awss_broadcast_enrollee_info(void)
{
    ...
    uint8_t g_manufact_ap_ssid[OS_MAX_SSID_LEN + 1] = "mercury_085";
    ...
    int i = 0;
    os_wifi_get_mac(mac);
    //haas200_wifi_get_mac_addr(&dev,mac);
    for( i =0 ;i < 6;i++) {
        probe[10+i] = mac[i];
        printf("%02X ",mac[i]);
    }
...
int awss_recv_callback_zconfig(struct parser_res *res)
{
    ...
    printf("ssid:%s password:%s bssid:%s\r\n", zc_ssid, zc_passwd, zc_bssid);
