...
{
	struct soc_mixer_control *private_value = NULL;
	if (! kcontrol) {
		return -EINVAL;
	}
	...
	LOGD(LOG_TAG, "%s:%d, ----kcontrol->id.device/subdevice: %d/%d", __func__, __LINE__, kcontrol->id.device, kcontrol->id.subdevice);
	...
	private_value = (struct soc_mixer_control *)kcontrol->private_value;
	if(private_value) {
...
int audio_ctl_elem_info(ctrl_device_t *dev, struct audio_ctl_elem_info *info)
{
	...
	LOGD(LOG_TAG, "%s:%d, ----id->numid: %d", __func__, __LINE__, info->id.numid);
