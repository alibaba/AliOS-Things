                        ...
                        unsigned int period_count)
{
    ...
    buffer = malloc(size);
    if (!buffer) {
        LOGE(LOG_TAG, "%s:%d: Unable to allocate %d bytes", __func__, __LINE__, size);
        free(buffer);
        aos_pcm_close(pcm);
        return -1;
    }
    ...
    do {
        ...
        if (num_read > 0) {
            if (aos_pcm_writei(pcm, buffer, num_read/frame_size) < 0) {
                LOGE(LOG_TAG, "%s:%d: Error playing sample", __func__, __LINE__);
                break;
            }
        }
        //LOGE(LOG_TAG, "%s:%d: push %d bytes data to buf, sleep %d ms", __func__, __LINE__, byte_len, xbuf->frames * 1000 / pcm_dev->params.rate);
    } while (num_read > 0);
...
static void *js_audio_player_loop(void *arg)
{
    FILE *file;
    ...
    while(1) {
        if(total_cnt <= 0) {
            LOGD(LOG_TAG, "%s:%d, ------ waiting for start signal ------", __func__, __LINE__);
            pthread_cond_wait(&g_play_cond, &g_play_mutex);
        }
        LOGD(LOG_TAG, "%s:%d, ------ wav player begin %d ------", __func__, __LINE__, total_cnt);
        total_cnt --;
        ...
        if (!file) {
            LOGE(LOG_TAG, "%s:%d, Unable to open file '%s'", __func__, __LINE__, filename);
            total_cnt = 0;
            continue;
        }
...
static int createAudioPlayerThread(void)
{
    ...
    pthread_create(&g_play_thread, &attr, &js_audio_player_loop, NULL);
...
void cmd_play_wav_handler(char *pbuffer, int outlen, int argc, char **argv)
{
    if (argc < 3) {
...
void set_master_volume(char *pbuffer, int outlen, int argc, char **argv)
{
    int volume = 0;
    if (argc < 2) {
        LOGD(LOG_TAG, "%s:%d: Usage: %s volume ", __func__, __LINE__, argv[0]);
        return;
    }
    volume = atoi(argv[1]);
...
void get_master_volume(char *pbuffer, int outlen, int argc, char **argv)
{
    int volume = 0;
