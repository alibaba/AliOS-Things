...
void k_poll_event_init(struct k_poll_event *event, bt_u32_t type, int mode,
                       void *obj)
{
...
static inline void set_event_state(struct k_poll_event *event, bt_u32_t state)
{
    event->poller = NULL;
    event->state |= state;
}
...
static int _signal_poll_event(struct k_poll_event *event, bt_u32_t state, int *must_reschedule)
{
    *must_reschedule = 0;
...
void _handle_obj_poll_events(sys_dlist_t *events, bt_u32_t state)
{
...
static inline int is_condition_met(struct k_poll_event *event, bt_u32_t *state)
{
    switch (event->type) {
...
static bool polling_events(struct k_poll_event *events, int num_events,
                           bt_s32_t timeout, int *last_registered)
{
    bool polling = true;
    unsigned int key;
...
int k_poll(struct k_poll_event *events, int num_events, bt_s32_t timeout)
{
