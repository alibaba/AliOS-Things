...
static void fixed_data_unref(struct net_buf *buf, u8_t *data)
{
	...
	ref_count = (bt_u32_t *)(data - sizeof(*ref_count));
	if (--(*ref_count)) {
		return;
	}
...
#endif
{
	bt_u32_t alloc_start = k_uptime_get_32();
	struct net_buf *buf;
	unsigned int key;
...
success:
	...
	if (size) {
		if (timeout != K_NO_WAIT && timeout != K_FOREVER) {
			bt_u32_t diff = k_uptime_get_32() - alloc_start;
...
bt_u32_t net_buf_simple_pull_le32(struct net_buf_simple *buf)
{
	...
	val = UNALIGNED_GET((bt_u32_t *)buf->data);
	net_buf_simple_pull(buf, sizeof(val));
