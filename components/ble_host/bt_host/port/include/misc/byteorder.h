...
#define __bswap_24(x) ((bt_u32_t) ((((x) >> 16) & 0xff) | \
				   (((x)) & 0xff00) | \
				   (((x) & 0xff) << 16)))
#define __bswap_32(x) ((bt_u32_t) ((((x) >> 24) & 0xff) | \
				   (((x) >> 8) & 0xff00) | \
				   (((x) & 0xff00) << 8) | \
				   (((x) & 0xff) << 24)))
 ...
 *  @param dst Destination memory address to store the result.
 */
static inline void sys_put_be24(bt_u32_t val, u8_t dst[3])
{
	dst[0] = val >> 16;
 ...
 *  @param dst Destination memory address to store the result.
 */
static inline void sys_put_be32(bt_u32_t val, u8_t dst[4])
{
 ...
 *  @param dst Destination memory address to store the result.
 */
static inline void sys_put_le24(bt_u32_t val, u8_t dst[3])
{
 ...
 *  @param dst Destination memory address to store the result.
 */
static inline void sys_put_le32(bt_u32_t val, u8_t dst[4])
{
	sys_put_le16(val, dst);
 ...
 *  @return 24-bit integer in host endianness.
 */
static inline bt_u32_t sys_get_be24(const u8_t src[3])
{
	return ((bt_u32_t)src[0] << 16) | sys_get_be16(&src[1]);
}
 ...
 *  @return 32-bit integer in host endianness.
 */
static inline bt_u32_t sys_get_be32(const u8_t src[4])
{
	return ((bt_u32_t)sys_get_be16(&src[0]) << 16) | sys_get_be16(&src[2]);
}
 ...
 *  @return 24-bit integer in host endianness.
 */
static inline bt_u32_t sys_get_le24(const u8_t src[3])
{
	return ((bt_u32_t)src[2] << 16) | sys_get_le16(&src[0]);
}
 ...
 *  @return 32-bit integer in host endianness.
 */
static inline bt_u32_t sys_get_le32(const u8_t src[4])
{
	return ((bt_u32_t)sys_get_le16(&src[2]) << 16) | sys_get_le16(&src[0]);
}
