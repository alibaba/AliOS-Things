...
static int smp_g2(const u8_t u[32], const u8_t v[32],
		  const u8_t x[16], const u8_t y[16], bt_u32_t *passkey)
{
	u8_t m[80], xs[16];
	int err;
...
{
	u8_t *m = msg;
	bt_u32_t cnt = UNALIGNED_GET((bt_u32_t *)&msg[len]);
	u8_t *sig = msg + len;
...
{
	u8_t msg[len + sizeof(bt_u32_t) + 8];
	u8_t orig[len + sizeof(bt_u32_t) + 8];
	u8_t *out = msg + len;
	int err;
	...
	memcpy(msg, m, len);
	(void)memset(msg + len, 0, sizeof(bt_u32_t));
	...
	if (!memcmp(msg, orig, len + sizeof(bt_u32_t))) {
...
static int smp_g2_test(void)
{
	...
	u8_t y[16] = { 0xcf, 0xc4, 0x3d, 0xff, 0xf7, 0x83, 0x65, 0x21,
			  0x6e, 0x5f, 0xa7, 0x25, 0xcc, 0xe7, 0xe8, 0xa6 };
	bt_u32_t exp_val = 0x2f9ed5ba % 1000000;
	bt_u32_t val;
	int err;
