...
static void cind_handle_values(struct at_client *hf_at, bt_u32_t index,
			       char *name, bt_u32_t min, bt_u32_t max)
{
	struct bt_hfp_hf *hf = CONTAINER_OF(hf_at, struct bt_hfp_hf, at);
	int i;
...
void ag_indicator_handle_values(struct at_client *hf_at, bt_u32_t index,
				bt_u32_t value)
{
	struct bt_hfp_hf *hf = CONTAINER_OF(hf_at, struct bt_hfp_hf, at);
