...
#ifndef ENOTSUP
#define ENOTSUP 134 /* unsupported*/
#endif
...
static int le_adv_set_random_addr(struct bt_le_ext_adv *adv, bt_u32_t options,
			   ...
			   const struct bt_data *sd, size_t sd_len)
{
...
#endif
	if (err) {
        BT_ERR("hci adv param err = %d", err);
		return err;
	}
	if (!dir_adv) {
		...
		if (err) {
            BT_ERR("le_adv_update err = %d", err);
			return err;
		}
	}
	...
	if (IS_ENABLED(CONFIG_BT_PERIPHERAL) &&
		...
		if (err) {
            BT_ERR("le adv add conn err = %d", err);
			return err;
		}
	}
