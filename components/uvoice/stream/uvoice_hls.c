...
#ifdef HTTP_LIST_FILE_LOAD
		if (os_fwrite(nc->buffer,
			nc->head_data_size, 1, hls->fp) != 1) {
...
#ifdef HTTP_LIST_FILE_LOAD
		ret = os_fwrite(nc->buffer, rd_ret, 1, hls->fp);
		if (ret != 1) {
			M_LOGE("write failed %d!\n", os_ferror(hls->fp));
			break;
		}
#else
		if (hls->pos + rd_ret - hls->content >
				hls->len) {
	...
	while (1) {
		...
		if (!strncmp(ptr, "#EXTINF:", strlen("#EXTINF:"))) {
			...
			if (!seg_found) {
				...
				if (hls->seg_begin >= 0) {
					char *temp = strstr(ptr, ".ts");
					if (!temp) {
						M_LOGE("seg desc invalid !\n");
						continue;
					}

					char first_seg_desc[16];
					memset(first_seg_desc, 0, sizeof(first_seg_desc));
					snprintf(first_seg_desc,
						sizeof(first_seg_desc), "%d", hls->seg_begin);
					temp -= strlen(first_seg_desc);

					if (strncmp(temp, first_seg_desc, strlen(first_seg_desc))) {
						M_LOGD("not first segment\n");
						continue;
					}
				}
				#endif
