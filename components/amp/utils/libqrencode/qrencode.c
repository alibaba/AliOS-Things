...
#include "mask.h"
#include "mmask.h"
#include "../../ui/aui/libs/lvgl/lv_misc/lv_mem.h"
...
static QRRawCode *QRraw_new(QRinput *input)
{
	...
	raw = (QRRawCode *)lv_mem_alloc(sizeof(QRRawCode));
	...
	raw->ecccode = (unsigned char *)lv_mem_alloc(raw->eccLength);
	if(raw->ecccode == NULL) {
		lv_mem_free(raw->datacode);
		lv_mem_free(raw);
		return NULL;
	}

	raw->blocks = QRspec_rsBlockNum(spec);
	raw->rsblock = (RSblock *)lv_mem_alloc(raw->blocks * sizeof(RSblock));
	memset(raw->rsblock, 0x0, raw->blocks * sizeof(RSblock));
	if(raw->rsblock == NULL) {
		QRraw_free(raw);
		return NULL;
	}
...
static void QRraw_free(QRRawCode *raw)
{
	if(raw != NULL) {
		lv_mem_free(raw->datacode);
		lv_mem_free(raw->ecccode);
		lv_mem_free(raw->rsblock);
		lv_mem_free(raw);
	}
}
...
static MQRRawCode *MQRraw_new(QRinput *input)
{
	...
	raw = (MQRRawCode *)lv_mem_alloc(sizeof(MQRRawCode));
	if(raw == NULL) return NULL;
	...
	raw->rsblock = (RSblock *)lv_mem_alloc(sizeof(RSblock));
	memset(raw->rsblock, 0x0, sizeof(RSblock));
	if(raw->rsblock == NULL) {
		MQRraw_free(raw);
		return NULL;
	}
...
static QRcode *QRcode_new(int version, int width, unsigned char *data)
{
	...
	qrcode = (QRcode *)lv_mem_alloc(sizeof(QRcode));
...
static QRcode *QRcode_encodeMask(QRinput *input, int mask)
{
	...
	if(mask == -2) { // just for debug purpose
		masked = (unsigned char *)lv_mem_alloc(width * width);
...
static QRcode_List *QRcode_List_newEntry(void)
{
	...
	entry = (QRcode_List *)lv_mem_alloc(sizeof(QRcode_List));
