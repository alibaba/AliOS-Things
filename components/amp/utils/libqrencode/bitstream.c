...
BitStream *BitStream_new(void)
{
	...
	bstream = (BitStream *)lv_mem_alloc(sizeof(BitStream));
	if(bstream == NULL) return NULL;

	bstream->length = 0;
	bstream->data = (unsigned char *)lv_mem_alloc(DEFAULT_BUFSIZE);
...
BitStream *BitStream_newWithBits(size_t size, unsigned char *bits)
{
	...
	bstream->data = (unsigned char *)lv_mem_alloc(size);
	if(bstream->data == NULL) {
		lv_mem_free(bstream);
		return NULL;
	}
...
static int BitStream_expand(BitStream *bstream)
{
	...
	data = (unsigned char *)lv_mem_realloc(bstream->data, bstream->datasize * 2);
	if(data == NULL) {
		return -1;
	}
...
unsigned char *BitStream_toByte(BitStream *bstream)
{
	...
	data = (unsigned char *)lv_mem_alloc((size + 7) / 8);
	if(data == NULL) {
		return NULL;
	}
