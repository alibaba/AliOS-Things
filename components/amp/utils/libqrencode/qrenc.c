...
static int writePNG(const QRcode *qrcode, const char *outfile, enum imageType type)
{
	...
	if(type == PNG_TYPE) {
		row = (unsigned char *)lv_mem_alloc((realwidth + 7) / 8);
	} else if(type == PNG32_TYPE) {
		row = (unsigned char *)lv_mem_alloc(realwidth * 4);
	} else {
	...
	if(type == PNG_TYPE) {
		palette = (png_colorp) lv_mem_alloc(sizeof(png_color) * 2);
		if(palette == NULL) {
			fprintf(stderr, "Failed to allocate memory.\n");
			exit(EXIT_FAILURE);
		}
...
static int writeANSI(const QRcode *qrcode, const char *outfile)
{
	...
	buffer = (char *)lv_mem_alloc(buffer_s);
	if(buffer == NULL) {
		fprintf(stderr, "Failed to allocate memory.\n");
		exit(EXIT_FAILURE);
	}
