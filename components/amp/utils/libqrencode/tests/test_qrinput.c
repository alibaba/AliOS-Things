...
static void test_encodeNumericPadded(void)
{
	...
	correct = (char *)lv_mem_alloc(19 * 8 + 1);
	correct[0] = '\0';
...
static void test_splitentry(void)
{
	...
	d1 = (unsigned char *)lv_mem_alloc(size1);
	e = i1->head;
	i = 0;
	while(e != NULL) {
	...
	d2 = (unsigned char *)lv_mem_alloc(size2);
	e = i2->head;
	i = 0;
	while(e != NULL) {
