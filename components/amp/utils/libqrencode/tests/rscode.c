...
//#define NULL ((void *)0)
  ...
  rs = (RS *)lv_mem_alloc(sizeof(RS));
  memset(rs, 0x0, sizeof(RS));
  if(rs == NULL)
    goto done;
  ...
  rs->alpha_to = (data_t *)lv_mem_alloc(sizeof(data_t)*(rs->nn+1));
  if(rs->alpha_to == NULL){
    lv_mem_free(rs);
    rs = NULL;
    goto done;
  }
  rs->index_of = (data_t *)lv_mem_alloc(sizeof(data_t)*(rs->nn+1));
  if(rs->index_of == NULL){
    lv_mem_free(rs->alpha_to);
    lv_mem_free(rs);
    rs = NULL;
    goto done;
  }
  ...
  rs->genpoly = (data_t *)lv_mem_alloc(sizeof(data_t)*(nroots+1));
  if(rs->genpoly == NULL){
    lv_mem_free(rs->alpha_to);
    lv_mem_free(rs->index_of);
    lv_mem_free(rs);
