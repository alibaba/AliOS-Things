...
static QRinput_List *QRinput_List_newEntry(QRencodeMode mode, int size, const unsigned char *data)
{
	...
	entry = (QRinput_List *)lv_mem_alloc(sizeof(QRinput_List));
	...
	entry->data = NULL;
	if(size > 0) {
		entry->data = (unsigned char *)lv_mem_alloc(size);
		if(entry->data == NULL) {
			lv_mem_free(entry);
			return NULL;
		}
...
static QRinput_List *QRinput_List_dup(QRinput_List *entry)
{
	...
	n = (QRinput_List *)lv_mem_alloc(sizeof(QRinput_List));
	if(n == NULL) return NULL;

	n->mode = entry->mode;
	n->size = entry->size;
	n->data = (unsigned char *)lv_mem_alloc(n->size);
	if(n->data == NULL) {
		lv_mem_free(n);
		return NULL;
	}
...
QRinput *QRinput_new2(int version, QRecLevel level)
{
	...
	input = (QRinput *)lv_mem_alloc(sizeof(QRinput));
...
static QRinput_InputList *QRinput_InputList_newEntry(QRinput *input)
{
	...
	entry = (QRinput_InputList *)lv_mem_alloc(sizeof(QRinput_InputList));
	if(entry == NULL) return NULL;
...
QRinput_Struct *QRinput_Struct_new(void)
{
	...
	s = (QRinput_Struct *)lv_mem_alloc(sizeof(QRinput_Struct));
...
static int QRinput_List_shrinkEntry(QRinput_List *entry, int bytes)
{
	...
	data = (unsigned char *)lv_mem_alloc(bytes);
