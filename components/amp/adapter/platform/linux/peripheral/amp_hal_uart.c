...
int32_t amp_hal_uart_init(uart_dev_t *uart)
{
    ...
    if (pdrv->status == _UART_STATUS_CLOSED)
    {
        ...
        if (0 != (err_num = pthread_mutex_init(&pdrv->mutex, NULL)))
        {
...
int32_t amp_hal_uart_recv_II(uart_dev_t *uart, void *data, uint32_t expect_size, uint32_t *recv_size, uint32_t timeout)
{
    fd_set fdset;
    int ret;
    char nbuf;
    ...
    *recv_size = 0;
    ...
    while (1)
    {
        ...
        {
            perror("amp_hal_uart_recv_II: select");
        }
    }
    return 0;
}