                                   ...
                                   const unsigned char *buf, size_t len)
{
    ...
    if (session->ticket_len > 0) {
        ...
        p += session->ticket_len;
        amp_debug(MOD_STR, "%s %d", "saved ticket len = ", (int)session->ticket_len);
    }
#endif
...
static void _ssl_debug(void *ctx, int level, const char *file, int line, const char *str)
{
    ...
    if (NULL != ctx) {
        ...
        fflush((FILE *) ctx);
#endif
        amp_debug(MOD_STR, str);
    }
}

static int _real_confirm(int verify_result)
{
    amp_debug(MOD_STR, "%s 0x%02x", "certificate verification result: ", verify_result);

#if defined(FORCE_SSL_VERIFY)
    if ((verify_result & MBEDTLS_X509_BADCERT_EXPIRED) != 0) {
        amp_debug(MOD_STR, "! fail ! ERROR_CERTIFICATE_EXPIRED");
        return -1;
    }

    if ((verify_result & MBEDTLS_X509_BADCERT_REVOKED) != 0) {
        amp_debug(MOD_STR, "! fail ! server certificate has been revoked");
        return -1;
    }

    if ((verify_result & MBEDTLS_X509_BADCERT_CN_MISMATCH) != 0) {
        amp_debug(MOD_STR, "! fail ! CN mismatch");
        return -1;
    }

    if ((verify_result & MBEDTLS_X509_BADCERT_NOT_TRUSTED) != 0) {
        amp_debug(MOD_STR, "! fail ! self-signed or not signed by a trusted CA");
        return -1;
    }
#endif
                           ...
                           )
{
    ...
    amp_debug(MOD_STR, "Loading the CA root certificate ...");
    if (NULL != ca_crt) {
        if (0 != (ret = mbedtls_x509_crt_parse(crt509_ca, (const unsigned char *)ca_crt, ca_len))) {
            amp_debug(MOD_STR, "%s -0x%04x", " failed ! x509parse_crt returned ", -ret);
            return ret;
        }
    }
    amp_debug(MOD_STR, "%s", " ok ");
    ...
    if (cli_crt != NULL && cli_key != NULL) {
#if defined(MBEDTLS_CERTS_C)
        amp_debug(MOD_STR, "start prepare client cert .");
        ret = mbedtls_x509_crt_parse(crt509_cli, (const unsigned char *) cli_crt, cli_len);
#else
        {
            ret = 1;
            amp_debug(MOD_STR, "MBEDTLS_CERTS_C not defined.");
        }
#endif
        if (ret != 0) {
            amp_debug(MOD_STR, "%s -0x%x", " failed!  mbedtls_x509_crt_parse returned ", -ret);
            return ret;
        }

#if defined(MBEDTLS_CERTS_C)
        amp_debug(MOD_STR, "%s %s", "start mbedtls_pk_parse_key ", cli_pwd);
        ...
        if (ret != 0) {
            amp_debug(MOD_STR, "%s -0x%x", " failed\n  !  mbedtls_pk_parse_key returned ", -ret);
            return ret;
        }
    }
                                       ...
                                       const char *port, int proto, unsigned int timeout)
{
    ...
    while (dns_retry++ < 8) {
        ...
        if (ret != 0) {
            ...
            if (ret == EAI_AGAIN) {
                int rc = res_init();
                amp_debug(MOD_STR, "%s rc is %d errno is %d", "getaddrinfo res_init", rc, errno);
            }
#endif
    ...
    for (cur = addr_list; cur != NULL; cur = cur->ai_next) {
        ...
        if (0 != setsockopt(ctx->fd, SOL_SOCKET, SO_SNDTIMEO, &sendtimeout, sizeof(sendtimeout))) {
            perror("setsockopt");
            amp_debug(MOD_STR, "setsockopt error\n");
        }
        amp_debug(MOD_STR, "setsockopt SO_SNDTIMEO timeout: %ds\n", (int)sendtimeout.tv_sec);

        inet_ntop(AF_INET, &((const struct sockaddr_in *)cur->ai_addr)->sin_addr, ip4_str, INET_ADDRSTRLEN);
        amp_debug(MOD_STR, "connecting IP_ADDRESS: %s\n", ip4_str);
...
void *_SSLCalloc_wrapper(size_t n, size_t size)
{
    ...
    /* amp_debug(MOD_STR, "INFO -- mbedtls malloc: %p %d  total used: %d  max used: %d\r\n",
...
void _SSLFree_wrapper(void *ptr)
{
    ...
    if (mem_info->magic != MBEDTLS_MEM_INFO_MAGIC) {
        amp_debug(MOD_STR,  "Warning - invalid mem info magic: 0x%x\r\n", mem_info->magic);
        return;
    }

    mbedtls_mem_used -= mem_info->size;
    /* amp_debug(MOD_STR, "INFO mbedtls free: %p %d  total used: %d  max used: %d\r\n",
                              ...
                              const char *client_pwd, size_t client_pwd_len)
{
    ...
    if (0 != (ret = _ssl_client_init(&(pTlsData->ssl), &(pTlsData->fd), &(pTlsData->conf),
        ...
        amp_debug(MOD_STR, " failed ! ssl_client_init returned -0x%04x\n", -ret);
        return ret;
    }
     ...
     */
    amp_debug(MOD_STR,  "Connecting to /%s/%s...\n", addr, port);
#if defined(_PLATFORM_IS_LINUX_)
    if (0 != (ret = mbedtls_net_connect_timeout(&(pTlsData->fd), addr, port, MBEDTLS_NET_PROTO_TCP,
                    SEND_TIMEOUT_SECONDS))) {
        amp_debug(MOD_STR, " failed ! net_connect returned -0x%04x\n", -ret);
        return ret;
    }
#else
...
#endif
    amp_debug(MOD_STR, " ok\n");

    /*
     * 2. Setup stuff
     */
    amp_debug(MOD_STR, "  . Setting up the SSL/TLS structure...\n");
    if ((ret = mbedtls_ssl_config_defaults(&(pTlsData->conf), MBEDTLS_SSL_IS_CLIENT, MBEDTLS_SSL_TRANSPORT_STREAM,
                                           MBEDTLS_SSL_PRESET_DEFAULT)) != 0) {
        amp_debug(MOD_STR, " failed! mbedtls_ssl_config_defaults returned %d\n", ret);
        return ret;
    }
...
#elif (TLS_AUTH_MODE == TLS_AUTH_MODE_PSK)
    {
        ...
        if (psk_identity == NULL) {
            amp_debug(MOD_STR, "psk_identity malloc failed\n");
            return -1;
        }
        ...
        /* amp_debug(MOD_STR, "psk_identity: %s\n",psk_identity);
        amp_debug(MOD_STR, "psk         : %s\n",sign_string); */
        ...
        amp_debug(MOD_STR, "mbedtls psk config finished\n");
    }
    ...
    if ((ret = mbedtls_ssl_setup(&(pTlsData->ssl), &(pTlsData->conf))) != 0) {
        amp_debug(MOD_STR, "failed! mbedtls_ssl_setup returned %d\n", ret);
        return ret;
    }
    ...
    if (NULL == saved_session) {
        do {
            ...
            if (save_buf ==  NULL) {
                amp_debug(MOD_STR, " malloc failed\r\n");
                break;
            }
            ...
            if (ret != 0 || len == 0) {
                amp_debug(MOD_STR, " kv get failed len=%d,ret = %d\r\n", len, ret);
            ...
            if (ret < 0) {
                amp_debug(MOD_STR, "ssl_deserialize_session err,ret = %d\r\n", ret);
                HAL_Free(saved_session);
                HAL_Free(save_buf);
                save_buf = NULL;
                saved_session = NULL;
    ...
    if (NULL != saved_session) {
        ...
        amp_debug(MOD_STR, "use saved session!!\r\n");
    }
#endif
    /*
      * 4. Handshake
    ...
    amp_debug(MOD_STR, "Performing the SSL/TLS handshake...\n");

    while ((ret = mbedtls_ssl_handshake(&(pTlsData->ssl))) != 0) {
        if ((ret != MBEDTLS_ERR_SSL_WANT_READ) && (ret != MBEDTLS_ERR_SSL_WANT_WRITE)) {
            amp_debug(MOD_STR,  "failed  ! mbedtls_ssl_handshake returned -0x%04x\n", -ret);
#if defined(TLS_SAVE_TICKET)
            if (saved_session != NULL) {
    ...
    do {
        ...
        if (ret != 0) {
            ...
            amp_debug(MOD_STR, "mbedtls_ssl_get_session_session return 0x%04x real_len=%d\r\n", ret, (int)real_session_len);
            if (ret == 0) {
                char key_buf[KEY_MAX_LEN] = {0};
                HAL_Snprintf(key_buf,KEY_MAX_LEN -1, KV_SESSION_KEY_FMT, addr);
                ret = HAL_Kv_Set(key_buf, (void *)save_buf, real_session_len, 1);
                if (ret < 0) {
                    amp_debug(MOD_STR, "save ticket to kv failed ret =%d ,len = %d\r\n", ret, (int)real_session_len);
                }
            }
            HAL_Free(save_buf);
        }
     ...
     */
    amp_debug(MOD_STR, "  . Verifying peer X.509 certificate..\n");
    if (0 != (ret = _real_confirm(mbedtls_ssl_get_verify_result(&(pTlsData->ssl))))) {
        amp_debug(MOD_STR, " failed  ! verify result not confirmed.\n");
        return ret;
    }
    ...
    while (readLen < len) {
        ...
        if (ret > 0) {
            ...
            if (MBEDTLS_ERR_SSL_PEER_CLOSE_NOTIFY == ret) {
                ...
                amp_debug(MOD_STR, "ssl recv error: code = -0x%04X, err_str = '%s'\n", -ret, err_str);
...
static int _network_ssl_write(TLSDataParams_t *pTlsData, const char *buffer, int len, int timeout_ms)
{
    ...
    while (writtenLen < len) {
        ...
        if (ret > 0) {
            ...
            amp_debug(MOD_STR, "ssl write timeout\n");
            return 0;
        } else {
            char err_str[33];
            mbedtls_strerror(ret, err_str, sizeof(err_str));
            amp_debug(MOD_STR, "ssl write fail, code=%d, str=%s\n", ret, err_str);
...
static void _network_ssl_disconnect(TLSDataParams_t *pTlsData)
{
    ...
    if ((pTlsData->pkey).pk_info != NULL) {
        amp_debug(MOD_STR, "need release client crt&key\n");
#if defined(MBEDTLS_CERTS_C)
    ...
    amp_debug(MOD_STR, "ssl_disconnect\n");
}
                            ...
                            uint32_t ca_crt_len)
{
    ...
    if (host == NULL || ca_crt == NULL) {
        amp_debug(MOD_STR, "input params are NULL, abort\n");
        return 0;
    }

    if (!strlen(host) || (strlen(host) < 8)) {
        amp_debug(MOD_STR, "invalid host: '%s'(len=%d), abort\n", host, (int)strlen(host));
        return 0;
    }
...
int32_t HAL_SSL_Destroy(uintptr_t handle)
{
    if ((uintptr_t)NULL == handle) {
        amp_debug(MOD_STR, "handle is NULL\n");
        return 0;
    }
