                                       ...
                                       const char *port, int proto, unsigned int timeout)
{
    ...
    while (dns_retry++ < 8) {
        ...
        if (ret != 0) {
            ...
            if (ret == EAI_AGAIN) {
                int rc = res_init();
                HAL_Printf("getaddrinfo res_init, rc is %d, errno is %d\n", rc, errno);
            }
#endif
                              ...
                              const char *client_pwd, size_t client_pwd_len)
{
...
#elif (TLS_AUTH_MODE == TLS_AUTH_MODE_PSK)
    {
        ...
        if (psk_identity == NULL) {
            HAL_Printf("psk_identity malloc failed\n");
            return -1;
        }
        ...
        HAL_Printf("mbedtls psk config finished\n");
    }
    ...
    do {
        ...
        if (ret != 0) {
            ...
            if (ret == 0) {
                ...
                if (ret < 0) {
                    HAL_Printf("save ticket to kv failed ret =%d ,len = %d\r\n", ret, (int)real_session_len);
                }
            }
            HAL_Free(save_buf);
        }
    ...
    while (readLen < len) {
        ...
        if (ret > 0) {
            ...
            if (MBEDTLS_ERR_SSL_PEER_CLOSE_NOTIFY == ret) {
                ...
                HAL_Printf("ssl recv error: code = -0x%04X, err_str = '%s'\n", -ret, err_str);
