...
{
        var regval = sensor.readMem(0xfa, 3);
        if(!regval) {
                return;
        }

        var tempval = (regval[0] << 12) | (regval[1] << 4) | (regval[2] >> 4);
        var ta = (((tempval >> 3) - (calibParamT1 << 1)) * calibParamT2) >> 11;
        var tb = (((((tempval >> 4) - calibParamT1) * ((tempval >> 4) - calibParamT1)) >> 12) * calibParamT3) >> 14;
        calibParamT_FINE = ta + tb;
        var temperature = ((ta + tb) * 4 + 128) >> 8;
        temperature /= 100;
