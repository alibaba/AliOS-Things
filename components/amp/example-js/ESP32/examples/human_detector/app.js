...
/* open GPIO ir */
var irObj = gpio.open('ir');
var irDev = ir.IR(irObj);
/* open GPIO led */
...
/* 3. 周期上报人形检测传感器数据至阿里云物联网平台 */
function reportHumanDetect() {
  var have_human = irDev.irDetect();
  console.log('human state is :' + have_human + '\r\n');
  ...
  if (last_have_human != have_human) {
    //console.log('human state is change:' + have_human + '\r\n');
    ledDev.write(have_human);

    /* post props */
    device.postProps(JSON.stringify({
        MotionDetected: have_human
    }));
  }

  last_have_human = have_human;
}
