...
static duk_ret_t native_udp_bind(duk_context *ctx)
{
    ...
    ret = HAL_UDP_SocketBind(sock_id, port);
    if (ret < 0) {
...
static int udp_send_routine(udp_send_param_t *send_param)
{
    ...
    ret = HAL_UDP_sendto(sock_id, &udp_options, send_param->msg, send_param->msg_len, 0);
...
static duk_ret_t native_udp_sendto(duk_context *ctx)
{
    ...
    msg     = (char *)amp_calloc(1, msg_len + 1);
    if (!msg) {
        amp_warn(MOD_STR, "allocate memory failed");
        goto out;
    }
    ...
    udp_send_param_t *send_param =
        (udp_send_param_t *)amp_malloc(sizeof(*send_param));
    if (!send_param) {
...
static void *udp_recv_routine(void *arg)
{
    ...
    while(1) {
        p->recv_len = HAL_UDP_recvfrom(sock_id, &addr_info, p->buf, sizeof(p->buf), MAX_UDP_RECV_TIMEOUT);
    ...
    HAL_UDP_close_without_connect(sock_id);
...
static duk_ret_t native_udp_recvfrom(duk_context *ctx)
{
    ...
    int sock_id = 0;
    void *udp_recv_task;
    int udp_recv_task_stack_used;
    amp_os_thread_param_t udp_recv_task_params = {0};
    ...
    udp_recv_task_params.name = "amp udp recv task";
    udp_recv_task_params.priority = ADDON_TSK_PRIORRITY;
    udp_recv_task_params.stack_size = 1024 * 4;

    ret = HAL_ThreadCreate(&udp_recv_task, udp_recv_routine, recv_param, &udp_recv_task_params, &udp_recv_task_stack_used);
    if (ret != 0) {
...
void module_udp_register(void)
{
    ...
    duk_push_c_function(ctx, native_udp_bind, 2);
    duk_put_prop_string(ctx, -2, "bind");

    duk_push_c_function(ctx, native_udp_sendto, 3);
    duk_put_prop_string(ctx, -2, "sendto");

    duk_push_c_function(ctx, native_udp_recvfrom, 2);
    duk_put_prop_string(ctx, -2, "recvfrom");
