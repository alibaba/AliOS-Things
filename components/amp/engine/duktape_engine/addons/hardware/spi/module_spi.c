...
static duk_ret_t native_open(duk_context *ctx)
{
    ...
    ret = amp_hal_spi_init(spi_device);
    if (0 != ret) {
        amp_error(MOD_STR, "amp_hal_spi_init fail!");
        goto out;
    }
out:
    if (0 != ret) {
...
static duk_ret_t native_close(duk_context *ctx)
{
    ...
    ret = amp_hal_spi_finalize(spi_device);
    if (0 != ret) {
...
static duk_ret_t native_write(duk_context *ctx)
{
    ...
    ret = amp_hal_spi_send(spi_device, data, len, SPI_TIMEOUT);
    if (-1 == ret) {
...
static duk_ret_t native_read(duk_context *ctx)
{
    ...
    ret = amp_hal_spi_recv(spi_device, data, len, SPI_TIMEOUT);
    if (-1 == ret) {
...
static duk_ret_t native_send_receive(duk_context *ctx)
{
    int8_t ret    = -1;
    ...
    send_data    = (uint8_t *)amp_malloc(sizeof(uint8_t) * send_len);
    if (NULL == send_data) {
        amp_warn(MOD_STR, "allocate memory failed");
        goto out;
    }
    ...
    receive_data = (uint8_t *)amp_malloc(sizeof(uint8_t) * receive_len);
    if (NULL == receive_data) {
        amp_error(MOD_STR, "allocate memory failed");
        goto out;
    }

    ret = amp_hal_spi_send_recv(spi_device, send_data, receive_data, receive_len, SPI_TIMEOUT);
    if (-1 == ret) {
        amp_error(MOD_STR, "amp_hal_spi_master_recv fail!");
    }
...
void module_spi_register(void)
{
    ...
    duk_push_c_function(ctx, native_send_receive, 3);
    duk_put_prop_string(ctx, -2, "send_recv");
