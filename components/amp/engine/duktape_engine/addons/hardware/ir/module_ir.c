...
static duk_ret_t native_ir_open(duk_context *ctx)
{
    ...
    if (0 != ret) {
        amp_error(MOD_STR, "amp_hal_gpio_init fail!\n");
        goto out;
    }
...
static duk_ret_t native_ir_close(duk_context *ctx)
{
    ...
    amp_hal_gpio_disable_irq(gpio_device);
    gpio_device->priv = NULL;
    board_disattach_item(MODULE_GPIO, &gpio_handle);
    result = 0;
out:
...
static void ir_handle(void *arg)
{
    ...
    struct gpio_irq_notify_param *p =
        (struct gpio_irq_notify_param *)amp_malloc(sizeof(*p));
...
static duk_ret_t native_ir_on(duk_context *ctx)
{
    ...
    ret = amp_hal_gpio_enable_irq(gpio_device, IRQ_TRIGGER_FALLING_EDGE, ir_handle,
                              gpio_device);
    if (ret < 0) {
        amp_error(MOD_STR, "amp_hal_gpio_enable_irq fail!\n");
        goto out;
    }
    duk_dup(ctx, 1);
...
static void ir_byte(gpio_dev_t *sda, gpio_dev_t *scl, unsigned char bData)
{
    int8_t i     = 0;
    uint32_t val = 0;
    amp_hal_gpio_output_low(scl);
    ir_delay(4);
    for (i = 7; i >= 0; i--) {
        ir_delay(4);
        if ((bData >> i) & 0x01) {
            amp_hal_gpio_output_high(sda);
        } else {
            amp_hal_gpio_output_low(sda);
        }
        ir_delay(4);
        amp_hal_gpio_output_high(scl);
        ir_delay(4);
        amp_hal_gpio_output_low(scl);
    }
    amp_hal_gpio_output_high(sda);
    ...
    amp_hal_gpio_output_high(scl);
    ir_delay(16);
    amp_hal_gpio_input_get(sda, &val);
    ir_delay(16);
    amp_hal_gpio_output_low(scl);
    ir_delay(16);
}
...
static duk_ret_t native_ir_send(duk_context *ctx)
{
    ...
    if (!duk_is_pointer(ctx, 0) || !duk_is_pointer(ctx, 1) ||
        !duk_is_array(ctx, 2)) {
        amp_warn(MOD_STR, "parameter must be handle, handle and array");
        goto out;
    }
    ...
    gpio_handle.handle = duk_get_pointer(ctx, 1);
...
static duk_ret_t native_ir_learn(duk_context *ctx)
{
    ...
    if (!duk_is_pointer(ctx, 0) || !duk_is_pointer(ctx, 1) ||
        ...
        amp_warn(MOD_STR, "parameter must be handle, handle and handle");
        goto failed;
    }
    ...
    gpio_handle.handle = duk_get_pointer(ctx, 2);
