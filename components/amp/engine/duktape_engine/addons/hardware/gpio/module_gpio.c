...
static duk_ret_t native_open(duk_context *ctx)
{
    ...
    if (gpio_init_flag & (1 << gpio_device->port)) {
        amp_debug(MOD_STR, "uart port [%d] is already inited", gpio_device->port);
        goto out;
    }
...
static duk_ret_t native_close(duk_context *ctx)
{
    ...
    if (0 != ret) {
        amp_error(MOD_STR, "amp_hal_gpio_finalize fail!");
        goto out;
    }
    board_disattach_item(MODULE_GPIO, &gpio_handle);
...
static duk_ret_t native_toggle(duk_context *ctx)
{
    int32_t ret = -1;
    item_handle_t gpio_handle;
    gpio_dev_t *gpio_device = NULL;
...
static void gpio_irq_notify(void *arg)
{
    ...
    uint32_t value = 0;
    amp_hal_gpio_input_get(param->dev, &value);
...
{
    static uint32_t irq_lasttime = 0;
    uint32_t irq_nowtime = HAL_UptimeMs();
    ...
    notify = amp_malloc(sizeof(gpio_irq_notify_param_t));
    if (!notify)
        return;
...
static duk_ret_t native_on(duk_context *ctx)
{
    ...
    ret = amp_hal_gpio_enable_irq(gpio_device, irq_edge, gpio_irq, gpio_device);
    if (ret < 0) {
...
void module_gpio_register(void)
{
    ...
    duk_push_c_function(ctx, native_toggle, 1);
    duk_put_prop_string(ctx, -2, "toggle");

    duk_push_c_function(ctx, native_on, 2);
