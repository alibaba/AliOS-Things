...
static int8_t lcd_init(uint32_t width, uint32_t height)
{
    ...
    lcd_handle_t *new_handle = aos_calloc(1, sizeof(*new_handle));
    if (NULL == new_handle) {
        return -1;
    }
...
static duk_ret_t native_lcd_open(duk_context *ctx)
{
    ...
    if (!duk_is_number(ctx, -1) || !duk_is_number(ctx, -2)) {
        amp_warn(MOD_STR, "parameter object must be `{height:number,width:number}`");
        duk_pop_2(ctx);
        goto out;
    }
...
void module_lcd_register(void)
{
    ...
    AMP_ADD_FUNCTION("open",  native_lcd_open, 1);
    AMP_ADD_FUNCTION("close", native_lcd_close, 1);
    AMP_ADD_FUNCTION("show",  native_lcd_show, 1);
    AMP_ADD_FUNCTION("fill",  native_lcd_fill, 1);

    duk_put_prop_string(ctx, -2, "LCD");
}
