...
static duk_ret_t native_open(duk_context *ctx)
{
    ...
    ret = amp_hal_i2c_init(i2c_device);
    if (0 != ret) {
        amp_error(MOD_STR, "amp_hal_i2c_init fail!");
        goto out;
    }
out:
    if (0 != ret) {
...
static duk_ret_t native_close(duk_context *ctx)
{
    ...
    ret = amp_hal_i2c_finalize(i2c_device);
    if (0 != ret) {
        amp_error(MOD_STR, "amp_hal_i2c_finalize fail!");
        goto out;
    }
    board_disattach_item(MODULE_I2C, &i2c_handle);
out:
...
static duk_ret_t native_write(duk_context *ctx)
{
    ...
    ret = amp_hal_i2c_master_send(i2c_device, i2c_device->config.dev_addr, data,
                              len, I2C_TIMEOUT);
    if (-1 == ret) {
        amp_error(MOD_STR, "amp_hal_i2c_master_send fail!");
        goto out;
    }
    err = 0;
out:
...
static duk_ret_t native_write_reg(duk_context *ctx)
{
    int8_t ret    = -1;
    uint8_t *data = NULL;
    uint32_t len  = 0;
    ...
    ret = amp_hal_i2c_mem_write(i2c_device, i2c_device->config.dev_addr,
...
static duk_ret_t native_read(duk_context *ctx)
{
    ...
    ret = amp_hal_i2c_master_recv(i2c_device, i2c_device->config.dev_addr, data,
                              len, I2C_TIMEOUT);
    if (-1 == ret) {
        amp_error(MOD_STR, "amp_hal_i2c_master_recv fail!");
    }
out:
    if (!ret) {
...
static duk_ret_t native_read_reg(duk_context *ctx)
{
    int8_t ret    = -1;
    uint8_t *data = NULL;
    uint32_t len  = 0;
    ...
    ret = amp_hal_i2c_mem_read(i2c_device, i2c_device->config.dev_addr,
                                mem_addr, 1, data, len, I2C_TIMEOUT);
...
void module_i2c_register(void)
{
    ...
    duk_push_c_function(ctx, native_read_reg, 3);
    duk_put_prop_string(ctx, -2, "readReg");

    duk_push_c_function(ctx, native_write_reg, 3);
    duk_put_prop_string(ctx, -2, "writeReg");
