...
static duk_ret_t native_open(duk_context *ctx)
{
    ...
    ret = amp_hal_pwm_init(pwm_device);
...
static duk_ret_t native_close(duk_context *ctx)
{
    ...
    ret = amp_hal_pwm_stop(pwm_device);
    ret |= amp_hal_pwm_finalize(pwm_device);
    board_disattach_item(MODULE_PWM, &pwm_handle);
    amp_debug(MOD_STR, "amp_hal_pwm_finalize ret: %d\n", ret);
out:
    duk_push_int(ctx, ret);
    return 1;
}

static duk_ret_t native_getConfig(duk_context *ctx)
{
    int32_t ret = -1;
    item_handle_t pwm_handle;
...
static duk_ret_t native_setConfig(duk_context *ctx)
{
    int8_t ret    = -1;
    int32_t freq;
    int duty    = 0;
    ...
    ret = amp_hal_pwm_start(pwm_device);
...
void module_pwm_register(void)
{
    ...
    duk_push_c_function(ctx, native_getConfig, 1);
    duk_put_prop_string(ctx, -2, "getConfig");

    duk_push_c_function(ctx, native_setConfig, 2);
    duk_put_prop_string(ctx, -2, "setConfig");
