...
static char *resolve_path(const char *parent, const char *module_id)
{
    ...
    /* parse package.json */
    char *pkg_data = amp_malloc(sb.st_size + 1);
    if (!pkg_data) {
    ...
    void *fp = HAL_Fopen(resolved_path, "r");
    HAL_Fread(pkg_data, 1, sb.st_size, fp);
    HAL_Fclose(fp);
    ...
    if (item_main->valuestring) {
        ...
        if (!HAL_stat(resolved_path, &sb) && HAL_S_ISREG(sb.st_mode)) {
            cJSON_Delete(json);
            amp_free(pkg_data);
            return resolved_path;
        }
...
static duk_ret_t cb_load_module(duk_context *ctx)
{
    ...
    HAL_Fseek(fp, 0, HAL_SEEK_SET, &curpos);
    char *data = (char *)amp_malloc(len);
    if (!data) {
        HAL_Fclose(fp);
...
#endif
#ifdef JSE_ADVANCED_ADDON_AUDIO
...
void jsengine_init()
{
    ...
    snprintf(node_modules_path, sizeof(node_modules_path), "%s/node_modules", root_dir);
...
void jsengine_eval_file(const char *filename)
{
    ...
    void *fp = HAL_Fopen(filename, "r");
    HAL_Fread(file_data, 1, sb.st_size, fp);
    HAL_Fclose(fp);
