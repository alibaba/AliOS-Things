...
void jsengine_exit()
{
        ...
        amp_warn(MOD_STR, "befor: %d Memory Records Used\n",
        be_jse_get_memory_usage());
#endif
       be_jse_executor_deinit(&beJseExecutor);

#ifdef DUMP_SYMBL_USAGE
        amp_warn(MOD_STR, "after: %d Memory Records Used (should be0!)\n", be_jse_get_memory_usage());
...
{
#ifdef SUPPORT_NODE_MODELES
    BE_JSE_DIRNAME_s *firstDir = amp_calloc(1, sizeof(BE_JSE_DIRNAME_s));
...
const char *load_js_module(const char *moduleName)
{
    ...
    amp_debug(MOD_STR, "moduleName =  %s \n", moduleName);
    ...
    if (fd == -1) {
        amp_error(MOD_STR, "load %s fail \r\n", moduleName);
        return NULL;
    }

    
    if((size = jse_lseek(fd, 0L, SEEK_END)) <= 0){
        amp_error(MOD_STR, "be_lseek error:%d\r\n", size);
        return NULL;
    }
    jse_lseek(fd, 0L, SEEK_SET);
    amp_debug(MOD_STR, "%s file size = %d \r\n", path, size);

    data    = amp_calloc(1, size + 1);
    data[0] = 0;
    len     = jse_read(fd, data, size);
    amp_debug(MOD_STR, "read, len = %d \r\n", len);
    if (len > 0) {
        data[len] = 0;
    } else {
        amp_debug(MOD_STR, "read, errno = %d\n", errno);
        amp_debug(MOD_STR, "%s\n", strerror(errno));
    }
...
void jsengine_eval_file(const char *filename)
{
    ...
    if (filename) {
        ...
        amp_debug(MOD_STR, "load_js_module: %s \r\n", filename);

        data = (char *)load_js_module(filename);
        if (data) {
            amp_debug(MOD_STR, "Run Js File: %s \r\n", filename);
            amp_debug(MOD_STR, "date length = %d, \r\n", strlen(data), data);
            amp_debug(MOD_STR, "content: %s\r\n", data);
            /* execute JS code */
            jsengine_start(data);
            amp_free(data);
        }
