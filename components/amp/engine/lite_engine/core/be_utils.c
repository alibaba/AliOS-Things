...
NO_INLINE void be_amp_error(MOD_STR, const char *message)
{
    /* amp_debug(MOD_STR, "ERROR: %s\n", message); */
    char *buf = amp_calloc(1, 256);
...
NO_INLINE void be_amp_error_at(const char *message, be_jse_lexer_ctx_t *lex,
                               int tokenPos)
{
    int line, col;
    lexer_dump_line_and_col(lex, tokenPos, &line, &col);
    be_jse_execute_error();
    /* amp_debug(MOD_STR, "ERROR: %s at %d:%d current tolken=%s \n", message, line, col,
              lex->token); */
    char *buf = amp_calloc(1, 256);
...
NO_INLINE void be_amp_warn(MOD_STR, const char *message)
{
    /* amp_debug(MOD_STR, "WARNING: %s\n", message); */
    char *buf = amp_calloc(1, 256);
...
NO_INLINE void be_amp_warn(MOD_STR,%s, _at(const char *message, be_jse_lexer_ctx_t *lex,
                              int tokenPos)
{
    int line, col;
    lexer_dump_line_and_col(lex, tokenPos, &line, &col);
    be_jse_execute_error();
    /* amp_debug(MOD_STR, "WARNING: %s at %d:%d current tolken=%s \n", message, line, col,
              lex->token); */
    char *buf = amp_calloc(1, 256);
...
{
    be_jse_execute_error();
    /* amp_debug(MOD_STR, "ASSERT FAIL AT %s:%d\n", file, line); */
    char *buf = amp_calloc(1, 256);
...
char *getNodeModulePath(char *path)
{
    ...
    if (fd >= 0) {
        ...
        root = aJSON_Parse(json_data);
        if (NULL == root) {
            amp_free(json_data);
            amp_free(fullPath);
            amp_error(MOD_STR, "%s %d aJSON_Parse failed \n", __FUNCTION__, __LINE__);
            return (path);
        }

        /* find the main */
        item = aJSON_GetObjectItem(root, "main");
        if (NULL != item && aJSON_String == item->type &&
            ...
            amp_debug(MOD_STR, "%s %d package find main, %s \n", __FUNCTION__, __LINE__,
                   fullPath);
            return fullPath;
        }
        ...
        amp_free(json_data);
        aJSON_Delete(root);
        amp_debug(MOD_STR, "%s %d package incomplete , package.json = %s \n", __FUNCTION__,
               __LINE__, fullPath);
        /* default index.js */
