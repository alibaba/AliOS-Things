...
static void append_char(be_jse_lexer_ctx_t *lex, char ch)
{
    if (lex->token_len < MAX_TOKEN_LENGTH - 1) {
        ...
        be_amp_warn(MOD_STR,%s, _at("Token name is too long! skipping character", lex,
                       lex->token_start);
    }
}
...
bool lexer_token_cmp(be_jse_lexer_ctx_t *lex, int expected_tk)
{
    if (lex->tk != expected_tk) {
        ...
        be_amp_error_at(buf, lex, lex->token_start);
#else
        be_amp_error_at("be_jse_lex_match error", lex, lex->token_start);
#endif
        next_token(lex);
        return false;
    }
