...
void be_jse_symbol_table_init()
{
    ...
    gSymbolBlocks    = amp_calloc(1, sizeof(be_jse_symbol_t *));
    gSymbolBlocks[0] =
        amp_calloc(1, sizeof(be_jse_symbol_t) * BE_JSE_SYMBOL_TABLE_BLOCK_SIZE);
#else
    ...
    amp_debug(MOD_STR, "%s %d \n", __FUNCTION__, __LINE__);
    amp_debug(MOD_STR, "gSymbolTableSize = %d gSymbolTableFirstFreeSlotID = %d \n",
              gSymbolTableSize, gSymbolTableFirstFreeSlotID);
#ifdef DUMP_SYMBL_USAGE
    amp_debug(MOD_STR, "beJSESymbolUsage=%d\n", beJSESymbolUsage);
#endif
}
...
void be_jse_symbol_table_deinit()
{
    ...
    for (i = 0; i<gSymbolTableSize>> BE_JSE_SYMBOL_TABLE_BLOCK_SHIFT; i++)
        amp_free(gSymbolBlocks[i]);
    amp_free(gSymbolBlocks);
...
void be_jse_set_symbol_table_size(unsigned int newSize)
{
    ...
    /* resize block table */
    gSymbolBlocks =
        amp_realloc(gSymbolBlocks, sizeof(be_jse_symbol_t *) * newBlockCount);
    amp_debug(MOD_STR, "gSymbolBlocks = %p \n", gSymbolBlocks);

    /* allocate more blocks */
    unsigned int i;

    for (i = oldBlockCount; i < newBlockCount; i++) {
        gSymbolBlocks[i] = amp_calloc(
            1, sizeof(be_jse_symbol_t) * BE_JSE_SYMBOL_TABLE_BLOCK_SIZE);
        amp_debug(MOD_STR, "gSymbolBlocks[%d] = %p \n", i, gSymbolBlocks[i]);
    }
    ...
    amp_debug(MOD_STR, "Resized memory from %d blocks to %d\n", oldBlockCount,
              newBlockCount);
    amp_debug(MOD_STR, "gSymbolTableFirstFreeSlotID = %d \n",
              gSymbolTableFirstFreeSlotID);
#else
    NOT_USED(newSize);
    be_assert(0);
...
static be_jse_symbol_t *new_symbol_node()
{
    ...
    amp_debug(MOD_STR, "gSymbolTableSize = %d \n", gSymbolTableSize);
#ifdef DUMP_SYMBL_USAGE
    amp_debug(MOD_STR, "beJSESymbolUsage = %d \n", beJSESymbolUsage);
#endif
    ...
    be_amp_error(MOD_STR, "Out of Memory!");
    return 0;
#endif
}
    ...
    if (symbol_is_string(s) || symbol_is_string_ext(s)) {
        ...
        if (stringDataRef) {
            ...
            if (child->refs > 1 || child->locks > 1) {
                /* amp_debug(MOD_STR, "%d, string ext,  nodeid  %d \n",
                   __LINE__, get_symbol_node_id(child)); amp_debug(MOD_STR, 
                   "%d, r %d, l %d, flag = %d \n", __LINE__, child->refs,
...
{
    be_jse_symbol_t *ret;
    char *evalstr = (char *)amp_calloc(1, json_str_len + 3);
...
be_jse_symbol_t *new_str_symbol(const char *str)
{
    ...
    if (!first) {
        be_amp_warn(MOD_STR, "Unable to create string ,Not enough memory");
        return 0;
    }
    ...
    if (str != NULL) {
        while (*str) {
            ...
            if (*str) {
                ...
                if (!next) {
                    be_amp_warn(MOD_STR, "Truncating string as not enough memory");
                    symbol_unlock(s);
                    return first;
                }
                                      ...
                                      int charTo)
{
    ...
    */
    /*
        ...
        char* buf = amp_malloc(len+1);
        symbol_to_str(first,buf, len);

        amp_debug(MOD_STR, "==============\n");
        amp_debug(MOD_STR, "len=%d\n", len);
        amp_debug(MOD_STR, "function:%s\n", buf);
        amp_free(buf);
...
be_jse_symbol_t *symbol_lock(be_jse_node_t id)
{
    ...
    if (s->locks == 0) {
        /* amp_debug(MOD_STR, "[%s][%d] id = %d ... \n", __FUNCTION__,
           __LINE__, id); amp_debug(MOD_STR, "[%s][%d] locks = %d ... \n",
           __FUNCTION__, __LINE__, s->locks); amp_debug(MOD_STR, "[%s][%d]
           refs = %d ... \n", __FUNCTION__,
           __LINE__, s->refs); */
        be_amp_error(MOD_STR, "Too many locks on the symbol!");
    }
...
{
    s->locks++;
    if (s->locks == 0) be_amp_error(MOD_STR, "Too many locks on the symbol!");
    return s;
}
...
be_jse_node_t symbol_unlock(be_jse_symbol_t *s)
{
    ...
    if (s->locks == 0) {
        amp_error(MOD_STR, "symbol_unlock error, ret = %d", ref);
    }
    be_assert(s->locks > 0);
...
void symbol_to_str(be_jse_symbol_t *s, char *str, size_t len)
{
    ...
    if (symbol_is_undefined(s)) {
        ...
        if (symbol_is_string_ext(s))
            be_amp_warn(MOD_STR, "INTERNAL: Calling symbol_to_str on a JSV_STRING_EXT");
        while (var) {
            ...
            for (i = 0; i < (int)symbol_get_max_char_len(var); i++) {
                ...
                if (len-- <= 0) {
                    *str = 0;
                    be_amp_warn(MOD_STR, "symbol_to_str overflowed\n");
                    symbol_unlock(var);
                    return;
                }
                /* *(str++) = var->data.str[i]; */
...
be_jse_int_t symbol_array_push(be_jse_symbol_t *arr, be_jse_symbol_t *value)
{
    ...
    if (!idx) {
        be_amp_warn(MOD_STR, "Out of memory while appending to array");
        return 0;
    }
...
be_jse_symbol_t *symbol_maths_op_error(int op, const char *datatype)
{
    ...
    bufpos = strlen(buf);
    strncat(&buf[bufpos], " datatype", BE_JSE_ERROR_BUF_SIZE - bufpos);
    be_amp_error(MOD_STR, buf);
#else
    NOT_USED(op);
    NOT_USED(datatype);
    be_amp_error(MOD_STR, "symbol_maths_op_error");
#endif
    return 0;
}
...
void be_jse_show_symbol_table_used()
{
    ...
    for (i = 1; i < gSymbolTableSize; i++) {
        if (get_symbol_addr(i)->refs != SYM_TABLE_UNUSED_REF) {
            amp_debug(MOD_STR, "USED VAR #%d:", i);
...
void dump_symbol_node_id(int id)
{
    ...
    s = get_symbol_addr(id);
    if (s) {
        amp_debug(MOD_STR, "nodeid: %d \n", id);
        amp_debug(MOD_STR, "r %d, l %d, flag = %d \n", s->refs, s->locks, s->flags);
        char buf[64];
        symbol_to_str(s, buf, 63);
        amp_debug(MOD_STR, "to str: %s \n", buf);
    }
}

ALWAYS_INLINE void trace_symbol_lock_info(be_jse_symbol_t *v)
{
    amp_debug(MOD_STR, "#%d [r%d,l%d] ", get_symbol_node_id(v), v->refs, v->locks - 1);
}
...
void trace_symbol_info(be_jse_node_t ref, int indent)
{
    ...
    for (i = 0; i < indent; i++) amp_debug(MOD_STR, " ");

    if (!ref) {
        amp_debug(MOD_STR, "undefined\n");
        return;
    }
    var = symbol_lock(ref);
    amp_debug(MOD_STR, "#%d[r%d,l%d] ", ref, var->refs, var->locks - 1);

    if (symbol_is_name(var)) {
        if (symbol_is_function_argv(var)) amp_debug(MOD_STR, "Param ");
        symbol_to_str(var, buf, BE_JSE_ERROR_BUF_SIZE - 1);
        if (symbol_is_int(var))
            amp_debug(MOD_STR, "Name: int %s  ", buf);
        else if (symbol_is_float(var))
            amp_debug(MOD_STR, "Name: flt %s  ", buf);
        else if (symbol_is_string(var) || symbol_is_function_argv(var))
            amp_debug(MOD_STR, "Name: '%s'  ", buf);
        else
            be_assert(0);
    ...
    if (symbol_is_object(var))
        amp_debug(MOD_STR, "Object {\n");
    else if (symbol_is_array(var))
        amp_debug(MOD_STR, "Array [\n");
    else if (symbol_is_int(var))
        amp_debug(MOD_STR, "Integer ");
    else if (symbol_is_float(var))
        amp_debug(MOD_STR, "Double ");
    else if (symbol_is_string(var))
        amp_debug(MOD_STR, "String ");
    else if (symbol_is_function(var))
        amp_debug(MOD_STR, "Function {\n");
    else
        amp_debug(MOD_STR, "Flags %d\n", var->flags);

    if (!symbol_is_object(var) && !symbol_is_array(var) &&
        !symbol_is_function(var)) {
        symbol_to_str(var, buf, BE_JSE_ERROR_BUF_SIZE - 1);
        amp_debug(MOD_STR, buf);
    }

    if (symbol_is_string(var) || symbol_is_string_ext(var)) {
        if (!symbol_is_string_ext(var) && var->first_child) {
            amp_debug(MOD_STR, "( Multi-block string ");
            be_jse_node_t child = var->first_child;
            while (child) {
                be_jse_symbol_t *childVar = symbol_lock(child);
