...
static bool be_jse_vm_scope_insert(be_jse_node_t scope)
{
    if (vm.scopeCount >= BE_JSE_PARSE_MAX_SCOPES) {
        be_amp_error(MOD_STR, "Scopes is exceeded");
...
{
    if (vm.scopeCount <= 0) {
        be_amp_error(MOD_STR, "Scopes is over removed ");
        be_jse_execute_error();
        return;
    }
                                       ...
                                       be_jse_symbol_t *src)
{
    if (!symbol_is_name(dst)) {
        be_amp_error_at("Unable to assign value to non-reference", vm.lex,
                            ...
                            const char *funcDesc, be_jse_callback callbackPtr)
{
    ...
    if (!success) {
        be_amp_error(MOD_STR, "Parsing Native Function failed!");
        be_jse_execute_error();
    }
                                         ...
                                         be_jse_symbol_t **argPtr)
{
    if (BE_VM_PARSE_SHOULD_EXECUTE && !function) {
        if (symbol_is_object(parent)) {
            ...
            /* for debugging */
            amp_warn(MOD_STR, "%s.%s not found \n", be_jse_get_tmp_token(),
                     vm.lex->token);
#endif
        }

        be_amp_warn(MOD_STR,%s, _at("FUNC:", vm.lex, vm.lex->token_last_end);
    }
    ...
    if (BE_VM_PARSE_SHOULD_EXECUTE && function) {
        ...
        if (!symbol_is_function(function)) {
            be_amp_error_at("Need a function to call", vm.lex,
                            vm.lex->token_last_end);
            be_jse_execute_error();
            return 0;
        }
            ...
            if (symbol_is_native(function)) {
                ...
                if (be_jse_vm_scope_insert(get_symbol_node_id(root))) {
                    ...
                    if (functionCode) {
                        ...
                        if (sourcePtr) amp_free(sourcePtr);
                        vm.lex = oldLex;
                        if (hasError) {
                            amp_error(MOD_STR, 
                                "[%s][%d] executor function call error  ...\n",
                                __FUNCTION__, __LINE__);
                            /* be_amp_error_at("executor function call error",
            ...
            if (vm.lex->tk == '(') {
...
#ifndef BE_JSE_SILENT
                        sprintf(errMsg, "OBJ:%s", objectName);
                        be_amp_error_at(errMsg, vm.lex, vm.lex->token_last_end);
#endif
                    }
                ...
                if (BE_VM_PARSE_SHOULD_EXECUTE) {
                    ...
                    if (aVar &&
                        ...
                        be_amp_warn(MOD_STR,%s, _at("Variable is not an Array or Object",
                                       vm.lex, vm.lex->token_last_end);
                        symbol_unlock(parent);
                        parent = 0;
                        symbol_unlock(a);
    ...
    if (vm.lex->tk == BE_TOKEN_KW_NEW) {
        ...
        if (BE_VM_PARSE_SHOULD_EXECUTE) {
            ...
            be_jse_symbol_t *objClassOrFunc = unlock_symbol_value(
                lookup_symbol_in_scopes(lexer_get_token(vm.lex)));
            if (!objClassOrFunc) {
                be_amp_warn(MOD_STR,%s, _at("Prototype used in NEW is not defined", vm.lex,
                               vm.lex->token_start);
            }
            LEXER_MATCH(BE_TOKEN_ID);
    ...
    if (MEET_ASSIGNMENT_OP) {
        ...
        if (BE_VM_PARSE_SHOULD_EXECUTE && left_ops && !left_ops->refs) {
            if (symbol_is_name(left_ops)) {
                ...
                be_amp_warn(MOD_STR,%s, _at("Trying to assign to an un-named type\n", vm.lex,
                               vm.lex->token_last_end);
            }
        }
    ...
    if (vm.lex->tk == BE_TOKEN_ID || vm.lex->tk == BE_TOKEN_INT ||
        ...
        if (vm.lex->tk == BE_TOKEN_KW_IN) {
            ...
            if (!symbol_is_name(forStatement)) {
                ...
                be_amp_error_at("FOR a IN b - 'a' must be a variable name",
                                vm.lex, vm.lex->token_last_end);
                be_jse_execute_error();
                return 0;
            }
        ...
        if (BE_VM_PARSE_SHOULD_EXECUTE) {
            ...
            if (resultVar) {
                ...
                be_amp_error_at("RETURN statement, but not in a function.\n",
                                vm.lex, vm.lex->token_last_end);
                be_jse_execute_error();
            }
            set_vm_idle();
...
void be_jse_async_event_cb(void *arg)
{
    ...
    if (async->params) amp_free(async->params);
    amp_free(async);
}
                       ...
                       int param_count)
{
    ...
    if (func && symbol_is_function(func)) {
        BE_ASYNC_S *async = (BE_ASYNC_S *)amp_calloc(1, sizeof(BE_ASYNC_S));
        if (async == NULL) {
            amp_error(MOD_STR, "[%s][%d] out of memory .\n", __FUNCTION__, __LINE__);
            symbol_unlock(func);
            return;
        }
        async->func        = func;
        ...
        ret = amp_task_schedule_call(be_jse_async_event_cb, async);
        if (ret < 0) {
            amp_error(MOD_STR, "jse_osal_schedule_call error");
            DEC_SYMBL_REF(async->func);
            amp_free(async->params);
            amp_free(async);
        }
    }
    symbol_unlock(func);
                                ...
                                const char *moduleName)
{
    ...
    if (moduleExport) {
        /* Found the module! */
        amp_warn(MOD_STR, "Found the module moduleName=%s\n", moduleName);
        return moduleExport;
    }
                                   ...
                                   be_jse_symbol_t *id, be_jse_symbol_t *code)
{
    ...
    char *moduleJS = amp_malloc(len + 1);
...
char *be_array_symbol_to_buffer(be_jse_symbol_t *arg)
{
    ...
    for (i = 0; i < len; ++i) {
        ...
        if (!item) {
            amp_free(src);
            symbol_unlock(item);
            amp_error(MOD_STR, "get_symbol_array_item fail\n\r");
            return NULL;
        }
