                                                  ...
                                                  const char* name)
{
    if (strcmp(name, "log") == 0) {
        ...
        buf_header = buf = (char*)amp_calloc(1, len + 1);
    ...
    be_jse_vm_ctx_t* execInfo, be_jse_symbol_t* var, const char* name)
{
    ...
    if (strcmp(name, "eval") == 0) {
        ...
        if (v) {
            ...
            if (len) {
                char* sourcePtr = amp_malloc(len + 1);
    ...
    if (strcmp(name, "require") == 0) {
        ...
        if (v) {
            ...
            if (len) {
                ...
                symbol_to_str(v, sourcePtr, len);
                amp_warn(MOD_STR, "");
                amp_warn(MOD_STR, "require(%s)", sourcePtr);
                ...
                /* amp_warn(MOD_STR, "firstDir = %p ", firstDir);
                amp_warn(MOD_STR, "firstDir->dirname = %s ",
                firstDir->dirname); */
...
#define NODE_MODULES "node_modules/"
                ...
                /* amp_warn(MOD_STR, "fullPath = %s ", fullPath); */
                /* amp_warn(MOD_STR, "clearPath = %s ", clearPath); */

                amp_free(fullPath);

                /* save current dirname */
                firstDir          = amp_calloc(1, sizeof(BE_JSE_DIRNAME_s));
                firstDir->dirname = getFilePath(clearPath);
                bone_engine_dirname_push(firstDir);

                /* module loading */
                amp_warn(MOD_STR, "be_jse_require  %s ", clearPath);
                                               ...
                                               const char* name)
{
    ...
    if (strcmp(name, "parse") == 0) {
        be_jse_symbol_t* text =
            str_to_symbol(be_jse_handle_single_arg_function(), true);
        char* buf = (char*)amp_calloc(1, symbol_str_len(text) + 1);
