...
static be_jse_symbol_t *setup_timer(int repeat)
{
    ...
    if (!cbv || !msv || (!symbol_is_function(cbv))) {
        amp_error(MOD_STR, "process timer parameter error\n");
        goto done;
    }
    ...
    timer_action_t *p_param =
        (timer_action_t *)amp_calloc(1, sizeof(timer_action_t));
    if (!p_param) goto done;
    p_param->magic  = MAGIC;
    p_param->func   = cbv;
    p_param->repeat = repeat;
    INC_SYMBL_REF(p_param->func);
    timer_id = amp_task_timer_action(ms, timer_call_action, p_param, repeat);
...
static be_jse_symbol_t *clear_timer()
{
    ...
    jse_osal_timer_delete((amp_osTimerId)p_timer_action->timerid);
    p_timer_action->magic = 0;
    DEC_SYMBL_REF(p_timer_action->func);
    amp_free(p_timer_action);
