...
{
    JSValue *js_cb_ref = (JSValue *)userdata;
    ...
    if (packet->type == AIOT_DYNREGRECV_STATUS_CODE) {
        ...
        if (packet->type == AIOT_DYNREGRECV_DEVICE_INFO) {
            ...
            JSValue sec = JS_NewString(ctx, packet->data.device_info.device_secret);
            JSValue val = JS_Call(ctx, *js_cb_ref, JS_UNDEFINED, 1, &sec);
