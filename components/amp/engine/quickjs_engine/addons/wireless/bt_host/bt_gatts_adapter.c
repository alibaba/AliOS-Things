...
int bt_gatts_adapter_update_chars(char *uuid, uint8_t *data, size_t len) {
    ...
    amp_debug(MOD_STR, "[%s] index = %d, uuid = %s, len = %d", __func__, index, uuid, len);
    if (index < 0) {
        return -1;
    }
    ...
    if(index < g_attr_num) {
        ...
        if (ccc_index < 0) {
            amp_error(MOD_STR, "[%s]ccc not found", __func__);
            return -1;
        }
        ...
        case EVENT_GATT_CHAR_READ:
            {
                ...
                for (int ii=0; ii<g_attr_num; ii++) {
                    if (g_attrs_list[ii].handle == read_data->char_handle) {
                        amp_debug(MOD_STR, "[%s]read request at handle.%d, uuid:0x%04x, offset:%d", __func__, read_data->char_handle
                                                                                        , ((struct ut_uuid_16 *)(g_attrs_list[ii].uuid))->val
                                                                                        , read_data->offset);
                        amp_bt_gatts_adapter_usr_data_t *data = (amp_bt_gatts_adapter_usr_data_t *)g_attrs_list[ii].user_data;
                        if (data) {
        ...
        case EVENT_GATT_CHAR_WRITE:
            {
                ...
                for (int ii=0; ii<g_attr_num; ii++) {
                    if (g_attrs_list[ii].handle == write_data->char_handle) {
                        amp_debug(MOD_STR, "[%s]read request at handle.%d, uuid:0x%04x, offset:%d\n" , __func__, write_data->char_handle
                                                                                         , ((struct ut_uuid_16 *)(g_attrs_list[ii].uuid))->val
                                                                                         , write_data->offset);
        ...
        case EVENT_GATT_CHAR_CCC_CHANGE:
            {
                ...
                for (int ii=0; ii<g_attr_num; ii++) {
                    if (g_attrs_list[ii].handle == data->char_handle) {
                        ...
                        amp_debug(MOD_STR, "[%s]ccc-changed at handle.%d 0x%04x, 0x%04x,\n", __func__, data->char_handle
