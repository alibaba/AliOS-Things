...
class HW_TIMER {
    ...
    setTimeout(cb, timeout) {
        ...
        ret = __native.TIMER.setTimeout(this.timerInstance, cb, timeout);
        if (ret != 0) {
    ...
    clearTimeout() {
        ...
        ret = __native.TIMER.clearTimeout(this.timerInstance);
        if (ret != 0) {
    ...
    setInterval(cb, timeout) {
        ...
        ret = __native.TIMER.setInterval(this.timerInstance, cb, timeout);
        if (ret != 0) {
    ...
    clearInterval() {
        ...
        ret = __native.TIMER.clearInterval(this.timerInstance);
        if (ret != 0) {
