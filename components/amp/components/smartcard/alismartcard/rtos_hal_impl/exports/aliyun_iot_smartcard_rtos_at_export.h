...
/*!
 * \brief   set if module need to be reset after changing operator
 *          default is false
 *
 * \param   [IN] reset: reset flag
 *
 * \retval  Possible returns are:
 *           0: succesful
 */
extern int aliyun_iot_smartcard_set_reset_flag(bool reset);

/*!
 * \brief   set module startup time after chaning operator
 *          default is 30s
 *
 * \param   [IN] second: startup time
 *
 * \retval  Possible returns are:
 *           0: succesful
 */
extern int aliyun_iot_smartcard_set_startup_time(uint16_t second);

/*!
 * \brief   set module type, sdk will do extra work depends on type
 *          default is null
 *
 * \param   [IN] type: module type
 *
 * \retval  Possible returns are:
 *           0: succesful
 */
extern int aliyun_iot_smartcard_set_module_type(const char *type);
 ...
 *          -1: func exec fail
 */
extern int aliyun_iot_smartcard_check(bool *support);

/**
 * @brief get aliiccid
 *
 * @param [out] iccid: pointer to iccid str
 * @param [in] len: iccid str len, len should not less than 21
 ...
 *          -1: func exec fail
 */
extern int aliyun_iot_smartcard_get_aliiccid(char *iccid, int len);
