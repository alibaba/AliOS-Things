...
static void deinit_dev(int fd)
{
    ...
    if ((dev->_send_delimiter != NULL) && (strcmp(dev->_send_delimiter, AT_SEND_DEFAULT_DELIMITER) != 0)) {
        aos_free(dev->_send_delimiter);
        dev->_send_delimiter = NULL;
    }
...
static int init_dev(int fd, at_config_t *config)
{
    ...
    if ((config->send_delimiter) && (strcmp(config->send_delimiter, AT_SEND_DEFAULT_DELIMITER) != 0)) {
        char *send_delimiter = aos_malloc(strlen(config->send_delimiter) + 1);
        memset(send_delimiter, 0, strlen(config->send_delimiter) + 1);
        memcpy(send_delimiter, config->send_delimiter, strlen(config->send_delimiter));
        dev->_send_delimiter = send_delimiter;
    } else {
...
int at_read(int fd, char *outbuf, int readsize)
{
    ...
    while (total_read < readsize) {
        ...
        if (ret != 0) {
            atpsr_err("at_read failed on uart_recv. ret %d\n", ret);
            break;
        }
                         ...
                         char *recvbuf, int bufsize, at_recv_cb cb, void *arg)
{
    ...
    oob->maxlen  = bufsize;
    if(prefix != NULL) {
        temp = aos_malloc(strlen(prefix) + 1);
        memset(temp, 0, strlen(prefix) + 1);
        memcpy(temp, prefix, strlen(prefix));
        oob->prefix  = temp;
    } else {
        oob->prefix  = (char *)prefix;
    }
    if(postfix != NULL) {
        temp = aos_malloc(strlen(postfix) + 1);
        memset(temp, 0, strlen(postfix) + 1);
        memcpy(temp, postfix, strlen(postfix));
        oob->postfix  = temp;
    } else {
