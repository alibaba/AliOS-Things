...
static int at_init_uart(void *dev)
{
    ...
    ret = aos_hal_uart_init((uart_dev_t *)dev);
                            ...
                            uint32_t *recv_size, uint32_t timeout)
{
    ...
    ret = aos_hal_uart_recv_II((uart_dev_t *)dev, data, expect_size, recv_size, timeout);
                          ...
                          uint32_t timeout)
{
    ...
    ret = aos_hal_uart_send((uart_dev_t *)dev, data, size, timeout);
...
static int at_deinit_uart(void *dev)
{
    ...
    ret = aos_hal_uart_finalize((uart_dev_t *)dev);
