...
extern unsigned char uart_recv_byte(unsigned char *c);
...
int ymodem_data_head_parse(unsigned char data_type)
{
    ...
    if( (buf_len != value)  || (0 != buffer[0]) || (0xFF != buffer[1]) ) {
        amp_debug(MOD_STR, "header error: %d %02x %02x", buf_len, buffer[0], buffer[1]);
        goto err_exit;
    }

    amp_debug(MOD_STR, "ymodem_recv_bytes head done, buf_len:%d", buf_len);
    /* check CRC */
    ...
    amp_debug(MOD_STR, "get file size:%d", ymodem_flash_size);
...
int ymodem_data_parse(unsigned char data_type)
{
    ...
    if ((buf_len != value) || (0xFF != buffer[0] + buffer[1])) {
        amp_error(MOD_STR, "ymodem_data_parse crc error:%02x %02x",buffer[buf_len - 2], buffer[buf_len - 1] );
        goto err_exit;
    }
    ...
    /* write data fo flash */
    amp_debug(MOD_STR, "write data, buf_len:%d", buf_len - 4);
...
int ymodem_upgrade(void (*func)(unsigned char *, int))
{
    ...
    if(ret == YMODEM_OK) {
        ...
        amp_debug(MOD_STR, "file too big len:0x%08x !!!", ymodem_flash_size);
    } else {
        amp_debug(MOD_STR, "Ymodem recv file Err:%d !!!", ret);
    }
    return ret;
}
