                   ...
                   int32_t complete)
{
    ...
    if (app_fp != NULL) {
        if (buf_len > 0) {
            ret = HAL_Fwrite(buf, 1, buf_len, app_fp);
        }

        if ((offset + buf_len) == file_size) {
            ret    = HAL_Fsync(app_fp);
            ret    = HAL_Fclose(app_fp);
            app_fp = NULL;
...
static void *download_work(void *arg)
{
    ...
    upgrading = 0;
    jse_task_exit_call(NULL, NULL);
    // jse_system_reboot();

    amp_warn(MOD_STR, "waiting jse taks exit completely\n");
    HAL_SemaphoreWait(jse_task_exit_sem, PLATFORM_WAIT_INFINITE);
    HAL_SleepMs(200);
    ...
    if(HAL_ThreadCreate(&restart_task, be_jse_task_restart_entrance, NULL, &task_params, &restart_task_stack_used) != 0)
    {
...
int apppack_upgrade(char *url)
{
    ...
    if (update_done) {
        ...
        if (HAL_ThreadCreate(&upgrade_task, download_work, (void *)url, &task_params, &upgrade_stack_used) != 0) {
            update_done = 1;
            apppack_final();
