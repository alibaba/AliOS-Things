...
static void handle_masterdev_request(dm_msg_t *dm_msg, void *ctx)
{
    ...
    switch (dm_msg->callback_type) {
        ...
        case dm_callback_type_local_connected:
            if (linkkit_ops->on_connect) {
                linkkit_ops->on_connect(context, dm_callback_type_cloud_connected == dm_msg->callback_type ? 1 : 0);
            }
#else
            if (linkkit_ops->on_connect) {
        ...
        case dm_callback_type_local_disconnected:
            if (linkkit_ops->on_disconnect) {
                linkkit_ops->on_disconnect(context, dm_callback_type_cloud_disconnected == dm_msg->callback_type ? 1 : 0);
            }
#else
            if (linkkit_ops->on_disconnect) {
        ...
        case dm_callback_type_new_thing_created:
            if (linkkit_ops->thing_create) {
                linkkit_ops->thing_create(dm_msg->thing_id, context);
            }
            break;
#ifdef RRPC_ENABLED
        ...
        case dm_callback_type_thing_enabled:
            if (linkkit_ops->thing_enable) {
                linkkit_ops->thing_enable(dm_msg->thing_id, context);
            }
            break;
        case dm_callback_type_thing_disabled:
            if (linkkit_ops->thing_disable) {
                linkkit_ops->thing_disable(dm_msg->thing_id, context);
            }
            break;
        case dm_callback_type_response_arrived:
            if (linkkit_ops->response_arrived) {
                linkkit_ops->response_arrived(dm_msg->thing_id, dm_msg->id, dm_msg->code, dm_msg->response_message, context);
            }
            break;
...
static void handle_subdev_request(dm_msg_t *msg, void *ctx)
{
    ...
    switch (msg->subcallback_type) {
        case dm_subdev_callback_type_topo_add_success:
            if (linkkit_subdev_ops->topo_add_success) {
                linkkit_subdev_ops->topo_add_success(msg->sub_thing_id, ctx);
            }
            break;
        case dm_subdev_callback_type_topo_add_fail:
            if (linkkit_subdev_ops->topo_add_fail) {
                linkkit_subdev_ops->topo_add_fail(msg->sub_thing_id, msg->code, msg->message, ctx);
            }
            break;
        case dm_subdev_callback_type_login_success:
            if (linkkit_subdev_ops->login_success) {
                linkkit_subdev_ops->login_success(msg->sub_thing_id, ctx);
            }
            break;
        case dm_subdev_callback_type_login_fail:
            if (linkkit_subdev_ops->login_fail) {
                linkkit_subdev_ops->login_fail(msg->sub_thing_id, msg->code, msg->message, ctx);
            }
            break;
        case dm_subdev_callback_type_new_thing_created:
            if (linkkit_subdev_ops->thing_create) {
                linkkit_subdev_ops->thing_create(msg->sub_thing_id, ctx);
            }
            break;
        case dm_subdev_callback_type_thing_disabled:
            if (linkkit_subdev_ops->thing_disable) {
                linkkit_subdev_ops->thing_disable(msg->sub_thing_id, ctx);
            }
            break;
        case dm_subdev_callback_type_thing_enabled:
            if (linkkit_subdev_ops->thing_enable) {
                linkkit_subdev_ops->thing_enable(msg->sub_thing_id, ctx);
            }
            break;
...
/* async type */
static int linkkit_start_routine(handle_dm_callback_fp_t callback_fp, int get_tsl_from_cloud,
                                 linkkit_loglevel_t log_level, linkkit_cloud_domain_type_t domain_type)
{
    int ret = -1;
#ifdef CM_SUPPORT_MULTI_THREAD
    int stack_used;
...
int linkkit_end()
{
    ...
    if (g_message_queue) {
        lite_queue_destroy(g_message_queue);
    }
#ifdef SERVICE_OTA_ENABLED
    if (fota_object) {
        delete_object(fota_object);
    }
#endif /* SERVICE_OTA_ENABLED */
    if (dm_object) {
        delete_object(dm_object);
    }
                      ...
                      const char *value_str)
{
    ...
    if (dm == NULL || *dm == NULL || thing_id == NULL || identifier == NULL || method_set >= linkkit_method_set_number ||
                      ...
                      char **value_str)
{
    ...
    if (dm == NULL || *dm == NULL || thing_id == NULL || identifier == NULL || method_get >= linkkit_method_get_number ||
        (value == NULL && value_str == NULL)) {
        return -1;
    }
...
#endif /* RRPC_ENABLED */
{
    ...
    if (dm == NULL || *dm == NULL || thing_id == NULL || service_identifier == NULL) {
        return -1;
    }
...
int linkkit_invoke_raw_service(const void *thing_id, int is_up_raw, void *raw_data, int raw_data_length)
{
    ...
    if (dm == NULL || *dm == NULL || thing_id == NULL || raw_data == NULL || raw_data_length == 0) {
        return -1;
    }
...
int linkkit_invoke_ota_service(void *data_buf, int data_buf_length)
{
    ...
    if (ota == NULL || *ota == NULL || (*ota)->perform_ota_service == NULL || data_buf == NULL || data_buf_length <= 0) {
        return -1;
    }
                                          ...
                                          linkkit_extended_info_operate_t linkkit_extended_info_operation)
{
    ...
    if (linkkit_extended_info_operation == linkkit_extended_info_operate_update) {
        if (dm == NULL || *dm == NULL || (*dm)->trigger_extended_info_update == NULL || thing_id == NULL) {
            return -1;
        }
        ...
        if (dm == NULL || *dm == NULL || (*dm)->trigger_extended_info_delete == NULL || thing_id == NULL) {
            return -1;
        }
...
int linkkit_trigger_event(const void *thing_id, const char *event_identifier)
{
    ...
    if (dm == NULL || *dm == NULL || (*dm)->trigger_event == NULL || thing_id == NULL || event_identifier == NULL ||
        strcmp(event_identifier, EVENT_PROPERTY_POST_IDENTIFIER) == 0) {
        return -1;
    }
...
int linkkit_post_property(const void *thing_id, const char *property_identifier)
{
    ...
    if (dm == NULL || *dm == NULL || (*dm)->trigger_event == NULL || thing_id == NULL) {
        return -1;
    }
...
int linkkit_subdev_init(linkkit_subdev_ops_t *subdev_ops, void *user_subdev_context)
{
    ...
    if (!subdev_ops || !user_subdev_context) {
        return ret;
    }
...
void *linkkit_add_subdev(const void *gw_thing_id, const char *product_key, const char *device_name,
                         const char *device_secret, const char *tsl, int tsl_len)
{
    void *thing = NULL;
    const dm_t **dm = dm_object;

    if (dm && *dm && tsl && tsl_len > 0 && *tsl == '{' && *(tsl + tsl_len - 1) == '}' && product_key && device_name &&
        device_secret) {
