...
static void refresh_chart_label(void)
{
    ...
    ret = get_humi_data(&humi_value);
    if (ret != 0) {
        printf("humi sensor read failed !\n");
        return;
    }
	
    ret = get_temp_data(&temp_value);
    if (ret != 0) {
    ...
    if (count == 0) {
        ...
        for (i = 0; i <= 29; i++) {
            dl1_1->points[i] = (float)temp_value*0.1 + 20;
            dl1_2->points[i] = (float)humi_value*0.1 + 20;
        }
    } else {
        for (i = 28; i >= 0; i--) {
        ...
        dl1_1->points[0] = (float)temp_value*0.1 + 20;
        dl1_2->points[0] = (float)humi_value*0.1 + 20;
    ...
    /* refresh label */
    sprintf(number_buf, "%3.2f", (float)temp_value*0.1);
    strcpy(msg_buffer, "Temp is ");
    strcat(msg_buffer, number_buf);
    strcat(msg_buffer, " degree");

    lv_label_set_text(label1, msg_buffer);

    sprintf(number_buf, "%3.2f", (float)humi_value*0.1);
    strcpy(msg_buffer, "Humi is ");
    strcat(msg_buffer, number_buf);
    strcat(msg_buffer, " %(RH)");

    lv_label_set_text(label2, msg_buffer);
    //printf("aos_read humi=%3.2f RH,temp:%3.2f degree.\n", (float)humi_value*0.1, (float)temp_value*0.1);
    count++;
}
...
static int get_temp_data(uint32_t *temp)
{
    ...
    if (size != sizeof(data)) {
        printf("aos_read temp sensor return error.\n");
    }
