...
void event_handle(void *pcontext, void *pclient, iotx_mqtt_event_msg_pt msg)
{
    ...
    switch (msg->event_type) {
        ...
        case IOTX_MQTT_EVENT_PUBLISH_RECEIVED:
            EXAMPLE_TRACE("topic message arrived but without any related "
                          "handle: topic=%.*s, topic_msg=%.*s",
...
int mqtt_client(void)
{
    ...
    /* Device AUTH */
    if (0 != IOT_SetupConnInfo(__product_key, __device_name, __device_secret,
                               (void **)&pconn_info)) {
    ...
    EXAMPLE_TRACE("\n publish message: \n topic: %s\n payload: \%s\n rc = %d",
                  TOPIC_UPDATE, topic_msg.payload, rc);
    ...
    EXAMPLE_TRACE("\n publish message: \n topic: %s\n payload: \%s\n rc = %d",
                  TOPIC_DATA, topic_msg.payload, rc);
    ...
    do {
        ...
        cnt++;
        msg_len = snprintf(
          msg_pub, sizeof(msg_pub),
          "{\"attr_name\":\"temperature\", \"attr_value\":\"%d\"}", cnt);
        if (msg_len < 0) {
        ...
        EXAMPLE_TRACE("packet-id=%u, publish topic msg='0x%02x%02x%02x%02x'...",
                      (uint32_t)rc, msg_pub[0], msg_pub[1], msg_pub[2],
                      msg_pub[3]);
#else
        EXAMPLE_TRACE("packet-id=%u, publish topic msg=%s", (uint32_t)rc,
                      msg_pub);
#endif
...
int  mqtt_client_secure()
{
    ...
    /* Device AUTH */
    rc = IOT_SetupConnInfoSecure(__product_key, __device_id2, __device_id2,
                                 (void **)&pconn_info);
    if (rc != 0) {
        EXAMPLE_TRACE("AUTH request failed!");
        goto do_exit;
    ...
    HAL_Snprintf(topic_update, IOTX_URI_MAX_LEN, TOPIC_UPDATE_FMT,
                 __product_key, __device_id2);
    HAL_Snprintf(topic_error, IOTX_URI_MAX_LEN, TOPIC_ERROR_FMT, __product_key,
                 __device_id2);
    HAL_Snprintf(topic_get, IOTX_URI_MAX_LEN, TOPIC_GET_FMT, __product_key,
                 __device_id2);
    HAL_Snprintf(topic_data, IOTX_URI_MAX_LEN, TOPIC_DATA_FMT, __product_key,
                 __device_id2);
