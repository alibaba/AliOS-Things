...
#define SERVER_NAME "living.aliyun.com"
...
static int tls_client_random( void *p_rng, unsigned char *output, size_t output_len )
{
    ((void) p_rng);

    srandom( time( NULL ) + 1000 );
    for( int i = 0; i < output_len; i++ )
        *( output + i ) = (unsigned char) (random() ^ random());
...
int tls_client_test( void )
{
    ...
    if( buf == NULL )
    {
        mbedtls_printf( "failed! out of memory.\n" );
        return 0;
    }
    ...
    ret = mbedtls_x509_crt_parse( &cacert, (const unsigned char *) tls_client_test_ca_pem,
    ...
    if( ( flags = mbedtls_ssl_get_verify_result( &ssl ) ) != 0 )
    {
        ...
        vrfy_buf = (char *) mbedtls_calloc( 1, 512 );
        if( vrfy_buf == NULL )
        {
            mbedtls_printf( " failed\n  ! out of memory\n" );
            goto exit;
    ...
    mbedtls_printf( "TLS client test PASSED!\n" );
