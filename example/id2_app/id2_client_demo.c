...
static void test_id2_client_get_challenge_auth_code(void)
{
    ...
    ret =
      id2_client_get_challenge_auth_code(challenge, NULL, 0, auth_code, &len);
    if (ret != IROT_SUCCESS) {
    ...
    ret = id2_client_get_challenge_auth_code(challenge, (const uint8_t *)extra,
                                             strlen(extra), auth_code, &len);
    if (ret != 0) {
        id2_log_debug("id2_client_get_challenge_auth_code return %d error!\n",
                      ret);
    }
...
static void test_id2_client_get_timestamp_auth_code(void)
{
    ...
    ret = id2_client_get_timestamp_auth_code(timestamp_str, NULL, 0, auth_code,
                                             &len);
    if (ret != IROT_SUCCESS) {
    ...
    ret = id2_client_get_timestamp_auth_code(timestamp_str, (uint8_t *)extra,
                                             strlen(extra), auth_code, &len);
    if (ret != 0) {
        id2_log_debug("id2_client_get_timestamp_auth_code return %d error!\n",
                      ret);
...
static void test_id2_client_decrypt(void)
{
    ...
    const uint8_t ciphertext[] = {
        ...
        0x5E,
        0x44, // CONST_AES_128
    // 0x8D, 0x07, 0xF6, 0x38, 0x52, 0x28, 0x25, 0xBD,   0x28, 0xD3, 0x1D, 0x8D,
    // 0xCA, 0x21, 0xB6, 0xE9, //CONST_AES_192 0x72, 0x8D, 0x1F, 0xDB, 0x02,
    // 0x3B, 0x7D, 0x37,   0xAA, 0x47, 0xDA, 0x0F, 0x19, 0x6A, 0x37, 0x13,
    // //CONST_AES_256
#else
        0x00,
        0x00,
...
#if (ID2_OTP_SUPPORTED)
int test_id2_client_get_prov_stat(bool expect)
{
    irot_result_t ret;
    bool          is_prov;
    ...
    if (is_prov != expect) {
        id2_log_debug("bad id2 prov stat, %d\n", is_prov);
        return -1;
    }
...
int test_id2_client_get_otp_auth_code(int type)
{
    ...
    if (type == 0x01) {
        ...
        otp_auth = "AAABACQOSl8KZrYD5hPmp3Ln4ssZ4KHv9vIDYdq3264F3kLH6Ma6B6L/"
                   "RmkocBtMR+0KRiSRRDhBIuvP0RNFxNtMO3sEGDJO7knIDlWKQ7wQoNR/"
                   "xFW3Tpd2W8WQTa+YrMYtFqAne+PU9hZPmL7gH69HmrY=";
    }
#elif (ID2_CRYPTO_TYPE_CONFIG == ID2_CRYPTO_TYPE_3DES)
    if (type == 0x04) {
        token = "i14ejGyODBUwsjKTWri789E01AAD601E";
        otp_auth = "AAABAGtpp3AyqRqSbOTyqBeM4G2SHUqk7eo35w5CrwAUX032NTAh8KIhX1Z"
                   "enldVnU9fFkPeQTyl5CJAPF90wZzKbLMjsY0t+9TTMUuunbErnsvH19aIS+"
                   "7JnB8FP/Dy5OEDxq84kz6fNm0j";
    }
#else
#error("OTP not supported for RSA.");
#endif

    ret = id2_client_get_otp_auth_code((uint8_t *)token, strlen(token),
                                       auth_code, &auth_len);
    if (ret != IROT_SUCCESS) {
        ...
        irot_pal_base64_encode(auth_code, auth_len, temp_data, &temp_len);
...
int test_id2_client_load_otp_data(int type)
{
    ...
    if (type == 0x01) {
        const uint8_t tst_data[] = {
            0x01, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x31, 0x41,
            0x45, 0x35, 0x45, 0x41, 0x36, 0x31, 0x39, 0x30, 0x42, 0x37,
            0x38, 0x35, 0x42, 0x30, 0x30, 0xcc, 0x38, 0xa3, 0x03, 0x99,
            0xe0, 0xfe, 0x73, 0xb8, 0x57, 0xb8, 0xeb, 0x08, 0x9d, 0x9f,
            0x33, 0x34, 0x6b, 0xce, 0x0b, 0x8e, 0xed, 0x34, 0xda, 0x10,
            0xf6, 0xa8, 0xfa, 0xbb, 0x84, 0x44, 0x94, 0x82, 0x59, 0x6a,
            0x9e, 0x2e, 0x5a, 0x5a, 0xa8, 0x55, 0x13, 0xa4, 0x7b, 0xad,
            0xde, 0xc4, 0xdc, 0x90, 0x1e, 0x3b, 0x10, 0x87, 0x36, 0xdb,
    ...
    if (type == 0x04) {
        const uint8_t tst_data[] = {
            0x04, 0x30, 0x30, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x42, 0x42,
            0x42, 0x42, 0x42, 0x42, 0x34, 0x38, 0x42, 0x36, 0x39, 0x43, 0x44,
            0x32, 0x30, 0x30, 0x4b, 0xcd, 0x91, 0x45, 0x7a, 0xe9, 0xb0, 0x67,
            0xda, 0x42, 0xd4, 0x44, 0x54, 0x76, 0x06, 0xf0, 0xad, 0x6a, 0x88,
            0xb4, 0xfa, 0x37, 0x83, 0x3d, 0x49, 0x4d, 0x87, 0x69, 0xae, 0x17,
            0xcf, 0x0a, 0xc4, 0x62, 0xd5, 0x19, 0x67, 0xaf, 0x52, 0x49, 0x60,
            0xa1, 0x86, 0x44, 0x14, 0xbb, 0x67, 0xf8, 0xdd, 0x14, 0xd0, 0x74,
            0x1e, 0x00, 0x19, 0xe5,
        };
...
int test_id2_client_set_id_and_key(void)
{
    ...
    const char *keydata  = "0102030405060708090A0B0C0D0E0F10";
#elif (ID2_CRYPTO_TYPE_CONFIG == ID2_CRYPTO_TYPE_AES)
    int key_type = 2;
    char *id2 = "2122232425262728292A2B2C";
    const char *keydata = "2122232425262728292A2B2C2D2E2F30";
#else
#endif
...
void id2_client_test(void)
{
    ...
    ret = test_id2_client_get_prov_stat(false);
    if (ret != 0) {
        id2_log_debug("id2 client get prov stat test fail\n");
    }
    ...
    id2_log_debug("\n");
    id2_log_debug("test: id2_client_get_prov_stat\n");
    ret = test_id2_client_get_prov_stat(true);
    if (ret != 0) {
        id2_log_debug("id2 client get prov stat test fail\n");
        return;
    }
...
#if (ID2_SET_ID_KEY_SUPPORTED)
        id2_log_debug("test: id2_client_set_id_and_key\n");
        test_id2_client_set_id_and_key();
#endif
