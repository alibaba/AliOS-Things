...
OSStatus scan_result_handler                    ( const aos_bt_smart_advertising_report_t *result );
...
int application_start(void)
{
    ...
    err = aos_bt_init(AOS_BT_HCI_MODE, "SmartBridge WL", MAX_CONCURRENT_CONNECTIONS, 1);  //Client + server connections
    ...
    aos_rtos_create_worker_thread(&hello_center_worker_thread,
                                  AOS_DEFAULT_WORKER_PRIORITY,
                                  2048,
                                  1);
    /* Register a timer and create a worker thread to control event. */
    aos_rtos_create_worker_thread(&hello_sensor_color_toggle_worker_thread,
                                  AOS_DEFAULT_WORKER_PRIORITY,
                                  1024,
                                  1);
    aos_rtos_register_timed_event(&hello_sensor_color_toggle_event,
...
OSStatus sensor_trigger_handler_t(void *arg)
{
    ...
    aos_bt_smart_attribute_create(&characteristic_value,
    ...
    for (i = 0; i < MAX_CONCURRENT_CONNECTIONS; i++) {
        ...
        if (status == SMARTBRIDGE_SOCKET_CONNECTED && smartbridge_socket[i].att_cache != NULL) {
            err = aos_bt_smartbridge_get_attribute_cache_by_handle(&smartbridge_socket[i],
            ...
            if (err == oNoErr) {
                ...
                aos_bt_smartbridge_disconnect(&smartbridge_socket[i], TRUE);
            }
        }
    }
    ...
    while (connect_list_get(&remote_device) == oNoErr) {
        ...
        for (i = 0; i < MAX_CONCURRENT_CONNECTIONS; i++) {
            ...
            if (status == SMARTBRIDGE_SOCKET_DISCONNECTED) {
                ...
                /* Connect */
                err = aos_bt_smartbridge_connect(&smartbridge_socket[i],
                                                 remote_device,
                                                 &connection_settings,
                ...
                /* Find service */
                err = aos_bt_smartbridge_get_service_from_attribute_cache_by_uuid(&smartbridge_socket[i],
                                                                                  &hello_service_uuid,
                                                                                  0x0,
                                                                                  0xFFFF,
                                                                                  attribute,
                                                                                  100);
                require_noerr_action_string(err, exit, aos_bt_smartbridge_disconnect(&smartbridge_socket[i], TRUE),
                                            "Hello service not found, disconnect.");

                /* Find characteristic, and save characteristic value handle */
                err = aos_bt_smartbridge_get_characteritics_from_attribute_cache_by_uuid(&smartbridge_socket[i],
                                                                                         &hello_color_uuid,
                                                                                         attribute->value.service.start_handle,
                                                                                         attribute->value.service.end_handle,
                                                                                         (aos_bt_smart_attribute_t *)attribute_buffer,
                                                                                         100);
                if (err != oNoErr) {
...
static OSStatus notification_handler(aos_bt_smartbridge_socket_t *socket, uint16_t attribute_handle)
{
    ...
    /* Read cached data */
    err = aos_bt_smart_attribute_create(&characteristic_value,
                                        AOS_ATTRIBUTE_TYPE_CHARACTERISTIC_VALUE,
                                        MAX_CHARACTERISTIC_VALUE_LENGTH);
    require_noerr(err, exit);
    err = aos_bt_smartbridge_get_attribute_cache_by_handle(socket,
                                                           attribute_handle,
                                                           characteristic_value,
...
static OSStatus auto_connection_handler(aos_bt_smartbridge_socket_t *socket)
{
    OSStatus err = oNoErr;
    ...
    /* Find service */
    err = aos_bt_smartbridge_get_service_from_attribute_cache_by_uuid(socket,
                                                                      &hello_service_uuid,
                                                                      0x0,
                                                                      0xFFFF,
                                                                      attribute,
                                                                      100);
    require_noerr_action_string(err, exit, aos_bt_smartbridge_disconnect(socket, FALSE),
                                "Hello service not found, disconnect.");

    /* Find characteristic, and save characteristic value handle */
    err = aos_bt_smartbridge_get_characteritics_from_attribute_cache_by_uuid(socket,
                                                                             &hello_color_uuid,
                                                                             attribute->value.service.start_handle,
                                                                             attribute->value.service.end_handle,
                                                                             (aos_bt_smart_attribute_t *)attribute_buffer,
    ...
    if (err != oNoErr) {
        printf("Hello color characteristic not found, remove att cache and disconnect.");
        aos_bt_smartbridge_remove_attribute_cache(socket);
        aos_bt_smartbridge_disconnect(socket, FALSE);
    }

    hello_color_value_handle[idx] = attribute->value.characteristic.value_handle;

    printf("Color value handle: %lu", hello_color_value_handle[idx]);
    /* Enable Attribute Cache notification */
    //err = aos_bt_smartbridge_enable_attribute_cache_notification( &smartbridge_socket[i], AOS_TRUE );
    //require_noerr_string( err, exit, "Attribute cache notification failed." );
...
static int8_t get_a_free_socket_for_auto_connection(aos_bt_smartbridge_socket_t *socket, uint8_t socket_size)
{
    int8_t idx = 0;
    aos_bt_smartbridge_socket_status_t status;
    ...
    for (idx = 0; idx < socket_size; idx++) {
        aos_bt_smartbridge_get_socket_status(&socket[idx], &status);
        if (status == SMARTBRIDGE_SOCKET_DISCONNECTED) {
            return idx;
        }
    }
...
OSStatus ble_auto_conn_parms_callback_handler(const aos_bt_device_address_t device_address,
                                              const char *name,
                                              ...
                                              aos_bt_smartbridge_auto_conn_cback_parms_t *parm)
{
    ...
    if (auto_idx < 0) {
        printf("There are not free sockes");
        return oNoResourcesErr;
    }

    parm->socket = &smartbridge_socket[auto_idx];
    memcpy((void *)parm->socket->remote_device.address, (void *)device_address, sizeof(aos_bt_device_address_t));
    memcpy((void *)parm->socket->remote_device.name, (void *)name, MIN(sizeof(parm->socket->remote_device.name),
                                                                       strlen(name)));
    ...
    memcpy((void *)&parm->conn_settings, (void *)&connection_settings, sizeof(aos_bt_smart_connection_settings_t));
    memcpy((void *)&parm->security_settings, (void *)&security_settings, sizeof(aos_bt_smart_security_settings_t));
    return oNoErr;
}
