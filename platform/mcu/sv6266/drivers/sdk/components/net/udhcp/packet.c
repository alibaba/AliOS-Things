...
#if DHCPD_SUPPORT
extern ssize_t FAST_FUNC safe_read(int fd, void *buf, size_t count);
...
void FAST_FUNC udhcp_dump_packet(struct dhcp_packet *packet)
{
    ...
    p = (char*)bin2hex(buf, (void *) packet->chaddr, sizeof(packet->chaddr));
	*p = '\0';
	bb_info_msg(" chaddr %s\r\n", buf);
}
#endif
...
//	setsockopt_reuseaddr(fd);
 ...
 ret_close:
	...
	if (result < 0) {
 ret_msg:
		bb_perror_msg((char *)msg, "UDP");
	}
	return result;
}

int FAST_FUNC udhcp_send_kernel_packet_dummy(char *buffer, int len,
		uint32_t dest_nip, int dest_port)
{
	struct sockaddr_in sa;
	int fd;
