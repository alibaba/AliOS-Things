...
#ifndef SPIFFS_INTERFACE_H_
#define SPIFFS_INTERFACE_H_
 ...
 */
s32_t prvSPIFFS_probe_fs(spiffs_config *config);
 ...
 */
s32_t prvSPIFFS_mount(spiffs *fs, spiffs_config *config, u8_t *work,
 ...
 * @param fs            the file system struct
 */
void prvSPIFFS_unmount(spiffs *fs);
 ...
 */
s32_t prvSPIFFS_creat(spiffs *fs, const char *path, spiffs_mode mode);
 ...
 * @param mode          ignored, for posix compliance
 */
spiffs_file prvSPIFFS_open(spiffs *fs, const char *path, spiffs_flags flags, spiffs_mode mode);
 ...
 * @param mode          ignored, for posix compliance
 */
spiffs_file prvSPIFFS_open_by_dirent(spiffs *fs, struct spiffs_dirent *e, spiffs_flags flags, spiffs_mode mode);
 ...
 * @param mode          ignored, for posix compliance
 */
spiffs_file prvSPIFFS_open_by_page(spiffs *fs, spiffs_page_ix page_ix, spiffs_flags flags, spiffs_mode mode);
 ...
 */
s32_t prvSPIFFS_read(spiffs *fs, spiffs_file fh, void *buf, s32_t len);
 ...
 */
s32_t prvSPIFFS_write(spiffs *fs, spiffs_file fh, void *buf, s32_t len);
 ...
 */
s32_t prvSPIFFS_lseek(spiffs *fs, spiffs_file fh, s32_t offs, int whence);
 ...
 */
s32_t prvSPIFFS_remove(spiffs *fs, const char *path);
 ...
 */
s32_t prvSPIFFS_fremove(spiffs *fs, spiffs_file fh);
 ...
 */
s32_t prvSPIFFS_stat(spiffs *fs, const char *path, spiffs_stat *s);
 ...
 * @param s             the stat struct to populate
 */
s32_t prvSPIFFS_fstat(spiffs *fs, spiffs_file fh, spiffs_stat *s);
 ...
 */
s32_t prvSPIFFS_fflush(spiffs *fs, spiffs_file fh);
 ...
 */
s32_t prvSPIFFS_close(spiffs *fs, spiffs_file fh);
 ...
 */
s32_t prvSPIFFS_rename(spiffs *fs, const char *old, const char *newPath);

/**
 * Returns last error of last file operation.
 * @param fs            the file system struct
 */
s32_t prvSPIFFS_errno(spiffs *fs);

/**
 * Clears last error.
 * @param fs            the file system struct
 */
void prvSPIFFS_clearerr(spiffs *fs);
 ...
 */
spiffs_DIR *prvSPIFFS_opendir(spiffs *fs, const char *name, spiffs_DIR *d);

/**
 * Closes a directory stream
 * @param d             the directory stream to close
 */
s32_t prvSPIFFS_closedir(spiffs_DIR *d);
 ...
 */
struct spiffs_dirent *prvSPIFFS_readdir(spiffs_DIR *d, struct spiffs_dirent *e);

/**
 * Runs a consistency check on given filesystem.
 * @param fs            the file system struct
 */
s32_t prvSPIFFS_check(spiffs *fs);
 ...
 */
s32_t prvSPIFFS_info(spiffs *fs, u32_t *total, u32_t *used);
 ...
 * @param fs            the file system struct
 */
s32_t prvSPIFFS_format(spiffs *fs);

/**
 * Returns nonzero if spiffs is mounted, or zero if unmounted.
 * @param fs            the file system struct
 */
u8_t prvSPIFFS_mounted(spiffs *fs);
 ...
 */
s32_t prvSPIFFS_gc_quick(spiffs *fs, u16_t max_free_pages);
 ...
 */
s32_t prvSPIFFS_gc(spiffs *fs, u32_t size);
 ...
 */
s32_t prvSPIFFS_eof(spiffs *fs, spiffs_file fh);
 ...
 * @param fh            the filehandle of the file to check
 */
s32_t prvSPIFFS_tell(spiffs *fs, spiffs_file fh);
 ...
 */
s32_t prvSPIFFS_set_file_callback_func(spiffs *fs, spiffs_file_callback cb_func);
 ...
 */
u32_t prvSPIFFS_buffer_bytes_for_filedescs(spiffs *fs, u32_t num_descs);
 ...
 */
u32_t prvSPIFFS_buffer_bytes_for_cache(spiffs *fs, u32_t num_pages);
#endif
#endif
