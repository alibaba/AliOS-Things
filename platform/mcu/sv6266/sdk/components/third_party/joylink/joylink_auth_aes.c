...
//AES crypt
int joylinkEnc2Crypt(uint8_t *key, uint32_t keyLen, uint8_t *iv, uint8_t *data, uint32_t *len, uint32_t maxLen, int isPKCS5, int type)
{
    int dl;
    int ret;
...
int JLdevice_aes_encrypt(const uint8_t * key, int keyLength,
        const uint8_t * iv, const uint8_t *pPlainIn,
        int plainLength, uint8_t *pEncOut, int maxOutLen)
{
    char TIV[128] = {0}; 
    //strncpy(TIV, (char*)iv, strlen((char *)iv));
	memcpy(TIV,  (char*)iv, keyLength);
    ...
    if(NULL != tdata){
        ...
        ret = joylinkEnc2Crypt((uint8_t *)key, 128, 
                (uint8_t*)TIV, (uint8_t*)tdata, (uint32_t*)&dlen, plainLength + 16, 1, JOYLINK_ENC2_ENCRYPT);
        if(ret == 0){
            if(dlen <= maxOutLen){
...
int JLdevice_aes_decrypt(const uint8_t * key, int keyLength, 
        const uint8_t * iv, const uint8_t *pEncIn, 
        int encLength, uint8_t *pPlainOut, int maxOutLen)
{
    char TIV[128] = {0}; 
    //strncpy(TIV, (char*)iv, strlen((char*)iv));
    memcpy(TIV,  (char*)iv, keyLength);
    ...
    if(NULL != tdata){
        ...
        ret = joylinkEnc2Crypt((uint8_t *)key, 128, 
                (uint8_t*)TIV, (uint8_t*)tdata, (uint32_t*)&dlen, encLength + 16, 1, JOYLINK_ENC2_DECRYPT);
        if(ret == 0){
            if(dlen <= maxOutLen){
