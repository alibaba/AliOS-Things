...
//TODO: learn how to use overflow option if we exhaust packet->options[]
	...
	memcpy((void*)(optionptr + end), addopt, len);
		...
		int length)
{
	...
	if (!existing) {
		...
		memcpy((void*)(new_opt->data + OPT_DATA), (void*)(allocated ? allocated : buffer), length);
	...
	if (optflag->flags & OPTION_LIST) {
		...
		if (old_len + length < 255) {
			...
			memcpy((void*)(existing->data + OPT_DATA + old_len), (void*)(allocated ? allocated : buffer), length);
