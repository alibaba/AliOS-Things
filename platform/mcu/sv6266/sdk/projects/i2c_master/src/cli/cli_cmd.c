...
void Cmd_i2c_master_rw_wac(s32 argc, char *argv[])
{
    ...
    uint16_t i2cmaster_clk = 0x0;

    i2cmaster_clk = strtoul(argv[1], NULL, 10);
    
    if ((argc != 2) || (i2cmaster_clk < 1) || (i2cmaster_clk > 1000)) {
        printf("Usage           : i2c_master_rw <clk_KHz>\n");
    ...
    if(0 != drv_i2c_mst_write(I2C_SLAVE_ADDR_WAC, read_addr, 1)){
        printf("write fail!\n");
        return ;
    }
        
    if(0 != drv_i2c_mst_read(I2C_SLAVE_ADDR_WAC, read_buf, AH_DEVICE_VERSION_LEN)){
        printf("read fail!\n");
        return ;
    }
    ...
    if(0 != drv_i2c_mst_read(I2C_SLAVE_ADDR_WAC, read_buf, AH_FIRMWARE_VERSON_LEN)){
        printf("read fail!\n");
        return ;
    }
    ...
    if(0 != drv_i2c_mst_read(I2C_SLAVE_ADDR_WAC, read_buf, AH_AUTHENTICATION_PROTOCOL_MAJOR_VERSION_LEN)){
        printf("read fail!\n");
        return ;
    }
    ...
    if(0 != drv_i2c_mst_read(I2C_SLAVE_ADDR_WAC, read_buf, AH_AUTHENTICATION_PROTOCOL_MINOR_VERSION_LEN)){
        printf("read fail!\n");
        return ;
    }
    ...
    if(0 != drv_i2c_mst_read(I2C_SLAVE_ADDR_WAC, read_buf, AH_DEVICE_ID_LEN)){
        printf("read fail!\n");
        return ;
    }
    ...
    if(0 != drv_i2c_mst_write(I2C_SLAVE_ADDR_WAC, write_buf, AH_IPOD_CERTIFICATE_DATA_LENGTH_LEN + 1)){
        printf("write fail!\n");
        return ;
    }
    ...
    if(0 != drv_i2c_mst_read(I2C_SLAVE_ADDR_WAC, read_buf, AH_IPOD_CERTIFICATE_DATA_LENGTH_LEN)){
        printf("read fail!\n");
        return ;
    }
...
void Cmd_i2c_master_rw(s32 argc, char *argv[])
{
    ...
    if(0 != drv_i2c_mst_write(I2C_SLAVE_ADDR, write_buf, I2C_SLAVE_RW_SIZE)){
        printf("write fail!\n");
        return ;
    }
    ...
    if(0 != drv_i2c_mst_write(I2C_SLAVE_ADDR, &data_start_addr, 1)){
        printf("write fail!\n");
        return ;
    }
    if(0 != drv_i2c_mst_read(I2C_SLAVE_ADDR, read_buf, I2C_SLAVE_RW_SIZE)){
        printf("read fail!\n");
        return ;
    }
