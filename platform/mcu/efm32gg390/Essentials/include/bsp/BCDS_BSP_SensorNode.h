 ...
 *  @file
 *
 *  @defgroup BCDS_HAL_BSP_Sensor_Node Sensor_Node
...
#ifndef BCDS_BSP_SensorNode_H
#define BCDS_BSP_SensorNode_H
 ...
 * process data.
 * @param id identifier of the sensor targeted by the enable request.
 *
 * @param callback: If the sensor has dedicated interrupt lines a callback
 * is required to process the generated events.
 *
 * @retval RETCODE_OK in the case of success or an error code otherwise.
 */
 ...
 * vendor datasheet.
 *
 * @param id: identifier of  the sensor targeted by the disable request. This is defined
 * in the Board Interface under the BSP_Sensor_E enumeration.
 *
 * @retval RETCODE_OK in the case of success or an error code otherwise.
 */
...
/**
 * @brief Returns the boolean (TRUE/FALSE) based on the sensor node Enable/Disable status used by the
 * peripheral in case I2C is used for the sensor node.
 * @details only after successful Disable of all the sensor nodes this API will return TRUE.
 * Even if one sensor node is enabled then we get FALSE from this API
