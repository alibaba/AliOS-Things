...
static int mmc_get_ext_csd(struct rt_mmcsd_card *card, uint8_t **new_ext_csd)
{
  ...
  */
  ext_csd = aos_malloc(512);
  if (!ext_csd) {
    printf("alloc memory failed when get ext csd\n");
    return -1;
  }
...
static int mmc_parse_ext_csd(struct rt_mmcsd_card *card, uint8_t *ext_csd)
{
  ...
  printf("emmc card capacity %ld KB\n", card->card_capacity);
    ...
    */
    ...
    if (!err) {
      ...
      switch(ext_csd_bits[idx]){
          case 0:
            printf("switch to bus width 1 bit failed\n");
            break;
          case 1:
            printf("switch to bus width 4 bit failed\n");
            break;
          case 2:
            printf("switch to bus width 8 bit failed\n");
            break;
          default:
            break;
      }
...
int32_t mmc_send_op_cond(struct rt_mmcsd_host *host, 
...
static int32_t mmc_set_card_addr(struct rt_mmcsd_host *host, uint32_t rca)
{
  int32_t err;
  struct rt_mmcsd_cmd cmd;
