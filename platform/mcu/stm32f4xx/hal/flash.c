...
static OSStatus hal_flash_init( hal_partition_t in_partition )
{
  ...
  require_action_quiet( p_partition_info->partition_owner != HAL_FLASH_NONE, exit, err = kNotFoundErr );
  
  if( platform_flash_drivers[ p_partition_info->partition_owner ].flash_mutex == NULL){
    err = mico_rtos_init_mutex( &platform_flash_drivers[ p_partition_info->partition_owner ].flash_mutex );
    require_noerr( err, exit );
  }
  
  mico_rtos_lock_mutex( &platform_flash_drivers[ p_partition_info->partition_owner ].flash_mutex );
  
  err = platform_flash_init( &platform_flash_peripherals[ p_partition_info->partition_owner ] );
  platform_flash_drivers[ p_partition_info->partition_owner ].peripheral = (platform_flash_t *)&platform_flash_peripherals[ p_partition_info->partition_owner ];
  platform_flash_drivers[ p_partition_info->partition_owner ].initialized = true;
  mico_rtos_unlock_mutex( &platform_flash_drivers[ p_partition_info->partition_owner ].flash_mutex );
...
int32_t hal_flash_erase(hal_partition_t in_partition, uint32_t off_set, uint32_t size)
{
  ...
  require_action_quiet( ( p_partition_info->partition_options & PAR_OPT_WRITE_MASK ) == PAR_OPT_WRITE_EN, exit, err = kPermissionErr );
#endif

  start_addr = p_partition_info->partition_start_addr + off_set;
  end_addr = p_partition_info->partition_start_addr + off_set + size - 1;

  require_action_quiet( end_addr < p_partition_info->partition_start_addr + p_partition_info->partition_length, exit, err = kParamErr );

  if( platform_flash_drivers[ p_partition_info->partition_owner ].initialized == false )
  {
  ...
  mico_rtos_lock_mutex( &platform_flash_drivers[ p_partition_info->partition_owner ].flash_mutex );
  err = platform_flash_erase( &platform_flash_peripherals[ p_partition_info->partition_owner ], start_addr, end_addr );
  mico_rtos_unlock_mutex( &platform_flash_drivers[ p_partition_info->partition_owner ].flash_mutex );
...
int32_t hal_flash_write(hal_partition_t in_partition, uint32_t *off_set, const void *in_buf , uint32_t in_buf_len)
{
  ...
  start_addr = p_partition_info->partition_start_addr + *off_set;
  end_addr = p_partition_info->partition_start_addr + *off_set + in_buf_len - 1;
  ...
  mico_rtos_lock_mutex( &platform_flash_drivers[ p_partition_info->partition_owner ].flash_mutex );
  err = platform_flash_write( &platform_flash_peripherals[ p_partition_info->partition_owner ], &start_addr, in_buf, in_buf_len );
  *off_set = start_addr - p_partition_info->partition_start_addr;
  mico_rtos_unlock_mutex( &platform_flash_drivers[ p_partition_info->partition_owner ].flash_mutex );
...
int32_t hal_flash_read(hal_partition_t in_partition, uint32_t *off_set, void *out_buf, uint32_t out_buf_len)
{
  ...
  require_action_quiet( p_partition_info->partition_owner != HAL_FLASH_NONE, exit, err = kNotFoundErr );
#if (!defined BOOTLOADER) && (!defined FIRMWARE_DOWNLOAD)
  require_action_quiet( ( p_partition_info->partition_options & PAR_OPT_READ_MASK ) == PAR_OPT_READ_EN, exit, err = kPermissionErr );
#endif

  start_addr = p_partition_info->partition_start_addr + *off_set;
  end_addr = p_partition_info->partition_start_addr + *off_set + out_buf_len - 1;
  require_action_quiet( end_addr < p_partition_info->partition_start_addr + p_partition_info->partition_length , exit, err = kParamErr );
  ...
  mico_rtos_lock_mutex( &platform_flash_drivers[ p_partition_info->partition_owner ].flash_mutex );
  err = platform_flash_read( &platform_flash_peripherals[ p_partition_info->partition_owner ], &start_addr, out_buf, out_buf_len );
  *off_set = start_addr - p_partition_info->partition_start_addr;
  mico_rtos_unlock_mutex( &platform_flash_drivers[ p_partition_info->partition_owner ].flash_mutex );
