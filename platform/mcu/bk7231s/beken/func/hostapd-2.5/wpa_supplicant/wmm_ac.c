			 ...
			 const struct wmm_tspec_element *tspec)
{
	...
	int idx = wmm_ac_direction_to_idx(dir); // enum ts_dir_idx
				    ...
				    u8 tsid, u8 ac, u8 dir)
{
	...
	cur_ac = wmm_ac_find_tsid(wpa_s, tsid, (enum ts_dir_idx *)&idx);
	if (cur_ac >= 0) {
		if (cur_ac != ac) {
			wpa_printf(MSG_DEBUG,
...
static void wmm_ac_del_ts(struct wpa_supplicant *wpa_s, u8 ac, int dir_bitmap)
{
	...
	for (idx = 0; idx < TS_DIR_IDX_COUNT; idx++) {
		...
		wmm_ac_del_ts_idx(wpa_s, ac, (enum ts_dir_idx)idx);
	}
}
