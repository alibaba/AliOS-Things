...
STRUCT_PACKED struct session_transition_ie {
	u8 element_id;
	u8 length;
	u32 fsts_id;
	u8 session_control;
...
STRUCT_PACKED struct fst_setup_res {
	u8 action;
	u8 dialog_token;
	u8 status_code;
	struct session_transition_ie stie;
...
STRUCT_PACKED struct fst_ack_req {
	u8 action;
	u8 dialog_token;
	u32 fsts_id;
}GCC_PACKED;

STRUCT_PACKED struct fst_ack_res {
	u8 action;
	u8 dialog_token;
	u32 fsts_id;
}GCC_PACKED;

STRUCT_PACKED struct fst_tear_down {
	u8 action;
	u32 fsts_id;
}GCC_PACKED;
