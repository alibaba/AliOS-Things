                               ...
                               dma_config_t *config, dma_event_cb_t cb_event)
{
    ...
    int32_t ret = ck_dma_set_transfertype(addr, config->type);
                      ...
                      void *pdstaddr, uint32_t length)
{
    ...
    int32_t ret = ck_dma_set_transferwidth(addr, dma_priv->src_tw, dma_priv->dst_tw);
    ...
    int32_t grouplen = ((length * dma_priv->src_tw / dma_priv->dst_tw) - 1) % 16;
