...
int32_t drv_sasc_config_region(uint32_t addr, sasc_size_e size, sasc_ap_e sap, sasc_ap_e uap, sasc_cd_e ucd, uint8_t is_security)
{
    ...
    if (IS_EFLASH_ADDR(addr)) {
        ret = eflash_region_config(addr, size - SASC_EFLASH_1S, sap, uap, user_cd, is_security);
    } else if (IS_SRAM_ADDR(addr)) {
        ret = sram_region_config(addr, size, sap, uap, user_cd, is_security);
    } else {
        return -1;
    }
