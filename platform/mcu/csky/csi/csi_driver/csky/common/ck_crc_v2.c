...
static const crc_capabilities_t driver_capabilities = {
    ...
    .X25 = 1, /* X25 mode */
    .CCITT = 1, /* CCITT mode */
    .CCITT_FALSE = 1, /* CCITT_FALSE mode */
    .USB = 1, /* USB mode */
    .IBM = 1, /* IBM mode */
    .MODBUS = 1,  /* MODBUS mode */
    .ITU = 1,  /* ITU mode */
    .PMEQ_2 = 1,  /* PMEQ_2 mode */
    ...
    if (mode == CRC_MODE_CRC16) {
        switch (standard) {
            case CRC_STANDARD_CRC_MODBUS:
                crc_reg->CONFIG_REG = 0x8;
                break;

            case CRC_STANDARD_CRC_IBM:
                crc_reg->CONFIG_REG = 0x5;
                break;

            case CRC_STANDARD_CRC_MAXIM:
                crc_reg->CONFIG_REG = 0x6;
                break;

            case CRC_STANDARD_CRC_USB:
                crc_reg->CONFIG_REG = 0x7;
                break;

            case CRC_STANDARD_CRC_CCITT:
                crc_reg->CONFIG_REG = 0x9;
                break;

            case CRC_STANDARD_CRC_CCITT_FALSE:
                crc_reg->CONFIG_REG = 0xa;
                break;

            case CRC_STANDARD_CRC_X25:
                crc_reg->CONFIG_REG = 0xb;
                break;

            case CRC_STANDARD_CRC_XMODEM:
                crc_reg->CONFIG_REG = 0xc;
                break;

            case CRC_STANDARD_CRC_DNP:
                crc_reg->CONFIG_REG = 0xd;
                break;
        ...
        switch (standard) {
            case CRC_STANDARD_CRC_MAXIM:
                crc_reg->CONFIG_REG = 0x4;
                break;

            case CRC_STANDARD_CRC_ROHC:
                crc_reg->CONFIG_REG = 0x3;
                break;

            case CRC_STANDARD_CRC_ITU:
                crc_reg->CONFIG_REG = 0x2;
                break;

            case CRC_STANDARD_CRC_NONE:
                crc_reg->CONFIG_REG = 0x1;
                break;

            default:
                return ERR_CRC(DRV_ERROR_PARAMETER);
        }
    } else if (mode == CRC_MODE_CRC32) {
        switch (standard) {
            case CRC_STANDARD_CRC_PMEQ_2:
                crc_reg->CONFIG_REG = 0xf;
                break;

            case CRC_STANDARD_CRC_NONE:
                crc_reg->CONFIG_REG = 0xe;
                break;
...
{
    crc_reg->NEW_DATA = data;
    return 0;
}

static int32_t crc_get_data(uint32_t *data)
{
    *data = crc_reg->CRC_RESULT;
    return 0;
}
