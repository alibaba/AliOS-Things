...
#ifndef _SPC_MAILBOX_V5_H_
#define _SPC_MAILBOX_V5_H_
...
#define MBOX_SAIC_STATE_STATE_POS   (0)
#define MBOX_SAIC_STATE_REQ_POS   (1)
#define MBOX_SAIC_STATE_RSP_POS   (2)
...
} SPC_MBOX_WIN_SPC_TO_PAL_RX_T; //Size : 0x12
...
} SPC_MBOX_WIN_SPC_TO_PAL_T; //Size : 0x12
...
typedef struct
{
    ...
    SPC_MBOX_STATIC_SPC_TO_PAL_T   statWin;                      //0x00000038
    /// decoding structure for VOC
    SPC_MBOX_SPEECH_DEC_IN_T       speechDecIn;                  //0x000000B4
    /// array of pointer given by SPC to store the data to encode +1 for CSD block
    /// to encode
    UINT8*                         encBuff[MBOX_MAX_BUF_IDX_QTY+1]; //0x000000E8
    /// used by SPC to warn PAL that DTX uplink has been used
    BOOL                           dtxUsed;                      //0x00000100
    UINT8                          padding[3];                   //0x00000101
    UINT8                          bstSnr[12];                   //0x00000104
    /// power measured on the burst for 4 slots
    UINT8                          pwrs[12];                     //0x00000110
    /// used to report multi-slot DC
    INT16                          multiDcI[6];                  //0x0000011C
    INT16                          multiDcQ[6];                  //0x00000128
    /// used by SPC to indicate the CRC result is from saic or normal process:1 saic,0
    /// normal
    UINT32                         decodeFlag;                   //0x00000134
    /// debug for BCPU
    UINT32                         debug[2];                     //0x00000138
} SPC_MBOX_SPC_TO_PAL_T; //Size : 0x140
...
typedef struct
{
    ...
    /// GSM TDMA counter
    SPC_MBOX_GSM_COUNTERS_T        counters[MBOX_CTX_SNAP_QTY];  //0x000000A4
    /// encoding structure comming from VOC
    SPC_MBOX_SPEECH_ENC_OUT_T      speechEncOut;                 //0x000000B0
    /// RF parameters
    SPC_MBOX_RF_PARAMS_T           rf;                           //0x000000DC
    /// indicates to SPC if the DTX uplink is allowed for the current TCH
    BOOL                           dtxAllowed;                   //0x000000F0
    /// FCCH Search Mode
    UINT8                          FchSearchMode;                //0x000000F1
} SPC_MBOX_PAL_TO_SPC_T; //Size : 0xF4
...
typedef struct
{
    ...
    SPC_MBOX_SPC_TO_PAL_T          spc2pal;                      //0x000000F4
    UINT16                         dummyPchResult;               //0x00000234
    UINT16                         dummyPchThresh;               //0x00000236
    /// #ifdef __MBOX_DEBUG__ This is used by the embedded debug stub
    SPC_MBOX_DEBUG_T               dbg[MBOX_CTX_SNAP_QTY];       //0x00000238
    UINT32                         palDbgReq;                    //0x00000274
    UINT32                         palDbgAck;                    //0x00000278
} SPC_MAILBOX_T; //Size : 0x27C



#define MBOX_SAIC_STATE_REQ_SET(value)  (1<<MBOX_SAIC_STATE_REQ_POS) |(value << MBOX_SAIC_STATE_STATE_POS)
#define MBOX_SAIC_STATE_RSP_SET  (1<<MBOX_SAIC_STATE_RSP_POS)

#define MBOX_SAIC_STATE_REQ_GET(value)  ((value >> MBOX_SAIC_STATE_REQ_POS) & 1)
#define MBOX_SAIC_STATE_RSP_GET(value)  ((value  >> MBOX_SAIC_STATE_RSP_POS) & 1)
#define MBOX_SAIC_STATE_STATE_GET(value)  ((value >>MBOX_SAIC_STATE_STATE_POS) & 1)
