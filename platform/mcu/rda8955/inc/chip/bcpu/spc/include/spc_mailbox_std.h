...
#define MBOX_BCPU_ROM_ERR_WORD                  (0XBB0E000E)
/// Ciphering algorithm
...
/// Block Decode done for egprs
    MBOX_DSP_NB_DECODING_COMPLETE               = 0x00000020,
/// cch decoding complete
    MBOX_DSP_CCH_DECODING_COMPLETE                      = 0x00000040
} SPC_MBOX_DSP_ITR_T;
...
/// packet data channel for egprs
    MBOX_EPDCH_CHN                              = 0x00000007
} SPC_MBOX_CHAN_TYPES_T;
...
typedef struct
{
    ...
    /// number of corrected bit in the header
    UINT16                         headdbitError;                //0x0000000C
    /// bit error probability on the block
    UINT8                          meanBep;                      //0x0000000E
    /// coefficient of variance of meanBep
    UINT8                          cvBep;                        //0x0000000F
    /// bad frame indicator
    UINT8                          bfi;                          //0x00000010
    /// decoded coding scheme in GPRS
    UINT8                          cs;                           //0x00000011
    /// indicates the silence frame detection
    UINT8                          sid;                          //0x00000012
    /// indicates when IR buffer is in overflow
    UINT8                          irBufOvfl;                    //0x00000013
...
typedef struct
{
    ...
    /// power measured on the burst for 4 slots
    UINT8                          pwrs[4];                      //0x0000000A
} SPC_MBOX_NBURST_RESULT_T; //Size : 0xE
...
typedef struct
{
    ...
    SPC_MBOX_MON_RESULT_T          monRes[MBOX_INTRF_SLOT_QTY];  //0x0000000E
    UINT8                          bstSnr[4];                    //0x00000016
} SPC_MBOX_WIN_SPC_TO_PAL_RX_T; //Size : 0x1A
...
} SPC_MBOX_WIN_SPC_TO_PAL_T; //Size : 0x1A
...
typedef struct
{
    ...
    /// whether modulation is 8psk, bits by bits
    UINT8                          modis8psk;                    //0x0000007C
} SPC_MBOX_STATIC_SPC_TO_PAL_RX_T; //Size : 0x80
...
} SPC_MBOX_STATIC_SPC_TO_PAL_T; //Size : 0x80
...
typedef struct
{
    /// IR Vq: MSB 16 bits, IR Vr: LSB 16 bits
    UINT32                         vqvr[4];                      //0x00000000
    /// BSN1: MSB 16 bits, BSN0: LSB 16 bits
    UINT32                         bsn[4];                       //0x00000010
    /// CPS1: MSB 4 bits, CPS0: LSB 4 bits
    UINT8                          cps[4];                       //0x00000020
    /// Header TFI
    UINT8                          tfi[4];                       //0x00000024
    /// IR Combination Number
    UINT8                          combnum[4];                   //0x00000028
    /// IR Ctx and Buffer Init Flag
    UINT8                          init;                         //0x0000002C
    /// block snr
    UINT32                         block_snr[4];                 //0x00000030
} SPC_IR_DEBUG_T; //Size : 0x40
...
typedef struct
{
    ...
    SPC_MBOX_STATIC_SPC_TO_PAL_T   statWin;                      //0x00000050
    /// decoding structure for VOC
    SPC_MBOX_SPEECH_DEC_IN_T       speechDecIn;                  //0x000000D0
    /// array of pointer given by SPC to store the data to encode +1 for CSD block
    /// to encode
    UINT8*                         encBuff[MBOX_MAX_BUF_IDX_QTY+1]; //0x00000104
    /// used by SPC to warn PAL that DTX uplink has been used
    BOOL                           dtxUsed;                      //0x0000011C
    /// debug for BCPU
    UINT32                         debug[15];                    //0x00000120
    /// ir debug infomation structure
    SPC_IR_DEBUG_T                 irInfo;                       //0x0000015C
} SPC_MBOX_SPC_TO_PAL_T; //Size : 0x19C
...
typedef struct
{
    ...
    /// indicates slots DC value
    INT16                          RxDcoI;                       //0x00000004
    /// indicates slots DC value
    INT16                          RxDcoQ;                       //0x00000006
} SPC_MBOX_RX_NBURST_T; //Size : 0x8
...
typedef struct
{
    ...
    /// header tfi
    UINT8                          tfi;                          //0x00000003
} SPC_MBOX_STATWIN_RX_CFG_T; //Size : 0x4
...
typedef struct
{
    ...
    /// aburst ts
    UINT8                          ts;                           //0x00000004
} SPC_MBOX_WIN_TX_ACCESS_ENC_T; //Size : 0x6
...
typedef struct
{
    ...
    /// windows
    UINT8                          monWinIdx[MBOX_WINDOWS_QTY];  //0x00000008
    /// parameters of the programmed NB Rx window
    SPC_MBOX_RX_NBURST_T           nBurst;                       //0x0000000F
    /// indicates for each Rx window if I and Q have to be swapped
    BOOL                           swapIQ[MBOX_WINDOWS_QTY];     //0x00000013
} SPC_MBOX_WIN_PAL_TO_SPC_RX_T; //Size : 0x1A
...
} SPC_MBOX_WIN_PAL_TO_SPC_T; //Size : 0x24
...
typedef struct
{
    ...
    SPC_MBOX_STATWIN_TX_CTX_T      ctx[MBOX_MAX_BUF_IDX_QTY];    //0x00000006
} SPC_MBOX_STATIC_PAL_TO_SPC_TX_T; //Size : 0x16
...
} SPC_MBOX_STATIC_PAL_TO_SPC_RX_T; //Size : 0x14
...
typedef struct
{
    ...
    /// Structure used for blocks related Rx parameters, from PAL to SPC
    SPC_MBOX_STATIC_PAL_TO_SPC_RX_T rx;                          //0x00000016
    /// Structure concerning the current dedicated TCH
    SPC_MBOX_STATIC_PAL_TO_SPC_DEDICATED_T dedicated;            //0x0000002A
    /// mode for test loop see 3GPP 44.014 for more details
    UINT8                          loopMode;                     //0x0000003D
    /// mode of ciphering if enabled
    UINT8                          ciphMode;                     //0x0000003E
    /// gprs downlink alloc info
    UINT8                          gprsdlAlloc;                  //0x0000003F
} SPC_MBOX_STATIC_PAL_TO_SPC_T; //Size : 0x40
...
typedef struct
{
    /// rx On/Off Tco settings
    UINT32                         rxOnTcoSettings;              //0x00000000
    /// settings for TCO during DTX uplink
    UINT32                         dtxTcoSettings;               //0x00000004
    /// DigRf settings
    BOOL                           digrfEnable;                  //0x00000008
    UINT8                          digrfRxRate;                  //0x00000009
    UINT8                          digrfRxSampleWidth;           //0x0000000A
    UINT8                          digrfRxSampleAlign;           //0x0000000B
    UINT8                          digrfRxClkPol;                //0x0000000C
    UINT8                          digrfTxMode;                  //0x0000000D
    UINT8                          digrfTxClkPol;                //0x0000000E
    SPC_MBOX_TX_BURST_MAPPING_PARAMS_T txBstMap;                 //0x0000000F
    /// Enable Automatic DC Offset calibration mechanism
    BOOL                           dcoCalEnable;                 //0x00000012
} SPC_MBOX_RF_PARAMS_T; //Size : 0x14
...
typedef struct
{
    ...
    SPC_MBOX_STATIC_PAL_TO_SPC_T   statWin;                      //0x0000006C
    /// GSM TDMA counter
    SPC_MBOX_GSM_COUNTERS_T        counters[MBOX_CTX_SNAP_QTY];  //0x000000AC
    /// encoding structure comming from VOC
    SPC_MBOX_SPEECH_ENC_OUT_T      speechEncOut;                 //0x000000B8
    /// RF parameters
    SPC_MBOX_RF_PARAMS_T           rf;                           //0x000000E4
    /// indicates to SPC if the DTX uplink is allowed for the current TCH
    BOOL                           dtxAllowed;                   //0x000000F8
    /// FCCH Search Mode
    UINT8                          FchSearchMode;                //0x000000F9
    /// ir reset signal
    BOOL                           irReset;                      //0x000000FA
} SPC_MBOX_PAL_TO_SPC_T; //Size : 0xFC
...
typedef struct
{
    ...
    SPC_MBOX_SPC_TO_PAL_T          spc2pal;                      //0x000000FC
    UINT16                         dummyPchResult;               //0x00000298
    UINT16                         dummyPchThresh;               //0x0000029A
    /// #ifdef __MBOX_DEBUG__ This is used by the embedded debug stub
    SPC_MBOX_DEBUG_T               dbg[MBOX_CTX_SNAP_QTY];       //0x0000029C
    UINT32                         palDbgReq;                    //0x000002D8
    UINT32                         palDbgAck;                    //0x000002DC
} SPC_MAILBOX_T; //Size : 0x2E0
