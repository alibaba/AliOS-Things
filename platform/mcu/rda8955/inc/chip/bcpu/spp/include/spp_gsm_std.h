...
#ifndef _SPP_GSM_STD_H_
#define _SPP_GSM_STD_H_
...
#define SPP_EGPRS_IR_MEM_MAX_LEN                 (64*1024)
/// TBF modulo
...
#define MCS_1_TO_4_BUFFER_LEN                    (SPP_EGPRS_IR_MEM_MAX_LEN/MCS_1_TO_4_BLOCK_LEN)
/// number of compressed MCS5 to MCS6 blocks that can be stored in the IR memory
#define MCS_5_TO_6_BUFFER_LEN                    (SPP_EGPRS_IR_MEM_MAX_LEN/MCS_5_TO_6_BLOCK_LEN)
/// number of compressed MCS7 to MCS9 blocks that can be stored in the IR memory
#define MCS_7_TO_9_BUFFER_LEN                    (SPP_EGPRS_IR_MEM_MAX_LEN/MCS_7_TO_9_BLOCK_LEN)
...
/// number of taps used in the SOVA equalizer
#define SOVA_ALL_TAP_LEN                         (8)
/// size of the treillis for 8PSK equal to 8**(MAIN_TAP_LEN-1)
#define STATE_LEN                                (8)
/// length of the HASH TABLE (for IR)
#define HASH_LEN                                 (64)
/// size of IR buffer unit in bytes
#define SPP_EGPRS_IR_UNIT_SIZE                   (MCS_5_TO_6_BLOCK_LEN/2)
/// unit number in IR buffer
#define SPP_EGPRS_IR_BUF_MAX_SIZE                (SPP_EGPRS_IR_MEM_MAX_LEN/SPP_EGPRS_IR_UNIT_SIZE)
/// head error counting flag
#define SPP_EDGE_HEAD_ERROR_COUNT                (0X1)
/// data error counting flag, block0
#define SPP_EDGE_DATA_ERROR_COUNT0               (0X2)
/// data error counting flag, block0
#define SPP_EDGE_DATA_ERROR_COUNT1               (0X4)
...
typedef enum
{
    ...
    ITLV_TYPE_D1                                = 0x00000006,
    ITLV_TYPE_D2                                = 0x00000007,
    ITLV_TYPE_D3                                = 0x00000008,
    ITLV_TYPE_H1                                = 0x00000009,
    ITLV_TYPE_H2                                = 0x0000000A,
    ITLV_TYPE_H3                                = 0x0000000B,
    ITLV_TYPE_H4                                = 0x0000000C
} SPP_ITLV_TYPE_T;
...
typedef struct
{
    ...
    UINT8                          tfi;                          //0x00000009
    UINT8                          biterr;                       //0x0000000A
} SPP_HEADER_RESULT_T; //Size : 0xC
...
typedef struct
{
    VOLATILE SPP_COMPLEX_T         InArray[157];                 //0x00000000
    VOLATILE SPP_COMPLEX_T         RLS_W[8];                     //0x00000274
    VOLATILE SPP_COMPLEX_T         PrefiltMin[10+8];             //0x00000294
    VOLATILE SPP_COMPLEX_T         PrefiltMax[10+8];             //0x000002DC
    VOLATILE SPP_COMPLEX_T         LpChan[8];                    //0x00000324
    VOLATILE SPP_COMPLEX_T         BmmlZfMetric[6/2 * 8 * 8 + 1]; //0x00000344
    VOLATILE UINT8                 SoftOutputFwd[8*61*4];        //0x00000648
    VOLATILE UINT8                 SoftOutputBck[8*61*4];        //0x00000DE8
...
// ============================================================================
// SPP_EGPRS_DDFSE_BUFS_PART1_T
// -----------------------------------------------------------------------------
/// EGPRS DDFSE Buffers, Part 1
// =============================================================================
typedef struct
{
    VOLATILE UINT8                 SoftOutputFwd[8*61*4];        //0x00000000
    VOLATILE SPP_COMPLEX_T         RLS_W[8];                     //0x000007A0
    VOLATILE SPP_COMPLEX_T         PrefiltMin[10+8];             //0x000007C0
    VOLATILE SPP_COMPLEX_T         PrefiltMax[10+8];             //0x00000808
    VOLATILE SPP_COMPLEX_T         LpChan[8];                    //0x00000850
} SPP_EGPRS_DDFSE_BUFS_PART1_T; //Size : 0x870



// ============================================================================
// SPP_EGPRS_DDFSE_BUFS_PART2_T
// -----------------------------------------------------------------------------
/// EGPRS DDFSE Buffers, Part 2
// =============================================================================
typedef struct
{
    VOLATILE SPP_COMPLEX_T         InArray[157];                 //0x00000000
    VOLATILE SPP_COMPLEX_T         BmmlZfMetric[6/2 * 8 * 8 + 1]; //0x00000274
    VOLATILE UINT8                 SoftOutputBck[8*61*4];        //0x00000578
    UINT8                          TscSym[26];                   //0x00000D18
} SPP_EGPRS_DDFSE_BUFS_PART2_T; //Size : 0xD32
...
// ============================================================================
// SPP_EGPRS_DECODE_VITAC_PART1_T
// -----------------------------------------------------------------------------
/// Viterbi Decoding Buffers Structure, PART1.[NOTE: CombinedData: extra 1 words
/// used to leave extra space for excor when ir recombining]
// =============================================================================
typedef struct
{
    VOLATILE UINT32                CombinedData[459+1];          //0x00000000
    VOLATILE UINT32                PunctTable[58];               //0x00000730
    VOLATILE UINT32                DecodedData[20];              //0x00000818
} SPP_EGPRS_DECODE_VITAC_PART1_T; //Size : 0x868



// ============================================================================
// SPP_EGPRS_DECODE_VITAC_PART2_T
// -----------------------------------------------------------------------------
/// Viterbi Decoding Buffers Structure, PART2
// =============================================================================
typedef struct
{
    VOLATILE UINT32                SurvivorData[(64*612)/(16*2)]; //0x00000000
} SPP_EGPRS_DECODE_VITAC_PART2_T; //Size : 0x1320



// ============================================================================
// SPP_EGPRS_IRCOMB_TMPBUF_T
// -----------------------------------------------------------------------------
/// IR combination combination temporary buffers. tansferring buffers for hardware
/// combination
// =============================================================================
typedef struct
{
    UINT8                          buf0[1836];                   //0x00000000
    UINT8                          buf1[1836];                   //0x0000072C
    UINT8                          transbuf0[SPP_EGPRS_IR_UNIT_SIZE]; //0x00000E58
    UINT8                          transbuf1[SPP_EGPRS_IR_UNIT_SIZE]; //0x00000F90
} SPP_EGPRS_IRCOMB_TMPBUF_T; //Size : 0x10C8



// ============================================================================
// SPP_EGPRS_IRCOMB_TMPBUF2_T
// -----------------------------------------------------------------------------
/// IR combination combination temporary buffers. tansferring buffers for hardware
/// combination
// =============================================================================
typedef struct
{
    UINT8                          buf0[1836+64*4];              //0x00000000
    UINT8                          buf1[1836+64*4];              //0x0000082C
    UINT8                          transbuf0[SPP_EGPRS_IR_UNIT_SIZE]; //0x00001058
    UINT8                          transbuf1[SPP_EGPRS_IR_UNIT_SIZE]; //0x00001190
} SPP_EGPRS_IRCOMB_TMPBUF2_T; //Size : 0x12C8
...
// SPP_LOGPWR_FILTER_T
// -----------------------------------------------------------------------------
/// logpwr filter structure
// =============================================================================
typedef struct
{
    INT8                           logpwr_save;                  //0x00000000
    INT8                           out;                          //0x00000001
    UINT8                          cnt;                          //0x00000002
    UINT8                          sum;                          //0x00000003
} SPP_LOGPWR_FILTER_T; //Size : 0x4
