...
#define PRINT_CONFIG(x) cut_printf("\33[0;35m%s=%s\33[0m\r\n", #x, PARSE(x))
    if (strlen(SYSINFO_ARCH)==0 || strlen(SYSINFO_MCU) == 0 || strlen(SYSINFO_DEVICE_NAME)==0) {
        cut_printf("Please set your device info first!\r\n");
        return -1;
    }
    else {
...
static void task4(void *arg)
{
    ...
    cut_printf("task name %s: decrease\r\n", aos_task_name());
...
static void task5(void *arg)
{
    ...
    cut_printf("task name %s: increase\r\n", aos_task_name());
    for(i=0; i<TEST_CONFIG_SYNC_TIMES; i++) {
        aos_mutex_lock(&g_mutex, -1);
        g_var++;
        aos_mutex_unlock(&g_mutex);
...
//    ASSERT_NE(ret, 0);
//    cut_printf("aos_queue_recv 1 ret %d\n", ret);
...
CASE(test_timer, aos_1_020)
{
    ...
    aos_timer_stop(&g_timer);
    cut_printf("timer stopped!\r\n");
    aos_msleep(1000);
    aos_timer_change(&g_timer, 1000);
    cut_printf("timer changed!\r\n");
    aos_timer_start(&g_timer);
    cut_printf("timer started!\r\n");
...
{
    cut_printf("ev_callback1 is called\r\n");
    g_var = 0x5A;
    aos_loop_exit();
}
...
{
    g_var++;
    cut_printf("delayed_action1 called, %d\r\n", g_var);
...
{
    g_var++;
    cut_printf("delayed_action2 called, %d\r\n", g_var);
    aos_loop_exit();
}
...
void test_certificate(void)
{
    if (0 == dump_test_config()) {
        ...
        cut_main(0, NULL);
        cut_printf("test finished!\r\n");
    }
    else {
        cut_printf("test error!\r\n");
...
/*
void start_aos_test(void)
{
    aos_task_new("test", test_certificate, NULL, 1024);
}
*/
