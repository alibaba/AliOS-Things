...
extern const GPIO_MAPPING GPIO_MAPPING_TABLE[];
...
int32_t hal_gpio_enable_irq(gpio_dev_t *gpio, gpio_irq_trigger_t trigger,gpio_irq_handler_t handler, void *arg)
{
	...
	if((0 == get_mapTable_pos(gpio->port, &CurrentPosition)) && (IRQ_MODE == gpio->config)) 
	{
        ...
        switch (trigger)
        { 
            case IRQ_TRIGGER_RISING_EDGE  : GPIO_InitStruct.Mode = GPIO_MODE_IT_RISING; break;
            case IRQ_TRIGGER_FALLING_EDGE : GPIO_InitStruct.Mode = GPIO_MODE_IT_FALLING ;break;       
            case IRQ_TRIGGER_BOTH_EDGES   : GPIO_InitStruct.Mode = GPIO_MODE_IT_RISING_FALLING; break;
            default:
            return -1;
        }
...
int get_irqn_type(IRQn_Type *IRQn, uint16_t pin)
{
	...
	switch (pin)
	{
		...
		case GPIO_PIN_13:
		case GPIO_PIN_14:
		case GPIO_PIN_15:	*IRQn = EXTI15_10_IRQn;break;
	    default:
	    ret = -1;
	    break;
	}
...
void enable_gpio_clk(GPIO_TypeDef *GPIOx)
{
    if (GPIOx == GPIOA) {
        ...
        printf("__HAL_RCC_GPIOB_CLK_ENABLE\n");
    } else if (GPIOx == GPIOC) {
