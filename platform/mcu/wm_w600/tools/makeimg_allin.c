...
int main(int argc,char *argv[])
{
	...
	//fout = fopen("out.img", "wb+");
	fout = fopen(argv[3],"wb+");
	if(NULL == fout)
	{
		printf("\ncreate output image file error\n");
		fclose(fpbin);
		return -4;
	}
	...
	final_len = sizeof(T_BOOTER) + 48*1024 + appimg_len - sizeof(T_BOOTER);
	...
	fseek(fout, sizeof(T_BOOTER) + 48 * 1024, SEEK_SET);
	...
	fseek(fout, sizeof(T_BOOTER) + 32 * 1024, SEEK_SET);
	fseek(fpimg, 0, SEEK_SET);
