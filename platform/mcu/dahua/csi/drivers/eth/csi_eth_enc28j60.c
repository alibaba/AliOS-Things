...
static void enc28j60_int_handle(void *arg);
...
static void enc28j60_int_handle(void *arg)
{
    ...
    if (int_stat & EIR_RXERIF) {
        ...
        if ((ptkcnt == 0xFF) || (freespace < MAX_FRAMELEN)) {
            ...
            enc28j60ReadBuffer(6, status_vec);
            printf("rx status vector:0x%02x,0x%02x\n", status_vec[4], status_vec[5]);
        }
...
static void enc28j60hardreset(void)
{
    ...
    pin = csi_gpio_pin_initialize(PIN_ETH_RST, NULL, NULL);
...
static int enc28j60_set_interrupt(pin_t gpio_pin)
{
    ...
    pin_int = csi_gpio_pin_initialize(gpio_pin, enc28j60_int_handle, NULL);
...
eth_mac_handle_t csi_eth_mac_initialize(eth_event_cb_t cb)
{
    ...
    if (eth_mac_init == 0) {
        ...
        g_net_spi_hd = csi_spi_initialize(ETH_SPI_TX, ETH_SPI_RX, ETH_SPI_CLK, ETH_SPI_CS, (spi_event_cb_t)enc28j60_spi_transfer_callback, NULL);

        //spi_handle success goto config spi
        if (g_net_spi_hd != NULL) {

            ret = csi_spi_config(g_net_spi_hd, SYSTEM_CLOCK, SYSTEM_CLOCK / 10, SPI_MODE_MASTER, SPI_FORMAT_CPOL0_CPHA0,
                                 SPI_ORDER_MSB2LSB, SPI_SS_MASTER_SW, 8);

            pgpio_pin_handle1 = csi_gpio_pin_initialize(ETH_SPI_CS, NULL, NULL);
...
{
    return csi_eth_mac_get_rx_framesize(handle);
}
