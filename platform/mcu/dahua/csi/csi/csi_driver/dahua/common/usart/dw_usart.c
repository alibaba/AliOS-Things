...
static void dw_usart_intr_threshold_empty(dw_usart_priv_t *usart_priv)
{
    ...
    if (usart_priv->tx_cnt >= usart_priv->tx_total_num) {
        ...
        if (usart_priv->cb_event) {
            usart_priv->cb_event(usart_priv->cb_arg, USART_EVENT_SEND_COMPLETE);
        }
    }else{
...
/*static*/ void dw_usart_intr_recv_data(dw_usart_priv_t *usart_priv)
{
    if ((usart_priv->rx_total_num == 0) || (usart_priv->rx_buf == NULL)) {
        usart_priv->cb_event(usart_priv->cb_arg, USART_EVENT_RECEIVED);
        return;
    }
    ...
    if (usart_priv->rx_cnt >= usart_priv->rx_total_num) {
        ...
        if (usart_priv->cb_event) {
            usart_priv->cb_event(usart_priv->cb_arg, USART_EVENT_RECEIVE_COMPLETE);
        }
    }

}

int8_t dw_usart_getchar(void)
{
    dw_usart_reg_t *addr = (dw_usart_reg_t *)(0x14030000);
    
    return addr->RBR;
}

uint32_t dw_usart_getfifocount(void)
{
    dw_usart_reg_t *addr = (dw_usart_reg_t *)(0x14030000);
...
static void dw_usart_intr_recv_line(dw_usart_priv_t *usart_priv)
{
    ...
    if (lsr_stat & DW_LSR_BI) {
        if (usart_priv->cb_event) {
            usart_priv->cb_event(usart_priv->cb_arg, USART_EVENT_RX_BREAK);
        }
        return;
    }
    ...
    if (lsr_stat & DW_LSR_FE) {
        if (usart_priv->cb_event) {
            usart_priv->cb_event(usart_priv->cb_arg, USART_EVENT_RX_FRAMING_ERROR);
        }
        return;
    ...
    if (lsr_stat & DW_LSR_PE) {
        if (usart_priv->cb_event) {
            usart_priv->cb_event(usart_priv->cb_arg, USART_EVENT_RX_PARITY_ERROR);
        }
        return;
    ...
    if (lsr_stat & DW_LSR_OE) {
        if (usart_priv->cb_event) {
            usart_priv->cb_event(usart_priv->cb_arg, USART_EVENT_RX_OVERFLOW);
        }
        return;
...
ATTRIBUTE_ISR int USART0_IRQHandler(int arg, void *arg1)
{
...
int32_t __attribute__((weak)) target_usart_flowctrl_init(pin_t tx_flow, pin_t rx_flow, uint32_t flag)
{
    return -1;
}
...
usart_handle_t csi_usart_initialize(pin_t tx, pin_t rx, usart_event_cb_t cb_event, void *cb_arg)
{
    uint32_t base = 0u;
    uint32_t irq = 0u;
    int32_t idx = 0; //target_usart_init(tx, rx, &base, &irq);
    base = 0x14030000;
    irq = UART0_IRQn;
...
#endif
    request_irq(irq, USART0_IRQHandler, 0, 0, 0);
