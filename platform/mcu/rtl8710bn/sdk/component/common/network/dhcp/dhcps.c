...
static ip4_addr_t dhcps_send_broadcast_address;
static ip4_addr_t dhcps_local_address;
static ip4_addr_t dhcps_pool_start;
static ip4_addr_t dhcps_pool_end;
static ip4_addr_t dhcps_local_mask;
static ip4_addr_t dhcps_local_gateway;
static ip4_addr_t dhcps_network_id;
static ip4_addr_t dhcps_subnet_broadcast;
static ip4_addr_t dhcps_allocated_client_address;
static int dhcps_addr_pool_set = 0;
static ip4_addr_t dhcps_addr_pool_start;
static ip4_addr_t dhcps_addr_pool_end;
#if 1
static ip4_addr_t dhcps_owned_first_ip;
static ip4_addr_t dhcps_owned_last_ip;
...
static ip4_addr_t client_request_ip;
...
static void save_client_addr(ip4_addr_t *client_ip, uint8_t *hwaddr)
{
...
static uint8_t check_client_request_ip(ip4_addr_t *client_req_ip, uint8_t *hwaddr)
{
	int ip_addr4 = 0, i;
...
//	dump_client_table();
	...
	memcpy(&dhcps_local_address, ip_2_ip4(&(pnetif->ip_addr)),
							sizeof( ip4_addr_t));
	memcpy(&dhcps_local_mask, ip_2_ip4(&(pnetif->netmask)),
						sizeof(ip4_addr_t));

	memcpy(&dhcps_local_gateway, ip_2_ip4(&(pnetif->gw)),
						sizeof(ip4_addr_t));

	/* calculate the usable network ip range */
	dhcps_network_id.addr = ((ip_2_ip4(&(pnetif->ip_addr))->addr) &
					(ip_2_ip4(&(pnetif->netmask))->addr));
  ...
  dhcps_pcb->so_options|=SOF_BROADCAST;
	...
	{
		memcpy(&dhcps_pool_start,&dhcps_local_address,sizeof(ip4_addr_t));
		ip = (uint8_t *)&dhcps_pool_start;
		ip[3] = DHCP_POOL_START;
		memcpy(&dhcps_pool_end,&dhcps_local_address,sizeof(ip4_addr_t));
