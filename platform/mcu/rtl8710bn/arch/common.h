...
extern void mico_malloc_print_mallocs           ( void );
#else
#define mico_calloc_named( name, nelems, elemsize) calloc ( nelems, elemsize )
#define mico_calloc_named_hideleak( name, nelems, elemsize )  calloc ( nelems, elemsize )
#define mico_realloc_named( name, ptr, size )      realloc( ptr, size )
#define mico_malloc_named( name, size )            malloc ( size )
#define mico_malloc_named_hideleak( name, size )   malloc ( size )
#define mico_malloc_set_name( name )
#define mico_malloc_leak_set_ignored( global_flag )
#define mico_malloc_leak_set_base( global_flag )
#define mico_malloc_leak_check( thread, global_flag )
#define mico_malloc_transfer_to_curr_thread( block )
#define mico_malloc_transfer_to_thread( block, thread )
#define mico_malloc_print_mallocs( void )
#define mico_malloc_debug_startup_finished( )
...
static inline char nibble_to_hexchar(uint8_t nibble)
{
	...
	if (nibble > 9) {
		hexchar = (char)('A' + (nibble - 10));
    } else {
		hexchar = (char) ('0' + nibble);
    }
...
{
	uint8_t nibble_temp = 0;

	if ((hexchar >= '0') && (hexchar <= '9')) {
		nibble_temp = (uint8_t)( hexchar - '0' );
    } else if ((hexchar >= 'A') && (hexchar <= 'F')) {
		nibble_temp = (uint8_t) ( hexchar - 'A' + 10 );
    } else if ((hexchar >= 'a') && (hexchar <= 'f')) {
		nibble_temp = (uint8_t) ( hexchar - 'a' + 10 );
    } else {
		return -1;
