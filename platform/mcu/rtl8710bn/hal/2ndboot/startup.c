...
void ota_2ndboot_os_entry(void)
{
    ...
    offset = 0;
    hal_flash_read(HAL_PARTITION_APPLICATION, &offset, buf, 16);
    offset = *(uint32_t *)&buf[8] + 0x20;
	start_addr = offset;
    hal_flash_read(HAL_PARTITION_APPLICATION, &offset, buf, 0x30);
    ...
    offset = start_addr + 0x20;
    hal_flash_read(HAL_PARTITION_APPLICATION, &offset, 0x10005000, bd_len);
