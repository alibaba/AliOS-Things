...
#ifdef CONFIG_IPV6
void tcpip_adapter_v6stack_init(struct netif *netif)
{
...
static int tcpip_adapter_ipc_check(tcpip_adapter_api_msg_t *msg)
{
    ...
    espos_task_t local_task = espos_task_get_current();
...
esp_err_t tcpip_adapter_start(tcpip_adapter_if_t tcpip_if, uint8_t *mac, tcpip_adapter_ip_info_t *ip_info)
{
    ...
    if (esp_netif[tcpip_if] == NULL || !netif_is_up(esp_netif[tcpip_if])) {
        ...
        if (netifapi_netif_add(esp_netif[tcpip_if], &ip_info->ip, &ip_info->netmask, &ip_info->gw, NULL, netif_init, tcpip_input)){
            ESP_LOGE(TAG, "tcpip adatper add netif fail, type %d \n", tcpip_if);
            return ESP_FAIL;
        }
...
#if 0 /*for not support wifi ap mode at first*/
