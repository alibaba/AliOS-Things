...
static enum ota_link ota_update_link = OTA_LINK_ERR;
extern const hal_logic_partition_t hal_partitions[];
extern osMutexId FlashMutex;
extern osMutexDef_t os_mutex_def_flash;
static int32_t ota_rtosb_write_flag = 0xff;
...
int tg_flash_write(const hal_partition_t partition, const uint32_t addr, const uint8_t *src, const uint32_t size)
{
    int ret = 0;
    uint32_t lock;
    uint32_t num = 0;
...
ODM_TYPE tg_ota_get_odm_type(void)
{
    int32_t ret = 0;
    const struct ota_boot_info boot_info;
...
int tg_ota_set_odm_type(ODM_TYPE type)
{
    int32_t ret = 0;
...
int tg_ota_copy_partitionA_to_partitionB(hal_partition_t dst_partition, hal_partition_t src_partition)
{
	uint8_t *buf = NULL;
    ...
    read_buf = (uint8_t *)malloc(FLASH_BLOCK_SIZE_IN_BYTES);
    if (!read_buf) {
        TRACE("%s %d, malloc fail", __func__, __LINE__);
        return OTA_FAILE;
    }
	...
	for (num = 0; num < partitionA_info.partition_length /FLASH_BLOCK_SIZE_IN_BYTES; num ++) {
		tg_flash_read(addrA, buf, FLASH_BLOCK_SIZE_IN_BYTES);

		ret = tg_flash_write(partitionB_index, addrB, buf, FLASH_BLOCK_SIZE_IN_BYTES);
		if (ret) {
			TRACE("error %s %d, return %d", __func__, __LINE__, ret);
			ret = OTA_FAILE;
			goto end;
		...
		tg_flash_read(addrB, read_buf, FLASH_BLOCK_SIZE_IN_BYTES);
		if (memcmp(read_buf, buf, FLASH_BLOCK_SIZE_IN_BYTES)) {
			TRACE("error %s %d, tg_flash_write fail", __func__, __LINE__);
			ret = OTA_FAILE;
			goto end;
		}
...
int ota_hal_boot(ota_boot_param_t *param)
{
    ...
    if(param != NULL) {
        ...
        param->boot_type = 0x01;//dual partiton boot
        printf("OTA after finish dst:0x%08x src:0x%08x len:0x%08x,crc:0x%04x param crc:0x%04x upg_flag:0x%04x \r\n",
               param->dst_adr, param->src_adr, param->len, param->crc,
               param->param_crc, param->upg_flag);
        ret = ota_update_parameter(param);
        if(ret < 0) {
            return OTA_UPGRADE_WRITE_FAIL;
        }
        else {
            if(param->upg_flag == 0x9669) {
               ret = ota_upgrade_link();
            }
        }
