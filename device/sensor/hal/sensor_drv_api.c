...
int32_t sensor_i2c_single_write(i2c_dev_t *i2c, uint8_t *data, uint16_t size,
                                uint32_t timeout)
{
    int32_t ret = 0;
    if (i2c == NULL) {
    ...
    ret = hal_i2c_master_send(i2c, i2c->config.dev_addr, data, size,
                              AOS_WAIT_FOREVER);

    return ret;
}

int32_t sensor_i2c_single_read(i2c_dev_t *i2c, uint8_t *data, uint16_t size,
                               uint32_t timeout)
{
    int32_t ret = 0;
    if (i2c == NULL) {
    ...
    ret = hal_i2c_master_recv(i2c, i2c->config.dev_addr, data, size,
                              AOS_WAIT_FOREVER);
    return ret;
}
