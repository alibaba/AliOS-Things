...
static int drv_acc_bosch_bma456_ioctl(int cmd, unsigned long arg)
{
    ...
    switch(cmd) {
        case SENSOR_IOCTL_ODR_SET: {
            ret = drv_acc_bosch_bma4xy_set_odr(&bma456_ctx, arg);
            if(unlikely(ret)) {
                return -1;
            }
        }
        ...
        case SENSOR_IOCTL_RANGE_SET: {
            ret = drv_acc_bosch_bma4xy_set_range(&bma456_ctx, arg);
            if(unlikely(ret)) {
                return -1;
            }
        }
        ...
        case SENSOR_IOCTL_SET_POWER: {
            ret = drv_acc_bosch_bma4xy_set_power_mode(&bma456_ctx, arg);
            if(unlikely(ret)) {
                return -1;
            }
        }
...
int drv_acc_bosch_bma456_init(void) {
    ...
    ret = drv_acc_bosch_bma4xy_validate_id(&bma456_ctx);
    if(unlikely(ret)) {
        printf("%s %s  drv_acc_bosch_bma4xy_validate_id \n", SENSOR_STR, __func__);
        return -1;
    }

    ret = drv_acc_bosch_bma4xy_soft_reset(&bma456_ctx);
    if(unlikely(ret)) {
        printf("%s %s  drv_acc_bosch_bma4xy_soft_reset \n", SENSOR_STR, __func__);
        return -1;
    }

    ret = drv_acc_bosch_bma4xy_set_range(&bma456_ctx, ACC_RANGE_8G);
    if(unlikely(ret)) {
    ...
    ret = drv_acc_bosch_bma4xy_set_odr(&bma456_ctx, BMA456_DEFAULT_ODR_100HZ);
    if(unlikely(ret)) {
        printf("%s %s  drv_acc_bosch_bma4xy_set_odr \n", SENSOR_STR, __func__);
        return -1;
    }
    ret = drv_acc_bosch_bma4xy_set_power_mode(&bma456_ctx, DEV_SLEEP);
    if(unlikely(ret)) {
