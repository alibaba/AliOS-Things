...
extern mb_handler_t* modbus_handler;
...
int modbus_sensor_read(void *buffer, size_t length, int index)
{
    ...
    //LOG("mb_read_holding_reginster  slave=,addr=\n");
    //LOG(" mb_read_holding_reginster  slave=%d,addr=%d\n", sns->slave,sns->addr);
    status = mb_read_holding_reginster(modbus_handler,sns->slave, sns->addr, sns->reg_cnt,
                                       data, &respond_cnt);
