...
static int drv_mag_st_lis3mdl_validate_id(i2c_dev_t* drv, uint8_t id_value)
{
    ...
    ret = sensor_i2c_read(drv, LIS3MDL_MAG_WHO_AM_I_REG, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
...
static int drv_mag_st_lis3mdl_set_power_mode(i2c_dev_t* drv, dev_power_mode_e mode)
{
    uint8_t value = 0x00;
    int ret = 0;
    
    ret = sensor_i2c_read(drv, LIS3MDL_MAG_CTRL_REG3, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
    ...
    ret = sensor_i2c_write(drv, LIS3MDL_MAG_CTRL_REG3, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
...
static int drv_mag_st_lis3mdl_set_odr(i2c_dev_t* drv, uint8_t odr)
{
    ...
    ret = sensor_i2c_read(drv, LIS3MDL_MAG_CTRL_REG1, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
...
static int drv_mag_st_lis3mdl_set_zaxes(i2c_dev_t* drv, uint8_t data_rate)
{
    ...
    ret = sensor_i2c_read(drv, LIS3MDL_MAG_CTRL_REG4, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
    switch (data_rate){
    ...
    ret = sensor_i2c_write(drv, LIS3MDL_MAG_CTRL_REG4, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
    return 0;
...
static int drv_mag_st_lis3mdl_lowpower_mode(i2c_dev_t* drv, uint8_t lowpower_mode)
{
    int ret = 0;
    uint8_t value = 0x00;
...
static int drv_mag_st_lis3mdl_set_range(i2c_dev_t* drv, uint32_t range)
{
    ...
    ret = sensor_i2c_read(drv, LIS3MDL_MAG_CTRL_REG2, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
...
static int drv_mag_st_lis3mdl_set_bdu(i2c_dev_t* drv, uint8_t bdu)
{
    ...
    ret = sensor_i2c_read(drv, LIS3MDL_MAG_CTRL_REG5, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
    ...
    ret = sensor_i2c_write(drv, LIS3MDL_MAG_CTRL_REG5, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
    return 0;
...
static int drv_mag_st_lis3mdl_read(void* buf, size_t len)
{
  ...
  ret = sensor_i2c_read(&LIS3MDL_ctx, LIS3MDL_MAG_CTRL_REG2, &ctrlm, I2C_DATA_LEN, I2C_OP_RETRIES);
  
  ret = sensor_i2c_read(&LIS3MDL_ctx, (LIS3MDL_MAG_OUTX_L | 0x80), buffer, 6, I2C_OP_RETRIES);
