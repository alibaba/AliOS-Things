...
static void drv_acc_senodia_sh200q_dump_reg(void)
{
  ...
  for(i=0; i<n;i++) {
    ...
    SH200Q_LOG("Reg 0x%x = 0x%x ", reg_map[i], rw_buffer[i]);
  }
...
static int drv_acc_gyro_senodia_sh200q_validate_id(i2c_dev_t* drv, uint8_t id_value)
{
	...
	do{
		...
		SH200Q_LOG("senodia sh200q id = %d.", value);	
...
static int drv_acc_senodia_sh200q_set_power_mode(i2c_dev_t* drv, dev_power_mode_e mode)
{
    ...
    switch(mode){
        case DEV_POWER_ON:{
			...
			SH200Q_LOG("DEV_POWER_ON value =%x", value);
			g_sleep = 0;
			g_powerdown = 0;
        }break;
...
static int drv_acc_senodia_sh200q_set_odr(i2c_dev_t* drv, uint32_t hz)
{
	...
	SH200Q_LOG("drv_acc_senodia_sh200q_set_odr");
...
static int drv_acc_senodia_sh200q_set_range(i2c_dev_t* drv, uint32_t range)
{
	...
	SH200Q_LOG("drv_acc_senodia_sh200q_set_range  range = %d", range);
...
static int drv_acc_senodia_sh200q_read(void *buf, size_t len)
{
    ...
    ret  = sensor_i2c_read(&sh200q_ctx, SH200Q_OUTPUT_ACC_X,  &reg[0], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&sh200q_ctx, SH200Q_OUTPUT_ACC_X + 1,  &reg[1], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&sh200q_ctx, SH200Q_OUTPUT_ACC_X + 2,  &reg[2], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&sh200q_ctx, SH200Q_OUTPUT_ACC_X + 3,  &reg[3], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&sh200q_ctx, SH200Q_OUTPUT_ACC_X + 4,  &reg[4], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&sh200q_ctx, SH200Q_OUTPUT_ACC_X + 5,  &reg[5], I2C_REG_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return -1;
    }
...
static int drv_acc_senodia_sh200q_ioctl(int cmd, unsigned long arg)
{
	...
	SH200Q_LOG("drv_acc_senodia_sh200q_ioctl cmd = %d, arg = %d", cmd, arg);
    switch(cmd){
...
int drv_acc_senodia_sh200q_init(void){
    ...
    if(0 == g_sh200qflag){
	    ...
	    SH200Q_LOG("Senodia drv_acc_senodia_sh200q_init begin \n");
	#if 0
	...
	#endif
		SH200Q_LOG("Senodia drv_acc_senodia_sh200q_init end \n");
        g_sh200qflag = 1;
    }
    else{
        SH200Q_LOG("%s %s acc do not need reset\n", SENSOR_STR, __func__);
    }
    

    SH200Q_LOG("%s %s Senodia successfully \n", SENSOR_STR, __func__);
    return 0;
}
...
static int drv_gyro_senodia_sh200q_read(void *buf, size_t len)
{
    ...
    ret  = sensor_i2c_read(&sh200q_ctx, SH200Q_OUTPUT_GYRO_X,  	   &reg[0], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&sh200q_ctx, SH200Q_OUTPUT_GYRO_X + 1,  &reg[1], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&sh200q_ctx, SH200Q_OUTPUT_GYRO_X + 2,  &reg[2], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&sh200q_ctx, SH200Q_OUTPUT_GYRO_X + 3,  &reg[3], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&sh200q_ctx, SH200Q_OUTPUT_GYRO_X + 4,  &reg[4], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&sh200q_ctx, SH200Q_OUTPUT_GYRO_X + 5,  &reg[5], I2C_REG_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return -1;
    }
...
static int drv_acc_gyro_senodia_sh200q_self_test(i2c_dev_t* drv, int32_t* diff)
{
    ...
    SH200Q_LOG("c_gyro_self[0] = %d, c_gyro_self[1] = %d, c_gyro_self[2] = %d",  c_gyro_self[0], c_gyro_self[1], c_gyro_self[2]);
	...
	for(i=30; i>0; i--){
		...
		ret = sensor_i2c_read(drv, SH200Q_OUTPUT_GYRO_X,  &temp, 6, I2C_OP_RETRIES);
		if(unlikely(ret)){
			return -1;
		}
    ...
    SH200Q_LOG("avgGyroData[0] = %d, avgGyroData[1] = %d, avgGyroData[2] = %d",  avgGyroData[0], avgGyroData[1], avgGyroData[2]);
    ...
    SH200Q_LOG("avgGyroData1[0] = %d, avgGyroData1[1] = %d, avgGyroData1[2] = %d , ret = %d",  avgGyroData1[0], avgGyroData1[1], avgGyroData1[2], ret);


	for(i = 0; i < 3; i++){
		diff[i] = avgGyroData[i] - avgGyroData1[i];
		SH200Q_LOG("diff[%d] = %d, avgGyroData[%d] = %d,   avgGyroData1[%d] = %d",  i, diff[i], i , avgGyroData[i], i,  avgGyroData1[i]);
...
int drv_gyro_senodia_sh200q_init(void){
    ...
    if(0 == g_sh200qflag){
        ...
        SH200Q_LOG("%s %s gyro do not need reset\n", SENSOR_STR, __func__);
    }

    /* update the phy sensor info to sensor hal */
    SH200Q_LOG("%s %s successfully \n", SENSOR_STR, __func__);
    return 0;
}
