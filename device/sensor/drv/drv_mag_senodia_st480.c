...
static int drv_mag_sen_st480_set_power_mode(i2c_dev_t *      drv,
...
static int drv_mag_sen_st480_read(void *buf, size_t len)
{
    ...
    if (!((buffer[0] >> 4) & 0X01)) {
        ...
        for (i = 0; i < 3; i++) {
            pnRawData[i] = (int16_t)((((uint16_t)buffer[2 * i + 3]) << 8) |
                                     ((uint16_t)buffer[2 * i + 4]));
        }
    }


    pdata->data[0] =
      (int32_t)(pnRawData[0] * ST480_MAG_XY_SENSITIVITY_FOR_48GA / 10);
    pdata->data[1] =
      (int32_t)(pnRawData[1] * ST480_MAG_XY_SENSITIVITY_FOR_48GA / 10);
    pdata->data[2] =
      (int32_t)(pnRawData[2] * ST480_MAG_Z_SENSITIVITY_FOR_48GA / 10);
