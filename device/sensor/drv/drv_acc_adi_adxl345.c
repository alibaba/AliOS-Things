...
static int drv_acc_adi_adxl345_validate_id(i2c_dev_t* drv, uint8_t id_value)
{
    ...
    ret = sensor_i2c_read(drv, ADXL345_DEVID, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return -1;
    }
...
static int drv_acc_adi_adxl345_set_power_mode(i2c_dev_t* drv, dev_power_mode_e mode)
{
    uint8_t value;
    int ret = 0;
    
    ret = sensor_i2c_read(drv, ADXL345_POWER_CTL, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
    ...
    switch(mode){
        case DEV_POWER_ON:{
            ...
            ret = sensor_i2c_write(drv, ADXL345_POWER_CTL, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
            if(unlikely(ret)){
                return ret;
            }
        }break;
...
static int drv_acc_adi_adxl345_set_odr(i2c_dev_t* drv, float odr)
{
    ...
    ret = sensor_i2c_read(drv, ADXL345_BW_RATE, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
    ...
    ret = sensor_i2c_write(drv, ADXL345_BW_RATE, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
    return 0;    
...
static int drv_acc_adi_adxl345_set_range(i2c_dev_t* drv, uint32_t range)
{
    ...
    ret = sensor_i2c_read(drv, ADXL345_DATA_FORMAT, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
    ...
    ret = sensor_i2c_write(drv, ADXL345_DATA_FORMAT, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
...
static int drv_acc_adi_adxl345_read(void *buf, size_t len)
{
    ...
    ret  = sensor_i2c_read(&adxl345_ctx, ADXL345_DATAX0,  &LocalBuf[0], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&adxl345_ctx, ADXL345_DATAX1,  &LocalBuf[1], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&adxl345_ctx, ADXL345_DATAY0,  &LocalBuf[2], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&adxl345_ctx, ADXL345_DATAY1,  &LocalBuf[3], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&adxl345_ctx, ADXL345_DATAZ0,  &LocalBuf[4], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&adxl345_ctx, ADXL345_DATAZ1,  &LocalBuf[5], I2C_REG_LEN, I2C_OP_RETRIES);
...
int drv_acc_adi_adxl345_init(void){
    ...
    ret = drv_acc_adi_adxl345_validate_id(&adxl345_ctx, ADXL345_DEVICE_ID_VALUE);
    if(unlikely(ret)){
        return -1;
    }
