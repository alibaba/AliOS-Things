...
typedef enum {
  ADT7410_OPMODE_CONTINUOUS       =  (uint8_t)0x00,   /*!< Operation Mode continuously */
  ADT7410_OPMODE_ONESHOT          =  (uint8_t)0x01,   /*!< Operation Mode one shot */
  ADT7410_OPMODE_FASTONESHOT      =  (uint8_t)0x02,   /*!< Operation Mode fast one shot */
...
static int drv_temp_adi_adt7410_validate_id(i2c_dev_t* drv, uint8_t id_value)
{
    ...
    ret = sensor_i2c_read(drv, ADT7410_WHO_AM_I_REG, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
...
static int drv_temp_adi_adt7410_set_power_mode(i2c_dev_t* drv, dev_power_mode_e mode)
{
    uint8_t value = 0x00;
    int ret = 0;
    
    ret = sensor_i2c_read(drv, ADT7410_CONFIG, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
    ...
    switch(mode){
        case DEV_POWER_ON:{
            ...
            ret = sensor_i2c_write(drv, ADT7410_CONFIG, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
            if(unlikely(ret)){
                return ret;
            }
        }break;
...
static int drv_temp_adi_adt7410_set_od_res(i2c_dev_t* drv, adt7410_od_resolution odr)
{
    uint8_t value = 0x00;
    int ret = 0;
...
static int drv_temp_adi_adt7410_read(void *buf, size_t len)
{
    ...
    ret  = sensor_i2c_read(&adt7410_ctx, ADT7410_TEMPREG_MSB, buffer, 2, I2C_OP_RETRIES);
