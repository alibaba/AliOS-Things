...
/*! @name Self test macro to show resulting minimum difference signal in mg */
...
/******************************************************************************/
/*! @name Gyroscope sense drive and low power mode */
...
static int32_t bmi260_acc_factor[ACC_RANGE_MAX] = { 16384, 8192, 4096, 2048 };
static int32_t bmi260_gyro_factor[GYRO_RANGE_MAX] = {262, 131, 65, 33, 16 };
...
static int8_t cfg_error_status(i2c_dev_t* drv)
{
    ...
    ret = sensor_i2c_read(drv, BMI2_EVENT_ADDR, &reg_data, I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
...
static int drv_acc_gyro_bosch_bmi260_soft_reset(i2c_dev_t* drv)
{
    ...
    ret = sensor_i2c_write(drv, BMI2_CMD_REG_ADDR, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    aos_msleep(2);
    if (unlikely(ret)) {
        return -1;
    }
...
static int drv_acc_gyro_bosch_bmi260_validate_id(i2c_dev_t* drv, uint8_t id_value)
{
    ...
    ret = sensor_i2c_read(drv, BMI2_CHIP_ID_ADDR, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        printf("read CHIPID failed \n");
        return ret;
    }
...
static int drv_acc_gyro_bosch_bmi260_enable(i2c_dev_t* drv)
{
    ...
    ret = sensor_i2c_read(drv, BMI2_PWR_CTRL_ADDR, &reg_data, I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
    ...
    ret = sensor_i2c_write(drv, BMI2_PWR_CTRL_ADDR, &reg_data, I2C_DATA_LEN, I2C_OP_RETRIES);
    aos_msleep(3);
    if (unlikely(ret)) {
    ...
    ret = sensor_i2c_read(drv, BMI2_PWR_CONF_ADDR, &reg_data, I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
	...
	/* Disable advance power save if enabled */
        ...
        ret = sensor_i2c_write(drv, BMI2_PWR_CONF_ADDR, &reg_data, I2C_DATA_LEN, I2C_OP_RETRIES);
        aos_msleep(3);
        if (unlikely(ret)) {
	...
	/* get ACC_CONFIG */
    ret = sensor_i2c_read(drv, BMI2_ACC_CONF_ADDR, &reg_data, I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
    ...
    ret = sensor_i2c_write(drv, BMI2_ACC_CONF_ADDR, &reg_data, I2C_DATA_LEN, I2C_OP_RETRIES);
    aos_msleep(3);
    if (unlikely(ret)) {
    ...
    /* get GYRO_CONFIG */
    ret = sensor_i2c_read(drv, BMI2_GYR_CONF_ADDR, &reg_data, I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
    ...
    ret = sensor_i2c_write(drv, BMI2_GYR_CONF_ADDR, &reg_data, I2C_DATA_LEN, I2C_OP_RETRIES);
    aos_msleep(3);
    if (unlikely(ret)) {
 ...
 * @return  the operation status, 0 is OK, others is error
 */
static int drv_acc_bosch_bmi260_set_power_mode(i2c_dev_t* drv, dev_power_mode_e mode)
{
    int ret = 0;
 ...
 * @return  the operation status, 0 is OK, others is error
 */
static uint8_t drv_acc_bosch_bmi260_getXYZ(i2c_dev_t* drv, int16_t* x, int16_t* y, int16_t* z)
{
    int ret = 0;
	...
	uint8_t reg_data[BMI2_ACC_GYR_NUM_BYTES] = {0}; /* Array to define data stored in register */
      ...
      ret = sensor_i2c_read(drv, BMI2_ACC_X_LSB_ADDR, &reg_data, BMI2_ACC_GYR_NUM_BYTES, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        printf("read BMI2_GYR_CONF_ADDR failed \n");
        return ret;
    }
...
static int drv_acc_bosch_bmi260_set_range(i2c_dev_t* drv, uint32_t range)
{
	...
	/* get ACC_CONFIG1 */
    ret = sensor_i2c_read(drv, BMI2_ACC_CONF1_ADDR, &reg_data, I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
    ...
    ret = sensor_i2c_write(drv, BMI2_ACC_CONF1_ADDR, &reg_data, I2C_DATA_LEN, I2C_OP_RETRIES);
    aos_msleep(3);
    if (unlikely(ret)) {
         printf("write BMI2_ACC_CONF_ADDR failed \n");
         return ret;
    ...
    if ((range >= ACC_RANGE_2G)&&(range <= ACC_RANGE_16G)) {
        cur_acc_factor = bmi260_acc_factor[range];
    }
...
static int drv_acc_bosch_bmi260_read(void *buf, size_t len)
{
	...
	/* Get status of advance power save mode */
    ret = sensor_i2c_read(&bmi260_ctx, BMI2_PWR_CONF_ADDR, &reg_data, I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        printf("read PWR_CONF_ADDR failed \n");
        return ret;
    }
	...
	/* Disable advance power save if enabled */
        reg_data = BMI2_SET_BIT_POS0(reg_data, BMI2_ADV_POW_EN, BMI2_DISABLE);
        ret = sensor_i2c_write(&bmi260_ctx, BMI2_PWR_CONF_ADDR, &reg_data, I2C_DATA_LEN, I2C_OP_RETRIES);
        aos_msleep(3);
        if (unlikely(ret)) {
         printf("write PWR_CONF_ADDR failed \n");
         return ret;
    ...
    if (cur_acc_factor != 0) {
        ...
        accel->data[DATA_AXIS_X] = x * ACCELEROMETER_UNIT_FACTOR / (int32_t)cur_acc_factor;
        accel->data[DATA_AXIS_Y] = y * ACCELEROMETER_UNIT_FACTOR / (int32_t)cur_acc_factor;
        accel->data[DATA_AXIS_Z] = z * ACCELEROMETER_UNIT_FACTOR / (int32_t)cur_acc_factor;
    } 
 ...
 * @return  the operation status, 0 is OK, others is error
 */
static int drv_gyro_bosch_bmi260_set_power_mode(i2c_dev_t* drv, dev_power_mode_e mode)
{
 ...
 * @return  the operation status, 0 is OK, others is error
 */
static uint8_t drv_gyro_bosch_bmi260_getXYZ(i2c_dev_t* drv, int16_t* x, int16_t* y, int16_t* z)
{
    int ret = 0;
	uint8_t msb;                                    /* Variables to store msb value */
	uint8_t lsb;                                    /* Variables to store lsb value */
    ...
    ret = sensor_i2c_read(drv, BMI2_GYR_X_LSB_ADDR, &reg_data, BMI2_ACC_GYR_NUM_BYTES, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        printf("read BMI2_GYR_CONF_ADDR failed \n");
        return ret;
    }
...
static int drv_gyro_bosch_bmi260_set_range(i2c_dev_t* drv, uint32_t range)
{
    ...
    /* get GYRO_CONFIG1 */
    ret = sensor_i2c_read(drv, BMI2_GYR_CONF1_ADDR, &reg_data, I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
    ...
    ret = sensor_i2c_write(drv, BMI2_GYR_CONF1_ADDR, &reg_data, I2C_DATA_LEN, I2C_OP_RETRIES);
    aos_msleep(3);
    if (unlikely(ret)) {
    ...
    if ((range >= GYRO_RANGE_250DPS)&&(range <= GYRO_RANGE_2000DPS)) {
        cur_gyro_factor = bmi260_gyro_factor[range];
    }
...
static int drv_gyro_bosch_bmi260_read(void *buf, size_t len)
{
   ...
   /* Get status of advance power save mode */
    ...
    if (cur_gyro_factor != 0) {
        gyro->data[DATA_AXIS_X] = x * GYROSCOPE_UNIT_FACTOR / cur_gyro_factor;
        gyro->data[DATA_AXIS_Y] = y * GYROSCOPE_UNIT_FACTOR / cur_gyro_factor;
        gyro->data[DATA_AXIS_Z] = z * GYROSCOPE_UNIT_FACTOR / cur_gyro_factor;
        printf("cur_gyro_factor = %d \n", cur_gyro_factor);
        printf("x = %d, y = %d, z = %d \n", x, y, z);
    } else 
       printf("cur_gyro_factor == 0 \n");
