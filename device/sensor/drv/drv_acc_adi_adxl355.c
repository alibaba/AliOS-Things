...
static int drv_acc_adi_adxl355_soft_reset(i2c_dev_t* drv)
{
    ...
    ret = sensor_i2c_write(drv, ADI_ADXL355_SRESET, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
...
static int drv_acc_adi_adxl355_validate_id(i2c_dev_t* drv, uint8_t id_value)
{
    ...
    ret = sensor_i2c_read(drv, ADI_ADXL355_DEVID, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return -1;
    }
...
static int drv_acc_adi_adxl355_set_power_mode(i2c_dev_t* drv, dev_power_mode_e mode)
{
    uint8_t value;
    int ret = 0;
    
    ret = sensor_i2c_read(drv, ADI_ADXL355_POWER_CTL, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
    ...
    switch(mode){
        case DEV_POWER_ON:{
            value &= 0xFE;
            ret = sensor_i2c_write(drv, ADI_ADXL355_POWER_CTL, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
            if(unlikely(ret)){
                return ret;
            }
        }break;
...
static int drv_acc_adi_adxl355_set_odr(i2c_dev_t* drv, uint32_t odr)
{
    ...
    ret = sensor_i2c_read(drv, ADI_ADXL355_FILTER, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
    ...
    ret = sensor_i2c_write(drv, ADI_ADXL355_FILTER, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
    return 0;    
...
static int drv_acc_adi_adxl355_set_range(i2c_dev_t* drv, uint32_t range)
{
    ...
    ret = sensor_i2c_read(drv, ADI_ADXL355_RANGE, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
    ...
    ret = sensor_i2c_write(drv, ADI_ADXL355_RANGE, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
...
static int drv_acc_adi_adxl355_read(void *buf, size_t len)
{
    ...
    ret  = sensor_i2c_read(&adxl355_ctx, ADI_ADXL355_XDATA3,  &LocalBuf[0], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&adxl355_ctx, ADI_ADXL355_XDATA2,  &LocalBuf[1], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&adxl355_ctx, ADI_ADXL355_XDATA1,  &LocalBuf[2], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&adxl355_ctx, ADI_ADXL355_YDATA3,  &LocalBuf[3], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&adxl355_ctx, ADI_ADXL355_YDATA2,  &LocalBuf[4], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&adxl355_ctx, ADI_ADXL355_YDATA1,  &LocalBuf[5], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&adxl355_ctx, ADI_ADXL355_ZDATA3,  &LocalBuf[6], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&adxl355_ctx, ADI_ADXL355_ZDATA2,  &LocalBuf[7], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&adxl355_ctx, ADI_ADXL355_ZDATA1,  &LocalBuf[8], I2C_REG_LEN, I2C_OP_RETRIES);
    
    if(unlikely(ret)){
        return -1;
    }
    
    x_val = (int32_t) (((LocalBuf[0]<<16) | (LocalBuf[1]<<8) | ((LocalBuf[2]<<0)&0xF0)) >> 4);                       
	SignBit = (x_val & (1 << 19)) != 0;
	if (SignBit)
		x_val = x_val | ~((1 << 20) - 1);
	   
	y_val = (int32_t) (((LocalBuf[3]<<16) | (LocalBuf[4]<<8) | ((LocalBuf[5]<<0)&0xF0)) >> 4);                        
	SignBit = (y_val & (1 << 19)) != 0;
	if (SignBit)
		y_val = y_val | ~((1 << 20) - 1);
	   
	z_val = (int32_t) (((LocalBuf[6]<<16) | (LocalBuf[7]<<8) | ((LocalBuf[8]<<0)&0xF0)) >> 4);                        
...
int drv_acc_adi_adxl355_init(void){
    ...
    ret = drv_acc_adi_adxl355_validate_id(&adxl355_ctx, ADXL355_DEVICE_ID_VALUE);
    if(unlikely(ret)){
        return -1;
    }
