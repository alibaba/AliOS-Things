...
static int drv_als_ams_tcs3400_validate_id(i2c_dev_t *drv, uint8_t id_value)
{
    ...
    ret = sensor_i2c_read(drv,TCS3400_CHIPID, &chipid_value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)) {
...
static int drv_als_ams_tcs3400_set_power_mode(i2c_dev_t * drv, dev_power_mode_e mode)
{
    int ret = 0;
    uint8_t temp_zero=0;
    switch(mode){
        case DEV_POWER_ON:{
             ...
             {
                ret = sensor_i2c_write(drv,TCS3400_CMD_ALL_INT_CLR,&temp_zero,I2C_DATA_LEN, I2C_OP_RETRIES);
                if(unlikely(ret))
                {
                    return ret;
                }
                tcs3400_chip.shadow[TCS3400_ENABLE] |= (TCS3400_EN_PWR_ON | TCS3400_EN_ALS | TCS3400_EN_ALS_IRQ);
                ret = sensor_i2c_write(drv,TCS3400_ENABLE,&(tcs3400_chip.shadow[TCS3400_ENABLE]),I2C_DATA_LEN, I2C_OP_RETRIES);
                if(unlikely(ret))
                {
                ...
                sensor_i2c_write(drv,TCS3400_REG_AUX,&(tcs3400_chip.shadow[TCS3400_REG_AUX]),I2C_DATA_LEN, I2C_OP_RETRIES);
...
static int tcs3400_reset_regs(i2c_dev_t *drv)
{
    ...
    sensor_i2c_write(drv,TCS3400_ENABLE,&temp_zero,I2C_DATA_LEN,I2C_OP_RETRIES);
    for(i=0; i<ARRAY_SIZE(regs);i++)
    {
        reg=regs[i];
        ret = sensor_i2c_write(drv,reg,&(tcs3400_chip.shadow[reg]),I2C_DATA_LEN, I2C_OP_RETRIES);
        if(unlikely(ret))
        {
...
static int tcs3400_read_als_data()
{
     ...
     uint8_t *buf;
     int16_t ir_data;
     ret = sensor_i2c_read(&tcs3400_ctx,TCS3400_CLR_CHANLO,&tcs3400_chip.shadow[TCS3400_CLR_CHANLO],8,I2C_OP_RETRIES);
     if(unlikely(ret))
     {
     ...
     ir_data = (tcs3400_chip.als_inf.red+tcs3400_chip.als_inf.green+tcs3400_chip.als_inf.blue- tcs3400_chip.als_inf.clear +1)/2;
     if(ir_data < 0)
         ir_data=0;
     tcs3400_chip.als_inf.ir_data = ir_data;
     //LOG("clear is %d ,red is %d, green is %d, blue is %d, ir_data is %d,atime is %d, again is %d \n",
...
static int tcs3400_als_enable(int on)
{
    ...
    {
        tcs3400_cal_cpl();
        ret = sensor_i2c_write(&tcs3400_ctx,TCS3400_CMD_ALL_INT_CLR,&temp_zero,I2C_DATA_LEN, I2C_OP_RETRIES);
        if(unlikely(ret))
        {
            return ret;
        }
        tcs3400_chip.shadow[TCS3400_ENABLE] |= (TCS3400_EN_PWR_ON | TCS3400_EN_ALS | TCS3400_EN_ALS_IRQ);
        ret = sensor_i2c_write(&tcs3400_ctx,TCS3400_ENABLE,&(tcs3400_chip.shadow[TCS3400_ENABLE]),I2C_DATA_LEN, I2C_OP_RETRIES);
        if(unlikely(ret))
        {
        ...
        tcs3400_chip.shadow[TCS3400_REG_AUX] |= TCS3400_EN_ALS_SAT_IRQ;
        sensor_i2c_write(&tcs3400_ctx,TCS3400_REG_AUX,&(tcs3400_chip.shadow[TCS3400_REG_AUX]),I2C_DATA_LEN, I2C_OP_RETRIES);
        tcs3400_chip.als_enabled = 1;
    }
    else
    {    
...
static int tcs3400_get_lux_cct()
{
    ...
    int32_t ir,cct=0, sat =0, r_coef = R_Coef, g_coef = G_Coef, b_coef = B_Coef;
    ...
    if((tcs3400_chip.als_inf.clear >= tcs3400_chip.als_inf.sat) || tcs3400_chip.in_asat){
...
static int drv_als_ams_tcs3400_read(const void *buf, size_t len)
{
    ...
    ret = sensor_i2c_read(&tcs3400_ctx,TCS3400_STATUS,&(tcs3400_chip.shadow[TCS3400_STATUS]),I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret))
    {
    ...
    else
    {
        ...
        if(((status & TCS3400_ST_ALS_IRQ)&&(status & TCS3400_ST_ALS_VALID)) ||(tcs3400_chip.in_asat))
        {
            //clear interrupt
            sensor_i2c_write(&tcs3400_ctx,TCS3400_CMD_ALL_INT_CLR,&temp_zero,I2C_DATA_LEN,I2C_OP_RETRIES);
