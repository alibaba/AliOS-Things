...
static int32_t bmi160_acc_factor[ACC_RANGE_MAX] = { BMI160_ACC_SENSITIVITY_2G, BMI160_ACC_SENSITIVITY_4G, 
                                     BMI160_ACC_SENSITIVITY_8G, BMI160_ACC_SENSITIVITY_16G };
static int32_t bmi160_gyro_factor[GYRO_RANGE_MAX] = {BMI160_GYRO_SENSITIVITY_125DPS, BMI160_GYRO_SENSITIVITY_250DPS, BMI160_GYRO_SENSITIVITY_500DPS, 
...
static int drv_acc_gyro_bosch_bmi160_soft_reset(i2c_dev_t* drv)
{
    ...
    ret = sensor_i2c_write(drv, BMI160_CMD_COMMANDS__REG, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    aos_msleep(2);
    if (unlikely(ret)) {
        return -1;
    }
...
static int drv_acc_gyro_bosch_bmi160_validate_id(i2c_dev_t* drv, uint8_t id_value)
{
    ...
    ret = sensor_i2c_read(drv, BMI160_USER_CHIP_ID__REG, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
 ...
 * @return  the operation status, 0 is OK, others is error
 */
static int drv_acc_bosch_bmi160_set_power_mode(i2c_dev_t* drv, dev_power_mode_e mode)
{
...
static int drv_acc_bosch_bmi160_set_odr(i2c_dev_t* drv, uint32_t hz)
{
    ...
    ret = sensor_i2c_read(drv, BMI160_USER_ACCEL_CONFIG_OUTPUT_DATA_RATE__REG, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        return ret;
    }

	value = BMI160_SET_BITSLICE(value, BMI160_USER_ACCEL_CONFIG_OUTPUT_DATA_RATE, odr);

    ret = sensor_i2c_write(drv, BMI160_USER_ACCEL_CONFIG_OUTPUT_DATA_RATE__REG, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    aos_msleep(2);
    if (unlikely(ret)) {
        return ret;
    }
...
static int drv_acc_bosch_bmi160_set_range(i2c_dev_t* drv, uint32_t range)
{
    ...
    ret = sensor_i2c_read(drv, BMI160_USER_ACCEL_RANGE__REG, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        return ret;
    }
    ...
    ret = sensor_i2c_write(drv, BMI160_USER_ACCEL_RANGE__REG, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
     aos_msleep(2);
    if (unlikely(ret)) {
        return ret;
    }
    ...
    if ((range >= ACC_RANGE_2G)&&(range <= ACC_RANGE_16G)) {
        cur_acc_factor = bmi160_acc_factor[range];
    }
...
static int drv_acc_bosch_bmi160_read(void *buf, size_t len)
{
    ...
    ret  = sensor_i2c_read(&bmi160_ctx, BMI160_USER_DATA_14_ACCEL_X_LSB__REG,  &reg[0], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&bmi160_ctx, BMI160_USER_DATA_15_ACCEL_X_MSB__REG ,  &reg[1], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&bmi160_ctx, BMI160_USER_DATA_16_ACCEL_Y_LSB__REG,  &reg[2], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&bmi160_ctx, BMI160_USER_DATA_17_ACCEL_Y_MSB__REG,  &reg[3], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&bmi160_ctx, BMI160_USER_DATA_18_ACCEL_Z_LSB__REG,  &reg[4], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&bmi160_ctx, BMI160_USER_DATA_19_ACCEL_Z_MSB__REG,  &reg[5], I2C_REG_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        return -1;
    }

    accel->data[DATA_AXIS_X] = (int16_t)((((int32_t)((int8_t)reg[1])) << BMI160_SHIFT_BIT_POSITION_BY_08_BITS) | (reg[0]));
    accel->data[DATA_AXIS_Y] = (int16_t)((((int32_t)((int8_t)reg[3])) << BMI160_SHIFT_BIT_POSITION_BY_08_BITS) | (reg[2]));
    accel->data[DATA_AXIS_Z] = (int16_t)((((int32_t)((int8_t)reg[5])) << BMI160_SHIFT_BIT_POSITION_BY_08_BITS) | (reg[4]));

    if (cur_acc_factor != 0) {
                // the unit of acc is mg, 1000 mg = 1 g.
        accel->data[DATA_AXIS_X] = accel->data[DATA_AXIS_X] * ACCELEROMETER_UNIT_FACTOR / (int32_t)cur_acc_factor;
        accel->data[DATA_AXIS_Y] = accel->data[DATA_AXIS_Y] * ACCELEROMETER_UNIT_FACTOR / (int32_t)cur_acc_factor;
        accel->data[DATA_AXIS_Z] = accel->data[DATA_AXIS_Z] * ACCELEROMETER_UNIT_FACTOR / (int32_t)cur_acc_factor;
        //printf("accel->data[DATA_AXIS_X]=%d****accel->data[DATA_AXIS_Y]=%d***accel->data[DATA_AXIS_Z]=%d \n", accel->data[DATA_AXIS_X], accel->data[DATA_AXIS_Y], accel->data[DATA_AXIS_Z]);
    }
 ...
 * @return  the operation status, 0 is OK, others is error
 */
static int drv_gyro_bosch_bmi160_set_power_mode(i2c_dev_t* drv, dev_power_mode_e mode)
{
    uint8_t value,value1 = 0x00;
    int ret = 0;
...
static int drv_gyro_bosch_bmi160_set_odr(i2c_dev_t* drv, uint32_t hz)
{
    ...
    ret = sensor_i2c_read(drv, BMI160_USER_GYRO_CONFIG_OUTPUT_DATA_RATE__REG, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        return ret;
    }

   
	value = BMI160_SET_BITSLICE(value, BMI160_USER_GYRO_CONFIG_OUTPUT_DATA_RATE, odr);
    ret = sensor_i2c_write(drv, BMI160_USER_GYRO_CONFIG_OUTPUT_DATA_RATE__REG, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    aos_msleep(2);
    if (unlikely(ret)) {
        return ret;
    }
...
static int drv_gyro_bosch_bmi160_set_range(i2c_dev_t* drv, uint32_t range)
{
    ...
    ret = sensor_i2c_read(drv, BMI160_USER_GYRO_RANGE__REG, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        return ret;
    }
    ...
    ret = sensor_i2c_write(drv, BMI160_USER_GYRO_RANGE__REG, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
     aos_msleep(2);
    if (unlikely(ret)) {
        return ret;
    }
    ...
    if ((range >= GYRO_RANGE_250DPS)&&(range <= GYRO_RANGE_2000DPS)) {
        cur_gyro_factor = bmi160_gyro_factor[range];
    }
...
static int drv_gyro_bosch_bmi160_read(void *buf, size_t len)
{
    ...
    ret  = sensor_i2c_read(&bmi160_ctx, BMI160_USER_DATA_8_GYRO_X_LSB__REG,  &reg[0], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&bmi160_ctx, BMI160_USER_DATA_9_GYRO_X_MSB__REG,  &reg[1], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&bmi160_ctx, BMI160_USER_DATA_10_GYRO_Y_LSB__REG,  &reg[2], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&bmi160_ctx, BMI160_USER_DATA_11_GYRO_Y_MSB__REG,  &reg[3], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&bmi160_ctx, BMI160_USER_DATA_12_GYRO_Z_LSB__REG,  &reg[4], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&bmi160_ctx, BMI160_USER_DATA_13_GYRO_Z_MSB__REG,  &reg[5], I2C_REG_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        return -1;
    }

	gyro->data[DATA_AXIS_X] = (int16_t)((((int32_t)((int8_t)reg[1])) << BMI160_SHIFT_BIT_POSITION_BY_08_BITS) | (reg[0]));
    gyro->data[DATA_AXIS_Y] = (int16_t)((((int32_t)((int8_t)reg[3])) << BMI160_SHIFT_BIT_POSITION_BY_08_BITS) | (reg[2]));
    gyro->data[DATA_AXIS_Z] = (int16_t)((((int32_t)((int8_t)reg[5])) << BMI160_SHIFT_BIT_POSITION_BY_08_BITS) | (reg[4]));

    if (cur_gyro_factor != 0) {
        gyro->data[DATA_AXIS_X] = gyro->data[DATA_AXIS_X] * GYROSCOPE_UNIT_FACTOR / cur_gyro_factor;
        gyro->data[DATA_AXIS_Y] = gyro->data[DATA_AXIS_Y] * GYROSCOPE_UNIT_FACTOR / cur_gyro_factor;
        gyro->data[DATA_AXIS_Z] = gyro->data[DATA_AXIS_Z] * GYROSCOPE_UNIT_FACTOR / cur_gyro_factor;
    }
