...
typedef enum {
  HTS221_BDU_CONTINUOUS_UPDATE      =  (uint8_t)0x00,  /*!< Data updated continuously */
  HTS221_BDU_NO_UPDATE              =  (uint8_t)0x04   /*!< Data updated after a read operation */
} HTS221_bdu_e;
...
static int drv_temp_humi_st_hts221_validate_id(i2c_dev_t* drv, uint8_t id_value)
{
    ...
    ret = sensor_i2c_read(drv, HTS221_WHO_AM_I_REG, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
...
static int drv_temp_humi_st_hts221_set_power_mode(i2c_dev_t* drv, dev_power_mode_e mode)
{
    uint8_t value = 0x00;
    int ret = 0;
    
    ret = sensor_i2c_read(drv, HTS221_CTRL_REG1, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
    ...
    switch(mode){
        case DEV_POWER_ON:{
            ...
            ret = sensor_i2c_write(drv, HTS221_CTRL_REG1, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
            if(unlikely(ret)){
                return ret;
            }
        }break;
...
static int drv_temp_st_hts221_read(void *buf, size_t len)
{
    ...
    ret  = sensor_i2c_read(&HTS221_ctx, (HTS221_T0_DEGC_X8 | 0x80), buffer, 2, I2C_OP_RETRIES);
    ret  = sensor_i2c_read(&HTS221_ctx, HTS221_T0_T1_DEGC_H2, &tmp, I2C_DATA_LEN, I2C_OP_RETRIES);
    T0_degC_x8_u16 = (((uint16_t)(tmp & 0x03)) << 8) | ((uint16_t)buffer[0]);
    T1_degC_x8_u16 = (((uint16_t)(tmp & 0x0C)) << 6) | ((uint16_t)buffer[1]);
    T0_degC = T0_degC_x8_u16 >> 3;
    T1_degC = T1_degC_x8_u16 >> 3;
    ret |= sensor_i2c_read(&HTS221_ctx, (HTS221_T0_OUT_L | 0x80), buffer, 4, I2C_OP_RETRIES);
    T0_out = (((uint16_t)buffer[1]) << 8) | (uint16_t)buffer[0];
    T1_out = (((uint16_t)buffer[3]) << 8) | (uint16_t)buffer[2];
    ret |= sensor_i2c_read(&HTS221_ctx, (HTS221_TEMP_OUT_L_REG | 0x80), buffer, 2, I2C_OP_RETRIES);
    T_out = (((uint16_t)buffer[1]) << 8) | (uint16_t)buffer[0];

    temp =10*(T_out - T0_out) * (T1_degC - T0_degC) / (T1_out - T0_out)  + 10*T0_degC;
...
static int drv_humi_st_hts221_read(void *buf, size_t len)
{
    ...
    ret  = sensor_i2c_read(&HTS221_ctx, (HTS221_H0_RH_X2 | 0x80), buffer, 2, I2C_OP_RETRIES);
