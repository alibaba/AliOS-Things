...
int16_t readBlock(uint8_t regAddress, uint8_t length, uint8_t *buffer)
{
	...
	ret = sensor_i2c_read(&(dps310_ctx.i2c), regAddress, buffer, length, I2C_OP_RETRIES);
	if (ret != 0)
	{
		return 0;
	}
...
{
	if (sensor_i2c_write(&(dps310_ctx.i2c), regAddress, &data, 1, AOS_WAIT_FOREVER) == 0)
	{
...
{
	uint8_t data;
	int ret = sensor_i2c_read(&(dps310_ctx.i2c), regAddress, &data, 1, I2C_OP_RETRIES);
	if(ret == 0)
		return data;
	else
		return -1;
 ...
 * 				or -1 on fail
 */
int16_t writeByteBitfield(uint8_t data,
										uint8_t regAddress,
										uint8_t mask,
										uint8_t shift)
{
...
int32_t calcPressure(int32_t raw)
{
	...
	prs =   dps310_ctx.m_c00
			+ prs * (dps310_ctx.m_c10 + prs * (dps310_ctx.m_c20 + prs * dps310_ctx.m_c30))
			+ dps310_ctx.m_lastTempScal * (dps310_ctx.m_c01 + prs * (dps310_ctx.m_c11 + prs * dps310_ctx.m_c21));
	...
	case 1: 						//measurement ready, expected case
		{
			...
			dps310_ctx.m_opMode = IDLE;				//opcode was automatically reseted by DPS310
			switch(oldMode)
			{
			case CMD_TEMP: 	//temperature
...
int16_t readcoeffs(void)
{
	...
	dps310_ctx.m_c0Half =    ((uint32_t)buffer[0] << 4)
	...
	dps310_ctx.m_c01 =   ((uint32_t)buffer[8] << 8)
			| (uint32_t)buffer[9];
	...
	dps310_ctx.m_c11 =   ((uint32_t)buffer[10] << 8)
			| (uint32_t)buffer[11];
	...
	dps310_ctx.m_c20 =   ((uint32_t)buffer[12] << 8)
			| (uint32_t)buffer[13];
	...
	dps310_ctx.m_c21 =   ((uint32_t)buffer[14] << 8)
			| (uint32_t)buffer[15];
	...
	dps310_ctx.m_c30 =   ((uint32_t)buffer[16] << 8)
			| (uint32_t)buffer[17];
