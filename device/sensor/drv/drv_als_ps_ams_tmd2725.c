...
static int drv_als_ps_ams_tmd2725_validate_id(i2c_dev_t *drv, uint8_t id_value)
{
    ...
    ret = sensor_i2c_read(drv,TMD2725_REG_CHIPID, &chipid_value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)) {
...
static int tmd2725_reset_als_regs(i2c_dev_t *drv)
{
    ...
    {
        reg = regs[i];
        ret = sensor_i2c_write(drv,reg,&(tmd2725_chip.shadow[reg]),I2C_DATA_LEN, I2C_OP_RETRIES);
        if(unlikely(ret))
        {
...
static int32_t sensor_i2c_modify(i2c_dev_t *drv, uint8_t *shadow, uint16_t reg, uint8_t mask, uint8_t value)
{
    int ret;
    uint8_t temp;
...
static int drv_als_ams_tmd2725_set_power_mode(i2c_dev_t * drv, dev_power_mode_e mode)
{
    int ret = 0;
    switch(mode){
        case DEV_POWER_ON:{
            if(!(tmd2725_chip.als_enabled))
            {
             ...
             sensor_i2c_modify(drv,tmd2725_chip.shadow,TMD2725_REG_INTENAB,TMD2725_AIEN,TMD2725_AIEN);
             sensor_i2c_modify(drv,tmd2725_chip.shadow,TMD2725_REG_ENABLE,TMD2725_WEN|TMD2725_AEN|TMD2725_PON,TMD2725_WEN|TMD2725_AEN|TMD2725_PON);
             tmd2725_chip.als_enabled = true;
            }
        }break;    
        case DEV_POWER_OFF:
        case DEV_SLEEP:{
            sensor_i2c_modify(drv,tmd2725_chip.shadow,TMD2725_REG_INTENAB,TMD2725_AIEN,0);
            sensor_i2c_modify(drv,tmd2725_chip.shadow,TMD2725_REG_ENABLE,TMD2725_WEN | TMD2725_AEN, 0);
            tmd2725_chip.als_enabled = false;
            if(!(tmd2725_chip.shadow[TMD2725_REG_ENABLE] & TMD2725_EN_ALL))
                sensor_i2c_modify(drv,tmd2725_chip.shadow,TMD2725_REG_ENABLE,TMD2725_PON, 0);
        }break;      
       default:break;
    }
    return 0;
...
static int drv_als_ps_ams_tmd2725_set_default_config(i2c_dev_t * drv)
{
   ...
   if(!init_flag)
   {
    ...
    tmd2725_chip.shadow[TMD2725_REG_PGCFG0] = tmd2725_chip.prox_pulse_cnt | tmd2725_chip.prox_pulse_len;
    tmd2725_chip.shadow[TMD2725_REG_ATIME] = tmd2725_chip.atime;
    tmd2725_chip.shadow[TMD2725_REG_CFG1] = tmd2725_chip.again;
    tmd2725_chip.shadow[TMD2725_REG_PTIME] = P_TIME_US(2816);
    tmd2725_chip.shadow[TMD2725_REG_PGCFG1] = (tmd2725_chip.prox_gain & TMD2725_MASK_PGAIN)|tmd2725_chip.prox_drive;
...
static int tmd2725_read_als_data()
{
    int ret;
    uint8_t *buf;
    ret = sensor_i2c_read(&tmd2725_ctx,TMD2725_REG_CH0DATA,&(tmd2725_chip.shadow[TMD2725_REG_CH0DATA]),4,I2C_OP_RETRIES);
    if(unlikely(ret))
    {
...
static int tmd2725_als_cal_cpl()
{
    ...
    cpl*=als_gains[tmd2725_chip.shadow[TMD2725_REG_CFG1] & TMD2725_MASK_AGAIN];
 ...
 //   LOG("CPL is %d  sat is %d, atime is  %d  again is %d\n",tmd2725_chip.als_inf.CPL,tmd2725_chip.als_inf.sat, tmd2725_chip.atime, tmd2725_chip.again);
}
...
static int tmd2725_set_als_gain(uint8_t gain)
{
    ...
    sensor_i2c_read(&tmd2725_ctx,TMD2725_REG_ENABLE,&temp,I2C_DATA_LEN, I2C_OP_RETRIES);
    sensor_i2c_write(&tmd2725_ctx,TMD2725_REG_ENABLE,&temp_zero,I2C_DATA_LEN, I2C_OP_RETRIES);
    sensor_i2c_modify(&tmd2725_ctx,tmd2725_chip.shadow,TMD2725_REG_CFG1,TMD2725_MASK_AGAIN,ctrl_reg);
    sensor_i2c_write(&tmd2725_ctx,TMD2725_REG_ENABLE,&temp,I2C_DATA_LEN, I2C_OP_RETRIES);
...
static int tmd2725_get_lux()
{
    ...
    tmd2725_als_cal_cpl();
    lux1 = ((tmd2725_chip.als_inf.lux1_ch0_coef *ch0) - (tmd2725_chip.als_inf.lux1_ch1_coef*ch1)) / tmd2725_chip.als_inf.CPL;
    lux2 = ((tmd2725_chip.als_inf.lux2_ch0_coef *ch0) - (tmd2725_chip.als_inf.lux2_ch1_coef*ch1)) / tmd2725_chip.als_inf.CPL;
    lux = max(lux1, lux2);
...
static int drv_als_ams_tmd2725_read(const void *buf, size_t len)
{
    ...
    ret = sensor_i2c_read(&tmd2725_ctx,TMD2725_REG_STATUS,&(tmd2725_chip.shadow[TMD2725_REG_STATUS]),I2C_DATA_LEN,I2C_OP_RETRIES);
    if(unlikely(ret))
    {
    ...
    {
        //clear AINT
         sensor_i2c_modify(&tmd2725_ctx,tmd2725_chip.shadow,TMD2725_REG_STATUS,TMD2725_ST_ALS_IRQ,0);
...
int drv_als_ams_tmd2725_init(void)
{
    ...
    ret = drv_als_ps_ams_tmd2725_validate_id(&tmd2725_ctx,TMD2725_CHIPID_VALUE);
    if(unlikely(ret)) {
        return -1;
    }
...
static uint32_t ams_sensor_i2c_write(i2c_dev_t *drv, uint8_t *shadow, uint16_t reg, uint8_t data)
{
...
int tmd2725_offset_calibration()
{
    ...
    sensor_i2c_read(&tmd2725_ctx,TMD2725_REG_ENABLE,&temp,I2C_DATA_LEN,I2C_OP_RETRIES);
    //enable prox enb bit
    sensor_i2c_modify(&tmd2725_ctx,tmd2725_chip.shadow,TMD2725_REG_ENABLE,TMD2725_PEN|TMD2725_PON,TMD2725_PON);
    sensor_i2c_modify(&tmd2725_ctx,tmd2725_chip.shadow,TMD2725_REG_INTENAB,TMD2725_PIEN|TMD2725_CIEN, TMD2725_CIEN);
    sensor_i2c_modify(&tmd2725_ctx,tmd2725_chip.shadow,TMD2725_REG_CALIBCFG,TMD2725_MASK_BINSRCH_TARGET|TMD2725_MASK_PROX_DATA_AVG|TMD2725_MASK_PROX_AUTO_OFFSET_ADJUST,
                     (0x03 << TMD2725_SHIFT_BINSRCH_TARGET) |(1 << TMD2725_SHIFT_PROX_DATA_AVG) |(1 << TMD2725_SHIFT_PROX_AUTO_OFFSET_ADJUST));   

    sensor_i2c_modify(&tmd2725_ctx,tmd2725_chip.shadow,TMD2725_REG_CALIB,TMD2725_MASK_START_OFFSET_CALIB,0x01);
    //set calibration default time 100ms
    aos_msleep(100);

    sensor_i2c_read(&tmd2725_ctx,TMD2725_REG_POFFSET_L,&tmd2725_chip.shadow[TMD2725_REG_POFFSET_L],2,I2C_OP_RETRIES);
    ...
    sensor_i2c_modify(&tmd2725_ctx,tmd2725_chip.shadow,TMD2725_REG_ENABLE,TMD2725_PEN,temp);
    return 0;
}

static int drv_ps_ams_tmd2725_set_power_mode(i2c_dev_t * drv, dev_power_mode_e mode)
{
int ret = 0;
    switch(mode){
        case DEV_POWER_ON:{
            if(!(tmd2725_chip.prox_enabled))
            {
             tmd2725_chip.prox_th_min =50;
             tmd2725_chip.prox_th_max =80;
             ams_sensor_i2c_write(&tmd2725_ctx,tmd2725_chip.shadow,TMD2725_REG_PILT,tmd2725_chip.prox_th_min);
             ams_sensor_i2c_write(&tmd2725_ctx,tmd2725_chip.shadow,TMD2725_REG_PIHT,tmd2725_chip.prox_th_max);
             tmd2725_offset_calibration();

             sensor_i2c_modify(&tmd2725_ctx,tmd2725_chip.shadow,TMD2725_REG_PERS,TMD2725_MASK_PROX_PERS,tmd2725_chip.persist & TMD2725_MASK_PROX_PERS);

             ams_sensor_i2c_write(&tmd2725_ctx,tmd2725_chip.shadow,TMD2725_REG_PGCFG0,tmd2725_chip.prox_pulse_cnt|tmd2725_chip.prox_pulse_len);
             
             sensor_i2c_modify(&tmd2725_ctx,tmd2725_chip.shadow,TMD2725_REG_PGCFG1,(tmd2725_chip.prox_gain & TMD2725_MASK_PGAIN)|(tmd2725_chip.prox_drive &TMD2725_MASK_PDRIVE),
                               TMD2725_MASK_PGAIN|TMD2725_MASK_PDRIVE);
             ams_sensor_i2c_write(&tmd2725_ctx,tmd2725_chip.shadow,TMD2725_REG_PTIME, P_TIME_US(2816));
             
             //enable proximity and interrupt        
             sensor_i2c_modify(&tmd2725_ctx,tmd2725_chip.shadow,TMD2725_REG_ENABLE,TMD2725_PEN | TMD2725_PON,TMD2725_PEN | TMD2725_PON);
             sensor_i2c_modify(&tmd2725_ctx,tmd2725_chip.shadow,TMD2725_REG_INTENAB,TMD2725_PIEN, TMD2725_PIEN);      
        ...
        case DEV_SLEEP:{
            if(tmd2725_chip.prox_enabled)
            {  
            sensor_i2c_modify(&tmd2725_ctx,tmd2725_chip.shadow,TMD2725_REG_ENABLE, TMD2725_PEN, 0);
            sensor_i2c_modify(&tmd2725_ctx,tmd2725_chip.shadow,TMD2725_REG_INTENAB, TMD2725_PIEN, 0);
             tmd2725_chip.prox_enabled = false;
             if(!(tmd2725_chip.shadow[TMD2725_REG_ENABLE] & TMD2725_EN_ALL))
                sensor_i2c_modify(drv,tmd2725_chip.shadow,TMD2725_REG_ENABLE,TMD2725_PON, 0);
            }
...
{
    sensor_i2c_read(&tmd2725_ctx,TMD2725_REG_PDATA,&tmd2725_chip.shadow[TMD2725_REG_PDATA],I2C_DATA_LEN,I2C_OP_RETRIES);
    tmd2725_chip.prx_inf.raw = tmd2725_chip.shadow[TMD2725_REG_PDATA];  
//    LOG("%s  %s  read_prox_data raw data is %d\n",SENSOR_STR,__func__,tmd2725_chip.prx_inf.raw);
}
