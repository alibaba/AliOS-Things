...
//static int drv_acc_adi_adxl372_self_test(i2c_dev_t* drv,int32_t* data);

static int drv_acc_adi_adxl372_soft_reset(i2c_dev_t* drv)
{
    int ret = 0;
    uint8_t value = ADXL372_RESET_CODE;
    ret = sensor_i2c_write(drv, ADXL372_RESET, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
	if(unlikely(ret)){
        return -1;
    }
    return 0;
...
static int drv_acc_adi_adxl372_validate_id(i2c_dev_t* drv, uint8_t id_value)
{
    ...
    ret = sensor_i2c_read(drv, ADXL372_PARTID, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return -1;
    }
...
static int drv_acc_adi_adxl372_set_power_mode(i2c_dev_t* drv, dev_power_mode_e mode)
{
    uint8_t value;
    int ret = 0;
    
    ret = sensor_i2c_read(drv, ADXL372_POWER_CTL, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
    ...
    switch(mode){
        case DEV_POWER_ON:{
            value &= 0xFC;
			value |= 0x03;
            ret = sensor_i2c_write(drv, ADXL372_POWER_CTL, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
            if(unlikely(ret)){
                return ret;
            }
        }break;
...
static int drv_acc_adi_adxl372_set_odr(i2c_dev_t* drv, uint32_t odr)
{
    ...
    ret = sensor_i2c_read(drv, ADXL372_TIMING, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
    ...
    ret = sensor_i2c_write(drv, ADXL372_TIMING, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
    return 0;    
...
static int drv_acc_adi_adxl372_set_bw(i2c_dev_t* drv, uint32_t bw)
{
    ...
    ret = sensor_i2c_read(drv, ADXL372_MEASURE, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
    ...
    ret = sensor_i2c_write(drv, ADXL372_MEASURE, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if(unlikely(ret)){
        return ret;
    }
...
static int drv_acc_adi_adxl372_read(void *buf, size_t len)
{
    ...
    ret  = sensor_i2c_read(&adxl372_ctx, ADXL372_X_DATA_H,  &LocalBuf[0], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&adxl372_ctx, ADXL372_X_DATA_L,  &LocalBuf[1], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&adxl372_ctx, ADXL372_Y_DATA_H,  &LocalBuf[2], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&adxl372_ctx, ADXL372_Y_DATA_L,  &LocalBuf[3], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&adxl372_ctx, ADXL372_Z_DATA_H,  &LocalBuf[4], I2C_REG_LEN, I2C_OP_RETRIES);
    ret |= sensor_i2c_read(&adxl372_ctx, ADXL372_Z_DATA_L,  &LocalBuf[5], I2C_REG_LEN, I2C_OP_RETRIES);
