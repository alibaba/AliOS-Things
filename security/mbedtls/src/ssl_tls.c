                            ...
                            const mbedtls_ssl_session *src)
{
    ...
    if (src->peer_cert != NULL) {
        ...
        if ((ret =
               mbedtls_x509_crt_parse_der(dst->peer_cert, src->peer_cert->raw.p,
...
int mbedtls_ssl_derive_keys(mbedtls_ssl_context *ssl)
{
    ...
    if (cipher_info->mode == MBEDTLS_MODE_GCM ||
        ...
        transform->minlen =
          ...
          (transform->ciphersuite_info->flags & MBEDTLS_CIPHERSUITE_SHORT_TAG
             ? 8
             : 16);
    } else {
  ...
  defined(MBEDTLS_SSL_PROTO_TLS1_2)
    ...
    if (mbedtls_ssl_hw_record_init != NULL) {
        ...
        if ((ret = mbedtls_ssl_hw_record_init(
               ssl, key1, key2, transform->keylen, transform->iv_enc,
               transform->iv_dec, iv_copy_len, mac_enc, mac_dec,
               transform->maclen)) != 0) {
    ...
    if (ssl->conf->f_export_keys != NULL) {
        ssl->conf->f_export_keys(ssl->conf->p_export_keys, session->master,
                                 keyblk, transform->maclen, transform->keylen,
                                 iv_copy_len);
    }
#endif
    ...
    if (session->compression == MBEDTLS_SSL_COMPRESS_DEFLATE) {
        if (ssl->compress_buf == NULL) {
            ...
            if (ssl->compress_buf == NULL) {
                MBEDTLS_SSL_DEBUG_MSG(
                  1, ("alloc(%d bytes) failed", MBEDTLS_SSL_BUFFER_LEN));
                return (MBEDTLS_ERR_SSL_ALLOC_FAILED);
            }
        }
        ...
        if (deflateInit(&transform->ctx_deflate, Z_DEFAULT_COMPRESSION) !=
              Z_OK ||
...
static void ssl_mac(mbedtls_md_context_t *md_ctx, unsigned char *secret,
                    unsigned char *buf, size_t len, unsigned char *ctr,
                    int type)
{
    ...
    if (mode == MBEDTLS_MODE_STREAM ||
        ...
        if (ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_0) {
            ssl_mac(&ssl->transform_out->md_ctx_enc,
                    ssl->transform_out->mac_enc, ssl->out_msg, ssl->out_msglen,
    ...
    if (mode == MBEDTLS_MODE_STREAM) {
      ...
      if (mode == MBEDTLS_MODE_GCM || mode == MBEDTLS_MODE_CCM) {
        ...
        if ((ret = mbedtls_cipher_auth_encrypt(
               &ssl->transform_out->cipher_ctx_enc, ssl->transform_out->iv_enc,
               ssl->transform_out->ivlen, add_data, 13, enc_msg, enc_msglen,
               enc_msg, &olen, enc_msg + enc_msglen, taglen)) != 0) {
  ...
  (defined(MBEDTLS_AES_C) || defined(MBEDTLS_CAMELLIA_C))
      if (mode == MBEDTLS_MODE_CBC) {
        ...
        if (auth_done == 0) {
            ...
            mbedtls_md_hmac_update(&ssl->transform_out->md_ctx_enc, pseudo_hdr,
                                   13);
...
static int ssl_decrypt_buf(mbedtls_ssl_context *ssl)
{
    ...
    if (mode == MBEDTLS_MODE_STREAM) {
        ...
        if ((ret = mbedtls_cipher_crypt(
               &ssl->transform_in->cipher_ctx_dec, ssl->transform_in->iv_dec,
               ssl->transform_in->ivlen, ssl->in_msg, ssl->in_msglen,
               ssl->in_msg, &olen)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_cipher_crypt", ret);
            return (ret);
        }
      ...
      if (mode == MBEDTLS_MODE_GCM || mode == MBEDTLS_MODE_CCM) {
        ...
        if ((ret = mbedtls_cipher_auth_decrypt(
               &ssl->transform_in->cipher_ctx_dec, ssl->transform_in->iv_dec,
               ssl->transform_in->ivlen, add_data, 13, dec_msg, dec_msglen,
  ...
  (defined(MBEDTLS_AES_C) || defined(MBEDTLS_CAMELLIA_C))
      if (mode == MBEDTLS_MODE_CBC) {
        ...
        if (ssl->session_in->encrypt_then_mac == MBEDTLS_SSL_ETM_ENABLED) {
            ...
            mbedtls_md_hmac_update(&ssl->transform_in->md_ctx_dec, pseudo_hdr,
                                   13);
        ...
        if ((ret = mbedtls_cipher_crypt(
               &ssl->transform_in->cipher_ctx_dec, ssl->transform_in->iv_dec,
               ssl->transform_in->ivlen, dec_msg, dec_msglen, dec_msg_result,
               &olen)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_cipher_crypt", ret);
            return (ret);
        }
  ...
  defined(MBEDTLS_SSL_PROTO_TLS1_2)
          if (ssl->minor_ver > MBEDTLS_SSL_MINOR_VERSION_0) {
            ...
            for (i = 1; i <= 256; i++) {
                real_count &= (i <= padlen);
                pad_count +=
                  real_count * (ssl->in_msg[padding_idx + i] == padlen - 1);
            }
  ...
  defined(MBEDTLS_SSL_PROTO_TLS1_2)
          if (ssl->minor_ver > MBEDTLS_SSL_MINOR_VERSION_0) {
            ...
            mbedtls_md_hmac_update(&ssl->transform_in->md_ctx_dec, ssl->in_ctr,
                                   8);
            mbedtls_md_hmac_update(&ssl->transform_in->md_ctx_dec, ssl->in_hdr,
                                   3);
            mbedtls_md_hmac_update(&ssl->transform_in->md_ctx_dec, ssl->in_len,
                                   2);
        ...
        MBEDTLS_SSL_DEBUG_BUF(4, "message  mac", tmp,
                              ssl->transform_in->maclen);
...
static int ssl_compress_buf(mbedtls_ssl_context *ssl)
{
    ...
    MBEDTLS_SSL_DEBUG_MSG(
      3, ("before compression: msglen = %d, ", ssl->out_msglen));
    ...
    ssl->out_msglen =
      MBEDTLS_SSL_BUFFER_LEN - ssl->transform_out->ctx_deflate.avail_out;

    MBEDTLS_SSL_DEBUG_MSG(
      3, ("after compression: msglen = %d, ", ssl->out_msglen));
...
static int ssl_decompress_buf(mbedtls_ssl_context *ssl)
{
    ...
    ssl->in_msglen =
      MBEDTLS_SSL_MAX_CONTENT_LEN - ssl->transform_in->ctx_inflate.avail_out;

    MBEDTLS_SSL_DEBUG_MSG(
      3, ("after decompression: msglen = %d, ", ssl->in_msglen));
...
int mbedtls_ssl_fetch_input(mbedtls_ssl_context *ssl, size_t nb_want)
{
    ...
    if (nb_want >
        MBEDTLS_SSL_BUFFER_LEN - (size_t)(ssl->in_hdr - ssl->in_buf)) {
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        ...
        if (ssl->next_record_offset != 0) {
            ...
            if (ssl->in_left != 0) {
                MBEDTLS_SSL_DEBUG_MSG(
                  2, ("next record in same datagram, offset: %d",
        ...
        while (ssl->in_left < nb_want) {
            ...
            if (ssl_check_timer(ssl) != 0) {
                ...
                if (ssl->f_recv_timeout != NULL) {
                    ...
                    ret =
                      ssl->f_recv(ssl->p_bio, ssl->in_hdr + ssl->in_left, len);
                }
            }
...
static int ssl_reassemble_dtls_handshake(mbedtls_ssl_context *ssl)
{
    ...
    frag_len =
      (ssl->in_msg[9] << 16) | (ssl->in_msg[10] << 8) | ssl->in_msg[11];
    ...
    MBEDTLS_SSL_DEBUG_BUF(3, "reassembled handshake message", ssl->in_msg,
                          ssl->in_hslen);
...
int mbedtls_ssl_prepare_handshake_record(mbedtls_ssl_context *ssl)
{
    if (ssl->in_msglen < mbedtls_ssl_hs_hdr_len(ssl)) {
        MBEDTLS_SSL_DEBUG_MSG(
          1, ("handshake message too short: %d", ssl->in_msglen));
        return (MBEDTLS_ERR_SSL_INVALID_RECORD);
    }
...
static int ssl_handle_possible_reconnect(mbedtls_ssl_context *ssl)
{
    ...
    ret = ssl_check_dtls_clihlo_cookie(
      ...
      MBEDTLS_SSL_MAX_CONTENT_LEN, &len);
...
static int ssl_parse_record_header(mbedtls_ssl_context *ssl)
{
    ...
    if (ssl->transform_in == NULL) {
        ...
        if (ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_0 &&
            ssl->in_msglen >
              ssl->transform_in->minlen + MBEDTLS_SSL_MAX_CONTENT_LEN) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("bad message length"));
            return (MBEDTLS_ERR_SSL_INVALID_RECORD);
        }
#endif
  ...
  defined(MBEDTLS_SSL_PROTO_TLS1_2)
        ...
        if (ssl->minor_ver >= MBEDTLS_SSL_MINOR_VERSION_1 &&
            ssl->in_msglen >
              ssl->transform_in->minlen + MBEDTLS_SSL_MAX_CONTENT_LEN + 256) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("bad message length"));
            return (MBEDTLS_ERR_SSL_INVALID_RECORD);
        }
#endif
    ...
    if ((ret = ssl_parse_record_header(ssl)) != 0) {
        ...
        if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM &&
            ...
            if (ret == MBEDTLS_ERR_SSL_UNEXPECTED_RECORD) {
                ...
                ssl->next_record_offset =
                  ssl->in_msglen + mbedtls_ssl_hdr_len(ssl);
  ...
  defined(MBEDTLS_SSL_PROTO_TLS1_2)
    if (ssl->conf->endpoint == MBEDTLS_SSL_IS_SERVER &&
        ...
        if (ssl->in_hslen == 3 + mbedtls_ssl_hs_hdr_len(ssl) &&
            ...
            memcmp(ssl->in_msg + mbedtls_ssl_hs_hdr_len(ssl), "\0\0\0", 3) ==
              0) {
    ...
    if ((ssl->session_negotiate->peer_cert =
        ...
        MBEDTLS_SSL_DEBUG_MSG(
          1, ("alloc(%d bytes) failed", sizeof(mbedtls_x509_crt)));
        return (MBEDTLS_ERR_SSL_ALLOC_FAILED);
    }
    ...
    while (i < ssl->in_hslen) {
        ...
        ret = mbedtls_x509_crt_parse_der(ssl->session_negotiate->peer_cert,
                                         ssl->in_msg + i, n);
        if (0 != ret && (MBEDTLS_ERR_X509_UNKNOWN_SIG_ALG +
                         MBEDTLS_ERR_OID_NOT_FOUND) != ret) {
    ...
    if (authmode != MBEDTLS_SSL_VERIFY_NONE) {
        ...
        if (mbedtls_ssl_check_cert_usage(
              ssl->session_negotiate->peer_cert, ciphersuite_info,
              !ssl->conf->endpoint,
...
void mbedtls_ssl_optimize_checksum(
  mbedtls_ssl_context *ssl, const mbedtls_ssl_ciphersuite_t *ciphersuite_info)
{
        ...
        if (ssl->in_msgtype == MBEDTLS_SSL_MSG_HANDSHAKE) {
            ...
            if (ssl->conf->disable_renegotiation ==
                ...
                if (ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_0) {
                     ...
                     */
                    if ((ret = mbedtls_ssl_send_fatal_handshake_failure(ssl)) !=
                        0) {
                        return (ret);
                    }
                } else
...
static mbedtls_ecp_group_id ssl_preset_suiteb_curves[] = {
    MBEDTLS_ECP_DP_SECP256R1, MBEDTLS_ECP_DP_SECP384R1, MBEDTLS_ECP_DP_NONE
};
#endif
                                ...
                                int transport, int preset)
{
    ...
    if (endpoint == MBEDTLS_SSL_IS_SERVER) {
        if ((ret = mbedtls_ssl_conf_dh_param(
               conf, MBEDTLS_DHM_RFC5114_MODP_2048_P,
