...
#if defined(CONFIG_AES_ROM_TABLES)
/*
 * Forward S-box
 */
...
#endif /* CONFIG_AES_ROM_TABLES */

void hal_aes_init(hal_aes_context *ctx)
{
    memset(ctx, 0, sizeof(hal_aes_context));
}
...
void hal_aes_free(hal_aes_context *ctx)
{
    ...
    zeroize(ctx, sizeof(hal_aes_context));
}
                      ...
                      unsigned int keybits)
{
    ...
    /* Also checks keybits */
    if ((ret = hal_aes_setkey_enc(&cty, key, keybits)) != 0)
        goto exit;
 ...
 */
int hal_aes_crypt_ecb(hal_aes_context *ctx, int mode,
                     const unsigned char input[16],
                     unsigned char       output[16])
{
 ...
 */
int hal_aes_crypt_cbc(hal_aes_context *ctx, int mode, size_t length,
                     ...
                     unsigned char *output)
{
    ...
    if (mode == HAL_AES_DECRYPT) {
        while (length > 0) {
            ...
            hal_aes_crypt_ecb(ctx, mode, input, output);
        ...
        while (length > 0) {
            ...
            hal_aes_crypt_ecb(ctx, mode, output, output);
            memcpy(iv, output, 16);
 ...
 */
int hal_aes_crypt_cfb128(hal_aes_context *ctx, int mode, size_t length,
                        ...
                        const unsigned char *input, unsigned char *output)
{
    ...
    if (mode == HAL_AES_DECRYPT) {
        while (length--) {
            if (n == 0)
                hal_aes_crypt_ecb(ctx, HAL_AES_ENCRYPT, iv, iv);
 ...
 */
int hal_aes_crypt_cfb8(hal_aes_context *ctx, int mode, size_t length,
                      unsigned char iv[16], const unsigned char *input,
                      unsigned char *output)
{
    unsigned char c;
 ...
 */
int hal_aes_crypt_ctr(hal_aes_context *ctx, size_t length,
                     ...
                     const unsigned char *input, unsigned char *output)
{
    ...
    while (length--) {
        if (n == 0) {
            hal_aes_crypt_ecb(ctx, HAL_AES_ENCRYPT, nonce_counter,
                             stream_block);
