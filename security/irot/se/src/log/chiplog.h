...
void chip_log_hex_dump(const char *name, const uint8_t *in_data, uint32_t in_len);

#if (CHIP_DEBUG)
#define chip_log_debug(fmt, args...)                        chip_log(fmt, ##args)
#define chip_log_hex_data(name, in_data, in_len)            chip_log_hex_dump(name, in_data, in_len)
#else
