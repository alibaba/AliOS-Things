...
irot_result_t se_open_session(void **handle)
{
    ...
    lRet = SCardConnect(g_scardContext, g_readerName, SCARD_SHARE_SHARED, SCARD_PROTOCOL_T0 | SCARD_PROTOCOL_T1, &g_hCard,
                        &dwPro);
    if (lRet != SCARD_S_SUCCESS) {
        return IROT_ERROR_COMMUNICATION;
    }
...
irot_result_t se_transmit(void *handle, const uint8_t *cmd_apdu, const uint32_t cmd_len, uint8_t *rsp_buf,
                          uint32_t *rsp_len)
{
