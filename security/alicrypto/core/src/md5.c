...
{
    osa_memset(ctx, 0, sizeof(ali_algo_md5_context));
}
...
void ali_algo_md5_free(ali_algo_md5_context *ctx)
{
    ...
    ali_algo_zeroize(ctx, sizeof(ali_algo_md5_context));
}

void ali_algo_md5_clone(ali_algo_md5_context *dst, const ali_algo_md5_context *src)
{
    *dst = *src;
}
 ...
 * MD5 process buffer
 */
void ali_algo_md5_update(ali_algo_md5_context *ctx, const unsigned char *input,
                        size_t ilen)
{
    size_t   fill;
    uint32_t left;
    ...
    if (left && ilen >= fill) {
        ...
        ali_algo_md5_process(ctx, ctx->buffer);
        input += fill;
        ilen -= fill;
        left = 0;
    }
    ...
    while (ilen >= 64) {
        ali_algo_md5_process(ctx, input);
        input += 64;
        ilen -= 64;
    }
...
void ali_algo_md5_finish(ali_algo_md5_context *ctx, unsigned char output[16])
{
    ...
    ali_algo_md5_update(ctx, md5_padding, padn);
    ali_algo_md5_update(ctx, msglen, 8);
 ...
 */
void ali_algo_md5(const unsigned char *input, size_t ilen,
                 unsigned char output[16])
{
    ali_algo_md5_context ctx;

    ali_algo_md5_init(&ctx);
    ali_algo_md5_starts(&ctx);
    ali_algo_md5_update(&ctx, input, ilen);
    ali_algo_md5_finish(&ctx, output);
    ali_algo_md5_free(&ctx);
}
