...
#if defined(ALI_ALGO_PADLOCK_C) && \
  (defined(ALI_ALGO_HAVE_X86) || defined(ALI_ALGO_PADLOCK_ALIGN16))
static int aes_padlock_ace = -1;
#endif

#if defined(ALI_ALGO_AES_ROM_TABLES)
/*
 * Forward S-box
 */
...
#endif /* ALI_ALGO_AES_ROM_TABLES */

void ali_algo_aes_init(ali_algo_aes_context *ctx)
{
    osa_memset(ctx, 0, sizeof(ali_algo_aes_context));
}
...
void ali_algo_aes_free(ali_algo_aes_context *ctx)
{
    ...
    ali_algo_zeroize(ctx, sizeof(ali_algo_aes_context));
}

/*
 * AES key schedule (encryption)
 */
#if !defined(ALI_ALGO_AES_SETKEY_ENC_ALT)
                           ...
                           unsigned int keybits)
{
    ...
    switch (keybits) {
        ...
        case 256:
            ...
            break;
        default:
            return (ALI_ALGO_ERR_AES_INVALID_KEY_LENGTH);
    }

#if defined(ALI_ALGO_PADLOCK_C) && defined(ALI_ALGO_PADLOCK_ALIGN16)
    if (aes_padlock_ace == -1)
        aes_padlock_ace = ali_algo_padlock_has_support(ALI_ALGO_PADLOCK_ACE);
...
#if defined(ALI_ALGO_AESNI_C) && defined(ALI_ALGO_HAVE_X86_64)
    if (ali_algo_aesni_has_support(ALI_ALGO_AESNI_AES))
        return (
          ali_algo_aesni_setkey_enc((unsigned char *)ctx->rk, key, keybits));
#endif
...
#endif /* !ALI_ALGO_AES_SETKEY_ENC_ALT */

/*
 * AES key schedule (decryption)
 */
#if !defined(ALI_ALGO_AES_SETKEY_DEC_ALT)
int ali_algo_aes_setkey_dec(ali_algo_aes_context *ctx, const unsigned char *key,
                           unsigned int keybits)
{
    int                 i, j, ret;
    ali_algo_aes_context cty;
    uint32_t *          RK;
    uint32_t *          SK;

    ali_algo_aes_init(&cty);
    ...
    /* Also checks keybits */
    if ((ret = ali_algo_aes_setkey_enc(&cty, key, keybits)) != 0)
        goto exit;
...
#if defined(ALI_ALGO_AESNI_C) && defined(ALI_ALGO_HAVE_X86_64)
    if (ali_algo_aesni_has_support(ALI_ALGO_AESNI_AES)) {
        ali_algo_aesni_inverse_key((unsigned char *)ctx->rk,
...
exit:
    ali_algo_aes_free(&cty);

    return (ret);
}
#endif /* !ALI_ALGO_AES_SETKEY_DEC_ALT */
 ...
 */
#if !defined(ALI_ALGO_AES_ENCRYPT_ALT)
...
#endif /* !ALI_ALGO_AES_ENCRYPT_ALT */

/*
 * AES-ECB block decryption
 */
#if !defined(ALI_ALGO_AES_DECRYPT_ALT)
...
#endif /* !ALI_ALGO_AES_DECRYPT_ALT */

/*
 * AES-ECB block encryption/decryption
 */
int ali_algo_aes_crypt_ecb(ali_algo_aes_context *ctx, int mode,
                          const unsigned char input[16],
                          unsigned char       output[16])
{
#if defined(ALI_ALGO_AESNI_C) && defined(ALI_ALGO_HAVE_X86_64)
    if (ali_algo_aesni_has_support(ALI_ALGO_AESNI_AES))
        return (ali_algo_aesni_crypt_ecb(ctx, mode, input, output));
#endif
    ...
    if (mode == ALI_ALGO_AES_ENCRYPT)
        ali_algo_aes_encrypt(ctx, input, output);
    else
        ali_algo_aes_decrypt(ctx, input, output);
 ...
 */
int ali_algo_aes_crypt_cbc(ali_algo_aes_context *ctx, int mode, size_t length,
                          ...
                          unsigned char *output)
{
    ...
    if (length % 16)
        return (ALI_ALGO_ERR_AES_INVALID_INPUT_LENGTH);

#if defined(ALI_ALGO_PADLOCK_C) && defined(ALI_ALGO_HAVE_X86)
    if (aes_padlock_ace) {
        if (ali_algo_padlock_xcryptcbc(ctx, mode, length, iv, input, output) ==
            0)
            return (0);
    ...
    if (mode == ALI_ALGO_AES_DECRYPT) {
        while (length > 0) {
            osa_memcpy(temp, input, 16);
            ali_algo_aes_crypt_ecb(ctx, mode, input, output);

            for (i = 0; i < 16; i++)
                output[i] = (unsigned char)(output[i] ^ iv[i]);

            osa_memcpy(iv, temp, 16);
        ...
        while (length > 0) {
            ...
            ali_algo_aes_crypt_ecb(ctx, mode, output, output);
 ...
 */
int ali_algo_aes_crypt_cfb128(ali_algo_aes_context *ctx, int mode, size_t length,
                             ...
                             const unsigned char *input, unsigned char *output)
{
    ...
    if (mode == ALI_ALGO_AES_DECRYPT) {
        while (length--) {
            if (n == 0)
                ali_algo_aes_crypt_ecb(ctx, ALI_ALGO_AES_ENCRYPT, iv, iv);
 ...
 */
int ali_algo_aes_crypt_cfb8(ali_algo_aes_context *ctx, int mode, size_t length,
                           unsigned char iv[16], const unsigned char *input,
                           unsigned char *output)
{
    unsigned char c;
 ...
 */
int ali_algo_aes_crypt_ctr(ali_algo_aes_context *ctx, size_t length,
                          ...
                          const unsigned char *input, unsigned char *output)
{
    ...
    while (length--) {
        if (n == 0) {
            ali_algo_aes_crypt_ecb(ctx, ALI_ALGO_AES_ENCRYPT, nonce_counter,
                                  stream_block);
...
int ali_algo_aes_self_test(int verbose)
{
...
#if defined(ALI_ALGO_CIPHER_MODE_CBC) || defined(ALI_ALGO_CIPHER_MODE_CFB)
    unsigned char iv[16];
#endif
#if defined(ALI_ALGO_CIPHER_MODE_CBC)
    unsigned char prv[16];
#endif
#if defined(ALI_ALGO_CIPHER_MODE_CTR) || defined(ALI_ALGO_CIPHER_MODE_CFB)
    size_t offset;
#endif
#if defined(ALI_ALGO_CIPHER_MODE_CTR)
    int           len;
    ...
    for (i = 0; i < 6; i++) {
        ...
        if (v == ALI_ALGO_AES_DECRYPT) {
            ali_algo_aes_setkey_dec(&ctx, key, 128 + u * 64);

            for (j = 0; j < 10000; j++)
                ali_algo_aes_crypt_ecb(&ctx, v, buf, buf);
            ...
            ali_algo_aes_setkey_enc(&ctx, key, 128 + u * 64);
    ...
    for (i = 0; i < 6; i++) {
        ...
        if (v == ALI_ALGO_AES_DECRYPT) {
            ...
            for (j = 0; j < 10000; j++)
                ali_algo_aes_crypt_cbc(&ctx, v, 16, iv, buf, buf);
    ...
    for (i = 0; i < 6; i++) {
        ...
        if (v == ALI_ALGO_AES_DECRYPT) {
            ...
            ali_algo_aes_crypt_cfb128(&ctx, v, 64, &offset, iv, buf, buf);
    ...
    for (i = 0; i < 6; i++) {
        ...
        offset = 0;
        ali_algo_aes_setkey_enc(&ctx, key, 128);

        if (v == ALI_ALGO_AES_DECRYPT) {
            len = aes_test_ctr_len[u];
            memcpy(buf, aes_test_ctr_ct[u], len);

            ali_algo_aes_crypt_ctr(&ctx, len, &offset, nonce_counter,
