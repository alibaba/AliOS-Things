...
ali_crypto_result ali_rsa_get_keypair_size(size_t keybits, size_t *size)
{
    ...
    rsa_impl_t *p_impl = NULL;

    if (size == NULL) {
        return ALI_CRYPTO_INVALID_ARG;
    }
    
    p_impl = ali_crypto_rsa_get_impl();
    if(NULL == p_impl) {
        PRINT_RET(ALI_CRYPTO_ERROR,  "invalid impl\n");
    }
	if(NULL == p_impl->ops.rsa_get_keypair_size) {
        PRINT_RET(ALI_CRYPTO_ERROR,  "invalid aes ops\n");
    }

    result = p_impl->ops.rsa_get_keypair_size(keybits, size);
    return result;
}
...
ali_crypto_result ali_rsa_get_pubkey_size(size_t keybits, size_t *size)
{
    ...
    if(NULL == p_impl->ops.rsa_get_pubkey_size) {
        PRINT_RET(ALI_CRYPTO_ERROR,  "invalid aes ops\n");
    }
	
    result = p_impl->ops.rsa_get_pubkey_size(keybits, size);
    return result;
      ...
      size_t qp_size, rsa_keypair_t *keypair)
{
    ...
    if(NULL == p_impl->ops.rsa_init_keypair) {
        PRINT_RET(ALI_CRYPTO_ERROR,  "invalid aes ops\n");
    }
    result = p_impl->ops.rsa_init_keypair(
                                      keybits,  n, n_size, e,
                                      e_size, d, d_size, p,
                                      p_size, q, q_size, dp,
                                      ...
                                      size_t e_size, rsa_pubkey_t *pubkey)
{
    ...
    if(NULL == p_impl->ops.rsa_init_pubkey) {
        PRINT_RET(ALI_CRYPTO_ERROR,  "invalid aes ops\n");
    }

    result = p_impl->ops.rsa_init_pubkey(keybits, n, n_size, e, e_size, pubkey);
    return result;
}
                                       ...
                                       size_t *size)
{
    ...
    if(NULL == p_impl->ops.rsa_get_key_attr) {
        PRINT_RET(ALI_CRYPTO_ERROR,  "invalid aes ops\n");
    }

    result = p_impl->ops.rsa_get_key_attr(attr, keypair, buffer, size);
    return result;
}
                                            ...
                                            size_t e_size, rsa_keypair_t *keypair)
{
    ...
    if(NULL == p_impl->ops.rsa_gen_keypair) {
        PRINT_RET(ALI_CRYPTO_ERROR,  "invalid aes ops\n");
    }

    result = p_impl->ops.rsa_gen_keypair(keybits, e, e_size, keypair);
    return result;
}
                                       ...
                                       rsa_padding_t padding)
{
    ...
    if(NULL == p_impl->ops.rsa_public_encrypt) {
        PRINT_RET(ALI_CRYPTO_ERROR,  "invalid aes ops\n");
    }

    result = p_impl->ops.rsa_public_encrypt(pub_key, src, src_size,
                                          ...
                                          rsa_padding_t padding)
{
    ...
    if(NULL == p_impl->ops.rsa_private_decrypt) {
        PRINT_RET(ALI_CRYPTO_ERROR,  "invalid aes ops\n");
    }

    result = p_impl->ops.rsa_private_decrypt(priv_key, src, src_size, dst, dst_size, padding);
    return result;
}
                               ...
                               rsa_padding_t padding)
{
    ...
    if(NULL == p_impl->ops.rsa_sign) {
        PRINT_RET(ALI_CRYPTO_ERROR,  "invalid aes ops\n");
    }

    result = p_impl->ops.rsa_sign(priv_key, dig, dig_size, 
                                 ...
                                 rsa_padding_t padding, bool *p_result)
{
    ...
    if(NULL == p_impl->ops.rsa_verify) {
        PRINT_RET(ALI_CRYPTO_ERROR,  "invalid aes ops\n");
    }
    
    result = p_impl->ops.rsa_verify(pub_key, dig, dig_size, sig, 
