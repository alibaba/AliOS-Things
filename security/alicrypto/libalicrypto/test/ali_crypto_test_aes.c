...
static int _aes_ecb_pkcs_encrypt_decrypt(uint32_t idx, bool is_enc,
                                         size_t key_len, uint8_t *src,
                                         size_t in_len, uint8_t *dst,
                                         size_t *dst_len, void *aes_ctx)
{
    ali_crypto_result result;
    size_t            dst_size;
    ...
    if ((idx & 0x1) != 0x0) {
...
#if EN_GCOV_COVERAGE
        /* for gcov coverage */
        result = ali_aes_finish(src + 48, in_len - 48, dst + 48, &dst_size,
                                SYM_PKCS5_PAD, NULL);
        if (result == ALI_CRYPTO_SUCCESS) {
            return -1;
        }
        ...
        *dst_len = 48 + dst_size;
        ...
        dst_size = *dst_len;
        result =
          ali_aes_finish(src, in_len, dst, &dst_size, SYM_PKCS5_PAD, aes_ctx);
...
static int _aes_cfb_encrypt_decrypt(uint32_t idx, aes_type_t type, bool is_enc,
                                    size_t key_len, uint8_t *src, uint8_t *dst,
                                    void *aes_ctx)
{
    ali_crypto_result result;
    size_t            dst_size;
    ...
    /* for gcov coverage */
    result =
      ali_aes_init(type, is_enc, _g_aes_key, NULL, key_len, NULL, aes_ctx);
    if (result == ALI_CRYPTO_SUCCESS) {
        return -1;
    }
#endif
    ...
    result =
      ali_aes_init(type, is_enc, _g_aes_key, NULL, key_len, _g_aes_iv, aes_ctx);
    if (result != ALI_CRYPTO_SUCCESS) {
    ...
    if ((idx & 0x1) != 0x0) {
        ...
        result   = ali_aes_finish(src, TEST_CTR_SIZE, dst, &dst_size, SYM_NOPAD,
                                aes_ctx);
        if (result != ALI_CRYPTO_SUCCESS || dst_size != TEST_CTR_SIZE) {
            PRINT_RET(-1, "aes_cfb finish fail(%08x)", result);
        }
...
static int _ali_crypto_aes_ecb_test(void)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        ret     = _aes_ecb_pkcs_encrypt_decrypt(i, true, key_len, _g_test_data,
                                            TEST_ECB_SIZE, ciphertext, &out_len,
                                            aes_ctx);
        if (ret < 0) {
            GO_RET(-1, "ecb encrypt fail!\n");
        }
        ...
        ret     = _aes_ecb_pkcs_encrypt_decrypt(
          i, false, key_len, ciphertext, out_len, plaintext, &dec_len, aes_ctx);
        if (ret < 0) {
            GO_RET(-1, "ecb decrypt fail!\n");
        }
        ...
        ret     = _aes_ecb_pkcs_encrypt_decrypt(i, true, key_len, _g_test_data,
                                            TEST_ECB_SIZE_NOALIGN, ciphertext,
                                            &out_len, aes_ctx);
        if (ret < 0) {
            GO_RET(-1, "ecb encrypt fail!\n");
        }
...
static int _ali_crypto_aes_cbc_test(void)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        ret     = _aes_cbc_pkcs_encrypt_decrypt(i, true, key_len, _g_test_data,
                                            TEST_CBC_SIZE, ciphertext, &out_len,
                                            aes_ctx);
        if (ret < 0) {
            GO_RET(-1, "cbc encrypt fail!\n");
        }
        ...
        ret     = _aes_cbc_pkcs_encrypt_decrypt(i, true, key_len, _g_test_data,
                                            TEST_CBC_SIZE_NOALIGN, ciphertext,
                                            &out_len, aes_ctx);
        if (ret < 0) {
            GO_RET(-1, "cbc encrypt fail!\n");
        }
...
static int _ali_crypto_aes_cfb_test(void)
{
    ...
    for (type = AES_CFB8; type <= AES_CFB128; type++) {
        for (i = 0; i < 3; i++) {
            ...
            /* encrypt */
            CRYPT_MEMSET(aes_ctx, 0, aes_ctx_size);
            ret = _aes_cfb_encrypt_decrypt(i, type, true, key_len, _g_test_data,
                                           ciphertext, aes_ctx);
            if (ret < 0) {
                GO_RET(-1, "ctr encrypt fail!\n");
            }
            ...
            /* decrypt */
            CRYPT_MEMSET(aes_ctx, 0, aes_ctx_size);
            ret = _aes_cfb_encrypt_decrypt(i, type, false, key_len, ciphertext,
                                           plaintext, aes_ctx);
            if (ret < 0) {
                GO_RET(-1, "ctr decrypt fail!\n");
            }
