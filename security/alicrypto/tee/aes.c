                               ...
                               void *context)
{
    ...
    tee_aes_init(aes_ctx->ctx);
    aes_ctx->is_enc = is_enc;

    OSA_memcpy(aes_ctx->key, key1, keybytes);
    aes_ctx->keylen = keybytes;
                                  ...
                                  void *context)
{
    ...
    switch (aes_ctx->type) {
        ...
        case AES_ECB: {
            ...
            while (cur_len < size) {
                if (aes_ctx->is_enc) {
                    ret = tee_aes_encrypt_ecb(src + cur_len, AES_BLOCK_SIZE,
                                                  aes_ctx->key, aes_ctx->keylen, aes_ctx,
                                                  dst + cur_len);
                } else {
                    ret = tee_aes_decrypt_ecb(src + cur_len, AES_BLOCK_SIZE,
                                                  aes_ctx->key, aes_ctx->keylen, aes_ctx,
                                                  dst + cur_len);
                }
        ...
        case AES_CBC: {
            ...
            if (aes_ctx->is_enc) {
                ret = tee_aes_encrypt_cbc(src, size, aes_ctx->key, aes_ctx->keylen,
                                              aes_ctx, dst);
            } else {
                ret = tee_aes_decrypt_cbc(src, size, aes_ctx->key, aes_ctx->keylen,
                                              aes_ctx, dst);
            }
                                 ...
                                 sym_padding_t padding, void *context)
{
    ...
    // TODO padding should be passed to TEE
    (void)padding;
    ...
    switch (aes_ctx->type) {
        case AES_ECB: {
            if (aes_ctx->is_enc) {
                ret = tee_aes_encrypt_ecb(src, src_size, aes_ctx->key,
                                              aes_ctx->keylen, aes_ctx, dst);
            } else {
                ret = tee_aes_decrypt_ecb(src, src_size, aes_ctx->key,
                                              aes_ctx->keylen, aes_ctx, dst);
            }
            break;
        }
        case AES_CBC: {
            if (aes_ctx->is_enc) {
                ret = tee_aes_encrypt_cbc(src, src_size, aes_ctx->key,
                                              aes_ctx->keylen, aes_ctx, dst);
            } else {
                ret = tee_aes_decrypt_cbc(src, src_size, aes_ctx->key,
                                              aes_ctx->keylen, aes_ctx, dst);
            }
            break;
        }
    ...
    if (ret != ALI_CRYPTO_SUCCESS) {
        tee_aes_free(aes_ctx->ctx);
        PRINT_RET(ret, "aes type(%d) final fail(%08x)\n",
                  aes_ctx->type, ret);
    }
...
ali_crypto_result ali_aes_reset(void *context)
{
    ...
    OSA_memset(aes_ctx, 0, sizeof(tee_aes_ctx_t));
    return ALI_CRYPTO_SUCCESS;
}

ali_crypto_result ali_aes_copy_context(void *dst_ctx, void *src_ctx)
{
    tee_aes_ctx_t *aes_ctx_src, *aes_ctx_dst;
    ...
    aes_ctx_src = (tee_aes_ctx_t *)src_ctx;
    ...
    aes_ctx_dst = (tee_aes_ctx_t *)dst_ctx;
    ...
    OSA_memcpy(aes_ctx_dst, aes_ctx_src, sizeof(tee_aes_ctx_t));
    return ALI_CRYPTO_SUCCESS;
}
