                                            ...
                                            aes_ctx_t *   ctx)
{
    ...
    if (ALI_ALGO_AES_ENCRYPT == mode) {
        ...
        while (round < (src_size / AES_BLOCK_SIZE)) {
            ret = ali_algo_aes_crypt_ecb(&(ctx->ctx), mode,
        ...
        if (padding == SYM_NOPAD) {
            ...
            *dst_size = cur_len;
            ...
            ret =
              ali_algo_aes_crypt_ecb(&(ctx->ctx), mode, block, dst + cur_len);
    ...
    if (padding == SYM_NOPAD) {
        ...
        if (ctx->is_enc) {
            ...
            if ((src_size - AES_BLOCK_SIZE) > *dst_size) {
                ...
                ret =
                  ali_algo_aes_crypt_cbc(&(ctx->ctx), mode, src_size,
                                        (unsigned char *)ctx->iv, src, tmp_dst);
                if (0 != ret) {
                    osa_free(tmp_dst);
                    PRINT_RET(ALI_CRYPTO_ERROR,
                              "cbc_final: ali_algo_aes_crypt_cbc fail(%d)\n",
                              ret);
                }
