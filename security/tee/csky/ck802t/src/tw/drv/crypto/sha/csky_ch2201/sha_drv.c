...
static uint8_t sha1_hash[20]   = { 0xda, 0x39, 0xa3, 0xee, 0x5e, 0x6b, 0x4b,
                                 0x0d, 0x32, 0x55, 0xbf, 0xef, 0x95, 0x60,
                                 0x18, 0x90, 0xaf, 0xd8, 0x07, 0x09 };
static uint8_t sha224_hash[28] = { 0xd1, 0x4a, 0x02, 0x8c, 0x2a, 0x3a, 0x2b,
                                   0xc9, 0x47, 0x61, 0x02, 0xbb, 0x28, 0x82,
                                   0x34, 0xc4, 0x15, 0xa2, 0xb0, 0x1f, 0x82,
                                   0x8e, 0xa6, 0x2a, 0xc5, 0xb3, 0xe4, 0x2f };
static uint8_t sha256_hash[32] = { 0xe3, 0xb0, 0xc4, 0x42, 0x98, 0xfc, 0x1c,
...
static void _print_msg_data(uint8_t *data, uint32_t len)
{
    ...
    for (i = 0; i < len; i += 8) {
        CRYPTO_DRV_DBG_I(
          "msg: 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x\n",
                              ...
                              uint32_t total_size, sha_mode_t mode)
{
    ...
    if (mode == SHA_MODE_SHA1 || mode == SHA_MODE_SHA224 ||
...
static int32_t _cal_sha_without_count(uint32_t mode, uint32_t msg,
                                      uint32_t msg_len, uint32_t hash)
{
    uint32_t            times    = 0;
    uint32_t            is_done  = 0;
    volatile sha_reg_t *sha_regs = (sha_reg_t *)PLATFORM_SHA_BADDR;
...
static int32_t _cal_sha_enable_initial(uint32_t mode, uint32_t msg,
                                       uint32_t msg_len, uint32_t hash)
{
    uint32_t            times    = 0;
    uint32_t            is_done  = 0;
...
int32_t sha_drv_process(void *arg)
{
    ...
    if (proc_arg->mode >= SHA_MODE_MAX || proc_arg->mode <= SHA_MODE_INVALID) {
        CRYPTO_DRV_DBG_E("sha_drv_process: bad mode(%d)\n", proc_arg->mode);
        return -1;
    }

    if (proc_arg->mode == SHA_MODE_SHA1 || proc_arg->mode == SHA_MODE_SHA256 ||
    ...
    if (sha_ctx->data_size + src_size <= block_size) {
        if (src_size) {
            memcpy(sha_ctx->data + sha_ctx->data_size, src_data, src_size);
    ...
    if (sha_ctx->data_size < block_size) {
        memcpy(sha_ctx->data + sha_ctx->data_size, src_data,
               block_size - sha_ctx->data_size);
        src_data += block_size - sha_ctx->data_size;
        src_size -= block_size - sha_ctx->data_size;
    }

    if (sha_ctx->state == SHA_DRV_INITED) {
        ret = _cal_sha_without_count(proc_arg->mode, (uint32_t)sha_ctx->data,
                                     block_size, (uint32_t)hash);
        if (ret < 0) {
            CRYPTO_DRV_DBG_E(
              "sha_drv_process: calc hash without count fail!\n");
            return -1;
        }
        sha_ctx->state = SHA_DRV_STARTED;
    } else {
        ret = _cal_sha_without_count_enable_initial(
          proc_arg->mode, (uint32_t)sha_ctx->data, block_size, (uint32_t)hash);
        if (ret < 0) {
            CRYPTO_DRV_DBG_E(
              "sha_drv_process: calc first block hash with continue fail!\n");
            return -1;
        }
    }
    sha_ctx->data_size = 0;
    ...
    while (src_size > block_size) {
        ...
        if (ret < 0) {
            CRYPTO_DRV_DBG_E(
              "sha_drv_process: calc hash with continue fail!\n");
            return -1;
        }
...
int32_t sha_drv_finish(void *arg)
{
    ...
    switch (final_arg->mode) {
        case SHA_MODE_SHA1: {
            ...
            if (total_size == 0) {
                ...
                memcpy((uint8_t *)final_arg->dst_addr, sha1_hash, hash_size);
                return 0;
            }
            break;
        }
        case SHA_MODE_SHA256: {
            ...
            if (total_size == 0) {
                TEE_ASSERT(sha_ctx->state == SHA_DRV_INITED);
                memcpy((uint8_t *)final_arg->dst_addr, sha256_hash, hash_size);
                return 0;
            }
            break;
        }
        case SHA_MODE_SHA224: {
            ...
            if (total_size == 0) {
                TEE_ASSERT(sha_ctx->state == SHA_DRV_INITED);
                memcpy((uint8_t *)final_arg->dst_addr, sha224_hash, hash_size);
                return 0;
            }
            break;
        }
        case SHA_MODE_SHA512: {
            ...
            if (total_size == 0) {
                TEE_ASSERT(sha_ctx->state == SHA_DRV_INITED);
                memcpy((uint8_t *)final_arg->dst_addr, sha512_hash, hash_size);
                return 0;
            }
            break;
        }
        case SHA_MODE_SHA384: {
            ...
            if (total_size == 0) {
                TEE_ASSERT(sha_ctx->state == SHA_DRV_INITED);
                memcpy((uint8_t *)final_arg->dst_addr, sha384_hash, hash_size);
                return 0;
            }
            break;
        }
    ...
    ret = _cal_sha_enable_initial(final_arg->mode, (uint32_t)msg_buf, msg_size,
                                  (uint32_t)hash);
    if (ret < 0) {
