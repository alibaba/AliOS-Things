...
void eflash_test(void)
{
    ...
    *((uint32_t *)write_args.buf + 1) = 0xccddeeff;
    tee_dbg_print(
      INF, "test write pos 0x%08x buffer 0x%08x,data 0x%08x 0x%08x\n",
      write_args.addr, write_args.buf, *((uint32_t *)write_args.buf),
      *((uint32_t *)write_args.buf + 1));
    ...
    tee_dbg_print(INF,
                  "test read eflash pos 0x%08x bufer 0x%08x "
                  "data 0x%08x 0x%08x 0x%08x 0x%08x\n",
                  read_args.addr, read_args.buf, *((uint32_t *)read_args.buf),
                  *((uint32_t *)read_args.buf + 1),
                  *((uint32_t *)read_args.buf + 2),
                  *((uint32_t *)read_args.buf + 3));
    free(read_args.buf);
    ...
    *((uint32_t *)write_args.buf) = 0x11223344;
    ...
    tee_dbg_print(INF,
                  "1111test read eflash pos 0x%08x bufer 0x%08x "
                  "data 0x%08x 0x%08x 0x%08x 0x%08x\n",
                  read_args.addr, read_args.buf, *((uint32_t *)read_args.buf),
                  *((uint32_t *)read_args.buf + 1),
                  *((uint32_t *)read_args.buf + 2),
