...
typedef enum _tee_crypto_result
{
    ...
    TEE_CRYPTO_INVALID_KEY,            /* 2:  Invalid Key in asymmetric scheme:
                                          RSA/DSA/ECCP/DH etc */
    TEE_CRYPTO_INVALID_TYPE,           /* 3:  Invalid
                                          aes_type/des_type/authenc_type/hash_type/cbcmac_type/cmac_type
                                        */
    TEE_CRYPTO_INVALID_CONTEXT,        /* 4:  Invalid context in multi-thread
                                          cipher/authenc/mac/hash etc */
    TEE_CRYPTO_INVALID_PADDING,        /* 5:  Invalid
                                          sym_padding/rsassa_padding/rsaes_padding */
    TEE_CRYPTO_INVALID_AUTHENTICATION, /* 6:  Invalid authentication in
    ...
    TEE_CRYPTO_BUFFER_OVERFLOW, /* b:  Output buffer is too short to store
                                   result */
    TEE_CRYPTO_NULL,            /* c:  NULL pointer in arguments */
    TEE_CRYPTO_ERR_STATE,       /* d:  Bad state in mulit-thread
                                   cipher/authenc/mac/hash etc */
 ...
 * context, which size is got through tee_aes_get_ctx_size
...
/* AES fips */
tee_crypto_result tee_aes_cipher_id2_fips(aes_type_t type, bool is_enc,
                                          const uint8_t *iv, uint8_t *src,
                                          uint32_t src_len, uint8_t *dest,
                                          uint32_t *dest_len);
...
'tee_des_get_ctx_size' as internal context. function will check the [status[ of
...
'tee_des_get_ctx_size' as internal context. function will check it is a valid
...
context:    function will use size which return from function
'tee_authenc_get_ctx_size' as internal context. function will check the [status]
...
function MUST equal to the 'aad_len' parameter in tee_authenc_init. context:
function will use size which return from function 'tee_authenc_get_ctx_size' as
...
*/
tee_crypto_result tee_authenc_update_aad(const uint8_t *aad, size_t aad_size,
                                         void *context);
/*
...
MUST equal to the 'payload_len' parameter in tee_authenc_init. context: function
will use size which return from function 'tee_authenc_get_ctx_size' as internal
...
return from function 'tee_authenc_get_ctx_size' as internal context. function
...
'tee_authenc_get_ctx_size' as internal context. function will check it is a
...
tee_crypto_result tee_hash_update(const uint8_t *src, size_t size,
                                  void *context);
...
tee_crypto_result tee_hash_digest(hash_type_t type, const uint8_t *src,
...
tee_crypto_result tee_hmac_init(hash_type_t type, const uint8_t *key,
                                size_t keybytes, void *context);
tee_crypto_result tee_hmac_update(const uint8_t *src, size_t size,
                                  void *context);
...
tee_crypto_result tee_hmac_digest(hash_type_t type, const uint8_t *key,
                                  size_t keybytes, const uint8_t *src,
                                  size_t size, uint8_t *dgst);

/* cbcmac */
tee_crypto_result tee_cbcmac_get_ctx_size(cbcmac_type_t type, size_t *size);
tee_crypto_result tee_cbcmac_init(cbcmac_type_t type, const uint8_t *key,
                                  size_t keybytes, void *context);
tee_crypto_result tee_cbcmac_update(const uint8_t *src, size_t size,
                                    void *context);
tee_crypto_result tee_cbcmac_final(sym_padding_t padding, uint8_t *dgst,
                                   void *context);
tee_crypto_result tee_cbcmac_reset(void *context);
tee_crypto_result tee_cbcmac_copy_context(void *dst_ctx, void *src_ctx);
tee_crypto_result tee_cbcmac_digest(cbcmac_type_t type, const uint8_t *src,
                                    size_t size, sym_padding_t padding,
                                    uint8_t *dgst);

/* cmac */
tee_crypto_result tee_cmac_get_ctx_size(cmac_type_t type, size_t *size);
tee_crypto_result tee_cmac_init(cmac_type_t type, const uint8_t *key,
                                size_t keybytes, void *context);
tee_crypto_result tee_cmac_update(const uint8_t *src, size_t size,
                                  void *context);
tee_crypto_result tee_cmac_final(sym_padding_t padding, uint8_t *dgst,
                                 void *context);
tee_crypto_result tee_cmac_reset(void *context);
tee_crypto_result tee_cmac_copy_context(void *dst_ctx, void *src_ctx);
tee_crypto_result tee_cmac_digest(cmac_type_t type, const uint8_t *src,
                                  size_t size, sym_padding_t padding,
                                  uint8_t *dgst);
 ...
 */
tee_crypto_result tee_rsa_init_keypair(
 ...
 * key
 */
tee_crypto_result tee_rsa_init_pubkey(size_t keybits, const uint8_t *n,
 ...
 */
tee_crypto_result tee_rsa_gen_keypair(size_t keybits, const uint8_t *e,
...
tee_crypto_result tee_rsa_verify_id2_fips(const uint8_t *dig, size_t dig_size,
                                          const uint8_t *sig, size_t sig_size,
                                          rsa_padding_t padding, bool *result);
tee_crypto_result tee_rsa_public_encrypt_id2_fips(const uint8_t *src,
                                                  size_t src_size, uint8_t *dst,
                                                  size_t *      dst_size,
                                                  rsa_padding_t padding);
tee_crypto_result tee_rsa_private_decrypt_id2_fips(const uint8_t *src,
                                                   size_t         src_size,
                                                   uint8_t *      dst,
                                                   size_t *       dst_size,
                                                   rsa_padding_t  padding);
...
tee_crypto_result tee_dsa_init_keypair(size_t keybits, const uint8_t *g,
...
tee_crypto_result tee_dsa_init_pubkey(size_t keybits, const uint8_t *g,
                                      size_t g_size, const uint8_t *p,
                                      size_t p_size, const uint8_t *q,
                                      size_t q_size, const uint8_t *y,
                                      size_t y_size, dsa_pubkey_t *pubkey);
tee_crypto_result tee_dsa_gen_keypair(size_t keybit, const uint8_t *g,
                                      size_t g_size, const uint8_t *p,
                                      size_t p_size, const uint8_t *q,
...
tee_crypto_result tee_dh_init_keypair(size_t keybits, const uint8_t *g,
                                      size_t g_size, const uint8_t *p,
...
tee_crypto_result tee_dh_init_pubkey(size_t keybits, const uint8_t *g,
                                     size_t g_size, const uint8_t *p,
                                     size_t p_size, const uint8_t *y,
                                     size_t y_size, dh_pubkey_t *pubkey);
tee_crypto_result tee_dh_gen_keypair(size_t keybit, const uint8_t *g,
                                     size_t g_size, const uint8_t *p,
                                     size_t p_size, const uint8_t *q,
                                     size_t q_size, size_t xbits,
                                     dh_keypair_t *keypair);
tee_crypto_result tee_dh_derive_secret(const dh_keypair_t *priv_key,
...
tee_crypto_result tee_ecc_init_keypair(const uint8_t *x, size_t x_size,
                                       const uint8_t *y, size_t y_size,
                                       const uint8_t *d, size_t d_size,
                                       size_t curve, ecc_keypair_t *keypair);
tee_crypto_result tee_ecc_init_pubkey(const uint8_t *x, size_t x_size,
                                      const uint8_t *y, size_t y_size,
                                      size_t curve, ecc_pubkey_t *pubkey);
tee_crypto_result tee_ecc_gen_keypair(size_t curve, ecc_keypair_t *keypair);
...
tee_crypto_result tee_ecdh_derive_secret(const ecc_keypair_t *priv_key,
