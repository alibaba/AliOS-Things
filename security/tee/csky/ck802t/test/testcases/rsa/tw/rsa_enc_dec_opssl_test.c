...
int32_t rsa_test_v2(void)
{
    ...
    res = tee_rsa_public_encrypt(pubkey, Digest_Padded, DIGEST_PADDED_SIZE, dst,
                                 (size_t *)&dst_size, padding);
    ...
    if (0 !=
        memcmp(Digest_Padded_public_encrypt_no_padding_out, dst, dst_size)) {
    ...
    res          = tee_rsa_private_decrypt(
      keypair, Digest_public_encrypt_pkcs1_padding_out,
      DIGEST_PUB_ENC_PKCS1_PADDING_OUT_SIZE, dst, (size_t *)&dst_size, padding);
    RSA_TEST_ASSERT(TEE_CRYPTO_SUCCESS == res);
    ...
    padding.type = RSAES_PKCS1_V1_5;
    res          = tee_rsa_public_encrypt(pubkey, Digest, DIGEST_SIZE, dst,
                                 (size_t *)&dst_size, padding);
    RSA_TEST_ASSERT(TEE_CRYPTO_SUCCESS == res);
