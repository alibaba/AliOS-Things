...
uint32_t km_envelope_begin(void **ctx, const char *name,
                           ...
                           uint32_t *protected_key_len, km_purpose_type is_enc)
{
    return KM_ERR_NOT_SUPPORTED;
}
                   ...
                   uint8_t *dest, size_t *dest_len)
{
    ...
    if (key_type == KM_DES3) {
        ret = des3_ecb(s_id2_key, s_key_len, src, src_len, dest, &output_len,
                       is_enc);
    }
#endif
#if (ID2_CRYPTO_TYPE_CONFIG == ID2_CRYPTO_TYPE_AES)
    if (key_type == KM_AES) {
        ret = aes_ecb(s_id2_key, s_key_len, src, src_len, dest, &output_len,
                      is_enc);
    }
#endif
    else {
