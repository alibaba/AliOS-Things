...
#define AUTH_CODE_BUF_LEN 256
  ...
  /**
   * @brief initialize the id2 client resource, must be called first before
   * other ID2 client API.
   *
   * @return @see irot_result_t
   */
  irot_result_t
  id2_client_init(void);
 ...
 * @param[inout]   len input with the ID2 buffer size, ouput the real data
 * length.
 *
 * @return @see irot_result_t
 */
 ...
 * @param[in]  extra             extra data, optional data, no more than 512
 * bytes.
 * @param[in]  extra_len         length of extra data.
 * @param[out] auth_code         the auth code output buffer.
 * @param[inout] auth_code_len   input with the output buffer size, ouput the
 * real data length.
 *
 * @return @see irot_result_t
 */
 ...
 *
 * @param[in]  timestamp         the number of milliseconds since the Epoch,
 ...
 * @param[in]  in_len           lenth of the input data, which must <= 4096
 * bytes.
 * @param[out] out              output buffer for decrypt data.
 * @param[inout] out_len        input with the output buffer size, ouput the
 * real data length.
 *
 * @return @see irot_result_t
 */
 ...
 * @param[inout] device_random_len      input with the output buffer size, ouput
 ...
 * @param[in] device_random          device challenge, may be NULL if the
 * get_device_challenge has been called.
 * @param[in] device_random_len      the length of device challenge, must set to
 * 0 if device_random is null.
 ...
 * @param[out] auth_code         the auth code output buffer.
 * @param[inout] len   input with the output buffer size, ouput the real data
 * length.
 *
 * @return @see irot_result_t
 */
 ...
 *
 * for example: id2_client_set_id2_and_key("0102030405060708090A0B0C", 0x01,
