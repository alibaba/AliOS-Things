...
extern "C"
{
    ...
    /**
     * @brief allocates size bytes and returns a pointer to the allocated
     * memory.
     *
    ...
    /**
     * @brief free the memory space pointed to by ptr, which must have been
     * returned by a previous call to irot_pal_memory_malloc.
     *
     ...
     * @param out_len input with the output buffer size, ouput the real output
     * data size.
     *
     * @return @see irot_result_t
     */
    irot_result_t irot_pal_base64_encode(const uint8_t *in, uint32_t in_len,
     ...
     * @return @see irot_result_t
     */
    irot_result_t irot_pal_hash_sum(const uint8_t *in, uint32_t in_len,
                                    uint8_t *out, uint32_t *out_len,
                                    digest_t type);
