...
static void di_interrup_proc(void *arg)
{
    ...
    if (value == GPIO_PinState_Set) {
        ...
        //LOGI(TAG, "port %d recv a rising edge interrupt and enable the falling edge", diport);
        ret = hal_gpio_enable_irq(&gpio_dev_input[diport].gpio_dev, IRQ_TRIGGER_FALLING_EDGE, 
                            di_interrup_proc, &gpio_dev_input[diport].gpio_dev.port);
    }

    if (value == GPIO_PinState_Reset) {
        /*falling edge interrupt*/
        //LOGI(TAG, "port %d recv a falling edge interrupt and enable the rising edge", diport);
        ret = hal_gpio_enable_irq(&gpio_dev_input[diport].gpio_dev, IRQ_TRIGGER_RISING_EDGE, 
                            di_interrup_proc, &gpio_dev_input[diport].gpio_dev.port);
    }

    if (ret) {
        LOGE(TAG, "Fail to enable gpio interrupt , gpio value is %d, ret %d", value, ret);
    }
    return ;
}
...
static void di_value_check(void *timer, void *arg)
{
    ...
    for (i = 0; i < DI_PORT_SIZE; i++) {
        if (gpio_dev_input[i].installed == 0 || gpio_dev_input[i].monitor_flag == 0) {
            continue;
        }
        ret = hal_gpio_input_get(&gpio_dev_input[i].gpio_dev, &gpio_value);
        if (ret) {
            LOGE(TAG, "Fail to get di %d port %d value at %s %d", i, gpio_dev_input[i].gpio_dev.port, __FILE__, __LINE__);
            continue;
        }
        if (gpio_value == gpio_dev_input[i].exactly_level) {
            ...
            if (gpio_dev_input[i].check_count >= DI_STABLE_COUNT) {
                ...
                if (NULL != g_di_notify_cb) {
                    ...
                    if (ret) {
                        LOGE(TAG, "Fail to notify di %d value changes to %d", i, gpio_value);
                    }
                }
            }           
        }
...
int32_t expansion_board_di_init()
{
    ...
    for (i = 0; i < DI_PORT_SIZE; i++) {
        ...
        if (ret) {
            LOGE(TAG, "di %d pin %d init fail ret", i, gpio_dev_input[i].gpio_dev.port, ret);
            return -1;
        }

        ret = hal_gpio_input_get(&gpio_dev_input[i].gpio_dev, &gpio_value);
        if (ret) {
            LOGE(TAG, "di %d pin %d fail to get value, ret %d", i, gpio_dev_input[i].gpio_dev.port, ret);
            return -1;
        }
        ...
        if (gpio_value == GPIO_PinState_Set) {
            ret = hal_gpio_enable_irq(&gpio_dev_input[i].gpio_dev, IRQ_TRIGGER_FALLING_EDGE, 
                                di_interrup_proc, &gpio_dev_input[i].gpio_dev.port);
        } else {
            ret = hal_gpio_enable_irq(&gpio_dev_input[i].gpio_dev, IRQ_TRIGGER_RISING_EDGE, 
                                di_interrup_proc, &gpio_dev_input[i].gpio_dev.port);
        }

        if (ret) {
            LOGE(TAG, "di %d pin %d fail enable irq ret %d", i, gpio_dev_input[i].gpio_dev.port, ret);
            return -1;
        }
    ...
    ret = aos_timer_new_ext(&st_di_check_timer, di_value_check, NULL, TIMER_CHECK_INTERVAL, 1, 1);
    if (ret) {
...
void expansion_board_di_change_notify_register(digital_input_value_change_notify cb)
{
    if (NULL == cb) {
