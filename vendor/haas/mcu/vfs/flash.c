...
int bes_hal_flash_read(const uint32_t addr, uint8_t *dst, const uint32_t size)
{
    int ret = 0;
    uint32_t lock = 0;
#if 1
...
#else
    ret = hal_norflash_read(HAL_NORFLASH_ID_0, addr, dst, size);
#endif
...
int32_t hal_flash_erase(hal_partition_t in_partition, uint32_t off_set, uint32_t size)
{
    ...
    ret = bes_hal_flash_erase(start_addr, size);
                        ...
                        const void *in_buf, uint32_t in_buf_len)
{
    ...
    ret = bes_hal_flash_write(start_addr, in_buf, in_buf_len);
    if (!ret) {
        *off_set += in_buf_len;
    }
                              ...
                              const void *in_buf, uint32_t in_buf_len)
{
    ...
    ret = bes_hal_flash_erase(start_addr, in_buf_len);
    if (ret) {
                       ...
                       void *out_buf, uint32_t in_buf_len)
{
    ...
    ret = bes_hal_flash_read(start_addr, out_buf, in_buf_len);
