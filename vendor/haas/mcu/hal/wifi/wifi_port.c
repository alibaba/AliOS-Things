...
static void on_wifi_disconnect(WIFI_USER_EVT_ID event_id, void *arg)
{
    ...
    if (event_id == WIFI_USER_EVT_DISCONNECTED) {
        ...
        printf("wifi disconnected (reason=%d, locally_generated=%d)\n",
               (uint32_t)(reason & 0x7F), (uint32_t)((reason & BIT(7)) >> 7));
#if 0
        if (m->ev_cb && m->ev_cb->stat_chg) {
    ...
    if (event_id == WIFI_USER_EVT_FATAL_ERROR) {
        ...
        if (rst_flag == 1) {
            ...
            nbytes = snprintf(str_buf, sizeof(str_buf),
                     ...
                     error,
                     (uint32_t)dump_info.pac_rxc_rx_buf_in_ptr,
                     (uint32_t)dump_info.pac_rxc_rx_buf_out_ptr,
                     (uint32_t)dump_info.scheduler_events,
                     (uint32_t)dump_info.lmac_pc0,
                     (uint32_t)dump_info.lmac_pc1,
                     (uint32_t)dump_info.lmac_lr,
                     (uint32_t)dump_info.lmac_sp,
                     (uint32_t)dump_info.pac_ntd_status_peek,
                     (uint32_t)dump_info.pac_txc_status,
                     (uint32_t)dump_info.QUEUE_0_CONTROL,
                     (uint32_t)dump_info.QUEUE_1_CONTROL,
                     (uint32_t)dump_info.QUEUE_2_CONTROL,
                     (uint32_t)dump_info.QUEUE_3_CONTROL,
                     (uint32_t)dump_info.wlan_sw_override_1,
                     (uint32_t)dump_info.tsq_in_prog,
                     (uint32_t)dump_info.tsq_in_cmpl);
...
//#define __WIFI_HEAP_START 0x2002c030
//#define __WIFI_HEAP_LEN 0x70000
extern void factory_section_init();
extern void factory_section_open();
extern void vendor_usapce_cli_register_init();
...
#ifdef AOS_COMP_IRQ
    vendor_usapce_cli_register_init();

    //printf("%s %d heap:%p:0x%X\n", __FUNCTION__, __LINE__, __WIFI_HEAP_START, __WIFI_HEAP_LEN);
    //rt_system_heap_init(__WIFI_HEAP_START, __WIFI_HEAP_START+__WIFI_HEAP_LEN);
    ulog_init();
    //hal_gpdma_open();
    // factory
    ret = hal_norflash_aos_open(HAL_NORFLASH_ID_0);
    printf("hal_norflash_aos_open return %d\r\n", ret);
    //norflash_api_init();
                       ...
                       wifi_mode_t mode)
{
    ...
    if (mode == WIFI_MODE_STA&& wifi_status.sta_connected) {
        ...
        printf("set mask: %s\n", in_net_para->mask);
        printf("set gw: %s\n", in_net_para->gate);
        inet_aton(in_net_para->ip, (struct in_addr *)(&ip.ip));
        inet_aton(in_net_para->mask, (struct in_addr *)(&ip.netmask));
        inet_aton(in_net_para->gate, (struct in_addr *)(&ip.gw));
...
static int haas1000_wifi_sta_get_link_status(netdev_t *dev, wifi_ap_record_t *out_stat)
{
    ...
    if (wifi_status.sta_connected) {
        ...
        bwifi_get_current_ssid((char *)(out_stat->ssid));
...
static int haas1000_wifi_start_monitor(netdev_t *dev, wifi_promiscuous_cb_t cb)
{
    if (WARN_ON(!wifi_status.wifi_started ||
                wifi_status.sta_connected))
        return -1;
...
static int haas1000_wifi_register_monitor_cb(netdev_t *dev, monitor_data_cb_t fn)
{
    promisc_cb = fn;
    return 0;
}
...
static int haas1000_wifi_register_mgnt_monitor_cb(netdev_t *dev, monitor_data_cb_t fn)
{
    mgnt_frame_cb = fn;
    return 0;
}

static int haas1000_wifi_send_80211_raw_frame(netdev_t *dev, void *buf, uint16_t len)
{
    if (WARN_ON(!wifi_status.sta_connected ||
                wifi_status.resetting))
        return -1;

    return bes_sniffer_send_mgmt_frame(bwifi_get_current_channel(), (uint8_t *)buf, (size_t)len);
}
#ifdef __AP_MODE__
...
// if builld kspace, such as alios_kernel, only open AOS_COMP_NETMGR_KSPACE_RUNABLE runable prebuild in this section
// if build uspace, such as ulwip, only open AOS_COMP_NETMGR_USPACE_RUNABLE runable prebuild in this section
#if (((AOS_KERNEL_MODE == 1) && (AOS_COMP_NETMGR_KSPACE_RUNABLE == 1)) || \
    ((AOS_KERNEL_MODE == 0) && (AOS_COMP_NETMGR_USPACE_RUNABLE == 1)))
