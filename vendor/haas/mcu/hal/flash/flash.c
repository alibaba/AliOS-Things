...
extern int pmu_flash_write_config();
extern int pmu_flash_read_config();
...
int32_t hal_flash_info_get(hal_partition_t in_partition, hal_logic_partition_t *partition)
{
    ...
    if ((partition != NULL) && (in_partition < HAL_PARTITION_MAX)) {
        ENTER_FUNCTION();
...
int32_t hal_flash_mtdpart_info_get(hal_mtdpart_info_t **result_array, int *cnt)
{
    ...
    *cnt = 0;
    for (int i = 0; i < total; i++) {
        p = (hal_logic_partition_t *)(&hal_partitions[i]);
...
int hal_flash_init(hal_partition_t in_partition)
{
    int ret = 0;
