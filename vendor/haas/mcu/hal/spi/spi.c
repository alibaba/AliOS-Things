...
int32_t hal_spi_send(spi_dev_t *spi, const uint8_t *data, uint16_t size, uint32_t timeout)
{
	...
	if (spi->config.t_mode == SPI_TRANSFER_DMA)
	{
		...
		if (ret)
		{
			TRACE("%s %d spi_dma_send fail ret=%d", __FUNCTION__, __LINE__, ret);
			goto OUT;
		}
		ret = osSemaphoreWait(spi_ctx[spi->port].spi_dma_semaphore, timeout);
		if (ret <= 0)
		{
			TRACE("%s %d spi dma tail send timeout ret=%d", __FUNCTION__, __LINE__, ret);
			goto OUT;
		}
	}
	else
...
void _hal_spi_test(void)
{
	...
	for (size_t i = 0; i < 100; i++)
	{
        ...
        hal_gpio_pin_set_dir(HAL_IOMUX_PIN_P3_3, HAL_GPIO_DIR_OUT, 1);
        TRACE("%s-%d;%x,%x,%x,%x,%x,%x\r",__func__,data[0],data[1],data[2],data[3],data[4],data[5]);
        osDelay(5);
