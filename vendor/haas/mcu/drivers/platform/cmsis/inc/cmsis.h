...
void cpu_intrpt_restore(uint32_t);

__attribute__((weak)) uint32_t __int_lock(void) {
...
__attribute__((weak)) void __int_unlock(uint32_t flag){
    if ((flag & 0x1) == 0) {
        __enable_irq();
    }
}
...
__STATIC_FORCEINLINE uint32_t int_lock_global(void)
{
    ...
    return st;
#else
#if defined(AOS_COMPILE_APP) || defined(AOS_COMP_IRQ)
    return __int_lock();
#else
	uint32_t pri = __get_PRIMASK();
	if ((pri & 0x1) == 0) {
