...
FS_OTA_BIN_OUTPUT_FILE:=$($(HOST_MCU_FAMILY)_LOCATION)release/write_flash_gui/ota_bin/littlefs_ota.bin
AOS_ROOT_DIR:=$(shell pwd)
IMAGE_FS_MAGIC:=0xabcdabcd

gen_standard_images:
	$(info gen signature and release image ...)
	$(info OUTPUT_DIR is $(OUTPUT_DIR))
	$(info AOS_LITTLE_FS_VER is $(AOS_LITTLE_FS_VER))
	$(QUITE) $(ECHO) "$(AOS_LITTLE_FS_VER)" > $($(HOST_MCU_FAMILY)_LOCATION)/prebuild/data/fs_ver.txt
	...
	$(QUIET)$(CP) $(TARGET_FS_FILE) $(FS_OTA_BIN_OUTPUT_FILE)
	echo "gen ota image ..."
	$(XZ) -f --lzma2=dict=32KiB --check=crc32 -k $(OTA_BIN_OUTPUT_FILE)
	$(PYTHON) $(SCRIPTS_PATH)/ota_gen_md5_bin.py -i $(OTA_BIN_OUTPUT_FILE) -m $(IMAGE_MAGIC)
	$(PYTHON) $(SCRIPTS_PATH)/ota_gen_md5_bin.py -i $(OTA_BIN_OUTPUT_FILE).xz -m $(IMAGE_MAGIC)
	$(PYTHON) $(SCRIPTS_PATH)/ota_gen_md5_bin.py -i $(FS_OTA_BIN_OUTPUT_FILE) -m $(IMAGE_FS_MAGIC)
