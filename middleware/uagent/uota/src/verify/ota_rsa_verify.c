...
int ota_make_public_key(unsigned char* pub_key)
{
    ...
    pubkey_e_size = 3;
    ret = ali_rsa_init_pubkey(bitnumb, pubkey_n, pubkey_n_size, pubkey_e, pubkey_e_size, (rsa_pubkey_t *)pub_key);
...
int ota_make_rsa_verify_dig_data(unsigned char* src_data, int src_len, unsigned char* dig_data, unsigned char hash_type)
{
    ...
    ali_hash_get_ctx_size(hash_type, (size_t*)&ctx_size);
    ...
    if(ALI_CRYPTO_SUCCESS != ali_hash_init(hash_type, hash_ctx)) {
            OTA_LOG_E("ota sign init fail\n ");
            ret = -1;
            goto OTA_DIG_OVER;
    }
    if(ALI_CRYPTO_SUCCESS != ali_hash_update((const uint8_t *)src_data, src_len, hash_ctx)) {
        OTA_LOG_E("ota verify rsa hash update fail");
        ret = -1;
        goto OTA_DIG_OVER;
    }
    if(ALI_CRYPTO_SUCCESS != ali_hash_final(dig_data, hash_ctx)) {
...
int ota_verify_rsa_sign(unsigned char* src_value, int src_len, int rsabitnumb, unsigned char* signature_value)
{
    ...
    ret = ali_rsa_get_pubkey_size(rsabitnumb, (size_t *)&pub_key_len);
    if (ret != ALI_CRYPTO_SUCCESS) {
    ...
    ret = ali_rsa_verify((rsa_pubkey_t *)pubkey, dig_value, dig_value_len,
...
int ota_verify_download_rsa_sign(unsigned char* sign_dat, const char* src_hash_dat, hash_type_t src_hash_method)
{
    char tmp_buf[32] = {0};
    int sign_bitnumb = 0;
    int src_dat_len = 0;
