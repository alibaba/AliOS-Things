...
static int ota_init(void *something)
{
    ...
    {
        ota_CRC16_Init( &ctx );
        return hal_flash_erase(boot_part, 0 ,part_info->partition_length);
    } else
    {
...
static int ota_write(int* off, char* in_buf ,int in_buf_len)
{
    ...
    return hal_flash_write(boot_part, (uint32_t*)&offset, in_buf, in_buf_len);
}
	...
	ota_CRC16_Final( &ctx, &param->crc);
        if (param->upg_flag == OTA_DIFF) {
            ...
            if(memcmp(&ota_param, &ota_param_r, sizeof(PatchStatus)) != 0) {
                 OTA_LOG_E("OTA DIFF compare failed, dst:0x%08x src:0x%08x len:0x%08x crc:0x%04x\r\n",ota_param_r.dst_adr, ota_param_r.src_adr, ota_param_r.len, ota_param_r.crc);
                 return -1;
            }
            OTA_LOG_I("OTA Diff finish dst:0x%08x src:0x%08x len:0x%08x, crc:0x%04x pcrc:0x%04x splict:%d.\r\n",ota_param_r.dst_adr,ota_param_r.src_adr,ota_param_r.len,ota_param_r.crc,ota_param_r.patch_crc,ota_param_r.splict_size);
#endif
        } else {
            ...
            if(memcmp(param, &param_r, sizeof(ota_boot_param_t)) != 0) {
                 OTA_LOG_E("OTA compare failed, dst:0x%08x src:0x%08x len:0x%08x crc:0x%04x\r\n",param_r.dst_adr,param_r.src_adr,param_r.len,param_r.crc);
                 return -1;
            }
