...
int iotx_http2_stream_open(http2_connection_t *connection, stream_data_info_t *info, header_ext_info_t *header)
{
    ...
    const http2_header static_header[] = { MAKE_HEADER(":method", "POST"),
                                           ...
                                           MAKE_HEADER("x-auth-param-client-id", ""),
    ...
    if (rv >= 0) {
        info->stream_id = h2_data.stream_id;
    }
    return rv;
}
...
int iotx_http2_stream_send(http2_connection_t *connection, stream_data_info_t *info)
{
    ...
    if (info->send_header == 0) { //first send,need header
        const http2_header static_header[] = { MAKE_HEADER(":method", "POST"),
                                               MAKE_HEADER_CS(":path", path),
                                               MAKE_HEADER(":scheme", "https"),
                                               MAKE_HEADER("content-length", data_len_str),
                                               MAKE_HEADER("x-data-stream-id", stream_id_str),
                                             };
        ...
        h2_data.len = 0;
        h2_data.flag = 0;
        h2_data.stream_id = info->stream_id;
        rv = iotx_http2_client_send((void *)connection, &h2_data);
        if (rv >= 0) {
            info->send_header = 1;
        }
