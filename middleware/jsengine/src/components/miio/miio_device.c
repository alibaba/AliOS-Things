                        ...
                        int input_len, char *output, int *output_len)
{
    ...
    jse_aes_context aes;
    jse_aes_init(&aes);
    jse_aes_setkey_enc(&aes, device->key, KEY_IV_SIZE * 8);
    unsigned char iv_tmp[KEY_IV_SIZE];
    memcpy(iv_tmp, device->iv, sizeof(iv_tmp));
    int err = jse_aes_crypt_cbc(&aes, JSE_AES_ENCRYPT, payload_len,
                                    iv_tmp, input, output);
    jse_aes_free(&aes);
    if (!err) {
                        ...
                        int input_len, char *output, int *output_len)
{
    jse_aes_context aes;
    jse_aes_init(&aes);
    jse_aes_setkey_dec(&aes, device->key, KEY_IV_SIZE * 8);
    unsigned char iv_tmp[KEY_IV_SIZE];
    memcpy(iv_tmp, device->iv, sizeof(iv_tmp));
    int err = jse_aes_crypt_cbc(&aes, JSE_AES_DECRYPT, input_len,
                                    iv_tmp, input, output);
    jse_aes_free(&aes);
    if (!err) {
        /* Remove padding */
                                ...
                                const char *args, const char *sid)
{
    ...
    *((unsigned long *)(pkt + 12)) = htonl(timestamp);
    ...
    jse_md5_init(&ctx);
    jse_md5_starts(&ctx);
    jse_md5_update(&ctx, pkt, 16);
    jse_md5_update(&ctx, device->token, sizeof(device->token));
    jse_md5_update(&ctx, pkt + 32, len - 32);
    jse_md5_finish(&ctx, pkt + 16); /* fill to [16,31] */
    jse_md5_free(&ctx);
...
miio_device_t *miio_device_create(const char *host, const char *token)
{
    ...
    /* generate key */
    jse_md5(device->token, sizeof(device->token), device->key);

    /* generate iv */
    jse_md5_context ctx;
    jse_md5_init(&ctx);
    jse_md5_starts(&ctx);
    jse_md5_update(&ctx, device->key, sizeof(device->key));
    jse_md5_update(&ctx, device->token, sizeof(device->token));
    jse_md5_finish(&ctx, device->iv);
    jse_md5_free(&ctx);
