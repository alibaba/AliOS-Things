...
static be_jse_symbol_t *module_crypto_encrypt()
{
    ...
    jse_aes_context aes;
    jse_aes_setkey_enc(&aes, key_array, 16 * 8);
    ...
    if (!err) {
        ...
        jse_warn("jse_aes_crypt_cbc JSE_AES_ENCRYPT failed, err: %d\n",
                 err);
    }
...
static be_jse_symbol_t *module_crypto_decrypt()
{
    ...
    jse_aes_context aes;
    jse_aes_setkey_dec(&aes, key_array, 16 * 8);
    int err = jse_aes_crypt_cbc(&aes, JSE_AES_DECRYPT,
    ...
    if (!err) {
        ...
        jse_warn("jse_aes_crypt_cbc JSE_AES_DECRYPT failed, err: %d\n",
                 err);
    }
