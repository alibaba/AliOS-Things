...
void checkEndian()
{
	...
	*(mem+3)=0x78;
	...
	if (low_addr_value == 0x12){
		printf("big endian\n\r");
	}else {
		printf("little endian=%x\n\r",low_addr_value);
		printf("u16=%x\n\r",*(unsigned short int*)(mem+1));
	}
}
...
s32_t spiffs_object_read(
  ...
  while (cur_offset < offset + len) {
    ...
    if (fd->ix_map && data_spix >= fd->ix_map->start_spix && data_spix <= fd->ix_map->end_spix
      ...
      if (cur_objix_spix == 0) {
        // get data page from object index header page
        printf("objix_hdr add=%x,sizeof(%d)\n\r",objix_hdr,sizeof(spiffs_page_object_ix_header));
		hexdump(objix_hdr,100);
        data_pix = ((spiffs_page_ix*)((u8_t *)objix_hdr + sizeof(spiffs_page_object_ix_header)))[data_spix];
			int i=0;
			for(i=0;i<=45+5;i++)
			printf("data_pix=i=%d, addr=%08x, value=%04x  --\n\r",i,(u8_t *)objix_hdr + i,((u16_t *)((u8_t *)objix_hdr + i))[data_spix]);
			printf("object u16=addr=%08x,  vaule=%04x\n\r",((u8_t *)objix_hdr+45)   ,*((short  *)((u8_t *)objix_hdr+45)));
			printf("object u8=addr=%08x,  vaule=%04x\n\r",((u8_t *)objix_hdr+45)	,*((char *)((u8_t *)objix_hdr+45)));
			printf("object u32=addr=%08x,  vaule=%04x\n\r",((u8_t *)objix_hdr+45)	,*((int *)((u8_t *)objix_hdr+45)));
			printf("object-1 u16=addr=%08x,  vaule=%04x\n\r",((u8_t *)objix_hdr+44)   ,*((short  *)((u8_t *)objix_hdr+44)));
			 checkEndian();
...
#endif
	//data_pix    = 0x0105; add by wuzhun for test
