...
typedef enum {
    ...
    BE_SYM_FUNCTION_ARGVS       = BE_SYM_FUNCTION | BE_SYM_NAME,    // 等同 BE_SYM_STRING
...
/*
    JSE编译时指定一个固定大小的be_jse_symbol_t 类型数组用于存所JS运行中所有的对象
    be_jse_node_t 类型数据: 记录对应var在数组中的位置, 从1开始对应数组下标+1
*/
/*
...
typedef struct {
    ...
    // 在多成员情况下, last_child记录最后一个成员, 每个成员的prev_sibling指向上一个成员,next_sibling指向下一个成员

    // 保存为BE_SYM_FUNCTION时,与上类似, first_child/last_child记录函数参数/CODE(JS函数内容)/SCOPE
