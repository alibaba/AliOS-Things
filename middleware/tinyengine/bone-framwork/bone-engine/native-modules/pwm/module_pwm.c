...
static be_jse_symbol_t * pwm_start(void){
	...
	be_debug("pwm","%s:%d:%d:%f\n",data,pwm_device->port,pwm_device->config.freq,pwm_device->config.duty_cycle);
...
static be_jse_symbol_t * pwm_module_handle_cb(be_jse_vm_ctx_t *execInfo, be_jse_symbol_t *var, const char *name){
