...
static BE_JSE_FUNCTION_EXECUTE_CB be_jse_find_modules(be_jse_node_t objRef)
{
    ...
    {
        if (objRef == gModulesRef[i])
            return gModules[i];
    }
    return NULL;
}

be_jse_symbol_t *be_jse_module_process(be_jse_vm_ctx_t *execInfo, be_jse_symbol_t *var, const char *name)
{
    ...
    {
        be_jse_symbol_t *objName = lookup_named_child_symbol(bone_engine_get_executor()->root, "String", false);
        if (objName)
            objRef = objName->first_child;
    ...
    {
        be_jse_symbol_t *objName = lookup_named_child_symbol(bone_engine_get_executor()->root, "Array", false);
        if (objName)
            objRef = objName->first_child;
        symbol_unlock(objName);
    }
...
void be_jse_module_load(const char *moduleName, BE_JSE_FUNCTION_EXECUTE_CB extension)
{
    ...
    // 判断模块是否已加载
    be_jse_symbol_t *name = lookup_named_child_symbol(bone_engine_get_executor()->root, moduleName, false);
    ...
    //printf("[%s][%s][%d] moduleName=%s objRef = %p \n", __FILE__, __FUNCTION__, __LINE__, moduleName, objRef);
