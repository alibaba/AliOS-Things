...
mb_status_t mb_serial_frame_send(mb_handler_t * handler)
{
    ...
    ret = hal_uart_send(uart, handler->mb_frame_buff, handler->mb_frame_length, 30000);
    if(ret == 0) {
        return MB_SUCCESS;
    }
...
mb_status_t mb_serial_frame_recv(mb_handler_t * handler)
{
    ...
    uart_dev_t *uart = (mb_handler_t *)handler->private;
    ret = hal_uart_recv_II(uart, handler->mb_frame_buff, ADU_BUF_MAX_LENGTH, &handler->mb_frame_length, krhino_ms_to_ticks(handler->respond_timeout));
    if(ret == 0) {
        return MB_SUCCESS;
    }
