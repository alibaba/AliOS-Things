...
#define TEST_TASK1_PRI  (TEST_TASK_PRIORITY-2)  /* low priority task */
...
{
    while (test_count < TEST_ITERATION) {
        krhino_task_suspend(&test_task2_tcb);         
    }
}
...
void rttest_schedule_resume_active(void)
{
    ...
    rttest_aux_show_result(SCHED_RESUME_ACTIVE_ID, "sched_resume_active", test_count, time_sum, time_max, time_min);
