 ...
 */
#ifndef _AOS_BT_DEV_H_
#define _AOS_BT_DEV_H_
...
/* Result/Status for aos_bt_dev */
typedef aos_bt_result_t  aos_bt_dev_status_t;      /*< Result/Status for aos_bt_dev */
...
} aos_bt_dev_vendor_specific_command_complete_params_t;
...
enum aos_bt_discoverability_mode_e {
...
enum aos_bt_connectability_mode_e {
 ...
 */
#ifndef BTM_INQUIRY_MODE            /* To avoid redefintions when including aos_bt_dev.h */
#define BTM_INQUIRY_MODE
enum aos_bt_inquiry_mode_e {
...
/* Inquiry Filter Condition types (see aos_bt_dev_inq_parms_t) */
#ifndef BTM_INQUIRY_FILTER
#define BTM_INQUIRY_FILTER
enum aos_bt_dev_filter_cond_e {
 ...
 * Used in the aos_bt_dev_inq_info_t structure, and returned in the
...
typedef struct {
    aos_bt_dev_class_t            dev_class;          /*< class of device */
    aos_bt_dev_class_t            dev_class_mask;     /*< class of device filter mask */
} aos_bt_dev_cod_cond_t;

/* Inquiry filter */
typedef union {
    aos_bt_device_address_t       bdaddr_cond;        /*< bluetooth address filter */
    aos_bt_dev_cod_cond_t         cod_cond;           /*< class of device filter */
} aos_bt_dev_inq_filt_cond_t;

/* Inquiry Parameters */
typedef struct {
    uint8_t                         mode;               /*< Inquiry mode (see #aos_bt_inquiry_mode_e) */
    uint8_t                         duration;           /*< Inquiry duration (1.28 sec increments) */
    uint8_t                         filter_cond_type;   /*< Inquiry filter type  (see #aos_bt_dev_filter_cond_e) */
    aos_bt_dev_inq_filt_cond_t    filter_cond;        /*< Inquiry filter */
} aos_bt_dev_inq_parms_t;

/* Inquiry Results */
typedef struct {
    uint16_t                        clock_offset;                           /*< Clock offset */
    aos_bt_device_address_t       remote_bd_addr;                         /*< Device address */
    aos_bt_dev_class_t            dev_class;                              /*< Class of device */
    ...
    uint8_t                         eir_complete_list;                      /*< TRUE if EIR array is complete */
} aos_bt_dev_inquiry_scan_result_t;

/* RSSI Result (in response to #aos_bt_dev_read_rssi) */
typedef struct {
    aos_bt_result_t                  status;             /*< Status of the operation */
    uint8_t                         hci_status;         /*< Status from controller */
    int8_t                          rssi;               /*< RSSI */
    aos_bt_device_address_t       rem_bda;            /*< Remote BD address */
} aos_bt_dev_rssi_result_t;

/* TX Power Result (in response to #aos_bt_dev_read_tx_power) */
typedef struct {
    aos_bt_result_t                  status;             /*< Status of the operation */
    uint8_t                         hci_status;         /*< Status from controller */
    int8_t                          tx_power;           /*< TX power */
    aos_bt_device_address_t       rem_bda;            /*< Remote BD address */
} aos_bt_tx_power_result_t;
...
#define BTM_SECURITY_KEY_DATA_LEN       132     /*< Security key data length (used by aos_bt_device_link_keys_t structure) */
#endif

#ifndef BTM_SECURITY_LOCAL_KEY_DATA_LEN
#define BTM_SECURITY_LOCAL_KEY_DATA_LEN 65      /*< Local security key data length (used by aos_bt_local_identity_keys_t structure) */
#endif

/* Pairing IO Capabilities */
enum aos_bt_dev_io_cap_e {
...
typedef uint8_t aos_bt_dev_io_cap_t;          /*< IO capabilities (see #aos_bt_dev_io_cap_e) */

/* BR/EDR Authentication requirement */
enum aos_bt_dev_auth_req_e {
...
typedef uint8_t
aos_bt_dev_auth_req_t;                /*< BR/EDR authentication requirement (see #aos_bt_dev_auth_req_e) */
...
typedef uint8_t aos_bt_security_mode_t;

/* LE Authentication requirement */
enum aos_bt_dev_le_auth_req_e {
...
typedef uint8_t
aos_bt_dev_le_auth_req_t;             /*< BLE authentication requirement (see #aos_bt_dev_le_auth_req_e) */

/* OOB Data status */
#ifndef BTM_OOB_STATE
#define BTM_OOB_STATE
enum aos_bt_dev_oob_data_e {
...
#endif
typedef uint8_t aos_bt_dev_oob_data_t;                /*< OOB data (see #aos_bt_dev_oob_data_e) */

/* Data type for IO capabalities response (BTM_PAIRING_IO_CAPABILITIES_BR_EDR_RESPONSE_EVT) */
typedef struct {
    aos_bt_device_address_t   bd_addr;                /*< Peer address */
    aos_bt_dev_io_cap_t       io_cap;                 /*< Peer IO capabilities */
    aos_bt_dev_oob_data_t     oob_data;               /*< OOB data present at peer device for the local device */
    aos_bt_dev_auth_req_t     auth_req;               /*< Authentication required for peer device */
...
typedef struct {
    ...
    aos_bool_t                just_works;                         /*< TRUE, if using "just works" association model */
    aos_bt_dev_auth_req_t     local_authentication_requirements;  /*< Authentication requirement for local device */
    aos_bt_dev_auth_req_t     remote_authentication_requirements; /*< Authentication requirement for peer device */
} aos_bt_dev_user_cfm_req_t;

/* Pairing user passkey request  (BTM_USER_PASSKEY_REQUEST_EVT event data type) */
typedef struct {
    aos_bt_device_address_t   bd_addr;            /*< peer address       */
} aos_bt_dev_user_key_req_t;

/* Data for pairing passkey notification (BTM_USER_PASSKEY_NOTIFICATION_EVT event data type) */
typedef struct {
    aos_bt_device_address_t   bd_addr;            /*< peer address       */
    uint32_t                    passkey;            /*< passkey            */
} aos_bt_dev_user_key_notif_t;


/* Pairing keypress types */
enum aos_bt_dev_passkey_entry_type_e {
...
typedef uint8_t
aos_bt_dev_passkey_entry_type_t;  /*< Bluetooth pairing keypress value (see #aos_bt_dev_passkey_entry_type_e)  */

/* Pairing keypress notification (BTM_USER_KEYPRESS_NOTIFICATION_EVT event data type) */
typedef struct {
    aos_bt_device_address_t           bd_addr;        /*< peer address       */
    aos_bt_dev_passkey_entry_type_t   keypress_type;  /*< type of keypress   */
} aos_bt_dev_user_keypress_t;

/* BR/EDR pairing complete infomation */
typedef struct {
    uint8_t
    status;                 /*< status of the simple pairing process (see defintions for HCI status codes) */
} aos_bt_dev_br_edr_pairing_info_t;

/* BLE pairing complete infomation */
typedef struct {
    aos_bt_result_t                 status;                 /*< status of the simple pairing process   */
    uint8_t                          reason;                 /*< failure reason (see #aos_bt_smp_status_t) */
    uint8_t                          sec_level;              /*< 0 - None, 1- Unauthenticated Key, 4-Authenticated Key  */
    aos_bool_t                      privacy_supported;      /*< True if privacy supported, False if not    */
    aos_bool_t                      is_pair_cancel;         /*< True if cancelled, else False   */
    aos_bt_device_address_t
    resolved_bd_addr;       /*< Resolved address (if remote device using private address) */
    aos_bt_ble_address_type_t       resolved_bd_addr_type;  /*< Resolved addr type of bonded device */
} aos_bt_dev_ble_pairing_info_t;

/* Transport dependent pairing complete infomation */
typedef union {
    aos_bt_dev_br_edr_pairing_info_t  br_edr;         /*< BR/EDR pairing complete infomation */
    aos_bt_dev_ble_pairing_info_t     ble;            /*< BLE pairing complete infomation */
} aos_bt_dev_pairing_info_t;

/* Pairing complete notification (BTM_PAIRING_COMPLETE_EVT event data type) */
typedef struct {
    uint8_t                     *bd_addr;               /*< peer address           */
    aos_bt_transport_t         transport;              /*< BT_TRANSPORT_BR_EDR or BT_TRANSPORT_LE */
    aos_bt_dev_pairing_info_t  pairing_complete_info;  /*< Transport dependent pairing complete infomation */
    aos_bt_result_t
    bonding_status;         /*< current status of bonding process to notify app of completion status of storing keys */
} aos_bt_dev_pairing_cplt_t;

/* Check if application wishes to upgrade security (BTM_SECURITY_UPGRADE_EVT event data type) */
typedef struct {
    aos_bt_device_address_t   bd_addr;                /*< [in]  Peer address */
    aos_bool_t                upgrade;                /*< [out] Set to TRUE to request security upgrade  */
} aos_bt_dev_security_upgrade_t;

/* Security request (BTM_SECURITY_REQUEST_EVT event data type) */
typedef struct {
    aos_bt_device_address_t      bd_addr;             /*< peer address           */
} aos_bt_dev_security_request_t;

/* LE Key type */
#ifndef BTM_LE_KEY_TYPES
#define BTM_LE_KEY_TYPES
enum aos_bt_dev_le_key_type_e {
...
typedef uint8_t aos_bt_dev_le_key_type_t;             /*< LE key type (see #aos_bt_dev_le_key_type_e) */


enum aos_bt_dev_link_key_type_e {
...
typedef uint8_t aos_bt_security_level;
...
typedef struct {
    ...
    BT_OCTET8               rand;
    UINT16                  ediv;
    aos_bt_security_level  sec_level;
    UINT8                   key_size;
} tBTM_LE_PENC_KEYS;
...
typedef struct {
    BT_OCTET16                  irk;
    aos_bt_ble_address_type_t  addr_type;
    aos_bt_device_address_t    static_addr;
} tBTM_LE_PID_KEYS;
...
} aos_bt_security_key_value_t;


/* Scan duty cycle (used for BTM_BLE_SCAN_STATE_CHANGED_EVT and aos_bt_dev_create_connection) */
#ifndef BTM_BLE_SCAN_TYPE
#define BTM_BLE_SCAN_TYPE
enum aos_bt_ble_scan_type_e {
...
#endif
typedef uint8_t aos_bt_ble_scan_type_t;   /*< scan type (see #aos_bt_ble_scan_type_e) */


/* bonding device information from aos_bt_dev_get_bonded_devices */
typedef struct {
    aos_bt_device_address_t     bd_addr;                 /*< peer address           */
    aos_bt_ble_address_type_t   addr_type;               /*< peer address type : BLE_ADDR_PUBLIC/BLE_ADDR_RANDOM */
    aos_bt_device_type_t
    device_type;             /*< peer device type : BT_DEVICE_TYPE_BREDR/BT_DEVICE_TYPE_BLE/BT_DEVICE_TYPE_BREDR_BLE  */
} aos_bt_dev_bonded_device_info_t;

/* LE Secure connection event data */
/* Type of OOB data required  */
#ifndef BTM_OOB_REQ_TYPE
#define BTM_OOB_REQ_TYPE
enum aos_bt_dev_oob_data_req_type_e {
...
#endif
typedef UINT8
aos_bt_dev_oob_data_req_type_t;         /*< OOB data type requested (see #aos_bt_dev_oob_data_req_type_t) */

/* SMP Pairing status codes */
enum aos_bt_smp_status_e {
...
} aos_bt_local_identity_keys_t;

/* SCO connected event related data */
typedef struct {
    uint16_t    sco_index;                  /*< SCO index */
} aos_bt_sco_connected_t;

/* SCO disconnected event related data */
typedef struct {
    uint16_t    sco_index;                  /*< SCO index */
} aos_bt_sco_disconnected_t;

/*  SCO connect request event related data */
typedef struct {
    uint16_t                    sco_index;      /*< SCO index */
    aos_bt_device_address_t   bd_addr;        /*< Peer bd address */
    aos_bt_dev_class_t        dev_class;      /*< Peer device class */
    aos_bt_sco_type_t         link_type;      /*< SCO link type */
} aos_bt_sco_connection_request_t;
...
#endif
typedef uint8_t aos_bt_dev_power_mgmt_status_t;   /*< Power management status (see #aos_bt_dev_power_mgmt_status_e) */

/* Bluetooth application tracing macro */
#ifndef WPRINT_BT_APP_INFO
//extern aos_mutex_t global_trace_mutex;
/*
#define WPRINT_BT_APP_INFO(info)    { \
                                        aos_rtos_lock_mutex(&global_trace_mutex);     \
                                        WPRINT_APP_INFO(info);                          \
                                        aos_rtos_unlock_mutex(&global_trace_mutex);   \
                                    }
*/
#endif
...
enum aos_bt_management_evt_e {
    /* Bluetooth status events */
    BTM_ENABLED_EVT,                                /*< Bluetooth controller and host stack enabled. Event data: aos_bt_dev_enabled_t */
    BTM_DISABLED_EVT,                               /*< Bluetooth controller and host stack disabled. Event data: NULL */
    BTM_POWER_MANAGEMENT_STATUS_EVT,                /*< Power management status change. Event data: aos_bt_power_mgmt_notification_t */

    /* Security events */
    BTM_PIN_REQUEST_EVT,                            /*< PIN request (used only with legacy devices). Event data: #aos_bt_dev_name_and_class_t */
    BTM_USER_CONFIRMATION_REQUEST_EVT,              /*< received USER_CONFIRMATION_REQUEST event (respond using #aos_bt_dev_confirm_req_reply). Event data: #aos_bt_dev_user_cfm_req_t */
    BTM_PASSKEY_NOTIFICATION_EVT,                   /*< received USER_PASSKEY_NOTIFY event. Event data: #aos_bt_dev_user_key_notif_t */
    BTM_PASSKEY_REQUEST_EVT,                        /*< received USER_PASSKEY_REQUEST event (respond using #aos_bt_dev_pass_key_req_reply). Event data: #aos_bt_dev_user_key_req_t */
    BTM_KEYPRESS_NOTIFICATION_EVT,                  /*< received KEYPRESS_NOTIFY event. Event data: #aos_bt_dev_user_keypress_t */
    BTM_PAIRING_IO_CAPABILITIES_BR_EDR_REQUEST_EVT, /*< Requesting IO capabilities for BR/EDR pairing. Event data: #aos_bt_dev_bredr_io_caps_req_t */
    BTM_PAIRING_IO_CAPABILITIES_BR_EDR_RESPONSE_EVT,/*< Received IO capabilities response for BR/EDR pairing. Event data: #aos_bt_dev_bredr_io_caps_rsp_t */
    BTM_PAIRING_IO_CAPABILITIES_BLE_REQUEST_EVT,    /*< Requesting IO capabilities for BLE pairing. Event data: #aos_bt_dev_ble_io_caps_req_t */
    BTM_PAIRING_COMPLETE_EVT,                       /*< received SIMPLE_PAIRING_COMPLETE event. Event data: #aos_bt_dev_pairing_cplt_t */
    BTM_ENCRYPTION_STATUS_EVT,                      /*< Encryption status change. Event data: #aos_bt_dev_encryption_status_t */
    BTM_SECURITY_REQUEST_EVT,                       /*< Security request (respond using #aos_bt_ble_security_grant). Event data: #aos_bt_dev_security_request_t */
    BTM_SECURITY_UPGRADE_EVT,                       /*< Check if the application wants to upgrade the link key. Event data: #aos_bt_dev_security_upgrade_t */
    BTM_SECURITY_ABORTED_EVT,                       /*< Security procedure aborted locally, or unexpected link drop. Event data: #aos_bt_dev_name_and_class_t */

    BTM_READ_LOCAL_OOB_DATA_COMPLETE_EVT,           /*< Result of reading local OOB data (aos_bt_dev_read_local_oob_data). Event data: #aos_bt_dev_local_oob_t */
    BTM_REMOTE_OOB_DATA_REQUEST_EVT,                /*< OOB data from remote device (respond using #aos_bt_dev_remote_oob_data_reply). Event data: #aos_bt_dev_remote_oob_t */

    BTM_PAIRED_DEVICE_LINK_KEYS_UPDATE_EVT,         /*< Updated remote device link keys (store device_link_keys to  NV memory). Event data: #aos_bt_device_link_keys_t */
    BTM_PAIRED_DEVICE_LINK_KEYS_REQUEST_EVT,        /*< Request for stored remote device link keys (restore device_link_keys from NV memory). If successful, return AOS_BT_SUCCESS. Event data: #aos_bt_device_link_keys_t */

    BTM_LOCAL_IDENTITY_KEYS_UPDATE_EVT,             /*< Update local identity key (stored local_identity_keys NV memory). Event data: #aos_bt_local_identity_keys_t */
    BTM_LOCAL_IDENTITY_KEYS_REQUEST_EVT,            /*< Request local identity key (get local_identity_keys from NV memory). If successful, return AOS_BT_SUCCESS. Event data: #aos_bt_local_identity_keys_t */

    BTM_BLE_SCAN_STATE_CHANGED_EVT,                 /*< BLE scan state change. Event data: #aos_bt_ble_scan_type_t */
    BTM_BLE_ADVERT_STATE_CHANGED_EVT,               /*< BLE advertisement state change. Event data: #aos_bt_ble_advert_mode_t */

    /* BLE Secure Connection events */
    BTM_SMP_REMOTE_OOB_DATA_REQUEST_EVT,            /*< SMP remote oob data request. Reply using aos_bt_smp_oob_data_reply. Event data: aos_bt_smp_remote_oob_req_t  */
    BTM_SMP_SC_REMOTE_OOB_DATA_REQUEST_EVT,         /*< LE secure connection remote oob data request. Reply using aos_bt_smp_sc_oob_reply. Event data: #aos_bt_smp_sc_remote_oob_req_t */
    BTM_SMP_SC_LOCAL_OOB_DATA_NOTIFICATION_EVT,     /*< LE secure connection local OOB data (aos_bt_smp_create_local_sc_oob_data). Event data: #aos_bt_smp_sc_local_oob_t*/

    BTM_SCO_CONNECTED_EVT,                          /*< SCO connected event. Event data: #aos_bt_sco_connected_t */
    BTM_SCO_DISCONNECTED_EVT,                       /*< SCO disconnected event. Event data: #aos_bt_sco_disconnected_t */
    BTM_SCO_CONNECTION_REQUEST_EVT,                 /*< SCO connection request event. Event data: #aos_bt_sco_connection_request_t */
    BTM_SCO_CONNECTION_CHANGE_EVT                   /*< SCO connection change event. Event data: #aos_bt_sco_connection_change_t */
};
#endif
typedef uint8_t aos_bt_management_evt_t;          /*< Bluetooth management events (see #aos_bt_management_evt_e) */

/* Device enabled (used by BTM_ENABLED_EVT) */
typedef struct {
    aos_bt_result_t          status;                     /*< Status */
} aos_bt_dev_enabled_t;

/* Remote device information (used by BTM_PIN_REQUEST_EVT, BTM_SECURITY_ABORTED_EVT) */
typedef struct {
    aos_bt_device_address_t  *bd_addr;                /*< BD Address of remote */
    aos_bt_dev_class_t    *dev_class;                 /*< peer class of device   */
...
typedef struct {
    aos_bt_device_address_t           bd_addr;        /*< BD Address of remote */
    aos_bt_dev_power_mgmt_status_t    status;         /*< PM status */
    uint16_t                            value;          /*< Additional mode data */
    uint8_t                             hci_status;     /*< HCI status */
} aos_bt_power_mgmt_notification_t;

/* Encryption status change (used by BTM_ENCRYPTION_STATUS_EVT) */
typedef struct {
    uint8_t                 *bd_addr;                   /*< BD Address of remote */
    aos_bt_transport_t    transport;                  /*< BT_TRANSPORT_BR_EDR or BT_TRANSPORT_LE */
    void                    *p_ref_data;                /*< Optional data passed in by aos_bt_dev_set_encryption */
    aos_bt_result_t          result;                     /*< Result of the operation */
} aos_bt_dev_encryption_status_t;

/* Local OOB data BTM_READ_LOCAL_OOB_DATA_COMPLETE_EVT */
typedef struct {
    aos_bt_result_t          status;                     /*< Status */
    aos_bool_t            is_extended_oob_data;       /*< TRUE if extended OOB data */
...
} aos_bt_dev_local_oob_t;

/* BTM_REMOTE_OOB_DATA_REQUEST_EVT */
typedef struct {
    aos_bt_device_address_t   bd_addr;                /*< BD Address of remote */
    aos_bool_t                extended_oob_data;      /*< TRUE if requesting extended OOB (P-256) */
} aos_bt_dev_remote_oob_t;

/* BR/EDR Pairing IO Capabilities (to be filled by application callback on BTM_PAIRING_IO_CAPABILITIES_REQUEST_EVT) */
typedef struct {
    aos_bt_device_address_t   bd_addr;                /*< [in] BD Address of remote   */
    aos_bt_dev_io_cap_t       local_io_cap;           /*< local IO capabilities (to be filled by application callback) */
    aos_bt_dev_oob_data_t     oob_data;               /*< OOB data present at peer device for the local device   */
    aos_bt_dev_auth_req_t     auth_req;               /*< Authentication required for peer device                */
    aos_bool_t                is_orig;                /*< TRUE, if local device initiated the pairing process    */
} aos_bt_dev_bredr_io_caps_req_t;
...
typedef struct {
    ...
    aos_bt_link_key_t                br_edr_key;             /*<  BR/EDR Link Key */

    /* LE Keys */
    aos_bt_dev_le_key_type_t         le_keys_available_mask; /*<  Mask of available BLE keys */
    aos_bt_ble_address_type_t        ble_addr_type;          /*<  LE device type: public or random address */
    aos_bt_ble_address_type_t        static_addr_type;       /*<  static address type */
    aos_bt_device_address_t          static_addr;            /*<  static address */
    aos_bt_ble_keys_t                le_keys;                /*<  LE keys */
} aos_bt_device_sec_keys_t;

/* Paired device link key notification (used by BTM_PAIRED_DEVICE_LINK_KEYS_UPDATE_EVT notication) */
typedef struct {
    aos_bt_device_address_t   bd_addr;         /*< [in] BD Address of remote */
    aos_bt_device_sec_keys_t  key_data;        /*< [in/out] Key data */
} aos_bt_device_link_keys_t;

/* advertisement type (used when calling aos_bt_start_advertisements) */
#ifndef BTM_BLE_ADVERT_MODE
#define BTM_BLE_ADVERT_MODE
enum aos_bt_ble_advert_mode_e {
...
#endif
typedef uint8_t aos_bt_ble_advert_mode_t;   /*< Advertisement type (see #aos_bt_ble_advert_mode_e) */
...
#endif
typedef uint8_t aos_bt_ble_conn_mode_t;       /*< Conn mode (see #aos_bt_ble_conn_mode_e) */

/* Structure definitions for Bluetooth Management (aos_bt_management_cback_t) event notifications */
typedef union {
    /* Bluetooth status event data types*/
    aos_bt_dev_enabled_t              enabled;                            /*< Data for BTM_ENABLED_EVT */
    aos_bt_power_mgmt_notification_t  power_mgmt_notification;            /*< Data for BTM_POWER_MANAGEMENT_STATUS_EVT */

    /* Security event data types */
    aos_bt_dev_name_and_class_t       pin_request;                        /*< Data for BTM_PIN_REQUEST_EVT */
    aos_bt_dev_user_cfm_req_t         user_confirmation_request;          /*< Data for BTM_USER_CONFIRMATION_REQUEST_EVT */
    aos_bt_dev_user_key_notif_t       user_passkey_notification;          /*< Data for BTM_USER_PASSKEY_NOTIFICATION_EVT */
    aos_bt_dev_user_key_req_t         user_passkey_request;               /*< Data for BTM_USER_PASSKEY_REQUEST_EVT */
    aos_bt_dev_user_keypress_t
    user_keypress_notification;         /*< Data for BTM_USER_KEYPRESS_NOTIFICATION_EVT - See #aos_bt_dev_user_keypress_t */
    aos_bt_dev_bredr_io_caps_req_t
    ...
    aos_bt_dev_pairing_cplt_t         pairing_complete;                   /*< Data for BTM_PAIRING_COMPLETE_EVT */
    aos_bt_dev_encryption_status_t    encryption_status;                  /*< Data for BTM_ENCRYPTION_STATUS_EVT */
    aos_bt_dev_security_request_t     security_request;                   /*< Data for BTM_SECURITY_REQUEST_EVT */
    aos_bt_dev_security_upgrade_t
    security_upgrade;                   /*< Data for BTM_SECURITY_UPGRADE_EVT See #aos_bt_dev_security_upgrade_t */
    aos_bt_dev_name_and_class_t       security_aborted;                   /*< Data for BTM_SECURITY_ABORTED_EVT */

    aos_bt_dev_local_oob_t
    read_local_oob_data_complete;       /*< Data for BTM_READ_LOCAL_OOB_DATA_COMPLETE_EVT */
    aos_bt_dev_remote_oob_t           remote_oob_data_request;            /*< Data for BTM_REMOTE_OOB_DATA_REQUEST_EVT */
    ...
    aos_bt_ble_scan_type_t            ble_scan_state_changed;             /*< Data for BTM_BLE_SCAN_STATE_CHANGED_EVT */
    aos_bt_ble_advert_mode_t          ble_advert_state_changed;           /*< Data for BTM_BLE_ADVERT_STATE_CHANGED_EVT */
    ...
    *p_smp_sc_local_oob_data;           /*< Data for BTM_SMP_SC_LOCAL_OOB_DATA_NOTIFICATION_EVT */
    ...
    aos_bt_sco_connected_t            sco_connected;                      /*< Data for BTM_SCO_CONNECTED_EVT */
    aos_bt_sco_disconnected_t         sco_disconnected;                   /*< Data for BTM_SCO_DISCONNECTED_EVT */
    aos_bt_sco_connection_request_t   sco_connection_request;             /*< Data for BTM_SCO_CONNECTION_REQUEST_EVT */
    aos_bt_sco_connection_change_t    sco_connection_change;              /*< Data for BTM_SCO_CONNECTION_CHANGE_EVT */

} aos_bt_management_evt_data_t;

/*
 * Bluetooth Management callback
 *
 * Callback for Bluetooth Management event notifications.
 * Registered using aos_bt_stack_init()
 *
 ...
 */
typedef aos_bt_result_t (aos_bt_management_cback_t) (aos_bt_management_evt_t event,
 ...
 *
 * Callback for Bluetooth Management event notifications.
 * Registered using aos_bt_register_connection_status_change()
 *
 ...
 * @return void
 */
typedef void (aos_bt_connection_status_change_cback_t) (aos_bt_device_address_t bd_addr, uint8_t *p_features,
                                                        aos_bool_t is_connected, uint16_t handle, aos_bt_transport_t transport);  /*<   connection status change callback */
 ...
 * @return Nothing
 */
typedef void (aos_bt_inquiry_result_cback_t) (aos_bt_dev_inquiry_scan_result_t *p_inquiry_result,
 ...
 * @return Nothing
 */
typedef void (aos_bt_dev_cmpl_cback_t) (void *p_data);
 ...
 * @return Nothing
 */
typedef void (aos_bt_dev_vendor_specific_command_complete_cback_t) (aos_bt_dev_vendor_specific_command_complete_params_t
 ...
 *
 * @addtogroup  aosbt_DeviceManagement    Device Management
 * @ingroup     aosbt
 *
 * @{
 */
 ...
 *
 * @addtogroup  aosbt_bredr    BR/EDR (Bluetooth Basic Rate / Enhanced Data Rate)
 * @ingroup     aosbt_DeviceManagement
 *
 * @{
 */
/***************************************************************************/
 ...
 *
 * @return          aos_bt_result_t
 *
 *                  AOS_BT_PENDING if successfully initiated
 *                  AOS_BT_BUSY if already in progress
 *                  AOS_BT_ILLEGAL_VALUE if parameter(s) are out of range
 *                  AOS_BT_NO_RESOURCES if could not allocate resources to start the command
 *                  AOS_BT_WRONG_MODE if the device is not up.
 */
aos_bt_result_t  aos_bt_start_inquiry (aos_bt_dev_inq_parms_t *p_inqparms,
                                       aos_bt_inquiry_result_cback_t *p_inquiry_result_cback);

/*
 * Function         aos_bt_cancel_inquiry
 *
 *                  Cancel inquiry
 *
 * @return
 *
 *                  AOS_BT_SUCCESS         if successful
 *                  AOS_BT_NO_RESOURCES    if could not allocate a message buffer
 *                  AOS_BT_WRONG_MODE      if the device is not up.
 */
aos_bt_result_t aos_bt_cancel_inquiry(void);

/*
 * Function         aos_bt_dev_read_local_addr
 *
...
/*
 * Function         aos_bt_dev_set_advanced_connection_params
 *
 ...
 *
 * @param[in]       p_inquiry_scan_result : Inquiry scan result (from #aos_bt_inquiry_result_cback_t)
 *
 * @return          aos_bt_result_t
 *
 *                  AOS_BT_SUCCESS : on success;
 *                  AOS_BT_FAILED  : if an error occurred
 */
aos_bt_result_t aos_bt_dev_set_advanced_connection_params (aos_bt_dev_inquiry_scan_result_t *p_inquiry_scan_result);


/*
 * Function         aos_bt_dev_set_local_device_address
 *
 *
 ...
 * @return
 *
 *                  AOS_BT_SUCCESS    : Command sent. Does not expect command complete event. (command complete callback param is NULL)
 *                  AOS_BT_NO_RESOURCES: Failure and no resources.
 *
 */
aos_bt_result_t aos_bt_dev_set_local_device_address(aos_bt_device_address_t bdaddr,
                                                    aos_bt_dev_cmpl_cback_t *p_cback);

/*
 * Function         aos_bt_dev_vendor_specific_command
 *
 ...
 *
 *                  AOS_BT_SUCCESS    : Command sent. Does not expect command complete event. (command complete callback param is NULL)
 *                  AOS_BT_PENDING    : Command sent. Waiting for command complete event.
 *                  AOS_BT_BUSY       : Command not sent. Waiting for command complete event for prior command.
 *
 */
aos_bt_result_t aos_bt_dev_vendor_specific_command (uint16_t opcode, uint8_t param_len, uint8_t *p_param_buf,
                                                    aos_bt_dev_vendor_specific_command_complete_cback_t *p_cback);

/*
 * Function         aos_bt_dev_set_discoverability
 *
 *                  Set discoverability
 *
 * @note            The duration must be less than or equal to the interval.
 *
 * @param[in]       inq_mode        : Discoverability mode (see #aos_bt_discoverability_mode_e)
 ...
 *
 *                  AOS_BT_SUCCESS:        If successful
 *                  AOS_BT_BUSY:           If a setting of the filter is already in progress
 *                  AOS_BT_NO_RESOURCES:   If couldn't get a memory pool buffer
 *                  AOS_BT_ILLEGAL_VALUE:  If a bad parameter was detected
 *                  AOS_BT_WRONG_MODE:     If the device is not up
 */
aos_bt_result_t  aos_bt_dev_set_discoverability (uint8_t inq_mode, uint16_t duration,
                                                 uint16_t interval);

/*
 * Function         aos_bt_dev_set_connectability
 *
 *                  Set connectablilty
 *
 * @note            The duration (window parameter) must be less than or equal to the interval.
 *
 * @param[in]       page_mode       : Connectability mode (see #aos_bt_connectability_mode_e)
 ...
 *
 *                  AOS_BT_SUCCESS:        If successful
 *                  AOS_BT_ILLEGAL_VALUE:  If a bad parameter is detected
 *                  AOS_BT_NO_RESOURCES:   If could not allocate a message buffer
 *                  AOS_BT_WRONG_MODE:     If the device is not up
 */
aos_bt_result_t aos_bt_dev_set_connectability (uint8_t page_mode, uint16_t window,
                                               uint16_t interval);
/*
 * Function         aos_bt_dev_register_connection_status_change
 *
 *                  Register callback for connection status change
 *
 *
 * @param[in]       p_aos_bt_connection_status_change_cback - Callback for connection status change
 *
 * @return          aos_bt_result_t
 *
 *                  AOS_BT_SUCCESS : on success;
 *                  AOS_BT_FAILED : if an error occurred
 */
aos_bt_result_t aos_bt_dev_register_connection_status_change(aos_bt_connection_status_change_cback_t
                                                             *p_aos_bt_connection_status_change_cback);

/*
 * Function         aos_bt_dev_set_sniff_mode
 *
 ...
 *
 * @return          AOS_BT_PENDING if successfully initiated, otherwise error
 */
aos_bt_result_t aos_bt_dev_set_sniff_mode (aos_bt_device_address_t remote_bda, uint16_t min_period,
                                           uint16_t max_period, uint16_t attempt,
                                           uint16_t timeout);


/*
 * Function         aos_bt_dev_cancel_sniff_mode
 *
 ...
 *
 */
aos_bt_result_t aos_bt_dev_cancel_sniff_mode (aos_bt_device_address_t remote_bda);


/*
 *
 * Function         aos_bt_dev_set_sniff_subrating
 *
 ...
 *
 *                  AOS_BT_SUCCESS        : on success;
 *                  AOS_BT_ILLEGAL_ACTION : if an error occurred
 */
 ...
 * @param[in]       p_cback         : Result callback (aos_bt_dev_rssi_result_t will be passed to the callback)
 *
 * @return
 *
 *                  AOS_BT_PENDING if command issued to controller.
 *                  AOS_BT_NO_RESOURCES if couldn't allocate memory to issue command
 *                  AOS_BT_UNKNOWN_ADDR if no active link with bd addr specified
 *                  AOS_BT_BUSY if command is already in progress
 *
 */
...
/*
 * Function         aos_bt_dev_read_tx_power
 *
 *                  Read the transmit power for the requested link
 *
 * @param[in]       remote_bda      : BD address of connection to read tx power
 * @param[in]       transport       : Transport type
 * @param[in]       p_cback         : Result callback (aos_bt_tx_power_result_t will be passed to the callback)
 *
 * @return
 *
 *                  AOS_BT_PENDING if command issued to controller.
 ...
 *
 * @return          AOS_BT_SUCCESS if successful
 *                  AOS_BT_NO_RESOURCES if couldn't allocate memory to issue command
 *                  AOS_BT_UNSUPPORTED if local device cannot support request
 *
 */
aos_bt_result_t aos_bt_dev_write_eir (uint8_t *p_buff, uint16_t len);
...
/*
 * Function         aos_bt_dev_set_pin_code_only
 *
 ...
 * @return          aos_bt_result_t
 */
aos_bt_result_t aos_bt_dev_set_pin_code_only(aos_bool_t enable);

/*
 * Function         aos_bt_dev_set_security_mode
 *
 ...
 *
 */
void aos_bt_dev_set_pin_type(aos_bool_t fixed_pin, uint8_t *pin_code, uint8_t pin_code_len);


/*
 * Function         aos_bt_dev_pin_code_reply
 *
 *                  PIN code reply (use in response to <b>BTM_PIN_REQUEST_EVT </b> in #aos_bt_management_cback_t)
 *
 *  @param[in]      bd_addr     : Address of the device for which PIN was requested
 *  @param[in]      res         : result of the operation AOS_BT_SUCCESS if success
 *  @param[in]      pin_len     : length in bytes of the PIN Code
 *  @param[in]      p_pin       : pointer to array with the PIN Code
 *
 * @return          void
 */
void aos_bt_dev_pin_code_reply (aos_bt_device_address_t bd_addr, aos_bt_result_t res, uint8_t pin_len, uint8_t *p_pin);

/*
 * Function         aos_bt_dev_sec_bond
 *
...
/*
 * Function         aos_bt_dev_sec_bond_cancel
 *
 ...
 *  @return
 *
 *                  AOS_BT_PENDING if cancel initiated,
...
/*
 * Function         aos_bt_dev_set_encryption
 *
 *                  Encrypt the specified connection.
 *                  Status is notified using <b>BTM_ENCRYPTION_STATUS_EVT </b> of #aos_bt_management_cback_t.
 *
 ...
 * @return
 *
 *                  AOS_BT_SUCCESS            : already encrypted
...
/*
 * Function         aos_bt_dev_confirm_req_reply
 *
 *                  Confirm the numeric value for pairing (in response to <b>BTM_USER_CONFIRMATION_REQUEST_EVT </b> of #aos_bt_management_cback_t)
 *
 * @param[in]       res           : result of the operation AOS_BT_SUCCESS if success
...
/*
 * Function         aos_bt_dev_pass_key_req_reply
 *
 *                  Provide the pairing passkey (in response to <b>BTM_PASSKEY_REQUEST_EVT </b> of #aos_bt_management_cback_t)
 *
 * @param[in]       res           : result of the operation AOS_BT_SUCCESS if success
 * @param[in]       bd_addr       : Address of the peer device
...
/*
 * Function         aos_bt_dev_send_key_press_notif
 *
 ...
 *
 */
void aos_bt_dev_send_key_press_notif(aos_bt_device_address_t bd_addr, aos_bt_dev_passkey_entry_type_t type);

/*
 *
 * Function         aos_bt_dev_read_local_oob_data
 *
 ...
 *
 */
aos_bt_result_t aos_bt_dev_read_local_oob_data(void);

/*
 * Function         aos_bt_dev_remote_oob_data_reply
 *
 ...
 *
 */
void aos_bt_dev_remote_oob_data_reply (aos_bt_result_t res, aos_bt_device_address_t bd_addr,
                                       aos_bool_t is_extended_oob_data,
                                       BT_OCTET16 c_192, BT_OCTET16 r_192,
                                       BT_OCTET16 c_256, BT_OCTET16 r_256);

/*
 *
 * Function         aos_bt_dev_build_oob_data
 *
 ...
 *
 */
uint16_t aos_bt_dev_build_oob_data(uint8_t *p_data, uint16_t max_len,
                                   aos_bool_t is_extended_oob_data,
                                   BT_OCTET16 c_192, BT_OCTET16 r_192,
                                   BT_OCTET16 c_256, BT_OCTET16 r_256);
...
/*
 *
 * Function         aos_bt_smp_oob_data_reply
 *
 ...
 *
 */
void aos_bt_smp_oob_data_reply(aos_bt_device_address_t bd_addr, aos_bt_result_t res, uint8_t len, uint8_t *p_data);

/*
 * Function         aos_bt_smp_create_local_sc_oob_data
 *
 ...
 *
 */
aos_bool_t aos_bt_smp_create_local_sc_oob_data (aos_bt_device_address_t bd_addr,
                                                aos_bt_ble_address_type_t bd_addr_type);

/*
 *
 * Function         aos_bt_smp_sc_oob_reply
 *
 ...
 *
 */
void aos_bt_smp_sc_oob_reply (uint8_t *p_oob_data);

/*@} btm_sec_api_functions */


/*@} aosbt_DeviceManagement */
...
} aos_bt_hci_trace_type_t;

/*
 * HCI trace callback
 *
 * Callback for HCI traces
 * Registered using aos_bt_dev_register_hci_trace()
 *
 ...
 * @return void
 */
typedef void ( aos_bt_hci_trace_cback_t )( aos_bt_hci_trace_type_t type, uint16_t length, uint8_t *p_data );

/*
 * Function         aos_bt_dev_register_hci_trace
 *
 ...
 *
 */
void aos_bt_dev_register_hci_trace( aos_bt_hci_trace_cback_t *p_cback );

/*
 * Function         aos_bt_dev_get_bonded_devices
 *
...
/*
 * Function         aos_bt_dev_find_bonded_device
 *
...
/*
 * Function         aos_bt_dev_get_key_by_keytype
 *
...
/*
 * Function         aos_bt_dev_delete_bonded_device
 *
...
/*
 * Function         aos_bt_dev_add_device_to_address_resolution_db
 *
 ...
 *
 */
aos_bt_result_t aos_bt_dev_add_device_to_address_resolution_db(aos_bt_device_link_keys_t *p_link_keys);

/*
 * Function         aos_bt_dev_remove_device_from_address_resolution_db
 *
 ...
 *
 */
aos_bt_result_t aos_bt_dev_remove_device_from_address_resolution_db(aos_bt_device_link_keys_t *p_link_keys);
