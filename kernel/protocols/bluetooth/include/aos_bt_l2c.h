...
#ifndef _AOS_BT_L2C_H_
#define _AOS_BT_L2C_H_
...
/* Result codes for aos_bt_l2cap_data_write() */
#define L2CAP_DATAWRITE_FAILED        FALSE
#define L2CAP_DATAWRITE_SUCCESS       TRUE
#define L2CAP_DATAWRITE_CONGESTED     2

/* Values for priority parameter to aos_bt_l2cap_set_acl_priority */
#define L2CAP_PRIORITY_NORMAL       0
#define L2CAP_PRIORITY_HIGH         1

/* Values for direction parameter to aos_bt_l2cap_set_acl_priority */
...
/* Values for priority parameter to aos_bt_l2cap_set_tx_priority */
#define L2CAP_CHNL_PRIORITY_HIGH    0
#define L2CAP_CHNL_PRIORITY_MEDIUM  1
#define L2CAP_CHNL_PRIORITY_LOW     2

typedef uint8_t aos_bt_l2cap_chnl_priority_t;

/* Values for Tx/Rx data rate parameter to aos_bt_l2cap_set_chnl_data_rate */
...
/* Used in aos_bt_l2cap_flush_channel num_to_flush definitions */
#define L2CAP_FLUSH_CHANNELS_ALL       0xffff
#define L2CAP_FLUSH_CHANNELS_GET       0x0000

/* Definition used for aos_bt_l2cap_set_desire_role */
...
/* Values for 'allowed_modes' field passed in structure aos_bt_l2cap_ertm_information_t */
...
typedef struct {
    ...
    aos_bool_t    mtu_present;            /* TRUE if MTU option present */
    uint16_t        mtu;                    /* Maximum transmission unit size */
    aos_bool_t    qos_present;            /* QoS configuration present */
    aos_bt_flow_spec_t qos;               /* QoS configuration */
    aos_bool_t    flush_timeout_present;  /* TRUE if flush option present */
    uint16_t        flush_timeout;          /* Flush timeout value (1 msec increments) */
    aos_bool_t    fcr_present;            /* TRUE if Enhanced retransmission & flow control option present */
    aos_bt_l2cap_fcr_options_t fcr;       /* Enhanced flow control and retransmission parameters */
    aos_bool_t    fcs_present;            /* TRUE if Frame check sequence option present */
    uint8_t         fcs;                    /* '0' if desire is to bypass FCS, otherwise '1' */
    uint16_t        flags;                  /* bit 0: 0-no continuation, 1-continuation */
} aos_bt_l2cap_cfg_information_t;
...
typedef uint16_t aos_bt_l2cap_ch_cfg_bits_t;
...
} aos_bt_l2cap_ertm_information_t;
 ...
 *
 *  @param context          : Caller context provided with aos_bt_l2cap_register()
 ...
 *  @return void
*/
typedef void (aos_bt_l2cap_connected_cback_t) (void *context, aos_bt_device_address_t bd_addr, uint16_t local_cid,
                                               uint16_t peer_mtu);
 ...
 *  @return void
*/
typedef void (aos_bt_l2cap_disconnect_indication_cback_t) (void *context, uint16_t local_cid, aos_bool_t ack);
 ...
 *  @return void
*/
typedef void (aos_bt_l2cap_disconnect_confirm_cback_t) (void *context, uint16_t local_cid, uint16_t result);
 ...
 *  @return void
*/
typedef void (aos_bt_l2cap_data_indication_cback_t) (void *context, uint16_t local_cid, uint8_t *p_buff,
                                                     uint16_t buf_len);
 ...
 *  @return void
*/
typedef void (aos_bt_l2cap_congestion_status_cback_t) (void *context, uint16_t local_cid, aos_bool_t congested);
 ...
 *  @return void
*/
typedef void (aos_bt_l2cap_nocp_cback_t) (aos_bt_device_address_t bd_addr);
 ...
 *  @return void
*/
typedef void (aos_bt_l2cap_tx_complete_cback_t) (void *context, uint16_t local_cid, uint16_t num_sdu);
...
*/
typedef struct {
    aos_bt_l2cap_connected_cback_t             *connected_cback;              /* BR/EDR connected event */
    aos_bt_l2cap_disconnect_indication_cback_t *disconnect_indication_cback;  /* BR/EDR disconnect indication event */
    aos_bt_l2cap_disconnect_confirm_cback_t    *disconnect_confirm_cback;     /* BR/EDR disconnect confirmation event */
    aos_bt_l2cap_data_indication_cback_t       *data_indication_cback;        /* BR/EDR data received indication */
    aos_bt_l2cap_congestion_status_cback_t     *congestion_status_cback;      /* Connection (un)congested event */
    aos_bt_l2cap_tx_complete_cback_t           *tx_complete_cback;            /* BR/EDR transmit complete event */

    uint16_t                        mtu;
    aos_bool_t                    qos_present;
    aos_bt_flow_spec_t            qos;
    aos_bool_t                    flush_timeout_present;
    uint16_t                        flush_timeout;
    aos_bool_t                    fcr_present;
    aos_bt_l2cap_fcr_options_t    fcr;
    aos_bool_t                    fcs_present;
    uint8_t                         fcs;            /* '0' if desire is to bypass FCS, otherwise '1' */
    aos_bool_t                    is_ob_only;     /* Set to TRUE if registration is for outbound only to a dynamic PSM */
} aos_bt_l2cap_appl_information_t;
 ...
 *  @return void
*/
typedef void (aos_bt_l2cap_fixed_chnl_cback_t) (aos_bt_device_address_t bd_addr, aos_bool_t connected,
 ...
 *  @return void
*/
typedef void (aos_bt_l2cap_fixed_data_cback_t) (aos_bt_device_address_t bd_addr, uint8_t *p_buff, uint16_t buf_len);
 ...
 *  the data will anyways be dropped.
 *
 *  @param bd_addr          : remote aos_bt_device_address_t
 *  @param congested        : TRUE if congested, FALSE if uncongested
 *
 *  @return void
*/
typedef void (aos_bt_l2cap_fixed_congestion_status_cback_t) (aos_bt_device_address_t bd_addr, aos_bool_t congested);

/*
 *  Fixed channel registration info (the callback addresses and channel config)
*/
typedef struct {
    aos_bt_l2cap_fixed_chnl_cback_t               *fixed_conn_cback;  /* TODO */
    aos_bt_l2cap_fixed_data_cback_t               *fixed_data_cback;  /* TODO */
    aos_bt_l2cap_fixed_congestion_status_cback_t  *fixed_cong_cback;  /* TODO */

    uint16_t                                        default_idle_timeout;  /* TODO */
} aos_bt_l2cap_fixed_chnl_reg_t;

/*
 *  LE Connection indication callback prototype.
 *
 *  @param context          : Caller context provided with aos_bt_l2cap_le_register()
 *  @param bd_addr          : BD Address of remote
 *  @param local_cid        : Local CID assigned to the connection
 ...
 *  @return void
*/
typedef void (aos_bt_l2cap_le_connect_indication_cback_t) (void *context, aos_bt_device_address_t bd_addr,
 ...
 *  @return void
*/
typedef void (aos_bt_l2cap_le_connect_confirm_cback_t) (void *context, uint16_t local_cid,
 ...
 *  @return void
*/
typedef void (aos_bt_l2cap_le_conformance_test_cback_t) (uint16_t);
#endif
...
*/
typedef struct {
    aos_bt_l2cap_le_connect_indication_cback_t  *le_connect_indication_cback; /* LE connect indication event */
    aos_bt_l2cap_le_connect_confirm_cback_t     *le_connect_confirm_cback;    /* LE connect confirm event */
    aos_bt_l2cap_disconnect_indication_cback_t  *disconnect_indication_cback; /* LE disconnect indication event */
    aos_bt_l2cap_disconnect_confirm_cback_t     *disconnect_confirm_cback;    /* LE disconnect confirm event */
    aos_bt_l2cap_data_indication_cback_t        *data_indication_cback;       /* LE data received indication */
    aos_bt_l2cap_congestion_status_cback_t      *congestion_status_cback;     /* TODO */
#if (L2CAP_LE_COC_CONFORMANCE_TESTING == TRUE)
    aos_bt_l2cap_le_conformance_test_cback_t    *conformance_test_cback;      /* TODO */
#endif

} aos_bt_l2cap_le_appl_information_t;
...
/*
 *
 *  Function        aos_bt_l2cap_register_fixed_channel
 *
 ...
 *
 */
aos_bool_t  aos_bt_l2cap_register_fixed_channel (uint16_t fixed_cid, aos_bt_l2cap_fixed_chnl_reg_t *p_freg);


/*
 *
 *  Function        aos_bt_l2cap_connect_fixed_chnl
 *
 ...
 *
 */
aos_bool_t aos_bt_l2cap_connect_fixed_chnl (uint16_t fixed_cid, aos_bt_device_address_t bd_addr);


/*
 *
 *  Function        aos_bt_l2cap_send_fixed_chnl_data
 *
 ...
 *
 */
uint16_t aos_bt_l2cap_send_fixed_chnl_data (uint16_t fixed_cid, aos_bt_device_address_t rem_bda,
...
/*
 *
 *  Function        aos_bt_l2cap_remove_fixed_chnl
 *
 ...
 *
 */
aos_bool_t aos_bt_l2cap_remove_fixed_chnl (uint16_t fixed_cid, aos_bt_device_address_t rem_bda);


/*
 *
 *  Function         aos_bt_l2cap_set_fixed_channel_timeout
 *
 ...
 *
 */
aos_bool_t aos_bt_l2cap_set_fixed_channel_timeout (aos_bt_device_address_t rem_bda, uint16_t fixed_cid,
...
/*
 *
 *  Function     aos_bt_l2cap_get_current_config
 *
 ...
 *
 */
aos_bool_t aos_bt_l2cap_get_current_config (uint16_t lcid,
                                            aos_bt_l2cap_cfg_information_t **pp_our_cfg,  aos_bt_l2cap_ch_cfg_bits_t *p_our_cfg_bits,
                                            aos_bt_l2cap_cfg_information_t **pp_peer_cfg, aos_bt_l2cap_ch_cfg_bits_t *p_peer_cfg_bits);


/*
 *
 *  Function         aos_bt_l2cap_register
 *
 ...
 *                  and should be used in the calls to aos_bt_l2cap_connect_req()
 *                  and BTM_SetSecurityLevel().
 *
 */
uint16_t aos_bt_l2cap_register (uint16_t psm, aos_bt_l2cap_appl_information_t *p_cb_information,  void *context);


/*
 *
 *  Function         aos_bt_l2cap_deregister
 *
 ...
 *
 */
void aos_bt_l2cap_deregister (uint16_t psm);


/*
 *
 *  Function         aos_bt_l2cap_allocate_psm
 *
 ...
 *
 */
uint16_t aos_bt_l2cap_allocate_psm (void);


/*
 *
 *  Function         aos_bt_l2cap_connect_req
 *
 ...
 *
 */
uint16_t aos_bt_l2cap_connect_req (uint16_t psm, aos_bt_device_address_t p_bd_addr,
                                   aos_bt_l2cap_ertm_information_t *p_ertm_information);


/*
 *
 * Function         aos_bt_l2cap_ertm_enable
 *
 ...
 *
 */
void aos_bt_l2cap_ertm_enable (void);


/*
 *
 *  Function         aos_bt_l2cap_ertm_connect_req
 *
 *                  Higher layers call this function to create an L2CAP connection
 ...
 *
 */
uint16_t aos_bt_l2cap_ertm_connect_req (uint16_t psm, aos_bt_device_address_t p_bd_addr,
                                        aos_bt_l2cap_ertm_information_t *p_ertm_information);


/*
 *
 *  Function         aos_bt_l2cap_disconnect_req
 *
 ...
 *
 */
aos_bool_t aos_bt_l2cap_disconnect_req (uint16_t cid);


/*
 *
 *  Function         aos_bt_l2cap_disconnect_rsp
 *
 ...
 *
 */
aos_bool_t aos_bt_l2cap_disconnect_rsp (uint16_t cid);


/*
 *
 *  Function        aos_bt_l2cap_data_write
 *
 ...
 *
 */
uint8_t aos_bt_l2cap_data_write (uint16_t cid, uint8_t *p_buf, uint16_t buf_len, uint16_t flags);

/*
 *
 *  Function         aos_bt_l2cap_set_idle_timeout
 *
 *                  Higher layers call this function to set the idle timeout for
 ...
 *
 */
aos_bool_t aos_bt_l2cap_set_idle_timeout (uint16_t cid, uint16_t timeout,
                                          aos_bool_t is_global);


/*
 *
 *  Function         aos_bt_l2cap_set_idle_timeout_by_bd_addr
 *
 *                  Higher layers call this function to set the idle timeout for
 ...
 *                  ACL link.
 */
aos_bool_t aos_bt_l2cap_set_idle_timeout_by_bd_addr (aos_bt_device_address_t bd_addr, uint16_t timeout,
                                                     tBT_TRANSPORT transport);

/*
 *
 *  Function         aos_bt_l2cap_set_trace_level
 *
 ...
 *
 */
uint8_t aos_bt_l2cap_set_trace_level (uint8_t trace_level);


/*
 *
 *  Function     aos_bt_l2cap_set_desire_role
 *
 ...
 *
 */
uint8_t aos_bt_l2cap_set_desire_role (uint8_t new_role);


/*
 *
 *  Function     aos_bt_l2cap_flush_channel
 *
 ...
 *
 */
uint16_t   aos_bt_l2cap_flush_channel (uint16_t lcid, uint16_t num_to_flush);


/*
 *
 *  Function         aos_bt_l2cap_set_acl_priority
 *
 ...
 *
 */
aos_bool_t aos_bt_l2cap_set_acl_priority (aos_bt_device_address_t bd_addr, uint8_t priority);

/*
 *
 *  Function         aos_bt_l2cap_set_acl_priority_ext
 *
 *                  Sets the priority for an ACL channel
 ...
 *
 */
aos_bool_t aos_bt_l2cap_set_acl_priority_ext (aos_bt_device_address_t bd_addr, uint8_t priority, uint8_t direction);
 ...
 *
 */
aos_bool_t aos_bt_l2cap_flow_control (uint16_t cid, aos_bool_t data_enabled);


/*
 *
 *  Function         aos_bt_l2cap_set_tx_priority
 *
 ...
 *
 */
aos_bool_t aos_bt_l2cap_set_tx_priority (uint16_t cid, aos_bt_l2cap_chnl_priority_t priority);


/*
 *
 *  Function         aos_bt_l2cap_register_for_nocp_evt
 *
 ...
 *
 */
aos_bool_t aos_bt_l2cap_register_for_nocp_evt (aos_bt_l2cap_nocp_cback_t *p_cb, aos_bt_device_address_t p_bda);

/*
 *
 *  Function         aos_bt_l2cap_set_flush_timeout
 *
 ...
 *                  ACL link.
 */
aos_bool_t aos_bt_l2cap_set_flush_timeout (aos_bt_device_address_t bd_addr, uint16_t flush_timeout);
...
/*
 *
 *  Function         aos_bt_l2cap_set_chnl_flushability
 *
 ...
 *
 */
aos_bool_t aos_bt_l2cap_set_chnl_flushability (uint16_t cid, aos_bool_t is_flushable);


/*
 *
 *  Function         aos_bt_l2cap_get_peer_features
 *
 ...
 *
 */
aos_bool_t aos_bt_l2cap_get_peer_features (aos_bt_device_address_t bd_addr, uint32_t *p_ext_feat, uint8_t *p_chnl_mask);


/*
 *
 *  Function         aos_bt_l2cap_get_bdaddrby_handle
 *
 ...
 *
 */
aos_bool_t aos_bt_l2cap_get_bdaddrby_handle (uint16_t handle, aos_bt_device_address_t bd_addr);


/*
 *
 *  Function         aos_bt_l2cap_get_chnl_fcr_mode
 *
 ...
 *
 */
uint8_t aos_bt_l2cap_get_chnl_fcr_mode (uint16_t lcid);


/*
 *
 *  Function        aos_bt_l2cap_cancel_ble_connect_req
 *
 ...
 *
 */
aos_bool_t aos_bt_l2cap_cancel_ble_connect_req (aos_bt_device_address_t rem_bda);


/*
 *
 *  Function        aos_bt_l2cap_update_ble_conn_params
 *
 ...
 *
 */
aos_bool_t aos_bt_l2cap_update_ble_conn_params (aos_bt_device_address_t rem_bdRa, uint16_t min_int, uint16_t max_int,
                                                uint16_t latency, uint16_t timeout);


/*
 *
 *  Function        aos_bt_l2cap_enable_update_ble_conn_params
 *
 *                  Update BLE connection parameters.
 *
 *  @param[in]      rem_bda: Remote Bd Address
 ...
 *
 */
aos_bool_t aos_bt_l2cap_enable_update_ble_conn_params (aos_bt_device_address_t rem_bda, aos_bool_t enable);


/*
 *
 *  Function         aos_bt_l2cap_get_ble_conn_role
 *
 ...
 *
 */
uint8_t aos_bt_l2cap_get_ble_conn_role (aos_bt_device_address_t bd_addr);


/*
 *
 *  Function         aos_bt_l2cap_get_disconnect_reason
 *
 ...
 *
 */
uint16_t aos_bt_l2cap_get_disconnect_reason (aos_bt_device_address_t remote_bda, tBT_TRANSPORT transport);


/*
 *
 *  Function         aos_bt_l2cap_le_register
 *
 ...
 *                  in the calls to aos_bt_l2cap_le_connect_req() and aos_bt_l2cap_le_deregister().
 *
 */
uint16_t aos_bt_l2cap_le_register (uint16_t le_psm, aos_bt_l2cap_le_appl_information_t *p_cb_information,
                                   void *context);


/*
 *
 *  Function         aos_bt_l2cap_le_deregister
 *
 ...
 *
 */
aos_bool_t aos_bt_l2cap_le_deregister (uint16_t le_psm);


/*
 *
 *  Function         aos_bt_l2cap_le_connect_req
 *
 *                  Higher layers call this function to create an L2CAP connection
 *                  for LE_PSM.
 *                  Note that the connection is not established at this time, but
 ...
 *
 */
uint16_t aos_bt_l2cap_le_connect_req (uint16_t le_psm, aos_bt_device_address_t p_bd_addr,
...
/*
 *
 *  Function         aos_bt_l2cap_le_connect_rsp
 *
 ...
 *
 */
aos_bool_t  aos_bt_l2cap_le_connect_rsp (aos_bt_device_address_t p_bd_addr, uint8_t id, uint16_t lcid,
                                         uint16_t result, uint16_t rx_mtu, uint8_t rx_sdu_pool_id);


/*
 *
 *  Function         aos_bt_l2cap_le_disconnect_req
 *
 ...
 *
 */
aos_bool_t aos_bt_l2cap_le_disconnect_req (uint16_t lcid);


/*
 *
 *  Function         aos_bt_l2cap_le_disconnect_rsp
 *
 *                  Higher layers call this function to acknowledge the
 ...
 *
 */
aos_bool_t aos_bt_l2cap_le_disconnect_rsp (uint16_t lcid);


/*
 *
 *  Function         aos_bt_l2cap_le_data_write
 *
 ...
 *
 */
uint8_t aos_bt_l2cap_le_data_write (uint16_t cid, uint8_t *p_data, uint16_t buf_len, uint16_t flags);


/*
 *
 *  Function         aos_bt_l2cap_le_set_user_congestion
 *
 ...
 *
 */
aos_bool_t  aos_bt_l2cap_le_set_user_congestion (uint16_t lcid, aos_bool_t is_congested);


/*
 *
 *  Function         aos_bt_l2cap_le_get_peer_mtu
 *
 ...
 *
 */
uint16_t aos_bt_l2cap_le_get_peer_mtu (uint16_t lcid);


/*
 *
 *  Function         aos_bt_l2cap_le_determ_secur_rsp
 *
 ...
 *
 */
uint16_t aos_bt_l2cap_le_determ_secur_rsp (aos_bt_device_address_t bd_addr, uint8_t req_secur,
