...
#define NET_BUF_DBG(fmt, ...) SYS_LOG_DBG(fmt, \
        ##__VA_ARGS__)
...
#endif
{
    ...
    NET_BUF_DBG("%s():%d: pool %p timeout %d,pool->uninit_count %d", func, line, pool, timeout, pool->uninit_count);
...
#endif
{
    ...
    if (!buf) {
        NET_BUF_DBG("Failed to get free buffer");
        return NULL;
    }
