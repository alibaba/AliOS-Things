...
typedef uint32_t (*co_objectdict_cb)(CO_Data *d, const indextable *table, uint8_t i);
...
typedef void (*co_heartbeatError_t)(CO_Data *d, uint8_t id);
typedef void (*co_nodeguardError_t)(CO_Data *d, uint8_t id);
typedef void (*co_initialisation_t)(CO_Data *d);
typedef void (*co_preOperational_t)(CO_Data *d);
typedef void (*co_operational_t)(CO_Data *d);
typedef void (*co_stopped_t)(CO_Data *d);
typedef void (*co_post_sync_t)(CO_Data *d);
typedef void (*co_post_TPDO_t)(CO_Data *d);
typedef void (*co_post_SlaveBootup_t)(CO_Data *d, uint8_t id);
typedef void (*co_post_emcy_t)(CO_Data *d, uint8_t id, uint16_t err_code,
                               uint8_t err_reg, const uint8_t err_spec);
typedef uint32_t (* co_storeODSubIndex_t)(CO_Data *d, uint16_t index, uint8_t sub_index);
