...
ssize_t aos_write(int fd, const void *buf, size_t nbytes)
{
    if((fd == STDOUT_FILENO)||(fd == STDERR_FILENO)) {
        return _vfs_to_aos_res(aos_print(NULL, buf, nbytes));
    }
...
{
    return _vfs_to_aos_res(vfs_fsync(fd));
}
...
{
    return _vfs_to_aos_res(vfs_dirfd((vfs_dir_t *)dir));
}
...
{
    return aos_register_driver(path, ops, arg);
}

int aos_union_unregister_driver(char *path)
{
    return aos_unregister_driver(path);
}
...
// int aos_register_network(network_ops_t *ops)
int aos_register_network(network_ops_t *funcops, network_ops_t *rpcops)
{
    return _vfs_to_aos_res(vfs_register_network("/socket_kernel", (vfs_network_ops_t *)funcops));
}

int aos_unregister_network(void)
{
    return _vfs_to_aos_res(vfs_unregister_network("/socket_kernel"));
}

int aos_fcntl(int fd, int cmd, int val)
{
    return _vfs_to_aos_res(vfs_fcntl(fd, cmd, val));
}
