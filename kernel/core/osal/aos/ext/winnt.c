...
int aos_task_new_ext(aos_task_t *task, const char *name, void (*fn)(void *),
                     void *arg, size_t stack_size, int prio, uint32_t flag)
{
    DWORD dwThreadId = 0;
    int autorun = flags ? 0 : 1;
...
int aos_queue_new(aos_queue_t *queue, unsigned int size, int max_msg)
{
