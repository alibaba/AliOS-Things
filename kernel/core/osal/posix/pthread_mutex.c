...
int pthread_mutex_init(pthread_mutex_t *p_mutex, const pthread_mutexattr_t *attr)
{
    ...
    ret = krhino_mutex_dyn_create((kmutex_t **)&p_mutex->mutex, "mutex");
...
int pthread_mutex_destroy(pthread_mutex_t *p_mutex)
{
    ...
    if (p_mutex->initted != PTHREAD_INITIALIZED_OBJ) {
        if (p_mutex->initted == PTHREAD_STATIC_INIT) {
            memset(p_mutex, 0, sizeof(pthread_mutex_t));
...
int pthread_mutex_lock(pthread_mutex_t *p_mutex)
{
    ...
    /* mutex is initted by PTHREAD_MUTEX_INITIALIZER */
    if (p_mutex->initted == PTHREAD_STATIC_INIT) {
