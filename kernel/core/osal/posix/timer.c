...
pthread_mutex_t g_timer_lock = PTHREAD_MUTEX_INITIALIZER;
...
int timer_create(clockid_t clockid, struct sigevent *restrict evp, timer_t *restrict timerid)
{
    ...
    /* create a timer */
    ret = krhino_timer_dyn_create(&timer_list_m->ktimer, "posix_timer", (timer_cb_t)evp->sigev_notify_function,
                                  1, 1, NULL, 0);
    if (ret != 0) {
        return -1;
    }
...
int clock_settime(clockid_t clock_id, const struct timespec *tp)
{
    ...
    value_ns = timespec_to_nanosecond((struct timespec *)tp) - timespec_to_nanosecond(&timer_spec_cur);
...
int nanosleep(const struct timespec *rqtp, struct timespec *rmtp)
{
    ...
    time_ns = timespec_to_nanosecond((struct timespec *)rqtp);
...
int clock_nanosleep(clockid_t clock_id, int flags, const struct timespec *rqtp, struct timespec *rmtp)
{
    ...
    /* if the time is absolute time transform it to relative time */
    if ((flags & TIMER_ABSTIME) == TIMER_ABSTIME) {
        ret = timespec_abs_to_relate((struct timespec *)rqtp, &value_spec);
        if (ret != 0) {
            return -1;
        }
        ...
        value_ns = timespec_to_nanosecond((struct timespec *)rqtp);
    }
...
{
    krhino_task_sleep((uint64_t)seconds * RHINO_CONFIG_TICKS_PER_SECOND);
...
int usleep(useconds_t us)
{
    ...
    tick = ((uint64_t)us * RHINO_CONFIG_TICKS_PER_SECOND) / MICROSECONDS_PER_SECOND;
    if (tick == 0) {
        tick = 1;
    }
...
{
    return ((uint64_t)value->tv_sec * NANOSECONDS_PER_SECOND + value->tv_nsec);
}
