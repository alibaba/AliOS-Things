...
/* interrupt handler nested counter */
uint8_t g_intrpt_nested_level[RHINO_CONFIG_CPU_NUM];

#if (RHINO_CONFIG_CPU_NUM > 1)
    /* schedule is needed (ready queue changed) */
    uint8_t g_sched_needed[RHINO_CONFIG_CPU_NUM];
#endif
...
/* global task ID */
uint32_t g_task_id[RHINO_CONFIG_TASK_NUM_MAX];
...
klist_t g_tick_head;
    ...
    /* instead _impure_ptr in muti-core interrupt context; core0 use _impure_ptr */
    struct _reent g_impure_reent_int[RHINO_CONFIG_CPU_NUM - 1];
#endif
