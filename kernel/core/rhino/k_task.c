...
{
    if (new_pri >= RHINO_CONFIG_PRI_MAX) {
        return;
    }
    ...
    } else if ((new_pri > RT_MAX_PRI) && (new_pri < RHINO_IDLE_PRI)) {
        task->sched_policy = KSCHED_CFS;
    } else if (new_pri == RHINO_IDLE_PRI) {
        //don't change idle task priority
    }
}
...
/* Make sure call it with protected critical area */
uint32_t alloc_tid()
{
    int i = 0;
    int j = 0;
    ...
    for (i = 0; i < RHINO_CONFIG_TASK_NUM_MAX; i++) {
        x = ~(g_task_id[i]);
        if (x == 0) {
            continue;
        } else {
...
static void free_tid(uint32_t tid)
{
    if ((tid < 1) || (tid > (RHINO_CONFIG_TASK_NUM_MAX << 5))) {
        return;
    }

    tid--;

    g_task_id[tid >> 5] &= ~(1UL << (tid & 0x1F));
}
...
#if (RHINO_CONFIG_SCHED_CFS > 0)
    if (sched_policy == KSCHED_CFS) {
        prio = CFS_TASK_PRI;
    }
    kstat_t ret = task_policy_check(prio, sched_policy);
...
{
#if RHINO_CONFIG_ARMV7_A9 || RHINO_CONFIG_ARMV7_A7
    return (ktask_t *)os_get_THREADIDR();
#else
    cpu_cpsr_t cpsr;
    ktask_t *task;
...
static kstat_t task_wakeup(ktask_t *task)
{
    switch (task->task_state) {
        case K_RDY:
            break;
...
{
    NULL_PARA_CHK(task);
    return krhino_sched_param_set(task, policy, pri);
}
