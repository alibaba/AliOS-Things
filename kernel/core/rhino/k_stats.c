...
void krhino_stack_ovf_check(void)
{
    ...
    for (i = 0; i < RHINO_CONFIG_STK_CHK_WORDS; i++) {
        if (*stack_start++ != RHINO_TASK_STACK_OVF_MAGIC) {
            debug_cpu_stop();
            printk("ERR! Task:%s Stack over flow!\r\n", cur->task_name);
    ...
    if ((cpu_stack_t *)(cur->task_stack) < stack_start) {
        debug_cpu_stop();
        printk("ERR! Task:%s Stack over flow! cur stack:0x%x, stack start:0x%x\r\n", cur->task_name,
               (int)cur->task_stack, (int)stack_start);
        k_err_proc(RHINO_TASK_STACK_OVF);
    }
    ...
    for (i = 0; i < RHINO_CONFIG_STK_CHK_WORDS; i++) {
        if (*stack_start++ != RHINO_TASK_STACK_OVF_MAGIC) {
            printk("stack base %p, value 0x%x\r\n", cur->task_ustack_base, *cur->task_ustack_base);
            printk("ERR! UserSpace Task:%s Stack over flow!\r\n", cur->task_name);
            k_err_proc(RHINO_TASK_STACK_OVF);
        }
    }

    if ((cpu_stack_t *)(cur->task_ustack) < stack_start) {
        printk("ERR! UserSpace Task:%s Stack over flow! cur stack:0x%x, stack start:0x%x\r\n", cur->task_name,
               (int)cur->task_stack, (int)stack_start);
        k_err_proc(RHINO_TASK_STACK_OVF);
    }
#endif
