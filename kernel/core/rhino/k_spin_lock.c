...
#endif
{
    ...
    if (((res & KRHINO_SPINLOCK_VAL_MASK) >> KRHINO_SPINLOCK_VAL_SHIFT) == cpu_cur_get()) {
        ...
        DBG_PRINTF("ERROR: k_cpu_spin_unlock: lock %p wasn't locked by this core (%d)   \
                        but by core %d (ret=%x, lock=%x).,lastfile:%s,lastline:%d,curfile:%s,curline:%d\n", lock,
                   cpu_cur_get(),      \
                   ((res & KRHINO_SPINLOCK_VAL_MASK) >> KRHINO_SPINLOCK_VAL_SHIFT), \
                   res, lock->owner, lastLockedFn, lastLockedLine, lock->last_lockfile, lock->last_lockline);
    }
    return;

}

extern int printk_direct(const char *fmt, ...);
void k_cpu_spin_unlock_force(kspinlock_t *lock)
{
    uint32_t val = 0;
    if (NULL == lock) {
        return;
    }
    if (lock->owner == KRHINO_SPINLOCK_FREE_VAL) {
        return;
    }
    val = lock->owner & (0xffUL << KRHINO_SPINLOCK_VAL_SHIFT);
    if ((val == cpu_cur_get()) || (val >= RHINO_CONFIG_CPU_NUM)) {
        if (lock->last_lockfile) {
            printk_direct("[Warning] force free lock,core:%d,lastlock file:%s,line:%d,val:0x%x\r\n", cpu_cur_get(),
                          lock->last_lockfile, lock->last_lockline, val);
        }
        lock->owner = KRHINO_SPINLOCK_FREE_VAL;
    }
