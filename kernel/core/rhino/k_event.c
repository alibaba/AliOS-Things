...
static kstat_t event_create(kevent_t *event, const name_t *name, uint32_t flags, uint8_t mm_alloc_flag)
{
...
#if (RHINO_CONFIG_USER_SPACE > 0)
    klist_init(&event->blk_obj.obj_list);
#endif
    ...
    if (event->mm_alloc_flag == K_OBJ_DYN_ALLOC && cur_task != NULL) {
        group = cur_task->task_group;
        if (group != NULL) {
            RHINO_CRITICAL_ENTER();
            klist_insert(&(group->kobj_list.event_head), &(event->blk_obj.obj_list));
            RHINO_CRITICAL_EXIT();
        }
    }
#endif
...
#if (RHINO_CONFIG_USER_SPACE > 0)
    if (!is_klist_empty(&(event->blk_obj.obj_list))) {
        klist_rm_init(&(event->blk_obj.obj_list));
    }
#endif
