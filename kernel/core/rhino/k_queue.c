                            ...
                            size_t msg_num, uint8_t mm_alloc_flag)
{
...
#if (RHINO_CONFIG_USER_SPACE > 0)
    klist_init(&queue->blk_obj.obj_list);
#endif
    ...
    if (queue->mm_alloc_flag == K_OBJ_DYN_ALLOC && cur_task != NULL) {
        group = cur_task->task_group;
        if (group != NULL) {
            RHINO_CRITICAL_ENTER();
            klist_insert(&(group->kobj_list.queue_head), &(queue->blk_obj.obj_list));
            RHINO_CRITICAL_EXIT();
        }
    }
#endif
                                ...
                                size_t msg_num)
{
    ...
    stat = queue_create(queue_obj, name, msg_start, msg_num,
                        K_OBJ_DYN_ALLOC);
...
#if (RHINO_CONFIG_USER_SPACE > 0)
    if (!is_klist_empty(&(queue->blk_obj.obj_list))) {
        klist_rm_init(&(queue->blk_obj.obj_list));
    }
#endif
