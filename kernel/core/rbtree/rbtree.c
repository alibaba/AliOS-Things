...
static inline void __rbt_rotate_set_parents(struct rbt_node *old, struct rbt_node *new_node,
        struct rbt_root *root, int color)
{
    struct rbt_node *parent = rbt_parent(old);
    new_node->__rbt_parent_color = old->__rbt_parent_color;
    rbt_set_parent_color(old, new_node, color);
    __rbt_change_child(old, new_node, parent, root);
}
...
static inline void dummy_copy(struct rbt_node *old, struct rbt_node *new_node) {}
static inline void dummy_rotate(struct rbt_node *old, struct rbt_node *new_node) {}
...
void rbt_replace_node(struct rbt_node *victim, struct rbt_node *new_node,
                      struct rbt_root *root)
{
    struct rbt_node *parent = rbt_parent(victim);

    __rbt_change_child(victim, new_node, parent, root);
    if (victim->rbt_left) {
        rbt_set_parent(victim->rbt_left, new_node);
    }
    if (victim->rbt_right) {
        rbt_set_parent(victim->rbt_right, new_node);
    }
