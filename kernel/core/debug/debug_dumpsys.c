...
{
    if (argc != 2) {
        aos_cli_printf("kdumpsys help:\r\n");
        aos_cli_printf("kdumpsys mm      : show kernel memory status.\r\n");
#if (RHINO_CONFIG_MM_DEBUG > 0)
        aos_cli_printf("kdumpsys mm_info : show kernel memory has alloced.\r\n");
#endif
        return;
    }
...
static const struct cli_command dumpsys_cli_cmd[] = {
    ...
    { "kdumpsys", "dump system info", dumpsys_cmd },
#if (RHINO_CONFIG_MM_DEBUG > 0)
    { "kmmlk", "memory leak info", mem_leak },
    { "kmmc", "memory info check", mem_check },
#endif
};
