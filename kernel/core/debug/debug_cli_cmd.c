...
#ifdef DEBUG_CONFIG_PUSH_FILE_ENABLE
static void push_cmd(char *buf, int32_t len, int32_t argc, char **argv);
#endif
...
#ifdef DEBUG_CONFIG_PUSH_FILE_ENABLE
    { "push","push file form host", push_cmd },
#endif
};
...
static void reboot_cmd(char *buf, int32_t len, int32_t argc, char **argv)
{
    ...
    debug_reboot_reason_update(DEBUG_REBOOT_REASON_REPOWER);
#endif
    hal_reboot();
}
...
int __attribute__((weak)) _system_ram_start = 0;
int __attribute__((weak)) _system_ram_len   = 0;
...
static void debug_panic_runto_cli(char *buf, int32_t len, int32_t argc, char **argv)
{
    if (argc == 2) {
        ...
        printk("set panic_runto_cli flag:0x%x\r\n", g_crash_not_reboot);
    }
}
...
#include "k_proc.h"
extern int ymodem_recv_file(unsigned char *addr);
extern void ymodem_uart_init(void);
...
static void push_cmd(char *buf, int32_t len, int32_t argc, char **argv)
{
    ...
    unsigned char *buffer = (unsigned char *)aos_zalloc(BUF_LEN);
    if (!buffer) {
        printf("%s %d fail\r\n", __func__, __LINE__);
        return;
    }
    ...
    /* start load app in*/
    ret = k_proc_ramload(buffer, NULL);
    if (ret) {
        printf("%s %d  ret : %d\r\n", __func__, __LINE__, ret);
    }
    aos_free(buffer);
