...
static int epoll_close(file_t *file)
{
    ...
    rbtree_postorder_for_each_entry_safe(node, rb_tmp, &pdev->rb_tree, tree_node) {
        epoll_delete_event_cb(node->fd);
        rbt_erase(&node->tree_node, &pdev->rb_tree);
        free(node);
    }
    ...
    switch (data->op) {
        case EPOLL_CTL_ADD: {
            ...
            if (ret < 0) {
                ...
                errno = -ret;
                printf("epoll:register cb failed \r\n");
                ret = -1;
            }
        }
        break;
