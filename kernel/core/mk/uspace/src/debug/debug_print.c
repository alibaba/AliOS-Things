...
#define NOT_NULL 0xA5A6A7A8
/* alios_debug_print depends on mcu*/
static int alios_debug_print_usr(const char *buf, int size)
{
    return aos_print((void *)NOT_NULL, (void *)buf, size);
}
...
#define OUTPUT_STRING(str, len)\
    do { \
        if (!buf) { \
            err = alios_debug_print_usr(str, len); \
            if (err < 0) { \
                goto exit; \
            } else { \
                chars_written += err; \
