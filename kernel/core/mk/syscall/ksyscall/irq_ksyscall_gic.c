...
{
    u_irq_end_interrupt_syscall_arg_t *_arg = arg;
    return __irq_end_interrupt_svc(_arg->irq, _arg->gp_flags);
}

int sys_irq_set_type_stub(void *arg)
{
    u_irq_set_type_syscall_arg_t *_arg = arg;
    return __irq_set_type_svc(_arg->irq, _arg->edge_flags);
}

int sys_irq_set_nmi_stub(void *arg)
{
    u_irq_one_syscall_arg_t *_arg = arg;
    return __irq_set_nmi_svc(_arg->irq);
}

int sys_irq_set_affinity_stub(void *arg)
{
    u_irq_set_affinity_syscall_arg_t *_arg = arg;
    return __irq_set_affinity_global(_arg->irq, _arg->cpumask);
}
