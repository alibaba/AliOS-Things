...
static nftl_block_mgr_t    g_block_mgr[NFTL_PARTITION_MAX] = {
    [NFTL_PARTITION0] = {
        .partition        = NFTL_PHYSICAL_PARTITION0,
        .logic_block_nums = NFTL_TAG_PAGE_IN_BLOCK * NFTL_PART0_BLOCK_NUMS,
        .phy_block_nums   = NFTL_PART0_BLOCK_NUMS + NFTL_PART0_BUFFER_BLOCK_NUMS,
        .initialized      = 0,
    },
    [NFTL_PARTITION1] = {
        .partition        = NFTL_PHYSICAL_PARTITION1,
        .logic_block_nums = NFTL_TAG_PAGE_IN_BLOCK * NFTL_PART1_BLOCK_NUMS,
        .phy_block_nums   = NFTL_PART1_BLOCK_NUMS + NFTL_PART1_BUFFER_BLOCK_NUMS,
        .initialized      = 0,
    },
    [NFTL_PARTITION2] = {
        .partition        = NFTL_PHYSICAL_PARTITION2,
        .logic_block_nums = NFTL_TAG_PAGE_IN_BLOCK * NFTL_PART2_BLOCK_NUMS,
        .phy_block_nums   = NFTL_PART2_BLOCK_NUMS + NFTL_PART2_BUFFER_BLOCK_NUMS,
        .initialized      = 0,
    },
};
...
static void dump_blk_mgr(nftl_block_mgr_t *mgr)
{
    ...
    if (mgr) {
        printf("partition: %d\r\n", mgr->partition);
        printf("logic_block_nums: %d\r\n", mgr->logic_block_nums);
        printf("phy_block_nums: %d\r\n", mgr->phy_block_nums);
        printf("initialized: %d\r\n", mgr->initialized);

        for (i = 0; i < 20; i++) {
            printf("logic: %d, phy: %d, rever: %d\r\n", i, mgr->logic_blocks[i].page_id, mgr->logic_blocks[i].reversion);
        }
    }
}
#endif
...
static uint32_t count_empty_block(nftl_block_mgr_t *mgr)
{
    ...
    for (i = 0; i < mgr->phy_block_nums; i++) {
        ...
        if (!hal_flash_isbad(mgr->partition, (NFTL_BLOCK_SIZE * i)) &&
...
static void nftl_proc_bad_block(nftl_block_mgr_t *blk_mgr, uint32_t blk)
{
    ...
    offset = blk * NFTL_BLOCK_SIZE;

    hal_flash_markbad(blk_mgr->partition, offset);

    blk_mgr->bad_block_nums++;

    NFTL_ERROR("[nftl] %s, process bad block: %d, total bad blocks: %d\r\n", __func__, blk, blk_mgr->bad_block_nums);

    return;
}

static void nftl_remove_logic_mapping(nftl_block_mgr_t *blk_mgr, uint32_t logic_blk, uint32_t blk, uint32_t page)
{
    int ret;
    ...
    if (0 == count_valid_page(blk_mgr, blk)) {

        ret = hal_flash_erase(blk_mgr->partition, blk * NFTL_BLOCK_SIZE, NFTL_BLOCK_SIZE);

        if (ret != 0) {
            NFTL_ERROR("[nftl] %s failed to erase block %d part %d", __func__, blk, blk_mgr->partition);
            hal_flash_markbad(blk_mgr->partition, blk * NFTL_BLOCK_SIZE);
        }
    }
}

static void nftl_add_logic_mapping(nftl_block_mgr_t *blk_mgr, uint32_t logic_blk, uint32_t blk, uint32_t page,
                                   uint32_t reversion)
{
...
static void nftl_create_mapping_tables(nftl_block_mgr_t *blk_mgr, nftl_block_tag *tag, uint32_t blk, uint32_t pg)
{
    uint32_t                blk_off;
    uint32_t                page_off;
...
static void nftl_recover_mapping_from_spare(uint32_t blk, nftl_block_mgr_t *blk_mgr)
{
    ...
    r_off = blk * NFTL_BLOCK_SIZE;

    /*  read start from the first page */
    for (i = 0; i < NFTL_TAG_PAGE_IN_BLOCK; i++) {

        ret = hal_flash_read_with_spare(blk_mgr->partition, &r_off, blk_mgr->page_buff, NFTL_PAGE_SIZE, blk_mgr->spare_buff,
                                        NFTL_SPARE_AREA_LEN);
...
static nftl_err_t nftl_immigrate_block(nftl_block_mgr_t *blk_mgr, uint32_t old_blk)
{
    ...
    do {
        ...
        for (i = 0; i < NFTL_TAG_PAGE_IN_BLOCK; i++) {
            ...
            if (nftl_isvalid_bitmap(blk_mgr->phy_blocks[old_blk].page_bitmap, i)) {
                ...
                NFTL_ASSERT(r_off == old_blk * NFTL_BLOCK_SIZE + i * NFTL_PAGE_SIZE);
                /* read the old block pages */
                ret = hal_flash_read_with_spare(blk_mgr->partition, &r_off, blk_mgr->page_buff, NFTL_PAGE_SIZE, blk_mgr->spare_buff,
                                                NFTL_SPARE_AREA_LEN);

                if (0 != ret) {
                    NFTL_ERROR("[fntl] %s, read error in immigrate: %d\r\n", __func__, ret);
                    continue;
                }
                ...
                if (NFTL_ERR_OK != nftl_alloc_new_page(blk_mgr, &page_id)) {
                    ...
                    NFTL_ERROR("[nftl] %s: no pages for immigrate", __func__);

                    return NFTL_ERR_NAND_NO_PAGE;
                }

                w_off = page_id * NFTL_PAGE_SIZE;

                /* write the valid pages into the allocating block */
                ret = hal_flash_write_with_spare(blk_mgr->partition, &w_off, blk_mgr->page_buff, NFTL_PAGE_SIZE, blk_mgr->spare_buff,
                                                 NFTL_SPARE_AREA_LEN);

                if (0 != ret) {

                    NFTL_TRACE("[%s]bad block found, w_off: %d", __func__, w_off);
                ...
                nftl_add_logic_mapping(blk_mgr, tag->logic_block, page_id / NFTL_PAGES_PER_BLK, page_id % NFTL_PAGES_PER_BLK,
                                       tag->reversion);

                blk_mgr->logic_blocks[tag->logic_block].page_id = page_id;
                blk_mgr->logic_blocks[tag->logic_block].reversion = tag->reversion;
            } else {
                r_off += NFTL_PAGE_SIZE;
            }
        }
    } while (ret);
    ...
    ret = hal_flash_erase(blk_mgr->partition, old_blk * NFTL_BLOCK_SIZE, NFTL_BLOCK_SIZE);
    if (ret != 0) {
        NFTL_ERROR("failed to erase block %d part %d", old_blk, blk_mgr->partition);
        hal_flash_markbad(blk_mgr->partition, old_blk * NFTL_BLOCK_SIZE);
    }
...
static void nftl_update_tagpage(nftl_block_mgr_t *blk_mgr, uint32_t blk)
{
    ...
    do {
        if (blk_mgr->allocating_page != NFTL_TAG_PAGE_IN_BLOCK) {
            NFTL_TRACE("[nftl] %s, allocating_page: %d\r\n", __func__, blk_mgr->allocating_page);
        }

        w_off = blk * NFTL_BLOCK_SIZE + NFTL_TAG_PAGE_IN_BLOCK * NFTL_PAGE_SIZE;
        ...
        for (i = 0; i < NFTL_TAG_PAGE_IN_BLOCK; i++) {
            ...
            hal_flash_read_with_spare(blk_mgr->partition, &r_off, blk_mgr->page_buff, NFTL_PAGE_SIZE, blk_mgr->spare_buff,
                                      NFTL_SPARE_AREA_LEN);
        ...
        ret = hal_flash_write(blk_mgr->partition, &w_off, (void *)blk_tags, sizeof(nftl_block_tag) * NFTL_TAG_PAGE_IN_BLOCK);
...
static void nftl_complete_gc(nftl_block_mgr_t *blk_mgr)
{
    ...
    for (i = 0; i < blk_mgr->phy_block_nums - 1; i++) {
        ...
        if (empty_blocks >= 8) {
            NFTL_WARN("%s end complete gc, part: %d, empty blocks: %d\r\n", __func__, blk_mgr->partition, empty_blocks);
            break;
        }
        ...
        if ((gc_blk != blk_mgr->allocating_blk) &&
            ...
            NFTL_WARN("%s complete gc, part: %d, empty blocks: %d, gc pages: %d", __func__, blk_mgr->partition, empty_blocks,
                      valid_pages);
            ...
            for (j = 0; j < NFTL_TAG_PAGE_IN_BLOCK; j++) {
                ...
                if (nftl_isvalid_bitmap(blk_mgr->phy_blocks[gc_blk].page_bitmap, j)) {
                    ...
                    r_off = gc_blk * NFTL_BLOCK_SIZE + j * NFTL_PAGE_SIZE;
                    ...
                    if (0 != ret) {
                        NFTL_ERROR("[fntl] %s, read error in complete gc: %d", __func__, ret);
...
static void nftl_gc_start(nftl_block_mgr_t *blk_mgr)
{
    ...
    for (i = 0; i < (blk_mgr->phy_block_nums - 1); i++) {
        ...
        if (valid_pages > 0) {
            ...
            if (min_pages == 0 || min_pages > valid_pages) {
    ...
    if (blk_mgr->gc_times % 50 == 0) {
        ...
        NFTL_WARN("[%s] patition: %d, dirtiest block: %d, min pages: %d, empty blocks: %d, gc times: %d", __func__,
                  blk_mgr->partition, dirtiest_block, min_pages, empty_blocks, blk_mgr->gc_times);
    }
    ...
    for (i = 0; i < NFTL_TAG_PAGE_IN_BLOCK; i++) {
        ...
        if (nftl_isvalid_bitmap(blk_mgr->phy_blocks[dirtiest_block].page_bitmap, i)) {
            ...
            r_off = dirtiest_block * NFTL_BLOCK_SIZE + i * NFTL_PAGE_SIZE;
            ...
            if (0 != ret) {
                NFTL_ERROR("[fntl] %s, read error in immigrate: %d", __func__, ret);
...
static uint32_t nftl_find_empty_block(nftl_block_mgr_t *blk_mgr)
{
    ...
    for (i = 0; i < blk_mgr->phy_block_nums - 1; i++) {
        ...
        offset = new_blk * NFTL_BLOCK_SIZE;
...
static uint32_t nftl_alloc_new_block(nftl_block_mgr_t *blk_mgr)
{
    ...
    do {
        ...
        NFTL_TRACE("[%s] erasing offset %d in part %d, blk: %d", __func__,
                   offset, blk_mgr->partition, blk_mgr->allocating_blk);

        ret = hal_flash_erase(blk_mgr->partition, offset, NFTL_BLOCK_SIZE);

        if (ret) {
            NFTL_ERROR("[%s] bad block %d found in part %d, ret: %d", __func__,
...
static nftl_err_t nftl_alloc_new_page(nftl_block_mgr_t *blk_mgr, uint32_t *page_id)
{
    ...
    if (blk_mgr->allocating_blk == NFTL_INVALID_BLOCK) {
        NFTL_ERROR("[nftl] %s allocating blk is invalid, empty blocks: %d", __func__, count_empty_block(blk_mgr));
...
static nftl_err_t nftl_init_partition(nftl_block_mgr_t *blk_mgr)
{
    ...
    for (i = 0; i < blk_mgr->phy_block_nums; i++) {
        ...
        r_off = i * NFTL_BLOCK_SIZE + NFTL_TAG_PAGE_IN_BLOCK * NFTL_PAGE_SIZE;

        if (hal_flash_isbad(blk_mgr->partition, r_off)) {
            blk_mgr->bad_block_nums++;
            NFTL_TRACE("[%s] bad block found: %d, total bad blocks: %d", __func__, i, blk_mgr->bad_block_nums);
            continue;
        }

        ret = hal_flash_read(blk_mgr->partition, &r_off, blk_mgr->page_buff, NFTL_PAGE_SIZE);
                    ...
                    void *outbuf, nftl_size_t len)
{
    ...
    while (len > 0) {
        ...
        if (phy_blk == NFTL_INVALID_BLOCK) { /*  erased pending write state */
            NFTL_DEBUG("[%s] logic block %d not be mapped in part %d, part_in: %d",
        ...
        ret = hal_flash_read(blk_mgr->partition, &r_off, p_buf, r_len);

        if (ret != 0) {
            NFTL_ERROR("[%s] read error, blk: %d, readlen: %d", __func__, blk, r_len);
            nftl_unlock(&blk_mgr->par_lock);
            return ret;
        }
...
{
    NFTL_ERROR("[XXXXXXXXXXXXXXXXXXX]flash notify %s part - %d block - %d", __func__, p, blk);
...
void nftl_statistics(void)
{
    ...
    for (partition = 0; partition < 3; partition++) {
        ...
        printf("%15u%15u%9u(%3u%%)%9u(%3u%%)%15u%9u(%3u%%)%15u\n",
               blk_mgr->partition,
...
static void handle_nftl_cmd(char *pwbuf, int blen, int argc, char **argv)
{
    nftl_statistics();
}

static struct cli_command    nftl_cmd[] = {"nftl_stat", "statistic nftl partition takeup info", handle_nftl_cmd};
...
void nftl_init(void)
{
    ...
    for (partition = NFTL_PARTITION0; partition < NFTL_PARTITION_MAX; partition++) {
        blk_mgr = &g_block_mgr[partition];
    ...
    aos_cli_register_command(&nftl_cmd);
