...
/* should be same as ./components/musllibc/project/src/internal/aos/vfs.h */
...
typedef const struct network_ops network_ops_t;

union inode_ops_t {
    const file_ops_t    *i_ops;  /* char driver operations */
    const fs_ops_t      *i_fops; /* FS operations */
    const network_ops_t *i_nops; /* network operations */
};
...
struct file_ops {
    ...
    //TODO: fix to off_t (*lseek)(file_t *fp, off_t off, int whence);
...
/**
 * Write the contents of a buffer to console
 *
 * @param[in]  nbytes  the number of bytes to write.
 * @param[in]  buf     the buffer to write from.
 *
 ...
 *
 */
ssize_t aos_console_write(const void *buf, size_t nbytes);
 ...
 *
 */
int aos_fsync(int fd);

/**
 * Flush all fils system file buffers.
 */
void aos_sync(void);
 ...
 */
void *aos_mmap(void *start, size_t len, int prot, int flags, int fd, off_t offset);
 ...
 * @return 0 on success, negative error on failure
 */
int aos_munmap(void *start, size_t len);
 ...
 *
 * @return  fd dircetory file descriptor value, negative error code on failure.
 *
 */
int aos_dirfd(aos_dir_t *dir);
...
/**
 * @brief union register driver from the file or device
 *
 * @param[in]  path  the path of the file or device
 * @param[in]  ops   the driver operations to bind
 * @param[in]  arg   the arguments of the driver operations
 * @param[in]  main_service_name   the arguments of the driver main rpc service name
 * @param[in]  device_service_name   the arguments of the driver device rpc service name
 *
 * @return 0 on success, negative error on failure
 *
 */
int aos_union_register_driver(const char *path, file_ops_t *ops, void *arg, char *main_service_name,
                              char *device_service_name);

/**
 * @brief union unregister driver from the file or device
 *
 * @param[in]  path  the path of the file or device
 *
 * @return 0 on success, negative error on failure
 *
 */
int aos_union_unregister_driver(char *path);
...
/**
 * @brief register network operations
 *
 * @param[in]  funcops  the network function operations
 * @param[in]  rpcops   the network rpc operations
 *
 * @return 0 on success, negative error on failure
 *
 */
int aos_register_network(network_ops_t *funcops, network_ops_t *rpcops);

/**
 * @brief unregister the network operations
 *
 *
 * @return 0 on success, negative error on failure
 *
 */
int aos_unregister_network(void);
...
/**
 * @brief  dump open files data
 *
 */
void aos_dump_open(void);

/**
 * @brief  init file system rpc service
 *
 * @return 0 on success, negative error on failure
 *
 */
int fs_rpc_service_init(void);
