...
static void timer_list_pri_insert(klist_t *head, ktimer_t *timer)
{
    ...
    for (q = start->next; q != end; q = q->next) {
        ...
        if ((task_iter_temp->match - u_timer_count) > val) {
            break;
        }
    }
...
static kstat_t timer_create(ktimer_t *timer, const name_t *name,
                            timer_cb_t cb, sys_time_t first,
                            sys_time_t round, void *arg,
                            uint8_t auto_run, uint8_t mm_alloc_flag)
{
...
kstat_t krhino_timer_create(ktimer_t *timer, const name_t *name,
                            timer_cb_t cb, sys_time_t first,
                            sys_time_t round, void *arg,
                            uint8_t auto_run)
{
    return timer_create(timer, name, cb, first, round, arg,
                        auto_run, K_OBJ_STATIC_ALLOC);
}
                                ...
                                uint8_t auto_run)
{
    ...
    timer_obj = malloc(sizeof(ktimer_t));
    if (timer_obj == NULL) {
        return RHINO_NO_MEM;
    }

    ret = timer_create(timer_obj, name, cb, first, round, arg,
                       auto_run, K_OBJ_DYN_ALLOC);
...
kstat_t krhino_timer_change(ktimer_t *timer, sys_time_t first,
                            sys_time_t round)
{
    k_timer_queue_cb cb;
    kstat_t err;
...
static void timer_cb_proc(void)
{
    ...
    start = end = &u_timer_head;

    for (q = start->next; q != end; q = q->next) {
        timer = krhino_list_entry(q, ktimer_t, timer_list);
        delta = (sys_time_i_t)timer->match - (sys_time_i_t)u_timer_count;
        ...
        if (delta <= 0) {
            ...
            if (timer->round_ticks > 0u) {
                ...
                timer->match   =  u_timer_count + timer->remain;
...
static void cmd_proc(k_timer_queue_cb *cb, uint8_t cmd)
{
    ...
    switch (cmd) {
        case TIMER_CMD_START:
            ...
            timer->match   =  u_timer_count + timer->init_count;
            /* sort by remain time */
...
static void timer_task(void *pa)
{
    ...
    while (RHINO_TRUE) {
        ...
        while (!is_klist_empty(&u_timer_head)) {
            timer = krhino_list_entry(u_timer_head.next, ktimer_t, timer_list);
...
int timer_init(void)
{
    ...
    ustack_buf = malloc(RHINO_CONFIG_TIMER_TASK_STACK_SIZE * sizeof(cpu_stack_t));
    if (NULL == ustack_buf) {
        ret = -2;
        goto del_queue;
    }

    ret = krhino_utask_create(&u_timer_task, "timer_task", NULL,
