...
int mbedtls_hmac_starts(mbedtls_hash_context_t *ctx, const unsigned char *key,
                        size_t keylen)
{
...
int mbedtls_hmac_update(mbedtls_hash_context_t *ctx, const unsigned char *input,
                        size_t ilen)
{
    if (ctx == NULL || ctx->md_info == NULL || ctx->hmac_ctx == NULL)
        return (MBEDTLS_ERR_MD_BAD_INPUT_DATA);
...
int mbedtls_hash_hmac(const mbedtls_hash_info_t *md_info,
