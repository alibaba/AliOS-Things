...
#ifndef CONFIG_PM_ASH_SCRIPT_FILE_NAME
    #define PM_ASH_SCRIPT_FILE_NAME "/data/aos.d"
#else
    #define PM_ASH_SCRIPT_FILE_NAME CONFIG_PM_ASH_SCRIPT_FILE_NAME
#endif
...
static void pm_auto_load_resolve(int argc, char *argv[])
{
    char * arg_load;
    char * arg_name;
    int ret;
    ...
    arg_load = argv[0];
    arg_name = argv[1];
    if(NULL == arg_load || NULL == arg_name){
        return;
    }

    if(strncmp("loadm", arg_load, 5) == 0){
        ret = aos_uproc_load(arg_name);
        if(ret > 0 ){
            printf("[Load:] process %s auto load successful!\r\n", arg_name);
        }
    }
...
//#endif
        ...
        pm_auto_load_resolve(argc, argv);
    }
...
void aos_pm_ash_script(void)
{
    interpret_script(PM_ASH_SCRIPT_FILE_NAME);
    return ;
}
