...
/** @brief    Initialises a mutex
  *
  * @Details  A mutex can be nested different to a semaphore.
  *
...
/** @brief    Obtains the lock on a mutex
  *
  * @Details  Trying to obtain the mutex, if the lock can not be got,
  * the thread be suspended until calling thread will be suspended until the mutex.
  *           mutex is released by the other thread.
...
/** @brief    Releases the lock on a mutex
  *
  * @Details  Releases a mutex and wake up other threads waiting for this mutex.
  *
...
/** @brief    Initialises a FIFO queue
  ...
  * @param    message_size : size of msg
  * @param    number_of_messages : num of the msg
  *
  * @return   kNoErr        : on success.
  * @return   kGeneralErr   : if an error occurred
  */
...
/** @brief    Pushes an object onto a queue
  *
  * @param    queue : pointer to the queue
  * @param    message : the buffer push to queue
  * @param    timeout_ms: the ms of timeout
  *
  * @return   kNoErr        : on success.
 ...
 *
 * This function is used to register time event, which can be triggered periodically.
 *
 * @param event_object  : the time event handle
 * @param worker_thread : the worker thread to run work time event
 * @param function      : the callback function
 * @param time_ms       : the time interval
 * @param arg           : an parameter supplied to the function
 *
 * @return    kNoErr        : on success.
 * @return    kGeneralErr   : if an error occurred
 */
 ...
 *
 * This function de-registers a time event that has been created before
 *
 * @param event_object : the time event handle
 *
 * @return    kNoErr        : on success.
