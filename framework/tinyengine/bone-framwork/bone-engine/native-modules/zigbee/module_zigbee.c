...
//  bone
//
//  Created by howie on 31/01/2018.
...
static void on_message(bone_zigbee_t* zigbee, uint16_t addr, unsigned char* msg, size_t msg_size)
{
    be_debug(JS_ZIGBEE_TAG, "on_message addr: %d, msg: %.*s (%d bytes)", addr, msg_size, msg, msg_size);
...
static void on_connect(bone_zigbee_t* zigbee, uint16_t addr, unsigned char* msg, size_t msg_size)
{
    be_debug(JS_ZIGBEE_TAG, "on_connect addr: %d, msg: %.*s (%d bytes)", addr, msg_size, msg, msg_size);
