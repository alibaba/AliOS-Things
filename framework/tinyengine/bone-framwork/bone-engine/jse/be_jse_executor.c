...
static void be_jse_vm_init(be_jse_executor_ctx_t *executor,be_jse_lexer_ctx_t *lex) 
{
...
static be_jse_symbol_t *handle_base_expr() 
{
    ...
    while (MEET_BIT_ARITHMATICAL_OP || MEET_LOGICAL_OP) {
        bool short_circuit = false; /* 逻辑判断短路原则：if (0&&(a>b)&&a>0))
                                       或者   if(1||(a>0)) */
        bool boolean = false;
        int op = vm.lex->tk;
...
static be_jse_node_t jswrap_modules_getModuleList(be_jse_executor_ctx_t *executor) 
{
...
be_jse_symbol_t *be_jse_require(be_jse_executor_ctx_t *executor, const char *moduleName) 
{
...
be_jse_symbol_t *be_jse_add_module(be_jse_executor_ctx_t *executor,be_jse_symbol_t *id, be_jse_symbol_t *code) 
{
    be_jse_node_t moduleList = jswrap_modules_getModuleList(executor);
...
be_jse_symbol_t *jspEvaluateModule(be_jse_executor_ctx_t *executor,const char *str) 
{
    be_execflag_e execute = EXEC_NORMAL;
    be_jse_lexer_ctx_t lex;
    be_jse_symbol_t *s = 0;
