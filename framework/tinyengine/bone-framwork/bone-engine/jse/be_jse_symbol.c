...
static be_jse_symbol_t * get_symbol_addr(be_jse_node_t ref)
{
	...
	return & gSymbolBlocks[t >> BE_JSE_SYMBOL_TABLE_BLOCK_SHIFT][t & (BE_JSE_SYMBOL_TABLE_BLOCK_SIZE - 1)];
#else
...
static be_jse_node_t symbol_table_init(be_jse_node_t start, unsigned int count)
{
	...
	get_symbol_addr((be_jse_node_t) (start + count - 1))->next_sibling = (be_jse_node_t) 0; // set the final one to 0
	return start;
}
...
be_jse_node_t get_symbol_node_id(be_jse_symbol_t * s)
{
	...
	for (i = 0; i < c; i++)
	{
		if (s >= gSymbolBlocks[i] && s < &gSymbolBlocks[i][BE_JSE_SYMBOL_TABLE_BLOCK_SIZE])
		{
			be_jse_node_t	r	= (be_jse_node_t) (1 + (i << BE_JSE_SYMBOL_TABLE_BLOCK_SHIFT) + (s - gSymbolBlocks[i]));
			return r;
		}
	}
	return 0;
...
void be_jse_symbol_table_init()
{
	...
	gSymbolBlocks[0]		= calloc(1, sizeof(be_jse_symbol_t) *BE_JSE_SYMBOL_TABLE_BLOCK_SIZE);
#else
    memset(gSymbolTable, 0, sizeof(gSymbolTable));

#endif

	gSymbolTableFirstFreeSlotID = symbol_table_init(1 /*first*/, gSymbolTableSize);

    printf("%s %d \n", __FUNCTION__, __LINE__);
	printf("gSymbolTableSize = %d  beJSESymbolUsage=%d gSymbolTableFirstFreeSlotID = %d \n", gSymbolTableSize, beJSESymbolUsage, gSymbolTableFirstFreeSlotID);
}
...
void be_jse_set_symbol_table_size(unsigned int newSize)
{
	...
	if (newSize <= gSymbolTableSize)
		return; // never allow us to have less!

	// When resizing, we just allocate a bunch more
	unsigned int	oldSize = gSymbolTableSize;
	unsigned int	oldBlockCount = gSymbolTableSize >> BE_JSE_SYMBOL_TABLE_BLOCK_SHIFT;
	unsigned int	newBlockCount = (newSize + BE_JSE_SYMBOL_TABLE_BLOCK_SIZE - 1) >> BE_JSE_SYMBOL_TABLE_BLOCK_SHIFT;

	gSymbolTableSize			= newBlockCount << BE_JSE_SYMBOL_TABLE_BLOCK_SHIFT;

	// resize block table
	gSymbolBlocks 		= realloc(gSymbolBlocks, sizeof(be_jse_symbol_t *) *newBlockCount);
	...
	for (i = oldBlockCount; i < newBlockCount; i++){
		gSymbolBlocks[i] = calloc(1, sizeof(be_jse_symbol_t) *BE_JSE_SYMBOL_TABLE_BLOCK_SIZE);
	...
	gSymbolTableFirstFreeSlotID 	= symbol_table_init(oldSize + 1, gSymbolTableSize - oldSize);
...
static be_jse_symbol_t *new_symbol_node()
{
	...
	printf("gSymbolTableSize = %d  beJSESymbolUsage=%d \n", gSymbolTableSize, beJSESymbolUsage);

    be_jse_set_symbol_table_size(gSymbolTableSize + BE_JSE_SYMBOL_TABLE_BLOCK_SIZE);
...
{
    be_assert( symbol_is_string_ext(s) || (!s->next_sibling && !s->prev_sibling));
    ...
    if (symbol_is_string(s) || symbol_is_string_ext(s)) {
        ...
        if (stringDataRef) {
            ...
            child->prev_sibling = 0; // these pointers may contain rubbish as StringEXT uses them for extra characters
            child->next_sibling = 0;
            DEC_SYMBL_REF(child);
            symbol_unlock(child);
        }

    } else if (symbol_is_object(s) || symbol_is_function(s) || symbol_is_array(s) || symbol_is_root(s)) {
...
be_jse_symbol_t *new_json_symbol(char* json_str, size_t json_str_len)
{
    ...
    ret = be_jse_eval_string(bone_engine_get_executor(), evalstr);
    free(evalstr);
...
static ALWAYS_INLINE be_jse_symbol_t* _new_func_code_symbol(be_jse_lexer_ctx_t *lex, int charFrom, int charTo)
{
    be_jse_symbol_t *first = NULL;

    int len = charTo - charFrom;
    char* buf = malloc(len+1); // 需要 +1
...
be_jse_symbol_t* new_func_code_symbol(be_jse_lexer_ctx_t *lex, int charFrom, int charTo)
{
...
be_jse_symbol_t *symbol_lock(be_jse_node_t id)
{
    ...
    if (s->locks==0) {
        //printf("[%s][%d] id = %d ... \n", __FUNCTION__, __LINE__, id);
        //printf("[%s][%d] locks = %d ... \n", __FUNCTION__, __LINE__, s->locks);
        //printf("[%s][%d] refs = %d ... \n", __FUNCTION__, __LINE__, s->refs);
...
ALWAYS_INLINE be_jse_int_t get_symbol_int(be_jse_symbol_t *v)
{
    ...
    if (symbol_is_array(v) && get_symbol_array_length(v) )
        return 1;
