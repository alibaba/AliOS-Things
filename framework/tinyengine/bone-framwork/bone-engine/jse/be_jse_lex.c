...
//初始化词法分析器
void be_jse_lexer_init(be_jse_lexer_ctx_t *lex, const char *src, int start, int end)
{
...
void be_jse_lexer_init2(be_jse_lexer_ctx_t *lex, be_jse_lexer_ctx_t *from, int start)
{
...
// 获取行与列信息便于查看出错位置
void lexer_dump_line_and_col(be_jse_lexer_ctx_t *lex, int charPos, int *line, int *col)
{
#ifdef BE_JSE_SILENT
    NOT_USED(lex);
    NOT_USED(charPos);
