...
static void task_uart_recv(int content_length)
{
    ...
    while (1) {
        ...
        int status = hal_uart_recv_II(&uart_stdio, buf, sizeof(buf),
                                      &recv_bytes, 0xFFFFFFFF);
        if (status == 0) {
...
static void handle_cli_cmd(char *pwbuf, int blen, int argc, char **argv)
{
    if (argc == 2) {
        ...
        // aos_cli_stop();
        be_osal_new_task("be-debuger-uart", task_uart_recv, (char *)argv[1],
                         8192, NULL);
    } else {
...
static struct cli_command be_uploadcmd = { .name = BE_DEBUGER_UPLOAD_CMD,
                                           .help =
                                             "be-debuger-upload 579123(size)",
...
static void on_udp_recv(void *arg, struct udp_pcb *upcb, struct pbuf *p,
...
static const char *http_headers_value(struct list_head *headers,
                                      const char *      field)
{
...
static int on_http_req_url(http_parser *parser, const char *at, size_t length)
{
    ...
    strncpy(req->url, at,
            length > sizeof(req->url) ? sizeof(req->url) : length);
    return 0;
}
...
static int on_http_header_field(http_parser *parser, const char *at,
                                size_t length)
{
    be_debug_http_header_t *header =
      (be_debug_http_header_t *)calloc(1, sizeof(be_debug_http_header_t));
    strncpy(header->field, at, length);
    list_add_tail(&header->list,
                  &((be_debug_http_req_t *)parser->data)->headers);
    return 0;
}
                                ...
                                size_t length)
{
    ...
    list_for_each_entry_reverse(
      header, &((be_debug_http_req_t *)parser->data)->headers, list)
    {
        size_t value_len = sizeof(header->value);
        strncpy(header->value, at,
                length >= value_len ? value_len - 1 : length);
        break;
    }
...
static void send_response(char *body, be_debug_http_req_t *req)
{
    be_debug_http_client_t *client =
      (be_debug_http_client_t *)container_of(req, be_debug_http_client_t, req);
    ...
    list_for_each_entry(client, &clients, list)
    {
        if (client->sock == sock) {
            ...
            if (recv_bytes == 0 ||
                ...
                http_parser_execute(parser, &settings, (char *)buf,
                                    strlen((char *)buf));
                ...
                if (parser->method == HTTP_POST) {
                    ...
                    const char *content_type =
                      http_headers_value(&req->headers, "content-type");
                    ...
                    if (strstr(content_type, "multipart/form-data") != NULL) {
                        ...
                        if (start != NULL) {
                            ...
                            if (req->content_nread > req->content_length) {
                                if (recv_complete_cb)
                                    recv_complete_cb(buf, req->content_length -
                                                            nread);
                                http_respone(req);
                            } else if (req->content_nread ==
                                       req->content_length) {
                                if (recv_complete_cb)
