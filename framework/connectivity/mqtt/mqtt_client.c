...
static int             iotx_mc_push_pubInfo_to(iotx_mc_client_t *c, int len,
                                               unsigned short msgId, list_node_t **node);
static int             iotx_mc_push_subInfo_to(iotx_mc_client_t *c, int len,
...
int MQTTConnect(iotx_mc_client_t *pClient)
{
    ...
    HAL_MutexLock(pClient->lock_write_buf);
    if ((len = MQTTSerialize_connect((unsigned char *)pClient->buf_send,
...

{
    ...
    if (len <= 0) {
        ...
        log_err(
          "MQTTSerialize_publish is error, len=%d, buf_size=%u, payloadlen=%u",
                         ...
                         void *                          pcontext)
{
    ...
    iotx_time_t            timer;
    MQTTString             topic   = MQTTString_initializer;
    iotx_mc_topic_handle_t handler = { topicFilter,
                                       { messageHandler, pcontext } };
    ...
    len =
      MQTTSerialize_subscribe((unsigned char *)c->buf_send, c->buf_size_send, 0,
                              (unsigned short)msgId, 1, &topic, (int *)&qos);
    if (len <= 0) {
        HAL_MutexUnlock(c->lock_write_buf);
    ...
    /*
     * NOTE: It prefer to push the element into list and then remove it when
     * send failed, because some of extreme cases
     * */

    /* push the element to list of wait subscribe ACK */
    if (SUCCESS_RETURN !=
        iotx_mc_push_subInfo_to(c, len, msgId, SUBSCRIBE, &handler, &node)) {
        log_err("push publish into to pubInfolist failed!");
        HAL_MutexUnlock(c->lock_write_buf);
        return MQTT_PUSH_TO_LIST_ERROR;
    }
                           ...
                           unsigned int msgId)
{
    ...
    if ((len = MQTTSerialize_unsubscribe(
           ...
           (unsigned short)msgId, 1, &topic)) <= 0) {
        HAL_MutexUnlock(c->lock_write_buf);
        return MQTT_UNSUBSCRIBE_PACKET_ERROR;
    }

    if (SUCCESS_RETURN !=
        iotx_mc_push_subInfo_to(c, len, msgId, UNSUBSCRIBE, &handler, &node)) {
        log_err("push publish into to pubInfolist failed!");
        HAL_MutexUnlock(c->lock_write_buf);
        return MQTT_PUSH_TO_LIST_ERROR;
    }
...
static int MQTTDisconnect(iotx_mc_client_t *c)
{
    ...
    HAL_MutexLock(c->lock_write_buf);
    int len =
      MQTTSerialize_disconnect((unsigned char *)c->buf_send, c->buf_size_send);
...
static int iotx_mc_mask_pubInfo_from(iotx_mc_client_t *c, uint16_t msgId)
{
    ...
    if (c->list_pub_wait_ack->len) {
        ...
        if (NULL ==
            (iter = list_iterator_new(c->list_pub_wait_ack, LIST_TAIL))) {
            HAL_MutexUnlock(c->lock_list_pub);
            return SUCCESS_RETURN;
        }
                                   ...
                                   unsigned short msgId, list_node_t **node)
{
    ...
    iotx_mc_pub_info_t *repubInfo =
      (iotx_mc_pub_info_t *)LITE_malloc(sizeof(iotx_mc_pub_info_t) + len);
    if (NULL == repubInfo) {
        HAL_MutexUnlock(c->lock_list_pub);
                                   ...
                                   list_node_t **          node)
{
    ...
    iotx_mc_subsribe_info_t *subInfo = (iotx_mc_subsribe_info_t *)LITE_malloc(
      sizeof(iotx_mc_subsribe_info_t) + len);
    if (NULL == subInfo) {
        HAL_MutexUnlock(c->lock_list_sub);
        log_err("run iotx_memory_malloc is error!");
        return FAIL_RETURN;
...
/* return: 0, success; NOT 0, fail; */
static int iotx_mc_mask_subInfo_from(iotx_mc_client_t *c, unsigned int msgId,
                                     iotx_mc_topic_handle_t *messageHandler)
{
    ...
    if (c->list_sub_wait_ack->len) {
        ...
        if (NULL ==
            (iter = list_iterator_new(c->list_sub_wait_ack, LIST_TAIL))) {
            HAL_MutexUnlock(c->lock_list_sub);
            return SUCCESS_RETURN;
        }
                               ...
                               unsigned int *packet_type)
{
    ...
    if ((rem_len > 0) && ((rem_len + len) > c->buf_size_read)) {
        log_err("mqtt read buffer is too short, mqttReadBufLen : %u, "
                "remainDataLen : %d",
...
static int iotx_mc_handle_recv_SUBACK(iotx_mc_client_t *c)
{
    ...
    if ((NULL == messagehandler.handle.h_fp) ||
        (NULL == messagehandler.topic_filter)) {
    ...
    if (0 == flag_dup) {
        iotx_mc_topic_handle_t *handle =
          LITE_malloc(sizeof(iotx_mc_topic_handle_t));
        if (!handle) {
            HAL_MutexUnlock(c->lock_generic);
            return FAIL_RETURN;
        }
...
static int remove_handle_from_list(iotx_mc_client_t *      c,
                                   iotx_mc_topic_handle_t *h)
{
...
/* subscribe */
static int iotx_mc_subscribe(iotx_mc_client_t *c, const char *topicFilter,
                             iotx_mqtt_qos_t                 qos,
...
/* publish */
static int iotx_mc_publish(iotx_mc_client_t *c, const char *topicName,
                           iotx_mqtt_topic_info_pt topic_msg)
{
    uint16_t msg_id = 0;
    int      rc     = FAIL_RETURN;
                                      ...
                                      MQTTPacket_connectData *pConnectParams)
{
    ...
    if (pConnectParams->keepAliveInterval < KEEP_ALIVE_INTERVAL_DEFAULT_MIN) {
        ...
        pClient->connect_data.keepAliveInterval =
          KEEP_ALIVE_INTERVAL_DEFAULT_MIN;
    } else if (pConnectParams->keepAliveInterval >
               KEEP_ALIVE_INTERVAL_DEFAULT_MAX) {
...
static int MQTTSubInfoProc(iotx_mc_client_t *pClient)
{
    ...
    do {
        ...
        if (NULL ==
            (iter = list_iterator_new(pClient->list_sub_wait_ack, LIST_TAIL))) {
        ...
        for (;;) {
            ...
            iotx_mc_subsribe_info_t *subInfo =
              (iotx_mc_subsribe_info_t *)node->val;
            if (NULL == subInfo) {
                log_err("node's value is invalid!");
                tempNode = node;
                continue;
            ...
            if (utils_time_spend(&subInfo->sub_start_time) <=
...
static int MQTTPubInfoProc(iotx_mc_client_t *pClient)
{
    ...
    do {
        ...
        if (NULL ==
            (iter = list_iterator_new(pClient->list_pub_wait_ack, LIST_TAIL))) {
            log_err("new list failed");
            break;
        }
        ...
        for (;;) {
            ...
            /* check the request if timeout or not */
            if (utils_time_spend(&repubInfo->pub_start_time) <=
                (pClient->request_timeout_ms * 2)) {
                continue;
            }
...
static void cb_recv_timeout(void *arg)
{
    ...
    aos_post_delayed_action(pClient->connect_data.keepAliveInterval * 1000,
                            cb_recv_timeout, arg);
}
...
static void cb_recv(int fd, void *arg)
{
    ...
    if (ret) {
        ...
        aos_cancel_poll_read_fd(get_iotx_fd(), cb_recv, pClient);
        // aos_cancel_poll_read_fd(HAL_SSL_GetFd(pClient->ipstack->handle),
        // cb_recv, pClient);
        return;
    }
    // LOG("system heap_size %d, iram mimi free heap size:%d,iram free heap size
    // :%d", system_get_free_heap_size(), xPortGetMinimumEverFreeHeapSize(),
    // xPortGetFreeHeapSize());
    aos_cancel_delayed_action(pClient->connect_data.keepAliveInterval * 1000,
                              cb_recv_timeout, arg);
    aos_post_delayed_action(pClient->connect_data.keepAliveInterval * 1000,
                            cb_recv_timeout, arg);
}
...
static int iotx_mc_connect(iotx_mc_client_t *pClient)
{
    ...
    /* remove */
    /*log_debug("start MQTT connection with parameters: clientid=%s,
       username=%s, password=%s", pClient->connect_data.clientID.cstring,
...
static int iotx_mc_attempt_reconnect(iotx_mc_client_t *pClient)
{
    ...
    log_info("reconnect params: MQTTVersion=%d, clientID=%s, "
             "keepAliveInterval=%d, username=%s",
...
static int iotx_mc_handle_reconnect(iotx_mc_client_t *pClient)
{
    ...
    interval_ms +=
      HAL_Random(pClient->reconnect_param.reconnect_time_interval_ms);
...
static int iotx_mc_report_mid(iotx_mc_client_t *pclient)
{
    ...
    iotx_midreport_reqid(requestId, dev->product_key, dev->device_name);
    ...
    iotx_midreport_payload(msg, requestId, mid, pid);
    ...
    ret =
      iotx_midreport_topic(topic_name, "", dev->product_key, dev->device_name);
...
int IOT_MQTT_Yield(void *handle, int timeout_ms)
{
    ...
    if (SUCCESS_RETURN == rc) {
        ...
        /* check list of wait subscribe(or unsubscribe) ACK to remove node that
         * is ACKED or timeout */
...
int IOT_MQTT_Subscribe(void *handle, const char *topic_filter,
                       iotx_mqtt_qos_t                 qos,
                       iotx_mqtt_event_handle_func_fpt topic_handle_func,
                       void *                          pcontext)
{
    ...
    if (qos > IOTX_MQTT_QOS2) {
        log_warning("Invalid qos(%d) out of [%d, %d], using %d", qos,
                    IOTX_MQTT_QOS0, IOTX_MQTT_QOS2, IOTX_MQTT_QOS0);
        qos = IOTX_MQTT_QOS0;
    }

    return iotx_mc_subscribe((iotx_mc_client_t *)handle, topic_filter, qos,
