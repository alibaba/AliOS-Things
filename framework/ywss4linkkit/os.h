...
#define OS_MANU_SECRET_LEN MANUFACTURE_SECRET_LEN
    ...
    /**
     * @brief Convert the data from byte order of current CPU(the "host") to
     big-endian. The size of the data is 16 bits.
     *
    ...
    /**
     * @brief Get unaligned 16 bits data which is big-endian and convert to host
     * byte order.
     *
    ...
    /**
     * @brief Get unaligned 16 bits data which is little-endian and convert to
     * host byte order.
     *
     * @param[in] ptr @n The start address of the unaligned data.
     * @return Result in host byte order.
    ...
    /**
     * @brief Get unaligned 32 bits data which is big-endian and convert to host
     * byte order.
     *
     * @param[in] ptr @n The start address of the unaligned data.
     * @return Result in host byte order.
    ...
    /**
     * @brief Get unaligned 32 bits data which is little-endian and convert to
     * host byte order.
     *
     * @param[in] ptr @n The start address of the unaligned data.
     * @return Result in host byte order.
     ...
     *
     * @param[in] ms @n the time interval for which execution is to be
     ...
     * @brief sleep thread itself.
     *
     * @param[in] sec @n the time interval for which execution is to be
    ...
    /**
     * @brief Destroy the specified semaphore object, it will free related
     * resource.
     *
     ...
     * @param[in] timeout_ms @n timeout interval in millisecond.
         If timeout_ms is OS_WAIT_INFINITE, the function will return only when
     ...
     * @note None.
     */
    static inline int os_semaphore_wait(_IN_ void *sem,
...
#define os_printf(fmt, args...) platform_printf(fmt, ##args)
     ...
     *
     * @param[in] ifname @n Specify the interface name.The NULL value indicates
     ...
     *
     * @param[in] timeout_ms @n during this period, wifi module enter power
     * saving mode
     ...
     * @note None.
     */
    static inline int os_awss_connect_ap(_IN_ uint32_t connection_timeout_ms,
...
#define os_wifi_scan_result_cb_t platform_wifi_scan_result_cb_t
     ...
     * @note awss use this API send raw frame in wifi monitor mode & station
     mode
     */
    static inline int os_wifi_send_80211_raw_frame(
      _IN_ enum platform_awss_frame_type type, _IN_ uint8_t *buffer,
      _IN_ int len)
    {
...
#define os_wifi_mgnt_frame_cb_t platform_wifi_mgnt_frame_cb_t
    ...
    /**
     * @brief enable/disable filter specific management frame in wifi station
     mode
     *
     * @param[in] filter_mask @n see mask macro in enum
     platform_awss_frame_type,
     ...
     * @note None.
     */
    static inline p_aes128_t os_aes128_init(_IN_ const uint8_t *key,
     ...
     * @note None.
     */
    static inline int os_aes128_destroy(_IN_ p_aes128_t aes)
    {
     ...
     * @note None.
     */
    static inline int os_aes128_cbc_encrypt(_IN_ p_aes128_t  aes,
                                            _IN_ const void *src,
     ...
     * @note None.
     */
    static inline int os_aes128_cfb_encrypt(_IN_ p_aes128_t  aes,
                                            _IN_ const void *src,
                                            _IN_ size_t blockNum,
                                            _OUT_ void *dst)
    {
     ...
     * @note None.
     */
    static inline int os_aes128_cbc_decrypt(_IN_ p_aes128_t  aes,
                                            _IN_ const void *src,
                                            _IN_ size_t blockNum,
                                            _OUT_ void *dst)
    {
     ...
     * @note None.
     */
    static inline int os_aes128_cfb_decrypt(_IN_ p_aes128_t  aes,
                                            _IN_ const void *src,
                                            _IN_ size_t blockNum,
                                            _OUT_ void *dst)
    {
    ...
    * @return
    ...
    static inline int os_wifi_get_ap_info(
      ...
      _OUT_ char passwd[PLATFORM_MAX_PASSWD_LEN], _OUT_ uint8_t bssid[ETH_ALEN])
    {
     ...
     * @note
     */
    static inline int os_product_get_version(
      char version_str[OS_PRODUCT_VERSION_LEN])
    {
     ...
     * @note None.
     */
    static inline int os_product_get_secret(
      char secret_str[OS_PRODUCT_SECRET_LEN])
    {
     ...
     * @note None.
     */
    static inline int os_get_device_secret(
      char secret_str[OS_DEVICE_SECRET_LEN])
    {
     ...
     * @note None.
     */
    static inline int os_get_manufacture_secret(
      char secret_str[OS_MANU_SECRET_LEN])
    {
