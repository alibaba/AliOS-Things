...
extern "C"
{
    ...
    static void *awss_get_dev_info(void *dev_info, int len)
    {
        ...
        snprintf(dev_info, len - 1, AWSS_DEV_INFO_FMT, AWSS_VER, pk, dev_name,
                 mac_str, ip_str, os_get_conn_encrypt_type());
    ...
    void *awss_build_dev_info(int type, void *dev_info, int info_len)
    {
        ...
        len += snprintf(dev_info + len, info_len - len - 1, "%s",
        ...
        switch (type) {
            case AWSS_NOTIFY_DEV_TOKEN: {
                ...
                utils_hex_to_str(aes_random, RANDOM_MAX_LEN, rand_str,
                                 sizeof(rand_str));
                len += snprintf(dev_info + len, info_len - len - 1,
                                AWSS_DEV_TOKEN_FMT, rand_str,
                                awss_token_remain_time(), 0);
                break;
            }
            case AWSS_NOTIFY_SUC: {
                len +=
                  snprintf(dev_info + len, info_len - len - 1, AWSS_SUC_FMT, 0);
                break;
            }
            case AWSS_NOTIFY_DEV_RAND: {
                ...
                char sign_str[ENROLLEE_SIGN_SIZE * 2 + 1] = { 0 };
                {
                    ...
                    utils_hex_to_str(sign, ENROLLEE_SIGN_SIZE, sign_str,
                                     sizeof(sign_str));
                }
                char rand_str[(RANDOM_MAX_LEN << 1) + 1] = { 0 };
                utils_hex_to_str(aes_random, RANDOM_MAX_LEN, rand_str,
                                 sizeof(rand_str));
                len += snprintf(dev_info + len, info_len - len - 1,
                                AWSS_DEV_RAND_FMT, rand_str, 0, sign_str);
                break;
            }
            default:
                break;
    ...
    int awss_build_packet(int type, void *id, void *ver, void *method,
                          void *data, int code, void *packet, int *packet_len)
    {
        ...
        if (type == AWSS_CMP_PKT_TYPE_REQ) {
            ...
            len = snprintf(packet, len - 1, AWSS_REQ_FMT, (char *)id,
                           (char *)ver, (char *)method, (char *)data);
            return 0;
        } else if (type == AWSS_CMP_PKT_TYPE_RSP) {
            len = snprintf(packet, len - 1, AWSS_ACK_FMT, (char *)id, code,
                           (char *)data);
            return 0;
        }
        return -1;
