...
#define AWSS_REQ_FMT \
    "{\"id\":%s, \"version\":\"%s\", \"method\":\"%s\", \"params\":%s}"
...
#define awss_cmp_debug(fmt, args...)
    ...
    /**
     * @brief this is a network address structure, including host(ip or host
     * name) and port.
     */
    typedef struct
    {
        char host[16];       /**< host ip(dotted-decimal notation) or host
                                name(string) */
    ...
    int awss_cmp_coap_send(void *buf, unsigned int len, void *sa,
                           const char *uri, void *cb, unsigned short *msgid);
    int awss_cmp_coap_send_resp(void *buf, unsigned int len, void *sa,
                                const char *uri, void *cb);
    int awss_cmp_coap_ob_send(void *buf, unsigned int len, void *sa,
                              const char *uri, void *cb);
    ...
    const char *  awss_build_topic(const char *topic_fmt, char *topic,
                                   unsigned int tlen);
    unsigned char awss_cmp_get_coap_code(void *request);

    int awss_build_packet(int type, void *id, void *ver, void *method,
                          void *data, int code, void *pkt, int *pkt_len);

    int online_connectap_monitor(void *ctx, void *resource, void *remote,
                                 void *request);
    int awss_enrollee_checkin(char *topic, int topic_len, void *payload,
                              int payload_len, void *ctx);
    int awss_online_switchap(char *topic, int topic_len, void *payload,
                             int payload_len, void *ctx);
    int awss_report_enrollee_reply(char *topic, int topic_len, void *payload,
                                   int payload_len, void *ctx);
    int awss_get_cipher_reply(char *topic, int topic_len, void *payload,
                              int payload_len, void *ctx);
