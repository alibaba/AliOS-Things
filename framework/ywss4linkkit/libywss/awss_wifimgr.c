...
extern "C"
{
    ...
    static void wifimgr_scan_tx_wifilist()
    {
        ...
        awss_build_topic((const char *)TOPIC_AWSS_WIFILIST, topic,
                         TOPIC_LEN_MAX);
        ...
        {
            if (item && item->data) {
                if (0 != awss_cmp_coap_ob_send(
                           item->data, strlen((char *)item->data),
    ...
    static int awss_scan_cb(const char          ssid[PLATFORM_MAX_SSID_LEN],
                            ...
                            uint8_t channel, signed char rssi, int last_ap)
    {
#define ONE_AP_INFO_LEN_MAX (141)
        ...
        if (aplist == NULL) {
            ...
            msg_len = 0;
            msg_len +=
              snprintf(aplist + msg_len, WIFI_APINFO_LIST_LEN - msg_len - 1,
                       "{\"awssVer\":%s, \"wifiList\":[", AWSS_VER);
        }
        ...
        if ((ssid != NULL) && (ssid[0] != '\0')) {
            ...
            if (other_apinfo && encode_ssid) {
                ...
                if (is_utf8(ssid, ssid_len)) {
                    ...
                    msg_len += snprintf(
                      aplist + msg_len, WIFI_APINFO_LIST_LEN - msg_len - 1,
                      "{\"ssid\":\"%s\",\"bssid\":\"%02X:%02X:%02X:%02X:%02X:%"
                      "02X\",\"rssi\":\"%d\",%s},",
                      encode_ssid, bssid[0], bssid[1], bssid[2], bssid[3],
                      bssid[4], bssid[5], rssi > 0 ? rssi - 256 : rssi,
                      other_apinfo);
                } else {
                    utils_hex_to_str((unsigned char *)ssid, ssid_len,
                                     encode_ssid, OS_MAX_SSID_LEN * 2);
                    msg_len += snprintf(
                      aplist + msg_len, WIFI_APINFO_LIST_LEN - msg_len - 1,
                      "{\"xssid\":\"%s\",\"bssid\":\"%02X:%02X:%02X:%02X:%02X:%"
                      "02X\",\"rssi\":\"%d\",%s},",
                      encode_ssid, bssid[0], bssid[1], bssid[2], bssid[3],
                      bssid[4], bssid[5], rssi > 0 ? rssi - 256 : rssi,
                      other_apinfo);
        ...
        if (last_ap || WIFI_APINFO_LIST_LEN <
                         msg_len + ONE_AP_INFO_LEN_MAX + strlen(AWSS_ACK_FMT)) {
            if (aplist[msg_len - 1] == ',') {
                msg_len--; /* eating the last ',' */
            }
            msg_len += snprintf(aplist + msg_len,
                                WIFI_APINFO_LIST_LEN - msg_len - 1, "]}");
            ...
            snprintf(msg_aplist, tlen - 1, AWSS_ACK_FMT, g_req_msg_id, 200,
                     aplist);
            os_free(aplist);
            aplist = NULL;
     ...
     *
     */
    int wifimgr_process_get_wifilist_request(void *ctx, void *resource,
                                             void *remote, void *request)
    {
        ...
        snprintf(buf, DEV_SIMPLE_ACK_LEN - 1, AWSS_ACK_FMT, g_req_msg_id, 200,
                 "\"success\"");
        ...
        if (0 != awss_cmp_coap_send_resp(buf, strlen(buf), &g_wifimgr_req_sa,
                                         topic, request)) {
            awss_debug("sending failed.");
        }
    ...
    static int wifimgr_process_get_device_info(void *ctx, void *resource,
                                               void *remote, void *request,
                                               char is_mcast)
    {
        char *buf      = NULL;
        ...
        char topic[TOPIC_LEN_MAX] = { 0 };
        if (is_mcast) {
            awss_build_topic((const char *)TOPIC_AWSS_GETDEVICEINFO_MCAST,
                             topic, TOPIC_LEN_MAX);
        } else {
            awss_build_topic((const char *)TOPIC_AWSS_GETDEVICEINFO_UCAST,
                             topic, TOPIC_LEN_MAX);
        }
        if (0 !=
            awss_cmp_coap_send_resp(buf, strlen(buf), remote, topic, request)) {
            awss_debug("sending failed.");
        }
    ...
    int wifimgr_process_mcast_get_device_info(void *ctx, void *resource,
                                              void *remote, void *request)
    {
        return wifimgr_process_get_device_info(ctx, resource, remote, request,
                                               1);
    }

    int wifimgr_process_ucast_get_device_info(void *ctx, void *resource,
                                              void *remote, void *request)
    {
        return wifimgr_process_get_device_info(ctx, resource, remote, request,
                                               0);
...
#define WLAN_CONNECTION_TIMEOUT (30 * 1000) // 30 seconds
    ...
    int wifimgr_process_switch_ap_request(void *ctx, void *resource,
                                          void *remote, void *request)
    {
        char ssid[PLATFORM_MAX_SSID_LEN * 2 + 1]        = { 0 },
                                              passwd[PLATFORM_MAX_PASSWD_LEN +
                                                     1] = { 0 };
        ...
        memcpy(req_msg_id, str,
               str_len > MSG_REQ_ID_LEN - 1 ? MSG_REQ_ID_LEN - 1 : str_len);
        awss_debug("switch ap, len:%u, %s\r\n", len, buf);
        buf = json_get_value_by_name(buf, len, "params", &len, 0);

        do {
            snprintf(msg, sizeof(msg) - 1, AWSS_ACK_FMT, req_msg_id, 200,
                     "\"success\"");
            ...
            if (!ssid_found) {
                ...
                if (str && (str_len < PLATFORM_MAX_SSID_LEN * 2 - 1)) {
                    ...
                    snprintf(msg, sizeof(msg) - 1, AWSS_ACK_FMT, req_msg_id, -1,
                             "\"ssid error\"");
                    success = 0;
                    break;
                }
            ...
            if (!str) {
                success = 0;
                snprintf(msg, sizeof(msg) - 1, AWSS_ACK_FMT, req_msg_id, -4,
                         "\"no security level error\"");
                break;
            }
            ...
            // TODO: empty passwd is allow? json parse "passwd":"" result is
            // NULL?
            switch (enc_lvl) {
                case SEC_LVL_AES256:
                    snprintf(msg, sizeof(msg) - 1, AWSS_ACK_FMT, req_msg_id, -4,
            ...
            if (0 == enc_lvl) {
                if (str_len < PLATFORM_MAX_PASSWD_LEN) {
                    ...
                    snprintf(msg, sizeof(msg) - 1, AWSS_ACK_FMT, req_msg_id, -2,
                             "\"passwd len error\"");
                    success = 0;
                }
            } else {
                if (str_len < (PLATFORM_MAX_PASSWD_LEN * 2) - 1) {
                    ...
                    aes_decrypt_string(encoded, passwd, str_len,
                                       os_get_conn_encrypt_type(),
                                       1); // 64bytes=2x32bytes
                } else {
                    snprintf(msg, sizeof(msg) - 1, AWSS_ACK_FMT, req_msg_id, -3,
                             "\"passwd len error\"");
                    success = 0;
                }
            }
            ...
            if (success && is_utf8(passwd, strlen(passwd)) == 0) {
                snprintf(msg, sizeof(msg) - 1, AWSS_ACK_FMT, req_msg_id,
                         enc_lvl == SEC_LVL_OPEN ? -2 : -3,
                         "\"passwd content error\"");
                success = 0;
            }
        } while (0);
        ...
        char topic[TOPIC_LEN_MAX] = { 0 };
        awss_build_topic((const char *)TOPIC_AWSS_SWITCHAP, topic,
                         TOPIC_LEN_MAX);
        for (i = 0; i < 5; i++) {
            if (0 != awss_cmp_coap_send_resp(msg, strlen(msg), remote, topic,
                                             request)) {
                awss_debug("sending failed.");
            } else {
        ...
        if (aplist) {
            bssid = aplist->mac;
            awss_debug("bssid: %02x:%02x:%02x:%02x:%02x:%02x", bssid[0],
        ...
        if (0 != os_awss_connect_ap(WLAN_CONNECTION_TIMEOUT, ssid, passwd,
                                    AWSS_AUTH_TYPE_INVALID,
                                    AWSS_ENC_TYPE_INVALID, bssid, 0)) {
        } else {
            switch_ap_done = 1;
