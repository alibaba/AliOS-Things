...
static void iotx_cm_cloud_conn_event_callback(void *_cm_ctx,
                                              iotx_connection_event_msg_t *msg)
{
        ...
        case IOTX_CONNECTION_EVENT_DISCONNECT: {
#ifdef CM_SUPPORT_MULTI_THREAD
            {
                ...
                ret = iotx_cm_process_list_push(
                  cm_ctx, IOTX_CM_CONNECTIVITY_TYPE_CLOUD, node);
                if (FAIL_RETURN == ret) {
...
static int iotx_cm_cloud_conn_response_callback(void *_cm_ctx,
    ...
    switch (msg->rsp_type) {
        case IOTX_CONNECTION_RESPONSE_SUBSCRIBE_SUCCESS: {
#ifdef CM_SUPPORT_MULTI_THREAD
            {
                ...
                node = iotx_cm_get_list_node(cm_ctx,
                                             IOTX_CM_CONNECTIVITY_TYPE_CLOUD);
                if (NULL == node)
                    return FAIL_RETURN;

                node->type = IOTX_CM_PROCESS_REGISTER_RESULT;
                if (NULL == (node->msg = CM_malloc(
                               sizeof(iotx_cm_process_register_result_t)))) {
...
#ifdef CM_SUPPORT_TOPIC_DISPATCH
                /* find mapping */
                if (NULL == (mapping = iotx_cm_find_mapping(
                               cm_ctx, IOTX_CM_MAPPING_TYPE_REGISTER,
...
int iotx_cm_cloud_conn_register_handler(iotx_cm_conntext_t *cm_ctx, char *URI,
                                        int result, int is_register)
{
    iotx_cm_event_msg_t    event;
...
int iotx_cm_cloud_conn_response_handler(iotx_cm_conntext_t *    cm_ctx,
                                        iotx_cm_message_info_t *message_info)
{
    ...
    if (cm_ctx->response_func)
        cm_ctx->response_func(cm_ctx, IOTX_CM_MAPPING_TYPE_REGISTER,
                              message_info);
...
void *iotx_cm_cloud_conn_init(void *handler, iotx_cm_init_param_t *pparam)
{
    ...
    strncpy(g_cloud_target.product_key, device_info->product_key,
            strlen(device_info->product_key));
    strncpy(g_cloud_target.device_name, device_info->device_name,
    ...
    HAL_ThreadCreate(&connectivity->pthread_process, iotx_cm_cloud_conn_process,
                     handler, NULL, NULL);
#else
    pthread_create(&connectivity->pthread_process, NULL,
                   iotx_cm_cloud_conn_process, (void *)handler);
#endif
#endif
...
int iotx_cm_cloud_conn_trigger_connected(void *handler, void *_connectivity,
...
int iotx_cm_cloud_conn_register(void *handler, void *_connectivity,
                                const char *topic_filter)
{
    ...
    return connection->sub_func(connection, topic_filter,
                                IOTX_CM_MESSAGE_NEED_ACK);
}


int iotx_cm_cloud_conn_unregister(void *handler, void *_connectivity,
                                  const char *topic_filter)
{
    iotx_cm_connectivity_t *connectivity =
      (iotx_cm_connectivity_t *)_connectivity;
...
int iotx_cm_cloud_conn_add_service(void *handler, void *_connectivity,
                                   const char *                 topic_filter,
...
int iotx_cm_cloud_conn_remove_service(void *handler, void *_connectivity,
                                      const char *topic_filter)
{
    return SUCCESS_RETURN;
}
...
int iotx_cm_cloud_conn_send(void *handler, void *_connectivity,
...
int iotx_cm_cloud_conn_send_sync(void *handler, void *_connectivity,
                                 iotx_cm_send_peer_t *       target,
                                 const char *                topic_filter,
                                 iotx_cm_message_ack_types_t ack_type,
                                 const void *payload, int payload_length,
...
int iotx_cm_cloud_conn_add_subdevice(void *handler, void *connectivity,
...
int iotx_cm_cloud_conn_remove_subdevice(void *handler, void *connectivity,
                                        const char *pk, const char *dn)
{
    return SUCCESS_RETURN;
}
...
#ifdef CM_SUPPORT_MULTI_THREAD
int iotx_cm_cloud_conn_add_send(void *handler, iotx_cm_send_peer_t *target,
                                iotx_cm_message_info_t *message_info)
{
    /* send message to itself thread */
    int                          ret  = 0;
    ...
    ret =
      iotx_cm_process_list_push(cm_ctx, IOTX_CM_CONNECTIVITY_TYPE_CLOUD, node);
...
void *iotx_cm_cloud_conn_process(void *pclient)
{
    ...
    iotx_cm_connectivity_t *connectivity =
      iotx_cm_find_connectivity(cm_ctx, iotx_cm_cloud_conn_get_target(), NULL);
    ...
    while (!cm_ctx->thread_stop) {
        ...
        if (iotx_cm_process_list_get_size(
              cm_ctx, IOTX_CM_CONNECTIVITY_TYPE_CLOUD) > 0) {
            iotx_cm_process_list_node_t *node = NULL;
                ...
                switch (node->type) {
                    case IOTX_CM_PROCESS_REGISTER: {
                        ...
                        if (FAIL_RETURN ==
                            iotx_cm_add_mapping(
                              cm_ctx, IOTX_CM_MAPPING_TYPE_REGISTER,
                              _register->URI, _register->type,
                              _register->register_func, _register->user_data,
                              _register->mail_box)) {
                            iotx_cm_event_msg_t    event;
                        ...
                        mapping = iotx_cm_find_mapping(
                          cm_ctx, IOTX_CM_MAPPING_TYPE_REGISTER, _register->URI,
                          strlen(_register->URI));
                        iotx_cm_cloud_conn_register(cm_ctx, connectivity,
                                                    mapping->URI);
                    ...
                    case IOTX_CM_PROCESS_UNREGISTER: {
                        ...
                        iotx_cm_cloud_conn_unregister(cm_ctx, connectivity,
                                                      URI);
                        iotx_cm_remove_mapping(
                          cm_ctx, IOTX_CM_MAPPING_TYPE_REGISTER, URI);
                        LITE_free(URI);
                    } break;
                    ...
                    case IOTX_CM_PROCESS_ADD_SERVICE: {
                        ...
                        iotx_cm_cloud_conn_add_service(cm_ctx, connectivity,
                                                       _service->URI,
                                                       _service->auth_type);
                    ...
                    case IOTX_CM_PROCESS_REMOVE_SERVICE: {
                        char *URI = (char *)node->msg;
                        iotx_cm_cloud_conn_remove_service(cm_ctx, connectivity,
                                                          URI);
                        LITE_free(URI);
                    }
                    ...
                    case IOTX_CM_PROCESS_CLOUD_SEND: {
                        ...
                        if (FAIL_RETURN ==
                            iotx_cm_send_data(
                              cm_ctx, _send->target,
                              iotx_cm_find_connectivity(cm_ctx, &g_cloud_target,
                                                        _send->conn_ctx),
                    ...
                    case IOTX_CM_PROCESS_UNREGISTER_RESULT: {
                        ...
                        if (_register->URI)
                            iotx_cm_cloud_conn_register_handler(
                              cm_ctx, _register->URI, _register->result,
                              _register->is_register);
                        LITE_free(_register->URI);
                        LITE_free(_register);
                    } break;
                    ...
                    case IOTX_CM_PROCESS_ADD_SUBDIVCE: {
                        ...
                        iotx_cm_cloud_conn_add_subdevice(
                    ...
                    case IOTX_CM_PROCESS_REMOVE_SUBDIVCE: {
                        iotx_cm_process_subdevice_t *subdevice =
                          (iotx_cm_process_subdevice_t *)node->msg;
                        iotx_cm_cloud_conn_remove_subdevice(
                          cm_ctx, connectivity, subdevice->pk, subdevice->dn);
                        LITE_free(node->msg);
                    } break;
                    ...
                    case IOTX_CM_PROCESS_CLOUD_NEW_DATA: {
                        ...
                        iotx_cm_cloud_conn_response_handler(cm_ctx,
                                                            message_info);
                        LITE_free(node->msg);
                        break;
                    }
