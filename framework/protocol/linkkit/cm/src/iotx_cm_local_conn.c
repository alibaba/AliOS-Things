...
static void iotx_local_conn_event_callback(
  void *pcontext, iotx_connection_event_msg_t *event_msg)
{
    ...
    switch (event_msg->event_id) {
...
#ifdef CM_SUPPORT_MULTI_THREAD
        {
            ...
            iotx_cm_process_list_node_t *node =
              iotx_cm_get_list_node(cm_ctx, IOTX_CM_CONNECTIVITY_TYPE_LOCAL);
            if (NULL == node)
                return;
            ...
            if (FAIL_RETURN ==
                (ret = iotx_cm_process_list_push(
                   cm_ctx, IOTX_CM_CONNECTIVITY_TYPE_LOCAL, node)))
        ...
        case IOTX_CONNECTION_EVENT_RECONNECT: {
#ifdef CM_SUPPORT_MULTI_THREAD
            {
                /* send message to itself thread */
                int                          ret  = 0;
                iotx_cm_process_list_node_t *node = iotx_cm_get_list_node(
                  cm_ctx, IOTX_CM_CONNECTIVITY_TYPE_LOCAL);
                if (NULL == node)
                    return;
...
static int iotx_cm_local_conn_response_callback(void *pcontext,
                                                iotx_connection_msg_rsp_t *msg)
{
    iotx_cm_conntext_t *cm_ctx = (iotx_cm_conntext_t *)pcontext;
    ...
    switch (msg->rsp_type) {
        case IOTX_CONNECTION_RESPONSE_ADD_SERVICE_SUCCESS: {
#ifdef CM_SUPPORT_MULTI_THREAD
            {
                ...
                if (NULL == (node = iotx_cm_get_list_node(
                               cm_ctx, IOTX_CM_CONNECTIVITY_TYPE_CLOUD)))
                    return FAIL_RETURN;

                node->type = IOTX_CM_PROCESS_ADD_SERVICE_RESULT;
                node->msg =
                  LITE_malloc(sizeof(iotx_cm_process_service_result_t));
                if (NULL == node) {
        ...
        case IOTX_CONNECTION_RESPONSE_NEW_DATA: {
#ifdef CM_SUPPORT_MULTI_THREAD
            {
                ...
                /* find mapping */
                mapping =
                  iotx_cm_find_mapping(cm_ctx, IOTX_CM_MAPPING_TYPE_SERVICE,
                                       (char *)msg->URI, msg->URI_length);
                if (NULL == mapping) {
                ...
                if (FAIL_RETURN == iotx_cm_parse_payload((char *)msg->payload,
                                                         msg->payload_length,
                                                         msg_info)) {
                    CM_ERR(cm_log_error_parse_URI);
...
#ifdef CM_SUPPORT_TOPIC_DISPATCH
            /* find mapping */
            mapping = iotx_cm_find_mapping(cm_ctx, IOTX_CM_MAPPING_TYPE_SERVICE,
                                           (char *)msg->URI, msg->URI_length);
            if (NULL == mapping) {
                CM_WARNING(cm_log_warning_not_mapping);
                iotx_cm_free_message_info(&message_info);
...
int iotx_cm_local_conn_service_handler(iotx_cm_conntext_t *cm_ctx, char *URI,
                                       int result, int is_add)
{
    ...
    event.event_id = is_add ? IOTX_CM_EVENT_ADD_SERVICE_RESULT
...
int iotx_cm_local_conn_found_device_handler(iotx_cm_conntext_t *    cm_ctx,
                                            iotx_cm_local_device_t *device)
{
#if 0
    iotx_cm_event_msg_t event;
...
int iotx_cm_local_conn_remove_device_handler(iotx_cm_conntext_t *    cm_ctx,
                                             iotx_cm_local_device_t *device)
{
#if 0
    iotx_cm_event_msg_t event;
                                        ...
                                        iotx_cm_message_info_t *message_info)
{
    ...
    if (cm_ctx->response_func)
        cm_ctx->response_func(cm_ctx, IOTX_CM_MAPPING_TYPE_SERVICE,
                              message_info);
...
void *iotx_cm_local_conn_init(void *handler, iotx_cm_init_param_t *pparam)
{
    ...
    if (NULL == connectivity) {
        ...
        if (g_local_device->node->device_info)
            LITE_free(g_local_device->node->device_info);
        if (g_local_device->node)
            LITE_free(g_local_device->node);
        if (g_local_device)
            LITE_free(g_local_device);
    ...
    HAL_ThreadCreate(&connectivity->pthread_process, iotx_cm_local_conn_process,
                     cm_ctx, NULL, NULL);
#else
    pthread_create(&connectivity->pthread_process, NULL,
                   iotx_cm_local_conn_process, (void *)cm_ctx);
#endif
#endif /* CM_SUPPORT_MULTI_THREAD */
...
int iotx_cm_local_conn_trigger_connected(void *handler, void *_connectivity,
                                         iotx_cm_event_handle_fp_t event_fp,
                                         void *                    user_data)
{
    ...
    if (1 == connectivity->is_connected) {
        ...
        event_fp ? event_fp(cm_ctx, &event_msg, user_data)
...
int iotx_cm_local_conn_register(void *handler, void *_connectivity,
...
int iotx_cm_local_conn_unregister(void *handler, void *_connectivity,
                                  const char *topic_filter)
{
    return SUCCESS_RETURN;
}
...
int iotx_cm_local_conn_add_service(void *handler, void *_connectivity,
                                   const char *                 topic_filter,
                                   iotx_cm_message_auth_types_t auth_type)
{
    ...
    return connection->add_service_func(connection, topic_filter,
                                        IOTX_CM_MESSAGE_NEED_ACK, auth_type);
}


int iotx_cm_local_conn_remove_service(void *handler, void *_connectivity,
                                      const char *topic_filter)
{
    iotx_cm_connectivity_t *connectivity =
      (iotx_cm_connectivity_t *)_connectivity;
...
int iotx_cm_local_conn_send(void *handler, void *_connectivity,
...
int iotx_cm_local_conn_send_sync(void *handler, void *_connectivity,
                                 iotx_cm_send_peer_t *       target,
                                 const char *                topic_filter,
                                 iotx_cm_message_ack_types_t ack_type,
                                 const void *payload, int payload_length,
...
int iotx_cm_local_conn_add_subdevice(void *handler, void *_connectivity,
                                     const char *pk, const char *dn)
{
    iotx_cm_connectivity_t *connectivity =
      (iotx_cm_connectivity_t *)_connectivity;
    iotx_connection_t *connection = NULL;
...
int iotx_cm_local_conn_remove_subdevice(void *handler, void *_connectivity,
                                        const char *pk, const char *dn)
{
    iotx_cm_connectivity_t *connectivity =
      (iotx_cm_connectivity_t *)_connectivity;
...
#ifdef CM_SUPPORT_MULTI_THREAD
int iotx_cm_local_conn_add_send(void *handler, iotx_cm_send_peer_t *target,
                                iotx_cm_message_info_t *message_info)
{
    /* send message to itself thread */
    int                          ret  = 0;
    ...
    if (target) {
        ...
        strncpy(msg->target->device_name, target->device_name,
                strlen(target->device_name));
        strncpy(msg->target->product_key, target->product_key,
                strlen(target->product_key));
    } else {
        msg->target = NULL;
    }

    if (FAIL_RETURN == iotx_cm_parse_message(cm_ctx, message_info, &URI,
    ...
    ret =
      iotx_cm_process_list_push(cm_ctx, IOTX_CM_CONNECTIVITY_TYPE_LOCAL, node);
...
void *iotx_cm_local_conn_process(void *pclient)
{
    ...
    iotx_cm_connectivity_t *connectivity =
      iotx_cm_find_connectivity(cm_ctx, iotx_cm_local_conn_get_target(), NULL);
    ...
    while (!cm_ctx->thread_stop) {
        ...
        if (iotx_cm_process_list_get_size(
              cm_ctx, IOTX_CM_CONNECTIVITY_TYPE_LOCAL) > 0) {
            iotx_cm_process_list_node_t *node = NULL;
            if (NULL != (node = iotx_cm_process_list_pop(
                           cm_ctx, IOTX_CM_CONNECTIVITY_TYPE_LOCAL))) {
                ...
                switch (node->type) {
                    case IOTX_CM_PROCESS_ADD_DEVICE: {
                        iotx_cm_local_device_t *device =
                          (iotx_cm_local_device_t *)node->msg;
                        iotx_cm_local_conn_found_device_handler(cm_ctx, device);
                    } break;
                    ...
                    case IOTX_CM_PROCESS_REMOVE_DEVICE: {
                        iotx_cm_local_device_t *device =
                          (iotx_cm_local_device_t *)node->msg;
                        iotx_cm_local_conn_remove_device_handler(cm_ctx,
                                                                 device);
                    } break;

                    case IOTX_CM_PROCESS_LOCAL_SEND: {
                        iotx_cm_process_send_t *_send =
                          (iotx_cm_process_send_t *)node->msg;
                        if (FAIL_RETURN ==
                            iotx_cm_send_data(
                              cm_ctx, _send->target,
                              iotx_cm_find_connectivity(
                                cm_ctx, iotx_cm_local_conn_get_target(),
                                _send->conn_ctx),
                              _send->URI, _send->ack_type, _send->payload,
                              _send->payload_length, _send->conn_ctx)) {
                            log_info("send fail");
                        }
                    ...
                    case IOTX_CM_PROCESS_REGISTER: {
                        ...
                        iotx_cm_local_conn_register(cm_ctx, connectivity,
                                                    _register->URI);
                    ...
                    case IOTX_CM_PROCESS_UNREGISTER: {
                        ...
                        iotx_cm_local_conn_unregister(cm_ctx, connectivity,
                                                      URI);
                        LITE_free(URI);
                    } break;
                    ...
                    case IOTX_CM_PROCESS_ADD_SERVICE: {
                        ...
                        if (FAIL_RETURN ==
                            iotx_cm_add_mapping(
                              cm_ctx, IOTX_CM_MAPPING_TYPE_SERVICE,
                              _service->URI, _service->type,
                              _service->register_func, _service->user_data,
                              _service->mail_box)) {
                            iotx_cm_event_msg_t    event;
                        ...
                        mapping = iotx_cm_find_mapping(
                          cm_ctx, IOTX_CM_MAPPING_TYPE_SERVICE, _service->URI,
                          strlen(_service->URI));

                        iotx_cm_local_conn_add_service(cm_ctx, connectivity,
                                                       mapping->URI,
                                                       _service->auth_type);
                    ...
                    case IOTX_CM_PROCESS_REMOVE_SERVICE: {
                        char *URI = (char *)node->msg;
                        iotx_cm_local_conn_remove_service(cm_ctx, connectivity,
                                                          URI);
                        iotx_cm_remove_mapping(
                          cm_ctx, IOTX_CM_MAPPING_TYPE_SERVICE, URI);
                        LITE_free(URI);
                    } break;

                    case IOTX_CM_PROCESS_ADD_SERVICE_RESULT:
                    case IOTX_CM_PROCESS_REMOVE_SERVICE_RESULT: {
                        iotx_cm_process_service_result_t *_service =
                          (iotx_cm_process_service_result_t *)node->msg;
                        iotx_cm_local_conn_service_handler(
                          cm_ctx, _service->URI, _service->result,
                          _service->is_add);
                        LITE_free(_service->URI);
                        LITE_free(_service);
                    } break;
                    ...
                    case IOTX_CM_PROCESS_ADD_SUBDIVCE: {
                        iotx_cm_process_subdevice_t *subdevice =
                          (iotx_cm_process_subdevice_t *)node->msg;
                        iotx_cm_local_conn_add_subdevice(
                          cm_ctx, connectivity, subdevice->pk, subdevice->dn);
                        LITE_free(node->msg);
                    } break;
                    ...
                    case IOTX_CM_PROCESS_REMOVE_SUBDIVCE: {
                        iotx_cm_process_subdevice_t *subdevice =
                          (iotx_cm_process_subdevice_t *)node->msg;
                        iotx_cm_local_conn_remove_subdevice(
                          cm_ctx, connectivity, subdevice->pk, subdevice->dn);
                        LITE_free(node->msg);
                    } break;
