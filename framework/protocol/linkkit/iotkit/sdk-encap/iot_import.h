...
#define PLATFORM_WAIT_INFINITE (~0)
    ...
    typedef enum
    {
        ...
        os_thread_priority_error = 0x84, /* system cannot determine priority or
    ...
    typedef struct _hal_os_thread
    {
        ...
        void *stack_addr; /* thread stack address malloced by caller, use system
                             stack by . */
        size_t stack_size; /* stack size requirements in bytes; 0 is default
                              stack size */
        int
              detach_state; /* 0: not detached state; otherwise: detached state. */
     ...
     * @brief   按照指定入参创建一个线程
     *
     * @param[out] thread_handle @n The new thread handle, memory allocated
     before thread created and return it, free it after thread joined or exit.
     * @param[in] start_routine @n A pointer to the application-defined function
     to be executed by the thread. This pointer represents the starting address
     of the thread.
     * @param[in] arg @n A pointer to a variable to be passed to the
     start_routine.
     * @param[in] hal_os_thread_param @n A pointer to stack params.
     * @param[out] stack_used @n if platform used stack buffer, set stack_used
     ...
     * @param[in] timeout_ms @n timeout interval in millisecond.
         If timeout_ms is PLATFORM_WAIT_INFINITE, the function will return only
     when the semaphore is signaled.
     * @return
       @verbatim
       =  0: The state of the specified object is signaled.
       =  -1: The time-out interval elapsed, and the object's state is
     nonsignaled.
       @endverbatim
     * @see None.
     * @note None.
     ...
     * @brief   按照入参ptr释放一片内存
     *
     * @param   ptr : 需要释放的内存起始地址,
     * 也是之前调用HAL_Malloc()的非空返回值
     * @return  无返回值
     */
     ...
     *
     * @param   fmt : 打印格式参数, 可以内含%s和%d等占位符,
     * 语法等同C标准函数printf()
     ...
     * @param   len : 可以向目标字符串中输出的最大长度, 单位是字节,
     * 包含结束符'\0'
     * @param   fmt : 打印格式参数, 可以内含%s和%d等占位符,
     * 语法等同C标准函数printf()
     * @param   ... : 变参列表, 用于替换fmt中内含的%s和%d等占位符
     * @return  fmt格式字符串中用变参替换掉所有占位符之后的实际字符串长度,
     * 可能超过入参len
     */
    ...
    /**
     * @brief   按照指定的目标主机名和目标端口创建一个TCP连接,
     并返回该连接的句柄
     *
    ...
    /**
     * @brief   在指定的TCP连接上发送指定缓冲区的指定长度,
     * 阻塞时间不超过指定时长, 且指定长度若发送完需提前返回
     *
     * @param   fd : TCP连接的句柄
     ...
     * @retval  (0, len] : 在指定的'timeout_ms'时间间隔内, 被成功发送的数据长度,
     * 单位是字节(Byte)
     */
    int32_t HAL_TCP_Write(_IN_ uintptr_t fd, _IN_ const char *buf,
                          _IN_ uint32_t len, _IN_ uint32_t timeout_ms);

    /**
     * @brief   从指定的TCP连接上接收指定长度到指定缓冲区,
     * 阻塞时间不超过指定时长, 且指定长度若接收完需提前返回
     *
     * @param   fd : TCP连接的句柄
     ...
     * @retval  (0, len] : 在指定的'timeout_ms'时间间隔内, 被成功接收的数据长度,
     * 单位是字节(Byte)
     */
    int32_t HAL_TCP_Read(_IN_ uintptr_t fd, _OU_ char *buf, _OU_ uint32_t len,
                         _IN_ uint32_t timeout_ms);

    /**
     * @brief   向指定的目标地址和端口建立一个TCP+SSL连接,
     * 使用参数中指定的CA证书, 并返回SSL连接的句柄
     *
     * @param   host : 指定目标主机的地址, 形式是域名或者点分十进制的IP地址
     * @param   port : 指定目标端口
     ...
     */
    uintptr_t HAL_SSL_Establish(_IN_ const char *host, _IN_ uint16_t port,
     ...
     * 单位是字节(Byte)
     */
    int32_t HAL_SSL_Write(_IN_ uintptr_t handle, _IN_ const char *buf,
                          _IN_ int len, _IN_ int timeout_ms);


    /**
     * @brief   从指定的TCP+SSL连接上接收指定长度到指定缓冲区,
     * 阻塞时间不超过指定时长, 且指定长度若接收完需提前返回
     *
     * @param   handle : TCP+SSL连接的句柄
     * @param   buf : 存放被接收数据的缓冲区起始地址
     ...
     * 单位是字节(Byte)
     */
    int32_t HAL_SSL_Read(_IN_ uintptr_t handle, _OU_ char *buf, _OU_ int len,
                         _IN_ int timeout_ms);
...
#define NETWORK_ADDR_LEN (16) /* UDP网络地址的长度 */
    ...
    /**
     * @brief   在指定的UDP socket上发送指定缓冲区的指定长度,
     * 阻塞时间不超过指定时长, 且指定长度若发送完需提前返回
     *
     ...
     * 单位是字节(Byte)
     */
    int HAL_UDP_sendto(_IN_ intptr_t sockfd, _IN_ const NetworkAddr *p_remote,
    ...
    /**
     * @brief   从指定的UDP句柄接收指定长度数据到缓冲区, 阻塞时间不超过指定时长,
     * 且指定长度若接收完需提前返回, 源地址保存在出参中
     *
     ...
     * 单位是字节(Byte)
     */
    int HAL_UDP_recvfrom(_IN_ intptr_t sockfd, _OU_ NetworkAddr *p_remote,
                         _OU_ unsigned char *p_data, _IN_ unsigned int datalen,
     ...
     * 单位是字节(Byte)
     */
    int HAL_UDP_recv(_IN_ intptr_t sockfd, _OU_ unsigned char *p_data,
                     _IN_ unsigned int datalen, _IN_ unsigned int timeout_ms);
     ...
     * @param[in] length: @n The length, in bytes, of the data pointed to by the
     * buffer parameter.
    ...
    /**
     * @brief indicate firmware upgrade data complete, and trigger data
     integrity checking, and then reboot the system.
     *
     * @param None.
     ...
     * @param[in] len: @n The length, in bytes, of the buffer pointed to by the
     * val parameter.
