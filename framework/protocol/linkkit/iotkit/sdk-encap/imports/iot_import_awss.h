...
#define HAL_CONFIG_SIZE (2048)
    ...
    /**
     * @brief   在设备的持久化外部存储器比如Flash上,
     * 从配置区域起始位置读取数据到指定的内存缓冲区中
     *
...
#define HAL_IP_LEN (15 + 1)
     ...
     *      If the STA connects the old AP, HAL should disconnect from the old
     AP firstly.
     *      If bssid specifies the dest AP, HAL should use bssid to connect dest
     AP.
     */
...
#define FRAME_DATA_MASK (1 << FRAME_DATA)
     ...
     * @param[in] rssi_dbm @n rssi in dbm, range of [-127, -1], set it to -1 if
     * not supported
     ...
     * @see None.
     * @note
     *     If the STA dosen't connect AP successfully, HAL should return -1 and
     not touch the ssid/passwd/bssid buffer.
     */
     ...
     * @note None.
     */
    p_HAL_Aes128_t HAL_Aes128_Init(_IN_ const uint8_t *key,
