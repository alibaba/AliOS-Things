                          ...
                          NetworkAddr *from, CoAPMessage *resMsg)
{
    ...
    CoAPMessage msg;
    char        keybuf[32];
    snprintf(keybuf, sizeof(keybuf), "\"accessKey\":\"%.*s\"", targetLen,
             targetKey);
    char payloadbuf[512];
    snprintf(payloadbuf, sizeof(payloadbuf), RES_FORMAT, seqlen, seq,
...
svr_key_info *is_legal_key(CoAPContext *ctx, const char *keyprefix,
                           ...
                           int *res_code)
{
    // COAP_DEBUG ("islegal prefix:%.*s, seq:%.*s", prefixlen, keyprefix,
    // seqlen, keyseq);
                   ...
                   CoAPMessage *resMsg)
{
    ...
    do {
        ...
        svr_key_info *item = is_legal_key(ctx, keyprefix, KEYPREFIX_LEN, keyseq,
                                          KEYSEQ_LEN, &res_code);
        if (!item) {
        ...
        int   pklen, dnlen;
        char *pk =
          json_get_value_by_name(data, datalen, "prodKey", &pklen, NULL);
        char *dn =
          json_get_value_by_name(data, datalen, "deviceName", &dnlen, NULL);
        ...
        snprintf(buf, sizeof(buf), "%.*s%s", randomkeylen, randomkey,
                 session->randomKey);
        utils_hmac_sha1_raw(buf, strlen(buf), session->sessionKey, accessToken,
                            tokenlen);
    ...
    CoAPMessage message;
    char        payloadbuf[512];
    snprintf(payloadbuf, sizeof(payloadbuf), RES_FORMAT, seqlen, seq, res_code,
             body);
    CoAPLenString payload = { strlen(payloadbuf), (unsigned char *)payloadbuf };
...
int add_svr_key(CoAPContext *ctx, const char *keyprefix, const char *secret,
                bool isGroup)
{
                      ...
                      NetworkAddr *remote, CoAPMessage *message)
{
    ...
    session_item *session =
      get_session_by_checksum(context, sessions, remote, node->pk_dn);
                         ...
                         NetworkAddr *remote, CoAPMessage *request)
{
    ...
    if (session) {
        snprintf(databuf, sizeof(databuf), "\"delayTime\":%d",
                 default_heart_expire / 1000);
        snprintf(payloadbuf, sizeof(payloadbuf), RES_FORMAT, seqlen, seq, 200,
                 databuf);
    } else {
        snprintf(payloadbuf, sizeof(payloadbuf), RES_FORMAT, seqlen, seq,
                 ALCS_HEART_FAILAUTH, "");
                         ...
                         CoAPLenString *dest)
{
    ...
    session_item *    session =
      get_session_by_checksum(ctx, sessions, from, node->pk_dn);
