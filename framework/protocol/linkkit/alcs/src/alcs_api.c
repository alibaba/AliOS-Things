...
session_item *get_session_by_checksum(CoAPContext *     ctx,
...
static session_item *get_session(CoAPContext *ctx, struct list_head *sessions,
                   ...
                   const char *deviceName, char role)
{
    ...
    if (role & ROLE_SERVER) {
        ...
        char path[256];
        snprintf(path, sizeof(path), "/dev/%s/%s/core/service/auth", productKey,
                 deviceName);
...
{
    char path[256];
    log_info("Subdev Register Resource, PK: %s, DN: %s", productKey,
             deviceName);
    snprintf(path, sizeof(path), "/dev/%s/%s/core/service/auth", productKey,
             deviceName);
    alcs_resource_register(ctx, productKey, deviceName, path, COAP_PERM_GET,
...
int alcs_encrypt(const char *src, int len, const char *key, void *out)
{
    ...
    if (len1) {
        p_HAL_Aes128_t aes_e_h =
          HAL_Aes128_Init((uint8_t *)key, (uint8_t *)iv, HAL_AES_ENCRYPTION);
...
int alcs_decrypt(const char *src, int len, const char *key, void *out)
{
    ...
    if (n > 0) {
        aes_d_h =
          HAL_Aes128_Init((uint8_t *)key, (uint8_t *)iv, HAL_AES_DECRYPTION);
                       ...
                       const char *accesskey, const char *accesstoken)
{
    ...
    ctl_group_item *item =
      (ctl_group_item *)coap_malloc(sizeof(ctl_group_item));
    if (!item) {
                       ...
                       const char *keyprefix, const char *secret)
{
    ...
    svr_group_item *item =
      (svr_group_item *)coap_malloc(sizeof(svr_group_item));
    if (!item) {
        return COAP_ERROR_MALLOC;
    }
