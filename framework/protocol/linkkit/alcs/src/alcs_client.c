...
{
    if (strlen(keyprefix) == KEYPREFIX_LEN &&
        strstr(accesskey, keyprefix) == accesskey) {
        return 1;
    }
...
int  do_auth(CoAPContext *ctx, NetworkAddr *addr, ctl_key_item *ctl_item,
             void *userdata, AuthHandler handler);
bool res_parse(const char *payload, int len, int *seq, ResponseMsg *res_msg,
...
bool fillAccessKey(CoAPContext *ctx, char *buf)
{
    ...
    list_for_each_entry_safe(gnode, gnext, &lst->lst_ctl_group, lst,
                             ctl_group_item)
    {
        char *format;
...
#define payload_format                                              \
    "{\"version\":\"1.0\",\"method\":\"%s\",\"id\":%d,\"params\":{" \
    "\"prodKey\":\"%s\", \"deviceName\":\"%s\"%s}}"
void nego_cb(CoAPContext *ctx, CoAPReqResult result, void *userdata,
             NetworkAddr *remote, CoAPMessage *message)
{
    ...
    if (COAP_RECV_RESP_TIMEOUT == result) {
        ...
        res_parse((const char *)message->payload, message->payloadlen, &seq,
                  &msg, &data, &datalen);
        do {
            if (msg.code != 200) {
                break;
            ...
            int   keylen;
            char *accessKey =
              json_get_value_by_name(data, datalen, "accessKey", &keylen, NULL);
            if (!accessKey || !keylen) {
                break;
            }
            ...
            list_for_each_entry_safe(node, next, &lst->lst_ctl, lst,
                                     ctl_key_item)
            {
            ...
            if (accessTokenFound) {
                ...
                do_auth(ctx, remote, &item, auth_param->user_data,
                        auth_param->handler);
...
void auth_cb(CoAPContext *ctx, CoAPReqResult result, void *userdata,
             NetworkAddr *remote, CoAPMessage *message)
{
    ...
    if (!session) {
        COAP_INFO("receive unknown auth_cb response, pk:%s, dn:%s", devKey.pk,
                  devKey.dn);
        ...
        if (msg.code == 200) {
            do {
                ...
                tmp = json_get_value_by_name(data, datalen, "sessionId",
                                             &tmplen, NULL);
                if (!tmp) {
                    msg.code = -1;
                    msg.msg  = "sessionid = NULL!";
                    COAP_ERR("sessionid = NULL!");
                    auth_param->handler(ctx, remote, auth_param->user_data,
                                        &msg);
                    break;
                }
                char back;
                ...
                tmp = json_get_value_by_name(data, datalen, "randomKey",
                                             &tmplen, NULL);
                if (!tmp) {
                    msg.code = -1;
                ...
                char buf[32];
                snprintf(buf, sizeof(buf), "%s%.*s", session->randomKey, tmplen,
                         tmp);
                utils_hmac_sha1_raw(buf, strlen(buf), session->sessionKey,
                                    auth_param->accessToken,
                                    strlen(auth_param->accessToken));
...
#define auth_payload_format                                               \
    "{\"version\":\"1.0\",\"method\":\"core/service/"                     \
    "auth\",\"id\":%d,\"params\":{\"prodKey\":\"%s\", "                   \
    "\"deviceName\":\"%s\",\"encrypt\":\"payload\",\"randomKey\":\"%s\"," \
    "\"sign\":\"%s\",\"accessKey\":\"%s\"}}"

int do_auth(CoAPContext *ctx, NetworkAddr *addr, ctl_key_item *ctl_item,
            void *user_data, AuthHandler handler)
{
    ...
    // create&save session item
    {
        ...
        COAP_INFO("pk:%s, dn:%s, checksum:%s", devKey.pk, devKey.dn,
                  session->pk_dn);
    ...
    utils_hmac_sha1_base64(session->randomKey, strlen(session->randomKey),
                           ctl_item->accessToken, strlen(ctl_item->accessToken),
                           sign, &sign_len);
    // COAP_INFO ("calc randomKey:%s,token:%s,sign:%.*s", session->randomKey,
    // ctl_item->accessToken, sign_len, sign);

    char payloadbuf[512];
    sprintf(payloadbuf, auth_payload_format, ++dev->seq, ctl_item->productKey,
            ctl_item->deviceName, session->randomKey, sign,
            ctl_item->accessKey);
    ...
    CoAPMessage message;
    alcs_msg_init(ctx, &message, COAP_MSG_CODE_GET, COAP_MESSAGE_TYPE_CON, 0,
                  &payload, NULL);

    char path[120];
    sprintf(path, "/dev/%s/%s/core/service/auth", ctl_item->productKey,
            ctl_item->deviceName);
    ...
    authParam->productKey =
      (char *)coap_malloc(strlen(ctl_item->productKey) + 1);
    strcpy(authParam->productKey, ctl_item->productKey);
    authParam->deviceName =
      (char *)coap_malloc(strlen(ctl_item->deviceName) + 1);
    strcpy(authParam->deviceName, ctl_item->deviceName);
    authParam->accessToken =
      (char *)coap_malloc(strlen(ctl_item->accessToken) + 1);
...
void alcs_auth_has_key(CoAPContext *ctx, NetworkAddr *addr,
                       AuthParam *auth_param)
{
    ...
    item.productKey = auth_param->productKey;
    item.accessToken =
      auth_param->accessToken; //(char*) coap_malloc
                               //(strlen(auth_param->accessToken) + 1);
    // strcpy (item.accessToken, auth_param->accessToken);
    do_auth(ctx, addr, &item, auth_param->user_data, auth_param->handler);
}

void alcs_auth_nego_key(CoAPContext *ctx, AlcsDeviceKey *devKey,
                        AuthHandler handler)
{
    ...
    sprintf(payloadbuf, payload_format, method, ++dev->seq, devKey->pk,
            devKey->dn, accesskeys);
    ...
    char path[120];
    sprintf(path, "/dev/%s/%s/core/service/auth/select", devKey->pk,
            devKey->dn);
    CoAPServerPath_2_option(path, &message);
...
int alcs_add_client_key(CoAPContext *ctx, const char *accesskey,
                        const char *accesstoken, const char *productKey,
...
void heart_beat_cb(CoAPContext *ctx, CoAPReqResult result, void *userdata,
                   NetworkAddr *remote, CoAPMessage *message)
{
    COAP_DEBUG("heart_beat_cb, message addr:%p, networkaddr:%p!", message,
               remote);
...
void on_client_auth_timer(CoAPContext *ctx)
{
    ...
    char              payloadbuf[64];
    sprintf(payloadbuf,
            "{\"id\":%d,\"version\":\"1.0\",\"params\":{\"delayTime\":%d}}",
            ++dev->seq, 5000);
    ...
    list_for_each_entry_safe(node, next, ctl_head, lst, session_item)
    {
        ...
        if (node->heart_time + node->interval > tick) {
            CoAPMessage message;
            alcs_msg_init(ctx, &message, COAP_MSG_CODE_GET,
                          COAP_MESSAGE_TYPE_CON, 0, &payload, NULL);
