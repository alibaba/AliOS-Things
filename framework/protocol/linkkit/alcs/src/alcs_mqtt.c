...
static alcs_mqtt_status_e __alcs_mqtt_publish(char *topic, int qos, void *data,
                                              int len)
{
    return (mqtt_publish(topic, qos, data, len) != 0)
             ? ALCS_MQTT_STATUS_ERROR
             : ALCS_MQTT_STATUS_SUCCESS;
}

static alcs_mqtt_status_e __alcs_mqtt_send_response(char *topic, int id,
                                                    int code, char *data)
{
    char *             msg_pub = NULL;
    uint16_t           msg_len = 0;
    ...
    msg_len =
      strlen(ALCS_MQTT_THING_LAN_PREFIX_RESPONSE_FMT) + 20 + strlen(data) + 1;

    if ((msg_pub = ALCS_ADAPTER_malloc(msg_len)) == NULL) {
        return ALCS_MQTT_STATUS_ERROR;
    }

    snprintf(msg_pub, msg_len, ALCS_MQTT_THING_LAN_PREFIX_RESPONSE_FMT, id,
             code, data);
...
static alcs_mqtt_status_e __alcs_mqtt_kv_set(const char *key, const void *val,
                                             int len, int sync)
{
...
static alcs_mqtt_status_e __alcs_mqtt_kv_get(const char *key, void *buffer,
                                             int *buffer_len)
{
...
alcs_mqtt_status_e __alcs_mqtt_prefix_secret_save(
  const char *pk, uint16_t pk_len, const char *dn, uint16_t dn_len,
  const char *prefix, uint16_t prefix_len, const char *secret,
  uint16_t secret_len)
{
    ...
    if (pk == NULL || pk_len >= PRODUCT_KEY_MAXLEN || dn == NULL ||
        dn_len >= DEVICE_NAME_MAXLEN || prefix == NULL || secret == NULL) {
    ...
    HAL_Snprintf(key_source, pk_len + dn_len + 1, "%.*s%.*s", pk_len, pk,
                 dn_len, dn);
    ...
    value[0] = prefix_len;
    value[1] = secret_len;
    HAL_Snprintf(&value[2], prefix_len + secret_len + 1, "%.*s%.*s", prefix_len,
                 prefix, secret_len, secret);

    if (ALCS_MQTT_STATUS_SUCCESS !=
        __alcs_mqtt_kv_set(key_md5_hexstr, value, prefix_len + secret_len + 3,
                           1)) {
                                                ...
                                                char *prefix, char *secret)
{
    ...
    if (pk == NULL || strlen(pk) >= PRODUCT_KEY_MAXLEN || dn == NULL ||
        strlen(dn) >= DEVICE_NAME_MAXLEN || prefix == NULL || secret == NULL) {
        log_err("Invalid Parameter");
        return ALCS_MQTT_STATUS_ERROR;
    }
    ...
    // Get Value
    if (ALCS_MQTT_STATUS_SUCCESS !=
        __alcs_mqtt_kv_get(key_md5_hexstr, value, &value_len)) {
...
static void __alcs_mqtt_subscribe_callback(char *topic, int topic_len,
                                           ...
                                           void *ctx)
{
    ...
    if (topic == NULL || payload == NULL || topic_len == 0 ||
        payload_len == 0) {
        return;
    }

    memset(topic_compare, 0, ALCS_MQTT_TOPIC_MAX_LEN);
    snprintf(topic_compare, ALCS_MQTT_TOPIC_MAX_LEN,
             ALCS_MQTT_PREFIX ALCS_MQTT_THING_LAN_PREFIX_GET_REPLY_FMT,
             alcs_mqtt_ctx->product_key, alcs_mqtt_ctx->device_name);

    log_info("Receivce Message, Topic: %.*s\n", topic_len, topic);
    // log_info("Receivce Message, Payload: %.*s\n",payload_len,payload);

    if ((strlen(topic_compare) == topic_len) &&
        (strncmp(topic_compare, topic, topic_len) == 0)) {
        int data_len = 0, prefix_len = 0, secret_len = 0, productKey_len = 0,
            deviceName_len = 0;
        char *data = NULL, *prefix = NULL, *secret = NULL, *productKey = NULL,
             *deviceName = NULL;
        data = json_get_value_by_name((char *)payload, payload_len, "data",
                                      &data_len, NULL);
        // log_info("Data: %.*s\n",data_len,data);

        if (NULL != data && 0 != data_len) {
            char back1, back2;
            prefix = json_get_value_by_name(
              data, data_len, ALCS_MQTT_JSON_KEY_PREFIX, &prefix_len, NULL);
            secret = json_get_value_by_name(
              data, data_len, ALCS_MQTT_JSON_KEY_SECRET, &secret_len, NULL);
            productKey = json_get_value_by_name(data, data_len,
                                                ALCS_MQTT_JSON_KEY_PRODUCT_KEY,
                                                &productKey_len, NULL);
            deviceName = json_get_value_by_name(data, data_len,
                                                ALCS_MQTT_JSON_KEY_DEVICE_NAME,
                                                &deviceName_len, NULL);

            // log_info("Get Reply, Product Key: %.*s, Device Name:
            ...
            if (NULL != alcs_mqtt_ctx->coap_ctx && prefix && secret) {
                ...
                if (productKey && deviceName) {
                    if (__alcs_mqtt_prefix_secret_save(
                          productKey, productKey_len, deviceName,
                          deviceName_len, prefix, prefix_len, secret,
                          secret_len) == ALCS_MQTT_STATUS_SUCCESS) {
                        iotx_alcs_subdev_item_t subdev_item;
                        memset(&subdev_item, 0,
                               sizeof(iotx_alcs_subdev_item_t));

                        memcpy(subdev_item.product_key, productKey,
                               productKey_len);
                        memcpy(subdev_item.device_name, deviceName,
                               deviceName_len);
                    ...
                    iotx_alcs_subdev_remove(alcs_mqtt_ctx->product_key,
                                            alcs_mqtt_ctx->device_name);
                    if (ALCS_MQTT_STATUS_SUCCESS !=
                        __alcs_mqtt_kv_set(ALCS_MQTT_JSON_KEY_PREFIX, prefix,
                                           prefix_len, 1)) {
                        log_err("ALCS KV Set Prefix Fail");
                        ;
                    }
                    if (ALCS_MQTT_STATUS_SUCCESS !=
                        __alcs_mqtt_kv_set(ALCS_MQTT_JSON_KEY_SECRET, secret,
                                           secret_len, 1)) {
            ...
            if (ALCS_MQTT_STATUS_SUCCESS ==
                  __alcs_mqtt_kv_get(ALCS_MQTT_JSON_KEY_PREFIX, prefix,
                                     &prefix_len) &&
                ALCS_MQTT_STATUS_SUCCESS ==
                  __alcs_mqtt_kv_get(ALCS_MQTT_JSON_KEY_SECRET, secret,
                                     &secret_len)) {
                if (NULL != alcs_mqtt_ctx->coap_ctx && prefix_len &&
                    secret_len) {
                    alcs_add_svr_key(alcs_mqtt_ctx->coap_ctx, prefix, secret);
                }
            }
    ...
    memset(topic_compare, 0, ALCS_MQTT_TOPIC_MAX_LEN);
    snprintf(topic_compare, ALCS_MQTT_TOPIC_MAX_LEN,
             ALCS_MQTT_PREFIX ALCS_MQTT_THING_LAN_PREFIX_UPDATE_FMT,
             alcs_mqtt_ctx->product_key, alcs_mqtt_ctx->device_name);
    ...
    if ((strlen(topic_compare) == topic_len) &&
        ...
        id = json_get_value_by_name((char *)payload, payload_len, "id", &id_len,
                                    NULL);

        if (NULL != id && 0 != id_len) {
            strncpy(reqid, id, sizeof(reqid) - 1);
        }
        param = json_get_value_by_name((char *)payload, payload_len, "params",
                                       &param_len, NULL);
        if (NULL != param && 0 != param_len) {
            prefix = json_get_value_by_name(
              param, param_len, ALCS_MQTT_JSON_KEY_PREFIX, &prefix_len, NULL);

            if (NULL != alcs_mqtt_ctx->coap_ctx && prefix)
                if (0 != alcs_remove_svr_key(alcs_mqtt_ctx->coap_ctx, prefix)) {
                }
            if (ALCS_MQTT_STATUS_SUCCESS !=
                __alcs_mqtt_kv_del(ALCS_MQTT_JSON_KEY_PREFIX)) {
            ...
            snprintf(
              reply_topic, ALCS_MQTT_TOPIC_MAX_LEN,
              ALCS_MQTT_PREFIX ALCS_MQTT_THING_LAN_PREFIX_UPDATE_REPLY_FMT,
              alcs_mqtt_ctx->product_key, alcs_mqtt_ctx->device_name);
    ...
    memset(topic_compare, 0, ALCS_MQTT_TOPIC_MAX_LEN);
    snprintf(topic_compare, ALCS_MQTT_TOPIC_MAX_LEN,
             ALCS_MQTT_PREFIX ALCS_MQTT_THING_LAN_PREFIX_BLACKLIST_UPDATE_FMT,
             alcs_mqtt_ctx->product_key, alcs_mqtt_ctx->device_name);
    ...
    if ((strlen(topic_compare) == topic_len) &&
        ...
        param = json_get_value_by_name((char *)payload, payload_len, "params",
                                       &param_len, NULL);
        if (NULL != param && 0 != param_len) {
            blacklist = json_get_value_by_name(
              param, param_len, ALCS_MQTT_JSON_KEY_BLACK, &blacklist_len, NULL);
            if (NULL != alcs_mqtt_ctx->coap_ctx && blacklist) {
                alcs_set_revocation(alcs_mqtt_ctx->coap_ctx, blacklist);
                if (ALCS_MQTT_STATUS_SUCCESS !=
                    __alcs_mqtt_kv_set(ALCS_MQTT_JSON_KEY_BLACK, blacklist,
                                       blacklist_len, 1)) {
            ...
            char reply_topic[ALCS_MQTT_TOPIC_MAX_LEN] = { 0 };
            snprintf(reply_topic, ALCS_MQTT_TOPIC_MAX_LEN,
                     ALCS_MQTT_PREFIX
            ...
            if (ALCS_MQTT_STATUS_SUCCESS ==
                __alcs_mqtt_kv_get(ALCS_MQTT_JSON_KEY_BLACK, blacklist,
                                   &blacklist_len)) {
...
{
    return (mqtt_subscribe(topic, __alcs_mqtt_subscribe_callback, ctx) != 0)
             ? ALCS_MQTT_STATUS_ERROR
             : ALCS_MQTT_STATUS_SUCCESS;
}
...
{
    return (mqtt_unsubscribe(topic) != 0) ? ALCS_MQTT_STATUS_ERROR
                                          : ALCS_MQTT_STATUS_SUCCESS;
}

alcs_mqtt_status_e alcs_mqtt_init(void *handle, char *product_key,
                                  char *device_name)
{
    ...
    if (handle == NULL || product_key == NULL ||
        strlen(product_key) > PRODUCT_KEY_LEN || device_name == NULL ||
        strlen(device_name) > DEVICE_NAME_LEN) {
        return ALCS_MQTT_STATUS_ERROR;
    }
    ...
    snprintf(topic, ALCS_MQTT_TOPIC_MAX_LEN,
             ALCS_MQTT_PREFIX ALCS_MQTT_THING_LAN_PREFIX_GET_REPLY_FMT,
...
alcs_mqtt_status_e alcs_mqtt_deinit(void *handle, char *product_key,
                                    char *device_name)
{
    char               topic[ALCS_MQTT_TOPIC_MAX_LEN] = { 0 };
    alcs_mqtt_status_e status = ALCS_MQTT_STATUS_SUCCESS;
    ...
    if (handle == NULL || product_key == NULL ||
        strlen(product_key) > PRODUCT_KEY_LEN || device_name == NULL ||
        strlen(device_name) > DEVICE_NAME_LEN || ctx == NULL) {
        return ALCS_MQTT_STATUS_ERROR;
    }
    ...
    snprintf(topic, ALCS_MQTT_TOPIC_MAX_LEN,
             ALCS_MQTT_PREFIX ALCS_MQTT_THING_LAN_PREFIX_GET_REPLY_FMT,
             ctx->product_key, ctx->device_name);
    if (__alcs_mqtt_unsubscribe((void *)ctx, topic) !=
        ALCS_MQTT_STATUS_SUCCESS) {
        log_err("ALCS Subscribe Failed, Topic: %s", topic);
        status = ALCS_MQTT_STATUS_ERROR;
    }
...
alcs_mqtt_status_e alcs_mqtt_blacklist_update(void *ctx)
{
    ...
    if (ALCS_MQTT_STATUS_SUCCESS == __alcs_mqtt_kv_get(ALCS_MQTT_JSON_KEY_BLACK,
                                                       blacklist,
                                                       &blacklist_len)) {
...
alcs_mqtt_status_e alcs_mqtt_prefixkey_update(void *ctx)
{
    ...
    int          prefix_len                       = ALCS_MQTT_PREFIX_MAX_LEN,
        secret_len                                = ALCS_MQTT_SECRET_MAX_LEN;
    ...
    if (ALCS_MQTT_STATUS_SUCCESS ==
          __alcs_mqtt_kv_get(ALCS_MQTT_JSON_KEY_PREFIX, prefix, &prefix_len) &&
        ALCS_MQTT_STATUS_SUCCESS ==
          __alcs_mqtt_kv_get(ALCS_MQTT_JSON_KEY_SECRET, secret, &secret_len)) {
        // log_info("The prefix is  %.*s, deviceSecret is %.*s", prefix_len,
        // prefix, secret_len, secret);
...
alcs_mqtt_status_e alcs_prefixkey_get(const char *product_key,
                                      const char *device_name)
{
    ...
    snprintf(topic, ALCS_MQTT_TOPIC_MAX_LEN,
             ALCS_MQTT_PREFIX ALCS_MQTT_THING_LAN_PREFIX_GET_FMT, product_key,
             device_name);
...
alcs_mqtt_status_e alcs_mqtt_subdev_prefix_get(const char *product_key,
                                               const char *device_name)
{
    // int ret = 0;
    char *             msg_pub                        = NULL;
    ...
    snprintf(topic, ALCS_MQTT_TOPIC_MAX_LEN,
             ALCS_MQTT_PREFIX ALCS_MQTT_THING_LAN_PREFIX_GET_FMT,
             ctx->product_key, ctx->device_name);

    msg_len = strlen(ALCS_MQTT_THING_ALCS_SUBDEV_REQUEST) + 10 +
    ...
    snprintf(msg_pub, msg_len, ALCS_MQTT_THING_ALCS_SUBDEV_REQUEST, id,
             strlen(product_key), product_key, strlen(device_name),
             device_name);
