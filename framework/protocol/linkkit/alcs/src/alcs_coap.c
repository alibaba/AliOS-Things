...
void alcs_msg_init(CoAPContext *ctx, CoAPMessage *message, int code,
                   unsigned char type, int keep, CoAPLenString *payload,
                   void *userdata)
{
...
static int do_sendmsg(CoAPContext *context, NetworkAddr *addr,
                      CoAPMessage *message, char observe, unsigned short msgid,
                      CoAPLenString *token)
{
...
int alcs_sendmsg(CoAPContext *context, NetworkAddr *addr, CoAPMessage *message,
                 char observe, CoAPSendMsgHandler handler)
{
    message->handler = handler;
    return do_sendmsg(context, addr, message, observe, message->header.msgid,
                      NULL);
}

// msgid & token从接收到CoAPMessage获取
//若发送
int alcs_sendrsp(CoAPContext *context, NetworkAddr *addr, CoAPMessage *message,
                 char observe, unsigned short msgid, CoAPLenString *token)
{
    return do_sendmsg(context, addr, message, observe, msgid, token);
}

// observe
int alcs_observe_notify(CoAPContext *context, const char *path,
                        CoAPLenString *payload)
{
    int needAuth = alcs_resource_need_auth(context, path);
    COAP_INFO("alcs_observe_notify, payload: %.*s", payload->len,
              payload->data);
...
static void send_err_rsp(CoAPContext *ctx, NetworkAddr *addr, int code,
                         CoAPMessage *fromMsg)
{
    CoAPMessage   sendMsg;
...
static void recv_msg_handler(CoAPContext *context, const char *path,
                             NetworkAddr *remote, CoAPMessage *message)
{
    ...
    list_for_each_entry_safe(node, next, &resource_cb_head, lst,
                             resource_cb_item)
    {
...
// resource
int alcs_resource_register(CoAPContext *context, const char *pk, const char *dn,
                           const char *path, unsigned short permission,
                           unsigned int ctype, unsigned int maxage,
                           char needAuth, CoAPRecvMsgHandler callback)
{
    COAP_INFO("alcs_resource_register, ctx:%p", context);

    if (!needAuth) {
        resource_cb_item *item =
          (resource_cb_item *)coap_malloc(sizeof(resource_cb_item));
...
#ifdef USE_ALCS_SECURE
        return alcs_resource_register_secure(context, pk, dn, path, permission,
                                             ctype, maxage, callback);
#else
        return -1;
#endif
    }
...
void alcs_start_loop(CoAPContext *ctx, int newThread)
{
    ...
    if (alcs_ctx && !alcs_ctx->loop) {
        int stack_used = 0;
        if (!newThread || 0 != HAL_ThreadCreate(&handle, thread_routine,
                                                alcs_ctx, NULL, &stack_used)) {
...
int alcs_msg_setAddr(CoAPMessage *message, const char *path, const char *query)
{
    ...
    if (query) {
        CoAPStrOption_add(message, COAP_OPTION_URI_QUERY,
