...
#ifdef HDLC_UART
    ret = hdlc_uart_send(&hdlc_encode_ctx, at._pstuart, (void *)&c, 1, at._timeout, false);
#else
...
static int at_getc(char *c)
{
    ...
    aos_mutex_lock(&at.at_mutex, AOS_WAIT_FOREVER);
#ifdef HDLC_UART
    ret = hdlc_uart_recv(&hdlc_decode_ctx, at._pstuart, (void *)&data,
                         1, &recv_size, at._timeout);
#else
    ret = hal_uart_recv_II(at._pstuart, (void *)&data, 1,
...
static int at_write(const char *data, int size)
{
    ...
    aos_mutex_lock(&at.at_uart_send_mutex, AOS_WAIT_FOREVER);
#ifdef HDLC_UART
    ret = hdlc_uart_send(&hdlc_encode_ctx, at._pstuart, (void *)data, size, AOS_WAIT_FOREVER, true);
#else
...
#ifdef HDLC_UART
        ret = hdlc_uart_recv(&hdlc_decode_ctx, at._pstuart, (void *)(data + total_read),
                             size - total_read, &recv_size, at._timeout);
#else
        ret = hal_uart_recv_II(at._pstuart, (void *)(data + total_read), size - total_read,
                               &recv_size, at._timeout);
#endif
        if (ret != 0) {
