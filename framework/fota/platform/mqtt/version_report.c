...
*/
// extern UINT32 aos_get_version_info(UINT8 version_num[4], UINT8 random_num[4],
// UINT8 mac_address[4], UINT8 chip_code[4], UINT8 *output_buffer, UINT32
// output_buffer_size);
...
static int active_gen_topic_name(char *buf, size_t buf_len,
                                 const char *product_key,
                                 const char *device_name, const char *topic)
{
    int ret;
    ret = snprintf(buf, buf_len, "/sys/%s/%s/thing/deviceinfo/%s", product_key,
                   device_name, topic);
...
static int active_publish(const char *topic_type, const char *msg)
{
    ...
    ret = active_gen_topic_name(topic_name, ACTIVE_TOPIC_LEN, product_key,
                                device_name, topic_type);
    if (ret < 0) {
...
static int active_gen_update_msg(char *buf, size_t buf_len, int id,
                                 const char *active_info)
{
    int ret;
    ret = snprintf(buf, buf_len,
                   "{\"id\":%d,\"version\":\"1.0\",\"params\":[{\"attrKey\":"
                   "\"SYS_ALIOS_ACTIVATION\",\"attrValue\":\"%s\"}],\"method\":"
...
static int active_gen_delete_msg(char *buf, size_t buf_len, int id,
                                 const char *active_info)
{
    int ret;
    ret =
      snprintf(buf, buf_len,
               "{\"id\":%d,\"version\":\"1.0\",\"params\":[{\"attrKey\":\"SYS_"
               "ALIOS_ACTIVATION\"}],\"method\":\"thing.deviceinfo.delete\"}",
               id, active_info);
...
static int active_parse_response(const char *response, int buf_len,
                                 active_resp_info_t *response_parmas)
{
...
int version_report()
{
    ...
    ret = aos_get_version_info(version, random_num, mac, chip_code,
                               (uint8_t *)output, ACTIVE_INFO_LEN);
    if (ret) {
