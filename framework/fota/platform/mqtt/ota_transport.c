...
{
    int ret;
    ret = snprintf(buf, buf_len, "/ota/device/%s/%s/%s", ota_topic_type,
...
static int ota_mqtt_publish(const char *topic_type, const char *msg)
{
    ...
    // ret = IOT_MQTT_Publish(g_ota_device_info.pclient, topic_name,
    // &topic_msg);
    if (ret < 0) {
...
static void ota_mqtt_sub_callback(char *topic, int topic_len, void *payload,
                                  int payload_len, void *ctx)
{
    OTA_LOG_I("----------------------ota_mqtt_sub_callback--------------------,"
              "payload=%s\n",
              payload);
...
// void aliot_mqtt_ota_callback(void *pcontext, void *pclient,
...
{
    int ret;
    ret = snprintf(buf, buf_len, "{\"id\":%d,\"params\":{\"version\":\"%s\"}}",
                   id, version);
...
int8_t platform_ota_subscribe_upgrade(aos_cloud_cb_t msgCallback)
{
    ...
    int ret = ota_mqtt_gen_topic_name(g_upgrad_topic, OTA_MQTT_TOPIC_LEN,
                                      "upgrade", g_ota_device_info.product_key,
                                      g_ota_device_info.device_name);
    if (ret < 0) {
    ...
    // ret = IOT_MQTT_Subscribe(g_ota_device_info.pclient, g_upgrad_topic,
    // IOTX_MQTT_QOS1,
    //                          aliot_mqtt_ota_callback, (void*)msgCallback);

    ret = mqtt_subscribe(g_upgrad_topic, ota_mqtt_sub_callback,
                         (void *)msgCallback);
