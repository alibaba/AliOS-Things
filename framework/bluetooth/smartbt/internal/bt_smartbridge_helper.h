...
#ifndef _BT_SMART_BRIDGE_HELPER_H_
#define _BT_SMART_BRIDGE_HELPER_H_
...
typedef struct {
    ...
    aos_mutex_t                 mutex;
    aos_semaphore_t             done_semaphore;
    OSStatus                     result;
    aos_bt_uuid_t               uuid;
    aos_bt_smart_attribute_t  *attr_head;
    aos_bt_smart_attribute_t  *attr_tail;
    uint32_t                     attr_count;
    uint16_t                     server_mtu;
...
typedef struct {
    aos_bt_timer_t        timer;      // AOS timer object
    aos_bool_t         is_started; // is it started?
    aos_bool_t         one_shot;   // one-shot?
...
OSStatus  smartbridge_helper_get_scan_results                 ( aos_bt_smart_scan_result_t **result_list,
                                                                uint32_t *count );

aos_bool_t    smartbridge_helper_socket_check_actions_enabled      ( aos_bt_smartbridge_socket_t *socket,
                                                                     uint8_t action_bits );
aos_bool_t    smartbridge_helper_socket_check_actions_disabled     ( aos_bt_smartbridge_socket_t *socket,
                                                                     uint8_t action_bits );
void           smartbridge_helper_socket_set_actions               ( aos_bt_smartbridge_socket_t *socket,
                                                                     uint8_t action_bits );
void           smartbridge_helper_socket_clear_actions             ( aos_bt_smartbridge_socket_t *socket,
                                                                     uint8_t action_bits );



aos_bool_t    peripheral_helper_socket_check_actions_enabled      ( aos_bt_peripheral_socket_t *socket,
                                                                    uint8_t action_bits );
aos_bool_t    peripheral_helper_socket_check_actions_disabled     ( aos_bt_peripheral_socket_t *socket,
                                                                    uint8_t action_bits );
void           peripheral_helper_socket_set_actions                ( aos_bt_peripheral_socket_t *socket,
                                                                     uint8_t action_bits );
void           peripheral_helper_socket_clear_actions              ( aos_bt_peripheral_socket_t *socket,
                                                                     uint8_t action_bits );
