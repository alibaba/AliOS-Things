...
typedef struct bt_smartbridge_att_cache {
    ...
    aos_bool_t                     is_active;
    aos_bool_t                     is_discovering;
    aos_bt_smart_device_t          remote_device;
    uint16_t                        connection_handle;
    aos_bt_smart_attribute_list_t  attribute_list;
    aos_mutex_t                    mutex;
} bt_smartbridge_att_cache_t;

typedef struct {
    uint32_t                   count;
    aos_bt_uuid_t            *att_cache_services;
...
OSStatus bt_smartbridge_att_cache_enable( uint32_t cache_count, aos_bt_uuid_t cache_services[], uint32_t service_count )
{
    uint32_t a;
    OSStatus result;
    bt_smartbridge_att_cache_manager_t *manager;
    aos_bt_uuid_t *services = NULL;
    ...
    if ( manager == NULL ) {
        return AOS_BT_OUT_OF_HEAP_SPACE;
    }

    if ( service_count != 0 ) {
        services = (aos_bt_uuid_t *)malloc_named( "cache_services", service_count * sizeof(aos_bt_uuid_t) );
        if ( services == NULL ) {
            return AOS_BT_OUT_OF_HEAP_SPACE;
        }
    }
    ...
    if ( service_count != 0 ) {
        ...
        memcpy( manager->att_cache_services, cache_services, service_count * sizeof(aos_bt_uuid_t) );
    }
    ...
    result = aos_rtos_init_mutex( &manager->mutex );
    if ( result != AOS_BT_SUCCESS ) {
    ...
    for ( a = 0; a < manager->count; a++ ) {
        result = aos_rtos_init_mutex( &manager->pool[a].mutex );
        if ( result != AOS_BT_SUCCESS ) {
            goto error;
        }
...
OSStatus bt_smartbridge_att_cache_disable( void )
{
    ...
    while ( linked_list_remove_node_from_front( &manager->free_list, &node ) == AOS_BT_SUCCESS ) {
        bt_smartbridge_att_cache_t *cache = (bt_smartbridge_att_cache_t *)node->data;

        aos_bt_smart_attribute_delete_list( &cache->attribute_list );
    }

    linked_list_deinit( &manager->free_list );

    while ( linked_list_remove_node_from_front( &manager->used_list, &node ) == AOS_BT_SUCCESS ) {
        bt_smartbridge_att_cache_t *cache = (bt_smartbridge_att_cache_t *)node->data;
    ...
    aos_rtos_deinit_mutex( &manager->mutex );

    /* Deinitialise mutexes for protecting access to cached attributes */
    for ( a = 0; a < manager->count; a++ ) {
        aos_rtos_deinit_mutex( &manager->pool[a].mutex );
    }
...
{
    return ( att_cache_manager == NULL ) ? AOS_FALSE : AOS_TRUE;
}
...
OSStatus bt_smartbridge_att_cache_lock( bt_smartbridge_att_cache_t *cache )
{
    ...
    return aos_rtos_lock_mutex( &cache->mutex );
}
...
OSStatus bt_smartbridge_att_cache_unlock( bt_smartbridge_att_cache_t *cache )
{
    ...
    return aos_rtos_unlock_mutex( &cache->mutex );
}

OSStatus bt_smartbridge_att_cache_find( const aos_bt_smart_device_t *remote_device, bt_smartbridge_att_cache_t **cache )
{
    OSStatus      result;
    ...
    /* Lock protection */
    result = aos_rtos_lock_mutex( &att_cache_manager->mutex );
    if ( result != AOS_BT_SUCCESS ) {
        return result;
    }
    ...
    /* Unlock protection */
    aos_rtos_unlock_mutex( &att_cache_manager->mutex );
    return result;
}
...
static OSStatus smartbridge_att_cache_get_free_cache( bt_smartbridge_att_cache_t **free_cache )
{
    ...
    if ( result != AOS_BT_SUCCESS ) {
        ...
        if ( result == AOS_BT_SUCCESS ) {
            ...
            if ( result == AOS_BT_SUCCESS ) {
                aos_bt_smart_attribute_list_t *list = (aos_bt_smart_attribute_list_t *)node->data;
...
static OSStatus smartbridge_att_cache_discover_all( bt_smartbridge_att_cache_t *cache, uint16_t connection_handle )
{
    ...
    aos_bt_smart_attribute_t     **primary_service_array    = NULL;
    aos_bt_smart_attribute_t     **characteristic_array     = NULL;
    aos_bt_smart_attribute_t      *characteristic_value     = NULL;
    aos_bt_smart_attribute_t      *descriptor_with_no_value = NULL;
    aos_bt_smart_attribute_t      *descriptor_with_value    = NULL;
    aos_bt_smart_attribute_t      *iterator                 = NULL;
    OSStatus                         result                   = AOS_BT_SUCCESS;
    OSStatus                         error_code_var           = AOS_BT_ERROR;
    uint32_t                         i                        = 0;
    uint32_t                         j                        = 0;
    uint32_t                         primary_service_count    = 0;
    uint32_t                         characteristic_count     = 0;
    aos_bt_smart_attribute_list_t  primary_service_list;
    aos_bt_smart_attribute_list_t  branch_primary_service_list;
    aos_bt_smart_attribute_list_t  included_service_list;
    aos_bt_smart_attribute_list_t  characteristic_list;
    aos_bt_smart_attribute_list_t  descriptor_list;

    aos_bool_t characteristic_list_merged = AOS_FALSE;
    aos_bool_t included_service_list_merged = AOS_FALSE;
    ...
    aos_rtos_lock_mutex( &cache->mutex );

    result = aos_bt_smart_attribute_create_list( &primary_service_list );

    aos_rtos_unlock_mutex( &cache->mutex );

    CHECK_FOR_ERROR( result != AOS_BT_SUCCESS, result );
    ...
    if ( att_cache_manager->att_cache_services_count == 0 ) {
        ...
        for ( i = 0; i < att_cache_manager->att_cache_services_count; i++ ) {
            ...
            CHECK_FOR_ERROR( result != AOS_BT_SUCCESS, result );
            result = aos_bt_smart_attribute_merge_lists( &primary_service_list, &branch_primary_service_list );
            CHECK_FOR_ERROR( result != AOS_BT_SUCCESS, result );
        }
    }

    aos_bt_smart_attribute_get_list_count( &primary_service_list, &primary_service_count );

    primary_service_array = (aos_bt_smart_attribute_t **)malloc_named( "svc_array",
                                                                       primary_service_count * sizeof(aos_bt_smart_attribute_t));

    CHECK_FOR_ERROR( primary_service_array == NULL, AOS_BT_OUT_OF_HEAP_SPACE );

    /* Keep the original pointers to the primary service list before the list gets merged */
    aos_bt_smart_attribute_get_list_head( &primary_service_list, &iterator );
    ...
    for ( i = 0; i < primary_service_count; i++ ) {
        ...
        CHECK_FOR_ERROR( result == AOS_BT_GATT_TIMEOUT, result );

        aos_rtos_lock_mutex( &cache->mutex );

        result = aos_bt_smart_attribute_merge_lists( &primary_service_list, &included_service_list );

        if( result == AOS_BT_SUCCES )
        {
            included_service_list_merged = AOS_TRUE;
        }
        ...
        */
        ...
        CHECK_FOR_ERROR( result == AOS_BT_GATT_TIMEOUT || result == AOS_BT_TIMEOUT, result );

        aos_bt_smart_attribute_get_list_count( &characteristic_list, &characteristic_count );


        characteristic_array = (aos_bt_smart_attribute_t **)malloc_named( "char_array",
                                                                          characteristic_count * sizeof(characteristic_list));

        CHECK_FOR_ERROR( characteristic_array == NULL, AOS_BT_OUT_OF_HEAP_SPACE );

        /* Keep the original pointers to the characteristic list before the list gets merged. */
        aos_bt_smart_attribute_get_list_head( &characteristic_list, &iterator );
        ...
        for ( j = 0; j < characteristic_count; j++ ) {
            ...
            if ( ( characteristic_array[j]->value.characteristic.properties & 0x02 ) != 0 ) {
                ...
                result = aos_bt_smart_attribute_create( &characteristic_value, AOS_ATTRIBUTE_TYPE_NO_VALUE, 0 );
            ...
            if ( characteristic_value != NULL ) {
                ...
                result = aos_bt_smart_attribute_add_to_list( &primary_service_list, characteristic_value );
            ...
            if ( characteristic_array[j]->value.characteristic.descriptor_start_handle <=
                ...
                aos_bt_smart_attribute_get_list_head( &descriptor_list, &descriptor_with_no_value );
                ...
                while ( descriptor_with_no_value != NULL ) {
                    ...
                    result = aos_bt_smart_attribute_add_to_list( &primary_service_list, descriptor_with_value );
                ...
                result = aos_bt_smart_attribute_delete_list( &descriptor_list );
        ...
        result = aos_bt_smart_attribute_merge_lists( &primary_service_list, &characteristic_list );

        if ( result == AOS_BT_SUCCESS ) {
            characteristic_list_merged = AOS_TRUE;
        }
    ...
    //aos_bt_smart_attribute_print_list( &(cache->attribute_list) );
...
error:
    ...
    if ( descriptor_with_value != NULL ) {
        aos_bt_smart_attribute_delete( descriptor_with_value );
    }
    ...
    if ( characteristic_list_merged == AOS_FALSE ) {
        aos_bt_smart_attribute_delete_list( &characteristic_list );
    }

    if ( included_service_list_merged == AOS_FALSE ) {
        aos_bt_smart_attribute_delete_list( &included_service_list );
    }

    aos_bt_smart_attribute_delete_list( &descriptor_list );
    aos_bt_smart_attribute_delete_list( &primary_service_list );
...
static bool   smartbridge_att_cache_find_by_device_callback( linked_list_node_t *node_to_compare, void *user_data )
{
    ...
    aos_bt_smart_device_t    *remote_device     = (aos_bt_smart_device_t *)user_data;
...
static bool   smartbridge_att_cache_get_free_callback( linked_list_node_t *node_to_compare, void *user_data )
{
    ...
    return ( cache->is_active == AOS_FALSE ) ? true : false;
}
