...
typedef struct {
    ...
    *socket;                         /**< A socket associated with the auto connection */
    aos_bt_smart_connection_settings_t
...
typedef struct {
    ...
    aos_bt_smart_advertising_report_t  report;        /* Remote BT device */
...
static void        smartbridge_gatt_auto_conn_handler            ( aos_bt_device_address_t bdaddr,
                                                                   uint16_t connection_handle );
static aos_bool_t smartbridge_gap_auto_conn_user_cfg            ( const aos_bt_device_address_t device_address,
...
static OSStatus    smartbridge_gap_auto_conn_scan_report_handler ( const aos_bt_smart_advertising_report_t *report );
...
static int8_t      smartbridge_auto_conn_find_dev_by_addr        ( const aos_bt_device_address_t device_address );
static OSStatus    smartbridge_auto_conn_list_init               ( void );
static OSStatus    smartbridge_auto_conn_list_deinit             ( void );
static OSStatus    smartbridge_auto_conn_list_add                ( const aos_bt_smart_advertising_report_t *report );
static OSStatus    smartbridge_auto_conn_list_remove             ( aos_bt_device_address_t bdaddr );
...
static OSStatus    smartbridge_auto_conn_list_set_state          ( const aos_bt_device_address_t bdaddr,
                                                                   smartbridge_auto_conn_state_t state );
static OSStatus    smartbridge_auto_conn_list_get_by_state       ( aos_bt_smart_advertising_report_t **report,
                                                                   smartbridge_auto_conn_state_t state );
...
aos_bt_smartbridge_socket_t           *connecting_socket = NULL;
...
aos_bt_gatt_char_declaration_t         current_characteristic;

/** Auto Connection Establishment Procedure */
static aos_semaphore_t                                       g_auto_conn_sem = NULL;
static smartbridge_auto_conn_entity_t                         g_auto_conn_dev_table[BTM_BLE_MAX_BG_CONN_DEV_NUM];
static aos_bt_smartbridge_auto_connection_parms_cback_t      g_auto_conn_cback = NULL;
static aos_bt_smart_scan_settings_t                          g_auto_conn_scan_cfg;
...
static void smartbridge_gatt_connection_handler( uint16_t connection_handle )
{
    ...
    aos_sem_signal( &connecting_socket->semaphore );
}

static void smartbridge_gatt_auto_conn_handler( aos_bt_device_address_t bdaddr, uint16_t connection_handle )
{
    int8_t                                   idx = 0;
    aos_bt_smartbridge_socket_t            *p_socket;
    ...
    aos_sem_signal(&g_auto_conn_sem);
}
...
static void smartbridge_gatt_disconnection_handler( uint16_t connection_handle )
{
    ...
    aos_bt_smartbridge_socket_t *removed_socket = NULL;

    /* Remove socket from the connected list */
    if ( bt_smartbridge_socket_manager_remove_socket( connection_handle, &removed_socket ) == AOS_BT_SUCCESS ) {
        ...
        bt_smartbridge_att_cache_set_active_state( (bt_smartbridge_att_cache_t *)removed_socket->att_cache, AOS_FALSE );
        removed_socket->att_cache = NULL;

        /* Check if disconnection is from host or remote device */
        if ( smartbridge_helper_socket_check_actions_enabled( removed_socket, SOCKET_ACTION_HOST_DISCONNECT ) == AOS_TRUE ) {
            /* Disconnection is originated from the host. Notify app thread that disconnection is complete */
            aos_sem_signal( &removed_socket->semaphore );
        } else {
            /* Notify app that connection is disconnected by the remote device */
            if ( removed_socket->disconnection_callback != NULL ) {
                aos_rtos_send_asynchronous_event( AOS_BT_EVT_WORKER_THREAD, smartbridge_app_disconnection_handler,
                                                  (void *)removed_socket );
            }
...
static void smartbridge_gatt_read_operation_complete_handler( aos_bt_gatt_data_t *response_data )
{
    aos_bt_smart_attribute_t *attr;
    uint8_t *data = NULL;
    /* Create a new attribute */
    aos_bt_smart_attribute_create( &attr, AOS_ATTRIBUTE_TYPE_CHARACTERISTIC_VALUE, response_data->len );
    ...
    if ( attr != NULL ) {
        ...
        //memcpy( &attr->type, &subprocedure.uuid, sizeof(aos_bt_uuid_t));

        /* Update temporary variables */
        if ( smartbridge_subprocedure.attr_head == NULL ) {
            smartbridge_subprocedure.attr_head = attr;
        }
        smartbridge_subprocedure.result = AOS_BT_SUCCESS;
        ...
        smartbridge_subprocedure.result = AOS_BT_OUT_OF_HEAP_SPACE;
    }

}

static OSStatus smartbridge_gatt_notification_indication_handler( aos_bt_gatt_operation_complete_t *operation_complete )
{
    ...
    if ( operation_complete->op == GATTC_OPTYPE_INDICATION ) {
        ...
        if ( aos_bt_gatt_send_indication_confirm( connection_handle, attribute_handle ) != 0 ) {
    ...
    if ( bt_smartbridge_socket_manager_find_socket_by_handle( connection_handle, &socket ) == AOS_BT_SUCCESS ) {
        if ( bt_smartbridge_att_cache_is_enabled() == AOS_TRUE && socket->att_cache != NULL ) {

            bt_smartbridge_att_cache_t      *cache          = (bt_smartbridge_att_cache_t *)socket->att_cache;
            aos_bt_smart_attribute_list_t *att_cache_list = NULL;
            aos_bt_smart_attribute_t      *att            = NULL;
            ...
            if ( aos_bt_smart_attribute_search_list_by_handle( att_cache_list, attribute_handle, &att ) == AOS_BT_SUCCESS ) {
                aos_bt_uuid_t uuid       = att->type;
                aos_bool_t    is_new_att = AOS_FALSE;
                ...
                if ( length > att->value_length ) {
                    ...
                    aos_bt_smart_attribute_remove_from_list( att_cache_list, attribute_handle );
                    att = NULL;

                    /* Create a new one and marked as new */
                    aos_bt_smart_attribute_create( &att, AOS_ATTRIBUTE_TYPE_CHARACTERISTIC_VALUE, length );
                    is_new_att = AOS_TRUE;
                }
                ...
                if ( is_new_att == AOS_TRUE ) {
                    /* Add newly created att to the list */
                    aos_bt_smart_attribute_add_to_list( att_cache_list, att );
                }
            }
        ...
        if ( socket->notification_callback != NULL ) {
            if ( oNoErr != aos_rtos_send_asynchronous_event( AOS_BT_EVT_WORKER_THREAD, smartbridge_app_notification_handler,
                                                             (void *)socket ) ) {
...
static void smartbridge_gatt_discover_characteristic_descriptor_result( aos_bt_gatt_event_data_t *p_event_data )
{
    /* Create attribute(s) based on information included in the response PDU */
    aos_bt_smart_attribute_t *attr;

    aos_bt_smart_attribute_create( &attr, AOS_ATTRIBUTE_TYPE_NO_VALUE, 0 );
...
static void smartbridge_gatt_discover_services_result( aos_bt_gatt_event_data_t *p_event_data )
{
    /* Create attribute(s) based on information included in the response PDU */
    aos_bt_smart_attribute_t *attr;

    aos_bt_smart_attribute_create( &attr, AOS_ATTRIBUTE_TYPE_PRIMARY_SERVICE, 0 );
...
static void smartbridge_gatt_discover_included_services_result( aos_bt_gatt_event_data_t *p_event_data )
{
    /* Create attribute(s) based on information included in the response PDU */
    aos_bt_smart_attribute_t *attr;
    //uint16_t start_handle = 0x0;

    aos_bt_smart_attribute_create( &attr, AOS_ATTRIBUTE_TYPE_INCLUDE, 0 );
...
static void smartbridge_gatt_discover_characteristic_result(  aos_bt_gatt_event_data_t *p_event_data )
{
    /* Create attribute(s) based on information included in the response PDU */
    aos_bt_smart_attribute_t *attr;

    aos_bt_smart_attribute_create( &attr, AOS_ATTRIBUTE_TYPE_CHARACTERISTIC, 0 );
    ...
    if ( attr != NULL ) {
        ...
        memcpy( &attr->value.characteristic.uuid, &current_characteristic.char_uuid, sizeof(aos_bt_uuid_t) );
...
static void smartbridge_gatt_discovery_complete_handler( aos_bt_gatt_event_data_t *p_event_data )
{
    ...
    switch ( discovery_complete_type ) {
        ...
        case GATT_DISCOVER_CHARACTERISTIC_DESCRIPTORS:
            if ( smartbridge_subprocedure.attr_count != 0 ) {
                ...
                smartbridge_subprocedure.result = AOS_BT_ITEM_NOT_IN_LIST;
            }
...
aos_bt_gatt_status_t smartbridge_gatt_callback( aos_bt_gatt_evt_t event, aos_bt_gatt_event_data_t *p_event_data )
{
...
OSStatus aos_bt_smartbridge_init( uint8_t count )
{
    OSStatus result;
    ...
    bt_smartbridge_log( "Initialising AOS SmartBridge ..." );
    ...
    result = aos_rtos_init_semaphore(&g_auto_conn_sem, 1);
    if (result != oNoErr) {
...
OSStatus aos_bt_smartbridge_deinit( void )
{
    ...
    /* Delete semaphore */
    aos_rtos_deinit_semaphore(&g_auto_conn_sem);
...
aos_bool_t aos_bt_smartbridge_is_scanning( void )
{
    return ( initialised == AOS_TRUE ) ? smartbridge_bt_interface_is_scanning( ) : AOS_FALSE;
}

aos_bool_t aos_bt_smartbridge_is_ready_to_connect( void )
{
    return ( initialised == AOS_FALSE || connecting_socket != NULL ) ? AOS_FALSE : AOS_TRUE;
}

OSStatus aos_bt_smartbridge_start_scan( const aos_bt_smart_scan_settings_t *settings,
...
OSStatus aos_bt_smartbridge_stop_scan( void )
{
    if ( initialised == AOS_FALSE ) {
        return AOS_BT_SMART_APPL_UNINITIALISED;
    }
...
OSStatus aos_bt_smartbridge_get_scan_result_list( aos_bt_smart_scan_result_t **result_list, uint32_t *count )
{
    if ( initialised == AOS_FALSE ) {
        return AOS_BT_SMART_APPL_UNINITIALISED;
    }
...
OSStatus aos_bt_smartbridge_set_auto_connection_action( aos_bool_t start_stop,
                                                        const aos_bt_smart_scan_settings_t *scan_settings, aos_bt_smartbridge_auto_connection_parms_cback_t p_auto_conn_cback )
{
    OSStatus                            err = oNoErr;
    uint32_t                            duration;
    ...
    if (start_stop) {
        ...
        memcpy(&g_auto_conn_scan_cfg, scan_settings, sizeof(aos_bt_smart_scan_settings_t));
        ...
        memset(&g_auto_conn_scan_cfg, 0, sizeof(aos_bt_smart_scan_settings_t));
        err = aos_bt_smartbridge_stop_scan();
    }
    return err;
}

OSStatus aos_bt_smartbridge_create_socket( aos_bt_smartbridge_socket_t *socket )
{
    if ( initialised == AOS_FALSE ) {
        return AOS_BT_SMART_APPL_UNINITIALISED;
    }
    ...
    return aos_rtos_init_semaphore( &socket->semaphore, 1 );
}

OSStatus aos_bt_smartbridge_delete_socket( aos_bt_smartbridge_socket_t *socket )
{
    OSStatus result;
    if ( initialised == AOS_FALSE ) {
        return AOS_BT_SMART_APPL_UNINITIALISED;
...
OSStatus aos_bt_smartbridge_get_socket_status( aos_bt_smartbridge_socket_t *socket,
    ...
    if ( socket->state == SOCKET_STATE_LINK_ENCRYPTED ) {
        *status = SMARTBRIDGE_SOCKET_CONNECTED;
        ...
        if ( smartbridge_helper_socket_check_actions_disabled( socket,
                                                               SOCKET_ACTION_ENCRYPT_USING_BOND_INFO | SOCKET_ACTION_INITIATE_PAIRING ) == AOS_TRUE ) {
            *status = SMARTBRIDGE_SOCKET_CONNECTED;
        } else {
...
OSStatus aos_bt_smartbridge_connect( aos_bt_smartbridge_socket_t *socket, const aos_bt_smart_device_t *remote_device,
...
{
    aos_bt_smartbridge_socket_t *found_socket;
    ...
    if ( connecting_socket != NULL ) {
        ...
        return AOS_BT_CONNECT_IN_PROGRESS;
    }

    if ( bt_smartbridge_socket_manager_is_full() == AOS_TRUE ) {
        return AOS_BT_MAX_CONNECTIONS_REACHED;
    }

    if ( bt_smartbridge_socket_manager_find_socket_by_address( &remote_device->address,
                                                               &found_socket ) == AOS_BT_SUCCESS ) {
        /* device is already connected */
        return AOS_BT_SOCKET_IN_USE;
    }

    bt_smartbridge_log( "connect()...socket things are okay" );

    /* Clean-up accidentally set semaphores */
    while ( aos_rtos_get_semaphore( &socket->semaphore, AOS_NO_WAIT ) == AOS_BT_SUCCESS ) {
    }
    ...
    if ( smartbridge_helper_socket_check_actions_enabled( socket, SOCKET_ACTION_INITIATE_PAIRING ) == AOS_TRUE ) {
        ...
        aos_bt_stop_pairing( socket->remote_device.address );
    }
    ...
    /* Wait for connection */
    aos_rtos_get_semaphore( &socket->semaphore, socket->connection_settings.timeout_second * 1000 );
    ...
    if ( socket->state == SOCKET_STATE_LINK_CONNECTED ) {
        ...
        if ( smartbridge_helper_socket_check_actions_enabled( socket, SOCKET_ACTION_INITIATE_PAIRING ) == AOS_TRUE ||
             smartbridge_helper_socket_check_actions_enabled( socket, SOCKET_ACTION_ENCRYPT_USING_BOND_INFO ) == AOS_TRUE ) {
            ...
            if ( socket->state != SOCKET_STATE_LINK_ENCRYPTED ) {
                result = AOS_BT_ENCRYPTION_FAILED;
                aos_bt_dev_delete_bonded_device( (uint8_t *)remote_device->address );
                goto error;
            }
        }
    } else {
        result = AOS_BT_SOCKET_NOT_CONNECTED;
        goto error;
    }

    /* Successful */
    if ( bt_smartbridge_att_cache_is_enabled() == AOS_TRUE ) {
        ...
        bt_smartbridge_att_cache_set_active_state( cache, AOS_TRUE );
...
OSStatus aos_bt_smartbridge_disconnect( aos_bt_smartbridge_socket_t *socket, aos_bool_t remove_it_from_whitelist )
{
    if ( initialised == AOS_FALSE ) {
        return AOS_BT_SMART_APPL_UNINITIALISED;
    }
    ...
    if ( socket->state != SOCKET_STATE_DISCONNECTED ) {
        ...
        /* Mark att cache as inactive and reset reference to cache */
        bt_smartbridge_att_cache_set_active_state( (bt_smartbridge_att_cache_t *)socket->att_cache, AOS_FALSE );
...
OSStatus aos_bt_smartbridge_set_transmit_power( aos_bt_smartbridge_socket_t *socket, int8_t transmit_power_dbm )
{
    if ( initialised == AOS_FALSE || socket->state == SOCKET_STATE_DISCONNECTED ) {
        return AOS_BT_SMART_APPL_UNINITIALISED;
    }
...
OSStatus aos_bt_smartbridge_set_bond_info( aos_bt_smartbridge_socket_t *socket,
                                           const aos_bt_smart_security_settings_t *settings, const aos_bt_smart_bond_info_t *bond_info )
{
    if ( initialised == AOS_FALSE ) {
        return AOS_BT_SMART_APPL_UNINITIALISED;
    }
...
OSStatus aos_bt_smartbridge_clear_bond_info( aos_bt_smartbridge_socket_t *socket )
{
    if ( initialised == AOS_FALSE ) {
        return AOS_BT_SMART_APPL_UNINITIALISED;
    }
...
OSStatus aos_bt_smartbridge_enable_pairing( aos_bt_smartbridge_socket_t *socket,
                                            const aos_bt_smart_security_settings_t *settings, aos_bt_smart_bonding_callback_t bonding_callback )
{
    if ( initialised == AOS_FALSE ) {
        return AOS_BT_SMART_APPL_UNINITIALISED;
    }
...
OSStatus aos_bt_smartbridge_disable_pairing( aos_bt_smartbridge_socket_t *socket )
{
    if ( initialised == AOS_FALSE ) {
        return AOS_BT_SMART_APPL_UNINITIALISED;
    }
...
OSStatus aos_bt_smartbridge_enable_attribute_cache( uint32_t cache_count, aos_bt_uuid_t cache_services[],
...
OSStatus aos_bt_smartbridge_disable_attribute_cache( void )
{
    if ( initialised == AOS_FALSE ) {
        return AOS_BT_SMART_APPL_UNINITIALISED;
    }
...
OSStatus aos_bt_smartbridge_remove_attribute_cache( aos_bt_smartbridge_socket_t *socket )
{
    if ( initialised == AOS_FALSE ) {
        return AOS_BT_SMART_APPL_UNINITIALISED;
    }
...
OSStatus aos_bt_smartbridge_enable_attribute_cache_notification( aos_bt_smartbridge_socket_t *socket,
                                                                 aos_bool_t is_notification_or_indication )
{
    aos_bt_smart_attribute_list_t *list;
    aos_bt_smart_attribute_t      *iterator;
    ...
    if ( bt_smartbridge_att_cache_is_enabled( ) == AOS_FALSE ) {
        return AOS_BT_ATT_CACHE_UNINITIALISED;
    }
    ...
    if ( bt_smartbridge_att_cache_is_discovering( cache ) == AOS_TRUE ) {
        return AOS_BT_DISCOVER_IN_PROGRESS;
    }
    ...
    result = aos_bt_smart_attribute_get_list_head( list, &iterator );
    if ( result != AOS_BT_SUCCESS ) {
        return result;
    }
...
OSStatus aos_bt_smartbridge_disable_attribute_cache_notification( aos_bt_smartbridge_socket_t *socket )
{
    aos_bt_smart_attribute_list_t *list;
    aos_bt_smart_attribute_t      *iterator;
    bt_smartbridge_att_cache_t      *cache;
...
OSStatus aos_bt_smartbridge_get_attribute_cache_list( aos_bt_smartbridge_socket_t *socket,
                                                      aos_bt_smart_attribute_list_t **att_cache_list )
{
    if ( initialised == AOS_FALSE ) {
        return AOS_BT_SMART_APPL_UNINITIALISED;
    }
...
OSStatus aos_bt_smartbridge_get_attribute_cache_by_handle( aos_bt_smartbridge_socket_t *socket, uint16_t handle,
                                                           aos_bt_smart_attribute_t *attribute, uint16_t size )
{
    bt_smartbridge_att_cache_t      *cache          = NULL;
    aos_bt_smart_attribute_list_t *att_cache_list = NULL;
    aos_bt_smart_attribute_t      *att            = NULL;
    ...
    result = aos_bt_smart_attribute_search_list_by_handle( att_cache_list, handle, &att );
...
OSStatus aos_bt_smartbridge_get_attribute_cache_by_uuid( aos_bt_smartbridge_socket_t *socket, const aos_bt_uuid_t *uuid,
                                                         uint16_t starting_handle, uint16_t ending_handle, aos_bt_smart_attribute_t *attribute, uint32_t size )
{
    bt_smartbridge_att_cache_t      *cache          = NULL;
    aos_bt_smart_attribute_list_t *att_cache_list = NULL;
    aos_bt_smart_attribute_t      *att            = NULL;
    ...
    result = aos_bt_smart_attribute_search_list_by_uuid( att_cache_list, uuid, starting_handle, ending_handle, &att );
    if ( result == AOS_BT_SUCCESS ) {
        if ( att->value_struct_size + ATTR_COMMON_FIELDS_SIZE > size ) {
            result = AOS_BT_ATTRIBUTE_VALUE_TOO_LONG;
        } else {
...
OSStatus aos_bt_smartbridge_get_service_from_attribute_cache_by_uuid( aos_bt_smartbridge_socket_t *socket,
                                                                      const aos_bt_uuid_t *uuid, uint16_t starting_handle, uint16_t ending_handle, aos_bt_smart_attribute_t *attribute,
                                                                      uint32_t size )
{
    OSStatus result;
    uint16_t _starting_handle = starting_handle;
    uint16_t _ending_handle = ending_handle;
    aos_bt_uuid_t service_uuid = { .len = LEN_UUID_16, .uu.uuid16 = GATT_UUID_PRI_SERVICE };

    /* Find service */
    while ( ( result = aos_bt_smartbridge_get_attribute_cache_by_uuid( socket, &service_uuid, _starting_handle,
                                                                       _ending_handle, attribute, size ) ) == AOS_BT_SUCCESS ) {
...
OSStatus aos_bt_smartbridge_get_characteritics_from_attribute_cache_by_uuid( aos_bt_smartbridge_socket_t *socket,
                                                                             const aos_bt_uuid_t *uuid, uint16_t starting_handle, uint16_t ending_handle, aos_bt_smart_attribute_t *attribute,
                                                                             uint32_t size )
{
    OSStatus result;
    uint16_t _starting_handle = starting_handle;
    uint16_t _ending_handle = ending_handle;
    aos_bt_uuid_t characteristic_uuid = { .len = LEN_UUID_16, .uu.uuid16 = GATT_UUID_CHAR_DECLARE };

    /* Find service */
    while ( ( result = aos_bt_smartbridge_get_attribute_cache_by_uuid( socket, &characteristic_uuid, _starting_handle,
                                                                       _ending_handle, attribute, size ) ) == AOS_BT_SUCCESS ) {
...
OSStatus aos_bt_smartbridge_refresh_attribute_cache_characteristic_value( aos_bt_smartbridge_socket_t *socket,
                                                                          uint16_t handle )
{
    bt_smartbridge_att_cache_t      *cache          = NULL;
    aos_bt_smart_attribute_list_t *att_cache_list = NULL;
    aos_bt_smart_attribute_t      *current_att    = NULL;
    aos_bt_smart_attribute_t      *refreshed_att  = NULL;
    OSStatus                         result;
    ...
    result = aos_bt_smart_attribute_search_list_by_handle( att_cache_list, handle, &current_att );
    if ( result == AOS_BT_SUCCESS ) {
        ...
        if ( result == AOS_BT_SUCCESS ) {
            ...
            result = aos_bt_smart_attribute_remove_from_list( att_cache_list, current_att->handle );
            if ( result == AOS_BT_SUCCESS ) {
                result = aos_bt_smart_attribute_add_to_list( att_cache_list, refreshed_att );
            }
        }
    }
...
OSStatus aos_bt_smartbridge_write_attribute_cache_characteristic_value( aos_bt_smartbridge_socket_t *socket,
                                                                        const aos_bt_smart_attribute_t *char_value )
{
    bt_smartbridge_att_cache_t      *cache          = NULL;
    aos_bt_smart_attribute_list_t *att_cache_list = NULL;
    aos_bt_smart_attribute_t      *att            = NULL;
    ...
    if ( char_value->value_length <= ATT_STANDARD_VALUE_LENGTH ) {
        result = smartbridge_bt_interface_write_characteristic_value( socket->connection_handle,
                                                                      (aos_bt_smart_attribute_t *)char_value );
    } else {
    ...
    result = aos_bt_smart_attribute_search_list_by_handle( att_cache_list, char_value->handle, &att );
    ...
    if ( result == AOS_BT_SUCCESS ) {
        ...
        if ( char_value->value_length != att->value_length ) {
            result = aos_bt_smart_attribute_remove_from_list( att_cache_list, att->handle );
            ...
            result = aos_bt_smart_attribute_create( &att, AOS_ATTRIBUTE_TYPE_CHARACTERISTIC_VALUE, char_value->value_length );
            ...
            result = aos_bt_smart_attribute_add_to_list( att_cache_list, att );
        } else {
            memcpy( att->value.value, char_value->value.value, char_value->value_length );
        }
    } else if ( result == AOS_BT_ITEM_NOT_IN_LIST ) {
...
static OSStatus smartbridge_app_notification_handler( void *arg )
{
    aos_bt_smartbridge_socket_t *socket = (aos_bt_smartbridge_socket_t *)arg;
...
// OSStatus aos_bt_smartbridge_bond_info_update( aos_bt_device_link_keys_t paired_device_keys )
// {
//     aos_bt_smart_bond_info_t          bond_info;
//     aos_bt_ble_keys_t                  *le_security_keys;
//     aos_bt_device_sec_keys_t           *security_keys;

//     OSStatus result = AOS_BT_ERROR;

//     memcpy(bond_info.peer_address, paired_device_keys.bd_addr, BD_ADDR_LEN );

//     security_keys       = (aos_bt_device_sec_keys_t*)&paired_device_keys.key_data;
...
//     le_security_keys    = ( aos_bt_ble_keys_t* )&security_keys->le_keys;
...
//     bond_info.address_type  = (aos_bt_smart_address_type_t)security_keys->ble_addr_type;
...
// static OSStatus smartbridge_gap_bonding_handler( uint16_t connection_handle, const aos_bt_smart_bond_info_t* bond_info )
// {
//     aos_bt_smartbridge_socket_t* socket;

//     if ( bt_smartbridge_socket_manager_find_socket_by_handle( connection_handle, &socket ) == AOS_BT_SUCCESS )
//     {
//         /* Bonding successful. Update socket's bond info and post callback to AOS_BT_EVT_WORKER_THREAD */
//         memcpy( &socket->bond_info, bond_info, sizeof( *bond_info ) );

//         if ( socket != NULL && socket->bonding_callback != NULL )
//         {
//             aos_rtos_send_asynchronous_event( AOS_BT_EVT_WORKER_THREAD, smartbridge_app_pairing_handler, (void*)socket );
//         }
...
{
    OSStatus                             err = oNoErr;
    aos_bt_gatt_status_t                gatt_status;
    aos_bt_smart_advertising_report_t  *report = NULL;
    ...
    if (err == oNoErr) {
        ...
        if (smartbridge_gap_auto_conn_user_cfg(report->remote_device.address,
            ...
            aos_bt_smartbridge_socket_t *p_socket = g_auto_conn_dev_table[idx].socket;

            /* Clear semaphore */
            while (aos_rtos_get_semaphore(&g_auto_conn_sem, AOS_NO_WAIT) == oNoErr);
            ...
            gatt_status = aos_bt_gatt_le_connect((uint8_t *)report->remote_device.address,
            ...
            /* Wait for completion */
            err = aos_rtos_get_semaphore(&g_auto_conn_sem, 5000);
            if (err != oNoErr) {
                bt_smartbridge_log("%s: Create LE Connection failed: %s",
                                   __FUNCTION__,
                                   err == oTimeoutErr ? "Timeout" : "Unknown Error");
err_exit:
            ...
            /* Next Connection */
            aos_rtos_send_asynchronous_event(AOS_BT_WORKER_THREAD,
...
{
    OSStatus                                err = oNoErr;
    aos_bt_smart_advertising_report_t     *match_report = NULL;
...
static OSStatus smartbridge_gap_auto_conn_scan_cmpl_handler(void *arg)
{
    ...
    if (g_auto_conn_scan_duration > 0) {
        ...
        if (err == AOS_BT_PENDING && g_auto_conn_scan_duration < (uint16_t)(-1)) {
...
static OSStatus smartbridge_gap_auto_conn_cryption( void *arg )
{
    ...
    if (socket != NULL && socket->auto_connection_callback != NULL) {
        if (socket->security_settings.authentication_requirements != BT_SMART_AUTH_REQ_NONE) {
            ...
            aos_bt_start_encryption( &socket->remote_device.address );
            aos_rtos_get_semaphore(&g_auto_conn_sem, AOS_NEVER_TIMEOUT);
                                                      ...
                                                      const uint8_t *device_name, const uint8_t *p_data, uint8_t length )
{
    ...
    aos_bt_cfg_settings.ble_scan_cfg.conn_min_interval        = g_auto_conn_dev_table[idx].conn_settings.interval_min;
    aos_bt_cfg_settings.ble_scan_cfg.conn_max_interval        = g_auto_conn_dev_table[idx].conn_settings.interval_max;
    aos_bt_cfg_settings.ble_scan_cfg.conn_latency             = g_auto_conn_dev_table[idx].conn_settings.latency;
    aos_bt_cfg_settings.ble_scan_cfg.conn_supervision_timeout =
...
static OSStatus smartbridge_auto_conn_user_parms( void *arg )
{
    ...
    if ( (err = g_auto_conn_cback(p_dev_info->bdaddr,
        ...
        memset( (void *) &p_entity->conn_settings, 0, sizeof(aos_bt_smart_connection_settings_t) );
        ...
        memcpy( (void *) &p_entity->conn_settings, (void *) &parm.conn_settings,
                sizeof(aos_bt_smart_connection_settings_t) );
        /* Security settings */
        memcpy( (void *) &p_entity->socket->security_settings, (void *) &parm.security_settings,
                sizeof(aos_bt_smart_security_settings_t) );
    }
    return err;
}
...
{
    int8_t idx = 0;
    aos_bt_smartbridge_socket_t *temp_socket;
    ...
    for (idx = 0; idx < sizeof(g_auto_conn_dev_table) / sizeof(g_auto_conn_dev_table[0]); idx++) {
        ...
        if ((temp_socket != (void *)0)
            && (memcmp((void *)temp_socket->remote_device.address,
                       (void *)device_address,
                       sizeof(aos_bt_device_address_t)) == 0)) {
            return idx;
        }
    }
    return -1;
...
void smartbridge_auto_connection_encryption_check(const aos_bt_dev_pairing_cplt_t *p_pairing_cplt)
{
...
static OSStatus smartbridge_auto_conn_list_init(void)
{
    ...
    return aos_rtos_init_mutex(&g_auto_conn_list.mutex);
}

static OSStatus smartbridge_auto_conn_list_deinit(void)
{
    return aos_rtos_deinit_mutex(&g_auto_conn_list.mutex);
}

static aos_bool_t smartbridge_auto_conn_compare_bdaddr(linked_list_node_t *node_to_compare, void *user_data)
{
...
static aos_bool_t smartbridge_auto_conn_compare_state(linked_list_node_t *node_to_compare, void *user_data)
{
    smartbridge_auto_conn_report_t *report = (smartbridge_auto_conn_report_t *)node_to_compare;
...
static OSStatus smartbridge_auto_conn_list_add(const aos_bt_smart_advertising_report_t *report)
{
    ...
    require_action(report != NULL, exit, err = oParamErr);

    /* Check if this report is existed. */
    aos_rtos_lock_mutex(&g_auto_conn_list.mutex);
    ...
    if (err == oNoErr) {
        ...
        if (report_found->state == SMARTBRIDGE_AUTO_CONN_STATE_FREE
             ...
             */
            err = oAlreadyInUseErr;
        }
        aos_rtos_unlock_mutex(&g_auto_conn_list.mutex);
        return err;
    } else {
    ...
    memcpy(&new_report->report, report, sizeof(aos_bt_smart_advertising_report_t));
                                                     ...
                                                     smartbridge_auto_conn_state_t state)
{
    ...
    require_action(bdaddr != NULL, exit, err = oParamErr);
