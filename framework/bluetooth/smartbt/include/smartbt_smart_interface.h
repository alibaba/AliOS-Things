...
#ifndef _SMART_BT_SMART_INTERFACE_H_
#define _SMART_BT_SMART_INTERFACE_H_
...
} aos_bt_smart_filter_duplicated_t;
...
} aos_bt_smart_advertising_event_t;
...
} aos_bt_smart_advertising_type_t;
...
} aos_bt_smart_auth_req_t;

/*
 * Bluetooth Smart device bonding request type
 */
typedef enum {
    /* Bonding event -- user should reply it with #aos_bt_smart_bond_reply() */
    AOS_BT_SMART_BOND_PASS_KEY_REQ     = 0x01,
    /* Bonding event -- user should reply it with #aos_bt_smart_bond_reply() */
    AOS_BT_SMART_BOND_USR_CONFIRM_REQ  = 0x02,
    /* Bonding event -- user should reply it with #aos_bt_smart_bond_reply() */
    AOS_BT_SMART_BOND_OOB_DATA_REQ     = 0x03,
    /* Bonding event -- user should not reply it */
    AOS_BT_SMART_BOND_PASS_KEY_NOTIFY  = 0x04,
} aos_bt_smart_bond_request_type_t;
...
} aos_bt_smart_key_distribution_t;
 ...
 */
typedef struct {
    aos_bt_device_address_t            address;      /* Bluetooth device address */
    aos_bt_smart_address_type_t        address_type; /* Address Type             */
...
} aos_bt_smart_scan_settings_t;

/*
 *  Bluetooth Smart advertising report
 */
typedef struct {
    aos_bt_smart_device_t              remote_device;   /* Remote device                                      */
    int8_t                              signal_strength; /* RSSI in dBm                                        */
    aos_bt_smart_advertising_event_t   event;           /* Advertising event received                         */
...
} aos_bt_smart_advertising_report_t;

/*
 *  Bluetooth Smart scan result
 */
typedef struct aos_bt_smart_scan_result {
    aos_bt_smart_device_t              remote_device;                   /* Remote device                       */
    int8_t                              signal_strength;                 /* RSSI in dBm                         */
    aos_bt_smart_advertising_report_t  last_scan_response_received;     /* Last scan response event received   */
    aos_bt_smart_advertising_report_t  last_advertising_event_received; /* Last advertising event received     */
    struct aos_bt_smart_scan_result   *next;                            /* Pointer to the next scan result     */

    /* Additional flag to help application filter scan results */
    aos_bool_t                         filter_display;                  /* Set to AOS_TRUE if filter display */

} aos_bt_smart_scan_result_t;
...
} aos_bt_smart_advertising_settings_t;
...
} aos_bt_smart_eir_data_structure_t;
...
typedef struct {
    ...
    aos_bt_io_capabilities_t         io_capabilities;             /* Device I/O capability                         */
    aos_bt_smart_auth_req_t          authentication_requirements; /* Authentication requirements                   */
    aos_bt_smart_oob_auth_t          oob_authentication;          /* OOB authentication data                       */
    uint8_t                           max_encryption_key_size;     /* Encryption key size (7 to 16 bytes)           */
    aos_bt_smart_key_distribution_t  master_key_distribution;     /* Bit mask of master/initiator key distribution */
    aos_bt_smart_key_distribution_t  slave_key_distribution;      /* Bit mask of slave/responder key distribution  */
...
typedef struct {
    /* Type of the request */
    aos_bt_smart_bond_request_type_t type;

    union {
        /* AOS_BT_SMART_BOND_PASS_KEY_REQ */
        struct {
            aos_bt_device_address_t addr;
        } passkey;

        /* AOS_BT_SMART_BOND_USR_CONFIRM_REQ */
        struct {
            aos_bt_device_address_t addr;
        ...
        /* AOS_BT_SMART_BOND_OOB_DATA_REQ */
        struct {
            aos_bt_device_address_t addr;
        } oob_data;

        /* AOS_BT_SMART_BOND_PASS_KEY_NOTIFY */
        struct {
            aos_bt_device_address_t addr;
            uint32_t                 passkey;
        } notify;
...
typedef struct {
    ...
    /* reply data -- only valid when 'res' is AOS_BT_SUCCESS */
    union {
        /* Used for AOS_BT_SMART_BOND_USER_CONFIRM */
        struct {
            aos_bt_device_address_t    addr;
        } confirm;

        /* Used for AOS_BT_SMART_BOND_PASS_KEY_REQ */
        struct {
            aos_bt_device_address_t    addr;
            uint32_t                    passkey;
        } passkey;

        /* Used for AOS_BT_SMART_BOND_OOB_DATA_REQ */
        struct {
            aos_bt_device_address_t    addr;
            uint8_t                    *data;
            uint8_t                     len;
 ...
 */
//typedef event_handler_t aos_bt_smart_scan_complete_callback_t;
typedef OSStatus (*aos_bt_smart_scan_complete_callback_t)( void *arg );

/*
 * Bluetooth Smart advertising report callback
 */
typedef OSStatus (*aos_bt_smart_advertising_report_callback_t)  ( const aos_bt_smart_advertising_report_t *result );

/*
 * Bluetooth Smart Device Bond Callback.
 */
typedef OSStatus (*aos_bt_smart_bonding_callback_t)(const aos_bt_smart_bond_request_t *request);

/*
 * Bluetooth Smart advertising complete callback
 */
//typedef event_handler_t aos_bt_smart_advertising_complete_callback_t;
typedef OSStatus (*aos_bt_smart_advertising_complete_callback_t)( void *arg );
 ...
 *  @return OSStatus
 */
extern OSStatus aos_bt_smart_bond_reply(const aos_bt_smart_bond_reply_t *response);
