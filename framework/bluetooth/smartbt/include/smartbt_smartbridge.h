...
#ifndef _SMART_BT_SMART_BRIDGE_H_
#define _SMART_BT_SMART_BRIDGE_H_
...
} aos_bt_smartbridge_socket_status_t;
 ...
 */
typedef struct aos_bt_smartbridge_socket aos_bt_smartbridge_socket_t;

/*
 * Auto connection callback parameters
 */
typedef struct aos_bt_smartbridge_auto_conn_cback_parm aos_bt_smartbridge_auto_conn_cback_parms_t;

/*
 * Socket disconnection callback
 */
typedef OSStatus (*aos_bt_smartbridge_disconnection_callback_t)       ( aos_bt_smartbridge_socket_t *socket );

/*
 * Socket GATT notification callback
 */
typedef OSStatus (*aos_bt_smartbridge_notification_callback_t)        ( aos_bt_smartbridge_socket_t *socket,
                                                                        uint16_t attribute_handle );

/*
 * BLE Auto connection callback
 */
typedef OSStatus (*aos_bt_smartbridge_auto_connection_callback_t)     ( aos_bt_smartbridge_socket_t *socket );

/*
 * BLE Auto connection parameters callback -- auto connection procedure callback
 */
typedef OSStatus (*aos_bt_smartbridge_auto_connection_parms_cback_t)   ( const aos_bt_device_address_t random_bda,
                                                                         const char *name, const uint8_t *p_data, uint8_t len, aos_bt_smartbridge_auto_conn_cback_parms_t *parm );
 ...
 */
struct aos_bt_smartbridge_socket {
    linked_list_node_t
    node;                           /* Socket list node                                              */
    aos_bt_smart_device_t
    remote_device;                  /* Remote Bluetooth device AOS is connected with (bridging)    */
    uint16_t
    ...
    aos_bt_smart_security_settings_t
    security_settings;              /* Security settings                                             */
    aos_bt_smart_bond_info_t
    bond_info;                      /* Bond Info                                                     */
    aos_bt_smart_bond_request_t
...
struct aos_bt_smartbridge_auto_conn_cback_parm {
...
/* Initialise the AOS SmartBridge
 *
 * @note
 ...
 *
 * After calling @ref aos_bt_smartbridge_init, you may call:
 * \li  @ref aos_bt_smartbridge_enable_attribute_cache() to enable Attribute Cache
 *
 * @return @ref OSStatus
 */
...
/* Deinitialise the AOS SmartBridge
 *
 * @note
 ...
 * of packet exhange required between AOS SmartBridge and the server.
 *
 ...
 *
 * Specified services can be used in ATT cache generation, it makes AOS SmartBridge
 * only cache the services that application can support and operate.
 *
 * @param[in]  cache_count    : the number of caches that will be supported by AOS SmartBridge
...
/* Check if the AOS SmartBridge is currently scanning for Bluetooth Smart devices
 *
 * @note
 * This function checks if the Bluetooth Controller is currently scanning for Bluetooth
 * Smart devices
 *
 * @return @ref aos_bool_t
 */
 ...
 *     are retrieved using @ref aos_bt_smartbridge_get_scan_result_list once scan
 *     is complete
 ...
 * \li complete_callback runs on the context of AOS_BT_EVT_WORKER_THREAD
 * \li If the whitelist filter is enabled in the scan settings, only devices
 *     in the whitelist appear in the scan results. Call @ref aos_bt_smartbridge_add_device_to_whitelist()
 ...
 * @return @ref OSStatus
 */
OSStatus aos_bt_smartbridge_connect( aos_bt_smartbridge_socket_t                *socket,
                                     const aos_bt_smart_device_t                *remote_device,
                                     const aos_bt_smart_connection_settings_t   *settings,
                                     aos_bt_smartbridge_disconnection_callback_t disconnection_callback,
 ...
 *
 * @warning
 * This function must be called before calling @ref aos_bt_smartbridge_connect
 *
...
OSStatus aos_bt_smartbridge_set_bond_info( aos_bt_smartbridge_socket_t            *socket,
                                           ...
                                           const aos_bt_smart_bond_info_t         *bond_info );
 ...
 *
 * @warning
 * \li This function must be called before calling @ref aos_bt_smartbridge_connect
 * \li Callback functions run on the context of AOS_BT_EVT_WORKER_THREAD
 *
