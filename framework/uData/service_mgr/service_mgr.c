...
#ifdef DATA_TO_CLOUD
                if ((true == service_dtc_is_connect()) && (true == service_dtc_is_publish(g_service_db[index]->type))) {
                    aos_post_event(EV_UDATA, CODE_UDATA_DATA_PUBLISH, index);
                }
#endif
...
#ifdef DATA_TO_CLOUD
    if (SENSOR_IOCTL_DTC_CYCLE_SET == sensor_config->config.id) {
        ret = service_dtc_publish_cycle_set(type, sensor_config->config.dtc_cycle);
        return ret;
    }
#endif
