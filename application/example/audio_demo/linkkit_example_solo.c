...
/** 请开发者根据个人在IoT平台上的产品/设备配置，更新下面的四元组信息  */
#define PRODUCT_KEY      ""
...
static int user_property_set_event_handler(const int devid, const char *request, const int request_len)
{
    ...
    if (item_payload != NULL && cJSON_IsNumber(item_payload)) {
        EXAMPLE_TRACE("PowerSwitch = %d", item_payload->valueint);
        if((1 == item_payload->valueint) && (0 == power_state)) {
            power_state = 1;
    ...
    if (item_payload != NULL && cJSON_IsNumber(item_payload)) {
        EXAMPLE_TRACE("startRecord = %d", item_payload->valueint);
    ...
    if (item_payload != NULL && cJSON_IsString(item_payload)) {
        EXAMPLE_TRACE("PlayURL = %s", item_payload->valuestring);
        if(!strcmp(item_payload->valuestring, "/data/rec.pcm")) {
            param_array[0] = "loopback";
            param_array[1] = "2";
            param_array[2] = "start";
            cmd_sound_loop_handler(NULL, 0, 3, param_array);
        } else {
            param_array[0] = "play_url";
            param_array[1] = item_payload->valuestring;
            cmd_play_url_handler(NULL, 0, 2, param_array);
        }
    }

    /* Payload: AddMusicUrl */
    item_payload = cJSON_GetObjectItem(root, "AddMusicUrl");
    if (item_payload != NULL && cJSON_IsString(item_payload)) {
        EXAMPLE_TRACE("url = %s", item_payload->valuestring);
        add_url_2_playlist(item_payload->valuestring);
    }

    /* Payload: TTSText */
    item_payload = cJSON_GetObjectItem(root, "TTSText");
    if (item_payload != NULL && cJSON_IsString(item_payload)) {
        EXAMPLE_TRACE("TTSText = %s", item_payload->valuestring);
        param_array[0] = "tts";
        param_array[1] = item_payload->valuestring;
        param_array[2] = "/data/tts.mp3";
        test_tts_handle(3, param_array);
    }

    /* Payload: SetVolume */
    item_payload = cJSON_GetObjectItem(root, "SetVolume");
    if (item_payload != NULL && cJSON_IsNumber(item_payload)) {
        EXAMPLE_TRACE("set volume = %d", item_payload->valueint);
        aos_set_master_volume(item_payload->valueint);
    }

    /* Payload: RepeatType */
    item_payload = cJSON_GetObjectItem(root, "RepeatType");
    if (item_payload != NULL && cJSON_IsNumber(item_payload)) {
        EXAMPLE_TRACE("RepeatType = %d", item_payload->valueint);
    }

    /* Payload: EqualizerType */
    item_payload = cJSON_GetObjectItem(root, "EqualizerType");
    if (item_payload != NULL && cJSON_IsNumber(item_payload)) {
        EXAMPLE_TRACE("EqualizerType = %d", item_payload->valueint);
    }

    /* Payload: MediaSource */
    item_payload = cJSON_GetObjectItem(root, "MediaSource");
    if (item_payload != NULL && cJSON_IsNumber(item_payload)) {
        EXAMPLE_TRACE("EqualizerType = %d", item_payload->valueint);
    }
    ...
    if (item_payload != NULL && cJSON_IsNumber(item_payload)) {
        EXAMPLE_TRACE("PlayType = %d", item_payload->valueint);
        switch(item_payload->valueint) {
            ...
            case 1: // 播放
                if(pause_state) {
                    ...
                    param_array[0] = "play_current";
                    cmd_play_current_handler(NULL, 0, 1, param_array);
                }
                pause_state = 0;
                break;
            case 2: //暂停
    ...
    /* Payload: MusicSelect */
    item_payload = cJSON_GetObjectItem(root, "MusicSelect");
    if (item_payload != NULL && cJSON_IsNumber(item_payload)) {
        EXAMPLE_TRACE("MusicSelect = %d", item_payload->valueint);
        if(1 == item_payload->valueint) {
            param_array[0] = "play_pre";
            cmd_play_pre_handler(NULL, 0, 1, param_array);
        } else if (2 == item_payload->valueint) {
            param_array[0] = "play_next";
            cmd_play_next_handler(NULL, 0, 1, param_array);
        }
    }
