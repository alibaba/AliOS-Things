...
static map_t *achilles_normal_maplist[3] = {&achilles_normal0_map, &achilles_normal1_map, &achilles_normal2_map};
...
static map_t *achilles_destory_maplist[3] = {&achilles_destory0_map, &achilles_destory1_map, &achilles_destory2_map};
...
static map_t *titan_destory_maplist[3] = {&titan_destory0_map, &titan_destory1_map, &titan_destory2_map};
...
static map_t *ares_destory_maplist[3] = {&ares_destory0_map, &ares_destory1_map, &ares_destory2_map};
...
static map_t *venture_destory_maplist[3] = {&venture_destory0_map, &venture_destory1_map, &venture_destory2_map};
...
dfo_t *create_titan()
{
    ...
    act_seq_t **titan_act_seq_list = (act_seq_t **)malloc(sizeof(act_seq_t *) * titan->act_seq_list_len);
...
dfo_t *create_ares()
{
    ...
    act_seq_t **ares_act_seq_list = (act_seq_t **)malloc(sizeof(act_seq_t *) * ares->act_seq_list_len);
...
dfo_t *create_venture()
{
    ...
    act_seq_t **venture_act_seq_list = (act_seq_t **)malloc(sizeof(act_seq_t *) * venture->act_seq_list_len);
...
dfo_t *create_bullet()
{
    ...
    act_seq_t **bullet_act_seq_list = (act_seq_t **)malloc(sizeof(act_seq_t *) * bullet->act_seq_list_len);
...
void global_create(void)
{
    ...
    enemy_crafts = (dfo_t **)malloc(sizeof(dfo_t *) * (MAX_L_CRAFT + MAX_M_CRAFT + MAX_S_CRAFT));
    for (int i = 0; i < MAX_L_CRAFT + MAX_M_CRAFT + MAX_S_CRAFT; i++)
    {
        ...
        else if (i < MAX_L_CRAFT + MAX_M_CRAFT + MAX_S_CRAFT && i >= MAX_L_CRAFT + MAX_M_CRAFT)
...
void reload_dfo(dfo_t *craft, int pos_x, int pos_y)
{
    ...
    if (craft->model != Bullet)
        printf("reset craft %d at %d,%d\n", craft->model, craft->start_x, craft->start_y);
...
int aircraftBattle_init(void)
{
    ...
    global_create();
    g_chance = MY_CHANCE;
    aos_task_new("aircraftBattle_task", aircraftBattle_task, NULL, 5000);
