...
void timer1_thread(void)
{
    ...
    printf("timer1_thread is running !\n");

    ret = clock_gettime(CLOCK_REALTIME, &timer_spec_get_cur);
    if (ret != 0) {
        test_failed_flag = 1;
    ...
    if (count == 0) {
        if ((timer_spec_get_cur.tv_sec - timer_spec_get_origin.tv_sec) != 3) {
            test_failed_flag = 1;
            printf("timer it_value error !\n");
        }
    } else {
        if ((timer_spec_get_cur.tv_sec - timer_spec_get_pre.tv_sec) != 2) {
            test_failed_flag = 1;
            printf("timer it_interval error !\n");
        }
    }
...
void posix_timer_case1(void)
{
    ...
    printf("*********** posix_timer_case1 start ***********\n");
    ...
    /* create the timer */
    ret = timer_create(CLOCK_REALTIME, &timer1_sig, &timer1_id);
    if (ret != 0) {
        test_failed_flag = 1;
    ...
    ret = clock_gettime(CLOCK_REALTIME, &timer_spec_get_origin);
    if (ret != 0) {
        test_failed_flag = 1;
        printf("clock_gettime error !\n");
    }
    ...
    if (test_failed_flag == 0) {
        printf("posix_timer_case1 test OK !\n");
    }

    printf("*********** posix_timer_case1 end ***********\n\n");
}
...
void posix_timer_case2(void)
{
    ...
    printf("*********** posix_timer_case2 start ***********\n");

    ret = clock_gettime(CLOCK_REALTIME, &timer_spec_get1);
    if (ret != 0) {
        printf("clock_gettime error !\n");
        return;
    }
    ...
    clock_nanosleep(CLOCK_REALTIME, 0, &timer_spec_set, NULL);

    ret = clock_gettime(CLOCK_REALTIME, &timer_spec_get2);
    if (ret != 0) {
        printf("clock_gettime error !\n");
        return;
    }
    ...
    printf("posix_timer_case2 test OK !\n");

    printf("*********** posix_timer_case2 end ***********\n\n");
}
