...
static void lowpri_intrpt_process(void)
{
...
void test_realtime_intrpt_preempt(void)
{
    ...
    rttest_aux_intrpt_test_init(highpri_intrpt_process, lowpri_intrpt_process);
    ...
    rttest_aux_show_result(INTRPT_PREEMPT_ID, "intrpt_preempt", test_count, time_sum, time_max, time_min);

    krhino_task_del(&test_task1_tcb);
    krhino_sem_del(&wait_test_end);
}

#endif /* ENABLE_INTRPT_PREEMPT > 0 */
