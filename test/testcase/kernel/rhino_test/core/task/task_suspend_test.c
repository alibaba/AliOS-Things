...
{
    (void)krhino_task_dyn_create(&task_1_test, "task_suspend_test_1", 0, 10,
                                 0, TASK_TEST_STACK_SIZE,
                                 task_suspend_entry, 1);

    /* for smp sync */
    (void)krhino_task_sleep(RHINO_CONFIG_TICKS_PER_SECOND / 10);

    (void)krhino_task_dyn_create(&task_2_test, "task_suspend_test_2", 0, 11,
                                 0, TASK_TEST_STACK_SIZE,
                                 task_resume_entry, 1);
}
