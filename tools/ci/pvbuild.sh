...
for (( i=1; i<${arraylength}+1; i=i+3 ));do
    ...
    if [[ ${target} == *coapapp@mk3060* ]]; then
        key_store_file="example/coapapp/coapapp.c"
    ...
    if [[ ${target} == *linkkitapp@mk3060* ]]; then
        ...
        sed -i 's/AOS_LL_DEBUG/AOS_LL_INFO/g' example/linkkitapp/linkkit-example.c
    ...
    if [[ ${target} == *linkkitapp* ]]; then
        git checkout ${key_store_file}
        git checkout example/linkkitapp/linkkit-example.c
    fi
done
