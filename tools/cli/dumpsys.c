...
static void dumpsys_get_total_cpu_usage(float *total_cpu_usage)
{
    ...
    for (tmp = taskhead->next; tmp != taskend; tmp = tmp->next) {
        ...
        if (task->task_name != NULL) {
            if (strncmp(task->task_name, "idle_task",
                        strlen(task->task_name)) == 0) {
                total_usage = 100 - ((float)(task->task_exec_time)) /
                                      CPU_USAGE_STAT_PERIOD * 100;
                *total_cpu_usage = total_usage < 0 ? 0 : total_usage;
                return;
            }
        }
    }
...
uint32_t dumpsys_task_func(char *buf, uint32_t len, int detail)
{
...
#if (RHINO_CONFIG_CPU_USAGE_STATS > 0)
#if (RHINO_CONFIG_TASK_SCHED_STATS > 0)
    dumpsys_get_total_cpu_usage(&total_cpu_usage);
#else
    total_cpu_usage = (float)(g_cpu_usage / 100);
#endif

    aos_cli_printf("CPU usage : %-10.1f                    \n",
                   total_cpu_usage);
    aos_cli_printf("-----------------------------------------------------------"
                   "-------------\r\n");
    ...
    for (tmp = taskhead->next; tmp != taskend; tmp = tmp->next) {
        ...
        task_cpu_usage =
          ((float)(task->task_exec_time)) / CPU_USAGE_STAT_PERIOD * 100;
#endif
...
#if (RHINO_CONFIG_CPU_NUM > 1)
        aos_cli_printf(
          "%-19.18s%-9s%-5d%-10d%-12zu%-9llu%-9.1f%-11c%-10d%-10d%-10d\r\n",
          task_name, cpu_stat[taskstate], task->prio, task->stack_size,
          free_size, (unsigned long long)time_total, task_cpu_usage, yes,
          task->cpu_binded, task->cpu_num, task->cur_exc);
#else
        aos_cli_printf("%-19.18s%-9s%-5d%-10d%-12zu%-9llu%-9.1f%-11c\r\n",
                       task_name, cpu_stat[taskstate], task->prio,
                       task->stack_size, free_size,
                       (unsigned long long)time_total, task_cpu_usage, yes);
#endif
...
#if (RHINO_CONFIG_CPU_NUM > 1)
        aos_cli_printf(
          "%-19s%-9s%-5d%-10d%-12u%-9u%-9.1f%-11c%-10d%-10d%-10d\r\n",
          task_name, cpu_stat[taskstate], task->prio, task->stack_size,
          free_size, (unsigned int)time_total, task_cpu_usage, yes,
          task->cpu_binded, task->cpu_num, task->cur_exc);
#else
        aos_cli_printf("%-19s%-9s%-5d%-10d%-12u%-9u%-9.1f%-11c\r\n", task_name,
                       cpu_stat[taskstate], task->prio, task->stack_size,
                       free_size, (unsigned int)time_total, task_cpu_usage,
                       yes);
#endif
#endif
