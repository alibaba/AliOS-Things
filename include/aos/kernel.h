...
#define AOS_DEFAULT_APP_PRI 32
     ...
     *
     * @param[in]  event    memory refered by hdl pointer in event will be
     * freed.
     *
     * @return  N/A.
     */
    ...
    /**
     * This function will try to get flag set from given event, if the request
     * flag set is satisfied, it will return immediately, if the request flag
     * set is not satisfied with timeout(RHINO_WAIT_FOREVER,0xFFFFFFFF), the
     * caller task will be pended on event until the flag is satisfied, if the
     * request flag is not satisfied with timeout(RHINO_NO_WAIT, 0x0), it will
     * also return immediately. Note, this function should not be called from
     * interrupt context because it has possible to lead context switch and an
     * interrupt has no TCB to save context.
     *
     * @param[in]  event        event object pointer.
     * @param[in]  flags        request flag set.
     * @param[in]  opt          operation type, such as
     * AND,OR,AND_CLEAR,OR_CLEAR.
    ...
    /**
     * This function will set flag set to given event, and it will check if any
     * task which is pending on the event should be waken up.
     *
     * @param[in]  event    event object pointer.
    ...
    int aos_event_set(aos_event_t *event, unsigned int flags,
                      unsigned char opt);
