...
#else
        uint32_t byte_pos =
          txt_utf8_get_byte_id(ext->pwd_tmp, ext->cursor.pos - 1);
...
void lv_ta_set_one_line(lv_obj_t *ta, bool en)
{
    ...
    if (en != false) {
        ...
        lv_obj_set_height(ta, font_h + (style_ta->body.padding.ver +
...
void lv_ta_set_style(lv_obj_t *ta, lv_ta_style_t type, lv_style_t *style)
{
    ...
    switch (type) {
        ...
        case LV_TA_STYLE_CURSOR:
            ...
            lv_obj_refresh_ext_size(lv_page_get_scrl(
              ta)); /*Refresh ext. size because of cursor drawing*/
            break;
    }
}
 ...
 * 'mask_p' area (return 'true' if yes) LV_DESIGN_DRAW_MAIN: draw the object
 * (always return 'true') LV_DESIGN_DRAW_POST: drawing after every children are
 * drawn
    ...
    if (mode == LV_DESIGN_COVER_CHK) {
        ...
        if (ta_ext->cursor.style != NULL) {
            ...
            cur_style.text.color =
              cur_style.body.main_color; /*In block mode the letter color will
        ...
        if (ta_ext->cursor.type == LV_CURSOR_LINE) {
            ...
            lv_draw_label(&cur_area, mask, &cur_style, letter_buf,
                          LV_TXT_FLAG_NONE, 0);
...
#if LV_TXT_UTF8 != 0
            /*Swap the bytes (UTF-8 is big endian, but the MCUs are little
             * endian)*/
            if ((c & 0x80) == 0) { /*ASCII*/
                ...
                for (i = 0; i < 4; i++) {
                    if (p[0] == 0)
                        p++; /*Ignore leading zeros (they were in the end
                                originally)*/
                }
                lv_ta_add_text(ta, p);
            }
