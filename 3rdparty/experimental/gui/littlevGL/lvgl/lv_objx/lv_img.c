...
lv_obj_t *lv_img_create(lv_obj_t *par, lv_obj_t *copy)
{
    ...
    if (copy == NULL) {
        ...
        if (par != NULL)
            ...
            lv_obj_set_style(new_img,
                             &lv_style_plain); /*Set a style for screens*/
    } else {
...
void lv_img_set_src(lv_obj_t *img, const void *src_img)
{
    ...
    if (src_type == LV_IMG_SRC_VARIABLE) {
        ...
        ext->h = size.y;
        ext->chroma_keyed =
          1; /*Symbols always have transparent parts, Important because of cover
                check in the design function*/
...
lv_img_src_t lv_img_get_src_type(const void *src)
{
    ...
    if (u8_p[0] >= 'A' && u8_p[0] <= 'Z')
        ...
        return LV_IMG_SRC_VARIABLE; /*Mask the file format part og of lv_img_t
                                       header. IT should be 0 which means C
                                       array */
    else if (u8_p[0] >= ' ')
                          ...
                          lv_design_mode_t mode)
{
    ...
    if (mode == LV_DESIGN_COVER_CHK) {
        ...
        if (ext->src_type == LV_IMG_SRC_FILE ||
            ...
            lv_draw_label(&coords, mask, style, ext->src, LV_TXT_FLAG_NONE,
                          NULL);
