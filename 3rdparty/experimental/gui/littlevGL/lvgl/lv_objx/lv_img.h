...
#include "../lv_draw/lv_draw.h"
    ...
    typedef struct
    {
        ...
        const void
          *src; /*Image source: Pointer to an array or a file or a symbol*/

        lv_coord_t w; /*Width of the image (Handled by the library)*/
        lv_coord_t h; /*Height of the image (Handled by the library)*/
        uint8_t    src_type : 2;  /*See: lv_img_src_t*/
        uint8_t    auto_size : 1; /*1: automatically set the object size to the
                                     image size*/
        uint8_t chroma_keyed : 1; /*1: Chroma keyed image, LV_COLOR_TRANSP
                                     (lv_conf.h) pixels will be transparent
                                     (Handled by the library)*/
        uint8_t
          alpha_byte : 1; /*1: Extra byte for every pixel to define opacity*/
    } lv_img_ext_t;
     ...
     * @param copy pointer to a image object, if not NULL then the new object
     ...
     * v6.0. Use 'lv_img_set_src()' instead.
     * @param img -
     * @param fn -
     */
     ...
     *  - pointer to an 'lv_img_t' variable (image stored internally and
