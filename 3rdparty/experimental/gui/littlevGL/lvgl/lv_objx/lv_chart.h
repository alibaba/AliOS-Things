...
#include "lv_line.h"
    ...
    typedef struct
    {
        ...
        uint8_t
          type : 3; /*Line, column or point chart (from 'lv_chart_type_t')*/
        struct
        {
     ...
     * background
     * @param copy pointer to a chart background object, if not NULL then the
     * new object will be copied from it
     ...
     */
    void lv_chart_set_div_line_count(lv_obj_t *chart, uint8_t hdiv,
                                     uint8_t vdiv);
     ...
     */
    void lv_chart_set_next(lv_obj_t *chart, lv_chart_series_t *ser,
                           lv_coord_t y);
