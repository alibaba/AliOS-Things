...
#define LV_LABEL_POS_LAST 0xFFFF
    ...
    typedef enum
    {
        ...
        LV_LABEL_LONG_BREAK,  /*Keep the object width, break the too long lines
                                 and expand the object height*/
        LV_LABEL_LONG_SCROLL, /*Expand the object size and scroll the text on
                                 the parent (move the label object)*/
        LV_LABEL_LONG_DOT, /*Keep the size and write dots at the end if the text
                              is too long*/
    ...
    typedef struct
    {
        ...
        char dot_tmp[LV_LABEL_DOT_NUM +
                     1]; /*Store the character which are replaced by dots
        ...
        uint16_t dot_end; /*The text end position in dot mode (Handled by the
                             library)*/
        uint16_t
                   anim_speed; /*Speed of scroll and roll animation in px/sec unit*/
     ...
     * be '\0' terminated. Memory will be allocated to store the array by the
     * label.
     * @param label pointer to a label object
     ...
     */
    void lv_label_set_long_mode(lv_obj_t            *label,
    ...
    /**
     * Set the label to draw (or not draw) background specified in its style's
     * body
     * @param label pointer to a label object
     ...
     * character index, not byte index (different in UTF-8)
     * @param pos store the result here (E.g. index = 0 gives 0;0 coordinates)
     */
    void lv_label_get_letter_pos(lv_obj_t *label, uint16_t index,
                                 lv_point_t *pos);
     ...
     * @return the index of the letter on the 'pos_p' point (E.g. on 0;0 is the
     * 0. letter) Expressed in character index and not byte index (different in
     * UTF-8)
     */
     ...
     * not byte index (Different in UTF-8) 0: before first char.
