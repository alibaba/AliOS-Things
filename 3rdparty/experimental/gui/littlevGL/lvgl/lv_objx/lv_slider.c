...
#define LV_SLIDER_SIZE_MIN                                                  \
    4 /*hor. pad and ver. pad cannot make the bar or indicator smaller then \
         this [px]*/
                             ...
                             lv_design_mode_t mode)
{
    ...
    /*Draw the object*/
        ...
        lv_style_t *style_knob =
          lv_slider_get_style(slider, LV_SLIDER_STYLE_KNOB);
        ...
        if (pad_ver_bg * 2 + LV_SLIDER_SIZE_MIN >
            lv_area_get_height(&area_bg)) {
            pad_ver_bg =
              (lv_area_get_height(&area_bg) - LV_SLIDER_SIZE_MIN) >> 1;
        }
        if (pad_hor_bg * 2 + LV_SLIDER_SIZE_MIN > lv_area_get_width(&area_bg)) {
            pad_hor_bg =
              (lv_area_get_width(&area_bg) - LV_SLIDER_SIZE_MIN) >> 1;
        }
        ...
        if (ext->knob_in) { /*Enable extra size if the knob is inside */
            ...
            area_bg.x1 += slider_w < slider_h
                            ? pad_hor_bg
                            : 0; /*Pad only for vertical slider*/
            area_bg.x2 -= slider_w < slider_h
                            ? pad_hor_bg
                            : 0; /*Pad only for vertical slider*/
            area_bg.y1 += slider_w > slider_h
                            ? pad_ver_bg
                            : 0; /*Pad only for horizontal slider*/
            area_bg.y2 -= slider_w > slider_h
                            ? pad_ver_bg
                            : 0; /*Pad only for horizontal slider*/
        }
        ...
        if (pad_ver_indic * 2 + LV_SLIDER_SIZE_MIN >
            lv_area_get_height(&area_bg)) {
            pad_ver_indic =
              (lv_area_get_height(&area_bg) - LV_SLIDER_SIZE_MIN) >> 1;
        }
        if (pad_hor_indic * 2 + LV_SLIDER_SIZE_MIN >
                                 ...
                                 void *param)
{
    ...
    if (sign == LV_SIGNAL_PRESSED) {
        ...
        if (w > h) {
            lv_coord_t knob_w = h;
            p.x -= slider->coords.x1 +
                   h / 2; /*Modify the point to shift with half knob (important
            ...
            lv_coord_t knob_h = w;
            p.y -= slider->coords.y1 +
                   w / 2; /*Modify the point to shift with half knob (important
                             on the start and end)*/
        ...
        if (ext->knob_in == 0) {
            ...
            lv_coord_t x =
              LV_MATH_MIN(w / 2 + 1 + shadow_w, h / 2 + 1 + shadow_w);
