...
#define LV_BTNM_INACTIVE_MASK       0x20
     ...
     * long pressed on the button matrix Parameters: button matrix, text of the
     * released button
    ...
    typedef struct
    {
        ...
        lv_btnm_action_t
          action; /*A function to call when a button is releases*/
        lv_style_t
          *styles_btn[LV_BTN_STATE_NUM]; /*Styles of buttons in each state*/
        uint16_t
                 btn_cnt; /*Number of button in 'map_p'(Handled by the library)*/
     ...
     *             Example (practically use octal numbers): "\224abc": "abc"
     * text with 4 width and no long press
     */
    void lv_btnm_set_map(lv_obj_t *btnm, const char **map);

    /**
     * Set a new callback function for the buttons (It will be called when a
     * button is released)
     ...
     * @param id index of the currently toggled button (ignored if 'en' ==
     * false)
     */
