                            ...
                            lv_design_mode_t mode)
{
    ...
    /*Draw the object*/
         ...
         * If the object is in focus 'lv_obj_get_style()' will give a pointer to
         * tmp style and to the real object style. It is important because of
         * style change tricks below*/
        ...
        /*Temporally modify the line meter to draw thicker and longer lines
         * where labels are*/
...
static void lv_gauge_draw_scale(lv_obj_t *gauge, const lv_area_t *mask)
{
    ...
    for (i = 0; i < label_num; i++) {
        ...
        int16_t scale_act =
          (int32_t)((int32_t)(max - min) * i) / (label_num - 1);
        scale_act += min;
        ...
        lv_draw_label(&label_cord, mask, style, scale_txt, LV_TXT_FLAG_NONE,
                      NULL);
    }
}
/**
...
static void lv_gauge_draw_needle(lv_obj_t *gauge, const lv_area_t *mask)
{
    ...
    for (i = 0; i < ext->needle_count; i++) {
        ...
        p_end.x =
          (lv_trigo_sin(needle_angle + 90) * r) / LV_TRIGO_SIN_MAX + x_ofs;
