    ...
    while (1) {
        ...
        if (btn_cnt != 0) {
            ...
            for (i = 0; i < btn_cnt; i++) {
                 ...
                 * greater number */
                act_unit_w =
                  (all_unit_w * get_button_width(map_p_tmp[i])) / unit_cnt;
                act_unit_w--; /*-1 because e.g. width = 100 means 101 pixels
                                 (0..100)*/

                /*Always recalculate act_x because of rounding errors */
                act_x = (unit_act_cnt * all_unit_w) / unit_cnt +
                        i * style_bg->body.padding.inner +
                        style_bg->body.padding.hor;
        ...
        for (btn_i = 0; btn_i < ext->btn_cnt; btn_i++, txt_i++) {
            ...
            if (button_is_inactive(ext->map_p[txt_i]))
                ...
                btn_style = lv_btnm_get_style(
                  btnm, LV_BTNM_STYLE_BTN_REL); /*Not possible option, just to
                                                   be sure*/

            /*On the right buttons clear the border if only right borders are
             * drawn*/
