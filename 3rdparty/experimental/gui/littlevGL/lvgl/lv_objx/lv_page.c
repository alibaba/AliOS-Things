...
lv_obj_t *lv_page_create(lv_obj_t *par, lv_obj_t *copy)
{
    ...
    if (copy == NULL) {
        ...
        lv_obj_set_protect(ext->scrl,
                           LV_PROTECT_PARENT | LV_PROTECT_PRESS_LOST);
...
static lv_res_t lv_page_signal(lv_obj_t *page, lv_signal_t sign, void *param)
{
    ...
    if (sign == LV_SIGNAL_CHILD_CHG) { /*Automatically move children to the
        ...
        while (child != NULL) {
            if (lv_obj_is_protected(child, LV_PROTECT_PARENT) == false) {
                ...
                child         = lv_obj_get_child(
                  page, child); /*Get the next child before move this*/
        ...
        if (lv_cont_get_hor_fit(ext->scrl) == false) {
            lv_obj_set_width(ext->scrl, lv_obj_get_width(page) -
                                          2 * style->body.padding.hor);
        } else {
            ext->scrl->signal_func(ext->scrl, LV_SIGNAL_CORD_CHG,
                                   &ext->scrl->coords);
        }
                                          ...
                                          void *param)
{
    ...
    if (sign == LV_SIGNAL_CORD_CHG) {
        ...
        if (lv_cont_get_hor_fit(scrl) == false) {
            lv_obj_set_width(scrl, lv_obj_get_width(page) -
                                     2 * page_style->body.padding.hor);
        }
...
static void lv_page_sb_refresh(lv_obj_t *page)
{
    ...
    if (scrl_w <= obj_w - 2 * hpad) { /*Full sized scroll bar*/
        ...
        lv_area_set_pos(&ext->sb.hor_area,
                        sb_hor_pad + (-(lv_obj_get_x(scrl) - hpad) *
    ...
    if (scrl_h <= obj_h - 2 * vpad) { /*Full sized scroll bar*/
        ...
        lv_area_set_pos(&ext->sb.ver_area,
                        obj_w - ext->sb.style->body.padding.inner -
                          ext->sb.style->body.padding.hor,
                        sb_ver_pad + (-(lv_obj_get_y(scrl) - vpad) *
