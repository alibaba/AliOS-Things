...
lv_chart_series_t *lv_chart_add_series(lv_obj_t *chart, lv_color_t color)
{
    ...
    lv_coord_t         def =
      (ext->ymin + ext->ymax) >> 1; /*half range as default value*/
...
void lv_chart_set_point_count(lv_obj_t *chart, uint16_t point_cnt)
{
    ...
    {
        ser->points =
          lv_mem_realloc(ser->points, sizeof(lv_coord_t) * point_cnt);
...
static void lv_chart_draw_div(lv_obj_t *chart, const lv_area_t *mask)
{
    ...
    if (ext->hdiv_cnt != 0) {
        ...
        for (div_i = div_i_start; div_i <= div_i_end; div_i++) {
            ...
            if (div_i == div_i_end)
                p1.y -= (style->line.width >> 1) +
                        1; /*The last line might not be visible*/
