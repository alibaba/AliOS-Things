...
lv_obj_t *lv_roller_create(lv_obj_t *par, lv_obj_t *copy)
{
    ...
    if (ancestor_scrl_signal == NULL)
        ancestor_scrl_signal =
          lv_obj_get_signal_func(lv_page_get_scrl(new_roller));
    ...
    if (copy == NULL) {
        ...
        if (th) {
            ...
            lv_obj_refresh_style(
              new_roller); /*To set scrollable size automatically*/
        }
    }
...
void lv_roller_set_selected(lv_obj_t *roller, uint16_t sel_opt, bool anim_en)
{
    ...
    if (lv_roller_get_selected(roller) == sel_opt)
        return;
...
void lv_roller_set_visible_row_count(lv_obj_t *roller, uint8_t row_cnt)
{
    ...
    lv_ddlist_set_fix_height(
      roller, lv_font_get_height(style_label->text.font) * row_cnt +
    ...
    /*Post draw when the children are drawn*/
        ...
        if (area_ok) {
            lv_style_t *sel_style =
              lv_roller_get_style(roller, LV_ROLLER_STYLE_SEL);
            lv_style_t new_style;
            lv_style_copy(&new_style, style);
            new_style.text.color = sel_style->text.color;
            new_style.text.opa   = sel_style->text.opa;
            lv_draw_label(&ext->ddlist.label->coords, &mask_sel, &new_style,
                          lv_label_get_text(ext->ddlist.label),
                          LV_TXT_FLAG_CENTER, NULL);
        }
    }
...
static void draw_bg(lv_obj_t *roller, const lv_area_t *mask)
{
    ...
    if ((h & 0x1) == 0)
        half_roller.y1++; /*With even height the pixels in the middle would be
                             drawn twice*/
