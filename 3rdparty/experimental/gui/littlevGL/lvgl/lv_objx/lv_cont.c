...
static void lv_cont_layout_pretty(lv_obj_t *cont)
{
    ...
    child_rc =
      child_rs; /*Initially the the row starter and closer is the same*/
    while (child_rs != NULL) {
        lv_coord_t h_row = 0;
            ...
            while (child_tmp != NULL) {
                if (lv_obj_get_hidden(child_tmp) == false &&
                    ...
                    lv_obj_align(child_tmp, cont, LV_ALIGN_IN_TOP_LEFT, act_x,
                                 act_y +
                                   (h_row - lv_obj_get_height(child_tmp)) / 2);
...
static void lv_cont_layout_grid(lv_obj_t *cont)
{
    ...
    uint16_t    obj_row =
      (w_tot - (2 * style->body.padding.hor)) /
