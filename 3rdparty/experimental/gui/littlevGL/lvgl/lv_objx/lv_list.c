...
lv_obj_t *lv_list_create(lv_obj_t *par, lv_obj_t *copy)
{
    ...
    if (copy == NULL) {
        ...
        lv_obj_t *copy_btn =
          lv_obj_get_child_back(lv_page_get_scrl(copy), NULL);
        lv_obj_t *new_btn;
        while (copy_btn) {
...
void lv_list_focus(lv_obj_t *btn, bool anim_en)
{
    ...
    lv_page_focus(list, btn,
                  anim_en == false ? 0 : lv_list_get_anim_time(list));
}
...
static lv_res_t lv_list_signal(lv_obj_t *list, lv_signal_t sign, void *param)
{
    ...
    if (sign == LV_SIGNAL_CORD_CHG) {
        ...
        /*Because of the possible change of horizontal and vertical padding
         * refresh buttons width */
        refr_btn_width(list);
