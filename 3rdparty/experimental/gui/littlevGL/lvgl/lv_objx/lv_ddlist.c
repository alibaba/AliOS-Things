...
lv_obj_t *lv_ddlist_create(lv_obj_t *par, lv_obj_t *copy)
{
    ...
    if (ancestor_scrl_signal == NULL)
        ancestor_scrl_signal =
          lv_obj_get_signal_func(lv_page_get_scrl(new_ddlist));
    ...
    switch (type) {
        ...
        case LV_DDLIST_STYLE_SEL:
            ...
            lv_obj_refresh_ext_size(
              scrl); /*Because of the wider selected rectangle*/
            break;
    }
}
    ...
    /*Post draw when the children are drawn*/
        ...
        if (area_ok) {
            lv_style_t *sel_style =
              lv_ddlist_get_style(ddlist, LV_DDLIST_STYLE_SEL);
            lv_style_t new_style;
            lv_style_copy(&new_style, style);
            new_style.text.color = sel_style->text.color;
            new_style.text.opa   = sel_style->text.opa;
            lv_draw_label(&ext->label->coords, &mask_sel, &new_style,
                          lv_label_get_text(ext->label), LV_TXT_FLAG_NONE,
                          NULL);
        }
    }
                                      ...
                                      void *param)
{
    ...
    if (sign == LV_SIGNAL_REFR_EXT_SIZE) {
         ...
         * In this way by dragging the scrollable part the wider rectangle area
         * can be redrawn too*/
        lv_style_t *style = lv_ddlist_get_style(ddlist, LV_DDLIST_STYLE_BG);
