 ...
 * '\0' terminated. Memory will be allocated to store the array by the label.
 * @param label pointer to a label object
...
void lv_label_set_align(lv_obj_t *label, lv_label_align_t align)
{
    ...
    lv_obj_invalidate(
      label); /*Enough to invalidate because alignment is only drawing related
...
void lv_label_get_letter_pos(lv_obj_t *label, uint16_t index, lv_point_t *pos)
{
    ...
    uint32_t   cnt =
      line_start; /*Count the letter (in UTF-8 1 letter not 1 byte)*/
 ...
 * letter) Expressed in character index and not byte index (different in UTF-8)
 */
 ...
 * byte index (Different in UTF-8) 0: before first char. LV_LABEL_POS_LAST:
 * after last char.
 ...
 * @param label pointer to a label object
 * @param pos character index to insert. Expressed in character index and not
 * byte index (Different in UTF-8) 0: before first char.
...
static lv_res_t lv_label_signal(lv_obj_t *label, lv_signal_t sign, void *param)
{
    ...
    if (sign == LV_SIGNAL_CLEANUP) {
        ...
        if (ext->body_draw) {
            ...
            label->ext_size =
              LV_MATH_MAX(label->ext_size, style->body.padding.hor);
            label->ext_size =
              LV_MATH_MAX(label->ext_size, style->body.padding.ver);
        }
...
static void lv_label_refr_text(lv_obj_t *label)
{
    ...
    if (ext->long_mode == LV_LABEL_LONG_EXPAND ||
        ...
        if (ext->long_mode == LV_LABEL_LONG_SCROLL) {
            ...
            if (lv_obj_get_width(label) > lv_obj_get_width(parent)) {
                ...
                anim.end = lv_obj_get_height(parent) -
                           lv_obj_get_height(label) - lv_font_get_height(font);
    ...
    /*In roll mode keep the size but start offset animations*/
        ...
        if (size.y > lv_obj_get_height(label) && hor_anim == false) {
            anim.end =
              lv_obj_get_height(label) - size.y - (lv_font_get_height(font));
        ...
        if (size.y <=
            ...
            p.y -=
              p.y % (lv_font_get_height(style->text.font) +
            ...
            ext->dot_tmp[LV_LABEL_DOT_NUM] =
              ext->text[letter_id + LV_LABEL_DOT_NUM];
