                              ...
                              lv_txt_flag_t flag)
{
    ...
    while (txt[i] != '\0') {
        ...
        if ((flag & LV_TXT_FLAG_NO_BREAK) == 0 &&
            (letter == '\n' || letter == '\r')) {
            /*Handle \r\n as well*/
            uint32_t i_tmp       = i;
            ...
            if (cur_w > max_width) {
                ...
                if (is_break_char(letter)) {
                    /* Now 'i' points to the next char because of
                     * txt_utf8_next() But we need the first char of the next
                     * line so keep it. Hence do nothing here*/
                }
                /*If already a break character is found, then break there*/
                if (last_break != NO_BREAK_FOUND) {
                    i = last_break;
                } else {
                    /* Now this character is out of the area so it will be first
                     * character of the next line*/
                    /* But 'i' already points to the next character (because of
                     * lv_txt_utf8_next) step beck one*/
 ...
 * state of command processing (Initied. to TXT_CMD_STATE_WAIT )
        ...
        if (*state == LV_TXT_CMD_STATE_WAIT) { /*Start char*/
            *state = LV_TXT_CMD_STATE_PAR;
            ret    = true;
                   ...
                   LV_TXT_CMD_STATE_PAR) { /*Other start char in parameter is
                                              escaped cmd. char */
 ...
 * index (Different in UTF-8) 0: before the original text, 1: after the first
 * char etc.
