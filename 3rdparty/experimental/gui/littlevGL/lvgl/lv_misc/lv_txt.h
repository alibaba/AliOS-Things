...
#define LV_TXT_COLOR_CMD "#"
    ...
    typedef enum
    {
        ...
        LV_TXT_FLAG_NO_BREAK =
          0x04,                    /*Ignore line breaks (Used by the library)*/
    ...
    uint16_t lv_txt_get_next_line(const char *txt, const lv_font_t *font_p,
                                  lv_coord_t letter_space, lv_coord_t max_l,
                                  lv_txt_flag_t flag);
    ...
    /**
     * Check next character in a string and decide if te character is part of
     * the command or not
     * @param state pointer to a txt_cmd_state_t variable which stores the
     * current state of command processing
     * @param c the current character
     * @return true: the character is part of a command and should not be
     * written, false: the character should be written
     */
     ...
     * @param pos position where to start the deleting (0: before the first
     * char, 1: after the first char etc.)
     ...
     *                After the call it will point to the next UTF-8 char in
     ...
     * @param txt pointer to '\0' terminated string
     * @param i_start index in 'txt' where to start. After the call it will
     * point to the next UTF-8 char in 'txt'.
     * @return the decoded Unicode character or 0 on invalid UTF-8 code
     */
     ...
     * E.g. in "AÁRT" index of 'R' is 2 but start at byte 3 because 'Á' is 2
     * bytes long
