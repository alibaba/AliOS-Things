...
static uint32_t
  zero_mem; /*Give the address of this variable if 0 byte should be allocated*/
...
void lv_mem_free(const void *data)
{
    ...
    lv_mem_ent_t *e =
      (lv_mem_ent_t *)((uint8_t *)data - sizeof(lv_mem_header_t));
    e->header.used = 0;
...
void lv_mem_defrag(void)
{
    ...
    while (1) {
        ...
        while (e_next != NULL) {
            if (e_next->header.used == 0) {
                e_free->header.d_size +=
                  e_next->header.d_size + sizeof(e_next->header);
            } else {
                break;
            }
