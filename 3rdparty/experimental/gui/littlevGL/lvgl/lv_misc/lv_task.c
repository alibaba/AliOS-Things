...
LV_ATTRIBUTE_TASK_HANDLER void lv_task_handler(void)
{
    ...
    do {
        ...
        while (act) {
            ...
            if (act == task_interruper) {
                task_interruper =
                  NULL; /*From this point only task after the interrupter comes,
                           so the interrupter is not interesting anymore*/
                act = next;
            ...
            /*Tasks with higher priority then the interrupted shall be run in
               every case*/
            else if (task_interruper) {
                if (act->prio > task_interruper->prio) {
                    if (lv_task_exec(act)) {
                        task_interruper = act; /*Check all tasks again from the
                                                  highest priority */
                        end_flag = false;
                        break;
                    }
             ...
             * Just run the remaining tasks*/
                ...
                if (lv_task_exec(act)) {
                    task_interruper =
                      act; /*Check all tasks again from the highest priority */
                    end_flag = false;
                    break;
                }
            }
    ...
    if (idle_period_time >= IDLE_MEAS_PERIOD) {
        ...
        idle_last =
          idle_last > 100 ? 0 : 100 - idle_last; /*But we need idle time*/
        busy_time         = 0;
