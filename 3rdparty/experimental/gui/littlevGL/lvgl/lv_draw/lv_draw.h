...
#define LV_IMG_PX_SIZE_ALPHA_BYTE 4
    ...
    typedef struct
    {
        union
        {
            ...
            {
                uint32_t
                  chroma_keyed : 1; /*1: The image contains transparent pixels
                                       with LV_COLOR_TRANSP color*/
                uint32_t alpha_byte : 1; /*Every pixel is extended with a 8 bit
                                            alpha channel*/
        ...
        union
        {
            const uint8_t *pixel_map; /*For internal images (c arrays) pointer
                                         to the pixels array*/
            uint8_t first_pixel; /*For external images (binary) the first byte
                                    of the pixels (just for convenient)*/
        };
    } lv_img_t;
    ...
    typedef enum
    {
        ...
        LV_IMG_FORMAT_INTERNAL_RAW,    /*'lv_img_t' variable compiled with the
                                          code*/
...
#define USE_LV_TRIANGLE 0
     ...
     * @param map_p pointer to a lv_color_t array which contains the pixels of
     * the image
     */
