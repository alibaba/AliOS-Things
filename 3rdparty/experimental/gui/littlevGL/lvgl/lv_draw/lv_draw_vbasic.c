              ...
              lv_color_t color, lv_opa_t opa)
{
...
#if USE_LV_GPU
    static lv_color_t
                      color_array_tmp[LV_HOR_RES]; /*Used by 'lv_disp_mem_blend'*/
    ...
    /*Not opaque fill*/
        ...
        /*Use hw blend if present and the area is not too small*/
            ...
            lv_coord_t row;
            for (row = vdb_rel_a.y1; row <= vdb_rel_a.y2; row++) {
                lv_disp_mem_blend(&vdb_buf_tmp[vdb_rel_a.x1], color_array_tmp,
                                  w, opa);
                vdb_buf_tmp += vdb_width;
            }
                ...
                lv_opa_t opa)
{
    ...
    uint8_t bpp =
      lv_font_get_bpp(font_p, letter); /*Bit per pixel (1,2, 4 or 8)*/
        ...
        for (col = col_start; col < col_end; col++) {
            ...
            if (letter_px != 0) {
                if (opa == LV_OPA_COVER) {
                    ...
                    *vdb_buf_tmp = lv_color_mix(
                      color, *vdb_buf_tmp,
                      bpp == 8 ? (uint16_t)((uint16_t)letter_px * opa) >> 8
                               : (uint16_t)((uint16_t)bpp_opa_table[letter_px] *
                                            opa) >>
                                   8);
                }
            }
             ...
             bool alpha_byte, lv_color_t recolor, lv_opa_t recolor_opa)
{
    ...
    if (cords_p->y1 < masked_a.y1) {
        map_p +=
          (uint32_t)map_width * ((masked_a.y1 - cords_p->y1)) * px_size_byte;
    }
    ...
    if (chroma_key == false && alpha_byte == false && opa == LV_OPA_COVER &&
        ...
        for (row = masked_a.y1; row <= masked_a.y2; row++) {
            ...
            if (lv_disp_is_mem_blend_supported() == false) {
                ...
                lv_disp_mem_blend(vdb_buf_tmp, (lv_color_t *)map_p,
                                  map_useful_w, opa);
            }
#else
    ...
    /*In the other cases every pixel need to be checked one-by-one*/
        ...
        lv_color_t recolored_px =
          lv_color_mix(recolor, last_img_px, recolor_opa);
        for (row = masked_a.y1; row <= masked_a.y2; row++) {
            for (col = 0; col < map_useful_w; col++) {
                lv_opa_t opa_result = opa;
                uint8_t *px_color_p =
                  (uint8_t *)&map_p[(uint32_t)col * px_size_byte];
                lv_color_t px_color;
                ...
                if (alpha_byte) {
                     ...
                     * address which can cause crash*/
                    px_color.full = px_color_p[0] + (px_color_p[1] << 8);
#elif LV_COLOR_DEPTH == 24
                    px_color = *((lv_color_t *)px_color_p);
#endif
                    lv_opa_t px_opa =
                      *(px_color_p + LV_IMG_PX_SIZE_ALPHA_BYTE - 1);
                    if (px_opa == LV_OPA_TRANSP)
                        continue;
                    else if (px_opa != LV_OPA_COVER)
                        opa_result =
                          (uint32_t)((uint32_t)px_opa * opa_result) >> 8;
                } else {
                    px_color = *((lv_color_t *)px_color_p);
                }
                ...
                if (recolor_opa != LV_OPA_TRANSP) {
                    ...
                    if (last_img_px.full !=
                        px_color.full) { /*Minor acceleration: calculate only
                                            for new colors (save the last)*/
                    ...
                    if (opa_result == LV_OPA_COVER)
                        ...
                        vdb_buf_tmp[col] = lv_color_mix(
                          recolored_px, vdb_buf_tmp[col], opa_result);
                } else {
                    if (opa_result == LV_OPA_COVER)
