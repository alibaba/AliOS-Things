...
#define SHADOW_OPA_EXTRA_PRECISION                                        \
    0 /*Calculate with 2^x bigger shadow opacity values to avoid rounding \
         errors*/
                      ...
                      lv_color_t color)
{
    ...
    while (1) {
        ...
        draw_area.x2--; /*Do not draw most right pixel because it will be drawn
                           by the adjacent triangle*/
                   ...
                   lv_point_t *offset)
{
    ...
    while (txt[line_start] != '\0') {
                ...
                if (letter == (uint32_t)LV_TXT_COLOR_CMD[0]) {
                    if (cmd_state == CMD_STATE_WAIT) { /*Start char*/
                        ...
                        continue;
                               ...
                               CMD_STATE_PAR) { /*Other start char in parameter
                                                   escaped cmd. char */
                        cmd_state = CMD_STATE_WAIT;
        ...
        if (flag & LV_TXT_FLAG_CENTER) {
            line_length =
              lv_txt_get_width(&txt[line_start], line_end - line_start, font,
                 ...
                 const lv_style_t *style, const void *src)
{
    ...
    if (u8_p[0] >= 'A' && u8_p[0] <= 'Z') { /*It will be a path of a file*/
        ...
        if (res == LV_FS_RES_OK) {
            ...
            start_offset += (lv_area_get_width(coords)) *
                            (mask_com.y1 - coords->y1) * px_size; /*First row*/
            ...
            uint32_t next_row =
              lv_area_get_width(coords) * px_size - useful_data;
...
#if LV_ANTIALIAS != 0
            lv_coord_t seg_h =
              lv_area_get_height(&draw_area); /*Segment height*/
            lv_point_t aa_p1;
            lv_point_t aa_p2;
                                  ...
                                  const lv_style_t *style)
{
    ...
    if (mcolor.full == gcolor.full) {
        ...
        for (row = row_start; row <= row_end; row++) {
            ...
            work_area.y2 = row;
            mix =
              (uint32_t)((uint32_t)(coords->y2 - work_area.y1) * 255) / height;
                                     ...
                                     const lv_style_t *style)
{
    ...
    while (lv_circ_cont(&cir)) {
        ...
        if (out_x_last != cir.x) {
            ...
            for (i = 0; i < seg_size; i++) {
                lv_opa_t aa_opa;
                if (seg_size >
                    CIRCLE_AA_NON_LINEAR_OPA_THRESHOLD) { /*Use non-linear opa
                                                             mapping on the
                                                             first segment*/
                    aa_opa =
                      antialias_get_opa_circ(seg_size, i, style->body.opa);
                } else {
                    aa_opa =
                      opa - antialias_get_opa(seg_size, i, style->body.opa);
                }
                ...
                mix =
                  (uint32_t)((uint32_t)(radius - out_y_seg_start + i) * 255) /
                  height;
        ...
        if (edge_top_refr != 0) {
            if (mcolor.full == gcolor.full)
                ...
                mix =
                  (uint32_t)((uint32_t)(coords->y2 - edge_top_area.y1) * 255) /
                  height;
                act_color = lv_color_mix(mcolor, gcolor, mix);
            }
        ...
        if (mid_top_refr != 0) {
            if (mcolor.full == gcolor.full)
                ...
                mix =
                  (uint32_t)((uint32_t)(coords->y2 - mid_top_area.y1) * 255) /
                  height;
                act_color = lv_color_mix(mcolor, gcolor, mix);
            }
        ...
        if (mid_bot_refr != 0) {
            if (mcolor.full == gcolor.full)
                ...
                mix =
                  (uint32_t)((uint32_t)(coords->y2 - mid_bot_area.y1) * 255) /
                  height;
                act_color = lv_color_mix(mcolor, gcolor, mix);
            }
        ...
        if (edge_bot_refr != 0) {
            ...
            if (mcolor.full == gcolor.full)
                ...
                mix =
                  (uint32_t)((uint32_t)(coords->y2 - edge_bot_area.y1) * 255) /
                  height;
                act_color = lv_color_mix(mcolor, gcolor, mix);
            }
    ...
    if (mcolor.full == gcolor.full)
        ...
        mix =
          (uint32_t)((uint32_t)(coords->y2 - edge_top_area.y1) * 255) / height;
        act_color = lv_color_mix(mcolor, gcolor, mix);
    }
    fill_fp(&edge_top_area, mask, act_color, opa);
    ...
    if (edge_top_area.y1 != mid_top_area.y1) {
        ...
        if (mcolor.full == gcolor.full)
            ...
            mix = (uint32_t)((uint32_t)(coords->y2 - mid_top_area.y1) * 255) /
                  height;
            act_color = lv_color_mix(mcolor, gcolor, mix);
        }
        fill_fp(&mid_top_area, mask, act_color, opa);
    ...
    if (LV_MATH_ABS(aa_p.x - aa_p.y) == seg_size) {
        ...
        px_fp(rb_origo.x + LV_CIRC_OCT2_X(aa_p),
              rb_origo.y + LV_CIRC_OCT2_Y(aa_p), mask, aa_color_hor_bottom,
              aa_opa);
        px_fp(lb_origo.x + LV_CIRC_OCT4_X(aa_p),
              lb_origo.y + LV_CIRC_OCT4_Y(aa_p), mask, aa_color_hor_bottom,
              aa_opa);
        px_fp(lt_origo.x + LV_CIRC_OCT6_X(aa_p),
              lt_origo.y + LV_CIRC_OCT6_Y(aa_p), mask, aa_color_hor_top,
              aa_opa);
        px_fp(rt_origo.x + LV_CIRC_OCT8_X(aa_p),
              rt_origo.y + LV_CIRC_OCT8_Y(aa_p), mask, aa_color_hor_top,
              aa_opa);
    }
                                       ...
                                       const lv_style_t *style)
{
            ...
            for (i = 0; i < seg_size; i++) {
                ...
                if (seg_size >
                    CIRCLE_AA_NON_LINEAR_OPA_THRESHOLD) { /*Use non-linear opa
                                                             mapping on the
                                                             first segment*/
                    aa_opa = antialias_get_opa_circ(seg_size, i,
            ...
            for (i = 0; i < seg_size; i++) {
                ...
                if (seg_size >
                    ...
                    aa_opa = style->body.border.opa -
                             antialias_get_opa_circ(seg_size, i,
                                                    style->body.border.opa);
                } else {
                    aa_opa =
                      antialias_get_opa(seg_size, i, style->body.border.opa);
    ...
    if (LV_MATH_ABS(aa_p.x - aa_p.y) == seg_size) {
        ...
        if ((part & LV_BORDER_BOTTOM) && (part & LV_BORDER_RIGHT)) {
            px_fp(rb_origo.x + LV_CIRC_OCT2_X(aa_p),
                  rb_origo.y + LV_CIRC_OCT2_Y(aa_p), mask,
                  style->body.border.color, aa_opa);
        }

        if ((part & LV_BORDER_BOTTOM) && (part & LV_BORDER_LEFT)) {
            px_fp(lb_origo.x + LV_CIRC_OCT4_X(aa_p),
                  lb_origo.y + LV_CIRC_OCT4_Y(aa_p), mask,
                  style->body.border.color, aa_opa);
        }

        if ((part & LV_BORDER_TOP) && (part & LV_BORDER_LEFT)) {
            px_fp(lt_origo.x + LV_CIRC_OCT6_X(aa_p),
                  lt_origo.y + LV_CIRC_OCT6_Y(aa_p), mask,
                  style->body.border.color, aa_opa);
        }

        if ((part & LV_BORDER_TOP) && (part & LV_BORDER_RIGHT)) {
            px_fp(rt_origo.x + LV_CIRC_OCT8_X(aa_p),
                  rt_origo.y + LV_CIRC_OCT8_Y(aa_p), mask,
                  style->body.border.color, aa_opa);
        }
    }
                                ...
                                const lv_style_t *style)
{
    ...
    for (line = 1; line <= radius + swidth;
        ...
        line_ready = false;
        for (col = 1; col < radius + swidth;
             col++) { /*Check all pixels in a 1D blur line (from the origo to
                         last shadow pixel (radius + swidth))*/
            ...
            for (line_rel = -swidth; line_rel <= swidth; line_rel++) {
                ...
                if (line + line_rel <
                    ...
                    col_rel = radius - curve_x[line] - col;
                           ...
                           radius) { /*Above the radius, here won't be more 1D
                                        blur*/
                    break;
                ...
                if (col_rel < -swidth) { /*Outside of the blurred area. */
                    if (line_rel == -swidth)
                        line_ready =
                          true; /*If no data even on the very first line then it
                                   wont't be anything else in this line*/
                    break; /*Break anyway because only smaller 'col_rel' values
                              will come */
                } else if (col_rel > swidth)
                    px_opa_sum +=
                      line_1d_blur[0]; /*Inside the not blurred area*/
                else
                    px_opa_sum +=
                      line_1d_blur[swidth -
                                   col_rel]; /*On the 1D blur (+ swidth to align
                                                to the center)*/
            }

            line_2d_blur[col] = px_opa_sum >> SHADOW_OPA_EXTRA_PRECISION;
            if (line_ready) {
                col++; /*To make this line to the last one ( drawing will go to
                          '< col')*/
                break;
            }
        }
                                  ...
                                  const lv_style_t *style)
{
    ...
    for (col = 0; col <= radius; col++) {
        ...
        for (d = 0; d < swidth; d++) {
             ...
             * from the prev. column to make a blur */
            if (diff == 0) {
                px_opa = line_1d_blur[d];
            } else {
                px_opa = (uint16_t)((uint16_t)line_1d_blur[d] +
                                    line_1d_blur[d - diff]) >>
                         1;
            }
            ...
            if (point_r.x > ofs_l.x) {
                px_fp(point_r.x, point_r.y, mask, style->body.shadow.color,
                      px_opa);
            }
            point_r.y++;
        }
