...
#define LV_MAX_ANCESTOR_NUM 8
    ...
    typedef bool (*lv_design_func_t)(struct _lv_obj_t *obj,
                                     const lv_area_t  *mask_p,
    ...
    typedef lv_res_t (*lv_signal_func_t)(struct _lv_obj_t *obj,
    ...
    typedef struct _lv_obj_t
    {
        ...
        uint8_t top : 1; /*1: If the object or its children is clicked it goes
                            to the foreground*/
        uint8_t reserved : 1;

        uint8_t protect; /*Automatically happening actions can be prevented.
                            'OR'ed values from lv_obj_prot_t*/

        lv_coord_t ext_size; /*EXTtend the size of the object in every
                                direction. E.g. for shadow drawing*/
     ...
     * @param copy pointer to a base object, if not NULL then the new object
     ...
     * @param type type of animation from 'lv_anim_builtin_t'. 'OR' it with
     * ANIM_IN or ANIM_OUT
    ...
    /**
     * Return with the top layer. (Same on every screen and it is above the
     * normal screen layer)
     * @return pointer to the top layer object  (transparent screen sized
     * lv_obj)
     */
    lv_obj_t *lv_layer_top(void);

    /**
     * Return with the system layer. (Same on every screen and it is above the
     * all other layers) It is used for example by the cursor
     * @return pointer to the system layer object (transparent screen sized
     * lv_obj)
     */
     ...
     * @return false: none of the given bits are set, true: at least one bit is
     * set
     */
     ...
     * starting with the current type. E.g. buf.type[0]="lv_btn",
