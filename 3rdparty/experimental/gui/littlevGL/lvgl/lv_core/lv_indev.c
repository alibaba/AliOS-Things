...
{
#if LV_INDEV_READ_PERIOD != 0
    lv_task_create(indev_proc_task, LV_INDEV_READ_PERIOD, LV_TASK_PRIO_MID,
                   NULL);
#endif
...
static void indev_keypad_proc(lv_indev_t *i, lv_indev_data_t *data)
{
    ...
    /*Release happened*/
             ...
             i->proc.last_state == LV_INDEV_STATE_PR) {
        /*The user might clear the key it was released. Always release the
         * pressed key*/
...
static void indev_proc_press(lv_indev_proc_t *proc)
{
    ...
    if (proc->act_obj != NULL) {
        proc->act_obj->signal_func(proc->act_obj, LV_SIGNAL_PRESSING,
                                   indev_act);
        if (proc->reset_query != 0)
            return;
        ...
        if (proc->drag_in_prog == 0 && proc->long_pr_sent == 1) {
            /*Send a signal about the long press repeate if enough time
             * elapsed*/
...
static void indev_proc_release(lv_indev_proc_t *proc)
{
    ...
    if (proc->act_obj != NULL) {
        proc->act_obj->signal_func(proc->act_obj, LV_SIGNAL_RELEASED,
                                   indev_act);
        if (proc->reset_query != 0)
            return;
        proc->act_obj              = NULL;
    ...
    if (state->drag_range_out != 0) {
        ...
        if (state->vect.x != 0 || state->vect.y != 0) {
            ...
            uint16_t   inv_buf_size =
              lv_refr_get_buf_size(); /*Get the number of currently invalidated
                                         areas*/

            lv_coord_t prev_x = drag_obj->coords.x1;
            lv_coord_t prev_y = drag_obj->coords.y1;

            lv_obj_set_pos(drag_obj, act_x + state->vect.x,
                           act_y + state->vect.y);

            /*Set the drag in progress flag if the object is really moved*/

            if (drag_obj->coords.x1 != prev_x ||
                drag_obj->coords.y1 != prev_y) {
                if (state->drag_range_out !=
                    0) { /*Send the drag begin signal on first move*/
                    drag_obj->signal_func(drag_obj, LV_SIGNAL_DRAG_BEGIN,
                                          indev_act);
