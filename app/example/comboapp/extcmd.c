...
const static char m_sdk_version[] = ":" EXT_VERSION;
...
static ret_code_t ext_cmd06_rsp(uint8_t *p_buff, uint8_t *p_blen, const uint8_t *p_data, uint8_t dlen)
{
    ret_code_t err_code = EXT_SUCCESS;
    ...
    *p_blen = sizeof(rsp);
	...
	aos_post_event(EV_BZ_COMBO, CODE_COMBO_AP_INFO_READY, (unsigned long)&comboinfo);
    }
...
ret_code_t extcmd_init(breeze_dev_info_t *p_dev_info)
{
    ...
    get_os_info();
    g_extcmd.model_id = p_dev_info->product_id;
    printf("dn %s pk %s ps %s\n", p_dev_info->device_name, p_dev_info->product_key, p_dev_info->product_secret);
    if (p_dev_info->device_name != NULL) {
        memcpy(g_extcmd.p_device_name, p_dev_info->device_name, strlen(p_dev_info->device_name));
    }
    memcpy(g_extcmd.p_product_key, p_dev_info->product_key, strlen(p_dev_info->product_key));
    memcpy(g_extcmd.p_secret, p_dev_info->product_secret, strlen(p_dev_info->product_secret));
#if 0
...
void extcmd_rx_command(uint8_t cmd, uint8_t *p_data, uint16_t length)
{
    ...
    uint32_t err_code = EXT_SUCCESS;

    /* process TLV one at a time (see specification v1.0.5, ch. 5.6) */
    while (length > 0 && err_code == EXT_SUCCESS) {
        if (length >= 2) {
            /* get TLV type. */
            tlv_type = *p_data++;
            /* get TLV length. */
    ...
    if (err_code == EXT_SUCCESS) {
	err_code = breeze_post_ext(CMD_EXT_UP, g_extcmd.tx_buff, sizeof(g_extcmd.tx_buff) - tx_buff_avail);
    } else {
	err_code = breeze_post_ext(CMD_EXT_ERR, NULL, 0);
    }

    if (err_code != EXT_SUCCESS) {
	BREEZE_LOG_ERR("EXT CMD RESPONSE ERR(%02x)\n", err_code);
    }
}
