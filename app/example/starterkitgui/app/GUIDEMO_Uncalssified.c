...
static void _Graph_Sensor_Demo()
{
    ...
    //
    Graph_xSize = xSize - (DIST_TO_BORDER <<
                           1);            // xsize = Screen size subtracting twice the distance to the border of the screen
    Graph_ySize = ySize - INFO_SIZE_Y - (DIST_TO_WIN <<
                                         1); // ysize = Screen size subtracting the window sizes and twice the distance to the windows
    ...
    hGraph = GRAPH_CreateEx(Graph_xPos, Graph_yPos, Graph_xSize, Graph_ySize, WM_HBKWIN, WM_CF_SHOW | WM_CF_CONST_OUTLINE,
                            0, 0);
    ...
    GRAPH_SCALE_SetPos(_hScaleH_sensor, Graph_ySize -
                       4);  // Sets the vertical position for showing the scale object with in the GRAPH widget
    GRAPH_SCALE_SetOff(_hScaleH_sensor,
                       -5);  // Sets an horizontal offset used to 'shift' the scale object in positive or negative direction
