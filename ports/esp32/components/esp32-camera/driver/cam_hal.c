...
static const char *TAG = "cam_hal_v1.4";
                ...
                if (cam_event == CAM_IN_SUC_EOF_EVENT) {
                    ...
                    if(!cam_obj->psram_mode){
                        if (cam_obj->fb_size < (frame_buffer_event->len + pixels_per_dma)) {
                            ESP_LOGW(TAG, "%s:%d FB-OVF (%d < %d + %d <- cnt:%d)", __func__, __LINE__, cam_obj->fb_size, frame_buffer_event->len, pixels_per_dma, cnt);
                            ll_cam_stop(cam_obj);
                            DBG_PIN_SET(0);
                            continue;
                        }
                    ...
                    if (cam_obj->jpeg_mode && cnt == 0 && cam_verify_jpeg_soi(frame_buffer_event->buf, frame_buffer_event->len) != 0) {
                        ll_cam_stop(cam_obj);
                        cam_obj->state = CAM_STATE_IDLE;
                        ESP_LOGW(TAG, " ====%s:%d set CAM_STATE_IDLE===== ", __func__, __LINE__);
                    }
                    cnt++;
                    // if (cnt > 7) {
                    //     ESP_LOGW(TAG, " ====%s:%d (len:%d, cnt:%d)===== ", __func__, __LINE__, frame_buffer_event->len, cnt);
                    // }
                    ...
                    if (cnt || !cam_obj->jpeg_mode || cam_obj->psram_mode) {
                        if (cam_obj->jpeg_mode) {
                            if (!cam_obj->psram_mode) {
                                if (cam_obj->fb_size < (frame_buffer_event->len + pixels_per_dma)) {
                                    ESP_LOGW(TAG, "%s:%d FB-OVF", __func__, __LINE__);
                                    cnt--;
                                } else {
                                    frame_buffer_event->len += ll_cam_memcpy(cam_obj,
                                        &frame_buffer_event->buf[frame_buffer_event->len], 
                        ...
                        // ESP_LOGW(TAG, " ====%s:%d (cnt=%d)===== ", __func__, __LINE__, cnt);
                        ...
                        if(!cam_obj->frames[frame_pos].en && xQueueSend(cam_obj->frame_buffer_queue, (void *)&frame_buffer_event, 0) != pdTRUE) {
                            ...
                            // ESP_LOGI(TAG, "xQueueSend msg to frame_buffer_queue(len=%d) ok", frame_buffer_event->len);
                        }
                    }
...
static esp_err_t cam_dma_config(const camera_config_t *config)
{
    ...
    ESP_LOGI(TAG, "buffer_size: %d, half_buffer_size: %d, node_buffer_size: %d, node_cnt: %d, total_cnt: %d, half_buffer_cnt:%d", 
             cam_obj->dma_buffer_size, cam_obj->dma_half_buffer_size, cam_obj->dma_node_buffer_size, cam_obj->dma_node_cnt, cam_obj->frame_copy_cnt, cam_obj->dma_half_buffer_cnt);
...
void print_cam_obj_info()
{
    ESP_LOGI(TAG, "===========  cam_obj infomation   ===========");
    ESP_LOGI(TAG, "=============cam_obj->state:%d==============", (uint8_t)(cam_obj->state));
    ESP_LOGI(TAG, "=============cam_obj->jpeg_mode:%d==============", cam_obj->jpeg_mode);
    ESP_LOGI(TAG, "=============cam_obj->psram_mode:%d==============", cam_obj->psram_mode);
    ESP_LOGI(TAG, "=============cam_obj->frame_cnt:%d==============", cam_obj->frame_cnt);
    ESP_LOGI(TAG, "=============cam_obj->width:%d==============", cam_obj->width);
    ESP_LOGI(TAG, "=============cam_obj->height:%d==============", cam_obj->height);
    ESP_LOGI(TAG, "=============cam_obj->recv_size:%d==============", cam_obj->recv_size);
    ESP_LOGI(TAG, "=============cam_obj->fb_size:%d==============", cam_obj->fb_size);
    ESP_LOGI(TAG, "=============cam_obj->dma_num:%d==============", cam_obj->dma_num);
    ESP_LOGI(TAG, "=============cam_obj->frame_copy_cnt:%d==============", cam_obj->frame_copy_cnt);
    ESP_LOGI(TAG, "=============cam_obj->dma_buffer_size:%d==============", cam_obj->dma_buffer_size);
    ESP_LOGI(TAG, "=============cam_obj->in_bytes_per_pixel:%d==============", cam_obj->in_bytes_per_pixel);
    ESP_LOGI(TAG, "=============cam_obj->fb_bytes_per_pixel:%d==============", cam_obj->fb_bytes_per_pixel);
    ESP_LOGI(TAG, "=============cam_obj->dma_bytes_per_item:%d==============", cam_obj->dma_bytes_per_item);
    ESP_LOGI(TAG, "=============cam_obj infomation end ==============");
}
...
esp_err_t cam_config(const camera_config_t *config, framesize_t frame_size, uint16_t sensor_pid)
{
    ...
    cam_obj->event_queue = xQueueCreate(cam_obj->dma_half_buffer_cnt , sizeof(cam_event_t));
    ...
    xTaskCreatePinnedToCore(cam_task, "cam_task", 2048 * 2, NULL, configMAX_PRIORITIES - 2, &cam_obj->task_handle, 0);
#elif CONFIG_CAMERA_CORE1
    xTaskCreatePinnedToCore(cam_task, "cam_task", 2048 * 2, NULL, configMAX_PRIORITIES - 2, &cam_obj->task_handle, 1);
#else
    xTaskCreate(cam_task, "cam_task", 2048 * 2, NULL, configMAX_PRIORITIES - 2, &cam_obj->task_handle);
#endif
...
camera_fb_t *cam_take(TickType_t timeout)
{
    ...
    if (dma_buffer) {
        // ESP_LOGW(TAG, "%s:%d -> jpeg_mode :%d", __func__, __LINE__, cam_obj->jpeg_mode);
        if(cam_obj->jpeg_mode){
        ...
        // ESP_LOGW(TAG, "%s:%d -> get frame buffer len :%d", __func__, __LINE__, dma_buffer->len);
        return dma_buffer;
    } else {
