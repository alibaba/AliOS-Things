...
uint32_t dumpsys_task_func(char *buf, uint32_t len, int32_t detail)
{
    ...
    for (corenum = 0; corenum < RHINO_CONFIG_CPU_NUM; corenum++) {
        candidate[corenum] = preferred_cpu_ready_task_get(&g_ready_queue, corenum);
    }

#else
    candidate = preferred_cpu_ready_task_get(&g_ready_queue, cpu_cur_get());
#endif
...
#ifdef AOS_COMP_DEBUG
#if (RHINO_CONFIG_SYS_STATS > 0)
        time_total = task->task_time_total_run;
