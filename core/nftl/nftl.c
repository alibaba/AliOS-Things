    ...
    if (NFTL_INVALID_BLOCK != logic_blk) {
        ...
        for (i = 0; i < blk_mgr->logic_block_nums; i++) {
            if (page_id == blk_mgr->logic_blocks[i].page_id) {
                ...
                NFTL_ERROR("[nftl] read error, so clear the mapping: lblk: %d, ppg: %d", logic_blk, page_id);
                break;
            }
        }
    }
        ...
        if (ret == 0) {
            ...
            if (tag->logic_block < blk_mgr->logic_block_nums) {
                ...
                if (tag->logic_block != NFTL_INVALID_BLOCK) {
                    NFTL_ERROR("[nftl] %s, invalid logic block, p: %d, lblk: %d, rev: %d, "
                               "nums: %d, pblk: %d, ppage: %d, roff: %d\r\n", __func__,
...
static void nftl_blk_release(nftl_block_mgr_t *blk_mgr ,uint32_t phy_blk)
{
    ...
    if (0 == count_valid_page(blk_mgr, phy_blk)) {
        ret = hal_flash_erase(blk_mgr->partition, phy_blk * NFTL_BLOCK_SIZE, NFTL_BLOCK_SIZE);

        if (ret != 0) {
            NFTL_ERROR("[nftl] %s failed to erase block %d part %d", __func__, phy_blk, blk_mgr->partition);
            hal_flash_markbad(blk_mgr->partition, phy_blk * NFTL_BLOCK_SIZE);
        }
    }
    ...
    do {
        ...
        for (i = 0; i < NFTL_TAG_PAGE_IN_BLOCK; i++) {
            if (nftl_isvalid_bitmap(blk_mgr->phy_blocks[old_blk].page_bitmap, i)) {
                ...
                if (0 != ret) {
                    NFTL_ERROR("[nftl] %s, read error in gc: %d, p: %d, blk: %d, page: %d",
                ...
                if (tag->logic_block >= blk_mgr->logic_block_nums) {
                    NFTL_ERROR("[nftl] %s, logic block error: p: %d, blk: %d, ver: %d, nums: %d",
                            __func__, blk_mgr->partition, tag->logic_block, tag->reversion, blk_mgr->logic_block_nums);
                    NFTL_ERROR("[nftl] %s, rblk: %d, rpage: %d, wpageid: %d",
...
static void nftl_complete_gc(nftl_block_mgr_t *blk_mgr, bool completely)
{
    ...
    for (i = 0; i < blk_mgr->phy_block_nums - 1; i++) {
        ...
        if ((gc_blk != blk_mgr->allocating_blk) &&
            ...
            NFTL_WARN("%s complete gc, part: %d, blk: %d, empty blocks: %d, gc pages: %d", __func__,
            ...
            for (j = 0; j < NFTL_TAG_PAGE_IN_BLOCK; j++) {
                if (nftl_isvalid_bitmap(blk_mgr->phy_blocks[gc_blk].page_bitmap, j)) {
                    ...
                    if (0 != ret) {
                        NFTL_ERROR("[fntl] %s, read error in complete gc: %d, p: %d, blk: %d, "
                                   "page: %d", __func__, ret, blk_mgr->partition, gc_blk, j);
...
static void nftl_gc_start(nftl_block_mgr_t *blk_mgr)
{
    ...
    if (blk_mgr->gc_times % 50 == 0) {
        NFTL_WARN("[%s] patition: %d, dirtiest block: %d, min pages: %d, empty blocks: %d, gc times: %d",
    ...
    for (i = 0; i < NFTL_TAG_PAGE_IN_BLOCK; i++) {
        if (nftl_isvalid_bitmap(blk_mgr->phy_blocks[dirtiest_block].page_bitmap, i)) {
            ...
            if (0 != ret) {
                NFTL_ERROR("[fntl] %s, read error in immigrate: %d, p: %d, blk: %d, page: %d",
                    ...
                    void *outbuf, nftl_size_t len)
{
    ...
    while (len > 0) {
        ...
        if (ret != 0) {
            NFTL_ERROR("[%s] read error, blk: %d, readlen: %d, p: %d, off: %d", __func__, blk, r_len, blk_mgr->partition, r_off);
            nftl_unlock(&blk_mgr->par_lock);
            return ret;
        }
...
nftl_err_t nftl_flash_notify_gabage(nftl_partition_t p, nftl_block_t blk)
{
    ...
    NFTL_TRACE("flash notify %s part - %d block - %d", __func__, p, blk);
...
static void handle_nftl_cmd(char *pwbuf, int blen, int argc, char **argv)
{
    ...
    if (0 == memcmp(argv[1], "stat", sizeof("stat"))) {
        nftl_statistics();
    } else if (0 == memcmp(argv[1], "occupy", sizeof("occupy"))) {
        ret = nftl_flash_occupation(NFTL_PARTITION2);
        printf("nftl occupatition: %d\r\n", ret);
    } else if (0 == memcmp(argv[1], "cleanup", sizeof("cleanup"))) {
        nftl_flash_cleanup(NFTL_PARTITION2);
    }
}

static struct cli_command    nftl_cmd = {"nftl", "statistic nftl partition takeup info", handle_nftl_cmd};
...
int nftl_flash_eraseall(nftl_partition_t p)
{
    ...
    for (int blk = 0; blk < blk_mgr->phy_block_nums; blk++) {
        ret = hal_flash_erase(blk_mgr->partition, blk * NFTL_BLOCK_SIZE, NFTL_BLOCK_SIZE);
        if (ret != 0) {
            NFTL_ERROR("failed to erase block %d part %d", blk, blk_mgr->partition);
